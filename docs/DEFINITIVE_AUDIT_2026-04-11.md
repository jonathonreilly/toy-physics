# Definitive Audit — After All Testing

**Date:** 2026-04-11 (end of session)

## Breakthroughs Under Scrutiny

### Wilson test-mass / continuum package: strong bounded companion, not full Newton closure
...
- **Continuum-limit companion**: the same-convention size sweep now gives
  `alpha_inf = -2.009 +/- 0.019`
- **Test-mass companion**: the weak-field heavy-source / light-test setup gives
  a source-mass exponent `1.002` with `R² = 1.000`
- The low-screening Wilson distance law is materially stronger than the earlier
  ...
  screening-controlled effect on this Wilson surface
- **But this is still not full Newton closure**:
  ...
  - the result still lives on the Wilson side lane, not the primary staggered
    architecture
  - the repo does not yet retain `F ∝ M1 M2 / r²` as a global architecture
    claim
  - this batch is same-convention Wilson evidence, not a global normalization
    verdict across mixed Wilson runners
...
- Missing control: random disorder with matched variance
- The transition is real but likely generic, not gravity-specific

### BMV prediction 0.35 rad: MEANINGLESS IN PHYSICAL UNITS
- 0.35 rad is in lattice units with unspecified lattice spacing
- At Planck scale, phase is O(1) by construction
- No path to laboratory-scale prediction without continuum limit
...
**PRD/PRL scope:** Two axioms, parity coupling correction,
self-consistency is real, spectral-trajectory dichotomy,
honest mapping of what derives and what fails.

**NOT claimable:** full Newton closure, Einstein equation, topological
transition as gravitational, BMV quantitative prediction.
...
5. ~~Wilson test-mass / continuum companions~~ **DONE (bounded same-convention Wilson package)**
6. Staggered-lattice reproduction of Newton distance law as a broader
   same-claim package is still bounded, not a full cross-architecture Newton closure
7. Two-body mutual attraction on non-resonant observable
8. Self-consistency and Anderson phase map recheck at μ²=0.001 (low priority)
