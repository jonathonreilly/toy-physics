...
sin(2 alpha_bar)  =  sin(pi + (sqrt(5)/10) alpha_s)
                  ...
                  =  -(sqrt(5)/10) alpha_s(v)  +  O(alpha_s^3).
```
...
### A pure rational, α_s-INDEPENDENT cross-system identity

The 3/5 cross-system ratio `(R4)` is the **first dimensionless,
α_s-independent identity** the framework derives that simultaneously
binds:
...
canonical coupling — a much sharper falsification surface than
α_s-dependent predictions allow.
...
The 3/5 ratio test is uniquely valuable: it requires **only ratios
of independently-measured B_d and B_s CP observables**, with no
input from QCD coupling determinations, lattice QCD, or hadronic
form factors. Among the framework's predictions, this is the
cleanest direct probe of atlas Thales geometry from B-physics.

### What this rules out

If a future precision joint measurement of `phi_s` and `sin(2 alpha)`
