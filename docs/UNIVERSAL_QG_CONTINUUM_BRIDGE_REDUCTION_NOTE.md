...
The direct-universal route already supplies:

1. exact discrete `3+1` Einstein/Regge stationary action family on
   `PL S^3 x R`;
2. exact finite-atlas global patching of the Lorentzian stationary family;
...
Because the local partition family is Gaussian on the positive-background
sector, it factorizes exactly on independent blocks:

`Z[K_1 ⊕ K_2, J_1 ⊕ J_2] = Z[K_1,J_1] Z[K_2,J_2]`.

Equivalently,

`log Z[K_1 ⊕ K_2, J_1 ⊕ J_2] = log Z[K_1,J_1] + log Z[K_2,J_2]`.
