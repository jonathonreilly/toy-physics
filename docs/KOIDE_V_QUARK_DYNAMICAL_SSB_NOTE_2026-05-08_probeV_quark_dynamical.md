...
**Date:** 2026-05-10
**Claim type:** bounded_theorem (negative for the heavy-quark closure
...
**Goal.** Test whether retained Cl(3)/Z³ content + chiral SSB
...
**Theorem (V-Quark-Dynamical; bounded, negative).** On retained
content of Cl(3)/Z³ plus retained Wilson-chain inputs `(M_Pl, ⟨P⟩,
...
respects A1+A2 plus the retained Ward identity.
...
3. **(Heavy-quark chiral SSB is structurally sub-dominant.)** Σ as a
   ...
   retained from Cl(3)/Z³ (same species-differentiation obstruction).
...
| **I4** y_b(M_Pl), y_c(M_Pl) species-DIFFERENTIATED BC | **NOT RETAINED** | Species-uniform extension is FALSIFIED (33× overshoot per YT_BOTTOM). Koide circulant `ρ_down ≈ 1.536`, `ρ_up ≈ 1.754` candidate; but A1 equipartition + P1 √m-identification are admitted non-retained primitives per YT_BOTTOM §5.2. |
...
- **Heavy-quark masses are NOT derivable from the union of retained
  Cl(3)/Z³ content + chiral SSB structural argument + retained 1L
...
- **Chiral SSB is structurally a LIGHT-quark mechanism.** Adding
  ...
  `m_q^current` itself is NOT retained from Cl(3)/Z³; the
...
- **Koide circulant Fourier-basis candidate mechanism** for
  ...
  positive-mechanism candidate, conditional on A1 + P1 admissions.
...
2. **A species-DIFFERENTIATION primitive on `y_q(M_Pl)` is required.**
   ...
   `A1 = √2-equipartition` and `P1 = √m-identification` are admitted
...
itself NOT retained from Cl(3)/Z³. The same species-differentiation
...
- **No new axioms.** All inputs from retained Cl(3)/Z³ (S1+Casimir+
...
spectrum with `ρ_down ≈ 1.536`, `ρ_up ≈ 1.754`; admissions A1+P1)
remains the open gap.

## 10. Validation

PASS = 27, FAIL = 0, ADMITTED = 6 across all probe checks (see
