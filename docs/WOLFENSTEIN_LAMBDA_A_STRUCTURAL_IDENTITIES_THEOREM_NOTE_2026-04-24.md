...
atlas/axiom package. This note names and regression-tests the `lambda` and
`A` identities already present inside
[`CKM_ATLAS_AXIOM_CLOSURE_NOTE.md`](CKM_ATLAS_AXIOM_CLOSURE_NOTE.md). It is a
companion to
`CKM_CP_PHASE_STRUCTURAL_IDENTITY_THEOREM_NOTE_2026-04-24.md` (sibling
companion subtheorem under the same CKM atlas; cross-reference only — the
parent atlas already lists both subtheorems)
and does not expand the parent CKM theorem's scope.

**Narrowed claim scope.** This note scopes the three Wolfenstein structural
identities `(W1)` `lambda^2 = alpha_s(v)/n_pair`, `(W2)` `A^2 = n_pair/n_color`,
`(W3)` `A^2 lambda^2 = alpha_s(v)/n_color`, and the CKM magnitude corollaries
`|V_cb| = alpha_s(v)/sqrt(n_quark)` and atlas-leading
`|V_ub|_0 = alpha_s(v)^(3/2)/(6 sqrt(2))` as exact algebra over the cited
structural-count inputs and the canonical coupling. The conditional surface
is:

1. structural counts `n_pair = 2`, `n_color = 3`, `n_quark = 6` are imported
   from the
   [`NATIVE_GAUGE_CLOSURE_NOTE.md`](NATIVE_GAUGE_CLOSURE_NOTE.md),
   [`GRAPH_FIRST_SU3_INTEGRATION_NOTE.md`](GRAPH_FIRST_SU3_INTEGRATION_NOTE.md),
   and parent
   [CKM atlas/axiom authority](CKM_ATLAS_AXIOM_CLOSURE_NOTE.md);
2. the canonical `alpha_s(v)` is imported from
   [`ALPHA_S_DERIVED_NOTE.md`](ALPHA_S_DERIVED_NOTE.md) via
   `scripts/canonical_plaquette_surface.py`;
3. the CP-radius input `rho^2 + eta^2 = 1/6` for the `|V_ub|_0` corollary
...
4. observation-side CKM magnitude comparators are downstream numerical
   readouts only and are not load-bearing for the algebra.

The local algebra closes exactly under those imported inputs. Retained-grade
closure of the upstream parent CKM atlas, structural-count, CP-phase, and
`alpha_s` authorities is owned by the upstream-audit lane and is not
...
On the cited CKM atlas/axiom surface, let
...
The first identity carries the separately cited canonical `alpha_s(v)`
...
| CKM CP radius and phase identities | `CKM_CP_PHASE_STRUCTURAL_IDENTITY_THEOREM_NOTE_2026-04-24.md` (sibling companion subtheorem under the same CKM atlas; backticked to avoid length-2 cycle) |
...
- `lambda^2 = alpha_s(v)/2` on the cited CKM atlas surface;
...
- [`CKM_ATLAS_AXIOM_CLOSURE_NOTE.md`](CKM_ATLAS_AXIOM_CLOSURE_NOTE.md)
  - parent CKM package.
- `CKM_CP_PHASE_STRUCTURAL_IDENTITY_THEOREM_NOTE_2026-04-24.md` (sibling
  companion subtheorem under the same CKM atlas; backticked to avoid
  length-2 cycle)
  - companion `rho`, `eta`, phase, and `J` identities.
- `CKM_FIRST_ROW_MAGNITUDES_THEOREM_NOTE_2026-04-24.md` (sibling companion
  subtheorem under the same CKM atlas; backticked to avoid length-2 cycle)
...
- [`GRAPH_FIRST_SU3_INTEGRATION_NOTE.md`](GRAPH_FIRST_SU3_INTEGRATION_NOTE.md)
  - cited `n_color = 3` color structure.
- [`NATIVE_GAUGE_CLOSURE_NOTE.md`](NATIVE_GAUGE_CLOSURE_NOTE.md)
  - cited weak-pair structure.
