...
- `docs/DM_LEPTOGENESIS_EXACT_KERNEL_CLOSURE_NOTE_2026-04-15.md`
  (runner: `scripts/frontier_dm_leptogenesis_exact_kernel_closure.py`)
...
- `docs/DM_NEUTRINO_BOSONIC_NORMALIZATION_THEOREM_NOTE_2026-04-15.md`,
  ...
  the upstream `ALPHA_LM` power-law `(k_A, k_B)` generators that fix
  the `(M1, M2, M3)` spectrum entries.
- `docs/COSMOLOGY_FROM_MASS_SPECTRUM_NOTE.md` — the upstream
  cosmology-side mass-spectrum reading on the same `ALPHA_LM` ladder.
- The Davidson-Ibarra bound `M_DI ≈ 2.4 × 10⁸ GeV` is a literature input
  (Davidson-Ibarra 2002), not a framework derivation; this note imports
  it to characterise viability and does not claim its derivation.

The transport-gap factor and the `M_N_target = 4.002 × M1` ratio are
characterisations of the imported numbers, not new first-principles
derivations from `Cl(3)` on `Z^3`. The bounded scope is the
