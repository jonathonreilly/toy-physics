...
**Date:** 2026-04-19  
**Status:** **RETAINED conditional theorem** — under σ_hier = (2,1,0) and the
...
This elevates A-BCC from "physically motivated axiom" to
"observationally grounded": C_neg is ruled out by concrete measurement.
...
- **A-BCC observational grounding**: A-BCC is no longer only "physically
  motivated" — it has a concrete observational no-go backing it.
...
The P3 flagship closure now rests on two observationally grounded inputs (not
zero internally derived ones). The final step — deriving A-BCC from the
