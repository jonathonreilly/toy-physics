# PR230 Two-Source Taste-Radial Chunks051-052 Package

**Status:** bounded-support / chunks001-052 packaged; no closure

**Checkpoint certificates:**
`outputs/yt_pr230_two_source_taste_radial_chunk051_checkpoint_2026-05-06.json`,
`outputs/yt_pr230_two_source_taste_radial_chunk052_checkpoint_2026-05-06.json`

## Result

Chunks051-052 now pass completed-mode checkpoints and are included in the
two-source taste-radial row package.  The row combiner reports `ready=52/63`
with `combined_rows_written=false`.

Both chunks preserve the production surface:

- fixed seeds `2026056051` and `2026056052`;
...
The row-wave supervisor has launched chunks053-054 under the two-worker cap.
Those active jobs are run-control state only and are not counted as evidence.
Chunks055-063 remain pending.
...
- Raw `C_ss` still fails the strict scalar-LSZ first-shell Stieltjes
  ...
  `z=178.01483337800587`.
- The Schur split remains bounded support only.  `C_s|x` fails
  (`z=162.2723812955626`), while `C_x|s` survives only the necessary
  first-shell check (`z=-568.5961022860091`).
- The refreshed `C_x|s` one-pole interpolation gives
  `C(0)=0.280831416666658`,
  `C(0.267949192431123)=0.26953406469401675`,
  `m^2=6.392775505475802`, and `R=1.7952922016346802`; positive two-pole
...
# SUMMARY: PASS=10 FAIL=0, active_ids=[53, 54]
...
# SUMMARY: PASS=13 FAIL=0, ready=52/63
