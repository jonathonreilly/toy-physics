...
**Cached output:** [`logs/runner-cache/cl3_closure_t1_z10_z20_2026_05_10_t1z10z20.txt`](../logs/runner-cache/cl3_closure_t1_z10_z20_2026_05_10_t1z10z20.txt)
...
structurally from retained content + universal RG algebra. The closure
...
**Theorem (T1; bounded, numerically positive at SU(3), N_f=0).** On
retained content of Cl(3)/Z^3 + retained S1 + retained SU(3) Casimirs +
retained `<P>_HK_SU(3)(s_t) = 1 - exp(-(4/3) s_t)` + the C-iso SU(3) NLO
bounded retention `(P_W - P_HK)_SU(3) = (7/9) s_t^2 + O(s_t^3)` (from
...
(P4) **`Z_10^{HK->W, SU(3), N_f=0}` mean-field = -7/48 = -0.14583**
     derived from the retained `<P>_HK` and bounded `<P>_W` expansions
     at the single-plaquette level. The leading-order mean-field
     ratio 7/12 in the s_t² coefficient of `<P>_HK - <P>_W` is exact
     framework retention.
...
(A1) **AFP `P_2 = 0.024013181`** (1-loop Symanzik integral). Not
...
(A4) **N_f > 0 corrections on HK side**. The framework's HK
     retention `<P>_HK_SU(3)(s_t) = 1 - exp(-(4/3) s_t)` is at the
     pure-gauge level; quark-loop corrections require the HK-action
     extension with N_f fermion loops which is not retained. The
     N_f=6 cross-check therefore inherits the X-L1-MSbar bounded
     admission unchanged.

(A5) **Normalization of `b_2^HK` (32/81 pure-gauge) to MS-beta units**.
     The C-iso SU(3) NLO retention gives the s_t Taylor coefficient
...
- **Mean-field Z_n^{HK->W} derived** from retained `<P>_HK` and
  bounded `<P>_W` expansions: Z_10 = -7/48 (exact framework retention),
...
### Not closed (frontier remaining; bounded admissions A1-A5)
...
- **Full HK BZ integrals** (genuine HK analog of AFP eq. (2.10)) require
  HK Feynman rules at quadratic + cubic vertex level beyond framework
  retention. Mean-field estimate is LEADING-order only.
- **Wilson `c_3^W` and HK-side quark-loop corrections** stay bounded
  to existing retentions (C-iso NLO, X-L1-MSbar).
...
derived via composition HK -> Wilson -> MSbar with:
  ...
  - Z_10^{HK->W, SU(3), N_f=0} = -7/48 (mean-field, retained content)
...
A1: AFP P_2 (1 Symanzik integral; literature) -- bounded import
A2: Wilson c_3^W asymptotic fit (-4.33) -- C-iso NLO note bounded
A3: Z_n^{HK->W} beyond mean-field -- HK Feynman rules not retained
A4: N_f > 0 on HK side -- X-L1-MSbar bounded admission unchanged
A5: b_2^HK normalization to MS-beta units -- X-L1-MSbar bounded
...
The retained content gives the two single-plaquette expansions:
...
with `alpha = -7/12` (exact retained) and `beta = 0.31935` (bounded).
Reading off:
...
closure since N_f-corrections to Z_n are bounded admissions (A4).

## 4. Conditional admissions

This bounded source theorem inherits the conditional admissions of the
existing retained substrate, plus the named bounded admissions A1-A5:
...
**Imported authorities (numerical comparators / literature constants,
NOT load-bearing for the closure structure):**
...
- **Framework-native closed-form derivation of full Z_n^{HK->MS}.**
  ...
  vertex level are bounded admission A3.
- **Re-derivation of AFP P_2.** This 1-loop Symanzik integral remains
  literature-imported (admission A1).
- **Framework-native derivation of c_3^W.** The Wilson NNLO single-
  ...
  per the C-iso SU(3) NLO note (admission A2).
...
3. **`Z_10^{HK->W, SU(3), N_f=0}` mean-field = -7/48**, an EXACT
   rational from retained `<P>_HK` and bounded `<P>_W` expansion at
   NLO (the s_t² coefficient ratio 7/9 is retained content).
...
### A2: Wilson c_3^W = -4.33 is reliable
...
### A3: Mean-field captures the dominant HK -> W matching
...
the mean-field. To genuinely close A3 requires:
  (a) Either: explicit HK lattice Feynman rules (action expansion
      to cubic + quartic in algebra-valued fields) — outside retained
      content.
  (b) Or: numerical MC measurement of the HK propagator and 3-pt
      function — empirical, not retained.

### A4: N_f = 0 on HK side is the framework's retained regime

**What if wrong?** The framework retains pure-gauge SU(3) HK via
...
**This note's finding.** N_f = 0 is the retained-clean regime; N_f = 6
inherits X-L1-MSbar admission unchanged.

### A5: 32/81 retains pure-gauge b_2^HK in MS-beta units

**What if wrong?** The C-iso SU(3) NLO note retains the s_t³ Taylor
...
the conversion is bounded admission A5.

## 8. Closure status summary

| Quantity | Retention status | Source |
|---|---|---|
| `P_1 = 0.15493339` 4D Wilson tadpole | **RETAINED** (numerically derived) | this note, Section 1 |
| `Z_10^{W->MS, SU(3), N_f=0}` | RETAINED (numerical via AFP eq. (2.10) + framework P_1) | this note, Section 2 |
| `Z_20^{W->MS, SU(3), N_f=0}` | RETAINED (back-solved from AFP eq. (3.4) round-trip) | this note, Section 3 |
| `Z_10^{HK->W}` mean-field = -7/48 | RETAINED (exact, retained content) | this note, Section 4 |
...
| AFP `P_2 = 0.024013181` Symanzik integral | imported, bounded admission A1 | AFP 1996 |
| Wilson `c_3^W ≈ -4.33` asymptotic | bounded admission A2 | C-iso SU(3) NLO |
| `Z_n^{HK->W}` beyond mean-field | bounded admission A3 (HK Feynman rules not retained) | this note |
| N_f > 0 corrections on HK side | bounded admission A4 (X-L1-MSbar unchanged) | this note |
| `b_2^HK` normalization to MS-beta units | bounded admission A5 (X-L1-MSbar unchanged) | this note |
...
retained functional form" to "numerical estimates at N_f=0 mean-field
+ five clean admissions A1-A5".
...
- Section 4: mean-field Z_n^{HK->W} derived from retained <P>_HK, <P>_W.
...
2. **Mean-field single-plaquette derivation of Z_10^{HK->W} = -7/48**
   as an EXACT rational from retained `<P>_HK = 1 - exp(-(4/3) s_t)`
...
4. **Reduction of PR #1059 sub-piece (a)** from "two scalar BZ
   integrals; retained functional form" to "numerical estimates
   at N_f=0 mean-field + five clean admissions A1-A5".

What this note did NOT deliver:

- **Full HK BZ integration over HK Feynman rules.** A3 remains a clean
  admission requiring HK action expansion to cubic + quartic vertex
  level, which is not in retained content.
- **Re-derivation of AFP P_2.** A1 remains; clean future work.
- **Closure of N_f > 0 on HK side.** A4 unchanged from X-L1-MSbar.
- **New axioms or new retained primitives.** None.
- **Promotion of any imported numerical comparator** (AFP b_2^W,
  TVZ b_2^MS, AFP P_2) **to retained.** None.

## 12. Diff against companion notes

| Note | Status before | Status after this closure |
|---|---|---|
| C-L1a (PR #1059) sub-piece (a) | "two scalar BZ integrals; retained functional form" | sharpened to "numerical estimates at SU(3), N_f=0 mean-field + 5 admissions" |
| C-iso SU(3) NLO bounded note | provides retained `<P>_HK` and bounded `<P>_W` NLO | unchanged on retention; used as input |
...
| X-L1-MSbar bounded admission | "HK <-> MSbar conversion at 3L is open" | unchanged on N_f, normalization (A4, A5) |
