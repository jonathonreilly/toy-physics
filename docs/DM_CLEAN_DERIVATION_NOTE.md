...
### Step 5: g_bare = 1 from Cl(3) normalization

**Status: BOUNDED**

**Argument (not theorem):** The Cl(3) generators satisfy the canonical
anticommutation relation {gamma_mu, gamma_nu} = 2 delta_{mu,nu}. The
holonomy U_mu(x) = exp(i g A_mu T_a dx_mu) on the lattice uses the
canonical Cl(3) connection. With a = l_Pl as the unique physical scale
and no continuum limit, the coupling g cannot run. The algebra
normalization fixes g = 1.
...
### Step 6: alpha_s = 0.0923 from plaquette at g = 1

**Status: DERIVED (inherits BOUNDED from Step 5)**

**Derivation:** At beta = 2*N_c/g^2 = 6 (g = 1), the plaquette
expectation value in the strong-coupling expansion gives:
...
bounded. The Cl(3) normalization argument makes g = 1 canonical, but
whether this is a constraint or a convention is foundational.
