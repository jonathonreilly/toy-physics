# Generated-Geometry Lane

**Status:** historical bounded family plus current staggered reopen

## Covers

- Gate B / grown-family / generated-geometry transfer program
- source-resolved generated architectures
- current staggered emergent-geometry reopen

## Start Here

- historical transfer side:
  - [`../../GATE_B_GROWN_JOINT_PACKAGE_NOTE.md`](../../GATE_B_GROWN_JOINT_PACKAGE_NOTE.md)
  - [`../../SOURCE_RESOLVED_GENERATED_ARCHITECTURE_BRIDGE_NOTE.md`](../../SOURCE_RESOLVED_GENERATED_ARCHITECTURE_BRIDGE_NOTE.md)
- current reopen side:
  - [`../../EMERGENT_GEOMETRY_GROWTH_NOTE_2026-04-10.md`](../../EMERGENT_GEOMETRY_GROWTH_NOTE_2026-04-10.md)

## Key Runners

- [`scripts/gate_b_grown_joint_package.py`](../../../scripts/gate_b_grown_joint_package.py)
- [`scripts/source_resolved_generated_architecture_bridge.py`](../../../scripts/source_resolved_generated_architecture_bridge.py)
- [`scripts/source_resolved_geometry_rule_repair_probe.py`](../../../scripts/source_resolved_geometry_rule_repair_probe.py)
- [`scripts/frontier_emergent_geometry_v2.py`](../../../scripts/frontier_emergent_geometry_v2.py)
- [`scripts/frontier_emergent_geometry_g_sweep.py`](../../../scripts/frontier_emergent_geometry_g_sweep.py)

## Current Standing

- old generated-geometry transfer branch has real historical positives
- current staggered growth result is only a partial reopen
- do not conflate the two
