# Moonshots Lane

**Status:** historical control / frontier ambition lane

## Covers

- Schwarzschild analogs
- Hawking / horizon probes
- Nature-level backlog items

## Start Here

- [`../../work_history/repo/backlog/NATURE_BACKLOG_2026-04-10.md`](../../work_history/repo/backlog/NATURE_BACKLOG_2026-04-10.md)
- [`../../MOONSHOT_FRONTIER_PORTFOLIO_NOTE.md`](../../MOONSHOT_FRONTIER_PORTFOLIO_NOTE.md)

## Key Runners

- [`scripts/frontier_emergent_schwarzschild.py`](../../../scripts/frontier_emergent_schwarzschild.py)
- [`scripts/frontier_hawking_analog.py`](../../../scripts/frontier_hawking_analog.py)
- [`scripts/frontier_quantized_energy_levels.py`](../../../scripts/frontier_quantized_energy_levels.py)

## Current Standing

- useful for ambition-setting and negative controls
- not part of the current retained headline claim surface
