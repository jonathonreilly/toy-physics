...
| Legacy absolute-path doc links | A broad historical slice of `main` docs still links scripts through `/Users/jonreilly/Projects/Physics/.claude/worktrees/sleepy-cerf/...`. The code may exist, but the docs still depend on one local worktree layout and can mislead readers about what is actually canonical on `main`. | Convert main-facing docs and lane readmes to repo-local script paths before treating them as a reproducible harness index. Until then, trust the corrected control-plane docs first and treat older readmes as historical navigation aids. | High |
