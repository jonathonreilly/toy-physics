...
|---|---|---|---|
| Historical all-architecture scorecards and prediction cards | Resolved on current `main`: the old evaluation-tool surfaces were scrubbed from the repo surface entirely. They no longer sit in the live doc tree where they can be overread as current truth; if their exact historical text is ever needed, use git history rather than the active control plane. | For current baseline use the lane board, canonical harness index, and retained/package notes. | Resolved |
| Legacy absolute-path doc links | Resolved on current `main` for the main-facing notes and historical lane readmes that were still linking through one local `.claude/worktrees/sleepy-cerf/...` layout. Historical branch/worktree mentions may remain as provenance text, but the reproducible runner links now point at repo-local script paths. | Keep older branch/worktree names only as historical provenance. For runnable references on `main`, prefer repo-local script paths and the canonical harness index. | Resolved |
| Irregular off-lattice direction/sign lane | `main` now has a bounded same-surface irregular core-packet separator across the audited families and both screening levels. What remains open is portability beyond that surface: transport observables, other packet families, and stronger graph-growth portability are still not closed. | A portability-grade irregular observable that survives beyond the bounded core-packet surface, or an invariant transport readout that remains sign-selective on the same irregular families. | High |
...
| Branch-entanglement externally imposed two-branch protocol lane | The retained side package is real as branch-mediated entanglement, but the current scripts are not full gravity-quantumness witnesses. The 3-body lane also already needed a GHZ-to-W correction. | Redesign the witness/classifier before promoting anything beyond the current bounded branch-mediated reading. | Medium |
