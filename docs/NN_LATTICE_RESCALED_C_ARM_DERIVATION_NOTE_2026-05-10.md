# Rescaled NN Lattice C_arm Analytic Derivation Note

**Date:** 2026-05-10
**Claim type:** positive_theorem (residual 8.3% in the h -> 0 saddle, < 2.5% per-point with the leading phase correction; both inside the 10% band)
...
**Trigger:** PR #968 (T_inf identified as geodesic continuum operator on the
slit-detector decoherence subblock with `sigma_arm(h) = C_arm h^alpha`,
fitted `C_arm = 2.7107`, `alpha = 0.5256`, R^2 = 0.9996 on 4 points, h <= 0.25).
...
is reproduced from first principles by a coherent path-integral saddle on the
rescaled NN harness's per-step lateral characteristic function. The closed form
is
...
In the `h -> 0` (continuum) limit the formula collapses to the parameter-free
geodesic constant
...
inside the 10% positive-theorem band. Per-h cross-checks with the full coherent
formula (retaining the `cos(k h (sqrt(2) - 1))` phase term) match the
rescaled-NN fit to better than 2.5% on all four fit points. The claim is
positive_theorem at the harness-fixed parameters.
...
re-anchors the centroid (PR #968 verified the centroid drift is bounded by
0.564, consistent with the slit-anchored picture); the post-slit propagation
then gives the standard saddle-point spread over `L_2`.
...
Residual versus the PR #968 fit:
...
inside the 10% positive-theorem band.
...
All four points agree with the rescaled-NN fit to within 2.5%. The fitted
...
| Coherent, `L = L_2`, with phase correction | per-h table above | matches each h to <= 2.5% | inside 10% |
...
This note derives `C_arm` from the harness parameters and the slit-plane
...
This source note is a closed-form derivation proposal. The audit lane sets the
effective status after independent review of the runner, the derivation steps,
and the slit-plane length identification.
