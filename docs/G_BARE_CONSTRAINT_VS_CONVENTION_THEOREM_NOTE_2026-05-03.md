...
The present note disambiguates the two readings by exhibiting the precise
...
> ```
> be the canonical Cl(3) connection normalization on the retained triplet
...
[PASS] constraint layer: given canonical normalization, g_bare = 1 is structurally forced
```
...
single_retained_one_hop_dep: g_bare_rescaling_freedom_removal_theorem_note_2026-05-03
...
and retained-grade closure of the declared dependency chain.

## 7. What this theorem closes

- The constraint-vs-convention ambiguity named on the parent
  `G_BARE_DERIVATION_NOTE.md` row, by explicitly disambiguating: the
  convention layer lives at the canonical Cl(3) normalization (CN), and
  `g_bare = 1` is a derived constraint relative to that convention.
- The repair target #3 from `G_BARE_DERIVATION_STATUS_CORRECTION_AUDIT_NOTE_2026-05-02`.
- A clean handoff to downstream rows depending on `g_bare = 1`: such
  rows can cite this note for the constraint reading, or the narrow
  convention theorem for the convention reading, depending on which side
  of the disambiguation they need.
...
- The retained promotion of `G_BARE_DERIVATION_NOTE.md` itself; this
  theorem closes one of the three named repair targets but not the full
  promotion pathway.
...
foundational question, separately tracked. This theorem closes the
*relative* constraint reading; the *absolute* derivation of `g_bare = 1`
from `A1 + A2` alone (independent of the canonical normalization) is a
strictly stronger Nature-grade target outside the present scope.

## 10. Cross-references

The single retained one-hop dep is the rescaling-freedom-removal theorem,
...
- Parent: `G_BARE_DERIVATION_NOTE.md` — the ambiguous note that this
  theorem disambiguates.
