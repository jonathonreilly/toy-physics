...
claim_scope: |
  Bounded support theorem closing the operator-level adjoint-channel
  ...
  subspace (color triplet, Y=+1/3) and is orthogonal to the 1D
  antisymmetric-base lepton singlet (Y=-1). SU(3)_c acts trivially on
  the lepton block and non-trivially on the dark state. The singlet
  Fierz projector annihilates traceless matrices T^a, so any
  gauge-mediated propagator on the color-charged dark state lives
  entirely in the adjoint Fierz channel. Per-color-row density
  rho_{adj/c} = (N_c^2-1)/N_c = 8/3, giving m_DM = (8/3)*6v = 16v on
  the canonical surface. No new axioms, no new dynamical mechanisms.
g1_bridge_step_status: closed_v1_via_carrier_orthogonality_fierz_selection
g1_dynamical_step_status: closed
g1_arithmetic_step_status: closed_via_operator_trace_v1
g1_algebraic_step_status: closed_v1
counterfactual_pass_done: true
counterfactual_pass_routes_scored: 3
counterfactual_pass_winner: route_b2_carrier_orthogonality_fierz
runner_pass_count: 15
runner_fail_count: 0
parent_status_unchanged: false  # G1 dynamical step now closed; parent lane bounded inputs only
```
