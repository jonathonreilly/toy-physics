...
The frozen-bank decomposition `K_Z3^sel(m) = K_frozen + m T_m^(K)` reduces the charged-lepton selected slice to one real coordinate `m = Tr K_Z3`. This note derives the exact Z³-invariant scalar potential `V(m)` for that coordinate and records what is proved versus what remains open.
...
The V_eff minimum **does not coincide with the physical selected point**. The gap is `|m_V - m_*| ≈ 0.73` in `m` units. The V_eff potential alone does not select `m_*`.
...
All errors < 0.05% on the √mass metric (used because the Koide relation and slot values are native to √mass; the upstream H_* witness yields the same bound on mass directly). The remaining work is deriving the one overall scale from the lattice.
