...
- the inertial-response lane now has a real mesoscopic-source extension:
  ...
  - the source-resolved exact Green pocket now has a larger exact-lattice
    size-transfer companion and a local robustness sweep, which makes it a
    real local basin rather than a one-off feasibility blip
