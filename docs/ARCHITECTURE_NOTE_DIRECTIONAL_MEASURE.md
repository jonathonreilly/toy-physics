...
2. `b`-dependence as a separate gravity-sector question, now narrowed to deriving the asymptotic `b` leading term, its `b - h_mass` finite-source correction, and explaining why dense families drive the occupancy-load bridge into the `mu <= 0` regime while tree-like controls stay safely positive
