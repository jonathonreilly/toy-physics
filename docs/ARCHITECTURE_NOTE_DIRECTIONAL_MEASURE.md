...
2. `b`-dependence as a separate gravity-sector question, now narrowed to deriving the asymptotic `b` leading term, its `b - h_mass` finite-source correction, and explaining the new mid-layer densification miss mode of the continuous target-plane density bridge (`mass_nodes / expected_target_count_4nn >= 2.7354`, with `expected_target_count_4nn = 4 / r4`) so the current occupancy-first bridge can be promoted to a sampler-robust continuous law without reopening a wider denominator search
