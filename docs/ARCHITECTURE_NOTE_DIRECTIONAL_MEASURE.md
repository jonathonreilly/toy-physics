...
2. `b`-dependence as a separate gravity-sector question, now narrowed to deriving the asymptotic `b` leading term, its `b - h_mass` finite-source correction, and stress-testing the new continuous target-plane density bridge (`mass_nodes / expected_target_count_4nn >= 2.7354`, with `expected_target_count_4nn = 4 / r4`) beyond the current dense-family sample so it can replace the coarser counted law as the cleaner layer-density / mass-geometry explanation for why dense families drive the overlap-margin diagnostic `mu = edge_b / h_mass` through zero while tree-like controls stay safely positive
