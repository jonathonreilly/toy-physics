...
part of `H_ret` and cannot be used for a continuum or finer-spacing claim.
...
2. `measure_full(h)` applies the same propagator and observable definitions to
   every requested spacing. No row in `H_ret` uses fan-out normalization, layer
   normalization, a fitted selector, or an observed target value.
3. For every `h` in `H_ret`, the runner returns a finite row with `k=0 =
   0.00e+00` and Born residual below the stated audit tolerance
   `1e-10`. The worst cached finite-window Born residual is `5.07e-16`.
...
5. The same runner then reports `FAIL` at `h = 0.125`. That failure blocks
   extending the window, so the derivation stops at `H_ret` and leaves the
...
raw NN harness + finite runner rows through h = 0.25
  => Born-clean finite-spacing refinement window H_ret
