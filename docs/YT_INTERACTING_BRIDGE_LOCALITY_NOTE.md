...
This is therefore a numerical-match / class-G proxy in the project's audit
taxonomy, not a closed first-principles derivation of UV-localization. A prior
audit recorded this boundary as `audited_numerical_match`; after this source
edit, independent re-audit owns the current status. The remaining structural
gaps are recorded below in "What remains open".
...
To upgrade the locality conclusion from conditional class-G scope to a clean
derivation, the following structural gaps must be closed; none are addressed
by the present runner:
...
verdict instead of relying on a trailing addendum. The runner output is
unchanged; the current audit status is owned by the regenerated audit pipeline
and the next independent re-audit. This brings the headline framing of this
note into agreement with the parallel 2026-05-16 demotion of its sister
`YT_CONSTRUCTIVE_UV_BRIDGE_NOTE.md`, which runs the same scope correction on
the three-family endpoint-stability scan.

## Audit dependency repair links

This graph-bookkeeping section records the upstream notes the locality
scan and endpoint data depend on. It does not promote this note or change its
current claim scope.
