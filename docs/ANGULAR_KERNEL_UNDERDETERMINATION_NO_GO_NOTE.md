...
**Date:** 2026-04-25
**Status:** retained no-go on the directional-path-measure kernel +
retained positive decoupling theorem for the boost-covariance program
**Runner:** `scripts/frontier_angular_kernel_underdetermination_nogo.py`
(PASS=64, FAIL=0)
**Companions:**
[LORENTZ_BOOST_COVARIANCE_GAP_NOTE.md](LORENTZ_BOOST_COVARIANCE_GAP_NOTE.md),
...
This note closes Phase 3 of the boost-covariance program in the form of a
**bounded no-go on the angular kernel** plus a **positive decoupling
theorem**. The two together unblock Phase 4 without forcing a derivation
of the empirical `beta = 0.8`.
...
This note closes Phase 3 of the boost-covariance program with two retained
results:

1. **Negative result (no-go):** the directional-measure angular kernel
...
2. **Positive result (decoupling):** Phase 4 does not need (1) to be
...
## Relation to Phase 1 gap audit

The [Phase 1 gap audit](LORENTZ_BOOST_COVARIANCE_GAP_NOTE.md) section 3.3
already flagged that the dispersion theorem and the directional-measure
walk are different objects. Phase 3 makes that observation **load-bearing**:
the boost-covariance question on the dispersion-theorem surface
(staggered/Laplacian) is decoupled from the kernel-derivation question on
the directional-measure surface, by an explicit theorem rather than by
narrative.
