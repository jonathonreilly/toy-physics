...
**Status:** retained zero-import end-to-end derivation of the down-type
quark mass ratios on the canonical plaquette/CMT surface. Closes the CKM
gate's "imported masses" gap. The 5/6 strong-coupling exponentiation
mechanism stays as bounded support (its non-perturbative derivation at
g=1 remains open).

**Primary runner:** `scripts/frontier_quark_mass_ratios_taste_staircase.py`
**Authority for the CKM gate quark-mass route:** this note.
...
- [`DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md`](./DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md) — predecessor bounded lane (this note supersedes it as the gate-closing route).

## 1. Theorem statement

On the retained plaquette / coupling-map (CMT) surface, the down-type
quark mass ratios are determined by the same group-theoretic inputs that
fix the CKM atlas package:
...
so the two named bridges become exact algebraic consequences of the
common surface,
...
with no PDG quark masses entering the derivation.
...
This note exhibits two **independent** derivation paths from the same
retained surface:

- **Path A (cascade).** Inputs `alpha_s(v), n_pair, n_quark, C_F - T_F`
  give the two ratios directly via the closed-form expressions in
  Section 1.

- **Path B (atlas + identities).** Inputs `V_us_atlas, V_cb_atlas`
  combine with GST and `5/6` to give the same two ratios.

Paths A and B agree algebraically (verified to machine precision in the
runner). The chain therefore admits **either reading**:

- mass ratios -> CKM, or
- CKM -> mass ratios,

with no PDG quark masses entering on either path. PDG values are used
only as the post-derivation comparator.
...
Before this note, the CKM gate carried the status

> `BOUNDED` with PDG quark masses imported in the `V_us = sqrt(m_d/m_s)`
> and `V_cb = (m_s/m_b)^{5/6}` cross-checks.

After this note, the gate carries the status

> `ZERO-IMPORT END-TO-END`. The two named CKM cross-checks become
> *self-consistent algebraic identities* on the retained surface:
> `V_us` and `V_cb` are derived from `alpha_s(v)`, `n_pair`,
> `n_quark`, and `C_F - T_F`; the mass ratios `m_d/m_s` and `m_s/m_b`
> are derived from the same set; and GST plus the `5/6` bridge connect
> them with no additional inputs. PDG values are used only as the
> post-derivation comparator.
...
2. A retained closure of the absolute bottom-quark scale `m_b` or the
   bottom Yukawa `y_b`. Only the down-type *ratios* are produced.
3. A retained derivation of the up-type ratios (`m_u/m_c`, `m_c/m_t`).
...
4. A retained derivation of the CKM CP phase `delta` or the Jarlskog
   invariant. Those are carried by the existing CKM atlas / CP package.
5. Promotion of GST to a higher-order (beyond leading) identity. GST
...
6. A unique prediction of the *threshold-local self-scale* comparator as
...
```bash
python3 scripts/frontier_quark_mass_ratios_taste_staircase.py
```
...
10. CKM gate closure (Part 9): `CKM_GATE_ZERO_IMPORT_VIA_TASTE_STAIRCASE=TRUE`,
    `FIVE_SIXTHS_BRIDGE_NONPERTURBATIVE_MECHANISM_THEOREM_GRADE=FALSE`.

## 10. Reading rule

This note is the **CKM gate quark-mass-route authority** on current
`main`. It supersedes the predecessor bounded lane
[`DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md`](./DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md)
as the controlling note for the down-type mass-ratio pillar of the gate.
The atlas / identities side remains the controlling authority for
`V_us, V_cb` and is unchanged.
