...
open_gate`, and load-bearing step class `C`. The audit chain-closure
explanation is exact: "The note's conclusion depends on the Born,
k=0, gravity, and alpha numerical row, but the live runner did not
reach those diagnostics in a bounded restricted run." The
audit-stated repair target is exact: "provide a current completed
...
The 2026-05-03 audit verdict (cited at top) flagged that
[`scripts/lattice_3d_l2_wide_h0125_w4.py`](../scripts/lattice_3d_l2_wide_h0125_w4.py)
...
audit-conditional perimeter is exactly the missing completed-stdout
