...
The audit verdict (2026-05-09) requested explicit pointers to the registered
runner sources and cached stdout backing the retained `N=15`/`N=25` rows.
These artifacts are present in the worktree:
...
This block exists so a hostile auditor can walk straight from the note to the
exact registered runner sources and cached stdout without grepping.
