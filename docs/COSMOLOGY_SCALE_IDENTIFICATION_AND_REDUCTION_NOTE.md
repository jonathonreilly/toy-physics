...
on the post-closure GR/QG surface, treating the upstream same-surface scale
identification as cite-chain dependencies and the matter-content bridge as a
cycle-safe candidate reference (see "Audit dependency repair links" below)
rather than as first-principles closures within this packet.
...
One-hop authorities cited (extended 2026-05-16 to register the runner and
to document the matter-content bridge candidate authority that reduces
`Omega_m` to the single ratio `H_inf/H_0`). The first two entries remain
markdown links and therefore graph edges; the later candidate references
are code refs because adding them as graph edges creates audit cycles
through existing cosmology notes.
...
- `OMEGA_LAMBDA_MATTER_BRIDGE_THEOREM_NOTE_2026-04-22.md`
  ...
  `scripts/frontier_omega_lambda_matter_bridge.py`. This cycle-safe
  textual reference reduces the human repair target from "no candidate
  authority named" to "candidate authority named, itself currently
  `unaudited`, that collapses the bridge to a single open ratio"; it is
  intentionally not a markdown dependency edge because that edge creates
  a cycle in the current audit graph. The remaining un-discharged piece
  is a retained derivation of `H_inf/H_0` (or equivalently
  `R_Lambda H_0 / c`), which is explicitly named as the open registration
  target in §"Honest Remaining Gap" item 1.
- `COSMOLOGICAL_CONSTANT_RESULT_2026-04-12.md`
  — current ledger status `unaudited` (audit row:
...
`audited_conditional` or `unaudited`, this note cannot become
retained-grade from this bookkeeping pass. Current status remains
pipeline-owned (`audit_status: unaudited`, `effective_status:
unaudited` at the time of this source edit); if a future audit accepts
the algebra while these dependencies remain non-retained, the
dependency chain should cap the result no stronger than conditional.
The residual single-ratio gap above is a real open class D (gap) entry
and is flagged explicitly so later audit sweeps can wire it to a
retained authority once one exists.
...
candidate authorities in the header + cite-chain section. It partially
addresses (2) by adding the cycle-safe code reference
`OMEGA_LAMBDA_MATTER_BRIDGE_THEOREM_NOTE_2026-04-22.md`
as the matter-bridge candidate authority — this collapses the
...
authority on the current package surface. It is not promoted to a
markdown dependency edge in this pass because that edge adds a graph
cycle.
...
authorities currently carry a retained-grade ledger status. This note
does not set or predict an audit verdict; the pipeline currently owns
the row as `unaudited` pending re-audit, and any later accepted audit
must still honor the non-retained dependency chain. The residual gap (a retained
...
authority named) by registering an existing live runner and naming an
existing candidate authority without adding new graph cycles. Mirrors
the live cite-chain pattern used by the
