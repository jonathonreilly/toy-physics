...
core of the parent g_bare two-Ward Rep-B independence row.
...
- Does **not** derive `H_unit`'s normalization from the free-theory
  ...
  argument is the load-bearing content of `yt_ward_identity_derivation_theorem`,
...
- Does **not** claim that the parent's `g_bare = 1` selection is forced by
  ...
  content of `g_bare_two_ward_same_1pi_pinning_theorem_note_2026-04-19`.
...
The parent g_bare two-Ward Rep-B independence row bundles
three components:
