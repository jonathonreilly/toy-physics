...
emerges from a discrete causal lattice at the correlator level on the
continuum-limit free-scalar surface.
...
- **3+1D extension.** The corresponding 3+1D correlator-level theorem now
  closes separately on the Hamiltonian-lattice surface. The 1+1D case
  remains the clean dimensional-reduction proof where the boost question is
  isolated from rotation.
...
not just the leading on-shell relation. The 3+1D analogue is
...
The 3+1D correlator-level statement now closes separately, while the
