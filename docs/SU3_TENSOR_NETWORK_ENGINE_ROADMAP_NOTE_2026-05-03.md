...
quantitatively (`W_native < epsilon_witness ≈ 3e-4`).
...
contracting an SU(3) tensor network of 11 unmarked plaquettes meeting
at 8 sites with 24 link variables, with all the SU(3) Wigner intertwiner
algebra it entails. The framework's existing scripts implement only:
...
| 5 | L_s=2 APBC cube geometry + `rho_(p,q)(6)` + final P(6) verdict | depends on PRs 1-4 | NMAX convergence, comparison to existing reference solves |

Each PR will produce its own theorem note + runner + audit-pipeline row.

## 2. PR 1 — Fusion engine (this commit)

**Status:** delivered in this commit
...
**Deliverable:** for any pair of SU(3) irreps `lambda, mu` in the
dominant-weight box, compute the fusion multiplicities `N^nu_(lambda,
mu)` such that `lambda ⊗ mu = ⊕ N^nu_(lambda, mu) nu`.
...
## 6. PR 5 — L_s=2 APBC cube + final P(6) verdict (planned)

**Goal:** apply the engine to the L_s=2 APBC spatial cube to compute
`rho_(p,q)(6)` explicitly, then plug into the source-sector
factorization to compute `P(6)` and report the no-go bypass verdict.

**Geometry:**
- 8 sites (2³ at L=2 PBC).
- 24 directed link variables (or 12 unique unoriented at L=2 PBC).
- 12 unique unoriented spatial plaquettes (each (plane, slice) with 2
  distinct loops via different starting corners due to L=2 PBC).
- 1 marked plaquette + 11 unmarked.
...
**Verdict logic:**
- If `|P_cube(6) - 0.59353| < epsilon_witness ≈ 3e-4`: HONEST PATH B
  (full quantitative bypass of the no-go's witness construction).
  Promote parent gauge-scalar-temporal-completion to fully retained
  at the observable level.
- Otherwise: HONEST PATH A (narrowing only). Report the achieved
  precision.
...
4. **Cross-checks of the K-Z bootstrap** (PR #484) at small-volume
   instances where exact results are computable.
...
verdict_rationale_template: |
  Roadmap meta-document laying out the 5-PR plan to build a properly-
  engineered SU(3) tensor-network engine for closing the gauge-scalar
  temporal observable bridge no-go via explicit cube Perron solve.
  PR 1 delivered in this commit. PRs 2-5 planned as separate sessions
  with explicit interface contracts and validation criteria. Estimated
  total scope: ~2200-3500 LOC, 7-12 sessions. Engine is permanent
  infrastructure useful beyond the immediate cube Perron application
  (other small-volume SU(3) computations, string tension, K-Z bootstrap
  cross-checks).
```
