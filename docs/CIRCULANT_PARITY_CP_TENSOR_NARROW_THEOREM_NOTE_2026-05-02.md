...
`DM_NEUTRINO_ODD_CIRCULANT_Z2_SLOT_THEOREM_NOTE_2026-04-15` applies this
circulant parity-decomposition algebra to the DM-neutrino-side Hermitian
circulant kernel and identifies the residual-Z_2-odd slot as the local
coefficient that must be activated to make the displayed scalar nonzero.
That application uses DM-side authority-stack inputs outside this note's
scope: a DM minimal Z_3 circulant CP tool, a two-Higgs right-Gram bridge,
an exact weak-axis 1+2 split, and a standard CP tensor readout.
...
- `DM_NEUTRINO_ODD_CIRCULANT_Z2_SLOT_THEOREM_NOTE_2026-04-15` —
  ...
  as the relevant odd slot for that application.
