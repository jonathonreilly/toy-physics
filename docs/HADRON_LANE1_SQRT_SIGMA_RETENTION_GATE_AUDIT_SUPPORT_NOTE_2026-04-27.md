...
**Date:** 2026-04-27
**Status:** support / open-lane gate-audit note on `main`; no theorem or
claim promotion. Decomposes the 5.6% gap between the bounded
`sqrt(sigma) ≈ 465 MeV` derivation and PDG `440 ± 20 MeV` into explicit
EFT-bridge contributions. Identifies the residual budget items that
must be tightened before `sqrt(sigma)` can move beyond bounded status.
...
component is the qualitative confinement statement (`T = 0`). The
numerical extraction remains bounded, and the framework `SU(3)` to
standard `SU(3)` Yang-Mills bridge remains an explicit residual budget
item rather than a closed retained premise.
...
| (B5) Framework `SU(3)` ↔ standard `SU(3) YM` identification | unquantified | open structural bridge |
...
- (B4) is bounded by selecting Method 2 as the preferred route after
...
The still-open structural bridge is:
...
Creutz-ratio lattice numerics applicable to the framework. It is tracked
as (B5) because it still needs a declared residual budget before the
numerical `sqrt(sigma)` readout can be promoted.
...
Selecting Method 2 as the preferred bounded route (and demoting Method 1 to
