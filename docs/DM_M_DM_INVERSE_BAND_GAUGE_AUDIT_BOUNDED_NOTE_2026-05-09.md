...
- promote any gauge-group choice (`SU(3)` is an upstream framework
  authority here, not a derived consequence of (II));
- promote the parent freeze-out-bypass note's status.

(II) is a self-consistency audit: under the framework's current `SU(3)`
authority, Origin B's structural form is the unique
...
- derive the `(8/3) = dim(adj_3)/N_c` color-enhancement factor in the
  ...
  derived enhancement);
- promote `SU(3)` from an upstream framework authority to a
  freeze-out-bypass consequence. (II) is a self-consistency audit:
  **given** the framework's current `SU(3)` authority, only `N = 3` of the Origin-B
