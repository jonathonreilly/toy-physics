...
**Axiom-first structural companion:** for the exact operator-classification
question "which bare quadratic Majorana term is even allowed on the
anomaly-fixed one-generation surface?", use
`NEUTRINO_MAJORANA_OPERATOR_AXIOM_FIRST_NOTE.md` and
`scripts/frontier_neutrino_majorana_operator.py`. That result is structural and
exact. This note remains a bounded phenomenology surface.
