...
- **Z_3 Cyclic-Subgroup Commutant on C^8** — applies Schur + cyclic-
  group character theory: C^8 ≅ 4·1 ⊕ 2·χ_ω ⊕ 2·χ_{ω²} as Z_3 rep
  (Z_3 ⊂ S_3 generated by (123)), giving dim End(C^8)^{Z_3} = 24.
  Exposes the complex-phase (ω) structure hidden by S_3 invariance.
  Note: `Z3_CYCLIC_COMMUTANT_NOTE.md`
  Runner: `frontier_z3_cyclic_commutant.py` (20/20 PASS)

- **Z_2-Invariant Hw=1 Mass-Matrix Parametrization** — applies
  Schur's lemma + Hermitian spectral theorem + secular equation:
  explicit 5-parameter family M(a, b, c, d) for Hermitian
  Z_2-invariant operators on the hw=1 generation triplet, with
  sign-eigenvalue λ_sgn = b − c separating out and λ_± from the
  trivial-subspace 2×2 secular equation. Exhibits the closed-form
  SSB relief (Batch 4 no-go → Batch 5 dim 5 → this parametrization).
  Note: `Z2_HW1_MASS_MATRIX_PARAMETRIZATION_NOTE.md`
  Runner: `frontier_z2_hw1_mass_matrix_parametrization.py` (10/10 PASS)

- **Pullback of S_3 Invariants via the Site-Phase/Cube-Shift
  Intertwiner** (composition) — applies standard functoriality of
  commutants under equivariant isometries: the Batch 1 intertwiner
  Φ: C^8 → C^{L³} transports every Batch 3/4/5 S_3-invariant result
  from the taste cube to the BZ-corner subspace of the lattice.
