# Rescaled NN Lattice c2_inf Phase-Saddle Support Note

**Date:** 2026-05-10
**Claim type:** bounded_theorem (phase-saddle support for the no-slit
quadratic phase coefficient; analytic-vs-diagnostic residual 0.12% in
the h -> 0 saddle and < 1% per-h on all four no-slit runner fit points)
...
**Cached log:** [`logs/runner-cache/lattice_nn_rescaled_c2_derivation.txt`](../logs/runner-cache/lattice_nn_rescaled_c2_derivation.txt)
...
**No-slit source note:** [`docs/NN_LATTICE_RESCALED_KERNEL_IDENTIFICATION_NOTE_2026-05-10.md`](NN_LATTICE_RESCALED_KERNEL_IDENTIFICATION_NOTE_2026-05-10.md):
the single-source,
...
diagnostic approximation `c2_inf ~= K_PHYS / (4 L_total) = 0.03125` accurate
to ~4%. The closed-form phase-saddle support for `c2_inf` is supplied here.
...
```text
c2_inf_diagnostic =  0.02995    (no-slit runner cache).
```

Residual `+0.12%`. The diagnostic heuristic `K_PHYS / (4 L_total) = 0.03125`
is recovered up to the factor `(2 - sqrt(2)) / c ~= 0.9596`, which is what
carries the residual ~4% gap between `K/(4L)` and the computed 0.02995.
The proximity of `(2 - sqrt(2)) / c` to 1 is the reason the diagnostic fit
looked like `K/(4L)` at the few-percent level.

## Imported Authorities

| Authority | Role |
|---|---|
| [`docs/NN_LATTICE_RESCALED_KERNEL_IDENTIFICATION_NOTE_2026-05-10.md`](NN_LATTICE_RESCALED_KERNEL_IDENTIFICATION_NOTE_2026-05-10.md) | diagnostic no-slit source note — computed `c2_inf` and the K/(4L) heuristic; this note's quantitative comparison target. |
| [`docs/NN_LATTICE_RESCALED_C_ARM_DERIVATION_NOTE_2026-05-10.md`](NN_LATTICE_RESCALED_C_ARM_DERIVATION_NOTE_2026-05-10.md) | companion bounded magnitude support — same saddle-point machinery, applied to `Re(r)`. This note applies the saddle to `Im(r)`. |
...
`theta = atan2(|diy|, 1)`. For the three NN choices `diy in {-1,0,+1}`,
the diagonal-to-straight phase offset is exact:

```text
sqrt(1 + diy^2) = 1           for diy = 0
sqrt(1 + diy^2) = sqrt(2)     for diy = +/-1.
```

The per-edge phase is therefore `k h` on straight steps and
`k h sqrt(2)` on diagonal steps.
Summing over `N = L_total / h` edges, the total path phase is

```text
phi_path  =  k h * N  +  k h (sqrt(2)-1) * #{diagonal steps}
          =  k L_total  +  k h (sqrt(2)-1) * #{diagonal steps}.
```

The first term is path-independent (a global drift). The second depends on the
path only through the number of diagonal steps, equivalently
`Sigma_i diy_i^2`, NOT through the lateral displacement
`Sigma_i diy_i = (y_d - y_s)/h`. The quadratic dependence on `y_d` therefore
comes from how diagonal-step count correlates with displacement under the
constraint `Sigma_i diy_i = (y_d - y_s)/h`.

This is exactly the saddle-point structure the companion support note used for the magnitude:
...
### 2. Lateral characteristic function (companion support convention)
...
- `Re(H) = 2 L_total Re(r) h`: sets the lateral Gaussian width of the
  saddle-integrated amplitude. (This is the companion support-note role.)
...
The real part is the Gaussian magnitude decay, as used by the companion
support note:
...
```text
c2_inf_diagnostic  =  0.02995    (no-slit runner cache)
K_PHYS / (4 L)     =  0.03125    (diagnostic heuristic, ~4% above c2_inf)
...
| Diagnostic heuristic `K/(4L)` | 0.031250 | +4.34% |
| Ray-optics `K/(2L)` | 0.062500 | +108.7% |
...
Evaluated at the four no-slit runner fit points:

| `h` | `c2_analytic(h)` | `c2_diagnostic(h)` | residual |
|---|---:|---:|---:|
...
All four points agree with the no-slit runner measurement to within ~1%, with the
...
The companion magnitude support note used `L_eff = L_2 = 2 L_total / 3`
...
side, in the no-slit harness (single source at origin, no slits, no
...
- In the companion slit harness, the magnitude saddle integrates over post-slit
  paths only, anchored at the slit -> `L_eff = L_2`.
- In the no-slit harness, the phase saddle integrates over all
...
| Diagnostic heuristic | `K_PHYS / (4 L_total)` | 0.031250 | +4.34% |
| Saddle, `L = L_2` (wrong) | `k (2 - sqrt(2)) / (4 c L_2)` | 0.044978 | +50.18% |
| Ray-optics | `K_PHYS / (2 L_total)` | 0.062500 | +108.7% |

The saddle-point form sharpens the diagnostic `K/(4L)` heuristic by a factor
of ~36 in residual. The two "wrong" alternatives (L_2 length, ray-optics
prefactor) both land far outside the bounded comparison band, providing
...
per-h table against the no-slit runner's measured `c2(h)`, and the saddle-Hessian
...
This note derives bounded phase-saddle support for `c2_inf` in the
**no-slit, single-source, field-free** harness. It does not:

- prove the magnitude side; the companion support note uses
...
- claim the analytic formula holds outside the rescaled-NN deterministic-
  ...
  amplitudes).
- claim a positive Schrödinger free-particle identification. The no-slit null
  ...
  width side is the geodesic mechanism, and the no-slit source note already
  documented that no single Schrödinger `m_eff` reconciles both.

## What this supports

- A bounded analytic derivation of `c2_inf` to 0.12% versus the no-slit
  runner extrapolation. The closed form is `c2_inf = k (2 - sqrt(2))
  / (4 c L_total)`.
- The factor `(2 - sqrt(2)) / c ~= 0.96` is isolated as the source of
  the ~4% gap between `K/(4L)` and the computed `c2_inf`.
  `K/(4L)` is the heuristic; the actual closed form has the explicit
  `(2 - sqrt(2)) / c` correction.
- A clean structural pairing with the companion support note: both derivations use the same
...
- A positive continuum identification of `T_infinity`'s scattering kernel as a
  recognized PDE propagator. The no-slit null-result stands: the magnitude
...
This source note is a bounded closed-form support proposal. The audit lane
sets the effective status after independent review of the runner, the
derivation steps, and the length identification (slit-free harness ->
`L_total`, not `L_2`).
