...
already present on the retained chart (`SELECTOR^2 = Q_Koide`,
...
- [`PMNS_THREE_IDENTITY_Q_KOIDE_FROM_V8_SUPPORT_LIFT_THEOREM_NOTE_2026-04-29.md`](PMNS_THREE_IDENTITY_Q_KOIDE_FROM_V8_SUPPORT_LIFT_THEOREM_NOTE_2026-04-29.md)
  ...
  Downstream support lift composing V8 (Block 1, Q_Koide = 2/3 on
  A_min) with the present three-identity package to upgrade the chart
  ...
  (Block 1) ratification, it does not promote the present note's
...
- The proposed selector law `delta * q_+ = Q_Koide` is imported as a
  ...
  packet. Closing it would require a retained derivation of
  `delta * q_+ = Q_Koide` from retained framework structure (the
  affine Hermitian chart plus retained source/transfer or
...
- The proposed selector law `det(H) = E2` is imported as a candidate
  ...
  a retained derivation of `det(H) = E2` from retained chart
  primitives.
...
load-bearing-step class G and `chain_closes=False`, observing that the
