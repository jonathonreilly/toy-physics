...
from the audited restricted source families and fits a symmetric
`K_tensor` from those probe directions. The retained primitives the
runner imports are each one-hop authorities on the current main
surface:
