...
**Date:** 2026-05-10
**Claim type:** bounded_theorem (mostly negative)
...
**Cached output:** [`logs/runner-cache/cl3_koide_x_l1_msbar_2026_05_08_probeX_L1_msbar.txt`](../logs/runner-cache/cl3_koide_x_l1_msbar_2026_05_08_probeX_L1_msbar.txt)
...
structural retention via the S1 Identification Source Theorem
...
This probe asks: can the framework's retained content
(Cl(3)/Z³ Casimir algebra, plaquette structure, Wilson-loop expectation
...
## 1. Theorem (bounded, mostly negative)

**Theorem (X-L1-MSbar; bounded).** On retained content of Cl(3)/Z³,
...
not present in the retained content. Specifically:

1. **(beta_0, beta_1 are universal and ALREADY retained.)** The 1-loop
   ...
   They are derivable from S1 + retained Casimirs `(C_F = 4/3, C_A = 3,
...
2. **(beta_2 in any scheme requires 3-loop integral primitives.)**
   ...
   T_F N_f`) is framework-derivable from retained Casimir algebra.
   ...
   On retained Cl(3)/Z³ content, the framework has NEITHER the
   ...
   propagator that are not part of the retained primitive stack).
...
3. **(beta_3 status: same obstruction at higher order.)** At 4-loop, the
   ...
   `d_F^{abcd} d_A^{abcd} / N_R` enter — these ARE retained Casimir
   algebra. But the scalar 4-loop coefficients are again scheme-dependent
   integral primitives outside the retained content. Same verdict.
...
5. **(But the <P>-scheme beta_2 still requires 3-loop lattice
   ...
   None of these primitives exist on retained Cl(3)/Z³ content. The
...
- **beta_0, beta_1 are universal and retained.** Confirmed: the 1-loop
  and 2-loop QCD beta-function coefficients are scheme-independent and
  derivable from retained Casimir algebra + S1. No scheme distinction
  affects them.
- **Color-tensor skeleton at 3-loop is retained.** The basis of nine
  Casimir-tensor channels at 3-loop QCD `(C_F³, C_F² C_A, C_F C_A²,
  C_A³, C_F² T_F N_f, C_F C_A T_F N_f, C_A² T_F N_f, C_F (T_F N_f)²,
  C_A (T_F N_f)²)` IS derivable on retained content; the four-loop
  basis with quartic Casimirs ALSO retained as algebra.
...
- **beta_2 closed-form derivation in any scheme.** The scalar
  ...
  `c_FFF, ..., c_Ann` are not retained; they must be either
...
- **Scheme conversion factor between MSbar and `<P>`-scheme.** In
  ...
  retained `<P>` plus 1-loop, 2-loop matching integrals. The 1-loop
  matching `delta_1 = -(4 pi)/(3 <P>)` could be a follow-on
  framework-native theorem; full 3-loop conversion is beyond retained
  content.
...
```
[POSITIVE]
beta_0 = (11 N_color − 2 N_quark)/3 = 7  (scheme-independent, retained via S1+Casimir)
beta_1 = ((34/3) C_A² − (20/3) C_A T_F N_f − 4 C_F T_F N_f) = 26 at N_f=6
        (scheme-independent universal, retained via S1+Casimir)
...
[BOUNDED ADMISSION]
beta_2 in <P>-scheme closed form: NOT derivable from retained content;
...
Total framework-native gain on Lane 1 bridge from this probe:
  - converts "MSbar β_0, β_1 are imports" → "β_0, β_1 are retained"
    (strong: removes 2-loop MSbar truncation envelope claim)
...
- MSbar `beta_2 = 2857/2 − 5033 N_f / 18 + 325 N_f² / 54` per
  ...
  2857/2 − 1461 = 65/2`. Note: the user-provided value `65/2` agrees
  with the canonical MSbar formula at `N_f = 6`.
- MSbar `beta_3 = 149753/6 + 3564 zeta_3 − (1078361/162 + 6508 zeta_3 / 27)
  ...
  is in any case not framework-derivable from retained content.
...
1. **POSITIVE retention check 1**: `beta_0 = (11 N_color − 2 N_quark)/3 = 7`
   at `N_f = 6` from S1 + retained Casimirs. Direct from
   `SU2_WEAK_BETA_COEFFICIENT_STRUCTURAL_CLOSED_FORM_THEOREM_NOTE_2026-04-26.md`
   companion form for QCD.

2. **POSITIVE retention check 2**: `beta_1 = (34/3) C_A² − (20/3) C_A T_F
   N_f − 4 C_F T_F N_f = 26` at `N_f = 6` from retained Casimirs. This
...
5. **BOUNDED admission check 5**: For each Casimir-tensor channel
   ...
   integral primitives outside retained content. Show that the
...
7. **BOUNDED admission check 7**: Document the lattice scheme β_2^lat
   ...
   primitives outside retained content.

8. **HONEST verdict**: bounded; MOSTLY NEGATIVE on the closed-form
   derivation, with positive structural retentions at 1-loop, 2-loop,
...
- **Direct contribution to closing Lane 1 alpha_s(M_Z).** Currently
  ...
  [`QCD_LOW_ENERGY_RUNNING_BRIDGE_NOTE_2026-05-01.md`](QCD_LOW_ENERGY_RUNNING_BRIDGE_NOTE_2026-05-01.md);
  the structurally retained `beta_0, beta_1` (already retained on main)
  cover that bridge, so this probe does NOT change Lane 1 status.
- **Closed-form derivation of the lattice → MSbar scheme conversion.**
  The 1-loop matching `delta_1 = -(4 pi)/(3 <P>)` is a candidate
  follow-on; full 3-loop conversion is beyond retained content.
...
| Quantity | Scheme | Retention status | Source |
|---|---|---|---|
| `beta_0 = 7` (N_f=6) | universal | RETAINED | S1 + Casimir, this probe |
| `beta_1 = 26` (N_f=6) | universal (2-loop) | RETAINED | S1 + Casimir, this probe |
| 3-loop Casimir-tensor skeleton (9 channels) | universal | RETAINED | This probe (color tensors) |
| 4-loop Casimir-tensor skeleton with quartic invariants | universal | RETAINED | This probe (extended Casimir algebra) |
| `<P>_HK_SU(3)` closed form | `<P>`-scheme native | RETAINED | C_ISO_SU3_NLO bounded note |
| Scheme distinction `<P>` vs MSbar | structural | RETAINED | This probe (positive observation) |
| `beta_2^MSbar(N_f=6) = 65/2` | MSbar | BOUNDED ADMISSION | Tarasov-Vladimirov-Zharkov 1980 |
| `beta_3^MSbar(N_f=6) ≈ 3863/6` | MSbar | BOUNDED ADMISSION | van Ritbergen et al. 1997 |
| `beta_2^<P>(N_f=6)` closed form | `<P>`-scheme | NOT DERIVABLE from retained content | This probe (negative result) |
| `beta_3^<P>(N_f=6)` closed form | `<P>`-scheme | NOT DERIVABLE from retained content | This probe (negative result) |
| 1-loop scheme conversion `delta_1 = -(4 pi)/(3 <P>)` | conversion | candidate follow-on | This probe (open) |

## 9. Falsifiable structural claims

1. `beta_0 = (11 N_color − 2 N_quark)/3 = 7` at retained `N_quark = 6`,
   `N_color = 3`, `N_f = N_quark = 6`. NEW retention via S1.
2. `beta_1 = (34/3) C_A² − (20/3) C_A T_F N_f − 4 C_F T_F N_f` evaluated
   at retained `(C_F = 4/3, C_A = 3, T_F = 1/2, N_f = 6)` gives
   `(34/3)·9 − (20/3)·3·(1/2)·6 − 4·(4/3)·(1/2)·6 = 102 − 60 − 16 = 26`.
3. The 3-loop and 4-loop QCD beta-function color-tensor skeletons are
   exactly determined by retained Casimirs alone; the channel weights
...
retentions (beta_0, beta_1, color-tensor skeleton, scheme distinction)
...
classifying the probe verdict as `bounded_theorem`
(positive on retentions, mostly negative on full closed-form derivation).
