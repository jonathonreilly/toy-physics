...
Claude time while credits are constrained. They are not routine portability
tasks. They are the specific lanes most likely to move the internal
`Nobel-scale breakthrough` sub-goals if they land.
...
1. **Truly unique causal discriminator**
   ...
   - Why it matters:
     - this is the cleanest route from “portable internal dynamics” to
       “genuinely irreducible causal mechanism”
...
2. **Persistent inertial object**
   ...
   - Fast falsifier:
     - every candidate remains only a broad steering surrogate and never
       separates persistence from ordinary test-particle transport
   - Why it matters:
