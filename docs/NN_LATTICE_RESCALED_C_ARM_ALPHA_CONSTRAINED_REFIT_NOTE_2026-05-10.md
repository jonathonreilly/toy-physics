...
**Date:** 2026-05-10
**Claim type:** positive_theorem (residual closure of the apparent 8.3 %
  gap between PR #968's numerical fit and PR #1003's closed-form
  derivation; sharpens both upstream PRs)
...
**Trigger:** PR #968 (numerical fit `C_arm = 2.7107`, `alpha = 0.5256`)
  + PR #1003 (closed-form `C_arm_analytic = 2.4855`); apparent residual
  `(C_arm_analytic - C_arm_unconstrained) / C_arm_unconstrained = -8.31 %`.

## TL;DR

The 8.3 % residual was a **fit-protocol artifact**, not a real
disagreement. Both upstream PRs are sharpened, neither is overturned:

- the analytic constant `C_arm_analytic = 2.4855` (PR #1003) is
  recovered pointwise at the finest measured spacing `h = 0.015625`
  with residual `+0.27 %`,
- the numerical fit `C_arm_unconstrained = 2.7107` (PR #968) was an
  unconstrained 2-parameter regression that let `alpha` drift to
  `0.5256` to absorb the finite-h cosine phase factor in the saddle,
  which biases the prefactor upward by exactly the observed 8 %.

Under the geodesic constraint `alpha = 1/2` exact (which is the
identification PR #968 itself argued for) the per-h estimator
...
The residual halves with each factor-of-two refinement (5.79, 2.24,
1.10, 0.50, 0.27 %), consistent with a sub-leading `O(h)` correction
to the leading-order saddle, not a missing leading-order mechanism.

## Why the unconstrained fit overshot

PR #1003's full coherent formula retains a phase factor:
...
A new measurement at `h = 0.015625` (one factor of 2 beyond PR #968's
finest grid point) using the same `measure_arm_distribution(...)`
...
d_TV_obs     = 1.000000
Born         = 1.32e-15
elapsed      = 54.4 s              (single arm-pair pass, no MI/dTV scan)
```
...
`Born = 1.32e-15` at `h = 0.015625`, which match the saturation
values established by PR #957 and PR #968 to machine precision; no
...
re-running the full PR #968 runner with a new `H_VALUES` list and
...
|---|---|---:|---:|
| PR #1003 | Closed-form coherent saddle, `h -> 0` | 2.4855 | 0 (definition) |
| PR #968 | Unconstrained log-linear fit, h <= 0.25 | 2.7107 | +9.06 % |
| **This PR** | **Constrained estimator at h = 0.03125** | **2.4981** | **+0.504 %** |
| **This PR** | **Constrained estimator at h = 0.015625** | **2.4922** | **+0.268 %** |

## Outcome class

This is the **best-case outcome** of the prompt: the residual at the
new finer point closes to `0.27 %`, well inside `1 %`. The residual at
`h = 0.03125` (the prompt's anchor for the acceptance criterion) is
`0.504 %`, also inside `1 %`. The 8.3 % bound was fit-protocol; the
analytic and numerical agree pointwise at the finest measured `h`.
...
not supersede the upstream PRs. It sharpens both: PR #968's
identification stands (the geodesic exponent `alpha = 1/2` is the
correct one and is exact in the `h -> 0` limit); PR #1003's analytic
prefactor stands (and is now recovered numerically pointwise). The
...
The runner takes ~55 s (dominated by the single `h = 0.015625`
arm-distribution measurement; the rest is closed-form arithmetic on
PR #968's hardcoded cache values). It exits 0 if the residual at the
...
This note inherits all bounded scope from PR #968 and PR #1003:
...
- the cosine phase factor is the leading sub-`sqrt(h)` correction;
  ...
  PR #1003 sec. 3) are not separately quantified here. The observed
  `O(h)` halving of the residual is consistent with that order being
  sub-leading at the measured grid.

## Status

This source note is a residual-closure proposal layered on the two
upstream positive theorems. The audit lane sets the effective status
