...
Framework provenance of the retained quark packet is now explicit at point of
use. The ambient bimodule

```text
B = Cl(3)/Z_3 ⊗ Cl_CKM(1⊕5)
```

is the retained scalar-ray / CKM carrier already used in
`QUARK_JTS_RESIDUE_NOTE_2026-04-19.md` and
`QUARK_STRC_OBSERVABLE_PRINCIPLE_NOTE_2026-04-19.md`. The exact scalar atoms
...
come from `CKM_ATLAS_AXIOM_CLOSURE_NOTE.md`. In particular `a_d = rho` is the
retained projector-ray parameter, while `supp` and `delta_A1` are the retained
support-side bridge data on the same packet.
