...
chain-closure explanation is exact: "The artifact chain computes a
...
excluding all instantaneous emulator models." The audit-stated
repair target is exact: `other` — "add a fast deterministic runner
...
the conditional perimeter and registers the runner-cache budget
...
runner's `main()` enumerates the full retained harness in one
...
prints all eight retained sections in the canonical "Frozen result"
log
...
The retained tables in this note (Difference curve at delay=5,
...
audit verdict's repair target ("fast deterministic runner with
...
each retained number; that change is deferred because it changes
...
forms the second leg of the audit verdict's repair target is a
