...
- **Coupling:** `G=2,5` (G=10 enters noisy nonlinear regime)
...
- **Mutual acceleration sign:** all 25 phase-jitter repeats attractive (5 seps × 5 seeds)
- **Individual CoM:** both packets approach each other in all 25 configs
- **Distance law:** `|a_mutual| ~ d^{-1.966}` with `R^2=0.9998` (5 separations, seed-averaged)
...
4. **Staggered architecture:** this is Wilson fermions only. The primary
   staggered architecture has no retained trajectory-level result due to
   parity oscillation contaminating CoM observables.
...
> On the audited open Wilson surface (`side=20`, `G=2-5`, `mu^2=0.001`),
> two-orbital Hartree dynamics produces a robust mutual attraction channel
> with `|a_mutual| ~ d^{-1.97}` (`R^2=0.9998`). Both packets approach
> each other, and the signal is deterministic across phase-jitter repeats.
> This result is bounded to one geometry, one placement family, equal
> masses only, and Wilson fermions. The both-masses law remains unresolved.
