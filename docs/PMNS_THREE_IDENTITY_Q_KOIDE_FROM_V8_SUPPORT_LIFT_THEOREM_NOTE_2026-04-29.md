...
**Date:** 2026-04-29
**Status (actual current surface):** `proposed_promoted` SUPPORT-grade
lift — composes V8 (Block 1, Q_Koide = 2/3 on A_min) with the
PMNS three-identity selector support (2026-04-21) to upgrade the
chart constant `Q_Koide = 2/3` from "imported numeric constant" to
...
broader PMNS/DM gate) are unchanged. Bare `retained` / `promoted` is
NOT used.
...
V8 (Block 1, 2026-04-29) closed Q_Koide = 2/3 on A_min via the
OP-locality structural chain. This V1 note composes the two: the
...
V1 is `proposed_promoted` SUPPORT-grade, not closure.
...
Status: `proposed_retained`, audit-required-before-effective-retained.
...
**Status:** `proposed_promoted` SUPPORT-grade. The three open PMNS
gaps (proposed selector laws not yet retained, basin uniqueness
bounded multi-start, broader PMNS/DM gate open) are unchanged.
...
proposal_allowed_reason: |
  V1 composes V8 (Block 1) Q_Koide closure with the PMNS three-
  identity support package. The chart constant Q_Koide = 2/3 lifts
  from "imported numeric" to "V8-derived structural value". Three
  open PMNS gaps are unchanged.
...
If V1 audit-ratifies (depends on V8 audit):
- PUBLICATION_MATRIX line for PMNS three-identity support: chart
  constant is now V8-derived; chart-side import retired.
- PMNS_SELECTOR_THREE_IDENTITY_SUPPORT_NOTE adds §Note that
  Q_Koide chart constant is structurally V8-derived.
