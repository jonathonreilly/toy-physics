# PR230 Two-Source Taste-Radial Chunks043-044 Package

**Status:** bounded-support / chunks001-044 packaged; no closure

**Checkpoint certificates:**
`outputs/yt_pr230_two_source_taste_radial_chunk043_checkpoint_2026-05-06.json`,
`outputs/yt_pr230_two_source_taste_radial_chunk044_checkpoint_2026-05-06.json`

## Result

Chunks043-044 now pass completed-mode checkpoints and are included in the
two-source taste-radial row package.  The row combiner reports `ready=44/63`
with `combined_rows_written=false`.

Both chunks preserve the production surface:

- fixed seeds `2026056043` and `2026056044`;
...
The row-wave launcher active-process filter was narrowed to require
`scripts/yt_direct_lattice_correlator_production.py` as an actual process
argument.  This prevents monitor or supervisor shell commands containing the
production-script string from blocking future waves as unknown active workers.
After that run-control fix, chunks045-046 launched under the two-worker cap
with PIDs `73711` and `73712`.  They are active run-control only and are not
counted as evidence.
...
- The Schur split remains bounded support only.  `C_s|x` fails
  (`z=148.50161996122023`), while `C_x|s` survives only the necessary
  first-shell check (`z=-501.5228370887908` in the repair gate).
- The refreshed `C_x|s` one-pole interpolation gives
  `C(0)=0.28082772941272105`,
  `C(0.267949192431123)=0.2695320149102949`,
  `m^2=6.3936536032346485`, and `R=1.795515224047849`; positive two-pole
...
python3 scripts/frontier_yt_pr230_two_source_taste_radial_row_wave_launcher.py --launch --max-concurrent 2 --output outputs/yt_pr230_two_source_taste_radial_row_wave_live_status_2026-05-06.json
# SUMMARY: PASS=12 FAIL=0, launched chunks045-046
```
