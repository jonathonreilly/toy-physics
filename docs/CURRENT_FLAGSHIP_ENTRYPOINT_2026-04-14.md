# Current Flagship Entrypoint

**Date:** 2026-04-14  
**Purpose:** one-page orientation for the current `Cl(3)` / `Z^3` paper surface

This is the correct root-level entrypoint for an external reviewer.

## What The Current Paper Actually Is

The current flagship paper is **not** a summary of everything in this repo.
It is a disciplined `Cl(3)` on `Z^3` package with:

1. a retained theorem core
2. a broader observation-facing bounded portfolio
3. three live flagship gates
4. an explicit frozen-out registry so important work is not lost

## Retained Core

- `Cl(3)` on `Z^3` as the working physical theory
- weak-field gravity through the Poisson / Newton chain
- weak-field WEP and time dilation
- restricted strong-field gravity closure on the current star-supported
  finite-rank class
- exact native `SU(2)`
- graph-first structural `SU(3)`
- anomaly-forced `3+1`
- derived electroweak scale `v = 245.080424447914 GeV`
- retained `S^3` compactification / topology closure
- full-framework one-generation matter closure
- retained three-generation matter structure
- exact `I_3 = 0`
- exact CPT on the free staggered lattice

## Observation-Facing Bounded Portfolio

- DM ratio `R = 5.48` vs `5.47`
- zero-import `alpha_s(M_Z) = 0.1182`
- best import-allowed `m_t = 171.0 GeV`
- zero-import `m_t = 165.4 GeV`
- CKM magnitude package
- Cabibbo and partial Jarlskog companions
- cosmology companions such as `Omega_Lambda`, `n_s`, `w = -1`, `Lambda`
- Higgs and sharp companion predictions

These results matter and should be visible, but they are **not all promoted**
into the retained flagship surface.

## Live Flagship Gates

1. DM relic mapping
2. renormalized `y_t` matching
3. CKM / quantitative flavor closure

## Where The Curated Publication Package Lives

The authoritative publication package currently lives on branch
`codex/publication-prep` under:

- `docs/publication/ci3_z3/README.md`
- `docs/CI3_Z3_PUBLICATION_STATE_2026-04-12.md`
- `docs/publication/ci3_z3/PUBLICATION_MATRIX.md`
- `docs/publication/ci3_z3/FULL_CLAIM_LEDGER.md`
- `docs/publication/ci3_z3/FROZEN_OUT_REGISTRY.md`

If you are reviewing the paper, that package is the authority surface. The root
repo contains much more work than the paper currently promotes.

## Reading Rule

- If a result is not in the publication matrix, it is not publication-captured yet.
- If it is in the matrix but not in the claims table, it is not on the retained
  flagship claim surface.
- If it is important but excluded, it should appear in the frozen-out registry.
