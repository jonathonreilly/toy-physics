...
3. the local-Pfaffian uniqueness note says that local bilinear realization is
   uniquely Pfaffian
...
- that the primitive must be realized by a Pfaffian/Nambu Gaussian beyond the
...
- on the retained local bilinear lane, that unique slot is realized on the
  Pfaffian surface

**Cannot claim**

- the axiom already forces the source slot to turn on
- the axiom already selects the Pfaffian realization uniquely beyond the
  retained local bilinear lane
