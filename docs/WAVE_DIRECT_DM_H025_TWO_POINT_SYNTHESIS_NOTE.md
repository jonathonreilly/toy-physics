...
**Audit-conditional perimeter (judicial review, post-2026-04-30):**
The audit lane has classified this row `audited_conditional` with
`auditor_confidence = judicial`, `chain_closes = false`, `claim_type
= positive_theorem`, `independence = judicial_review`, and
load-bearing step class `B`. The audit chain-closure explanation is
exact: "the note depends on a missing artifact-chain provenance and
...
or third-seed rule. The audit-conditional perimeter is exactly the
