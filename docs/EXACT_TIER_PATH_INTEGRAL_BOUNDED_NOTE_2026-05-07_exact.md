# Path-Integral W1 Bounded-Tier Tightening Note

**Date:** 2026-05-07
**Claim type:** bounded_theorem
**Sub-gate:** W1 (multi-plaquette numerics — bounded path-integral support)
...
- [`SPINNETWORK_FULL_ED_LOOP_BASIS_BOUNDED_THEOREM_NOTE_2026-05-07_w1exactv2.md`](SPINNETWORK_FULL_ED_LOOP_BASIS_BOUNDED_THEOREM_NOTE_2026-05-07_w1exactv2.md)
  for loop-supporting basis enumeration W1.exactv2.
...
## 1. Bounded theorem (three-path tightening)
...
4. **(Path C, Convention C-iso analytic refinement.)** The
   ...
   This is computed for SU(2) as a tractable proxy. It is evidence
   for the size of the Convention C-iso correction, not an SU(3)
   heat-kernel theorem. Extending the analytic comparison to SU(3)
   remains open.

## 2. What this closes vs does not close

### Established as bounded support (this work)
...
- **Convention C-iso single-plaquette proxy calculation**:
  `rel_shift ≈ 0.21*s_t = 0.105*g^2/xi` in the SU(2) proxy,
  quantifying the expected O(s_t) scale. This does not promote to an
  SU(3) analytic bound until the SU(3) heat-kernel calculation is done.

### Not closed

- **Exact-tier closure target epsilon_witness ~ 3e-4**: the volume- and N_t-converged
...
- Exact-tier closure of the W1 sub-gate (epsilon_witness ~ 3e-4).
...
For exact-tier closure (epsilon_witness ~ 3e-4), the named compute
frontier is:
