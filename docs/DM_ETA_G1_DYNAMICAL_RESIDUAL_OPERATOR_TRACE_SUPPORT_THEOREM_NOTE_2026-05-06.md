...
DM-eta G1 closure. This note attempts the audit-ratifiable upgrade of
the previously-flagged dynamical residual by deriving `rho_{adj/c} = 8/3`
...
This V1 attempts the audit-ratifiable upgrade of the dynamical step.
After a 5-way Counterfactual Pass on dynamical mechanisms, the unique
...
claim_scope: |
  Bounded support theorem deriving rho_{adj/c} = 8/3 from the operator-
  trace projection through the adjoint Fierz channel on End(C^N_c),
  conditional on the named open bridge step that the dark hw=3
  ...
  Composition with the cited bare Wilson mass m_S3_bare = 6 v gives
  m_DM = 16 v on the canonical surface. Six wrong-channel candidates
  are explicitly ruled out by the runner.
g1_dynamical_step_status: bounded_support_with_open_bridge
g1_arithmetic_step_status: closed_via_operator_trace
g1_bridge_step_status: open_residual_of_residual
g1_algebraic_step_status: closed_v1   # inherited
counterfactual_pass_done: true
counterfactual_pass_routes_scored: 5
counterfactual_pass_winner: route_c_adjoint_fierz_channel
