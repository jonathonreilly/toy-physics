...
load-bearing step class `B`. The audit verdict-rationale records the
boundary explicitly:
...
The audit-stated repair is `scope_too_broad`: "split the claim into
...
The single load-bearing item that is retained-grade through the
authority chain is the V_eff coefficient assignment / V_eff-vs-m_* gap
fixed in the cited parent
...
(retained, `audited_clean`). The §3a / §3b / §3c / §3d route
"exhaustion" sub-claims, the j=1/2 Kramers-doublet structural
identification (§1), the doublet-A equal-diagonal exact value m_DA =
−√(2/3) (§2), the H_* witness ratio kappa_* ≈ −0.608 (§4), and the
"open-routes" inventory (§5) are *not* covered by the cited one-hop
authority and remain conditional pending direct retained dependencies
or runners per route. The Per-route conditional perimeter table below
makes this explicit.

This rigorization edit only sharpens the conditional perimeter and
narrows the load-bearing scope to what the cited parent supports;
nothing here promotes audit status, no new derivation is asserted, and
no audit JSON is modified. The note has no audited runner — the §1 -
§4 numerics ("≈ −0.816", "GAMMA = 1/2 exact", "miss m_* by > 5%",
"eigenvalues (−2.507, −0.848, +2.195)") are recorded as printed claims
unsupported by an in-repo retained derivation chain, except where the
parent V(m) coefficient assignment carries them.
...
inventory*, not as a closed bounded theorem, exactly as the audit
verdict's "claim boundary until fixed" prescribes.
...
Per the audit-stated repair target, this table records each listed
route's current retained-grade dependency posture. None of the §3 / §4
/ §5 sub-claims is independently retained by the cited one-hop
authority; the only retained-grade item is the V_eff coefficient
assignment carried by the parent mass-tower note.

| Section | Sub-claim | Retained-grade in-repo dependency? | Conditional posture |
|---|---|---|---|
...
treated, per the audit verdict, as an *open selector-gap inventory*,
