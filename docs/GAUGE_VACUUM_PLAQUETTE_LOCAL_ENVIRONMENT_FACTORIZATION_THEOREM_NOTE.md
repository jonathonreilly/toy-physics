...
- [gauge_vacuum_plaquette_temporal_gauge_mixed_kernel_factorization_bridge_theorem_note_2026-05-10](GAUGE_VACUUM_PLAQUETTE_TEMPORAL_GAUGE_MIXED_KERNEL_FACTORIZATION_BRIDGE_THEOREM_NOTE_2026-05-10.md) — bridge note added 2026-05-10 to discharge the `missing_bridge_theorem` repair target named by the 2026-05-10 audit verdict on this note. Derives (B1) the temporal-gauge mixed-kernel factorization over spatial links and (B2) the marked / non-marked compression map directly from the retained one-clock Wilson transfer kernel and SU(3) Peter-Weyl decomposition. Bridge note is `unaudited` until the independent audit lane reviews it.
