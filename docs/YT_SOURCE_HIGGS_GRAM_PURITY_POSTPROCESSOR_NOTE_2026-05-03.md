...
**Status:** open / postprocessor awaiting production certificate
...
route.  When a future production certificate supplies same-surface pole
residues for `C_ss`, `C_sH`, and `C_HH`, the runner computes:
...
and checks whether `Delta = 0` and `|rho_sH| = 1` within the declared
tolerance or supplied residue uncertainties.
...
it is the claim firewall before the missing rows exist.
