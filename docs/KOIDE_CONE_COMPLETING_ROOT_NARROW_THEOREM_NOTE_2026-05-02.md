...
uses this completing-root identity in the selected-line context to
argue that `u_small v w = 1` is a Koide-completed reparameterization
condition rather than a native pre-Koide forcing law. The selected-line
`H_sel(m)` inputs and near-miss / physical-point comparisons are
physical-context inputs that this note does not consume.
