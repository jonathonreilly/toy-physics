# G1 Physics Validation — eta/eta_obs at Z_3 Doublet-Block Selector Candidates
...
**Script:** `scripts/frontier_g1_physics_validation_eta_at_candidates.py`
**Framework convention:** "axiom" means only the single framework axiom `Cl(3)` on `Z^3`.

## Scope and discipline

The G1 gate, on the integration branch `claude/g1-complete`, carries:

- one partial-closure theorem (Schur baseline forces `D = m I_3`),
- three obstruction / cross-check theorems (Path A information-geometric,
  Path B Z_3 cubic variational, Path C holonomy).
...
| B | `(0.613372, 0.964443, 1.552431)` | Path-C `det(H)` chamber-interior critical point |
| C | `(0.385132, 1.267881, 0.365112)` | Path-C `Tr(H^2)` chamber-boundary minimum |
| D | `delta ~ 0.800, q_+ free` | Path-C `K_12` character-match (a curve) |
...
The runner `scripts/frontier_g1_physics_validation_eta_at_candidates.py`
...
**G1 NOT CLOSED by the physics-validation test.**
...
2. **Independent microscopic consistency condition.** A
   ...
   `sym(H)` or `anti(H)` alone (Path-C Theorem 2 obstruction) and does
...
- Appropriate placement: obstruction row under the DM neutrino
  source-surface / G1 family in
  `docs/publication/ci3_z3/DERIVATION_ATLAS.md`.
- The DM flagship gate stays **open**.
- The Schur-baseline partial closure, the three Path A/B/C obstruction
  theorems, and this physics-validation obstruction together delimit
  the G1 open object as: *a selector that couples to a bank-unblinded
  observable on the chamber*.

## Atlas inputs used

All retained / theorem-grade on the current `claude/g1-complete`:
...
PYTHONPATH=scripts python3 scripts/frontier_g1_physics_validation_eta_at_candidates.py
```
...
- that the Schur-Q candidate A has been ruled out (it has not; this
  ...
  it from B, C, or D);
- that transport alone is sufficient to close G1 (it is not).
...
read is: **narrower-gap obstruction; G1 open; transport chain
chamber-blind**.
