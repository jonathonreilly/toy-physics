...
**Loop:** bridge-gap-new-physics-20260506 (Block 01 / R1.A)
**Branch (intended):** physics-loop/bridge-gap-new-physics-block01-20260506
...
admitted_observation_status: |
  ...
  is admitted standard machinery in a narrow non-derivation role;
  Brownian motion on compact Lie groups (Helgason; Liao 2004) is
  admitted standard machinery. Neither imports a numerical PDG/MC value
  as a derivation input.
claim_type_reason: |
  Theorem 1 is a derivation chain from canonical Tr-form + standard
  small-U matching to a specific exact-rational Brownian time. It is not
  a Resolution-A closure of the bridge gap because (i) action-form
  uniqueness (Wilson vs heat-kernel) remains open; (ii) finite-β
  correction structure remains open. It is bounded support: useful for
  the four cluster lanes if and only if heat-kernel turns out to be the
  framework's actually-derived action, which Block 04 of this loop
  attacks.
