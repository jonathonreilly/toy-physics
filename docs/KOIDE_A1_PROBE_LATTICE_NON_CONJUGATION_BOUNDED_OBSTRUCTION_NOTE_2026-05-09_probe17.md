...
| PerSite | Per-site Hilbert ≅ `M_2(ℂ)` (qubit), generated by `σ_i` | retained per [`AXIOM_FIRST_CL3_PER_SITE_UNIQUENESS_THEOREM_NOTE_2026-04-29.md`](AXIOM_FIRST_CL3_PER_SITE_UNIQUENESS_THEOREM_NOTE_2026-04-29.md) U2 |
| PerSiteU1 | Per-site qubit U(1) ⊂ U(`M_2(ℂ)`) is the continuous Lie group of phases on each site | retained per per-site uniqueness + standard exponentiation |
| Probe14 | Every retained U(1) acts on `M_3(ℂ)` as conjugation; on `A^{C_3}` trivial | retained per Probe 14 (PR #784) |
| Probe13 | `K`-real-structure supplies Z_2 part of doublet combination, not SO(2) | retained per [`KOIDE_A1_PROBE_REAL_STRUCTURE_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe13.md`](KOIDE_A1_PROBE_REAL_STRUCTURE_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe13.md) |
...
3. **Sharpens the named admission residue.** The Probe 13 sharpening
   ...
   remaining option compatible with retained content; no algebra-level
   route exists.
...
- **Identification of the only remaining option.** The readout-level pivot
  ...
  retained content is a separate question (and a candidate for a future
  probe).
