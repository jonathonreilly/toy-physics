...
**Date:** 2026-05-10
**Status:** sharp_bounded_null — operator-Cauchy continuum-bridge method does
not extend cleanly to the alt-connectivity grown DAG family. Structural
reason identified.
...
- companions on the rescaled NN harness (positive existence on a different
  harness):
  - PR #957 — `docs/NN_LATTICE_RESCALED_OPERATOR_CAUCHY_CONVERGENCE_NOTE_2026-05-10.md`
  - PR #968 — geodesic identification of T_inf on the slit-detector
    decoherence subblock (σ_arm = C·h^0.526)
  - PR #1003 — closed-form C_arm = 2.49 from saddle-point
...
Does the operator-Cauchy continuum-bridge method established on the rescaled
NN harness (PRs #957/#968/#1003) extend to the alt-connectivity grown DAG
family — the largest C-class subgroup in the 19-row sub-lane mapping?
...
- **Vector dimension:** 5 x 3 = **15**, identical to PR #957's basis
  dimension
- **Cauchy increment:** `||vec(N) - vec(2N)||_2`
- **Fit:** geometric decay `||vec(N) - vec(2N)||_2 ~ C * N^r`
- **Gate:** Cauchy convergence requires `r < -0.4` and `R^2 >= 0.85` (a
  looser bar than #957's `r > 0.5` / `R^2 >= 0.95` because the ensemble
  variance dominates the increment at small `N`; the CLT prediction is
  `r = -1/2`).
...
## Diagnosis (Structural Reason for the Null)
...
## Comparison with the NN Harness (PR #957)

|                          | NN harness (PR #957)      | alt-connectivity (this note) |
| ---                      | ---                       | ---                          |
...
| Cauchy decay rate `r`    | +1.5130                   | +0.1775 (all) / +0.0585 (basin) |
| R^2                      | 0.9939                    | 0.0318 / 0.0032              |
| verdict                  | T_inf EXISTS               | sharp bounded null           |
| identification           | geodesic operator (PR #968) | none — non-convergent       |
...
This is the **structural reason for the alt-connectivity family's bounded
scope** in the 19-row sub-lane mapping (PR #996): the family is not on a
harness with a clean continuum bridge, so the C-class identification was
never available for these rows.

## What This Does Close

- A sharp bounded null-result on the **largest C-class subgroup** in the
  19-row sub-lane mapping: the alt-connectivity grown DAG family does not
  admit operator-Cauchy continuum-bridge identification.
- Structural reason for the bounded scope, with quantitative evidence:
...
- This does not rule out **some** continuum-bridge method working on the
  alt-connectivity family — only that the operator-Cauchy approach of
  PR #957 (averaged or per-component) does not. A method that explicitly
  models the heavy-tailed seed distribution (e.g. median-of-seeds, or
  on-basin orientation-aligned averaging with explicit sign-correction)
  might recover ensemble-Cauchy but would require a separate registered
  derivation.
- The cousin alt-connectivity tables (sixth-family sheared, moving-source
  ...
  drift/seed structure but with different connectivity rules. The
  structural diagnosis (no h-like axis; seed-selective basin) applies to
  them by inheritance.

## Conclusion

**The operator-Cauchy continuum-bridge method does not extend to the
alt-connectivity grown DAG family.** This is a sharp bounded null-result:
the harness lacks a clean refinement parameter, and the ensemble-refinement
substitute fails because the seed distribution is heavy-tailed with
sign-orientation flips. The structural reason for the alt-connectivity
cluster's bounded scope in the 19-row sub-lane mapping is now
mechanistically understood: this harness lives in a different metric world
than the rescaled NN lattice.
