...
| joint Feynman-Hellmann / scalar-LSZ chunked production manifest | L12 can be chunked into 63 production-targeted 16-measurement chunks estimated at `11.3186` hours each, but this is launch planning only and does not cover L16/L24 |
| joint Feynman-Hellmann / scalar-LSZ chunk combiner gate | requires all 63 L12 chunks to be production phase with run-control provenance and same-source FH/LSZ observables; zero chunks are present, so it is an acceptance gate only |
