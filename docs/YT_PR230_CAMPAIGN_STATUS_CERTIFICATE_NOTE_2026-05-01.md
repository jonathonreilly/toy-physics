...
| joint Feynman-Hellmann / scalar-LSZ chunk combiner gate | requires all 63 L12 chunks to be production phase with run-control provenance, unique chunk artifact dirs, and same-source FH/LSZ observables; one chunk is present/ready, so this remains an acceptance gate only |
