...
**Date:** 2026-05-10
**Claim type:** positive_identification (translation-invariant Gaussian-magnitude
quadratic-phase kernel on this scoped harness)
...
- **PR #997** ([`scripts/lattice_nn_rescaled_kernel_identification.py`](../scripts/lattice_nn_rescaled_kernel_identification.py))
...
- **PR #968** ([`scripts/lattice_nn_rescaled_continuum_identification.py`](../scripts/lattice_nn_rescaled_continuum_identification.py))
...
- **PR #1007** ([`scripts/lattice_nn_rescaled_c2_derivation.py`](../scripts/lattice_nn_rescaled_c2_derivation.py))
...
The load-bearing physics step is connecting the slit-anchored per-arm
width `σ_arm(h) = C_arm · √h` (PR #968) to the no-slit kernel width
`σ_amp(h) = C_amp · √h` (this runner). The saddle-point analysis
(`scripts/lattice_nn_rescaled_C_arm_derivation.py`) shows the lateral
variance of the field-free Gaussian kernel scales as `σ²(L; h) = K_amp · L · h`
with `K_amp = C_amp² / L_total` independent of `L`. So the per-arm
...
**Positive identification, source-note status.**
...
This is a bounded source-note identification, not a retained-status
...
The slit-anchored cross-check is the load-bearing connection: the
