...
of the parent CKM magnitudes structural-counts row.
...
- Does **not** derive `lambda^2 = alpha_s / n_pair`, `A^2 = n_pair / n_color`,
  ...
  / CP-phase / Thales authorities carried by separate rows, specifically
...
- Does **not** supply or claim any value of `alpha_s`. `alpha_s` is treated
  as an abstract positive symbol; no `alpha_s_derived_note`
...
The parent CKM magnitudes structural-counts row bundles
...
comparator appears. The algebraic substitution can be reviewed independently
...
- `ckm_magnitudes_structural_counts_theorem_note_2026-04-25` — parent
  bundled note that supplies the parametric input identities, `alpha_s` value,
  and PDG comparators.
- `thales_right_angle_narrow_theorem_note_2026-05-02` — sister
