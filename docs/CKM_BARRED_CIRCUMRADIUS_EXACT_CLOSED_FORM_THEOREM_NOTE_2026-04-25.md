...
**EXACT all-orders-in-α_s closed form** for the **circumradius** `R̄`
of the barred unitarity triangle on the retained NLO Wolfenstein
protected-γ̄ surface, plus the **circumcenter coordinates** in the
`(ρ̄, η̄)` plane.
...
The x-coordinate is **α_s-independent** at `1/2` (midpoint of the
hypotenuse, as forced by the retained right-angle hypotenuse-as-
diameter geometry). The y-coordinate is **EXACTLY linear** in `α_s`
with structural coefficient `-1/(N_pair³ √(N_quark - 1))`.
...
(C7)  Geometric chord-distance interpretation:
      ...
      The signed perpendicular distance from circumcenter to the
      hypotenuse V_1 V_2 equals y_cc; the unsigned version
      |y_cc| = R_bar |cos(alpha_bar)| is the chord-distance theorem.

(C8)  Inscribed-angle theorem corollary:

      central angle subtended by hypotenuse V_1 V_2 = 2 alpha_bar
...
- Perpendicular bisector of `V_1 V_2` (the hypotenuse on the real
...
circumcenter to a chord (here `V_1 V_2`, the hypotenuse) equals
...
below the hypotenuse for `α̅ > π/2`).
...
| `2R̄ sin(α̅)` (chord) | `1` (hypotenuse length) | `1.000000000000` |
...
This note delivers EXACT closed forms for all of the above:
...
- `x_cc = 1/N_pair = 1/2` — frozen at the atlas-LO value (midpoint
  of hypotenuse), α_s-INDEPENDENT to all orders on this surface.
...
turns on, with constant linear slope. No quadratic, cubic, or
higher corrections to either coordinate.
...
yields a non-zero `α_s¹` or `α_s³` term in `R̄`, then either:

- the protected-γ̄ closed forms `(N4)`, `(N5)`, `(N6)` themselves
  break at NNLO, or
- the law of sines fails (which it does not in flat space).

This makes the EVEN-only selection rule a **sharp** test of the
protected-γ̄ surface's all-orders structure.
...
the circumcenter to the hypotenuse equals `R̄ |cos(α̅)|`. Combined
with `2R̄ sin(α̅) = 1` (chord length = hypotenuse length), this
encodes the law of sines in geometric language.

The circumcenter's depression below the hypotenuse is therefore
...
satisfy `R̄² - 1/4 = α_s²/320` to all orders. The deviation is
`3.3 × 10⁻⁵` at canonical `α_s` — currently below experimental
precision, but a clean theoretical target for NNLO calculations.
...
The selection rule `(C6)` rules out:

- Any NNLO Wolfenstein modification to `R̄` containing `α_s¹`.
- Any modification to `R̄²` with terms higher than `α_s²` (the
  EXACT closed form `R̄² = 1/4 + α_s²/320` has no `α_s³` or above).

If experiment + theory together find an `α_s¹` correction to `R̄`
that survives careful NNLO Wolfenstein analysis, the protected-γ̄
surface's `(N4)`/`(N5)` retained closed forms must be revised. This
is a sharp test that future precision work can perform.
...
3. **EVEN-only selection rule.** A parity-like statement that all
   odd-order corrections to `R̄` are EXACTLY zero on the protected-γ̄
   surface. This is a sharp falsifiable prediction.

These propositions about the all-orders structure of the
protected-γ̄ surface were not visible from `(N1)`-`(N9)` alone.
