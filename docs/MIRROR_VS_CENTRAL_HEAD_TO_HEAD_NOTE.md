...
not a re-derivation of either lane's retained content. The underlying
retained rows are owned by their respective notes (mirror chokepoint via
[`MIRROR_CHOKEPOINT_NOTE.md`](MIRROR_CHOKEPOINT_NOTE.md) plus its
registered certificate runner; the dense central-band retained pocket via
...
retained `N=15`/`N=25` rows against the strict joint cache). The
