# Probe V-BAE-S3-Reflection -- S_3 = C_3 ⋊ Z_2 Representation Theory Does NOT Force BAE: Bounded-Tier Source Note (Negative)

**Date:** 2026-05-08
**Type:** bounded_theorem (negative -- structural rejection extends 3 -> 4 levels; no positive closure; no new admission)
...
symmetry `S_3 = C_3 ⋊ Z_2` (where the `Z_2` is the retained residual
reflection `P_{23}` per `CIRCULANT_PARITY_CP_TENSOR` retained
positive theorem, equivalently the `K = T` factor of CPT per
`CPT_EXACT_NOTE`) structurally forces the Brannen Amplitude
Equipartition (BAE) condition `|b|²/a² = 1/2` on `C_3`-equivariant
Hermitian circulants `H = a I + b C + b̄ C²` on `hw=1`.
**Status:** source-note proposal for a NEGATIVE bounded obstruction
...
> **Within retained content, the (1,2) real-dim weighting on
...
Probes 25, 27, 28 collectively verified this is robust under free
Gaussian dynamics (Probe 25), all `hw=N` sectors (Probe 27), and
full retained interactions (Probe 28).
...
### Premises (A_min for Probe V-BAE-S3-Reflection)
...
| CircParity | P_{23}-Z_2 acts on Hermitian-circulant family decomposing it into (d, c_even) trivial and c_odd sign | retained: [`CIRCULANT_PARITY_CP_TENSOR_NARROW_THEOREM_NOTE_2026-05-02.md`](CIRCULANT_PARITY_CP_TENSOR_NARROW_THEOREM_NOTE_2026-05-02.md) T1, T2 |
| CPTexact | `T = K` (entry-wise complex conjugation); CPT antiunitary, `(CPT)² = id` | retained per [`CPT_EXACT_NOTE.md`](CPT_EXACT_NOTE.md), [`CPT_SQUARED_IS_IDENTITY_THEOREM_NOTE_2026-05-02.md`](CPT_SQUARED_IS_IDENTITY_THEOREM_NOTE_2026-05-02.md) |
| Probe7 | Five retained Z_2 candidates fail to force BAE; `S_3 = Z_3 ⋊ Z_2` semidirect (NOT direct `Z_6 = Z_2 × C_3`) | source dependency; see [`KOIDE_A1_PROBE_Z2_C3_PAIRING_BOUNDED_OBSTRUCTION_NOTE_2026-05-08_probe7.md`](KOIDE_A1_PROBE_Z2_C3_PAIRING_BOUNDED_OBSTRUCTION_NOTE_2026-05-08_probe7.md) |
...
- NO new axioms or imports -- `S_3` is constructed entirely from
  RETAINED `C_3` (per `KOIDE_CIRCULANT_CHARACTER` + BZ-corner) and
  RETAINED `Z_2 = ⟨P_{23}⟩` (per `CIRCULANT_PARITY_CP_TENSOR`
  positive theorem)

## The structural argument

### S_3 retention (Section 11 of runner)

`S_3 = C_3 ⋊ Z_2` is built entirely from retained pieces:

1. `C_3 = ⟨C⟩` (cyclic shift): RETAINED per
...
2. `Z_2 = ⟨P_{23}⟩` (residual transposition): RETAINED per
...
(Per `CIRCULANT_PARITY_CP_TENSOR` T2 retained.)
...
## Theorem (Probe V-BAE-S3-Reflection bounded obstruction)

**Theorem.** On A1 + A2 + retained `C_3`-equivariance + retained
`P_{23}` (CIRCULANT_PARITY_CP_TENSOR positive theorem) + retained
CPT (`T = K`) + retained Block-Total Frobenius isotype decomposition
+ retained MRU + retained Probes 7, 12, 13, 18, 25, 27, 28:

```
(a) The natural symmetry of the Z³ × C_3 substrate is S_3 = C_3 ⋊ Z_2,
    where Z_2 = ⟨P_{23}⟩ is RETAINED via CIRCULANT_PARITY_CP_TENSOR T1
    + T2 + the CPT_EXACT_NOTE T-factor K. The defining relation
...
Section 1 (retained sanity); Section 2 (S_3 semidirect structure);
...
Section 11 (Z_2 retention status); Section 12 (verdict synthesis). ∎
...
- `claim_type`: `bounded_theorem` (NEGATIVE -- structural rejection
...
- `admitted_context_inputs`: `["BAE-condition: |b|²/a² = 1/2 (legacy:
  ...
  (where Z_2 is retained via CIRCULANT_PARITY_CP_TENSOR + CPT_EXACT_NOTE),
...
2. Verifies that `Z_2 = ⟨P_{23}⟩` is RETAINED in the framework
   (via CIRCULANT_PARITY_CP_TENSOR positive theorem +
   CPT_EXACT_NOTE T-factor K).
...
1. **`S_3 = C_3 ⋊ Z_2` is RETAINED**, built from `C_3` (cyclic
   shift, retained per BZ-corner forcing + circulant character)
   and `Z_2 = ⟨P_{23}⟩` (residual transposition, retained per
   CIRCULANT_PARITY_CP_TENSOR positive theorem and equivalently
   the `K = T` factor of CPT_EXACT_NOTE).
...
3. **Closes a discrete-group symmetry-extension lane**: `S_3` is
   the maximal natural symmetry of the `Z³ × C_3` substrate (any
   larger group would require non-retained primitives like `O(2)`
   continuous extension). The discrete-group extension lane is
   exhausted within retained content.
...
> log-isotype-functional class. Retained `C_3` representation
...
explicit as exhausted within retained content.
...
### Retained Z_2 / reflection structure (load-bearing for the S_3 construction)

- Circulant parity (Z_2 = ⟨P_{23}⟩, T1, T2 retained): [`CIRCULANT_PARITY_CP_TENSOR_NARROW_THEOREM_NOTE_2026-05-02.md`](CIRCULANT_PARITY_CP_TENSOR_NARROW_THEOREM_NOTE_2026-05-02.md)
...
- Probe 14 (retained-U(1) hunt): [`KOIDE_A1_PROBE_REAL_STRUCTURE_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe13.md`](KOIDE_A1_PROBE_REAL_STRUCTURE_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe13.md)
...
- Probe 28 (full retained interactions): [`KOIDE_BAE_PROBE_INTERACTING_DYNAMICS_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe28.md`](KOIDE_BAE_PROBE_INTERACTING_DYNAMICS_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe28.md)
...
1. **Section 1** -- Retained input sanity: C unitary, order 3,
...
4. **Section 4** -- P_{23} action on Herm_circ basis: (a, u, v) →
   (a, u, -v) (equivalent to CIRCULANT_PARITY T2 retained).
...
11. **Section 11** -- Z_2 retention status: 4 retention sources
    confirmed (CIRCULANT_PARITY, Probe 7, CPT_EXACT, Probe 13).
...
- `feedback_retained_tier_purity_and_package_wiring.md`: no
  ...
  (negative) obstruction; the parent BAE admission remains at its
...
- `feedback_compute_speed_not_human_timelines.md`: alternative
  ...
  characterized as exhausted within retained content; further
  extensions would require non-retained primitives like O(2)
...
- `feedback_primitives_means_derivations.md`: S_3 is constructed
  entirely from RETAINED primitives (C_3 + Z_2 = ⟨P_{23}⟩); no
  new axioms admitted.
...
- [circulant_parity_cp_tensor_narrow_theorem_note_2026-05-02](CIRCULANT_PARITY_CP_TENSOR_NARROW_THEOREM_NOTE_2026-05-02.md) (Z_2 retained)
- [cpt_exact_note](CPT_EXACT_NOTE.md) (T = K retained)
- [cpt_squared_is_identity_theorem_note_2026-05-02](CPT_SQUARED_IS_IDENTITY_THEOREM_NOTE_2026-05-02.md) ((CPT)² = id)
- [koide_circulant_character_derivation_note_2026-04-18](KOIDE_CIRCULANT_CHARACTER_DERIVATION_NOTE_2026-04-18.md) (C_3 retained)
