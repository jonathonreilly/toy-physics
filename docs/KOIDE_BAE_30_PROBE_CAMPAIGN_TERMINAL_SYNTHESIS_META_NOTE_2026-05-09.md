...
two positive new derivations, multiple structural impossibility theorems,
and the precisely-localized partial-falsification finding (Probe 29:
framework's retained content predicts κ=1 for charged-lepton spectral
relations, vs empirical κ=2 = BAE).
**Companion to:**
...
- User constraint (2026-05-09): no new axioms, no external imports;
  derivations from existing A1+A2+retained content only
...
| 25 | Probe 25 | Physical-extremization / dynamics | #820 | sharpened: F1 STRUCTURALLY rejected by retained dynamics |
...
Exponent decomposition `18 = 16 (taste doublers, retained) + 2 (Yukawa-vertex factor)`. Five equivalent algebraic forms all give the same numerical value. Same retained-tier precision as `v_EW` (0.03%) and `m_t` (0.07%). Genuine new retained-grade derivation.
...
Where `n_eff = 2` (C_3 conjugate-pair forcing: doublet winding number) and `d² = 9` (`dim_R(Herm_3)`). This is retained character-algebra content.
...
Probes 25 + 27 + 28 collectively establish:

> **The (1,2) real-dim weighting on `Herm_circ(3)` is C_3 representation-theoretic (1 trivial real-dim + 2 doublet real-dims since `b ∈ ℂ`). It is robust under**:
...
The (1,1) multiplicity weighting that would give BAE is **structurally absent** from retained dynamics. Closing BAE requires a multiplicity-counting principle distinct from C_3 rep theory on `Herm_circ(3)` — a principle not retained.

## The partial-falsification finding (Probe 29)

The framework's combined retained content gives a **unique canonical κ-predictor functional**:
...
Nine attack vectors (Probes 12, 19, 22, 24, 25, 26 + AV6/AV7/AV8/AV9 in Probe 29) converge: the framework's retained content as a κ-predictor gives κ = 1.

This is a precisely-localized **partial falsification** of the framework's charged-lepton SPECTRAL-RELATION prediction. It does NOT falsify:
...
It DOES falsify the framework's prediction of charged-lepton Koide ratio `Q = 2/3` from C_3-circulant matter-sector dynamics.

## Three structural impossibility theorems (the campaign's deepest results)
...
3. **Probe 25 + 27 + 28**: F1 (multiplicity (1,1)) is structurally absent from retained dynamics across free + interacting + all hw=N sectors
...
The audit lane has authority over whether the partial-falsification finding stands, whether closure is required at the lepton-spectral-relation level, and which strategic path the framework pursues.
...
3. The 2 positive derivations are recorded (m_τ Wilson chain, φ_dimensionless=2/9).
4. The structural impossibility theorems are recorded (Probes 14, 17, 25+27+28).
5. The partial-falsification finding (Probe 29 κ=1 vs empirical κ=2) is recorded.
...
3. The 2 positive new derivations (Probe 19 m_τ scale, Probe 24 Step 1 φ_dimensionless) are independent retained-grade derivations awaiting their own audit-lane review.
