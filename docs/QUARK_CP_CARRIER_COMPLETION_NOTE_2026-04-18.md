...
This is therefore an `audited_numerical_match` (Class G) result in the
...
[`QUARK_CP_CARRIER_COMPLETION_AUDITED_SCOPE_NARROW_BOUNDED_NOTE_2026-05-10.md`](QUARK_CP_CARRIER_COMPLETION_AUDITED_SCOPE_NARROW_BOUNDED_NOTE_2026-05-10.md).
...
that note continues to live as a separate bounded surface and is **not**
load-bearing for the present note.
...
To upgrade this row from `audited_numerical_match` to a retained
...
Class-G load-bearing step. The auditor verdict was:
...
load-bearing structural gaps that any future upgrade to `audited_clean`
would have to close. No runner, audit-data, or publication file is
changed by the demotion; the bounded numerical-match status is preserved,
and the underlying runner output (`PASS=11, FAIL=0`) is unchanged.
