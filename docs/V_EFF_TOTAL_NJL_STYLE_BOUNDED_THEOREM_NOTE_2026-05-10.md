...
**no interior minimum** by itself. Standard composite-Higgs analyses
(NJL, Bardeen–Hill–Lindner top-condensation, walking technicolor) ALL
include a tree-level σ²/(2 G_eff) term that, together with the fermion-
loop log-determinant, yields a stable broken-phase saddle when the
four-fermion coupling exceeds a critical value. Without that tree term
the framework's V_taste can never give an EWSB minimum; with it, the
question of whether the framework's lattice action is above or below
the NJL critical coupling becomes a precise, decidable algebraic
statement.
...
sector does not by itself drive chiral SSB, and EWSB requires the
SU(2)_L × U(1)_Y electroweak gauge structure (which is not part of
the lattice gauge action this note examines). This is a precise
bounded statement consistent with the framework's existing
admitted-context inputs and does NOT close any gap. It records the
...
> ```
>
> on the canonical Cl(3)/Z³ Wilson-plaquette + staggered-Dirac
...
**Five of six plausible forms put the framework in the SYMMETRIC
phase**, with G_eff_LO / G_critical ≈ 0.5–0.9. Two forms (with
