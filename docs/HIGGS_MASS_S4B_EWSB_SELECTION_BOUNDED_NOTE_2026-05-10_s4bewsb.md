...
framework's RETAINED EWSB content (Wilson chain `v_EW = 246.28 GeV`,
...
> Use the framework's RETAINED EWSB content to SELECT the post-EWSB
...
- **(B1.1) Retained `v_EW` selects the evaluation-point location.**
  ...
  α_LM¹⁶ = 246.28 GeV` as a derivation from the framework axioms plus
  the single computed plaquette `<P> = 0.5934`. This numerical value
...
as a derivation from the framework axioms (Cl(3), Z³) plus the single
computed plaquette and the staircase taste threshold structure. The
...
246.28 GeV` as a derivation from framework axioms + computed plaquette
...
- `feedback_bridge_gap_fragmentation_2026_05_07.md`: the parent
  ...
  and S4b-op (open). No new framework axioms; admissions named
