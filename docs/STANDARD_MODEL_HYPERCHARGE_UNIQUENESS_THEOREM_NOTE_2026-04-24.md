...
**Date:** 2026-04-24
**Status:** **retained standalone structural-uniqueness theorem** on `main`. Extracts and packages as its own theorem the uniqueness claim that is *used but not separately retained* in [`ANOMALY_FORCES_TIME_THEOREM.md`](ANOMALY_FORCES_TIME_THEOREM.md) Step 2 and implicitly carried by [`ONE_GENERATION_MATTER_CLOSURE_NOTE.md`](ONE_GENERATION_MATTER_CLOSURE_NOTE.md). The retained anomaly-forces-time theorem explicitly states: *"The time theorem only needs the existence of an SU(2)-singlet opposite-chirality completion; it does not rely on uniqueness from anomaly arithmetic alone."* This note closes the uniqueness statement as a standalone retained row.
...
3. vanishing of the three non-trivial gauge anomaly traces
   `Tr[Y] = 0`, `Tr[Y³] = 0`, `Tr[SU(3)² Y] = 0`
   for the full (LH + RH) content;
...
| electric-charge convention `Q = T_3 + Y/2` (or equivalent) | standard SM bookkeeping |
...
The electric-charge formula `Q = Y` on SU(2) singlets selects the `up-type` label as the component with positive electric charge. Identifying `u_R` as up-type requires `Q(u_R) = y_1 > 0`, fixing `y_1 = +4/3` and `y_2 = −2/3`. ∎
...
with denominators exactly `{1, 3}`, i.e. the framework **forces** baryon-number-dependent fractional quantization `1/3` and the unit lepton charge `±1`. No continuous family, no additional branch survives.
...
- A proton decay whose rate is incompatible with baryon-number conservation at the level implied by the SM hypercharges (e.g. `p → e⁺ π⁰`) would test a longer chain including this theorem.
...
1. Sets up the anomaly system (E1, E2, E3) symbolically in sympy.
