...
identification `Y(nu_R)=0`, and the electric-charge labelling `Q(u_R)>0`,
the right-handed hypercharges are uniquely fixed at
`(y_1, y_2, y_3, y_4) = (+4/3, -2/3, -2, 0)` and the one-generation
electric-charge spectrum is `{0, +/-1/3, +/-2/3, +/-1}` with denominators
exactly `{1, 3}`. This note packages that uniqueness step as a standalone
row; it does not re-derive the upstream left-handed surface, the
singlet-RH completion requirement, the neutral-singlet input, the gauge
group, or the Cl(3) chain that supplies the SM gauge structure.
**Status authority:** independent audit lane only. This source note does
not set or predict an audit outcome; later status is generated by the
audit pipeline after independent review. Author headers in this file are
scope declarations, not audit verdicts. Earlier `proposed_retained` /
"retained structural uniqueness theorem" framing is interpreted as the
author's role-statement for this row, not an audit-grade label.
...
| Row | Author-stated role before | Author-stated role after |
|-----|---------------------------|--------------------------|
| LH content `Q_L + L_L` | upstream cited authority ([`LEFT_HANDED_CHARGE_MATCHING_NOTE.md`](LEFT_HANDED_CHARGE_MATCHING_NOTE.md)) | unchanged |
| anomaly-forced `3+1` | upstream cited authority ([`ANOMALY_FORCES_TIME_THEOREM.md`](ANOMALY_FORCES_TIME_THEOREM.md)) | unchanged |
| existence of anomaly-cancelling RH completion | upstream cited authority (ditto, Step 2) | unchanged |
| one-generation closure at SM hypercharges | upstream cited authority ([`ONE_GENERATION_MATTER_CLOSURE_NOTE.md`](ONE_GENERATION_MATTER_CLOSURE_NOTE.md)) | unchanged |
| **RH hypercharges uniqueness at SM values** | *(not separately packaged; used but not separately stated as standalone)* | **standalone structural-uniqueness theorem packaged here** |
| electric-charge quantization `Q ∈ {0, ±1/3, ±2/3, ±1}` | implicit | **standalone structural corollary** |

The uniqueness claim was previously a load-bearing but unlabelled step inside the time theorem and the one-generation closure note. This note packages it as a named row so that:

1. The electric-charge quantization statement has an explicit standalone source.
...
Audit-grade status (`retained` / `audited_conditional` / `audited_failed`)
is set independently by the audit pipeline; this note's table records only
the author-stated standalone role, not an audit verdict.
