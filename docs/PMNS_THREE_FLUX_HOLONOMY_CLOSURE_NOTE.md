...
on, in response to the 2026-05-10 audit verdict's
...
- [`PMNS_TWISTED_FLUX_TRANSFER_HOLONOMY_BOUNDARY_NOTE.md`](PMNS_TWISTED_FLUX_TRANSFER_HOLONOMY_BOUNDARY_NOTE.md)
  ...
  `frontier_pmns_twisted_flux_transfer_holonomy_boundary`. Because
  this sibling is `unaudited`, it does not by itself lift the present
  note's effective status under the cite-chain rule. Closing this
  one-hop authority audits the one-angle holonomy law named in the
  verdict's `notes_for_re_audit_if_any` field as the missing
  dependency edge.
- [`PMNS_C3_CHARACTER_HOLONOMY_CLOSURE_NOTE.md`](PMNS_C3_CHARACTER_HOLONOMY_CLOSURE_NOTE.md)
  ...
  three-flux family. Because this sibling is `unaudited`, it does not
  by itself lift the present note's effective status under the
  cite-chain rule.
- [`PMNS_ORIENTED_CYCLE_CHANNEL_VALUE_LAW_NOTE.md`](PMNS_ORIENTED_CYCLE_CHANNEL_VALUE_LAW_NOTE.md)
  — currently `retained` (audit row:
  `pmns_oriented_cycle_channel_value_law_note`). Sibling retained
  ...
  `(u, v, w)` lives in. Under the cite-chain rule a `retained` one-hop
  authority on the oriented-cycle channel value law supports but does
  not by itself promote the present row, since the holonomy linear
  functional itself remains a class B construction-on-data import.
- [`PMNS_GRAPH_FIRST_CYCLE_FRAME_SUPPORT_NOTE.md`](PMNS_GRAPH_FIRST_CYCLE_FRAME_SUPPORT_NOTE.md)
  ...
  bounded-conditional authority on the graph-first cycle frame
  carrying the reduced family `A_fwd(u,v,w)` substrate.

Open class D registration targets named by the 2026-05-10 audit
...
- A retained-grade source-note authority establishing
  ...
  bounded-grade three-flux closure to chain closure. The verdict's
  `notes_for_re_audit_if_any` field states this explicitly:
...
matrix-inversion step closes algebraically once the one-angle
holonomy law is granted, but that the law is imported from an uncited
runner module (`frontier_pmns_twisted_flux_transfer_holonomy_boundary`)
and no cited authority is provided for the definitions of the reduced
cycle family, flux holonomy, or the claimed coefficient formula. The
runner `scripts/frontier_pmns_three_flux_holonomy_closure.py` is
...
`C_3`-character holonomy closure sibling, the retained oriented-cycle
channel value law authority, and the bounded-conditional cycle-frame
support, and explicitly registers the missing-dependency-edge target
named by the verdict's `notes_for_re_audit_if_any` field. Effective
...
missing-dependency-edge target named by the verdict's
