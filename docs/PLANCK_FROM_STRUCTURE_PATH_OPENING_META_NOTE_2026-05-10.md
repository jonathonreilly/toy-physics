...
Documents the four open-PR source-note proposals (P1, P2, P3, P4)
that target the Planck-from-structure missing-theorem trio plus the
G_Newton self-consistency lane, identifies what each closes, and
records the conditional path to **zero conventional anchors** under
the framework's existing axiom set.
...
any retained theorem, and does not select among the strategic options
that the path-opening exposes. The audit lane retains full authority
...
- **"framework axiom A1"** = retained `Cl(3)` algebra axiom per
  [`MINIMAL_AXIOMS_2026-05-03.md`](MINIMAL_AXIOMS_2026-05-03.md).
- **"framework axiom A2"** = retained `Z³` spatial substrate axiom per
  [`MINIMAL_AXIOMS_2026-05-03.md`](MINIMAL_AXIOMS_2026-05-03.md).
...
- **"path-opening"** = the synthesis-level observation that the round's
  ...
  axioms + one conventional scale anchor + named admissions" toward
  "two axioms + named admissions only."
...
proposals, each in a separate open PR awaiting independent audit:

| PR | Probe | Subject | Proposed claim type | Verdict per PR body |
|----|-------|---------|---------------------|--------------------|
| [#877](https://github.com/jonathonreilly/cl3-lattice-framework/pull/877) | P1 | Substrate-to-carrier forcing — RP route | `bounded_theorem` | bounded closure; new audit-clean candidate route via retained reflection-positivity OS Cauchy-Schwarz; selects `P_A` uniquely from 17 rank-four projector classes via "vacuum-reachable degree-1 sector dim = rank" criterion; Hodge degeneracy `P_1 ↔ P_3` is broken by RP's degree-1 restriction |
| [#876](https://github.com/jonathonreilly/cl3-lattice-framework/pull/876) | P2 | Hidden-character `δ = 0` via source-free state | `positive_theorem` | closure proposed; source-free state `ρ_cell = I_16/16` collapses every Schur trace to `Tr(O)/16`; identity-multiple `Q = (Tr(Q)/16) I` is a normalization redefinition of `c_cell`, not affine freedom; traceless `Q` integrates to zero against the maximally-mixed state |
| [#874](https://github.com/jonathonreilly/cl3-lattice-framework/pull/874) | P3 | Orientation principle — retained 3+1 single-clock time-asymmetry | `bounded_theorem` | bounded closure; action-level `Z_2` grading `Θ_RP = (-1)^{n_t}` anticommutes with Hodge star; asymmetric eigenspace dimensions `dim(P_1 ∩ E_-) = 1` (just `e^t`) vs `dim(P_3 ∩ E_-) = 3` break the Hodge degeneracy; `P_1` uniquely selected as the carrier for the time-direction identification |
| [#875](https://github.com/jonathonreilly/cl3-lattice-framework/pull/875) | P4 | G_Newton self-consistency bounded sharpening | bounded sharpening, **not full closure** | sharpened with a positive dimensional-rigidity sub-theorem (`G ~ ℏc / M_Pl²` is the unique integer-exponent combination over `{a_s, c_LR, M_lat, ℏ}` carrying SI units of Newton's constant) plus three explicitly named admissions: (B(a)) skeleton-selection, (B(b)) Born-as-source, (B(c)) valley-linear weak-field action |

All four source notes adhere to the existing audit-honest rules:

- No new framework axioms (A1+A2 unchanged).
...
1. **The missing-theorem trio is no longer a wholesale gap.** Each of
   ...
   in flight (P1, P2, P3) with explicit retained-content load-bearing
...
2. **The G_Newton self-consistency lane has explicit residual
   ...
   `G_Newton` from retained content; it sharpens what was previously a
...
3. **The path to zero conventional anchors is conditional.** Under
   audit ratification of P1+P2+P3 plus closure of the substep-4
...
Under the framework's existing two-axiom surface
(`MINIMAL_AXIOMS_2026-05-03.md` A1 + A2), import categories on the
prior surface were:

| Category | Pre-round count | Description |
|---|---|---|
| Framework axioms | 2 | A1 (`Cl(3)`) + A2 (`Z³`) |
...
**Under audit ratification of P1+P2+P3** and **conditional on closure
...
|---|---|---|
| Framework axioms | 2 | unchanged (A1 + A2) |
...
Each of the four PRs (#874, #875, #876, #877) is a source-note
proposal awaiting independent audit verdict. Until each receives an
audit-clean verdict, the synthesis-level "path-opening" remains
...
- **B(a) Skeleton-selection.** Multiple retained propagator skeletons
  exist (Hamiltonian, d'Alembertian, complex-action); no retained
  skeleton-selection theorem is in the audit ledger.
- **B(b) Born-as-source.** Born map is target-side; pure-state and
  density-matrix-trace readings diverge on mixed states; no retained
...
- **B(c) Valley-linear weak-field action.** Valley-linear vs
  ...
  empirical `F~M=1` match, not by retained derivation.

Each admission has a separate companion-probe path identified
(referred to in the task framing as Probes G1, G2, G3); those probes'
results may not be on main yet at synthesis time.
...
1. Produced four source-note proposals (PRs #874, #875, #876, #877)
   ...
   bounded or positive sub-theorem on retained-content surfaces.
2. Decomposed the previously-bundled "missing-theorem trio" into named
   bounded/positive sub-theorems with explicit retained-content
   load-bearing inputs.
...
3. **Add a new framework axiom.** A1+A2 are unchanged.
4. **Modify any retained theorem or audit row.** Retained content,
...
2. **Attack G_Newton's three admissions in parallel.** The B(a) /
   ...
   (referred to as G1, G2, G3 in the task framing) can be dispatched
   without serial dependence.
...
6. The framework's two-axiom surface (A1+A2) is unchanged.
7. No PDG values are loaded as derivation input.
8. No new axiom is added.
...
1. The synthesis-level "path-opening" framing remains in force only
   while all four open PRs (#874, #875, #876, #877) remain candidate
   surfaces; if any of P1+P2+P3 is rejected by audit, the
