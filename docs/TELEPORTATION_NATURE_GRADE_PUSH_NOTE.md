...
evidence, not as derivations from retained axioms; the noisy pulse
...
one. The audit ledger row for `teleportation_nature_grade_push_note`
classified this source as `audited_conditional` / `open_gate` with
auditor's repair target:

> other: Derive the selection principles from retained axioms or
...
deriving the selection principles from retained axioms, promoting them
