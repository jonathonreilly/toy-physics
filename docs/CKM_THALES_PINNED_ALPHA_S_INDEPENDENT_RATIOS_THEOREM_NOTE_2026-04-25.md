...
forward** by deriving a structural classification of the framework's
...
Each identity matches its PDG comparator at `< 0.2 sigma`:
...
The cross-row identity `(R3)` is a NEW pure-framework prediction: it
combines two independently-measured CKM products into a sharp test of
the framework's **specific Thales point**, with no canonical-coupling
ambiguity. It is the **only known** cross-row CKM identity that
probes pure atlas geometry; standard SM phenomenology has no
analogous α_s-independent cross-row test.
...
All five identities match PDG measurements without any CKM-fit
adjustment. The largest deviation is in `R_b^2` itself (1.7σ), which
is the known atlas-LO ↔ standard-fit `|V_ub|` tension; the cross-row
ratio identity `(R3)` is sharper because the systematic biases in
`|V_ub|` partially cancel.
