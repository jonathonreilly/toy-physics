...
upgrades the cross-reference to theorem-grade — a table of which note
holds which piece of the gap-chain at which current claim tier — without
attempting any new derivation. Each row is a pointer; this note neither
extends nor retires its sister.

| Gap correction | Sister authority | Current effective_status | Closes the gap from / to | Open content |
|---|---|---|---|---|
| 2-loop CW + RGE running | [`HIGGS_MASS_DERIVED_NOTE.md`](HIGGS_MASS_DERIVED_NOTE.md) (`bounded_theorem`, td=309) + `scripts/frontier_higgs_mass_corrected_yt.py` (corrected-y_t RGE) | `audited_conditional` (2026-05-07) | tree-level → ~119.93 GeV via corrected-y_t at 3L+NNLO | conditional on `y_t` Ward + RGE-transport scaffolding |
| Lattice spacing convergence (`m_H/m_W` flow as `a → 0`) | [`HIGGS_FROM_LATTICE_NOTE.md`](HIGGS_FROM_LATTICE_NOTE.md) (`bounded_theorem`, td=310) | `retained_bounded` (2026-05-07) | `m_H/m_W = 1.85` at `a=1` → 1.64 at `a=0.5` → 1.558 SM in continuum | continuum-limit theorem grade |
...
### What this Step 7 retires

Nothing. The +12% gap remains an open chain across three sister notes
(two of which are `bounded_theorem`/audited 2026-05-07 and one of which
has no dedicated authority yet). This note continues to claim only the
tree-level formula `m_H_tree = v/(2 u_0) = 140.3 GeV`, with the gap-
closure load explicitly delegated.
...
- The **2026-05-02 status correction audit packet**
  ...
  **same-shape obstruction** with cycle 5 (yt_ew matching M, PR #260)
  and cycle 9 (gauge-scalar observable bridge, PR #268) — i.e., as
  a member of the **lattice → continuum / physical matching cluster**
  identified in [`AUDIT_BACKLOG_CAMPAIGN_PROGRESS_SYNTHESIS_2026-05-02.md`](AUDIT_BACKLOG_CAMPAIGN_PROGRESS_SYNTHESIS_2026-05-02.md)
  §2.3. That cluster requires a Nature-grade non-perturbative matching
  theorem not derivable from minimal premises within standard QFT.
...
- The **Wilson taste-breaking row is the most concrete follow-on
  ...
  the resulting `m_H_tree` correction is the cleanest single-PR addition
  that would bring this note's table to fully-cited closure on
  `bounded_theorem` tier. Out of scope for the current PR, but flagged.
...
- [higgs_mass_from_axiom_status_correction_audit_note_2026-05-02](HIGGS_MASS_FROM_AXIOM_STATUS_CORRECTION_AUDIT_NOTE_2026-05-02.md)
