...
COMBINATION of three retained ingredients —
...
S4b) as a positive theorem. Earlier probe W-S4b-EWSB found that
...
simultaneously.
**Status:** source-note proposal. Verdict is **BOUNDED, NOT CLOSED**.
...
none of the three retained ingredients can close, and combining them
does not add new content beyond the symmetric-point identification
(component (i)) re-evaluated at the retained `v_EW` location
...
**Primary runner:** [`scripts/cl3_koide_x_s4b_combined_2026_05_08_probeX_S4b_combined.py`](../scripts/cl3_koide_x_s4b_combined_2026_05_08_probeX_S4b_combined.py)
**Cache:** [`logs/runner-cache/cl3_koide_x_s4b_combined_2026_05_08_probeX_S4b_combined.txt`](../logs/runner-cache/cl3_koide_x_s4b_combined_2026_05_08_probeX_S4b_combined.txt)
...
> AND a source-coupling rule; combining these three retained
...
single ingredient. The joint hypothesis instantiated on retained
content gives:
...
retained ingredients. The combination is at best a multiplicative
...
three retained pieces — the SM Higgs potential FORM remains the
admitted SM convention, and no combination of admitted/retained
...
- NO new physics inputs beyond cited baseline + retained content + standard QM expectation-value formalism.
...
- **(K1.6) S4b-op remains structurally orthogonal to (i)-(iii).**
  ...
  on the post-EWSB physical Hilbert space. None of the retained
  ...
  retained content does not turn admitted content into derivation.

- **(K1.7) Sister-prediction asymmetry preserved.** The retained
  ...
  (gap-closure functional), not in the retained chain.
...
```
with each component instantiated from retained content:
...
The runner computes this from cited retained inputs (no PDG used as
...
The retained taste-sector formula:
```
...
retained content, or (ii') a derivation of post-EWSB stationary-point
curvature from the lattice partition function (per the cluster
obstruction synthesis), or (iii') a separate retained chain. None
...
obstruction is in S4 ∧ S7, not in the retained chain itself
...
retained `v_EW` and S4b-op open. This note ratifies the
sub-decomposition (component (ii) supplies S4b-loc) and confirms
S4b-op remains open under the combination of all three retained
ingredients (K1.6).
...
1. **Combined-ingredient bounded support.** Combining (i)
   ...
   Numerical verification anchored in retained content only (no PDG
   inputs).
...
4. **S4b-op orthogonal to (i)-(iii).** None of the three retained
...
- It does **NOT** derive the SM Higgs potential FORM from retained
...
| Claim | Falsifier |
|---|---|
| K1.1 combined evaluation = `140.31 GeV` | Demonstrate that combining (i)-(iii) on retained content gives `m_H ≠ 140.31 GeV` (e.g., < 130 or > 150 GeV). Numerically false; runner verifies. |
...
| K1.6 S4b-op orthogonal | Demonstrate that combination (i)-(iii) constructs `Ô_{m_H²} = ∂²V_phys/∂φ²|_{v_EW}` on the post-EWSB physical Hilbert space. Requires deriving SM Higgs potential FORM from retained content (currently admitted SM convention). |
...
proposed_claim_scope: |
  Combined-content stress test of S4b-op closure hypothesis. Tests
  whether the combination of three retained ingredients —
    (i) Higgs structural ratio m_H/v = 1/(2 u_0)
    (ii) EWSB Wilson chain v_EW = 246.28 GeV
    (iii) G2 Born-as-source position-density trace Tr(rho_hat M_hat(x))
  jointly selects the post-EWSB Higgs-mass operator O_{m_H^2} at the
  v_EW location.

  Verdict: combination yields m_H(combined) = v_EW/(2 u_0) = 140.31 GeV,
  identical to the tree-level mean-field shortcut, with relative gap
  +12.03% vs PDG m_H = 125.25 GeV. The combination collapses to the
  symmetric-point identification (component (i) per HIGGS_MASS_FROM_AXIOM
  Step 5(b)) multiplied by v_EW (component (ii)) under unit-factor G2
  trace (component (iii)). No interaction term emerges.

  The +12.03% gap is the load-bearing S7 gap-closure functional Delta^2
  residual (per HIGGS_MASS_12PCT_GAP_DECOMPOSITION). S4b-op (operator
  construction at v_EW) remains structurally orthogonal to (i)-(iii):
  none of the retained ingredients constructs the post-EWSB Higgs-mass
  curvature operator. The SM Higgs potential FORM V = -mu^2 H+H +
  lambda(H+H)^2 remains admitted SM convention per EWSB_PATTERN_FROM_HIGGS_Y
  §5.

  Tier per brief: BOUNDED. S4b-loc bounded-supported (component (ii));
  S4b-op open. Combination ratifies prior W-S4b-EWSB sub-decomposition
...
residual_structural_admissions:
  ...
  - state_and_operator_selection_for_born_trace_mass_readout  # S4b
  - post_ewsb_higgs_mass_squared_operator_construction  # S4b-op (load-bearing)
  ...
  - g_newton_source_coupling_derivation  # gnewtonG2 admits open
...
load_bearing_step_class: bounded_theorem  # combination preserves S4b-loc bounded support; S4b-op remains open
...
```bash
python3 scripts/cl3_koide_x_s4b_combined_2026_05_08_probeX_S4b_combined.py
```
...
Cached: [`logs/runner-cache/cl3_koide_x_s4b_combined_2026_05_08_probeX_S4b_combined.txt`](../logs/runner-cache/cl3_koide_x_s4b_combined_2026_05_08_probeX_S4b_combined.txt)

## 13. User-memory feedback rules respected

- `feedback_consistency_vs_derivation_below_w2.md`: this note does
  NOT assert "consistency = derivation". The combination yielding
  the same prediction as a single ingredient is FLAGGED as the
  symmetric-point identification self-reference (K1.2), not bridged.
- `feedback_hostile_review_semantics.md`: stress-tests the brief's
  joint hypothesis at the operator-construction layer. K1.4
  identifies the G2 trace as a unit factor under canonical
  normalization (semantic stress-test of "source-coupling rule").
- `feedback_retained_tier_purity_and_package_wiring.md`: no
  cross-tier promotion. This note is a bounded-theorem proposal
  testing whether combining three retained pieces closes a
  load-bearing residual. Audit-lane authority preserved.
- `feedback_physics_loop_corollary_churn.md`: this is NOT a
  one-step relabel. The combined-ingredient stress test is
  structurally new content: the prior probes tested individual
  pieces; this probe tests the joint hypothesis and identifies
  the K1.2 collapse-to-symmetric-point structural reason.
- `feedback_compute_speed_not_human_timelines.md`: no time
  estimates. Verdict described in terms of structural blockages
  (K1.1-K1.6) and the +12.03% gap.
- `feedback_special_forces_seven_agent_pattern.md`: this note
  packages the combined-ingredient attack with sharp PASS/FAIL
  deliverables in the runner: K1.1 numerical evaluation, K1.2
  symmetric-point collapse, K1.3 lattice numerical match, K1.4
  G2 unit factor, K1.5 +12.03% gap, K1.6 S4b-op orthogonal, K1.7
  sister asymmetry.
...
- `feedback_bridge_gap_fragmentation_2026_05_07.md`: the parent
  Lane 2 S4b residual is being further attacked at the joint-
  hypothesis layer. No new framework axioms; admissions named
  explicitly. The probe does NOT claim closure; it ratifies the
  prior decomposition without improving on it.
- `feedback_primitives_means_derivations.md`: no new axioms or
  imports. All derivations are from A1+A2+retained content.
...
- Is **NOT** a relabel of W-S4b-EWSB (which tested EWSB content
  alone). This probe tests the joint hypothesis of three retained
  ingredients.
- Is **NOT** a relabel of T-S4H (parent decomposition). This probe
  operates at the combined-ingredient layer, not the operational/
  selection layer.
- Is **NOT** a relabel of HIGGS_MASS_FROM_AXIOM (Step 4 tree-level
  prediction). This probe explicitly tests whether combining
  HIGGS_MASS_FROM_AXIOM's Step 4 ratio with the EWSB Wilson chain
  and the G2 Born trace gives a NEW prediction, and finds it does
  NOT (the K1.2 collapse is the structural reason).
- Provides **structurally new content**: the K1.2 collapse-to-
  symmetric-point self-reference identifies WHY combining the
  three pieces gives the same prediction as one piece — the G2
  trace is a unit factor (K1.4), and the structural ratio (i) IS
  the symmetric-point identification (K1.2). This is a structural
  diagnosis, not a relabel.

## 15. Promotion-Value Gate (V1-V5)

| # | Question | Answer |
|---|---|---|
| V1 | Verdict-identified obstruction closed? | No — S4b-op (post-EWSB operator construction at retained `v_EW` location) remains open and structurally fused with S7 (Δ² gap-closure). The combination does not close S4b-op; it ratifies S4b-loc bounded support without improving on prior results. |
| V2 | New bounded support? | Yes — (i) the joint hypothesis instantiation gives `m_H(combined) = 140.31 GeV` numerically equal to the tree-level shortcut (K1.1, new evaluation under combined ingredients); (ii) K1.2 structural diagnosis that the combination collapses to the symmetric-point identification (new); (iii) K1.4 G2 trace unit-factor analysis (new). |
| V3 | Audit lane could complete? | Yes — the audit lane can review (i) the combined evaluation (K1.1, with explicit numerical computation), (ii) the symmetric-point collapse (K1.2, citing HIGGS_MASS_FROM_AXIOM Step 5(b)), (iii) the lattice-curvature numerical match (K1.3), (iv) the G2 unit-factor structure (K1.4, citing gnewtonG2), (v) the +12.03% gap (K1.5), (vi) S4b-op orthogonality (K1.6, citing EWSB-PotForm admission), (vii) sister-precision asymmetry (K1.7). |
| V4 | Marginal content non-trivial? | Yes — testing whether combining three retained ingredients closes a load-bearing residual of a Nature-grade matching theorem (Lane 2) is non-trivial. The K1.2 structural insight (combination IS the symmetric-point identification multiplied by `v_EW` under a unit G2 trace) explains why none of the three pieces individually OR combined can close S4b-op. |
| V5 | One-step variant? | No — this is not a relabel of W-S4b-EWSB (single-ingredient EWSB test), HIGGS_MASS_FROM_AXIOM (single-ingredient Step 4 tree-level), or gnewtonG2 (single-ingredient G2 Born trace). The joint-hypothesis stress test and the K1.2 collapse-to-symmetric-point self-reference are structurally new. |
