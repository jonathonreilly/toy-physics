...
- [`scripts/poisson_self_gravity_loop.py`](../scripts/poisson_self_gravity_loop.py)
- [`logs/2026-04-05-poisson-self-gravity-loop.txt`](../logs/2026-04-05-poisson-self-gravity-loop.txt)
...
## Audit packet repair references

This section records the dependency artifacts named by the 2026-05-16
runner-artifact audit repair so the audit packet can expose the dep
runner. It does not promote this note or change the audited claim scope:
the inherited primitives are recorded above as bounded admissions
(BA-1)–(BA-3), not as load-bearing retained chain transit through the
sister note. The sister note is intentionally code-formatted here
(`MINIMAL_SOURCE_DRIVEN_FIELD_PROBE_NOTE.md`) rather than markdown-linked
so the citation graph does not continue to treat the upstream conditional
row as a retained-grade dependency for this bounded-admission repair.
