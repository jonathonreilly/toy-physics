...
**All-R argument (finite type enumeration).**  We give a uniform argument
that does NOT invoke the Jordan curve theorem on S^2.
...
of B_R at R=2..10, covering 5,778 boundary vertices and all distinct
configuration types that arise across these R values.  For every type,
H_1(K; Z) = 0 is verified directly.  Because the type enumeration is
exhaustive within the configurations B_R can produce, this finite check
covers all R simultaneously: any boundary vertex of any B_R has a
configuration that has already been verified at some smaller R or is
covered by the symmetry group of {0,-1}^3.

**Symmetry argument completing all-R coverage.**  The configurations of
present/absent partitions on {0,-1}^3 are closed under the natural
action of the symmetry group S_3 x (Z/2)^3 (coordinate permutation x
sign flips).  Up to this symmetry, the number of orbit representatives
of nonempty proper downsets in {0,-1}^3 is FINITE (less than 50).  The
runner verifies H_1 = 0 for representatives of every orbit by exhausting
the boundary vertices at R = 2..10.  Since boundary vertices at larger R
produce configurations that are symmetric to those at smaller R (the
same f_i preference structures recur), the all-R conclusion follows by
finite orbit exhaustion.

**Computational confirmation.**  The above all-R argument is independently
confirmed for every boundary vertex at R = 2..10 by direct integer Smith
Normal Form computation in the runner (P3a check, 5,778/5,778 PASS).
