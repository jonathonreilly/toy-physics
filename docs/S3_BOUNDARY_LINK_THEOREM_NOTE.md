...
These additions are checked by the runner as EXACT certificates
(not bounded support).  The runner reports 102/102 realized downset
...
Proposition Z is therefore a fully closed all-R finite-combinatorial
theorem; the bridge lemma is the remaining engineering frontier for
an unconditional all-R disk theorem.
...
(Proposition Z) closes the all-R disk property given the bridge lemma,
by exhaustive enumeration over the 126 candidate subsets of the
3-cube.
...
bridge lemma's v_i <= -2 analytic closure is the single remaining
algebraic gap.  Proposition Z is itself a fully closed all-R finite-
