...
This is **not** full axiom-native DM closure from `Cl(3)/Z^3` alone.

What still remains outside the current closure grade is the stricter native
source-chart / branch-choice derivation:

- derive the relevant five-basin source chart and branch-choice structure from
  `Cl(3)/Z^3` alone, without the retained measurement pin.

So the branch now separates cleanly:

- **review surface:** DM flagship gate closed
- **strict native surface:** source-chart / branch-choice derivation still open
