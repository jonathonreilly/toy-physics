# Two-Representation Ward Closure on `g_bare`: Path 2 Candidate

**Date:** 2026-04-18
**Status:** DERIVED candidate — absolute pinning of `(y_t_bare, g_bare)`
from the two-representation consistency of the retained tree-level
Ward-identity 1PI Green's function on the Q_L scalar-singlet channel.
Offered as a Path 2 closure on the `g_bare = 1` internal-fixation
program. Re-uses only content already established in
...
- the rigidity theorem
  ...
  an additional route to the same conclusion (`g_bare = 1` is not a
  free parameter) at the 1PI-amplitude level, rather than at the
  operator-algebra level.
- the older bounded normalization note
  (`docs/G_BARE_DERIVATION_NOTE.md`).

What this note adds: a pinning of `g_bare` as a numerical quantity
from a system of two independent equations, both of which already
live inside the retained tree-level Ward theorem, neither of which
in its content uses `g_bare = 1` as input.

---

## Claim

**Theorem (Two-Representation Ward Closure).** On the retained
...
Treating these as two equations, the joint consistency

```
Rep A = Rep B
```

on the same Green's function forces
...
so that `(y_t_bare, g_bare) = (1/√6, 1)` is uniquely determined
as an absolute pair, not merely as a ratio.
...
**The two-representation consistency PINS `g_bare² = 1` as an
absolute number, given only:**
...
The present note gives an **independent route to the same
conclusion** at the 1PI-amplitude level. The two-representation
consistency of `Γ⁽⁴⁾` forces `g_bare² = 1` from the OGE-vs-H_unit
matching on the Q_L scalar-singlet channel, without reference to
the operator-algebra trace form.

The rigidity theorem and the two-Ward closure are therefore
mutually supporting rather than competing. They both answer the
old "you can always rescale `A → A/g`" objection, from different
angles:

- Rigidity: the canonical generator basis has no scalar dilation
  freedom.
- Two-Ward closure: even if one tries to treat `g_bare` as free,
  the two representations of the retained 1PI Green's function
  force it to 1.
...
The present note answers YES, under the reading that Rep A and Rep
B of the retained Ward theorem are two **independent** evaluations
of the same 1PI Green's function. The pair (A1) and (B3) are:
...
fixed point gives the absolute pinning.

This is a Path 2 closure **modulo the interpretive move** that
...
the same retained theory. A rigorous Path 2 closure therefore rests
on the claim that Rep B's evaluation `y_t_bare = 1/√6` is a
...
rather than as independently derivable facts, the two-Ward closure
...
| `y_t_bare = 1/√6` (from Rep B alone) | 0.40825 | DERIVED | Rep B evaluation; no g_bare input |
| `g_bare² = 1` (from Rep A = Rep B) | 1.0 | DERIVED (this note) | Two-representation consistency |
| `|c_S| = 1` | 1.0 | RETAINED | S2 Clifford; Block 8 of Ward runner |
| `N_c = N_iso · 3/2` (Q_L dim = 6) | 6 | RETAINED | NATIVE_GAUGE_CLOSURE + CKM_ATLAS |

No IMPORTED rows. No BOUNDED rows. The pinning is algebraic from
retained Ward-theorem content.
...
- That the retained Ward theorem already contains, without
  ...
  `y_t_bare = 1/√6` without invoking `g_bare`; Rep A = Rep B then
  gives `g_bare² = 1`.
- That reading these as a system pins the absolute pair
  `(y_t_bare, g_bare)` and not merely the ratio `y_t/g_s`.
- That this constitutes a Path 2-compatible closure of the
