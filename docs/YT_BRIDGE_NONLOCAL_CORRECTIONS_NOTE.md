...
`|<K_nonloc, phi>| <= ||K_nonloc||_2 = 5.024e-3`
...
> at most the residual operator norm `5.024e-3` times the profile L2
> norm, for every profile in that class, given the standard SM inputs
> at `M_Z`.
...
family-agnostic upper bound `5.024e-3 * ||phi||_2`, which is intrinsic
to the kernel pair on the forced UV window.
