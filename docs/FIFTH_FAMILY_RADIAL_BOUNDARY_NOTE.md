...
The 2026-05-03 runner repair points the imports at the current API:
`_build_radial_shell_connectivity` from `DISTANCE_LAW_PORTABILITY_COMPARE`
and `_measure_family` from `CONNECTIVITY_FAMILY_V2_ELLIPTICAL_SWEEP`.
The runner now executes and reproduces the exact `(drift = 0.20,
seed = 0)` boundary row this note claims:
