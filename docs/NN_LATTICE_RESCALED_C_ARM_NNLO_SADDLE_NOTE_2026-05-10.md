...
`positive`: the finite-slit-aperture NNLO correction reduces the residual to
`|alpha_NNLO - 0.5256| = 0.0009 <= 0.005`, closing the 33% alpha gap left by
PR #1028 analytically.

## Companion work

| PR     | Outcome                                                    |
| ------ | ---------------------------------------------------------- |
| #968   | Numerical fit: `sigma_arm(h) = 2.7107 * h^{0.5256}` on the 4-point window `h <= 0.25`. |
| #1003  | LO saddle: `alpha_LO = 0.5000`, `C_arm_LO = 2.4855`. |
| #1027  | Alpha-constrained refit: per-h residual <1% at h = 0.015625; confirms LO asymptote pointwise. |
| #1028  | NLO phase: `alpha_NLO = 0.5170`, residual `|delta alpha| = 0.0086` (closes 67%). |
| **this PR** | **NNLO with finite-slit aperture: `alpha_NNLO = 0.5247`, residual `|delta alpha| = 0.0009` (POSITIVE).** |
...
The leading-order saddle (PR #1003) gives `sigma^2_LO(h) = C_LO^2 h` with
```
C_LO^2 = L_2 (c^2 / (2 F)) / (A + B) = 6.17786,    C_LO = 2.48553
```
where `L_2 = 2 L_total / 3 = 26.667` is the post-slit propagation length.

PR #1028 supplemented the saddle with the cos(omega h) phase factor in the
...
PR #1028's runner already uses the EXACT cos(omega h), so further orders of
...
(The `d2` value matches PR #1028 exactly.)
...
**Sharp null.** PR #1028's runner uses the closed-form
...
with
```
alpha_2 = L_2 s h                            (complex; PR #1003 LO)
alpha_4 = L_2 (s^2/2 - s/12) h^3              (complex; PR #1028 NLO)
...
### Numerical agreement on the PR #968 four-point fit window
...
fit window. PR #1027 already verified the LO asymptote pointwise at this h
(C_arm(h) = 2.4922, 0.27% residual against analytic 2.4855), so the
finite-window alpha fit -- which is what PR #968 reports -- is the
...
PR #1027's `sigma_arm / sqrt(h) -> C_LO`).

---

## 4) Combined alpha_NNLO_eff

Applying PR #968's log-log linear fit protocol to each model on the fit
window `h in {0.25, 0.125, 0.0625, 0.03125}`:

| Model                          | `alpha_eff` | `C_eff`  | `|alpha - 0.5256|` |
| ------------------------------ | ----------- | -------- | ------------------- |
| LO     (PR #1003)              | 0.5000      | 2.4855   | 0.0256              |
| NLO    (PR #1028, phase exact) | 0.5165      | 2.6186   | 0.0091              |
| NNLO_q (phase + q^4 + q^6)     | 0.5170      | 2.6234   | 0.0086              |
| **NNLO_full (+ aperture)**     | **0.5247**  | **2.7041** | **0.0009**       |
| EMPIRICAL  (PR #968)           | 0.5256      | 2.7107   | --                  |
...
`|alpha_NNLO_eff - 0.5256| = 0.0009 <= 0.005`. **POSITIVE band**.
...
- `|alpha_NNLO - 0.5256| <= 0.005` (positive band).
...
- `h -> 0` recovery of `C_LO^2` (cross-confirmed by PR #1027's pointwise asymptote).

## Decomposition of the original 0.0256 alpha residual

| Source                             | `alpha_eff` contribution | running cumulative      |
| ---------------------------------- | ------------------------ | ----------------------- |
| LO   saddle (PR #1003)             | 0.5000                   | residual 0.0256 left    |
...
| Combined                           | 0.5247                   | -- (POSITIVE)           |

The 33% gap left by PR #1028 is explained primarily by the finite-slit-
...
## Cross-validation against PR #1027

PR #1027's alpha-constrained refit gives `C_arm(h)` per h:
...
analytic asymptote `C_LO = 2.4855` is recovered as h -> 0 (PR #1027's
...
- No re-measurement of any sigma_arm value. The empirical data are the
  measurements from PR #968 and PR #1027, unchanged.
- No fit-protocol change. The same 4-point log-log fit of PR #968 is
  applied to each model uniformly.
- No claim that the closed-form NNLO applies below h = 0.03125. PR #1027
  ...
  by sub-leading non-Gaussian lattice modes. PR #1027 already documents
