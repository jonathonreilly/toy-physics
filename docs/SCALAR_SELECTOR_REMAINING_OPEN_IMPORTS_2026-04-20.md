...
- The Koide `Q = 2/3` physical/source-law extremal-principle bridge
  ...
  Closing it requires a retained derivation explaining why the
...
- The physical Brannen-phase `δ = 2/9` ambient transport / Wilson
  ...
  requires a retained ambient one-clock `3+1` transport / endpoint /
  Wilson law whose selected-line pullback is the physical Brannen
  phase, or an equivalent retained Wilson/lattice phase datum on the
  same ambient.
...
targets named by the verdict. Effective status remains
