...
`hw=1` source/transfer, and retained scalar routes; (Part 4) the
...
- [`PMNS_C3_CHARACTER_HOLONOMY_CLOSURE_NOTE.md`](PMNS_C3_CHARACTER_HOLONOMY_CLOSURE_NOTE.md)
  ...
  `0, 2pi/3, 4pi/3` on the retained `hw=1` triplet via the projected
...
- [`PMNS_SELECTOR_CURRENT_STACK_ZERO_LAW_NOTE.md`](PMNS_SELECTOR_CURRENT_STACK_ZERO_LAW_NOTE.md)
  ...
  recording the current-stack zero law on the retained bank,
  consistent with the present note's `J_chi = 0` annihilation claim
  on all three listed retained routes.
...
- The exact native C3 holonomy readout on `A_fwd` is imported via the
  Fourier projection definition. Closing it requires a retained
  source note proving that the holonomies `h_0, h_1, h_2` constructed
  on the retained `hw=1` triplet evaluate the Fourier projection
  ...
  `missing_bridge_theorem: provide a retained derivation of the C3
  holonomy readout on A_fwd and the proof that the free, hw=1
  source/transfer, and retained scalar routes force J_chi=0`.
- The route-wise annihilation `J_chi = 0` on the free, sole-axiom
  `hw=1` source/transfer, and retained scalar routes is imported as
  a premise. Closing it requires a retained source note proving
