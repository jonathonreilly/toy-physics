...
**Date:** 2026-05-10
**Claim type:** no_go (bounded operator-Cauchy gate failure)
**Scope:** On the compressed action-power 3D ordered-lattice companion
family and the tested grid `h in {1.0, 0.5, 0.25, 0.125}`,
`p in {0.5, 1.0, 2.0}`, no tested action-power exponent clears the
operator-Cauchy positive-identification gate. The result is a sharp
ratio-observable null for `p = 0.5` and `p = 1.0`; the `p = 2.0`
case is a partial/inconclusive gate failure rather than a sharp
non-convergence proof.
**Status authority:** source-note proposal only; independent audit lane
controls any status change.
...
- Companion bounded-null context (operator-Cauchy on a non-promotable family):
  - [`docs/ALT_CONNECTIVITY_FAMILY_OPERATOR_CAUCHY_NOTE_2026-05-10.md`](ALT_CONNECTIVITY_FAMILY_OPERATOR_CAUCHY_NOTE_2026-05-10.md)
...
The operator-Cauchy method used by the rescaled-NN source-note chain is
the canonical tool for asking whether a discrete family of operators
`T_h` converges to a continuum object `T_inf` as `h -> 0`. This note
applies that method to the action-power 3D ordered-lattice harness.
...
family in `ALT_CONNECTIVITY_FAMILY_OPERATOR_CAUCHY_NOTE_2026-05-10.md`,
where `H = 0.5` was a module constant. So h-refinement is
**structurally available** on this harness. What is
...
Verdict: **bounded operator-Cauchy gate failure on the tested grid.**
No `p` in `{0.5, 1.0, 2.0}` clears the operator-Cauchy positive gate
(`r > 0.5 AND R^2 >= 0.95`). The `p = 0.5` and `p = 1.0` cases are
sharp non-decaying nulls. The `p = 2.0` case is only a partial
positive-slope / poor-fit gate failure, so it is not treated here as
a proof of non-convergence.

## Diagnosis: Two Structural Reasons for the Null

The bounded gate failure is informative: it is the same kind of
operator-Cauchy structural no-go registered for the alt-connectivity DAG
...
signal of non-unitarity of the unnormalised amplitude operator. This is
strong bounded numerical evidence against a clean continuum bridge for
the unnormalised amplitudes on this kernel.
...
Unlike the alt-connectivity DAG family (where `H = 0.5` was hard-coded),
this harness EXPOSES `h` as a constructor parameter. The
gate failure is therefore not a "no h-axis" obstruction but a
...
| Harness | h-axis exposed? | obstruction mechanism                       | source context |
|---------|-----------------|---------------------------------------------|-----------------|
| rescaled NN              | yes (with `step_scale = h / sqrt(FANOUT)`) | none (Cauchy r >= 1.5) | rescaled-NN operator-Cauchy notes |
| alt-connectivity DAG     | no  (H constant)                            | no refinement axis      | alt-connectivity operator-Cauchy note |
...
- the uniqueness note's "F~M tracks the weak-field power of f" remains
  ...
  the universality classes themselves are not lifted to continuum
...
1. the dense `(h^2 / L^2) exp(-BETA θ²)` kernel appears non-unitary
   on this companion grid; the
   field-free total detector throughput `p_total_f` diverges
   polynomially in `1 / h` (78 orders of magnitude across the tested
   `h` window). This is strong evidence against a bounded continuum
   `T_inf` for the unnormalised amplitude operator on this kernel.
...
This bounded gate failure constrains the candidate space: any future
positive continuum identification on this harness must first either
(a) introduce a per-edge norm-preserving rescaling that absorbs the
`p_total_f` divergence (analog of the rescaled-NN
`step_scale = h / sqrt(FANOUT)`), or (b) exhibit a longer refinement
window where the ratio observables are monotone enough for a reliable
Cauchy fit. The tested window does not do that.

## What This Closes

- Bounded operator-Cauchy gate failure for the action-power 3D
  ordered-lattice companion harness on the tested grid: no tested
  `p in {0.5, 1.0, 2.0}` clears the positive-identification gate.
- Sharp ratio-observable nulls for `p = 0.5` and `p = 1.0`.
- Partial / inconclusive positive-slope gate failure for `p = 2.0`;
  this note does not treat that case as a proof of non-convergence.
- Adds a third distinct structural-reason class to the bounded-null
  ...
  distinct from the alt-connectivity "no h-axis" obstruction.
...
- A proof that the `p = 2.0` action-power lane cannot converge at
  finer `h`; the runner records a gate failure on the tested window,
  not a terminal asymptotic theorem for that lane.
...
- [`docs/NN_LATTICE_RESCALED_OPERATOR_CAUCHY_CONVERGENCE_NOTE_2026-05-10.md`](NN_LATTICE_RESCALED_OPERATOR_CAUCHY_CONVERGENCE_NOTE_2026-05-10.md)
- [`docs/NN_LATTICE_RESCALED_KERNEL_IDENTIFICATION_NOTE_2026-05-10.md`](NN_LATTICE_RESCALED_KERNEL_IDENTIFICATION_NOTE_2026-05-10.md)
- [`docs/ALT_CONNECTIVITY_FAMILY_OPERATOR_CAUCHY_NOTE_2026-05-10.md`](ALT_CONNECTIVITY_FAMILY_OPERATOR_CAUCHY_NOTE_2026-05-10.md)

It sits in the action-power 3D ordered-lattice cluster. Any audit
status change or repo-wide mapping consequence is controlled by the
