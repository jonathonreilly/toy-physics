...
The framework's physical-lattice axiom is no longer the exact live boundary
between the retained `hw=1` triplet and physical species semantics, and it is
no longer the exact live boundary for substrate physicality once the accepted
one-axiom Hilbert/locality/information surface is used.
