...
|---|---|---|---|
| Weak-field GR-signature bundle (WEP, time dilation, conformal metric, geodesic, light bending) | keep as bounded support, not retained backbone | parts of the chain are strong, but the full GR package still mixes exact, consistency, and conditional steps | [EMERGENT_GR_SIGNATURES_NOTE.md](EMERGENT_GR_SIGNATURES_NOTE.md), [GEODESIC_EQUATION_NOTE.md](GEODESIC_EQUATION_NOTE.md), [BACKGROUND_INDEPENDENCE_NOTE.md](BACKGROUND_INDEPENDENCE_NOTE.md) |
...
| Cosmology companions (`w=-1`, graviton mass, `Omega_Lambda`, `n_s`) | bounded / conditional companion only | good signals, but they sit on bounded topology / imported cosmology inputs | [W_MINUS_ONE_NOTE.md](W_MINUS_ONE_NOTE.md), [GRAVITON_MASS_DERIVED_NOTE.md](GRAVITON_MASS_DERIVED_NOTE.md), [OMEGA_LAMBDA_DERIVATION_NOTE.md](OMEGA_LAMBDA_DERIVATION_NOTE.md) |
