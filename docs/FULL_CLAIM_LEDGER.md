...
| Anomaly-forced `3+1` closure | promoted | main text | [ANOMALY_FORCES_TIME_THEOREM.md](ANOMALY_FORCES_TIME_THEOREM.md) | `frontier_anomaly_forces_time.py` (theorem note is the authority; runner score mixes computed checks and labeled assertions) |
...
| Exact `I_3 = 0` / no third-order interference | promoted exact companion | main text or Extended Data | [I3_ZERO_EXACT_THEOREM_NOTE.md](I3_ZERO_EXACT_THEOREM_NOTE.md) | `frontier_born_rule_derived.py` (historical runner name; safe claim is exact `I_3 = 0`) |
...
| Cosmology companions (`w=-1`, graviton mass, `Omega_Lambda`, `n_s`) | bounded / conditional companion only | good signals, but they still sit on imported cosmology inputs or additional matching structure beyond the retained topology result | [W_MINUS_ONE_NOTE.md](W_MINUS_ONE_NOTE.md), [GRAVITON_MASS_DERIVED_NOTE.md](GRAVITON_MASS_DERIVED_NOTE.md), [OMEGA_LAMBDA_DERIVATION_NOTE.md](OMEGA_LAMBDA_DERIVATION_NOTE.md) |
...
| `S3_SHELLABILITY_NOTE.md` as standalone all-`R` closure authority | support only, not primary authority | finite-`R` constructive support is useful, but this note is not the accepted closure route by itself |
...
| `WILSONIAN_EFT_DERIVATION_NOTE.md` / `frontier_wilsonian_eft.py` as `y_t` closure authority | stale / bounded-only | the current evidence is on toy Hamiltonians and does not discharge the real low-energy matching bridge on the physical lattice surface |
