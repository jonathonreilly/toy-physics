...
being independently accepted; it does not re-derive their content here.
...
- [`S3_ANOMALY_SPACETIME_LIFT_NOTE.md`](S3_ANOMALY_SPACETIME_LIFT_NOTE.md)
  — the discrete `3+1` lift `PL S^3 x R`
...
- [`UNIVERSAL_GR_LORENTZIAN_SIGNATURE_EXTENSION_NOTE.md`](UNIVERSAL_GR_LORENTZIAN_SIGNATURE_EXTENSION_NOTE.md)
  — extension of the
...
- the slice generator `Lambda_R` is taken as already present in the
  support-tier closure stack
...
A prior review found that the old note asserted the global closure stack
without carrying the needed authorities or runner context. The finding was:
...
4. Reframes "**full discrete `3+1` GR on the project's route:** YES" as a
   **conditional** YES (conditional on the cited closure-stack inputs and
   ...
   theorem).
5. Re-queues the changed source for independent audit.
