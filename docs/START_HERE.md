...
- **Mirror remains the flagship coexistence architecture.**
  - Born-clean at machine precision
  - positive gravity on retained windows
  - nontrivial MI / decoherence on the same graphs
...
- **Nearest-neighbor refinement is a bounded refinement bridge.**
  ...
  - continuum / RG closure is still open
- **Structured chokepoint bridge is now real.**
...
- **Valley-linear is a bounded action fork on the ordered-lattice `1/L^2` family.**
  ...
  - bounded derivation-side harnesses are now artifact-backed on that family:
    ...
    - a second-family 2D ordered-lattice cross-check now keeps the same
      additivity split
...
- **Distance law on the flagship connected symmetry family:** closed negative
...
- **Moonshot strong-field branch is now real, but still bounded.**
  ...
  - a local robustness sweep now shows that exact-lattice Green pocket is not
    ...
    and near-linear `F~M` on the retained ladder
  - a larger exact-lattice Green scaling companion now shows the same
    source-resolved pocket survives one size step, keeping exact zero-source
    reduction, `TOWARD`, and near-linear `F~M` on the retained source ladder
