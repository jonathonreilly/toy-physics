...
**Script:** `scripts/frontier_g1_path_b_z3_cubic_selector.py`
**Framework convention:** "axiom" means only the single framework axiom `Cl(3)` on `Z^3`.

## Scope and discipline

This note establishes a precise **obstruction theorem** for Path B of the G1
...
The Path-B question: does the cubic piece `2 Re(w^3)`, which breaks the
quadratic isotropy `delta <-> q_+`, pick a unique axiom-native chamber point?

## Obstruction theorem

**Theorem (Path-B obstruction).** Let `W[J_act]` be the Schur-baseline scalar
...
**Corollary.** Any Path-B closure of G1 requires at least one post-axiom
...
Before this note, the G1 narrowed-gap statement read (from the upstream
Schur-baseline note):

> "Any future closure of G1 must supply one of:
...
The runner `scripts/frontier_g1_path_b_z3_cubic_selector.py` verifies each
...
Running: `PYTHONPATH=scripts python3 scripts/frontier_g1_path_b_z3_cubic_selector.py`.
...
- a theorem-grade **obstruction**: the cubic Path-B route cannot produce an
  m-invariant, finite, unique chamber selector from axiom-native objects
  alone
- a narrowing of the G1 open-surface: route (b) in the upstream narrowed-gap
...
**cubic Path-B obstructed; G1 selector principle still open; route (b)
closed as stand-alone**.

## Command

```bash
PYTHONPATH=scripts python3 scripts/frontier_g1_path_b_z3_cubic_selector.py
```
