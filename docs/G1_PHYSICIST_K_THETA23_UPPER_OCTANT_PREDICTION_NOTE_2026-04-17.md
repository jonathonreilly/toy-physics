# G1 Physicist-K — θ_23 Upper-Octant Chamber-Closure Prediction

**Date:** 2026-04-17
**Branch:** `claude/g1-physicist-k` (off `claude/g1-complete`)
**Status:** RETAINED-GRADE PREDICTION — a falsifiable consequence of the
Physicist-H PMNS-as-f(H) closure, derived from the retained H-diagonalization
map alone.
**Script:** `scripts/frontier_g1_physicist_k_theta23_chamber_closure_prediction.py`
...
An adversarial input-sensitivity test on the Physicist-H pinning (fixing
...
- For `s_23^2 >= s_23^2_min`, the Physicist-H map admits a unique chamber-
...
```
G1 chamber closure ADMITS a solution only if
...
Since the threshold surface lies entirely above `0.5`, the G1 closure
...
All retained on `claude/g1-complete` at the time of writing:

- The Physicist-H retained PMNS-as-f(H) map
...
- The Schur-Q candidate point `δ = q_+ = sqrt(6)/3` as a retained landmark
  from the Path-A/B information-geometric obstruction tour.
...
The Physicist-H map `Φ : (m, δ, q_+) → (s_12^2, s_13^2, s_23^2)` is a
...
points (the Physicist-H 60-seed multi-start found a unique one).
...
|---|---|---:|---|
| PDG 2024 central `(0.307, 0.0218, 0.545)` | `(0.6571, 0.9338, 0.7150)` | +0.0159 | YES (Physicist-H pin) |
...
```
arose in the Path-A / Path-B information-geometric obstruction tour as a
...
lives (Quadratic Unanimity Theorem, Physicist-E Candidate A). The Schur-Q
...
2. **PMNS-pinning interpretation.** The Physicist-H inverse image
...
is analytically significant: the retained variational route (Path A/B/E) and
the retained observational route (Physicist-H) meet on a common boundary
...
**Retained prediction (G1 θ_23 upper-octant).** Given the retained
Physicist-H PMNS-as-f(H) closure and the retained chamber
`q_+ + δ ≥ sqrt(8/3)`, the G1 inverse problem
...
**Corollary (θ_23 upper-octant).** The G1 chamber closure requires
...
- A future global fit settling on `sin^2 theta_23 < 0.5` at `>3σ`
  UNCONDITIONALLY FALSIFIES the G1 closure (since the threshold surface is
  entirely above `0.5`).
- A future global fit with `sin^2 theta_23 < 0.534` at `>3σ` falsifies the
  G1 closure at PDG central `(s_12^2, s_13^2)` (since the minimum threshold
...
- **DUNE (first data 2030+).** Long-baseline appearance and disappearance;
  ...
  sharpen the central value to well below the G1 threshold margin.

**Timeline.** The G1 θ_23 upper-octant prediction is expected to be decisively
...
1. **Retained-grade prediction** (not an obstruction). Derived purely from
   the retained H-diagonalization map (Physicist-H) and the retained chamber
...
4. **θ_23 upper-octant** is the G1 prediction. Lower octant at `>3σ` would
   falsify G1 closure unconditionally.
...
`G1_OMNIBUS_CLOSURE_REVIEW_NOTE_2026-04-17.md` or the ARXIV_DRAFT:

> **θ_23 upper-octant retained prediction (Physicist-K).** The Physicist-H
> PMNS-as-f(H) closure has a structural threshold: at PDG 2024 central
> `(s_12^2, s_13^2) = (0.307, 0.0218)`, the G1 chamber closure admits a
...
> Schur-Q variational candidate `(sqrt(6)/3, sqrt(6)/3)`. The G1 closure
...
> unconditionally falsify the G1 closure.
...
PYTHONPATH=scripts python3 \
  scripts/frontier_g1_physicist_k_theta23_chamber_closure_prediction.py
```
