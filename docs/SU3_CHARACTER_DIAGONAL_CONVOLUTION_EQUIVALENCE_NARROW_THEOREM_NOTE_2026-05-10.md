...
the standard Schur character-orthogonality relation
`int chi_(p,q)(W) chi_(p',q')(W^{-1}) dW = delta_{(p,q),(q',p')}`,
which after relabelling `(p', q') -> (q', p')` matches the form `(5)`.
...
- self-adjoint (diagonal in an orthonormal basis after rescaling),
...
eigenvalues. In the basis `{chi_(p,q)}` rescaled to orthonormal via
`(5)` (i.e. `phi_(p,q) = sqrt(d_(p,q)) chi_(p,q)`), `R` is diagonal
with real entries, hence self-adjoint. Conjugation symmetry
...
1. Schur orthogonality `(5)` to machine precision via Weyl integration
   ...
   = delta_((p,q),(p',q')) / d_(p,q)` for all pairs in `B_4`.
2. Symbolic Conclusion `(T2)`: for an abstract positive symmetric
   ...
   diagonal operator `R` agree on each basis character, both via
   direct Haar-integral evaluation and via the formal Schur-orthogonal
   reduction `(9)`.
...
4. Conclusion `(T4)` positivity/Hermiticity/swap-symmetry: under the
   ...
   in the rescaled orthonormal basis, and commutes with the
   conjugation swap involution.
5. Concrete instance with `(rho_(p,q)) = (1, 2/3, 2/3, 1/4, ...)`
   (abstract rational positive symmetric data) and confirmation that
