...
- `koide_gamma_axis_covariant_full_cube_orbit_law_note_2026-04-18`
  (downstream consumer; backticked to avoid length-2 cycle — the
  axis-covariant note already lists this bridge as one of its earlier
  inputs in its body, so citation graph direction is *axis_law → bridge*)
- `koide_gamma_orbit_cyclic_return_candidate_note_2026-04-18`
  (sibling/downstream consumer; backticked to avoid length-2 cycle —
  citation graph direction is *cyclic_return → bridge*)
