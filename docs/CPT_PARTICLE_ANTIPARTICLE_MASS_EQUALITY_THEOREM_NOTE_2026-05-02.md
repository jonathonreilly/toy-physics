...
**Claim scope:** for any CPT-exchanged single-particle/antiparticle sector pair of the framework's declared CPT-invariant Hamiltonian H with energy E, the paired antiparticle state is also an eigenstate of H with the same energy E. In particular, the rest masses of CPT-exchanged particle/antiparticle pairs are equal: m_particle = m_antiparticle.
...
- [`CPT_EXACT_NOTE.md`](CPT_EXACT_NOTE.md) — declared audit dependency; live
  effective status is pipeline-derived and may be audit-pending after
  dependency-graph strengthening. Provides exact CPT invariance of the framework's
...
- **CPT-exchanged sector convention.** In this note, "particle" and
  "antiparticle" denote a pair of single-particle sectors exchanged by the
  declared CPT operator. Identifying every named framework species with such a
  sector pair is a separate matter-sector bookkeeping step, not part of this
  theorem's load-bearing proof.
...
Let `H` be the framework's declared Cl(3) staggered Hamiltonian on Z^3
with periodic boundary and even side length L, satisfying
`[CPT, H] = 0` per the declared CPT_EXACT_NOTE dependency. Let
`|particle; p, σ⟩` be a single-particle energy eigenstate in a sector paired by
CPT, with momentum `p`, spin/helicity `σ`, and energy `E_particle(p, σ)`.
Define the antiparticle state by
...
Then, conditional on the declared CPT dependency:
...
**(M3) Uniformity across CPT-exchanged sectors.** (M2) holds for every
single-particle sector pair to which the declared CPT operator applies. This is
independent of any specific mass values; only CPT invariance and the
CPT-exchanged sector convention are used.

(M1)–(M3) constitute the particle/antiparticle mass equality theorem candidate
on the framework's declared CPT surface.
...
### Step 4 — Uniformity over CPT-exchanged sectors (proves M3)

Steps 1–3 used **only** the retained CPT identity `[CPT, H] = 0` and
the structural definition of CPT. Neither step used any species-
specific property. The argument applies uniformly to every single-particle
sector pair exchanged by the declared CPT operator. Hence (M3): mass equality
holds for every such pair. ∎

## Hypothesis set used

- `cpt_exact_note` (declared audit dependency; live status is pipeline-derived):
...
**Positive theorem candidate on the declared CPT surface.** Steps 1–4 close
from the CPT exact dependency alone, plus the structural convention that
"particle" and "antiparticle" are sectors exchanged by the declared CPT
operator. No fitted or observed input is used. The chain is single-hop, and
retained-family status is not asserted by this source note.
