# G_Newton Weak-Field Response Form — Bounded Closure (gnewtonG3 probe)

**Date:** 2026-05-10
**Type:** bounded_theorem (closure of admission B(c) of planckP4 to retained Hamiltonian flow)
**Claim type:** bounded_theorem
**Scope:** review-loop source-note proposal — closes the third of three named
...
response of the framework's retained Hamiltonian-flow propagator under
the canonical Newtonian-limit coupling `V_grav = m*phi(x)`. The
alternative spent-delay form `S = L sqrt(1 - phi)` requires a retained
metric tensor `g_munu`, which is not present in the audit ledger.
**Status:** source-note proposal for a bounded-closure of admission B(c)
of `G_NEWTON_SELF_CONSISTENCY_BOUNDED_SHARPENING_NOTE_2026-05-10_planckP4`.
The valley-linear weak-field response form is forced by retained content
once the canonical coupling `V_grav = m*phi` is granted; the residual
admission collapses onto admission B(b) of the planckP4 note.
...
chain, and runner. The `claim_type`, scope, and bounded-closure
...
profile in `DIMENSIONAL_GRAVITY_TABLE.md`, not by retained derivation:

> The valley-linear selection produces `F~M = 1.00` (Newtonian); the
> others do not. So the framework's own retained content compares both
> as candidates and selects valley-linear by **empirical match** to
> `F~M = 1`, not by retained derivation.

The probe question for the gnewtonG3 angle:

> Can the retained framework's Hamiltonian-flow propagator structure
...
gravitational perturbation directly from retained Hamiltonian dynamics,
not from a comparison of empirical lattice profiles.

## Answer

**Bounded closure.** The valley-linear weak-field response form
`S = L(1 - phi)` is the unique leading-order response of the framework's
retained Hamiltonian-flow propagator under the canonical Newtonian-limit
...
- **Section 1:** retained Hamiltonian-flow structure (Hermitian `H_0`,
...
- **Section 4:** spent-delay obstruction requires a retained metric
  tensor `g_munu`, which is absent from retained content (the framework
  retains only the Z³ graph metric `d : Z³ × Z³ → Z_≥0`, not a smooth
  Lorentzian `g_munu(x)`).
...
**Important boundary:** the closure is bounded because:
1. it FORCES valley-linear ON retained content + canonical `V_grav = m*phi` coupling, but
...
The net effect is to **collapse admission (c) onto admission (b)**:
the weak-field response form is now derived from canonical `V_grav`
coupling, and that coupling itself is downstream of `ρ = |ψ|²` (admission b).
...
requires a retained covariant line-element interpretation
`ds² = g_munu dx^mu dx^nu` with `g_00 = 1 - phi`, which is not present
in retained Cl(3)/Z³ content (the framework retains only the Z³ graph
metric `d : Z³ × Z³ → Z_≥0`).

**Corollary (admission collapse).** Admission (c) of `GRAVITY_CLEAN_DERIVATION_NOTE`
no longer requires empirical pinning. It is derivable FROM admission (b)
plus retained Hamiltonian flow:

```
ρ = |ψ|²    (admission b)
V_grav = m*phi    (canonical coupling, m from ρ)
H = H_0 + V_grav    (single-clock dynamics)
U(t) = exp(-iHt/hbar)    (retained unitarity)
==> S(phi)/L = 1 - phi    (admission c, derived)
```

The G_Newton three-admission frontier of the planckP4 note becomes
effectively a **two-admission frontier**: `(a)` self-consistency
`L^{-1} = G_0` plus `(b)` Born-as-gravity-source (with `(c)` derived as
a corollary of `(b)` plus retained Hamiltonian flow).

## Proof

### Step 1 — Retained Hamiltonian-flow structure
...
S1.5 No retained metric tensor (graph distance only): PASS
```

The load-bearing absence in S1.5 — no retained metric tensor — is what
...
This requires three structures NOT present in retained Cl(3)/Z³ content:

1. **A smooth pseudo-Riemannian metric tensor `g_munu(x)`** — the framework
   retains only the Z³ graph metric `d : Z³ × Z³ → Z_≥0` (a discrete
   distance function), not a smooth tensor field on a manifold.
2. **The metric identification `g_00 = 1 - phi`** — this is the
   Newtonian-limit static metric of GR, requiring an additional
   structural axiom not in retained content.
...
S4.3 Spent-delay form requires absent retained primitive (g_munu): PASS
S4.4 Valley-linear is forced by retained Hamiltonian flow: PASS
S4.4 Spent-delay requires retained content not in ledger: PASS
```
...
sense — it just means it requires content (a retained metric tensor) that
the retained framework does not provide. A future retained "metric-tensor
...
S5.8 Audit ledger has retained Hamiltonian flow: PASS
S5.8 Audit ledger has retained Lieb-Robinson: PASS
S5.8 Audit ledger does NOT have retained metric tensor: PASS
S5.9 Bounded closure: weak-field response derived: PASS
```
...
The only structural input is the retained Hamiltonian flow + canonical
coupling. ∎

## What this closes

- **Closes admission B(c) of the planckP4 note** for the canonical
  coupling `V_grav = m*phi`. The valley-linear weak-field response form
  `S = L(1 - phi)` is now derived from retained content + canonical
  Newtonian-limit coupling, no longer requiring empirical pinning from
  the `F~M = 1` match in `DIMENSIONAL_GRAVITY_TABLE`.
- **Collapses the three-admission frontier to two-admission.** Admission
  (c) is now a corollary of admission (b) plus retained Hamiltonian flow.
  The frontier of `GRAVITY_CLEAN_DERIVATION_NOTE` becomes:

  ```
  (a) L^{-1} = G_0       (still independently admitted)
  (b) ρ = |ψ|^2          (still independently admitted)
  (c) S = L(1 - phi)     ==> derived from (b) + retained Hamiltonian flow
  ```

- **Identifies the residual barrier.** The remaining empirical / external
  load on (c) collapses entirely onto (b): if (b) is closed unconditionally,
  then (c) follows automatically by this derivation. So the audit-ledger
  deliverable narrows from "three retained primitives" to "two retained
  primitives" (skeleton-selection for (a), Born-as-gravity-source for (b)).
- **Strengthens the parent planckP4 sharpening note's V1 verdict.** The
  G_Newton lane status sharpens further from "positive P1 + three named
  obstructions" to "positive P1 + two named obstructions ((a), (b)) +
  derived corollary (c)."
...
- A retained "metric tensor theorem" that would permit the alternative
  spent-delay form. This note shows spent-delay is NOT in the retained
...
audit lane. The bounded theorem is the closure of admission B(c) of
the planckP4 sharpening note: the valley-linear weak-field response
form `S = L(1 - phi)` is forced at leading order in `phi` by the
retained Hamiltonian flow + canonical Newtonian-limit coupling.

The closure is BOUNDED because it requires the canonical coupling
`V_grav = m * phi` as an input — and that coupling is downstream of
admission (b) (Born-as-gravity-source), which remains open in the
audit ledger.

No new admissions are proposed. The independent audit lane may retag,
...
| # | Question | Answer |
|---|---|---|
| V1 | Verdict-identified obstruction closed? | Admission B(c) of the planckP4 sharpening note is closed for the canonical coupling `V_grav = m*phi`. The weak-field response form `S = L(1 - phi)` is now derived from retained Hamiltonian flow + canonical coupling, not empirically pinned. The G_Newton three-admission frontier collapses to two admissions plus a derived corollary. |
| V2 | New derivation? | Yes — the leading-order linear-in-phi response of the framework's retained Hamiltonian flow under canonical V_grav coupling is computed structurally, not measured empirically. The slope = -1 result is exact (R² = 1.0 to 1e-10), independent of finite-size cutoff and spatial dimension. The spent-delay obstruction is identified explicitly as requiring a retained metric tensor, which is absent. |
| V3 | Audit lane could complete? | Yes — the audit lane can review (i) the retained Hamiltonian-flow structure, (ii) the canonical coupling V_grav = m*phi, (iii) the leading-order BCH/Magnus expansion, (iv) the action-level interpretation, (v) the spent-delay obstruction with explicit metric-tensor absence. |
| V4 | Marginal content non-trivial? | Yes — the closure of admission (c) of `GRAVITY_CLEAN_DERIVATION_NOTE` reduces the open frontier from three retained primitives to two. This is non-trivial because the empirical-pinning critique of admission (c) was the load-bearing obstruction in the planckP4 sharpening note. |
| V5 | One-step variant? | No — the derivation of valley-linear from retained Hamiltonian flow is structurally distinct from the planckP4 dimensional-rigidity result and from the empirical F~M=1 selection in `DIMENSIONAL_GRAVITY_TABLE`. The new content is the exact slope = -1 from unitary evolution + canonical coupling, plus the explicit metric-tensor absence as the obstruction for spent-delay. |

**Source-note V1-V5 screen: pass for bounded-closure audit seeding.**
...
- Is NOT a relabel of any prior weak-field-action note. The leading-order
  derivation from retained Hamiltonian flow is structurally new content,
...
- Provides explicit closure of admission (c) of the planckP4 sharpening
  note, reducing the open frontier from three to two retained
  primitives.
- Sharpens the user-prompt question "can retained content close the
  weak-field response form admission?" from "open" to "closed for
...
- Lieb-Robinson microcausality (retained Hamiltonian flow):
...
- RP transfer matrix (retained H_phys):
...
- Sister A1 gravity-phase obstruction:
...
Expected output: structural verification of (i) retained Hamiltonian-flow
...
- `feedback_consistency_vs_derivation_below_w2.md`: this note applies
  ...
  form `S = L(1 - phi)` is DERIVED from retained content + canonical
...
- `feedback_retained_tier_purity_and_package_wiring.md`: no automatic
  cross-tier promotion. This note proposes bounded-closure of admission
  (c); the parent `GRAVITY_CLEAN_DERIVATION_NOTE` remains at its prior
  `audited_conditional` status until admissions (a) and (b) are also
  closed.
...
- `feedback_bridge_gap_fragmentation_2026_05_07.md`: the parent
  G_Newton lane is fragmenting from three admissions to two with
  derived corollary. No new admissions introduced; existing admission
  (c) closed via canonical coupling.
