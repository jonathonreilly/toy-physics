...
weighted) is forced from retained content, distinguishing it from
...
**Question:** Does any retained content canonically distinguish F1
from F2 and F3?
...
(rank/dimension (1,2)) selection is not forced by retained content."
```
...
| BlockTotalFrob | `E_+ = 3a²`, `E_⊥ = 6\|b\|²` on `M_3(ℂ)_Herm`; §4 enumerates F1 and F3 (det carrier) as the two natural retained log-laws | retained per [`KOIDE_KAPPA_BLOCK_TOTAL_FROBENIUS_MEASURE_THEOREM_NOTE_2026-04-19.md`](KOIDE_KAPPA_BLOCK_TOTAL_FROBENIUS_MEASURE_THEOREM_NOTE_2026-04-19.md) |
| FrobIsoSplit | Frobenius pairing is the unique Ad-invariant inner product on `M_3(ℂ)_Herm` (up to overall positive scalar) | retained per [`KOIDE_FROBENIUS_ISOTYPE_SPLIT_UNIQUENESS_NOTE_2026-04-21.md`](KOIDE_FROBENIUS_ISOTYPE_SPLIT_UNIQUENESS_NOTE_2026-04-21.md) |
...
| Q-readout | Q-functional factors through diagonal-species returned operator (rank-3 quotient) | retained per [`KOIDE_Q_READOUT_FACTORIZATION_THEOREM_2026-04-22.md`](KOIDE_Q_READOUT_FACTORIZATION_THEOREM_2026-04-22.md) |
| HSRig | Hilbert–Schmidt rigidity: `B_HS = Tr(XY)` is unique Ad-invariant on `su(3)` up to scalar | retained per [`G_BARE_HILBERT_SCHMIDT_RIGIDITY_THEOREM_NOTE_2026-05-07.md`](G_BARE_HILBERT_SCHMIDT_RIGIDITY_THEOREM_NOTE_2026-05-07.md) |
| Probe1 | RP/GNS does not force `(1,1)` Frobenius pairing on hw=1 from retained content | retained per [`KOIDE_A1_PROBE_RP_FROBENIUS_BOUNDED_OBSTRUCTION_NOTE_2026-05-08_probe1.md`](KOIDE_A1_PROBE_RP_FROBENIUS_BOUNDED_OBSTRUCTION_NOTE_2026-05-08_probe1.md) |
| Probe12 | Plancherel-uniform on `Ĉ_3` gives `(1,2)` weighting; `(1,1)` requires combining ω, ω̄ | retained per [`KOIDE_A1_PROBE_PLANCHEREL_PETER_WEYL_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe12.md`](KOIDE_A1_PROBE_PLANCHEREL_PETER_WEYL_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe12.md) |
| Probe13 | K-real-structure supplies Z_2 part of (1,1) but not SO(2) | retained per [`KOIDE_A1_PROBE_REAL_STRUCTURE_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe13.md`](KOIDE_A1_PROBE_REAL_STRUCTURE_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe13.md) |
...
both, and the framework's retained content does not pin both choices
...
But Probes 12, 13 establish that retained content does not select
...
cannot be derived from retained content alone. The discrete
...
pinned by retained content. What is **not pinned** is the choice
...
2. **F1 vs F3 within the canonical class is unchanged** from Probes
   ...
   non-canonical state on `A^{C_3}` not pinned by retained content.
...
4. **Max-entropy** (AV4) reaches F1 only conditionally on a specific
   measure-and-constraint convention. Without retained content
...
- `feedback_consistency_vs_derivation_below_w2.md`: this note
  ...
  (forcing the convention from retained content) is what fails.
