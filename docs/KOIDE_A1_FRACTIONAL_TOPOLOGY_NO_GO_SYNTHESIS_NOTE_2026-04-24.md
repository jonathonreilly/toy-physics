...
Empirically `P_A1` is pinned: under canonical `χ` the framework would
predict `δ = 4π/9 rad ≈ 1.396 rad`, which disagrees with the PDG-fit value
`δ ≈ 0.2222 rad` at parts-per-10⁵. The data forces `χ'`. So `P_A1` is an
empirically-pinned single-observable convention — but it is still a
convention choice not derivable from retained Cl(3)/Z³ structure, and the
five Round-10 probes confirm this is true under each canonical
fractional-rational extension of the math literature.
...
P_A1_EMPIRICALLY_PINNED=TRUE_AT_PDG_DELTA_2_OVER_9_RAD
