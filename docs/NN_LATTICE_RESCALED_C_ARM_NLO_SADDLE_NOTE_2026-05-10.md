...
gap |delta alpha| = 0.0086 outside the +/- 0.005 positive band)
...
**Diagnostic comparator:** PR #968 four-point fit
`sigma_arm(h) = C_arm h^alpha`, `C_arm = 2.7107`, `alpha = 0.5256`,
`R^2 = 0.9996` on `h in {0.25, 0.125, 0.0625, 0.03125}`.

## Context

The leading-order saddle of PR #1003 closes the geodesic constant
...
The same derivation predicts `alpha = 1/2` exactly. The empirical four-point
...
The PR #968 measurements directly support a power-law approach to the LO
constant:
...
prediction propagated through PR #968's four-point log-log fit gives
...
gap is `|delta alpha| = 0.0086`. This is a bounded theorem, not a positive
closure.
...
protocol of PR #968. It does not introduce a new axiom and does not
modify any retained theorem family.
...
### 5. Per-h cross-check vs PR #968 measurements
...
PR #968 uses the protocol: fit `log sigma_arm = log C_arm + alpha log h`
...
the LO formula is exact at `h -> 0` and the fit-window extracts a
...
positive band `+/- 0.005` set by the task acceptance criterion. Three
...
3. **Finite-slit-aperture corrections.** PR #1003 observes `sigma_1 ~
...
- propagates `delta(h)` through PR #968's log-log fit protocol and
...
- close the alpha-residual to the `+/- 0.005` positive band;
- promote any retained-theorem-family row;
- close PR #968 or promote the diagnostic fit;
...
extends the parent LO note (PR #1003) by one order in the saddle
