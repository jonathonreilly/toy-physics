...
Combined with the retained single-clock theorem's identification of the
...
This note asks whether retained 3+1 single-clock time-asymmetry —
...
**Yes, conditionally.** Under retained {RP staggered phase rule;
...
Combined with the retained single-clock theorem's identification of
...
to the exterior algebra. Three retained authorities supply that bridge:
...
| ID | Statement | Class |
|---|---|---|
| RP | Action-level temporal-axis sign rule `eta_t(theta x) = - eta_t(x)`, `eta_i(theta x) = + eta_i(x)` | retained per `AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29` (proposed_retained, audit-pending) |
| SC | Temporal direction is the unique RP-admissible reflection axis on the staggered-Dirac action | retained per `AXIOM_FIRST_SINGLE_CLOCK_CODIMENSION1_EVOLUTION_THEOREM_NOTE_2026-05-03` Step 4 (proposed_retained, audit-pending; PR 418) |
| LR | Forward-time `alpha_t(O) = e^{itH} O e^{-itH}` with `v_LR < infty`; `H = -log(T)/a_tau` | retained per `AXIOM_FIRST_MICROCAUSALITY_LIEB_ROBINSON_THEOREM_NOTE_2026-05-01` (proposed_retained, audit-pending) |
...
Under retained {RP, SC, LR, AT}, the rank-four boundary/orientation
...
on retained 3+1 single-clock time-asymmetry.
...
(see "Status" section above): under retained RP and SC, the staggered
...
The retained microcausality / Lieb-Robinson theorem (R-LR) gives the
...
retained forward-time generator.

Therefore Hodge does NOT carry the time-direction identification from
`P_1` to `P_3`. The retained 3+1 single-clock time-asymmetry breaks
the Hodge degeneracy, and the orientation principle uniquely selects
`P_1`. This is (O6). ∎

### Step 6 — Conclusion

Steps 1–5 establish the orientation principle: under retained
...
- (R-SC) retained single-clock codimension-1 evolution theorem,
...
- (R-AT) `ANOMALY_FORCES_TIME_THEOREM` (bounded_theorem on main with
  ...
  retained gauge content + chirality + single-clock cascade
...
proposed_claim_scope: |
  ...
  none of which is e^t). Combined with the retained single-clock theorem's
...
conditional_surface_status: derived support theorem on retained RP +
  retained single-clock + retained microcausality + bounded
...
proposal_allowed_reason: |
  All four load-bearing inputs are retained (or bounded_theorem) on the
  current authority surface. The proof proceeds entirely by exterior-
...
is a conditional bridge. The retained RP and single-clock theorems
...
exterior algebra. It is forced by the retained physical structure —
...
identification comes from the retained physical structure, not from
purely algebraic substrate symmetries. This note explicitly relies
on retained RP / SC / LR / AT to make the identification.
...
2. orientation principle from retained 3+1 single-clock time-asymmetry
   (THIS note) — `bounded_theorem` proposed.
3. (campaign-specific third theorem) — not in scope of this note.

The orientation principle proposed here closes the
...
supplied by retained RP + retained single-clock + retained
microcausality.
