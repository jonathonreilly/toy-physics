...
2. the strongest exact closure attainable on the retained `3d+1` ambient is now
   identified;
3. that strongest attainable closure is enough to close the flagship route on
   the reduced projected-source packet and live target;
...
classification, or carrier choice, not internal inconsistency in the theorem
chain currently claimed.
