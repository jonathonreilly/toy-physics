...
The derivation is already exact on the retained active-half-plane chart. From
`DM_NEUTRINO_SOURCE_SURFACE_ACTIVE_HALF_PLANE_THEOREM_NOTE_2026-04-16.md`,
with `E1 = sqrt(8/3)`, one has

```text
q_+ = E1 - δ + sqrt(r31^2 - 1/4),
r31 >= 1/2.
```
...
```text
q_+ + δ = E1 + sqrt(r31^2 - 1/4) >= E1 = sqrt(8/3),
```

and equivalently

```text
q_+ - E1 + δ = sqrt(r31^2 - 1/4) >= 0.
```

So the chamber boundary is exactly the geometric wall `r31 = 1/2`, and the
half-space inequality `q_+ + δ >= sqrt(8/3)` is not a scratch-side heuristic.
It is the affine rewrite of the retained positive-sheet existence condition.
