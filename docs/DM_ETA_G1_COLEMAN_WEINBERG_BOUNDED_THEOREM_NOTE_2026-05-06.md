...
Audit authority belongs to the independent audit lane. This note should
remain a bounded support surface until that lane checks the scope and
dependency chain.
...
This bounded support theorem derives the Wilson-link Coleman-Weinberg
structure underlying the V1 operator-level bridge proof. The one-loop
expansion of the SU(3)-gauged Cl(3) chiral cube's Wilson kinetic kernel
`U_mu + U_mu^dagger` to `O(g^2)` gives the per-link Casimir `C_F = 4/3`;
forward plus backward Wilson-hop pairing per direction doubles this to
`rho_{adj/c} = 2 * C_F = (N^2-1)/N = 8/3`. The equivalent per-color-row
Fierz adjoint trace-density reading gives the same answer. Composition
with the cited bare Wilson kinetic mass gives `m_DM = 16 v` on the
canonical surface.

No new axioms or repo-wide dynamical premises are introduced. The
standard one-loop Coleman-Weinberg and gauge-propagator inputs are
explicit imported machinery.
...
The flagship paper line `eta` can cite this note as bounded support for
the gauge-mediated structure: it replaces a structural citation with an
explicit Wilson-link CW expansion. Any publication-facing status change
requires independent audit of the full dependency chain.
...
on the DM-eta G1 dynamical residual. It sharpens the DM-eta G1 lane by
supplying an explicit Wilson-link CW expansion at `O(g^2)` where earlier
text cited CL3 color-automorphism structure. Any downstream parent-status
change requires independent audit of the full dependency chain.
