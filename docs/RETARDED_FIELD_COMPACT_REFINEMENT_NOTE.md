...
chain-closure explanation is exact: "The runner reproduces a
...
assertion runner in the ledger." The audit-stated repair target is
exact: `other` — "change the source status to bounded/support, or
...
the boundary of the conditional perimeter and registers the
...
audit verdict's repair target offers as the smaller of the two
...
the audit-conditional perimeter the verdict cites: a bounded
two-family smoke-probe readout that survives one compact refinement
but is row-sign-dependent on each family.

The audit verdict's repair target offers two acceptable paths: the
