...
**Date:** 2026-05-07
**Status:** retained_bounded / coordinated closure for three open-gate notes
...
The RALA closure theorem promotes three previously open gates from
`open_gate` to `retained_bounded` in the following finite algebraic sense:
...
This is a structural promotion. The earlier note enumerated the result;
this theorem characterizes it as RALA membership and proves it
algebraically.
...
curvature). The RALA closure theorem promotes the *algebraic content* of T1
and T2 to retained_bounded:
...
require physical content beyond the RALA algebra — they are not closed by
this theorem.

## What Remains Open

The following gates are *not* closed by RALA and remain open:
...
The RALA theorem closes the *algebraic backbone* shared by all four; the
...
Claim type: `retained_bounded`. The retained content is the finite
algebraic theorem set T1-T8 above. The retained interpretation is that
RALA(a) is the unique *-subalgebra of single-site operators on the
KS-decomposed Hilbert space whose elements act trivially on the
environment, that the standard teleportation primitives sit inside it
exactly when the retained-axis matches, and that this is the operational
backbone shared across the four physical-implementation gates listed above.
