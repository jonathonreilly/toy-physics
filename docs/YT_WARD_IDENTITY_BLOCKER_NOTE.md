...
What is still missing is the normalization theorem that turns those pieces into
the gauge-matching step used by the `y_t = g_s / sqrt(6)` relation.
...
This repo does not currently derive that identity on the current surface.

## Imported assumptions

The current theorem chain still imports the following:
...
```text
If the Ward identity fixes the Yukawa normalization to the gauge-link
normalization, then y_t = g_s / sqrt(6).
```
