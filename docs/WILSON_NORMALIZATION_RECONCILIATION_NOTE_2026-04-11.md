...
meta`, `independence = cross_family`, and load-bearing step class `A`.
The audit chain-closure explanation is exact: "The methodological
...
differed in the named ways." The audit-stated rationale notes that
...
of the conditional perimeter; nothing here promotes audit status.
...
The 2026-05-05 audit verdict (cited at top) flagged that the
...
| Cache contract | [`scripts/runner_cache.py`](../scripts/runner_cache.py) | declares the SHA-pinned cache header and the per-runner `AUDIT_TIMEOUT_SEC` declaration mechanism that the audit-lane precompute uses to regenerate completed stdout for any of the four runners on demand |

The audit-conditional perimeter is exactly the absence of completed
...
to the four runner sources above. The `Reading Rule` already
