...
admitted-context audit comparators only.
**Status:** audit pending. Under the scope-aware classification
framework, `effective_status` is computed by the audit pipeline from
`audit_status` + `claim_type` + dependency chain; no author-side tier
is asserted in source. The current audit verdict is
`audited_conditional` and audit-lane ratification is required before
any retained-grade status applies. The verdict identifies the
unresolved physical 3D Wilson environment boundary character measure
and the full interacting plaquette reduction at `beta = 6` as the
load-bearing items that prevent this note from being cited as a
retained physical plaquette theorem.
...
items (1)-(2) are the **specific** unclosed dependencies that prevent
...
1. **Physical 3D Wilson environment boundary character measure
   ...
   plaquette is **not** derived in this note. The audit verdict
   identifies this as the principal load-bearing gap.
...
3. **Reference Perron solves.** `P_loc(6) = 0.4524071590` and
   ...
   They appear in this note as reference audit-comparator readouts
   under the explicit structural hypotheses, not as load-bearing
   physical claims.
