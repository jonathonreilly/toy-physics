# Koide BAE Probe 28 — Full Retained Interacting Matter-Sector Dynamics: Bounded Obstruction
...
retained interactions** (Yukawa coupling per `CL3_SM_EMBEDDING_THEOREM`,
...
The retained interacting matter-sector dynamics canonically continues
...
is NOT realized by any retained dynamics, free or interacting. The
...
After Probe 25 (commit `79de75429`) established that the **free**
retained Hamiltonian dynamics canonically selects F3 (real-dim
weighting (1, 2), giving κ=1, NOT BAE), the natural next attack is to
ask whether the **full retained interactions** — which break the
pure-bilinear/Gaussian structure of Probe 25 — can shift the
canonical functional from F3 to F1.

The retained framework has SUBSTANTIAL non-Gaussian interaction content:
...
**Question (Probe 28):** Does adding these full retained interactions
...
ALL retained interactions preserve the (1, 2) real-dim weighting, so
F1 = (1, 1) multiplicity is structurally absent from retained dynamics
...
| RP-H | Reflection positivity → reconstructed Hamiltonian | retained per [`AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29.md`](AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29.md) |
| LR | Lieb-Robinson microcausality on retained Hamiltonian, finite range r=1 | retained per [`AXIOM_FIRST_MICROCAUSALITY_LIEB_ROBINSON_THEOREM_NOTE_2026-05-01.md`](AXIOM_FIRST_MICROCAUSALITY_LIEB_ROBINSON_THEOREM_NOTE_2026-05-01.md) |
| Probe21 | Native bilinear matter Hamiltonian | retained |
| Probe25 | Free-Gaussian dynamics → F3 (real-dim (1, 2)); F1 structurally absent at free level | retained per Probe 25 (commit `79de75429`) |
| WilsonChain | `v_EW = M_Pl × (7/8)^{1/4} × α_LM^{16}` and α_LM = α_bare/u_0; α_bare = 1/(4π); u_0 = ⟨P⟩^{1/4}; ⟨P⟩=0.5934 | retained per [`COMPLETE_PREDICTION_CHAIN_2026_04_15.md`](COMPLETE_PREDICTION_CHAIN_2026_04_15.md) |
| Probe19 | `m_τ = M_Pl × (7/8)^{1/4} × u_0 × α_LM^{18}` with exponent 18 = 16 + 2 = EW+Yukawa-vertex; vertex factor α_bare × α_LM | retained per Probe 19 (commit `f0666719f`) |
| YukawaSM | `Y_e · ψ̄ · H · ψ` couples lepton fields to circulant H on hw=1 | retained per [`CL3_SM_EMBEDDING_THEOREM.md`](CL3_SM_EMBEDDING_THEOREM.md) |
| HiggsCW | `λ(M_Pl) = 0` boundary; radiative quartic from y_t-loop; composite Higgs from taste condensate | retained per [`COMPLETE_PREDICTION_CHAIN_2026_04_15.md`](COMPLETE_PREDICTION_CHAIN_2026_04_15.md) Sec 7 |
| Z3Pot | `V(m) = V₀ + linear + (3/2) m² + (1/6) m³`; cubic piece breaks Z_2 → Z_3 | retained per [`KOIDE_Z3_SCALAR_POTENTIAL_LEPTON_MASS_TOWER_NOTE_2026-04-19.md`](KOIDE_Z3_SCALAR_POTENTIAL_LEPTON_MASS_TOWER_NOTE_2026-04-19.md) |
...
replace (1, 2)), an additional retained interaction would need to
...
retained interaction contributes such a term**.
...
Probe 19 (commit `f0666719f`) gave `m_τ = M_Pl × (7/8)^{1/4} × u_0 ×
...
Combining ALL retained interactions:
...
The structural reason: every retained interaction is C_3-covariant
...
For F3 → F1 to occur, a retained interaction would need to contribute
...
`Herm_circ(3)`. No retained interaction modifies the C_3 representation
structure.** Hence no retained interaction supplies multiplicity-counting (1, 1).
...
```
(a) The retained interactions on Herm_circ(3) — Yukawa Y_e ψ̄.H.ψ,
...
(c) Eight independent retained-interaction-extension routes converge
    on F3 (real-dim weighting (1, 2)):
      ...
      INT-AV8: No retained interaction supplies (1, 1) multiplicity
...
(e) Adding any retained interaction to Probe 25's free-Gaussian
...
Therefore: retained interacting matter-sector dynamics canonically
...
retained content: F1 is absent from BOTH free and interacting retained
...
- `admitted_context_inputs`: `["BAE-condition: |b|²/a² = 1/2 (legacy:
  ...
  retained dynamics — both free (Probe 25) and interacting (Probe 28).
...
3. Demonstrates that no retained interaction contributes a `−log E_⊥`
   term to the effective potential that could shift F3 → F1.
4. Demonstrates that no retained interaction induces a zero mode in
...
5. Sharpens the campaign's terminal residue: F1 is absent from BOTH
   free and interacting retained dynamics. Closing BAE requires a
   non-retained primitive.
6. Cross-validates Probes 12, 13, 14, 18, 21, 25: same conclusion
   under the addition of full retained interactions.
...
2. **The (1, 2) real-dim count is C_3 representation-theoretic on
   ...
   itself, independent of the action. No retained interaction modifies
...
5. **BAE is therefore NOT canonical from retained dynamics, free OR
...
2. **Sharpened residue characterization**: F1 is absent from BOTH
   free and interacting retained dynamics; the (1, 2) count is fixed
...
> `Herm_circ(3)`; no retained interaction modifies it.**
...
- Probe 19 (Wilson chain m_τ): commit `f0666719f`, doc `KOIDE_BAE_PROBE_WILSON_CHAIN_MASS_SHARPENED_NOTE_2026-05-09_probe19.md`
...
- Probe 19 (Wilson chain m_τ): `KOIDE_BAE_PROBE_WILSON_CHAIN_MASS_SHARPENED_NOTE_2026-05-09_probe19.md` (commit `f0666719f`)
- Probe 21 (native bilinear flow): `KOIDE_BAE_PROBE_NATIVE_LATTICE_FLOW_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe21.md`
- Probe 25 (free Gaussian extremization): `KOIDE_BAE_PROBE_PHYSICAL_EXTREMIZATION_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe25.md` (commit `79de75429`)
- Probe 26 (Wilson dim consistency): commit `f1d1594f3`

### Naming convention

- BAE rename note (PR #790, 2026-05-09, commit `2cce201b9`): "Brannen
...
10. Section 9 — INT-AV8: Multiplicity-counting search; doublet block
    has rank 2 in full Hessian; no retained interaction supplies (1, 1).
...
- `feedback_consistency_vs_derivation_below_w2.md`: this note
  ...
  level from the retained C_3-covariance of all retained interactions;
  ...
  equality, not a derivation from retained content.
- `feedback_hostile_review_semantics.md`: this note stress-tests the
  ...
  retained interaction angle. Each INT-AV's action-level
...
- `feedback_compute_speed_not_human_timelines.md`: alternative routes
  ...
  retained dynamics or interactions), not how-long-they-would-take.
- `feedback_special_forces_seven_agent_pattern.md`: this note
  packages a multi-angle attack (eight independent INT-AVs) on a
  single load-bearing structural hypothesis (retained interactions
