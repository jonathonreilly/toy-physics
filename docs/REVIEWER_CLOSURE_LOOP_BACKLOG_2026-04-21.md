...
| 11 | Current-bank quantitative DM observable mapping | **🎯 CLOSED at Nature-grade** | `REVIEWER_CLOSURE_LOOP_ITER11_CURRENT_BANK_DM_MAPPING_CLOSED_NOTE_2026-04-21.md` — 17/17 PASS. All retained current-bank quantities (γ, E_1, E_2, M_{1,2,3}, ε_1, m̃, m_*, k_decay, κ_fit, η_fit/η_obs) map cleanly to quantitative DM observables. η_fit/η_obs = 0.5579 reproduces Planck-measured baryon asymmetry η_obs = 6.12e-10 at O(1) level. **ALL 4 Gate-2 reviewer items now CLOSED.** |
| 12 | Brannen-phase physical bridge via ambient conjugation-odd one-clock Wilson law | **🎯 Bridge B strong-reading REDUCED to Bridge A at Nature-grade** | `REVIEWER_CLOSURE_LOOP_ITER12_BRANNEN_PHASE_AMBIENT_CONJUGATION_ODD_LAW_NOTE_2026-04-21.md` — 18/18 PASS. Explicit ambient `L_odd(A) = arg(b_std(A))` verified conjugation-odd, orientation-sensitive, one-clock, ambient. Selected-line pullback = physical Brannen phase δ to 5 decimals at PDG precision. Retained Brannen reduction theorem: δ = Q/d = n_eff/d² = 2/9 conditional on Q = 2/3. Bridge B strong-reading closes once Bridge A closes. **Two primitives collapse to one.** |
| 13 | Bridge A (Q = 2/3) physical/source-law mechanism attack | **Narrowed further, not closed** | `REVIEWER_CLOSURE_LOOP_ITER13_BRIDGE_A_KOIDE_Q_NARROWED_NOTE_2026-04-21.md` — 15/15 PASS. Q = 2/3 ⟺ 45° cone / equal-isotype-projection (geometric reformulation). Block-total Frobenius on selected-line m parameter RULED OUT (extremum at wrong m). λ_* = 0.5456 doesn't match 5 natural retained-constant combinations. Narrowest residual: elevate one of the 5 iter-2 multi-principle functionals to retained-source-law status, or derive the 4×4 singlet-extension λ(m) non-constant functional. |
| 14 | Bridge A observable-principle test (D = H_sel identification) | **Observable principle with D=H_sel RULED OUT** | `scripts/frontier_reviewer_closure_iter14_bridge_a_observable_principle_test.py` — 7/8 PASS (1 expected FAIL is the hypothesis disproven). W = log\|det H_sel(m)\| monotonic on selected line; no extremum at Koide m_*. Eliminates one candidate identification for Bridge A. Narrowest residual: observable principle with DIFFERENT D, OR λ(m) non-constant, OR multi-principle retention. |
| 15 | v_0 overall lepton scale attack | **Narrowed (0.11% fit, 7/8 double-counting caveat)** | `REVIEWER_CLOSURE_LOOP_ITER15_V0_NARROWED_NOTE_2026-04-21.md` — 10/10 PASS. Retained candidate v_0 ≈ √[v_EW · α_LM² · (7/8)] / (1 + √2 cos(2/9)) gives 17.696 √MeV vs observed 17.716 √MeV (0.11%). Tested 5 alternative retained-constant combinations without (7/8) reuse; all >6% off. Narrowest residual: resolve (7/8) double-counting status, or find equivalent formula without ambiguity. v_0 is downstream of Bridge B via m_*/w/v witness. |
...
8. **Current-bank quantitative DM mapping** — **🎯 CLOSED iter 11**
   - Claim to close: quantitative DM observable mapping from the
     current exact bank.
   - Current state: closed at Nature-grade (iter 11). All 17/17 tests
     pass. η_fit/η_obs = 0.5579 reproduces Planck-measured baryon
     asymmetry at O(1) level from retained bank quantities.
   - Priority: CLOSED.
   - Iter: 11.

**ALL 4 Gate-2 reviewer items now CLOSED at Nature-grade.**
