# PR #230 FH/LSZ Polefit8x8 Chunks037-042 Checkpoint

Status: bounded production support only.

Chunks037-042 of the separate L12/T24 eight-mode/x8 FH/LSZ pole-fit stream
completed with fixed seeds `2026051937` through `2026051942`, selected mass
`0.75`, eight scalar two-point momentum modes, x8 noise, and isolated
production output directories.

The dedicated polefit8x8 combiner now audits `42/63` ready chunks and writes
...
diagnostic fit with `672` saved configurations:
...
monotonicity, with adjacent violations now at least `90.213 sigma`;
finite-row contact-subtraction restoration remains non-identifying, with
representative contact choices shifting the max-q residual by `3153.018` row
