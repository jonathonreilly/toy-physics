...
These exhaust the standard "anomaly cancellation" channels available
...
the anomaly chain than R3. Both are required to fully bar the
...
- Routes A (Koide-Nishiura quartic), D (Newton-Girard), E (Kostant
  Weyl-vector geometry without Casimir-difference normalization)
...
are required to fully bar the flavor-anomaly route to A1.
...
```yaml
actual_current_surface_status: bounded_theorem (flavor anomaly obstruction)
proposed_claim_type: bounded_theorem
audit_review_points: |
  Conditional on:
   (a) independent audit confirmation that the three F1, F2, F3 channels
       constitute a representative sweep of flavor-sector anomaly
       cancellation channels relevant to A1;
   (b) independent audit confirmation that P2-S1 (universal flavor-
       anomaly category mismatch) is a valid sharpening of R3-S1
       distinguishing the two obstructions at independent points in
       the anomaly-route chain;
   (c) independent audit confirmation that the listed upstream
       authorities (CL3_SM_EMBEDDING, gauge-selection,
       Yukawa-no-Ward, C_3-equivariance, KoideCone-algebraic-
       equivalence, R3 anomaly-inflow, SM hypercharge uniqueness)
       are correctly cited and load-bearing only at one hop;
   (d) independent audit confirmation that the claim of "P2-S1
       sharpens R3 at an independent layer" is fairly characterized
       — both obstructions must be present to fully bar flavor
       anomaly closure of A1.
hypothetical_axiom_status: null
admitted_observation_status: |
  A1 admission unchanged. Probe 2 sharpens the obstruction map by
  identifying a structurally distinct flavor-anomaly category-mismatch
  lemma (P2-S1). Routes A, D, E (with caveats) remain open.
claim_type_reason: |
  This probe enumerates flavor-sector anomaly channels and
  identifies P2-S1 as a unifying obstruction that sharpens R3-S1
  at a different layer. It does not close A1; A1 remains
  bounded-tier admission. Probe 2 is itself bounded_theorem (negative
  result on a closure attempt).
...
3. Compress the closure-path map for A1:
   - 4 prior routes (E, A, D, F) barred by prior obstruction notes;
   - flavor-anomaly route now barred by P2-S1 (this note);
   - remaining open candidates: A (Koide-Nishiura quartic), D
     (Newton-Girard structure), and parts of E (Kostant Weyl-vector
     geometry not requiring the Casimir-difference normalization).
...
or (c) explicit user-approved A3-class admission for one of the
