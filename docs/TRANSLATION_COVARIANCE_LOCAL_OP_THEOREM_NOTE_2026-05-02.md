...
supported around x), the framework's translation operator T_a (cited via
...
T_a (cited per N1, supplied by axiom_first_lattice_noether_theorem). Let
...
By N1 of the cited lattice Noether theorem, T_a acts unitarily on H_phys
...
C5. **Microcausality is translation covariant.** The cited
...
derived from a single cited one-hop dependency by elementary lattice
