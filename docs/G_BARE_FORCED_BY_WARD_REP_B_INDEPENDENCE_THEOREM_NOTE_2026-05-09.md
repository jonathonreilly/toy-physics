...
theorem becoming retained-grade. The bounded conditional statement is itself
audit-clean: the load-bearing algebra is class (A) substitution into two
retained / candidate identities. The note does **not** claim that a
...
>    `audited_conditional`),
>
> the unique `g_bare` consistent with both retained identities is
> ```
> g_bare = 1                                                          (FD)
> ```
...
"derived constraint" — under the retained Ward + same-1PI authorities — and
...
|---|---|---|
| [`G_BARE_TWO_WARD_REP_B_INDEPENDENCE_THEOREM_NOTE_2026-04-19.md`](G_BARE_TWO_WARD_REP_B_INDEPENDENCE_THEOREM_NOTE_2026-04-19.md) | `retained_bounded` (cross-confirmed `audited_clean`) | provides the `g_bare`-independent Rep-B form factor `F_Htt^(0)(g_bare) = 1/sqrt(6)` |
...
The conditional upgrade closes immediately under both retained-grade
authorities. The current `audited_conditional` status of the same-1PI
pinning row is acknowledged in the bounded scope: until that row is
ratified, the present note is itself effective `audited_conditional`.
...
proposed_claim_scope: |
  ...
  the unique positive g_bare consistent with both retained identities at
...
to land as an unaudited graph-visible candidate; retained-family effective
status requires independent audit of this row and retained-grade closure of
...
- Retired the historical narrow convention reading
  [`G_BARE_CANONICAL_CONVENTION_NARROW_THEOREM_NOTE_2026-05-02.md`](G_BARE_CANONICAL_CONVENTION_NARROW_THEOREM_NOTE_2026-05-02.md)
...
class (A) algebraic consequence of two retained identities — i.e., a forced
determination, not a free convention.
