...
Therefore the Brannen ansatz IS the correct operator class for
stating A1; the failures of the 8 prior attacks (Routes A, D, E, F
+ Probes 1-4) cannot be attributed to a wrong operator class. They
reflect a genuine structural absence of forcing principle for A1
within the entire C_3-equivariant operator landscape on retained
content.
...
- **Sharpens the "Brannen ansatz wrong class?" question**: prior
  ...
  that none of the three natural class expansions opens a closure
  path. The Brannen ansatz IS the correct operator class.
- **Sister-route implications**: confirms Routes A, D, E + remaining
  Round-2 Probes are the open candidates; the "wider class" criticism
  is now formally answered. The 8 prior attack failures are
  structural, not class-restriction artifacts.
...
- Routes A (Koide-Nishiura quartic), D (Newton-Girard), E
  (Kostant Weyl-vector), and the remaining Round-2 probes retain
  their open status.
