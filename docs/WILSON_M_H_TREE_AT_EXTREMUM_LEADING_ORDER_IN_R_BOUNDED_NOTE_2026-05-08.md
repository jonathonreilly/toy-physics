...
at `u_0 ≈ 0.8776`, i.e. a Wilson coefficient `r ≈ 1/4` would close the
+12% Higgs gap at *leading order in `r`* under the uniform-`N_taste = 16`
admission.
...
| Wilson-shifted total curvature `d^2 V^W/dm^2 \|_{m=-4r} = -4/u_0^2 + 12 r^2/u_0^4 + O(r^4)` | sister forward-reference [`WILSON_VTASTE_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md`](WILSON_VTASTE_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md) | no |
...
| Closure `r` value: setting (3) equal to `m_H_PDG = 125.10 GeV` and solving gives `r / u_0 ≈ 0.268` at leading order; with `u_0 = 0.8776`, `r ≈ 0.235` | scalar algebra | no |
...
- [`WILSON_VTASTE_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md`](WILSON_VTASTE_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md)
  for the curvature at `m^* = -4r`, leading-order in `r`. **Forward-
  reference:** on a sister branch.
- [`WILSON_CORRECTED_V_TASTE_TREE_LEVEL_BOUNDED_NOTE_2026-05-08.md`](WILSON_CORRECTED_V_TASTE_TREE_LEVEL_BOUNDED_NOTE_2026-05-08.md)
  for the explicit `V_taste^W` formula. **Forward-reference:** on a
  sister branch.
...
The runner handles the forward-references gracefully (per the
established pattern: `[INFO]` rather than `[FAIL]` when a sister-branch
note is not yet on `origin/main`).
...
- the value of the Wilson coefficient `r` itself (a separate normalization choice; the `r ≈ 0.235` value is the *leading-order closure value under the channel admission*, not a derivation of `r`);
...
Leading-order closure of the +12% gap occurs at r ≈ 0.235 with u_0 ≈
