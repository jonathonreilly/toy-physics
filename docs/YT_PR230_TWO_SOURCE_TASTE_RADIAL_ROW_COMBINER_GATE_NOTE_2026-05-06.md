...
Chunks001-012 are present and schema-clean.  The set is still partial:
`ready=12/63`, so no combined row packet is written.  The finite-mode
`rho_sx`/`Delta_sx` scouts are recorded for the ready chunks only.

Chunks013-014 are active under the row-wave supervisor's two-worker cap.  Live
