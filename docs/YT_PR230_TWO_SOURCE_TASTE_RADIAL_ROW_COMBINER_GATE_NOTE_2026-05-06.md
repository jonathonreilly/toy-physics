...
Chunks001-008 are present and schema-clean.  The set is still partial:
`ready=8/63`, so no combined row packet is written.  The finite-mode
`rho_sx`/`Delta_sx` scouts are recorded for the ready chunks only.

Chunks009-010 are active under the row-wave supervisor's two-worker cap.  Live
