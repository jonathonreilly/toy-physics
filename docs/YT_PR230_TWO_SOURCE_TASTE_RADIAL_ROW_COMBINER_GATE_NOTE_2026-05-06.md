...
Chunks001-020 are present and schema-clean.  The set is still partial:
`ready=20/63`, so no combined row packet is written.  The finite-mode
`rho_sx`/`Delta_sx` scouts are recorded for the ready chunks only.

Chunks021-022 are active under the row-wave supervisor's two-worker cap.  Live
