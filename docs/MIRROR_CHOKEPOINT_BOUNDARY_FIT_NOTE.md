...
at `N = 120`, and has a weak descriptive fit on the four selected fit rows. The
fit is not used to select the card or the fit rows.
...
`+0.0000±0.000`, so the row is excluded and is not included in the fit.
...
The registered certificate recomputes the unrounded values from the selected
fit rows:
