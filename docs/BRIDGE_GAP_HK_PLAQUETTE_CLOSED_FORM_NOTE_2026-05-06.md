...
This bounded theorem inherits Block 01's conditionals: canonical
trace form, `g_bare = 1`, heat-kernel as a candidate action form, and
leading-order small-`U` matching as the time-selection criterion.

The closed form is for the single-plaquette heat-kernel evaluation only.
It does not compute the multi-plaquette or thermodynamic value. Schur
orthogonality and the heat-kernel character expansion are standard
mathematical machinery; no PDG or lattice-MC value is load-bearing.
