...
on hw=1** can derive `m_e` and `m_μ` from `m_τ` plus retained content
alone, and whether the resulting triplet automatically satisfies
Koide `Q = 2/3` (= BAE) at the spectrum/triplet level.
**Authority role:** source-note proposal; effective_status set only
...
- **"BAE"** = Brannen Amplitude Equipartition, the amplitude-ratio
  ...
  meta; see commit `2cce201b9`, PR #790).
...
Any closure must come from already-retained content. PDG charged-lepton
...
If the framework's retained content uniquely determines `(a, |b|, arg(b))`
...
| Spectrum | `λ_k = a + 2\|b\| cos(arg(b) + 2πk/3)` | retained per same source |
| Bridge | `a₀² − 2\|z\|² = 3(a² − 2\|b\|²)` (after rescaling) | retained per [`KOIDE_CIRCULANT_CHARACTER_BRIDGE_NARROW_THEOREM_NOTE_2026-05-09.md`](KOIDE_CIRCULANT_CHARACTER_BRIDGE_NARROW_THEOREM_NOTE_2026-05-09.md) |
...
| Cone3Form | Cone localization ⟺ Q=2/3 (polynomial T2 identity) | retained per [`KOIDE_CONE_THREE_FORM_EQUIVALENCE_NARROW_THEOREM_NOTE_2026-05-02.md`](KOIDE_CONE_THREE_FORM_EQUIVALENCE_NARROW_THEOREM_NOTE_2026-05-02.md) |
| Z3Pot | V(m) = V₀ + lin·m + (3/2)m² + (1/6)m³ | retained per [`KOIDE_Z3_SCALAR_POTENTIAL_LEPTON_MASS_TOWER_NOTE_2026-04-19.md`](KOIDE_Z3_SCALAR_POTENTIAL_LEPTON_MASS_TOWER_NOTE_2026-04-19.md) |
...
retained content; it is curve-fitting to PDG comparators.)
