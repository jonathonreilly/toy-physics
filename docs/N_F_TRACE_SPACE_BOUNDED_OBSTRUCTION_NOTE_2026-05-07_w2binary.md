...
and **documents the structural obstruction that prevents closure under
Cl(3) + Z^3 primitives**.
...
The present note's verdict: **the binary reduction cannot be closed
to `N_F = 1/2` from Cl(3) + Z^3 primitives alone**. Reduction is
possible only *conditionally*, under a framework-level identification
(matter rep = V_3, irrep-trace convention, or per-site Cl(3)
identification). Each such identification is itself a non-trivial
...
3. **Conditional V_3 selection chain.** Under any one of:
   ...
   `N_F = 1/2` follows. None of these is itself derivable from
...
Bridging these two retained results to a single positive theorem
...
```yaml
target_claim_type: bounded_obstruction
proposed_claim_scope: |
  The binary trace-surface admission N_F in {1/2, 1} from the prior W2
  reduction (V_3 vs V trace) cannot be reduced to N_F = 1/2 from
  Cl(3) + Z^3 primitives alone. Eight independent attack vectors are
  enumerated; six give positive V_3 selection conditional on
