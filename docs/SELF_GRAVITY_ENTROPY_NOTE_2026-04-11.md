...
imported from the retained parity-coupled staggered self-gravity lane
documented in:

- `docs/STAGGERED_3D_SELF_GRAVITY_SIGN_NOTE_2026-04-11.md` (runner:
  `scripts/frontier_staggered_3d_self_gravity_sign.py`) — the
  parity-coupled 3D staggered self-gravity sign lane and its retained
  operating point.
- `docs/SELF_GRAVITY_BACKREACTION_CLOSURE_NOTE.md` — the bounded
  backreaction-closure no-go that pins the strict reduction/Born
  controls used here as a baseline.
- `docs/POISSON_SELF_GRAVITY_MECHANISM_NOTE.md` — the upstream
  Poisson-like mechanism that fixes the `MASS = 0.30, MU2 = 0.22,
  DT = 0.12, G_SELF = 50.0, N_ITER = 20` operating point.

This note is a **negative / inconclusive entropy diagnostic** layered on
top of the retained lane above; it does not modify any of those
authorities. The bipartition-entropy observable is bounded above by
`ln(2) = 0.6931` and is therefore a single-particle subsystem-occupancy
diagnostic, not an area-law-quality entanglement measure (see "Honest
Conclusion").
