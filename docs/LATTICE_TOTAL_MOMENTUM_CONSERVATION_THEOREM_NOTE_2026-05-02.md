...
**Claim scope:** integrated total lattice momentum P_total^μ(t) = sum over spatial sites x at fixed time t of P^μ_x is exactly conserved across time slices on the cited framework lattice action: P_total^μ(t_2) = P_total^μ(t_1) for any t_1, t_2 in the periodic time block, on shell.
...
- [`AXIOM_FIRST_LATTICE_NOETHER_THEOREM_NOTE_2026-04-29.md`](AXIOM_FIRST_LATTICE_NOETHER_THEOREM_NOTE_2026-04-29.md)
  — provides the cited lattice Noether identity:
...
No physics conventions admitted beyond what the cited lattice
...
the canonical staggered lattice momentum density from the cited
...
Then, assuming the cited lattice Noether authority:
...
on the framework's cited Noether surface.

## Proof

The proof is a one-step integration of the cited lattice Noether
identity (N1) over a fixed time slice.

### Step 1 — local divergence identity (cited from N1)

By the cited lattice Noether theorem N1
([`AXIOM_FIRST_LATTICE_NOETHER_THEOREM_NOTE_2026-04-29.md`](AXIOM_FIRST_LATTICE_NOETHER_THEOREM_NOTE_2026-04-29.md)),
...
admitted beyond what the cited lattice Noether theorem already
provides.

## Corollaries

C1. **Crystal momentum is a quantum number on the framework lattice.**
Single-particle states on the framework's cited lattice can be
...
surface and would be a corollary of (M1)+(M2)
plus that.

C4. **No mass-energy / momentum mixing at the bare level.** Energy
and momentum are independently conserved on the cited surface.
Their relationship is the standard `E² = p² + m²` dispersion
relation in the smooth-limit Lorentz regime (separate cited
work).

## Honest status

**Positive theorem on the cited surface.** Steps 1–5 close from
the cited lattice Noether theorem alone, plus the elementary
discrete divergence identity. No physics admission. Single one-hop
chain.

The runner sanity-checks the finite-lattice identities used by (M1)–(M3) by:
