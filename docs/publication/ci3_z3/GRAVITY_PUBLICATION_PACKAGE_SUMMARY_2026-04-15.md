...
The current gravity package has three layers:

1. **Retained weak-field core**
   - Poisson / Newton gravity on `Z^3`
   - weak-field WEP
   - weak-field time dilation

2. **Retained restricted strong-field theorem**
   - exact closure on the star-supported finite-rank class
   - exact shell source
...
3. **Direct-universal discrete global closure**
   - exact discrete `3+1` Einstein/Regge stationary action family
...
- “exact discrete `3+1` Einstein/Regge closure on `PL S^3 x R`”
- “restricted strong-field closure on the star-supported finite-rank class”

Avoid unless the relevant separate theorem is explicitly present:
