...
| Higgs / vacuum package | Higgs package | `m_H=119.8 GeV` (2-loop support route), `125.3 GeV` (framework-side 3-loop route), qualitative vacuum stability | `125.25 GeV`, SM metastability comparator | bounded | one computed lattice input + derived running | bounded quantitative lane; mechanism derived and framework-native 3-loop Higgs implementation exists, but the package still inherits the bounded `y_t` / QFP route | [HIGGS_VACUUM_PROMOTED_NOTE.md](../../HIGGS_VACUUM_PROMOTED_NOTE.md) | n/a |
