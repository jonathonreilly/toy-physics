...
| Down-type mass-ratio CKM-dual lane | retained structural bridge-identity theorem + bounded quantitative mass-ratio readout (confinement-pattern split status) | bridge-identity theorem: `\sqrt{6}` in `|V_{cb}|_{atlas}` and in the `5/6` bridge is the retained Ward-theorem Clebsch-Gordan `\sqrt{N_c \cdot N_{iso}}` on `Q_L`, the GST exponent `1/2 = 1/n_{pair}` and the `5/6` bridge exponent are fixed by the retained atlas `\{n_{pair}, n_{color}, n_{quark}\} = \{2, 3, 6\}` and the `1 + 5` orthogonal-complement projector split on the six-state quark block, GST `|V_{us}| = \sqrt{m_d/m_s}` and `5/6` `|V_{cb}| = (m_s/m_b)^{5/6}` are exact algebraic identities on the retained identification surface, and the `C_F - T_F = 5/6` Casimir combination is a numerical coincidence check (not the retained origin); quantitative mass-ratio readout `m_d/m_s = alpha_s(v)/2`, `m_s/m_b = [alpha_s(v)/sqrt(6)]^{6/5}`, `m_d/m_b = alpha_s(v)^{11/5} / (2 \cdot 6^{3/5})` inherits the bounded mass-hierarchy systematic; framework-internal RG/transport derivation of the identification surface and theorem-grade scale-selection rule remain open | [CKM_DUAL_BRIDGE_IDENTITY_THEOREM_NOTE_2026-04-17.md](../../CKM_DUAL_BRIDGE_IDENTITY_THEOREM_NOTE_2026-04-17.md), [DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md](../../DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md), [CKM_FIVE_SIXTHS_BRIDGE_SUPPORT_NOTE.md](../../CKM_FIVE_SIXTHS_BRIDGE_SUPPORT_NOTE.md) | matrix row `Down-type mass-ratio CKM-dual lane` |
