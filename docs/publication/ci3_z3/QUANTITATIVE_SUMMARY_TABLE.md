...
| down-type CKM-dual mass-ratio lane | `m_d/m_s = 0.05165`, `m_s/m_b = 0.02239`, `m_d/m_b = 0.001156` | PDG mixed-scale comparators `0.05000`, `0.02234`, `0.001117` | `+3.3%`, `+0.2%`, `+3.5%`; same-scale `m_s(m_b)/m_b(m_b)` remains `+15.0%` away | bounded secondary flavor-mass lane | promoted CKM closure plus GST and bounded `5/6` mass-ratio bridge; no observed masses as derivation inputs, but the same-scale interpretation of `m_s/m_b` remains open | [DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md](../../DOWN_TYPE_MASS_RATIO_CKM_DUAL_NOTE.md) |
