...
|---|---|---|---|---|
| `S^3` compactification / topology closure | bounded `S3_*` authority on `review-active`; currently [S3_GENERAL_R_DERIVATION_NOTE.md](/Users/jonreilly/Projects/Physics/.claude/worktrees/youthful-neumann/docs/S3_GENERAL_R_DERIVATION_NOTE.md) is the cleanest family theorem | current `S3_*` runners on `review-active` and `youthful-neumann` | bounded/open | compactification uniqueness and framework-level selection are still open |
