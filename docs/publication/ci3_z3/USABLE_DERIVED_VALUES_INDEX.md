...
The updated local/environment factorization theorem then strips off the exact
local Wilson marked-link contribution, and the updated Perron/Jacobi
underdetermination theorem shows that the remaining open object is specifically
the residual environment-response sequence `E_6`, equivalently the Perron
eigenvector / moments of the factorized operator
