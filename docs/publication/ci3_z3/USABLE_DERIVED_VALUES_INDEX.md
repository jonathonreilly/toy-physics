...
So the remaining open object is now the explicit nonpolynomial connected
plaquette hierarchy at `beta = 6`, whose first projection is the connected
Wilson plaquette susceptibility profile. The new infinite-hierarchy
obstruction shows that no exact finite-order cumulant truncation can close
that object.
