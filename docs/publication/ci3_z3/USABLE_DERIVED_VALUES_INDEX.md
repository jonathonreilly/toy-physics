...
So the remaining open object is now the explicit spectral measure, or
equivalently the explicit nonpolynomial connected plaquette hierarchy, at
`beta = 6`, whose first projection is the connected Wilson plaquette
susceptibility profile. The new infinite-hierarchy obstruction shows that no
exact finite-order cumulant truncation can close that object, and the new
spectral-measure theorem shows that the equivalent exact generating object
already exists uniquely on each finite Wilson surface.
