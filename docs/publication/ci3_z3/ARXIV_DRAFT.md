...
**Theorem 4 (Strong CP on the retained action surface).**
...
The two legs are structural:

1. $\det(D+m)$ carries no phase for real $m$ because the staggered operator is
   anti-Hermitian and its spectrum pairs as $m\pm i\lambda$;
2. the gauge action is the CP-even Wilson plaquette action [9], so no bare
   $\theta$ term appears on the retained action surface.
...
with each factor real and positive on that surface. The new support stack now
includes an explicit $3+1$ APBC determinant-positivity audit and an exact
fermion-effective-action spectral audit. What is **not** claimed is full
interacting gauge-sector/topological closure beyond that retained action
surface; the theorem is exact on the current action surface, not a universal
all-formulations statement. The positivity logic is aligned with the standard
...
The strong-CP theorem has two structural legs. First, for real masses the
staggered operator is anti-Hermitian, so $D+m$ has spectrum $m\pm i\lambda$ and
$\det(D+m)>0$. Second, the retained gauge action is the CP-even Wilson
plaquette action [9], so no bare $\theta$ term appears. The retained partition
function is therefore real and positive, and the fermion-effective-action audit
shows the CP-odd phase cancels pairwise on the retained $3+1$ APBC surface.
This proves $\theta_{\mathrm{eff}}=0$ on the retained action surface. It does
not, by itself, prove full topological-sector closure beyond that surface.
