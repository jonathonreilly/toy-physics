...
The audit lane has classified this row `audited_numerical_match` with
`auditor_confidence = high`, `chain_closes = false`, `claim_type =
positive_theorem`, `independence = cross_family`, and load-bearing
step class `G`. The audit chain-closure explanation is exact: "The
...
audit-stated repair note is exact: "Re-check with the actual runner
...
audit status. The
`audited_numerical_match` verdict is terminal under
`effective_status_reason = terminal_audit`, so the source note
remains a numerical fit (not a closed theorem) on the bounded `b >=
7` window. The active runner is
...
the bounded `~ 1/b` numerical fit, or move the `audited_numerical_match`
verdict (which is terminal). The bounded interpretation in the
