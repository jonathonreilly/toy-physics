...
C3-trivial-rep (even) and C3-fundamental-rep (odd) split, and a derivable
non-vanishing of the odd part?
...
- the odd modes are nonzero **iff** the projected generator is outside the
  projected `Cl(3)` span
...
3-vector, and a derivable non-vanishing criterion on the odd mode.
...
> ```
> with `ω = exp(2πi/3)`. For real `M`, `v_- = conj(v_+)`. The odd modes
> `(v_+, v_-)` vanish identically when `M` lies in the projected `Cl(3)` span
> (because then `v` is C3-invariant), and they are nonzero on the
...
- `(v_+, v_-)` are the C3-fundamental-rep (odd) modes = conjugate pair on
  real profiles
- `(v_+, v_-)` are nonzero on the demonstrated projected non-`Cl(3)`
  generator and identically zero on any element of the projected `Cl(3)`
  span
...
2. the C3-fundamental-rep (odd) Fourier modes form a conjugate pair on real
   profiles
3. the odd modes vanish identically on the projected `Cl(3)` span and are
   nonzero on the demonstrated projected non-`Cl(3)` commutant generator
...
vanishing/non-vanishing criterion for the odd mode.
