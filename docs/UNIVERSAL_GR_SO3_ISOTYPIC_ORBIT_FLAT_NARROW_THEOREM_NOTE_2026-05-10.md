...
>
> 1. **(T1) A1 block is pointwise SO(3)-invariant:** `Pi_A1(h') = Pi_A1(h)`.
...
The framework's axiom set (the public memo `MINIMAL_AXIOMS_2026-05-03`
in plain text, not as a load-bearing markdown link) is `Cl(3) + Z^3`,
and the spatial SO(3) symmetry of the substrate is a derived structural
property; that connection is informational, not load-bearing for the
present theorem.

## Load-bearing step (class (A))

**Step 1 — A1 block is pointwise invariant.** Write `h` in the canonical
`3+1` block form:
...
The A1 block is `(h_{00}, s/3 · I_3)` with `s = tr(h_{ij}) = h_{kk}`.
...
```text
||h'||^2_d  =  tr(h'^T W h' W)  =  tr(R^T h^T W h R · R^T W R · R^T R / R^T R)
```
...
**Step 4 — A1 block energy is invariant.** `Pi_A1(h')` and `Pi_A1(h)`
...
A1 block is pointwise fixed (Step 1) and `||·||^2_d` is additive across
the orthogonal direct-sum `Pi_A1 ⊕ Pi_perp` (the A1 block is supported
...
`||·||^2_d` across the A1 ⊕ perp split). Class **(A)** load-bearing
step throughout.
...
10. **Cited authority is retained-grade.** `MINIMAL_AXIOMS_2026-05-03`
    is the framework axiom set (meta, no upstream).
...
proposed_claim_scope: |
  pure SO(3)-representation-theory / linear-algebra orbit-flat
  invariance: under spatial-block SO(3) action on Sym^2(R^4) and
  isotropic spatial diagonal weight, the rank-2 A1 block (lapse +
  spatial trace) is pointwise invariant and the orthogonal-complement
  weighted energy is orbit-flat, so no quadratic-energy minimization in
  this class can canonically select a unique complement-frame section.
  No physical Einstein-Hilbert / lattice-gravity-action claim.
proposed_load_bearing_step_class: A
audit_required_before_effective_retained: true
```

Audit status is set only by the independent audit lane. If the theorem
is ratified and dependencies remain retained-grade, the audit pipeline
derives the retained-family positive effective status from `claim_type`
...
isotropic spatial weight is now a standalone retained-grade fact (after
audit ratification), independent of the universal-GR route's other
...
`MINIMAL_AXIOMS_2026-05-03` is the framework's two-axiom memo
(`Cl(3) + Z^3`), where the spatial SO(3) symmetry appears as a derived
structural property of the substrate. The present narrow theorem
isolates a pure linear-algebra orbit-flatness fact and does not consume
that axiom set as a load-bearing premise.
