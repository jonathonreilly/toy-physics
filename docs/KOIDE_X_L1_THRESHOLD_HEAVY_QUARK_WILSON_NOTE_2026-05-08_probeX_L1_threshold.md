...
**Date:** 2026-05-08
**Type:** bounded_theorem (negative result on Wilson-chain extension to heavy-quark mass scales)
**Claim type:** bounded_theorem
**Status:** source-note proposal — Probe X-L1-Threshold of the
Lane 1 (alpha_s) bridge campaign. Tests whether the retained
Wilson chain `m = M_Pl * (7/8)^(1/4) * u_0 * alpha_LM^n` from
...
from a literature import to a retained derivation.
**Authority role:** source-note proposal; effective status set only by
the independent audit lane.
**Loop:** koide-x-l1-threshold-heavy-quark-2026-05-08
...
not promote any downstream theorem. The verdict is **negative**: the
Wilson chain in the Probe-19 form does **not** reach heavy-quark mass
scales with integer or simple-rational exponents.
...
themselves are not derived from retained Cl(3)/Z^3 content.
...
for some retained-derivable integer or simple-rational `n_q` for each
of `m_c`, `m_b`, `m_t` — **then** threshold matching coefficients become
a retained derivation rather than a literature import, closing a known
...
- **No new axioms** beyond retained `Cl(3)/Z^3` (A1 + A2).
- **No new imports** beyond the retained Probe-19 chain.
...
## Setup — retained Wilson chain (no derivation, no admission)
...
| Symbol | Value | Origin |
|---|---|---|
| `<P>` | 0.5934 | SU(3) plaquette MC at beta=6 (retained) |
| `M_Pl` | 1.221 * 10^19 GeV | Framework UV cutoff (retained) |
| `alpha_bare` | 1/(4 pi) ~= 0.07957747 | Cl(3) canonical (retained) |
| `u_0` | `<P>^(1/4)` ~= 0.87768 | Lepage-Mackenzie tadpole (retained) |
| `alpha_LM` | `alpha_bare/u_0` ~= 0.09067 | Geometric-mean coupling (retained) |
| `(7/8)^(1/4)` | ~= 0.96717 | APBC eigenvalue ratio (retained) |
| `A := M_Pl * (7/8)^(1/4) * u_0` | ~= 1.0365 * 10^19 GeV | Wilson chain prefix (retained) |

The Wilson chain prefix `A = M_Pl * (7/8)^(1/4) * u_0` is the common
factor across Probe-19's lepton chain, so the question reduces to:

> for each heavy quark `q in {c, b, t}`, is there a retained-derivable
> integer (or simple rational) `n_q` such that `A * alpha_LM^(n_q)`
> matches `m_q` to the same retained-tier precision (~0.01-0.5%) that
...
**Verdict for Test 1:** the Wilson chain in Probe-19's exact form does
NOT extend to heavy-quark mass scales with integer exponents.

## Test 2 — does any retained simple structural factor close the gap?

If `m_q = A * alpha_LM^(n_q) * F_q` for integer `n_q` and a simple
retained factor `F_q`, perhaps the Wilson chain still extends with
quark-specific structural prefactors.

**Candidate retained factors `F`:** `1/sqrt(2)`, `1/sqrt(3)`, `1/sqrt(6)`
...
- The factors `1/sqrt(2)` and `1/sqrt(3)` are not picked out by any
  retained generation-distinguishing principle.
...
**Verdict for Test 2:** no consistent retained simple structural factor
...
**Verdict for Test 3:** the Brannen-BAE-circulant mechanism that closes
the lepton triplet at Probe 19 does NOT extend to either quark sector.

## Test 4 — sanity check: m_t = v_EW / sqrt(2) (y_t = 1 attractor) ?

The retained framework derives m_t through a separate route in
...
(the y_t IR quasi-fixed-point attractor, retained), and the additional
...
explicitly establishes that the species-uniform reading of the retained
...
absolute m_b scale. This is a known retained admission separate from
the threshold-matching question.

**Verdict for Test 4:** the m_t scale is closed retained but via a
...
**(R3) Lane 1 admission landscape clarified.** The retained chain has
m_t closed via the y_t QFP + RGE route (separately retained).
...
1. **Promote any retained theorem.** No retained theorem is modified.
2. **Add a new axiom.** A1 + A2 still suffice on the retained stack.
...
4. **Falsify any retained Probe-19 theorem.** Probe 19's m_tau closure
...
5. **Falsify the retained y_t / m_t chain.** The retained m_t prediction
...
1. **Records a clean negative result.** Probe-19's exact Wilson chain
   ...
   and do not admit consistent retained simple-rational corrections.
...
2. **Sharpens the L1 bridge admission landscape.** Heavy-quark
   ...
   converting them to retained derivations.
...
4. **Provides a quantitative falsification basis** for any future
   ...
   mechanism must reach <0.5% precision to match the retained tier.
...
### Retained Wilson chain (Probe 19 source)
...
Runner verifies:
1. Retained Wilson chain constants reproduce Probe-19 reference (m_tau
   to 0.017%).
...
3. Test 2: no consistent retained simple structural factor closes
...
4. Test 3: up-type and down-type quark Koide Q values are NOT 2/3
   ...
   BAE-circulant.
5. Test 4: sanity checks on the retained y_t / m_t alternative chain
...
7. Honest negative verdict recorded.
...
3. m_b remains a known retained admission (species-differentiation
   primitive); m_c and threshold matching coefficients are literature
   imports unless explicitly derived.
4. Any candidate mechanism must reach retained-tier precision
