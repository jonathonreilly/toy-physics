...
bounded_theorem`, and load-bearing step class `C`. The audit
chain-closure explanation is exact: "The one-hop scout dependency is
...
runs, including a narrowed full-window invocation." The audit-stated
repair target is exact: `runner_artifact_issue` — "provide a current
...
The 2026-05-03 audit verdict (cited at top) flagged a
...
SHA-pinned cache. The audit-conditional perimeter is exactly the
