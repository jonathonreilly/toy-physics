# Primitive P-L1 Channel Weight — Candidate-Primitive Design Note for QCD β_2/β_3 Scalar Channel-Weight Terminal Admission

**Date:** 2026-05-10
**Claim type:** primitive_design_proposal (no audit promotion claim)
**Sub-gate:** Lane 1 (alpha_s) — terminal admission on β_2 / β_3 scalar
channel weights at 3-loop and 4-loop QCD running.
**Status authority:** independent audit lane only; effective status is
pipeline-derived. This note proposes candidate primitives for review;
it does not claim retention of any new content.
...
## 0. Context — what is the terminal admission?

The Lane 1 strong-coupling running chain currently relies on the standard
SM RGE with `β_0 = 7, β_1 = 26` (retained) at `N_f = 6`, and
literature-imported `β_2, β_3` (TVZ 1980; van Ritbergen-Vermaseren-Larin
1997 in MSbar). The 4-attack-type terminality campaign closed:

- **X-L1-MSbar** ([`KOIDE_X_L1_MSBAR_NATIVE_SCHEME_NOTE_2026-05-08_probeX_L1_msbar.md`](KOIDE_X_L1_MSBAR_NATIVE_SCHEME_NOTE_2026-05-08_probeX_L1_msbar.md),
  PR #934) — neither MSbar dim-reg nor lattice perturbation theory
  (LPT) primitives are retained; the framework retains the 9-channel
...
- **V-L1-Quartic** ([`KOIDE_V_L1_QUARTIC_CASIMIR_BETA2_NOTE_2026-05-08_probeV_L1_quartic.md`](KOIDE_V_L1_QUARTIC_CASIMIR_BETA2_NOTE_2026-05-08_probeV_L1_quartic.md),
  ...
  retained at SU(3); β_2 has no quartic-Casimir channels; β_3 has
  them but their scalar weights are 4-loop master integrals.
- **U-L1-Resurgence** (PR #989) — Borel-plane renormalon ladder at
  `z* = 4π/β_0 = 4π/7` is retained; Stokes constant `S_IR` and finite
...
- **S-L1-Topological** ([`KOIDE_S_L1_TOPOLOGICAL_CHERN_SIMONS_NOTE_2026-05-08_probeS_L1_topological.md`](KOIDE_S_L1_TOPOLOGICAL_CHERN_SIMONS_NOTE_2026-05-08_probeS_L1_topological.md),
  ...
  reproduces only the already-retained Casimir skeleton at classical
  limit.

The terminal admission, restated cleanly, is:

```
For each of the 6 independent 3-loop QCD Casimir-tensor channels
(C_F^3, C_F^2 C_A, C_F C_A^2, C_A^3, C_F^2 T_F N_f, C_F C_A T_F N_f,
 C_A^2 T_F N_f, C_F (T_F N_f)^2, C_A (T_F N_f)^2),
and for each of the 14+ independent 4-loop channels, the scalar
rational weight (resp. rational + ζ_3 weight) is NOT retained.
```

This terminality is what THIS NOTE is asked to either (a) propose a
candidate primitive for, or (b) honestly conclude that no clean
primitive exists.

## 1. What a candidate primitive must provide

A primitive that closes this admission must, by definition, supply a
**function**
...
A primitive closes the admission **strictly** if it derives ALL six
non-trivial weights jointly from retained content + the new primitive.
A primitive **partially** closes the admission if it derives some
subset (e.g., the pure-gauge `c_AAA = 2857/54` only). A primitive
**weakly** closes the admission if it derives one structural
relation (e.g., `c_AAA + c_FAA + c_FFA + c_FFF = ?`) that is not
already implied by retained content.

## 2. Three candidate primitives

This note designs and stress-tests three candidate primitives. None
are admitted by this note; each is presented with formal statement,
...
**Formal statement.** *Admit the Connes-Kreimer Hopf algebra
`H_CK^{QCD}` of 1-particle-irreducible Feynman graphs of QCD as a
combinatorial primitive. Specifically:*
...
**Derivation of β_2.** With P_L1-A admitted, β_2 is computed by
...
**Assumptions inventory.**
  ...
  1. *1PI Feynman graphs of QCD* form a well-defined combinatorial
     ...
     are themselves retained or derivable).
  2. *Period evaluation `R(Γ)`* admits a well-defined rational +
     `ζ_n` decomposition. Assumes BPHZ regularization is applicable
     to retained content — but the retained substrate is a `Z^3`
     lattice, not 4D Minkowski + dim-reg.
  ...
  4. *The β_n formula* assumes Gell-Mann–Low evolution applies to
     the framework's running coupling. This is itself an admission:
     the framework's Cl(3)/Z³ substrate has a discrete spatial
     structure, NOT a continuous renormalization-group flow.

**What-if-wrong analysis.**

  - *Wrong-Assumption 1 (1PI Feynman graphs).* If QCD's 1PI graph
    species is NOT retained, the primitive cannot be applied. Currently
    the framework retains an `S^P` plaquette action (lattice gauge
  ...
  - *Wrong-Assumption 2 (BPHZ-applicable).* If BPHZ is applicable to
    Z³ lattice Feynman expansions only modulo a regulator translation,
    then `R(Γ)` would carry lattice spacing-dependence that is
    NOT pure rational. **Open obstruction**: this is exactly the
    regulator dependence the X-L1-MSbar probe identified.
...
*primitive for closure*, it admits the entire QCD perturbative
...
master-integral evaluation step the X-L1-MSbar probe identified as
the missing primitive. Admitting P_L1-A without an independent
period-evaluation primitive amounts to a notational rewrap of
the existing terminal admission.
...
sharpens the *structure* of the terminal admission but does not
close it.

### Primitive P_L1-B — Lattice-<P>-Period Bootstrap Primitive

**Formal statement.** *Admit a single new structural primitive
...
**Assumptions inventory.**
  ...
  1. *⟨P⟩_HK_SU(3) Taylor coefficients ARE the running-coupling
     ...
     load-bearing structural admission of P_L1-B.
  2. *The ⟨P⟩-scheme is a valid renormalization scheme* in which
     ...
     the scheme distinction from MSbar is structurally real per the
     X-L1-MSbar Section 6.)
...
**What-if-wrong analysis.**
  ...
  - *Wrong-Assumption 2.* In the ⟨P⟩-scheme, `α^⟨P⟩(β_W)` is
    ...
    this matching is the same lattice-PT computation X-L1-MSbar
    foreclosed.
  - *Wrong-Assumption 3.* Even if we admit the bootstrap formula,
    it would predict β_2^⟨P⟩ = −8/9 ≈ −0.889. The MSbar value at
    `N_f = 6` is `β_2^MSbar ≈ −32.5`. These differ by ~36×; to
    match them via scheme conversion would itself require 3-loop
    LPT integrals — circling back to the X-L1-MSbar admission.
...
real (X-L1-MSbar Section 6). What P_L1-B fails to provide is the
flow-to-flow conversion, not the scheme retention.

### Primitive P_L1-C — Combinatorial Symmetry-Factor Sum-Rule Primitive

**Formal statement.** *Admit the following structural sum-rule for
...
admitted as combinatorial primitives derivable from QCD vertex rules;
the period classes `P_α^{rational}(Γ)` are admitted as a NEW
PRIMITIVE — a function from primitive graph cohomology classes to
...
to:
  ...
  2. Compute each graph's Casimir tensor (Lie-algebra primitive,
     retained — this is the channel-skeleton retention X-L1-MSbar
     already established).
  3. Compute each graph's symmetry factor (combinatorial, retained).
  4. Compute each graph's *period class* (NEW PRIMITIVE).
...
**What-if-wrong analysis.**
  ...
  - *Wrong-Assumption 1 (period-class field).* For 4-loop QCD, the
    ...
    framework's natural scheme, P_L1-C's `Q + Q·ζ_3` admission is
  ...
  - *Wrong-Assumption 2 (period-class computability).* The "graph
    ...
    (Schnetz HyperInt, 2014). The combinatorial primitive admission
    therefore admits a NON-TRIVIAL computational primitive
    (period evaluation = master integral computation). **Critical
    objection**: this is the same primitive X-L1-MSbar identified
    as foreign.
...
the obstruction into two parts:
  - the *combinatorial* part (graph enumeration + symmetry
    factors + Casimir contraction), which IS retained;
  - the *number-theoretic* part (period class), which is the new
    primitive.

If the period-class part can be admitted as a "Brown-Schnetz oracle"
returning the Q[ζ_n] value of any primitive graph period, then
β_2, β_3 are determined. But this admission is essentially
"admit a Feynman period evaluation function," which is what the
X-L1-MSbar bounded admission already names as the missing primitive.
...
class" admission is NOT structurally weaker than the X-L1-MSbar
"3-loop master integral" admission — they are the SAME primitive
in different mathematical clothing (Schwinger-parameter Symanzik
determinants vs. dim-reg integrals). Admitting P_L1-C would
formally close the terminal admission, but at the cost of admitting
a primitive of comparable strength to the original missing one.

**Sub-finding (positive structural).** P_L1-C *does* sharpen the
*shape* of the terminal admission: the missing primitive is a
...
## 3. What this note closes vs. does not close

### Closed (positive findings)

- **Sharper formulation of the missing primitive.** The terminal
  admission can be cleanly stated as: "a function from primitive
  1PI Feynman graph cohomology classes to `Q + Q·ζ_3 + Q·ζ_5 + ...`
  is not retained." This is structurally a NUMBER-THEORETIC
...
- **P_L1-A does not close the terminal admission** — Connes-Kreimer
  organizes BPHZ but does not evaluate periods.
- **P_L1-B does not close the terminal admission** — the
  ⟨P⟩-scheme single-plaquette expansion is not the flow-equation
  derivative beyond 1-loop / 2-loop universality.
- **P_L1-C does not close the terminal admission** — it formally
  reduces it to a Brown-Schnetz period-evaluation primitive of
  comparable strength.

### Honest verdict

**No clean primitive closes the L1 channel-weight terminal admission**
without admitting a new primitive of comparable mathematical strength
to the original missing one. The terminal status of β_2, β_3 scalar
channel weights is genuinely terminal — the "best" candidate (P_L1-C)
sharpens the *shape* of the admission to a "Feynman period oracle"
but does not eliminate it.

This conclusion is consistent with the 4-attack-type campaign:
β_2, β_3 channel weights at 3-loop+ are *structurally irreducible*
from existing retained content. The L1 strong-coupling chain must
...
A primitive that *would* close the admission:

- **Hypothetical P_L1-D (Cl(3)/Z³-Native Period Functor)** — an
  algorithmically specifiable function `P_Cl(3) : 1PI Graph → Q[ζ_n]`
  that:
  1. Reproduces the 3-loop TVZ values `2857/54, ..., 1/2` from
     the Cl(3) lattice substrate alone, *without* importing dim-reg
...
This note inherits the conditional admissions of the existing
retained substrate:

- A1 + A2 (`MINIMAL_AXIOMS_2026-04-11.md`).
...
non-load-bearing (each fails to close the admission cleanly).
...
These are imported authorities for a primitive design proposal;
...
1. **PASS Section 1**: Document the terminal-admission shape (6
   independent 3-loop channels with TVZ rationals; 14+ 4-loop
   channels with VVL rationals + ζ_3) for reference.
2. **PASS Section 2**: Verify the retained `β_0, β_1` reductions
   at `N_f = 6` from S1 + Casimir algebra (cross-check with X-L1-MSbar
   PASS Sections 1, 2).
...
4. **PASS Section 4 (P_L1-B)**: Reproduce the `⟨P⟩_HK_SU(3)` Taylor
   ...
   match the MSbar β-function values `−65/2 = −32.5` (off by ~36×
...
5. **PASS Section 5 (P_L1-C)**: Demonstrate the species
   ...
   primitive.
6. **ADMITTED Section 6**: The terminal admission stands; no
   primitive closes it cleanly. Document the hypothetical P_L1-D
   shape (Cl(3)/Z³-native period functor) as the open design problem.
7. **HONEST verdict**: primitive_design_proposal; NEGATIVE on
   closure; positive on sharpening the *shape* of the open admission.

## 6. Dependencies

- [`MINIMAL_AXIOMS_2026-04-11.md`](MINIMAL_AXIOMS_2026-04-11.md) — A1, A2.
- [`KOIDE_X_L1_MSBAR_NATIVE_SCHEME_NOTE_2026-05-08_probeX_L1_msbar.md`](KOIDE_X_L1_MSBAR_NATIVE_SCHEME_NOTE_2026-05-08_probeX_L1_msbar.md)
  for the original terminal admission this note's primitives target.
- [`KOIDE_V_L1_QUARTIC_CASIMIR_BETA2_NOTE_2026-05-08_probeV_L1_quartic.md`](KOIDE_V_L1_QUARTIC_CASIMIR_BETA2_NOTE_2026-05-08_probeV_L1_quartic.md)
...
- **A framework-native closed form for β_2 or β_3 in any scheme.**
  The honest verdict is that the X-L1-MSbar bounded admission stands
...
- **A new axiom.** The candidate primitives operate on retained
  Cl(3)/Z³ content + admissions of well-known mathematical structures
  (Hopf algebra of graphs; ⟨P⟩-scheme bootstrap; combinatorial
  symmetry-factor sum-rule). All three are shown to admit a primitive
  of comparable strength to the original missing one.
...
| **P_L1-D** (hypothetical) Cl(3)/Z³-native period functor | new science | OPEN DESIGN PROBLEM | not yet known to exist |

| Quantity | Retention status (after this note) | Source |
|---|---|---|
| `β_0 = 7, β_1 = 26` at N_f = 6 | RETAINED | X-L1-MSbar |
| 9-channel 3-loop Casimir skeleton | RETAINED (group theory) | X-L1-MSbar |
| 14+-channel 4-loop quartic Casimir extension | RETAINED (group theory) | V-L1-Quartic + X-L1-MSbar |
...
| Combinatorial sum-rule species factorization | sharpened admission shape | this probe |
...
4. Each of the 6 independent 3-loop Casimir-tensor channels at
   ...
   from the retained `(C_F, C_A, T_F)` Casimirs (parallel to
   X-L1-MSbar Section 3).
...
Expected: a sequence of PASS lines for the support retentions
...
`primitive_design_proposal` with NEGATIVE-on-closure and
POSITIVE-on-shape-sharpening sub-findings.
