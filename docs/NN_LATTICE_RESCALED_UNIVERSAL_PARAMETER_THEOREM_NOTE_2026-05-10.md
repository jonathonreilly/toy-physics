# Rescaled NN Lattice Universal-Parameter Theorem Note
...
This note converts the two harness-specific theorems into a
harness-parameterized universal theorem by:
...
table at canonical. The mechanism is documented in PR #1003 (and #1027/
#1028 closes the residual to <1% via the next-order saddle correction
PR #1007's formula did not need).
...
## Universal-parameter verdict

**Positive theorem at the 10% bounded-comparison band.**
...
a fully harness-parameterized universal statement is closed at the
bounded-theorem level: the formulas are not subtly fixed to the
canonical parameter values, and the structural ~10% gap between the
strict h->0 saddle and the log-linear extrapolation is the same gap PR
#1003 documents at canonical (recovered to <2.5% by the per-h coherent
formula, recovered fully to <1% by PR #1027/#1028's next-order
correction).
...
- claim exact closure of the log-linear-fit `C_arm` constant — that
  closure is in PR #1027/#1028 via the next-order saddle correction;
