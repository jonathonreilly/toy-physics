...
- the high-scale boundary input `lambda(M_Pl) = 0` is consumed as
  *admitted-context, literature-standard* (Gap #7, 2026-05-10): the
  ...
  slogan was retired (PR #937, NJL counterexample) and no theorem-grade
  framework derivation of the boundary is currently in place. See
  [`VACUUM_CRITICAL_STABILITY_NOTE.md`](./VACUUM_CRITICAL_STABILITY_NOTE.md)
  for the open-gate audit and the three candidate routes
...
- `lambda(M_Pl) = 0` is a framework-native / framework-derived Higgs
  ...
  "composite-Higgs / no-elementary-scalar" slogan was retired (PR #937,
  NJL counterexample) and no theorem-grade derivation is currently in
  place. The boundary is consumed as admitted-context input.
