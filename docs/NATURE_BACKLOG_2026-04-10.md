...
**Status:** EXPLORATORY POSITIVE / NOT RETAINED.
Single-seed growth gives `d_eff≈2.03` and strong matter clustering, but node
density does not track `Φ` (`R²≈0.0005`), `d_eff` does not vary systematically
with coupling, and the gravity sign on the grown graph is mixed under the
stronger audit (`shell_prob` AWAY while `edge_radial` can stay TOWARD).
