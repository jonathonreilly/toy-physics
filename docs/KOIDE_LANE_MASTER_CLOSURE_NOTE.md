...
**End-to-end verification:** 142/142 PASS across 20 runners.
...
| 17 | `frontier_koide_a1_quartic_potential_derivation.py` | 5/5 | Koide-Nishiura V(Φ) quartic has unique minimum at A1 (characterization, not derivation) |
| 18 | `frontier_koide_a1_n3_structural_uniqueness.py` | 5/5 | Four natural Q-formulas converge at 2/3 only at n=3 |
| 19 | `frontier_koide_a1_cv_equals_one.py` | 4/4 | A1 ⟺ eigenvalue CV = 1 (characterization, not derivation) |
| 20 | `frontier_koide_a1_block_democracy_max_entropy.py` | 5/5 | A1 ⟺ block-democracy max-entropy (characterization, not derivation) |

**Total: 142/142 PASS** across 20 runners. Each runner independently
verifiable with `python3 scripts/<name>.py`. Master regression:
`python3 scripts/frontier_koide_lane_regression.py` (all 20 runners, 142/142).

Note: runners 17-20 are A1 CHARACTERIZATIONS (equivalent algebraic forms
of the A1 condition), NOT derivations. A1 as a physical source-law remains
the honest open item documented in `KOIDE_A1_DERIVATION_STATUS_NOTE.md`.
...
```bash
# Single-command verification: 142/142 PASS across 20 runners
