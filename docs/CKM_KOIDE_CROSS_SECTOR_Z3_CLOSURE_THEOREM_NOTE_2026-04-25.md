...
- Any Koide-bridge support-tier branch to retained Koide closure.
...
- Numerically, the formal target-class ratios
  `(N_gen − 1)/N_gen²` and `(N_color − 1)/N_color²` both evaluate to `2/9`.
- The framework's `(N_color − 1)/N_color² = 2/9` is independently derivable
  from retained N_color = 3 (basic arithmetic).
- The generation-side target-class ratio `(N_gen − 1)/N_gen²` also
  evaluates to `2/9` from retained N_gen = 3.
- These two target-class evaluations agree at `2/9` because both N_color
  and N_gen equal 3.

This is **numerical agreement** of two target-class readouts, not a
charged-lepton Koide theorem and not a structural unification. The structural
unification motif (shared Z³ provenance) is support-tier reading only.
