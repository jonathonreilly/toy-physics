# 20 Science Frontiers — Full-Repo Brainstorm

**Date:** 2026-04-11
**Source:** Full repo survey + all session results + frontier Wilson / Anderson follow-up

## Top 5 (Highest Impact — Build on Wilson Two-Body)

| # | Frontier | Risk | Key Test |
|---|---------|------|----------|
| 1 | **Wilson distance-law crossover** | Med | does clean `d^-3.4` soften toward `d^-2` with lattice refinement? |
| 2 | **Wilson partner-source / inertial scaling** | Med | does the current sublinear law cross over toward linear mass scaling? |
| 3 | **Hartree N-body classical limit** | Low-Med | Penrose timescale at N→∞ |
| 4 | **Continuum limit of spectral hierarchy** | Med | Do trajectory observables recover at a→0? |
| 5 | **BMV entanglement prediction** | Med | S(ρ_A) growth rate in physical units |

## Mid-Tier (Strong Science, Existing Infrastructure)

| # | Frontier | Risk | Key Test |
|---|---------|------|----------|
| 6 | Ollivier curvature = Einstein equation | High | Δκ ∝ G·T edge-by-edge? |
| 7 | CDT d_s flow quantitative match | Low-Med | σ*(G) → Planck length analog? |
| 8 | Graph selection from spectral action | High | Minimize Tr(f(D/Λ)) → lattice? |
| 9 | Anderson-gravity phase diagram | Low | Crossover line in (G,L) plane |
| 10 | Three-body gravitational dynamics | High | Bound triple forms? |

## Cross-Architecture (Leverage Full Repo)

| # | Frontier | Risk | Key Test |
|---|---------|------|----------|
| 11 | Mirror Z2 on staggered lattice | Med | MI > 0.5 + CLT ceiling broken? |
| 12 | Eigenvalue statistics GOE/GUE/Poisson | Low | Transition at Zeno threshold? |
| 13 | Topological phases of staggered H | Med | Gravity-driven SSH transition? |
| 14 | Entanglement phase diagram FSS | High | G_c converges at large L? |
| 15 | Stochastic Φ (beyond mean-field) | High | Hawking temperature from noise? |

## Ambitious (High Risk, Transformative if Successful)

| # | Frontier | Risk | Key Test |
|---|---------|------|----------|
| 16 | Chiral + staggered hybrid | High | v=1 cone + clean gravity? |
| 17 | Dynamic growth + staggered physics | High | Grown graph d_eff → 4? |
| 18 | Error correction structure | High | Code distance ∝ boundary? |
| 19 | Cosmological perturbation spectrum | V.High | n_s ≈ 0.965? |
| 20 | Born rule from self-consistency | V.High | Only α=2 is stable? |

## Strategic Assessment

**The Wilson two-body line (1-3, 5, 10) is still the single highest-leverage
direction, but its state is now narrower and cleaner.** The open-lattice
Wilson surface supports a real mutual-attraction channel, but the current law
is `|a_mut| ~ d^-3.4` at the default screened operating point. The later
`mu^2` sweep shows that exponent softens toward `-2` as screening is reduced,
so the immediate question is now whether the remaining gap to Newton closes
fully in the weak-screening limit or whether a residual discrete exponent
survives.

**Low-hanging fruit with guaranteed publication value:** Frontiers 7 (CDT),
9 (Anderson phase diagram), 12 (eigenvalue stats) remain strong because they
already have clean bounded results and do not depend on a Newton-law claim.

**The graph selection problem (8, 15, 17) is the theoretical frontier.**
Solving it elevates "toy model on a graph" to "theory of emergent spacetime."

**Cross-architecture hybridization (11, 13, 16) leverages the repo's unique
asset:** 8+ architectures with complementary strengths that no single paper
has exploited.
