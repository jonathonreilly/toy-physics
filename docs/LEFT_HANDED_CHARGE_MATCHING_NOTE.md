...
verdict for this row is `audited_conditional` (2026-05-05); the source
...
2. *specific normalization `(+1/3, -1)` is outside the closed scope of
   ...
   ratio theorem, and any retained promotion of the exact-pattern
...
- Choosing `β = -1` is a **labelling convention**, not a derivation
  from retained framework primitives in this note's restricted
  ...
  scale.
- Until a separate retained authority derives the absolute
  ...
  **convention-normalized corollary** of the retained ratio theorem.
...
either accept this convention or cite a separate retained authority.
