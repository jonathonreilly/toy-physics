...
This note is the selector half of the retained graph-first gauge package. By
itself it does not identify the abelian factor beyond the selected-axis
commutant surface, and it does not promote downstream phenomenology.

What it does remove is the old objection:

> “There is no derived weak-axis selector on the same graph surface.”

On the canonical graph-shift surface, there now is one. The structural color
closure is completed in [`GRAPH_FIRST_SU3_INTEGRATION_NOTE.md`](GRAPH_FIRST_SU3_INTEGRATION_NOTE.md).
