...
**Authority role:** canonical main-branch note for the *structural*
three-generation algebra row (corner/orbit + no-quotient + no-rooting).
The substrate-semantic upgrade is delegated to the cross-referenced
sibling notes; this note does not carry it.
...
**downstream conditional** carried by the cross-referenced sibling
notes and is *not* part of the load-bearing claim of this row.
...
The narrowed load-bearing stack relies on items 1--3 only. Items 4--6
are preserved as cross-references to the conditional substrate-semantic
upgrade carried by sibling notes; they are **not** load-bearing inputs
for the narrowed claim of this row.
...
1. `frontier_generation_fermi_point.py` establishes the exact spectral side:
   ...
   `1 + 3 + 3 + 1` degeneracy structure with `C(3,1)=3`. (load-bearing)
...
*Cross-referenced conditional upgrades (NOT load-bearing for this row):*
...
closure, see `PHYSICAL_LATTICE_NECESSITY_NOTE.md` (cross-reference).

## Validation

Load-bearing runners for the narrowed claim:
...
Cross-referenced runners for the conditional substrate-physicality
upgrade (not load-bearing for this row):
...
- `frontier_physical_lattice_necessity.py`: closed no-same-stack /
  no-same-surface regulator reinterpretation
  (cross-reference, not load-bearing)
