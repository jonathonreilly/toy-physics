# Graviton Spectral Tower Theorem on Retained S³

**Date:** 2026-04-24
**Status:** **retained structural-identity extension theorem** on `main`. Extends the retained graviton-mass theorem ([`GRAVITON_MASS_SPECTRAL_GAP_IDENTITY_THEOREM_NOTE.md`](GRAVITON_MASS_SPECTRAL_GAP_IDENTITY_THEOREM_NOTE.md)) — which packages only the lowest Lichnerowicz TT mode (`l = 2`, the graviton) — to the **full structural tower** on the retained S³ spatial topology. The full tower of spin-2 transverse-traceless compactness masses is a pure-integer-ratio set of structural masses, each satisfying the Higuchi non-ghost bound with a calculable margin.
...
The retained graviton-mass identity theorem promotes only the **lowest** Lichnerowicz TT eigenvalue on `S³` (`λ_2^TT = 6/R²`) to a structural identity `m_g² = 2ℏ²Λ/c²`. The **higher** Lichnerowicz TT eigenvalues sit implicitly in the same retained surface but are not explicitly packaged.
...
1. **(T1) Tower mass identity.** The structural spin-2 TT compactness masses are
   ```text
...
The whole tower is thus pinned by the **same** single open number `R` as the retained `Λ` and `m_g` rows.
...
**No tower mode is currently observable.** The lightest mode (`l = 2`, the graviton) is at `m_g ≈ 3.5 × 10⁻³³ eV`, below any current detector sensitivity by ~10¹⁰ (strongest model-independent bound `m_g < 1.76 × 10⁻²³ eV`). Higher-l modes are heavier by at most `√(l²/6)` but this is still ~10⁻³² eV at `l = 10`, far below observation.

**The ratio identities (T2) are in principle falsifiable** if two or more tower modes are ever detected simultaneously. A ratio inconsistent with `√([l(l+2)−2]/[k(k+2)−2])` for any integer pair `(l, k)` would falsify the retained S³ + spectral-gap structure.
