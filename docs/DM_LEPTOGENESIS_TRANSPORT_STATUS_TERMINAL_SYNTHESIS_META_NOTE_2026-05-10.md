...
- A separate, narrowly scoped bounded theorem note that derives one of
  ...
  `Cl(3)` on `Z^3`) — landed as its own scoped source change,
  audited on its own authority, and only afterwards considered as a
  repair candidate for the corresponding terminal upstream.
