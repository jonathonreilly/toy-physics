...
If this extraction lands at `1/5` ± experimental error (within the
α_s region of confidence), the framework's atlas 1+5 decomposition
is confirmed for that NLO Wolfenstein order.
...
| HL-LHC era sub-percent precision | `~ 1σ` test of the ratio coefficient |

The future precision target is the `1/5` coefficient itself. With
HL-LHC and Belle II Stage II projecting sub-percent precision on
`sin(2 beta)`, and assuming the ratio test framework has a
well-determined `sin(2 beta_0)` reference, the `1/5 = 1/(N_quark - 1)`
coefficient becomes a single-number falsification target for the
atlas 1+5 decomposition.
