...
free-energy density formula. Audit verdict (2026-05-05): `audited_conditional`
on two named missing bridge theorems (the Matsubara free-energy formula
itself + the dimension-4 normalization insertion into the electroweak scale
map).
...
Matsubara free-energy density formula. The audit ledger records
`claim_type: bounded_theorem` with verdict `audited_conditional`; this
in-note framing matches the audit verdict, without proposing any
audit-side `claim_type` revision.
...
bearing-step class A, criticality `critical`, transitive descendants 265).
...
The hierarchy route is still **not fully closed**. The audit verdict
records this row as `audited_conditional` on two named bridge theorems.
