...
coefficient `B(L_t)` has **never been computed in any retained note**.
...
records the structural ratio `|B(L_t=4)|/|B(L_t=2)| = (8/7)²` as a clean
class (A) identity. It does NOT itself promote any λ-UV-anchor claim.
...
slogan, in line with the audit lane's no-go discipline.

### 4.2 The `(8/7)²` ratio as a structural identity

The cross-endpoint identity
`|B(L_t=4)|/|B(L_t=2)| = (A(L_t=2)/A(L_t=4))² = (8/7)²`
...
5. `|B_4|/|B_2| = 64/49 = (8/7)² = (A_2/A_4)²` exactly.
...
proposed_claim_scope: |
  Pure algebraic m^4 coefficient computation on the staggered Dirac
  free-energy density at L_s = 2 APBC mean field:
  B(L_t) = -(1/(4 L_t u_0^4)) * Sum_omega 1/(3+sin^2 omega)^2.
  Specialized at L_t in {2, 4}: B_2 = -1/(64 u_0^4), B_4 = -1/(49 u_0^4),
  with cross-endpoint ratio |B_4|/|B_2| = 64/49 = (8/7)^2.
  Plus a NAMED OBSTRUCTION corollary: the slogan "composite Higgs implies
  lambda(M_Pl) = 0" is structurally retired by the finite, nonzero
  induced quartic at the lattice scale.
...
admitted_context_inputs: []  # the bare m^4 computation requires no new admissions
...
- **Cross-endpoint ratio identity** `|B_4|/|B_2| = (A_2/A_4)² = (8/7)²`
