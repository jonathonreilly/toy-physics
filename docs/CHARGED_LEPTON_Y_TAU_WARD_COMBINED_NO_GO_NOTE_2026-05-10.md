...
- (M5-a) D17-prime on the (2, 1) block — currently unestablished
...
(M5-b), or (M5-c) has been retained.
...
1. A worked SU(2)-Fierz-analog on the (2, 1) block reproducing the
   YT-T1 structure with `sqrt(2 N_w) = 2`, including a verified
   D17-prime on (2, 1).
...
It claims only that **no single-cycle attemptable construction
exists on the current retained framework surface**, with explicit
identification of which structural primitives are missing.
