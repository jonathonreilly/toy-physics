...
**Narrowed claim scope.** Audited the package-level closure formulas in
...
1. the canonical `alpha_s(v) = alpha_bare / u_0^2` with
   ...
   `ALPHA_S_DERIVED_NOTE.md` and `canonical_plaquette_surface.py`
   authorities);
...
- canonical coupling imported from the same-surface CMT plaquette
  derivation (`ALPHA_S_DERIVED_NOTE.md`,
