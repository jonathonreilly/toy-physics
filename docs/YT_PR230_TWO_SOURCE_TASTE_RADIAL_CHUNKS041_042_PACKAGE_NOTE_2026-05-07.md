# PR230 Two-Source Taste-Radial Chunks041-042 Package

**Status:** bounded-support / chunks001-042 packaged; no closure

**Checkpoint certificates:**
`outputs/yt_pr230_two_source_taste_radial_chunk041_checkpoint_2026-05-06.json`,
`outputs/yt_pr230_two_source_taste_radial_chunk042_checkpoint_2026-05-06.json`

## Result

Chunks041-042 now pass completed-mode checkpoints and are included in the
two-source taste-radial row package.  The row combiner reports `ready=42/63`
with `combined_rows_written=false`.

Both chunks preserve the production surface:

- fixed seeds `2026056041` and `2026056042`;
...
chunks043-044 are active run-control only and are not counted as evidence.
...
- Raw `C_ss` still fails the strict scalar-LSZ first-shell Stieltjes
  ...
  `z=161.93089030677183`.
- The Schur split remains bounded support only.  `C_s|x` fails
  (`z=141.83518270927092`), while `C_x|s` survives only the necessary
  first-shell check (`z=-484.2781158031889` in the repair gate).
