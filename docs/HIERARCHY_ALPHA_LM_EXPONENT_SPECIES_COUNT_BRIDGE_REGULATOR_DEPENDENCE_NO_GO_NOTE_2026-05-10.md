...
regulator on `Z^d` at `d = 4` equals the exponent in the framework's
...
(parent narrow theorem PR #1097), but distinct standard regulators on the
same `Z^4` substrate produce distinct physical-species counts (Wilson: 1,
twisted-mass: 2, staggered after Kawamoto-Smit spin-taste decomposition:
4, domain-wall: 1, overlap: 1), and Symanzik improvement / continuum-limit
...
of regulator remains a substrate-imposed admission, recorded in the open
...
> `Z^4` whose continuum limit is the SM, the identification of the
...
> regulator-independent QFT properties on `Z^4` alone, and the
...
- that the framework's hierarchy formula
  ...
  is a separate derivation track on its own admissions);
...
- that any specific framework lane is retracted, demoted, or closed by
  this no-go;
- that the framework's substrate choice (`A1 + A2` plus the staggered
  realization admission) is wrong; the no-go only constrains what the
...
realization" as an open gate, not closed by `A1 + A2` alone. This no-go
note is consistent with that admission: under the open gate, the
...
specific `alpha_LM`, regulator-specific `v/M_Pl`, or admitted regulator-
...
carries its own admissions.

## 7. Audit-named dependencies

| Authority | Role | Status (verify against `docs/audit/data/audit_ledger.json`) |
|---|---|---|
| `NAIVE_LATTICE_FERMION_TWO_POWER_D_SPECIES_COUNT_NARROW_THEOREM_NOTE_2026-05-10` | parent narrow theorem (naive count `2^d`) | `unaudited / positive_theorem` |
| `STAGGERED_DIRAC_REALIZATION_GATE_NOTE_2026-05-03` | substrate-imposed realisation admission | `open_gate / audited_clean` |
...
admission that the framework's regulator choice is substrate-imposed;
this no-go does **not** close, modify, or promote that gate.

No retained framework lane is cited as a load-bearing input; the no-go
...
5. **T5** — at `d != 4`, the naive count is `2^d`, and the bridge would
   ...
   `d=4` substrate-fixing (via `A2 = Z^3` spatial + 1 Matsubara) is the
...
6. **T6** — regulator-independence formal check: three exhaustive routes
   (O1, O2, O3) around the no-go all require substrate-imposed
   admissions.
...
proposed_claim_scope: |
  ...
  uniformly to all renormalisable lattice fermion regulators on Z^4
  with continuum limit SM, the identification "physical species count
...
4. **"The bridge `16 = hierarchy exponent` can be promoted to a
   ...
   substrate selection (admission O1 or O2 in §5), but would not by
...
The counterfactual exercise confirmed the no-go as a class A structural
result on the regulator-independence question, with admissions (O1, O2,
