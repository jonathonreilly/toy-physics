...
3. **(P3, Schwinger 1-loop is ADMITTED, not derived.)** The one-loop
   ...
   covariant photon propagator. The Cl(3)/Z^3 surface has not yet
...
- **Schwinger 1-loop derivation.** The 1-loop universal
  ...
  Cl(3)/Z^3 surface. A future closure would require a retained
...
1. A closed retained `a_μ^SM` from current Cl(3)/Z^3 content (impossible
   today per (P6)).
...
| (P3) Schwinger 1-loop is admitted | Demonstrate that the framework DOES derive `α/(2π)` from retained Cl(3)/Z^3 primitives. The runner checks that no such derivation note exists. |
