...
The fine-`H` evidence is still bounded to `Fam1` and `Fam2`, and the current
`Fam2 seed0` result is only a boundary point while `Fam2 seed1` has the same
resolution control ladder.
