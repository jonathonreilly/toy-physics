# G1 Parity-Mixing Selector Law — Narrower-Gap + Closure-Candidate-Pending-Physics-Cross-Check

**Date:** 2026-04-17
**Branch:** `claude/g1-parity-mixing` (off `claude/g1-complete`)
**Status:** narrower-gap + closure-candidate-pending-physics-cross-check
**Script:** `scripts/frontier_g1_parity_mixing_selector_law.py`
...
selector law on `(delta, q_+)`. The Path-C Z_3-parity split theorem in
...
The Path-C Z_3-parity-decomposition theorem establishes that the active
generators split:
```
...
| 12 | Cubic `Tr(J_act³) = 12 Re(w³)` | retained structural lemma | genuine parity-mixing | Path B already obstructed — m-dependent boundary extremum, joint CPs singular |
...
- **Sufficient (parity-mixing + evades Path C + couples both δ and q_+):** candidates 4, 5 (and 12 in the Path-B-obstructed sense).
- **Insufficient (parity-definite or already-obstructed):** 1, 2, 3, 6, 7 on active sheet, 12 (already Path-B-obstructed).
...
**Significance.** This construction **evades** the Path-C single-parity
obstruction theorem (Path-C Theorem 2 obstructs any scalar depending on
...
the parity-definite class ruled out in Path C.
...
This is the **parity-mixing analog of the Path-A Cubic Splitting
Obstruction (Theorem B)**. The info-geometric Quadratic Unanimity Theorem
from Path A states that all natural info-geometric functionals agree at
...
requires a canonical-functional selection axiom to close G1.

## Verdict

**Classification of all parity-mixing candidates:**

- **SUFFICIENT (passes all three Path A/B/C obstructions, produces a
...
- **INSUFFICIENT:** det K_doublet (saddle, unconstrained crit outside
  ...
  Path-B-obstructed); CP bilinear c_even c_odd (constant on active
...
- evades the Path-C single-parity obstruction by construction,
- is **inequivalent** to Schur-Q `(sqrt(6)/3, sqrt(6)/3)` and to every
  Path-C mixed invariant,
...
|-----------|----------|--------|
| Schur-Q variational | `(sqrt(6)/3, sqrt(6)/3) ≈ (0.8165, 0.8165)` | Path A/B, Schur-baseline minimum-coupling |
| det(H) stationary | `(0.9644, 1.5524)` | Path C-a chamber-interior |
| Tr(H²) chamber-bdy min | `(1.2679, 0.3651)` | Path C-b |
| K_12 char-match | `(0.8000, q_+ free)` | Path C-c |
...
`eta / eta_obs = 1`, it is a compelling candidate closure for G1 that
...
point. If none lands at `1`, the G1 gate is obstructed across all
...
(G-Non-Var)    parity-mixing sub-route TESTED:
               - evades Path C Theorem 2 by sum-of-parity-definite construction
...
candidate points; each requires an additional selection axiom. The G1
...
selection ambiguity", matching the sibling status of Path-A `(G-Var)`.

## Atlas inputs used

All retained / theorem-grade on the G1-complete integration branch:
...
advance on the G1 gap structure: the parity-mixing class is no longer
classified as "untested" — it is now classified as "narrower-gap via
functional-selection ambiguity", matching the sibling status of Path-A
`(G-Var)`.
...
```bash
PYTHONPATH=scripts python3 scripts/frontier_g1_parity_mixing_selector_law.py
```
...
pending-physics-cross-check; G1 remains OPEN with five inequivalent
