...
**Exact closure** of the Poisson-forcing step on the full framework surface.
...
**Theorem.** Let H = -Delta_lat be the nearest-neighbor hopping Hamiltonian on
Z^3. Let G_0 = H^{-1} be the propagator's Green's function. Let L be the
linear operator in the field equation L phi = -rho. If self-consistency
requires L^{-1} = G_0, then L = G_0^{-1} = H = -Delta_lat (Poisson).
...
is the closure condition that makes the framework internally consistent
(the propagator sources the field it propagates in).
...
**Numerical verification (all EXACT, zero BOUNDED):**
...
Total: PASS=12, FAIL=0, BOUNDED=0.
...
|---|---|---|
| **Input** | L is TI + SA + NN | Self-consistency: L^{-1} = G_0 |
| **Method** | Search over operator family | Algebraic inversion |
...
> L^{-1} = G_0, which determines L = H = -Delta_lat (the graph Laplacian)
> by operator inversion. The nearest-neighbor structure of L is a
> consequence of L = H, not an a priori restriction on the operator class.
...
- **GRAVITY_SUB_BUNDLE_NOTE.md**: The sub-bundle note marks Tier 1a (Poisson
  ...
  be upgraded to EXACT CLOSURE, provided Codex accepts the self-consistency
  condition L^{-1} = G_0 as part of the framework's closure logic.
...
Exit code: 0. PASS=12, FAIL=0, BOUNDED=0 (of 12 checks). All EXACT.
