...
- `audit-loop` — before issuing an `audited_conditional` or `audited_no_go`
  verdict whose rationale names walls (independent audit authority; this gate
  prevents the audit lane from inheriting the source-note's overclaim);
