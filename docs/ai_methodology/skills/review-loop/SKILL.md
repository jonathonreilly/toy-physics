...
commit the regenerated files. The CI workflow at `.github/workflows/audit.yml`
enforces the same gate on every push and PR; review-loop must not let a
branch reach merge with pipeline-derived files out of date with the source
notes.
