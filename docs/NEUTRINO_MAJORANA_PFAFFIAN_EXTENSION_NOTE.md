...
- on the retained **local bilinear** lane, the Pfaffian realization is now
  forced, not optional
- what remains open is not the local realization family, but whether the full
  axiom actually turns the charge-`2` primitive on

So the Pfaffian construction should now be read as:

- the unique local bilinear realization of the missing charge-`2` primitive
- but not yet as an axiom-forced primitive existence theorem
