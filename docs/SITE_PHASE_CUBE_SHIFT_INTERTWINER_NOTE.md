...
**Status:** AIRTIGHT — direct plane-wave algebra + bit-flip identification.
**Runner:** `scripts/frontier_site_phase_cube_shift_intertwiner.py` (60/60 PASS)
**Reusability:** the canonical bridge between C^8 taste-cube arguments
and C^{L³} lattice arguments.
...
2. From (1), P_μ on the BZ-corner basis acts as α → α ⊕ e_μ. The
   cube-shift S_μ = σ_x in tensor position μ on C^8 = (C²)^⊗³ also
   acts as α → α ⊕ e_μ in the computational basis. Conjugating by Φ
   identifies the two.

3. Because P_μ and S_μ are intertwined by Φ (isometry), all spectral
   structure of S_μ on C^8 transfers to P_μ on the BZ-corner subspace
   of C^{L³}. The joint eigenbasis of {S_1, S_2, S_3} is the Z_2^3
   Hadamard transform of the computational basis; under Φ this maps
   to the joint eigenbasis of {P_1, P_2, P_3} on the BZ-corner
   subspace, as stated in part (3) of the theorem.
