...
- **carrying an explicit branch-local transport budget:
...
The current branch stack narrows the live transport bound materially below the
...
- but the current branch support stack now narrows the live endpoint budget to
...
- [YT_BRIDGE_MOMENT_CLOSURE_NOTE.md](./YT_BRIDGE_MOMENT_CLOSURE_NOTE.md)
  ...
  window, the accepted response kernel is nearly affine, so the branch closes
...
- [YT_EXACT_SCHUR_NORMAL_FORM_UNIQUENESS_NOTE.md](./YT_EXACT_SCHUR_NORMAL_FORM_UNIQUENESS_NOTE.md)
  closes the next remaining ambiguity at branch scale: within the intrinsic
...
- [YT_MICROSCOPIC_SCHUR_CLASS_ADMISSIBILITY_NOTE.md](./YT_MICROSCOPIC_SCHUR_CLASS_ADMISSIBILITY_NOTE.md)
  closes the remaining branch-scale structural loophole: local positive
...
- [YT_AXIOM_FIRST_MICROSCOPIC_BRIDGE_THEOREM.md](./YT_AXIOM_FIRST_MICROSCOPIC_BRIDGE_THEOREM.md)
  ...
  branch-scale microscopic admissibility, but only the explicit endpoint
...
> lattice blocking flow above `v`. On the current branch support stack, that
...
> class is closed at branch scale. The remaining uncertainty is explicit rather
> than structural.
...
- the exact bridge endpoint is confined to an explicit branch-local budget
  around the selector
