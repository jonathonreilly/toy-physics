...
**Narrowed claim scope.** Audited the algebraic identities `(A1)`-`(A2)` and
...
1. the cited apex coordinates `rho = 1/6`, `eta = sqrt(5)/6` are imported
   from the upstream CKM CP-phase structural-identity authority and not
   independently re-derived in this note;
