...
integer that appeared in the Brocard-angle theorem (B5 of the Brocard
branch: `sin²(ω̄|_LO) = (N_quark − 1)/(N_quark² + N_quark − 1) = 5/41`,
`cos²(ω̄|_LO) = N_quark²/(N_quark² + N_quark − 1) = 36/41`). This is
no coincidence: both Brocard points and the Brocard angle of the same
triangle are tied via `(N_quark² + N_quark − 1)` to the LO Wolfenstein
imaginary part.
...
companion Brocard-angle, Symmedian, Steiner-Marden, and Weitzenbock
branches are referenced for context, but the Brocard-points closed
forms here are derived directly from the retained vertex coordinates
plus classical Brocard-point definition (barycentric `(ac/b, ab/c, bc/a)`).
...
This complements the Weitzenbock form of P (W7c of the Weitzenbock
branch):
...
appears in `sin²(ω̄|_LO) = 5/41`, `cos²(ω̄|_LO) = 36/41` from the
Brocard-angle theorem. This algebraically ties the Brocard angle and
Brocard points of the same triangle.
...
The Brocard-Brocard distance grows mildly with α_s.
...
angle (companion theorem), the Brocard circle (companion theorem from
the symmedian branch), the Steiner inellipse and Marden foci, and the
Weitzenbock excess. The two **Brocard points** themselves — the
classical interior triangle points whose triple-equal-angle property
generates the Brocard angle — were not previously expressed in closed
...
denominator (in B6, B7, and the Brocard-angle theorem's
`sin²(ω̄|_LO)`) is structurally striking: a single irrational-free
integer governs the entire Brocard structure of the LO unitarity
triangle. This is **algebraic rigidity**: the framework's protected-γ̄
and structural-integer inputs force `41 = N_quark² + N_quark − 1` to
appear as the universal Brocard denominator at LO.
...
- Does **not** cite unmerged branches as load-bearing (companion
  Brocard-angle, Symmedian, Steiner-Marden, and Weitzenbock branches
  are referenced only for context in the structural-integer
  observations).
...
- Any unmerged branch (companion Brocard-angle, Symmedian-Brocard-circle,
  Steiner-Marden, Weitzenbock, and Pedoe branches under review are
  referenced only for context in structural observations).
