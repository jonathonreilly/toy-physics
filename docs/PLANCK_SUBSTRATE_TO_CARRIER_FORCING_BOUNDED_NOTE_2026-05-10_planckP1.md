...
not derive that the retained substrate action selects the rank-four
...
selects `P_A` from the support of the retained link-local first variation. That
route is conditional on accepting the staggered-Dirac/Grassmann action source
domain as retained substrate content — which is currently an open gate per
[MINIMAL_AXIOMS_2026-05-03.md](./MINIMAL_AXIOMS_2026-05-03.md).

This note records an **independent** positive route through a different
retained primitive: **reflection positivity** as recorded by the axiom-first
RP support theorem
...
retained RP positivity bilinear singles out `P_A` over `P_3` as the unique
...
sesquilinear factorization — is currently retained at the support level on the
...
action. The framework retains RP on the staggered-only sector unconditionally,
and on the staggered+Wilson sector with the named runner-supported residual.
...
- **Admission RP1.** OS sesquilinear factorization (eqs. (7) and (10) of the
  ...
  retained on `A_min` for the staggered-only sector by closed-form derivation,
...
Under RP1+RP2, this note's main claim is a positive theorem. The bounded label
records the staggered+Wilson runner-supported residual on RP1.
...
- the staggered-Dirac action *source domain* `U = span(u_t,u_x,u_y,u_z)` as
  retained substrate content (that is the link-local first-variation route);
...
The proof uses these retained framework inputs:

- [MINIMAL_AXIOMS_2026-05-03.md](./MINIMAL_AXIOMS_2026-05-03.md):
  framework axioms A1 (`Cl(3)` local algebra) and A2 (`Z^3` substrate).
- [AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29.md](./AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29.md):
  the retained RP support theorem on the canonical action surface.
  Specifically, the **OS sesquilinear factorization** of `<Theta(F)*F>` for
  field-degree-one F.
- [CL3_SM_EMBEDDING_THEOREM.md](./CL3_SM_EMBEDDING_THEOREM.md):
  retained `Cl(3)` automorphism and Cl⁺(3) ≅ ℍ structure on the
...
- [SUBSTRATE_TO_P_A_FORCING_THEOREM_NOTE_2026-04-30.md](./SUBSTRATE_TO_P_A_FORCING_THEOREM_NOTE_2026-04-30.md)
  ...
  note's job is to show that, **over the same 17 candidates**, the retained
...
Therefore: under RP1 and RP2, the retained RP positivity bilinear, applied
...
The retained RP bilinear is sesquilinear: equation (10) of the parent RP
note,
...
irreducible `Cl_4(C)` carrier under retained RP. Therefore `P_3` is excluded
...
- on which the retained RP bilinear `G_A` (statement R1+R2 of the parent
...
The retained RP bilinear `G_A(F, F') = <Theta(F) F'>` on
...
([PLANCK_PRIMITIVE_CLIFFORD_MAJORANA_EDGE_DERIVATION_THEOREM_NOTE_2026-04-30.md](./PLANCK_PRIMITIVE_CLIFFORD_MAJORANA_EDGE_DERIVATION_THEOREM_NOTE_2026-04-30.md))
applies without needing rank equality as a selector. The retained spatial
...
**Bounded positive theorem on retained RP.** The selector criterion
("single-link bilinear sector has full rank-four primitive-cell support") is
derived from retained RP positivity (admissions RP1, RP2) and not assumed.
Under those admissions, `P_A` is the unique rank-four equivariant projector
class compatible with the retained RP positivity bilinear. The bounded label
...
an audit-clean candidate route through retained RP, distinct from the prior
...
surface by bringing in retained RP as an additional load-bearing primitive
not present in the prior symmetry-only enumeration.

| Prior result | Still valid? | What changes here |
|---|---:|---|
| symmetry-only substrate-to-`P_A` no-go | yes | RP is a positivity bilinear, not a discrete symmetry; it is extra retained content |
