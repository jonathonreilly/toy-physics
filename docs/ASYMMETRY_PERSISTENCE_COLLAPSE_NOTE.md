...
of the conditional perimeter; this source edit does not set audit
status or hand-author audit JSON. Generated audit outputs are
regenerated by the review pipeline.
...
This source note does not set audit status or hand-author audit JSON, does not assert a generic collapse rescue, and does not assert
