# Primitive P-HeavyQ Species Differentiation — Three Candidate Proposals

**Date:** 2026-05-10
**Claim type:** primitive_design_proposal (three candidate primitives, no closure)
**Sub-gate:** Heavy-quark species-DIFFERENTIATION primitive on `y_q(M_Pl)` UV
boundary condition.
**Status authority:** independent audit lane only; effective status is
pipeline-derived. This note proposes candidate primitives; it does not
promote any of them to retained.
...
The four-mechanism exhaustion campaign closed:

- **Probe X-L1-Threshold (PR #933):** EW Wilson-chain heavy-quark absolute
  mass — single-coupling chain `m_q = M_Pl × C × α_LM^{n_q}` foreclosed.
- **Probe Y-L1-Ratios (PR #946):** EW-chain heavy-quark mass ratios via
  integer-difference exponents foreclosed.
- **Probe Z-Quark-QCD-Chain (PR #958):** parallel QCD-confinement chain
  `m_q = Λ_QCD × C × α_s^{n_q}` foreclosed for the `(m_t, m_b, m_c)`
...
- **Probe V-Quark-Dynamical (PR #981):** Yukawa-dominated current-mass +
  chiral SSB constituent shift foreclosed; chiral SSB Σ ~ Λ_QCD³ bridges
  <0.2% of the m_b gap with species-uniform Ward BC.
- **Probe U-Heavy-Modular (PR #994):** SL(2,ℤ) modular flavor on Γ(3) at
  τ = ω foreclosed; canonical weight-2 Yukawa matrix at τ = ω is rank-1
  (eigenvalues (9, 0, 0)) — structurally inconsistent with PDG 3-mass
  spectrum.

After this exhaustion, the strategic sharpening (recorded in V-Quark-Dynamical
PR #981) is:
...
This note proposes **three candidate primitives** (P-Heavy-A, P-Heavy-B,
P-Heavy-C) for the species-differentiation gap. It does NOT close the
gap. It explicitly enumerates which existing framework admissions each
candidate inherits, and which new admissions each requires.

## 1. Assumptions exercise (Elon first-principles)

### 1.1 Existing framework admissions retained as background

The framework's retained content for the heavy-quark sector at this date:

| Item | Content | Status |
|---|---|---|
| Z1 | `α_LM = α_bare/u_0 = 0.090668` (geometric-mean coupling) | DERIVED |
| Z2 | `α_s(v) = α_bare/u_0² = 0.1033` (CMT vertex-power chain) | DERIVED |
| Z3 | `g_lattice = √(4π α_LM) = 1.0673` | DERIVED |
| Z4 | Ward identity `y_t(M_Pl) = g_lattice/√6 = 0.4358` | RETAINED |
| Z5 | `m_t = y_t(v) v/√2 = 169.5 GeV` (-1.84% from PDG) | RETAINED |
| Z6 | `v = M_Pl × (7/8)^{1/4} × α_LM^{16}` (hierarchy theorem) | DERIVED |
| Z7 | C_3[111] cyclic structure on hw=1 generation triplet | RETAINED |
| Z8 | Hermitian circulants on T_1: `H = aI + bC + b̄C²` | RETAINED |
| Z9 | Circulant eigenvalues: `λ_k = a + 2|b|cos(arg(b) + 2πk/3)` | RETAINED |
| Z10 | Z_3 Fourier basis diagonalizes C_3[111] (PR landing 2026-05-03) | RETAINED |
| Z11 | Down-type ratio `m_s/m_b = [α_s(v)/√6]^{6/5}` (+0.2% threshold-local) | BOUNDED |
| Z12 | Brannen-Rivero charged-lepton form `√m_k = v_0(1+√2 cos(δ + 2πk/3))` | A1+P1 admission |
| Z13 | A1 equipartition: `2|b|/a = √2` for charged leptons (ρ_lep = √2) | A1 admission |
| Z14 | P1 √m identification: `λ_k = √m_k` not `m_k` | P1 admission |
...
- A derivation of `ρ_q = 2|b_q|/a_q` for the up-type and down-type quark
  sectors (charged-lepton-specific value `ρ_lep = √2` from A1 does not
  extend; Appendix A.3 of `KOIDE_CIRCULANT_CHARACTER_DERIVATION_NOTE`
...
**If wrong:** the BC could be at Λ_QCD (rejected by Probe Z), at the EW
scale `v` (Yukawa+SSB; rejected by Probe V), or at a previously
...
into Probe W-RGFP, foreclosed; but a hybrid where the BC supplies three
...
Under P1 identification `λ_k = √m_k` (P1 admission inherited):
...
**Is the primitive BAE-locked?** No, P-Heavy-A is INDEPENDENT of BAE
(Brannen Amplitude Equipartition, the renamed A1 condition). BAE forces
ρ = √2 universally; P-Heavy-A explicitly relaxes BAE per sector. The
admission cost: replacing 1 universal scalar (BAE) with 4 sector-specific
parameters `(ρ_up, δ_up, ρ_dn, δ_dn)`.

**Hostile-review tier classification.**
- **NEW admissions:** 4 sector-specific real parameters `(ρ_up, δ_up,
  ρ_dn, δ_dn)`. None recognizable as framework constants in current
  inspection.
- **INHERITED admissions:** P1 (√m identification), Z_3 Fourier basis
  on H_{hw=1}.
- **DROPPED admissions:** A1 (BAE) — relaxed from universal to charged-
  lepton-specific.
...
- `ρ_lep = 1.4150` matches `√2` to **0.06%** (consistency with retained
  charged-lepton A1 admission, as expected).
...
- The `2 × F_adj` candidate identification for `ρ_up` is structurally
  ...
  Fierz channel fraction (RETAINED-BOUNDED per
...
4-parameter admission collapses to identifications of the existing
F_adj, APBC, and √2 (BAE) inputs in N_c-graded combinations. This is a
candidate route to retained — pursuing it requires deriving why the up-
...
(Nucl. Phys. B147, 277 (1979)) applied to the Cl(3)/Z³ surface. The key
...
**Is the primitive BAE-locked?** No, P-Heavy-B is INDEPENDENT of BAE,
A1, P1, and the Brannen-Rivero circulant entirely. It uses only the
retained Z_3 generation structure (Z7, Z10) plus the new `n(g, T_3)`
integer assignment.

**Hostile-review tier classification.**
- **NEW admissions:** 1 integer-valued function `n: {gen} × {isospin} →
  ℤ` (5 free integers if `n(top)=0` is fixed and lepton mapping is
  optional).
- **INHERITED admissions:** Ward `y_t(M_Pl) = g_lattice/√6` (Z4),
  α_LM = α_bare/u_0 (Z1).
- **DROPPED admissions:** none from current Brannen-Rivero stack
...
The retained 1PI algebra is preserved (each component still has weight
1/√6 in the singlet wavefunction); the species identification mixes
components with C_g-weighted contractions.
...
×Z_n). The framework version: lattice-bare Cl(3)/Z³ Casimirs (no GUT
embedding), with generation grading from the Z_3 Fourier basis.

**Is the primitive BAE-locked?** No, P-Heavy-C is INDEPENDENT of BAE,
A1, P1. It engages only with the retained Ward Block 6 algebra (Z4 and
the underlying Clebsch-Gordan structure) plus the new C_g function.

**Hostile-review tier classification.**
- **NEW admissions:** 1 generation-graded Casimir function `C_g: {gen}
  × {isospin} → R+` (5 positive reals if `C_t = 1` fixed).
- **INHERITED admissions:** Ward Block 6 1PI algebra (Z4 underlying
  structure), framework Casimirs `C_F, T_F, C_A, F_adj`.
- **DROPPED admissions:** species-uniform Block-6 reading (replaced
  with Casimir-graded reading).
...
|---|---|---|---|---|---|
| P-Heavy-A | 4 (ρ_up, δ_up, ρ_dn, δ_dn) | NO (relaxes BAE) | yes (top fixed by retained Ward Z4) | by construction with 4 fits | Brannen-Rivero, sector-dependent extension |
...
circulant infrastructure (re-uses Z7-Z10, drops A1=BAE per sector).
Best-positioned for "extends existing retained backbone" framing.
Runner verdict: STRONGEST candidate. `ρ_up` matches `2 F_adj = 16/9` at
1.01%; the 4-parameter admission may collapse to ~2 framework Casimir
...
for "species-differentiation as Casimir-graded reading of retained Ward
algebra" framing.
...
- **Any positive closure** of the heavy-quark species-differentiation
  gap. All three primitives admit new species-dependent parameters; the
...
- **Modification of the retained m_t prediction** (Z5 = 169.5 GeV @
  -1.84%). All three primitives normalize to the retained Ward value at
  the top channel.
...
- **Promotion of any Brannen-Rivero or Koide-A1+P1 ingredient** to
  retained. All three primitives explicitly DROP or remain orthogonal
  to BAE/A1; none promote a Brannen-magic-angle or sector-uniform
...
- **Probe X-L1-Threshold (PR #933):** EW Wilson-chain absolute mass
  foreclosed.
- **Probe Y-L1-Ratios (PR #946):** EW-chain mass-ratio integer-difference
  foreclosed.
- **Probe Z-Quark-QCD-Chain (PR #958):**
  ...
  parallel QCD-chain foreclosed.
- **Probe W-Quark-RGFP (PR #1022):** QFP attractor for top mass
  foreclosed.
- **Probe V-Quark-Dynamical (PR #981):** chiral SSB does NOT bridge
  heavy-quark Yukawa gap.
- **Probe U-Heavy-Modular (PR #994):** SL(2,ℤ) modular flavor at τ=ω
  foreclosed.
...
- **Literature anchors:**
  ...
  - Pendleton & Ross, Phys. Lett. B98, 291 (1981) — RG quasi-fixed
    point for top Yukawa (already foreclosed by Probe W-Quark-RGFP
    PR #1022 for the framework UV BC).
  - Hill, Phys. Rev. D24, 691 (1981) — IR fixed point of SM Yukawa.

## 7. Constraints respected

- **No new repo-wide axioms.** The baseline remains Cl(3)/Z³.
- **No new derivational imports.** PDG fermion masses are observational
  targets; no PDG mass enters as a derivation premise.
- **No fitting of new constants to retained surface.** All three
  primitives carry NEW species-dependent parameters whose framework-
  constant interpretation is the open question this note proposes,
  not closes.
