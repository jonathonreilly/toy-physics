...
**Claim scope:** Under the canonical conventions asserted in
...
`Λ = (Z/L_τ Z) × (Z/L_s Z)^{d_s}` with even site count and `n_+ = n_- = n/2`),
the canonical-surface Dirac operator `M = M_KS + M_W + m · I` satisfies
`det(M) > 0` configuration-by-configuration on every SU(3) gauge background
for any `m > 0`. The proof is a structural two-sublattice block
decomposition; no per-configuration numerical input is required.
...
This note supplies the missing bridge **without narrowing** the parent
claim. It proves `det(M_KS + M_W + m · I) > 0` in closed form on the
canonical staggered + Wilson surface under the conventions the parent note
already asserts (specifically: `ε M_W ε = M_W`, parent §3a; mass term
`m · I`, parent eq. (1) and the A3 carrier).
...
unconditional in `m > 0` and in any SU(3) gauge background.

## Setup and conventions

Adopt the parent-note conventions verbatim:

- **A1 / A2 / A3 / A4** as in `MINIMAL_AXIOMS_2026-04-11.md` and the parent
  reflection-positivity note. The lattice block is
...
```text
    ε M ε  =  ε M_KS ε  +  ε M_W ε  +  m · ε² ·  =  -M_KS  +  M_W  +  m · I  (2)
```
...
the symmetric-canonical surface.

## What this does not close

This bridge proves `det(M) > 0` per-configuration on the
**symmetric-canonical** surface where the diagonal blocks of `M_W + m · I`
are equal multiples of the identity (D4). The audit lane should treat
this as a closed bridge **for the canonical-A_min surface as the parent
note explicitly defines it** (the parent's hypothesis-match table fixes
`r = 1`, mass `m · I`, and `M_W` commuting with `ε`).
...
parent note's claim
`det(M_KS + M_W + m · I) ≥ 0 on the canonical surface` (parent §3a,
last paragraph) is now a closed-form theorem under the parent's own
asserted conventions. Adding this dependency edge to the parent note
should resolve the parent's `missing_bridge_theorem` audit condition
...
- Parent: `docs/AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29.md`
- A_min carrier: `docs/MINIMAL_AXIOMS_2026-04-11.md`
