...
dependency edge from this note. The audit verdict is terminal under
`effective_status_reason = terminal_audit`. Nothing in this edit
...
one-hop dependency edge here this row remains `audited_conditional`
and the heuristic is read as derivation-**adjacent** (not derived
