...
an exact transcendental equation in `r`. Bisecting in `Fraction`
arithmetic (canonical `u_0 = 0.8776`, `v = 246.22 GeV`) on the
bracket `[0.26, 0.28]` gives the all-orders closure value
...
Any of (1)–(3) failing voids the closure.
...
| All-orders closure equation: `(m_H_W / v)^2 = (m_H_PDG / v)^2` with `m_H_PDG = 125.10` (comparison input only), giving `(2)` evaluated at unknown `r` equal to a known rational target | algebraic equation in one variable `r` | no |
...
#761, which solves the linear-Taylor closure equation). Relative
...
- [`WILSON_VTASTE_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md`](WILSON_VTASTE_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md)
  ...
  sister branch.
- [`WILSON_M_H_TREE_AT_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md`](WILSON_M_H_TREE_AT_EXTREMUM_LEADING_ORDER_IN_R_BOUNDED_NOTE_2026-05-08.md)
  for the leading-order closure value `r ≈ 0.235`. **Forward-reference;**
  on a sister branch.
...
- the +12% Higgs gap chain in [`HIGGS_MASS_FROM_AXIOM_NOTE.md`](HIGGS_MASS_FROM_AXIOM_NOTE.md). The all-orders closure value `r ≈ 0.269` is **conditional** on:
  1. the uniform-`N_taste = 16` channel admission (non-derived);
  2. the tree-level mean-field formalism (no CW corrections, no RGE
     running);
...
- the value of the Wilson coefficient `r` itself (the `r ≈ 0.269`
  value is the *all-orders closure value under the admissions*, not a
  derivation of `r`);
...
- the *exact* (irrational) closure value of `r`. The bisection result
  `r ≈ 0.26855 ± 10^{-5}` is approximate; the exact closure is
  irrational (it depends on the rational `m_H_PDG / v` and `u_0` in
...
closure value 0.23572. The all-orders shift is non-trivial: leading-
order linear-Taylor under-estimates r by ~14% under the canonical
admissions. Closure is conditional on uniform-N_taste=16 + tree-level
+ non-zero r.
```
