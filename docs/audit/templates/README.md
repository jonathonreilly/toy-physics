...
On `main` pushes and scheduled runs the workflow auto-commits regenerated
audit-data and publication-effective-status views back to the branch as
`audit-bot`. PR runs only surface a `::warning::` annotation if the
pipeline produces a diff (no auto-commit on PRs to avoid write-permission
issues with fork PRs).
