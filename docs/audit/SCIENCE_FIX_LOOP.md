...
while leaving the hard problems for a human. The audit lane verifies
correctness on the next pipeline run; this loop just makes attempts.
...
that a human reviews, merges (or closes), and the audit lane picks
up via the next pipeline run.
...
|---|---|---|
| `pr_opened` | Codex made edits, PR exists | Review the PR; merge or close |
