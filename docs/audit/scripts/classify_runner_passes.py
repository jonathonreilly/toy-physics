...
# Match either `import sympy as sp` or `from sympy import simplify, Eq` patterns.
# We only need the alias form; the bare `import sympy` case is already handled
# by the literal `sympy.X` patterns. The `from sympy import` form would require
# a different rule since the called name is just `simplify(...)` with no module
# prefix, which is too noisy to match without false positives.
