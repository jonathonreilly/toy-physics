...
def main() -> int:
    ...
    for cid, row in rows.items():
        ...
        if cc_status in completed_statuses:
            for side in (first, second):
                side_fam = side.get("auditor_family")
                if side_fam == fam:
                    bucket["cross_confirmation_pairs_seen"] += 1
                    if cc_status == "confirmed":
                        bucket["cross_confirmation_pairs_agreed_first_try"] += 1
                    break  # count this row once per family even if the family
                           # appears on both sides (it shouldn't per policy)
    ...
    # Overall agreement rate across the whole pipeline (informational).
    overall_pairs = sum(
        b.get("cross_confirmation_pairs_seen", 0) for b in summary.values()
    )
    overall_agreed = sum(
        b.get("cross_confirmation_pairs_agreed_first_try", 0) for b in summary.values()
    )
