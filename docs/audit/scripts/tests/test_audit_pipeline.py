...
class ResetUnverifiedAuditsTest(unittest.TestCase):
    """The reset_unverified_codex_audits.py migration must invalidate
    rows where auditor_family is below the codex floor AND there's no
    previous_auditor_family marker (= not a legacy-migration relabel)."""
    ...
    def test_invalidates_below_floor_without_previous_marker(self):
        m = _import("reset_unverified_codex_audits")
        row = {
            "audit_status": "audited_clean",
            "auditor_family": "codex-gpt-5",
            "claim_type": "positive_theorem",
        ...
        self.assertTrue(m.is_unverified_audit(row))
    ...
    def test_skips_canonical_relabels(self):
        ...
        legacy strings and shouldn't be invalidated by this script."""
        m = _import("reset_unverified_codex_audits")
        row = {
            "audit_status": "audited_clean",
            "auditor_family": "codex-gpt-5",
        ...
        self.assertFalse(m.is_unverified_audit(row))
    ...
    def test_skips_rows_at_or_above_floor(self):
        ...
        for fam in ("codex-gpt-5.5", "codex-gpt-5.6", "codex-gpt-6"):
            ...
            self.assertFalse(m.is_unverified_audit(row), f"family={fam}")
    ...
    def test_archive_and_reset_preserves_history(self):
        m = _import("reset_unverified_codex_audits")
        row = {
            "audit_status": "audited_clean",
            "auditor_family": "codex-gpt-5",
        ...
        new_row = m.archive_and_reset(dict(row))
        # Reset to unaudited
        self.assertEqual(new_row["audit_status"], "unaudited")
        self.assertIsNone(new_row["auditor_family"])
        # Prior preserved
        self.assertEqual(len(new_row["previous_audits"]), 1)
        prior = new_row["previous_audits"][0]
        self.assertEqual(prior["audit_status"], "audited_clean")
        self.assertEqual(prior["auditor_family"], "codex-gpt-5")
        self.assertEqual(prior["invalidation_reason"], m.INVALIDATION_REASON)
