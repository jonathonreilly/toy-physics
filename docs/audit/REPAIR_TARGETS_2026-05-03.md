...
- DONE #7 `circulant_response_master_identity_narrow_theorem_note_2026-05-02` — kappa domain restriction
- DONE #9 `dm_pmns_z3_doublet_block_center_positive_sheet_no_go_theorem_note_2026-04-20` — runner register check
- DONE #11 `fifth_family_radial_boundary_note` — runner import drift

### Partially mechanical (1 of 13, partial)

- PARTIAL #8 `complete_prediction_chain_2026_04_15` — value reconciliation done; structural dependency-registration and runner-bridge issues remain

### Substantive (theorem restatements) — 3 of 3 done

- DONE #2 `axiom_first_cl3_per_site_uniqueness_theorem_note_2026-04-29` — `Cl(3) ⊗_R C ≅ M_2(C) ⊕ M_2(C)` chirality split; canonical positive-chirality convention; runner E6 chirality exhibit
- DONE #3 `axiom_first_lattice_noether_theorem_note_2026-04-29` — restated to `(2Z)^3` sublattice symmetry; one-step shifts documented as out-of-scope staggered/taste symmetries
- DONE #5 `axiom_first_spin_statistics_theorem_note_2026-04-29` — chained on #2; chirality-independent dimensional conclusion preserves the chain
...
- OPEN #1 `architecture_note_directional_measure` — derive `beta = 0.8` or demote
- OPEN #4 `axiom_first_reflection_positivity_theorem_note_2026-04-29` — derive OS factorisation for exact A_min, not citation
- OPEN #6 `bh_entropy_derived_note` — derive BH coefficient or demote class G match
- OPEN #10 `ew_coupling_derivation_note` — derive `g_2(v)`, `lambda(v)`; runner re-sync
- OPEN #12 `gauge_vacuum_plaquette_first_sector_minimal_bulk_completion_3plus1_line_exact_solve_doublet_theorem_note_2026-04-20` — global root count certificate
- OPEN #13 `higgs_mass_from_axiom_note` — derive curvature-to-physical-readout normalisation

## Net effect on the ledger

This pass was scoped to the 13 failed rows visible at `2026-05-03T18:27Z`.
By landing time, current `main` also had the unrelated
`fifth_family_complex_boundary_note` and `higher_symmetry_gravity_probe_note`
failures. Therefore the live post-pipeline count is `audited_failed = 10`,
while the scoped repair pass still resets five of the original failed rows for
re-audit.

| stage | `audited_failed (effective)` | `unaudited` (reset for re-audit) |
|---|---:|---:|
| Before this scoped pass | 13 | — |
...
| Live current-main count after this pass | 10 | includes unrelated `fifth_family_complex_boundary_note` and `higher_symmetry_gravity_probe_note` |
