...
The audit lane's auditor (Codex GPT-5.5 by default) judges each claim
from a restricted packet that includes the runner's source code and the
runner's stdout. If the runner changes but the cached stdout doesn't,
the auditor sees a stale picture and may issue a verdict that doesn't
match the current code. The cache must therefore stay synchronized with
its runner.
