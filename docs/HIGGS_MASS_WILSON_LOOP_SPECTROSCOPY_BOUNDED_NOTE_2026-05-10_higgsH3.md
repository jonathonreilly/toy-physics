...
1. **Tree-level mean-field axiom path** (`HIGGS_MASS_FROM_AXIOM_NOTE.md`):
...
physical matching by extracting the leading scalar-channel mass
**directly from the exponential decay of gauge-invariant scalar
correlators** on the framework's retained Monte Carlo infrastructure.
...
without new axioms or imports, and extracts a leading mass `m_eff` at
the canonical operating point `β_W = 6` (`g_bare = 1`).
...
WITHOUT new axioms, new imports, or new lattice content beyond what is
already retained at PR #685 / PR #845.

**Claim 2 (mass extraction).** On the canonical operating point
`β_W = 6` on a periodic `L³ × T` lattice with `L = 4, T = 8`, three
seeds, and `n_measure = 160` sweeps per seed, the connected correlator
`C(t)` exhibits a measurable exponential decay with leading effective
mass

```text
m_eff^(lattice) > 0,    finite,    extracted from log-ratio plateau
                                      C(d) / C(d+1) on d ∈ [1, T/2 - 1].   (3)
```

The runner reports the numerical value of `m_eff^(lattice)` and its
statistical error.
...
- **Operator buildability**: scalar-channel timeslice correlators are
  buildable on the retained numba MC infrastructure with no new axioms
  or imports.
- **Numerical demonstration at canonical operating point**: at
  ...
  `C(0) ≥ |C(t)|`, and yields a positive finite local effective mass
  `m_eff(d) = log(C(d) / C(d + 1))`.
- **Plateau extraction**: a weighted plateau average of `m_eff(d)`
  over `d ∈ [1, T/2 - 1]` is reported with a stat error from
  block-jackknife pooled across seeds.
...
- **Channel identification**: identifying the leading mass extracted
...
```text
m_eff^(lattice)         = (runner-reported)   ± (runner-reported, stat)
...
| Extract local effective mass `m_eff(d) = log(C(d) / C(d + 1))` for `d ∈ [1, T/2 - 1]` | standard lattice-spectroscopy estimator | no |
| Plateau average via inverse-variance-weighted mean | standard lattice-spectroscopy practice | no |
...
spectroscopy estimators. No new axioms, no new lattice content, no new
...
**(1) Channel-identification admission.** The leading mass extracted
...
**(4) Plateau-fit admission.** The plateau is read off from a small
`d`-window via the log-ratio estimator. The bounded result inherits the
systematic of the plateau window. A variational-smearing multi-state
fit (Lüscher–Weisz 1985 / Morningstar–Peardon 1999) would tighten this
admission but requires content beyond this campaign's compute frontier.
...
8. **Local m_eff plateau extraction** via the log-ratio estimator
...
- [`MINIMAL_AXIOMS_2026-05-03.md`](MINIMAL_AXIOMS_2026-05-03.md) for
  the framework axioms `A1` (`Cl(3)`) and `A2` (`Z³`).
...
VERDICT: bounded_theorem source-note proposal verified.
  - scalar-channel correlators ARE buildable on the retained
    numba-jitted MC infrastructure (no new axioms / imports);
