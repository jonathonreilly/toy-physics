# Per-Site SU(2) ↔ Color-SU(3) (1,2)-Block SU(2) Bridge — Bounded Obstruction
...
So the bridge is FORCED if and only if L3a is admitted. There is no
independent admission L3b separate from L3a.
...
has EXACTLY ONE admission scalar in the L3 layer (the L3a trace-surface
choice), not two. The W2.binary V8 result raised the question of a
potential separate admission L3b; this work establishes that L3b ≡ L3a.
...
- a lift of any bounded-theorem row to retained-clean.

What it DOES establish:

- The bridge admission L3b reduces to the L3a trace-surface admission.
- The framework has ONE admission scalar in the g_bare chain, not two.
...
| [`CL3_COLOR_AUTOMORPHISM_THEOREM.md`](CL3_COLOR_AUTOMORPHISM_THEOREM.md) | retained-bounded | provides the `V_3 ⊂ V` symmetric-base / SU(3) embedding |
...
| [`AXIOM_FIRST_CL3_PER_SITE_UNIQUENESS_THEOREM_NOTE_2026-04-29.md`](AXIOM_FIRST_CL3_PER_SITE_UNIQUENESS_THEOREM_NOTE_2026-04-29.md) | retained | provides Cl(3) per-site uniqueness up to chirality |
| [`CL3_TASTE_GENERATION_THEOREM.md`](CL3_TASTE_GENERATION_THEOREM.md) | retained-bounded | provides `V = (C²)^{⊗3} = C^8` taste cube structure |
...
2. **Algebraic equivalence (B2).** They are algebraically equivalent
   ...
   normalization).
3. **L3b ≡ L3a (B3).** The bridge identification reduces to the L3a
   trace-surface admission; closing L3a closes L3b automatically.
4. **g_bare chain has 1 admission, not 2 (B4).** Net structural
   tightening of the W2.binary BOUNDED_OBSTRUCTION.
...
sub-problem of the W2.binary BOUNDED_OBSTRUCTION result. It SHARPENS
...
claim_scope: |
  ...
  forced by L3a + Killing rigidity + matching normalization. The
  framework's g_bare chain has exactly ONE admission scalar (L3a only),
  not two.
