...
```text
κ_curv  :=  |∂²V_taste/∂m²|_{m=0}|  /  (N_taste · 2 v²)  =  1 / (8 u_0²).
```
...
2. Even with `λ_curv` as the symbol, **calling a mass²-derived ratio
   "λ" is misleading by first-principles QFT standards** — λ is the
   coefficient of `|φ|⁴` in V_eff, not the v²-normalized magnitude of a
   mass² coefficient. This v3 iteration replaces the misleading λ-name
   with the first-principles-honest **κ** (kappa, generic dimensionless
   ratio).
...
its **m² coefficient** ≈ -4/u_0² and its **m⁴ coefficient** ≈
+1/(4u_0⁴). These are different objects. The v²-normalized magnitude
of the mass² coefficient (what the framework calls `m_H_tree²/v²`) is
not a quartic coupling in any first-principles sense — it is a
mass²-to-v² ratio. Naming it κ_curv (not λ_curv) makes this honest.
...
>
> ```text
> κ_curv  :=  |∂²V_taste/∂m²|_{m=0}|  /  (N_taste · 2 v²)              (1)
> ```
>
> on the canonical Cl(3)/Z³ Wilson-plaquette + staggered-Dirac mean-field
> surface, where:
...
> - `v` is the EW VEV (admitted external scale).
>
> Then
>
> ```text
...
> ```
>
> with the v dependence cancelling exactly. At
...
- a precise scale identification (the natural matching scale of κ_curv
  ...
  Planck pin Gate #4 currently `audited_renaming`).
...
| 4 | The (2v²) divisor in (1) is a definitional choice, not a physical claim | Forced (this note's choice) | If the divisor is interpreted as identifying SM Higgs structure, this would smuggle a class-(F) renaming. Honest scope says it's a convenient normalization for downstream comparison work, no SM identification claimed. |
...
```text
κ_curv  =  |∂²V_taste/∂m²|_{m=0}|  /  (N_taste · 2 v²)
        =  (4/u_0²)  /  (32 v²)
        =  1 / (8 u_0² v²)        (in mass⁻² units)
```

Wait — that has units of 1/v², not dim-0. The framework's `(2v²)`
divisor restores dim-0 only because the curvature has units of mass²
and v² has units of mass²:
...
So κ_curv is a dim-0 ratio of [mass²-coefficient of V_taste] /
[v² normalization]. **It is structurally a (mass-squared)/(v²) ratio,
NOT a |φ|⁴ coefficient.** Concretely:

- κ_curv = (dimensionless ratio of) m_H_tree² / v²
  where m_H_tree² is the framework's "rescaled curvature" (magnitude
  divided by N_taste).
- SM λ (first-principles definition) = |φ|⁴ coefficient in V_eff.

These are different kinds of objects. κ_curv is a mass²-to-v² ratio;
SM λ is a quartic coupling. The numerical match `m_H² = 2λv²` in the
SM (which would equate the two via division by v²) holds only
**post-EWSB at the broken-phase pole**, where the m_H IS the pole
mass and λ IS the broken-phase quartic. The framework's m_H_tree is
NOT the broken-phase pole — it is the symmetric-point curvature scale
(rescaled).

**Net first-principles statement:**

κ_curv is a **well-defined dimensionless lattice quantity**: it is the
v²-normalized magnitude of V_taste's symmetric-point mass²
coefficient, divided by N_taste. It is NOT structurally the SM Higgs
quartic. The relation to SM λ requires identifying the symmetric-point
curvature with the broken-phase pole, which is not a first-principles
identity (the framework's +12% gap from m_H_tree=140.3 to
m_H_pole_obs=125.10 GeV is the explicit acknowledgement of the
non-identity).
...
|---|---|---|
| `HIGGS_MASS_FROM_AXIOM_NOTE.md` Step 4 | V_taste form `-8 log(m² + 4u_0²)` per channel | unaudited |
...
The v² in numerator and denominator: |curvature| has units of mass²;
in dimensionless lattice units (with v measured in lattice mass units)
both factors carry the same v²-normalization, so the ratio is dim-0.
Equivalently, expressing curvature in lattice units (where the
framework computes 4/u_0² in units of (mass scale)², the dimensionless
ratio is

  κ_curv  =  1 / (8 u_0²).                                              (iii)

The v dependence cancels in the ratio: κ_curv depends only on u_0.
Class (A) algebraic substitution in (iii). ∎
```
...
The relation between κ_curv (mass²-ratio at lattice scale) and the SM
...
2. **Compute the first-principles fermion-loop quartic at the lattice
   ...
   relation between κ_curv (mass²-ratio) and this fermion-loop quartic
...
1. V_taste curvature `∂²V_taste/∂m²|_{m=0} = -4/u_0²` symbolically via
   SymPy.
2. κ_curv definition `|∂²V_taste/∂m²|_{m=0}|/(N_taste · 2 v²)` with
   sign-magnitude convention.
3. Clifford-fixed identity `κ_curv = 1/(8u_0²)` after v cancellation.
...
proposed_claim_scope: |
  ...
  |d^2 V_taste/dm^2|_{m=0}| / (N_taste * 2 v^2) on L_s=2 mean-field
  surface. kappa_curv is structurally a mass^2-derived ratio, NOT a
  quartic coupling. EXPLICITLY DISTINCT from the SM EFT Higgs quartic
  lambda; the kappa_curv <-> SM lambda bridge is named as open work,
  not closed. At u_0 = 0.8776 (Gate #7), kappa_curv ≈ 0.1623.
...
- **`κ_curv = 1/(8u_0²)` is an exact Clifford-fixed dimensionless
  ...
  curvature plus the (E) definitional choice for the divisor.
