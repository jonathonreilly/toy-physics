...
| Staggered self-gravity probe | `frontier_staggered_self_gravity.py` | **5/5 on 3 cycle-bearing families** under the current prescribed attractive sign: random geometric (6% contraction), growing (1%), layered cycle (36%). No external source — |ψ|² generates its own Φ. Norm <7e-16, 0 sign flips, 3/3 state families. | First endogenous self-field result in the staggered lane. After the two-sign audit, the irregular sign rows should not be promoted as direction evidence by themselves; the retained nontrivial part is stable self-field evolution with measurable contraction under the prescribed attractive coupling. See `CYCLE_BATTERY_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Staggered self-gravity scaling | `frontier_staggered_self_gravity_scaling.py` | Scaling sweep across retained graph families. Layered cycle contracts strongly and monotonically (`0.6364 -> 0.6054` width ratio as size grows). Growing and random geometric stay norm-clean, while contraction is weak-to-mixed and settles near unity by `n=100`. | Retained scaling readout for self-gravity. After the two-sign audit, the irregular sign rows are no longer promotable as direction evidence; the durable signal here is topology-sensitive contraction strength under the prescribed attractive coupling. See `SELF_GRAVITY_SCALING_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
...
| Two-field wave coupling | `frontier_two_field_wave.py` | **Hard 5/5 on 3 cycle-bearing families** under the prescribed attractive sign: random geometric, growing, layered cycle. Φ obeys `d²Φ/dt² = -c²(L+μ²)Φ + β|ψ|²`, ψ norm stays exact, Φ remains bounded, and all 3/3 tested state families stay inward under the retained shell-force measure. Width response is retained as a diagnostic (`0.9997`, `1.0006`, `1.0028`), not a gate. | Current retained two-field structural result. It upgrades the scalar field from relaxation dynamics to a graph-native wave law without losing the retained interaction battery. After the two-sign audit, the irregular sign rows should be read as field-profile diagnostics rather than directional gravity evidence. See `STAGGERED_TWO_FIELD_WAVE_NOTE.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
