...
| Staggered fermion force-based card | `frontier_staggered_17card.py` | **Force-based canonical card**: gravity rows use the exact lattice force `F=-⟨dV/dz⟩` instead of centroid shift. 1D n=61: `17/17` (6/6 families, no qualifiers). 3D n=9: `17/17` (6/6 families). 3D n=11,13: `17/17` (4/6 families, energy projections skipped). Full suite baseline: 1D `29/38`, 3D `28/38`. The centroid oscillates in a permanent period-4 pattern on the staggered periodic lattice; the exact lattice force converges at all tested sizes. | The retained staggered result. Rows C5/C9/C10/C15/C16 use force semantics (different from the repo-wide centroid card). C12 is a native persistent-current test (1D ring or 3D torus on the actual card lattice). This remains the only retained directional gravity result in the stack because it uses the exact lattice-coordinate force; the attractive sign is still prescribed by `V<0`, not derived. See `STAGGERED_FERMION_CARD_2026-04-10.md` for the full semantic-differences table. |
...
| Staggered cycle-bearing battery, scaled | `frontier_staggered_cycle_battery_scaled.py` | **9/9 on random geometric and growing, 8/9 on layered cycle across all 9 larger-size runs**: random geometric, growing, and layered cycle at side `8`, `10`, `12` (`64`, `100`, `144` nodes). The inward retained proxy survives, iterative stability stays `15/15`, norm drift stays `~1e-16`, and native gauge remains strong (`R² = 0.9853-0.9985`). | Retained larger-graph sibling to the base cycle battery. It freezes the claim that the force-first battery is not just a tiny-graph artifact, but the layered-cycle linearity miss persists at scale under the corrected parity coupling. After the two-sign audit, the scaled irregular sign rows should be read structurally, not as evidence of attractive gravity direction. `B9` remains characterization-only, with the layered cycle family staying the cleanest and the growing family showing the largest source-scale gap. See `CYCLE_BATTERY_SCALED_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
...
| Two-field retarded / hybrid family closure | `frontier_two_field_retarded_family_closure.py` | Sibling closure probe that lifts the retarded-family row to `3/3` on all three admissible cycle-bearing families while preserving the rest of the battery, and extends the same operating-point closure to the causal DAG at `8/9` (`R8` gauge N/A). Rerun: `9/9`, `9/9`, `9/9`, `8/9` on random geometric, growing, layered cycle, and causal DAG. | Positive sibling result, not yet the canonical retarded harness. It shows that the retarded / hybrid law can be made family-robust under an explicit closure loop on both cycle-bearing and DAG families, but that loop still needs to be justified as a retained default rather than a tuned closure recipe. After the two-sign audit, the irregular sign rows are no longer promoted as evidence of attractive gravity direction. See `TWO_FIELD_RETARDED_FAMILY_CLOSURE_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
