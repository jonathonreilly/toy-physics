...
| Two-field retarded / hybrid probe | `frontier_two_field_retarded_probe.py` | Retarded-memory plus damped-wave Φ law on the retained cycle-bearing graph families. Scores `8/9`, `9/9`, `8/9` on random geometric, growing, and layered cycle. Core rows hold: zero-source, linearity, additivity, TOWARD force, iterative stability, norm, and native gauge all pass. The miss is family robustness (`2/3`) on random geometric and layered cycle. | First retained causal-memory field-law candidate beyond the wave prototype. Promising on the main trajectory, but not yet a retained replacement because family-sector robustness still fails on two admissible families. See `TWO_FIELD_RETARDED_PROBE_NOTE_2026-04-10.md`. |
| Two-field retarded / hybrid family closure | `frontier_two_field_retarded_family_closure.py` | Sibling closure probe that lifts the retarded-family row to `3/3` on all three admissible families while preserving the rest of the battery. Scores `9/9` on random geometric, growing, and layered cycle, with strong gauge rows and machine-clean norm. | Positive sibling result, not yet the canonical retarded harness. It shows that the retarded / hybrid law can be made family-robust under an explicit closure loop, but that loop still needs to be justified as a retained default rather than a tuned closure recipe. See `TWO_FIELD_RETARDED_FAMILY_CLOSURE_NOTE_2026-04-10.md`. |
