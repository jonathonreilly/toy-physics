...
| Two-field wave coupling | `frontier_two_field_wave.py` | **Hard 5/5 on 3 cycle-bearing families** under the corrected parity coupling: random geometric, growing, layered cycle. Φ obeys `d²Φ/dt² = -c²(L+μ²)Φ + β|ψ|²`, ψ norm stays exact, Φ remains bounded, and all 3/3 tested state families stay inward under the retained shell-force measure. Width response is retained as a diagnostic (`1.0047`, `1.0019`, `0.9923`), not a gate. | Current retained two-field structural result. It upgrades the scalar field from relaxation dynamics to a graph-native wave law without losing the retained interaction battery. After the two-sign audit, the irregular sign rows should be read as field-profile diagnostics rather than directional gravity evidence. See `STAGGERED_TWO_FIELD_WAVE_NOTE.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
...
| Two-field retarded / hybrid family closure | `frontier_two_field_retarded_family_closure.py` | Sibling closure probe that lifts the retarded-family row to `3/3` on all three admissible cycle-bearing families while preserving the rest of the battery, and extends the same operating-point closure to the causal DAG at `8/9` (`R8` gauge N/A). Current rerun: `9/9`, `9/9`, `9/9`, `8/9` on random geometric, growing, layered cycle, and causal DAG. | Positive sibling result, now fully retained on the three cycle-bearing families. The DAG operating point remains `8/9` with gauge structurally N/A. After the two-sign audit, the irregular sign rows are no longer promoted as evidence of attractive gravity direction. See `TWO_FIELD_RETARDED_FAMILY_CLOSURE_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Staggered fermion on causal DAG | `frontier_staggered_dag.py` | **6/6 on 3 layered DAG configurations** under the corrected parity-coupled sign: inward retained proxy response, `14/14` N-stability, Born at machine zero, exact norm, measurable forward propagation (`forward_frac=0.1266`), and `3/3` family robustness. | Retained causal-compatibility result for the staggered lane. It shows the force-first architecture is not restricted to periodic lattices or undirected cycle-bearing graphs, but after the two-sign audit the DAG sign row should be read as a structural proxy rather than standalone direction evidence. See `STAGGERED_DAG_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
