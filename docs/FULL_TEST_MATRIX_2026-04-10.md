...
| Staggered fermion + potential 17-card | `frontier_staggered_potential_16card.py`, `frontier_staggered_17card.py` | Potential-coupled staggered closes strong gravity rows cleanly. The physical-state-gated harness now scores `17/17`, using a persistent-current gauge row and excluding the antisymmetric/Nyquist-like family from `C17`; the stricter audited harness still scores `14/17`, with `C6`, `C12`, and `C17` failing because the antisymmetric/Nyquist-like family goes AWAY. | This lane is the strongest current candidate for a unified no-coin Dirac-plus-gravity architecture, but only if the repo is explicit about the robustness definition. `17/17` is a physical-state-only score with a different `C12`; it is not the stricter universal audited score. |
...
| C17 | state-family robustness | Same transport/gravity claim under packet-family / polarization / chirality / sublattice preparations. | weak-coin chiral + potential closes the card on the default `R` packet, but flips sign on `L` and symmetric families; potential-coupled staggered reaches `17/17` only under a physical-state-only gate and `14/17` under the stricter all-family audited gate |
