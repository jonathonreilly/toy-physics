...
| Staggered cycle-bearing battery | `frontier_staggered_cycle_battery.py` | **9/9 on random geometric and growing; 8/9 on layered cycle**: random geometric (36), growing (48), layered cycle (24). Includes native gauge (B8: sin(A) persistent current on graph cycle, R²=0.95-0.9999), iterative backreaction, and shell/spectral diagnostics (B9: G_eff=29-130, spectral ratio 8-19%). | Retained structural interaction battery under the corrected parity coupling. See `CYCLE_BATTERY_NOTE_2026-04-10.md`. After the two-sign audit, the irregular sign rows are no longer promoted as evidence of attractive gravity direction; they are inward field-profile diagnostics under the corrected parity-coupled scalar potential. The force-scale gap (G_eff) is structural (Poisson Green function smoother than 1/r on small graphs), not a sign/linearity failure on the two non-layered families. Layered gauge holdout is closed (layered cycle family has cycles + gauge PASS), but the layered-cycle family now exposes a B2 linearity miss under the corrected coupling. |
| Staggered cycle-bearing battery, scaled | `frontier_staggered_cycle_battery_scaled.py` | **Random geometric stays `9/9`; growing is `8/9` at side `8` then `9/9` at side `10/12`; layered cycle is `8/9` at side `8/10/12`**. The sweep covers random geometric, growing, and layered cycle at side `8`, `10`, `12` (`64`, `100`, `144` nodes). The inward retained proxy survives, iterative stability stays `15/15`, norm drift stays `~1e-16`, and native gauge remains strong (`R² = 0.9853-0.9985`). | Retained larger-graph sibling to the base cycle battery. It freezes the claim that the force-first battery is not just a tiny-graph artifact, while showing that the current post-parity miss pattern is `5/9` fully passing runs rather than a uniform `9/9` at scale. After the two-sign audit, the scaled irregular sign rows should be read structurally, not as evidence of attractive gravity direction. `B9` remains characterization-only, with the layered cycle family staying the cleanest and the growing family showing the largest source-scale gap. See `CYCLE_BATTERY_SCALED_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
...
| Two-field retarded / hybrid family closure | `frontier_two_field_retarded_family_closure.py` | Sibling closure probe that lifts the retarded-family row to `3/3` on all three admissible cycle-bearing families while preserving the rest of the battery, and extends the same operating-point closure to the causal DAG at `8/9` (`R8` gauge N/A). Current rerun: `9/9`, `8/9`, `9/9`, `8/9` on random geometric, growing, layered cycle, and causal DAG. | Positive sibling result, not yet the canonical retarded harness. It shows that the retarded / hybrid law can be made family-robust under an explicit closure loop on both cycle-bearing and DAG families, but the growing family still misses `R5` iterative stability at the retained operating point and the loop still needs to be justified as a retained default rather than a tuned closure recipe. After the two-sign audit, the irregular sign rows are no longer promoted as evidence of attractive gravity direction. See `TWO_FIELD_RETARDED_FAMILY_CLOSURE_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
...
| Emergent geometry growth v2 | `frontier_emergent_geometry.py`, `frontier_emergent_geometry_v2.py` | Matter-coupled growth can shift coarse geometry (`d_eff≈2.03` vs `1.54` control, `5.43x` clustering), but follow-up audit finds no useful density-to-`Φ` correlation, no clean `d_eff(G)` trend, and mixed grown-graph gravity signs under stronger measures. | Exploratory only. The growth rule clearly shapes geometry, but it does not yet produce a retained gravity-compatible emergent-geometry result. See `EMERGENT_GEOMETRY_GROWTH_NOTE_2026-04-10.md`. |
| Two-sign comparison / gravity sign audit | `frontier_two_sign_comparison.py`, `frontier_emergent_geometry_v2.py` | The exact cubic card remains directional because it uses `F=-⟨dV/dx⟩`. On irregular graph families, the shell and edge-radial sign measures are not sign-selective: both attractive and repulsive coupling can remain inward, and the exploratory grown-geometry lane is mixed (`shell_prob` AWAY while `edge_radial` can stay TOWARD). | Interpretation correction, not a numerical collapse. The irregular graph batteries remain strong structural interaction results, but their sign rows are no longer promotable as evidence that the architecture predicts attractive gravity direction. See `GRAVITY_SIGN_AUDIT_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
