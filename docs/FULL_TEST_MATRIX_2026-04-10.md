...
| Staggered fermion force-based card | `frontier_staggered_17card.py` | **Force-based canonical card**: gravity rows use the exact lattice force `F=-⟨dV/dz⟩` instead of centroid shift. 1D n=61: `17/17` (6/6 families, no qualifiers). 3D n=9: `17/17` (6/6 families). 3D n=11,13: `17/17` (4/6 families, energy projections skipped). Full suite baseline: 1D `29/38`, 3D `28/38`. The centroid oscillates in a permanent period-4 pattern on the staggered periodic lattice; the exact lattice force converges at all tested sizes. | The retained staggered result. Rows C5/C9/C10/C15/C16 use force semantics (different from the repo-wide centroid card). C12 is a native persistent-current test (1D ring or 3D torus on the actual card lattice). This remains the only retained directional gravity result in the stack because it uses the exact lattice-coordinate force. The corrected parity coupling restores sign sensitivity in direct well/hill tests, but the card itself still benchmarks a fixed attractive operating point rather than deriving attraction from an irregular graph observable. See `STAGGERED_FERMION_CARD_2026-04-10.md` for the full semantic-differences table. |
| Staggered graph portability probe | `frontier_staggered_graph_portable.py` | Bipartite random geometric, bipartite growing, and cubic lattice all pass `7/7` retained rows under the corrected parity-coupled scalar potential. | Portability established; superceded by the cycle battery for gauge and backreaction testing. |
| Staggered cycle-bearing battery | `frontier_staggered_cycle_battery.py` | **9/9 on random geometric, growing, and layered cycle**: random geometric (36), growing (48), layered cycle (24). Includes native gauge (B8: sin(A) persistent current on graph cycle, R²=0.95-0.9999), iterative backreaction, and shell/spectral diagnostics (B9: G_eff characterization, spectral ratio 8-19%). | Retained structural interaction battery under the corrected parity coupling. See `CYCLE_BATTERY_NOTE_2026-04-10.md`. The layered-cycle family is now fully retained rather than a miss case; the irregular sign rows remain inward field-profile diagnostics under the corrected parity-coupled scalar potential, not evidence of attractive gravity direction. B9 remains characterization-only. |
| Staggered cycle-bearing battery, scaled | `frontier_staggered_cycle_battery_scaled.py` | **9/9 on all nine retained larger-size runs**: random geometric, growing, and layered cycle all pass at side `8`, `10`, and `12` (`64`, `100`, `144` nodes). The inward retained proxy survives, iterative stability stays `15/15`, norm drift stays `~1e-16`, and native gauge remains strong (`R² = 0.9853-0.9985`). | Retained larger-graph sibling to the base cycle battery. It freezes the claim that the force-first battery is not just a tiny-graph artifact. After the two-sign audit, the scaled irregular sign rows should be read structurally, not as evidence of attractive gravity direction. `B9` remains characterization-only. See `CYCLE_BATTERY_SCALED_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
...
| Staggered self-gravity probe | `frontier_staggered_self_gravity.py` | **5/5 on 3 cycle-bearing families** under the corrected parity coupling: random geometric (37% contraction), growing (40%), layered cycle (56%). No external source — |ψ|² generates its own Φ. Norm <7e-16, 0 sign flips, 3/3 state families. | First endogenous self-field result in the staggered lane. After the two-sign audit, the irregular sign rows should not be promoted as direction evidence by themselves; the retained nontrivial part is stable self-field evolution with measurable contraction under the corrected parity coupling. See `CYCLE_BATTERY_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Staggered self-gravity scaling | `frontier_staggered_self_gravity_scaling.py` | Scaling sweep across retained graph families. All three families contract under the corrected parity coupling: random geometric `0.88 -> 0.60`, growing `0.68 -> 0.68`, layered cycle `0.45 -> 0.42` width ratio as size grows. Norm remains machine-clean and force stays TOWARD across the sweep. | Retained scaling readout for self-gravity. After the two-sign audit, the irregular sign rows are no longer promotable as direction evidence; the durable signal here is topology-sensitive contraction strength under the corrected parity coupling. See `SELF_GRAVITY_SCALING_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Two-field coupling prototype | `frontier_two_field_coupling.py` | Separate Φ field (relaxation+source) and staggered ψ (CN). Φ grows from zero sourced by |ψ|². Under the prescribed attractive sign, the retained proxy stays inward, norm is `9e-16`, and width contracts to `0.9992` (binding). | First two-field endogenous gravity proof of concept. It is now superseded by the wave-field hardening in `frontier_two_field_wave.py`, and after the two-sign audit its irregular sign row should be read structurally rather than directionally. |
| Two-field wave coupling | `frontier_two_field_wave.py` | After the 2026-04-18 clean-family rerun, `W1`-`W4` still hold on all three cycle-bearing families, but independent `W6` restarts give `2/3`, `3/3`, `2/3` on random geometric, growing, and layered cycle, for hard scores `4/5`, `5/5`, `4/5`. Φ obeys `d²Φ/dt² = -c²(L+μ²)Φ + β|ψ|²`, ψ norm stays exact, Φ remains bounded, and width remains diagnostic (`1.0047`, `1.0019`, `0.9923`). | Bounded two-field wave result after the family-history bug fix. It still upgrades the scalar field from relaxation dynamics to a graph-native wave law, but it is no longer a retained family-robust hardening. Use the retarded family-closure sibling for the retained two-field result. See `STAGGERED_TWO_FIELD_WAVE_NOTE.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Two-field retarded / hybrid probe | `frontier_two_field_retarded_probe.py` | Retarded-memory plus damped-wave Φ law on the retained cycle-bearing graph families. Scores `8/9`, `9/9`, `8/9` on random geometric, growing, and layered cycle. Core rows hold: zero-source, linearity, additivity, inward shell-radial sign, iterative stability, norm, and native gauge all pass. The miss is family robustness (`2/3`) on random geometric and layered cycle. | First retained causal-memory field-law candidate beyond the wave prototype. Promising on the main trajectory, but not yet a retained replacement because family-sector robustness still fails on two admissible families. After the two-sign audit, the irregular sign rows are no longer promoted as evidence of attractive gravity direction. See `TWO_FIELD_RETARDED_PROBE_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Two-field retarded / hybrid family closure | `frontier_two_field_retarded_family_closure.py` | Sibling closure probe that lifts the retarded-family row to `3/3` on all three admissible cycle-bearing families while preserving the rest of the battery, and extends the same operating-point closure to a layered DAG-derived control at `8/9` (`R8` gauge N/A). Current rerun: `9/9`, `9/9`, `9/9`, `8/9` on random geometric, growing, layered cycle, and the layered DAG-derived control. | Positive sibling result, now fully retained on the three cycle-bearing families. The layered DAG-derived operating point remains `8/9` with gauge structurally N/A, but should not be read as a genuinely directed DAG closure because the live builder still symmetrizes adjacency. After the two-sign audit, the irregular sign rows are no longer promoted as evidence of attractive gravity direction. See `TWO_FIELD_RETARDED_FAMILY_CLOSURE_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Staggered fermion on layered DAG-derived family | `frontier_staggered_dag.py` | `6/6` on 3 layered acyclic configurations under the corrected parity-coupled sign: inward proxy response, `14/14` N-stability, Born at machine zero, exact norm, measurable forward-depth bias (`forward_frac=0.1266`), and `3/3` family robustness. | Bounded layered-geometry control, not a retained causal-DAG compatibility theorem. The live Hamiltonian symmetrizes the layered adjacency, so this surface now supports only the narrower claim that the force-first battery survives on a DAG-derived layered template. See `STAGGERED_DAG_NOTE_2026-04-10.md` and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Critical exponents vs topology | `frontier_critical_exponents.py` | Finite-size onset scout across admissible families. Fitted `beta` varies strongly across representative graphs (`~0.08` to `~0.65`), while one DAG configuration is degenerate. | Exploratory quantitative result, not yet a retained universality claim. The current evidence supports topology-dependent finite-size onset behavior, but not a full critical-scaling law. See `CRITICAL_EXPONENTS_TOPOLOGY_NOTE_2026-04-10.md`. |
| Emergent geometry growth v2 | `frontier_emergent_geometry.py`, `frontier_emergent_geometry_v2.py` | Matter-coupled growth can shift coarse geometry (`d_eff≈2.03` vs `1.54` control, `5.43x` clustering). The new G sweep reopens a narrow high-`G` window: `G=100` is robust TOWARD, while lower/higher `G` remains mixed. Density-to-`Φ` correlation is positive but weak/seed-dependent, `d_eff(G)` is not clean, and 3D seeds still do not preserve `d_eff≈3`. | Exploratory only. The growth rule clearly shapes geometry, and the new sweep partially reopens the lane at strong coupling, but it does not yet produce a retained gravity-compatible emergent-geometry result. See `EMERGENT_GEOMETRY_GROWTH_NOTE_2026-04-10.md`. |
| Irregular same-surface directional observable probe | `frontier_irregular_directional_observable.py` | Off-center endogenous probe on retained irregular families: `Δ<depth>=0/9`, signed cut flux `0/9`, frontier current bias `4/9`. No frozen sign-selective observable across families. | Blocker result for off-lattice direction claims. Synthetic external-source sign tests are distinguishable, but endogenous same-surface sign selection is not yet frozen. See `IRREGULAR_DIRECTIONAL_OBSERVABLE_NOTE_2026-04-11.md`. |
| Two-sign comparison / gravity sign audit | `frontier_two_sign_comparison.py`, `frontier_emergent_geometry_v2.py` | The exact cubic card remains directional because it uses `F=-⟨dV/dx⟩`, and the direct external-potential sign test under parity/lapse distinguishes well from hill. On irregular graph families, the shell and edge-radial sign measures are not sign-selective, and the exploratory grown-geometry lane is mixed (`shell_prob` AWAY while `edge_radial` can stay TOWARD). | Interpretation correction, not a numerical collapse. Do not conflate the exact-cubic external-source sign test with the endogenous same-surface probe. The irregular graph batteries remain strong structural interaction results, but their sign rows are no longer promotable as evidence that the architecture predicts attractive gravity direction. See `GRAVITY_SIGN_AUDIT_2026-04-10.md`, `IRREGULAR_DIRECTIONAL_OBSERVABLE_NOTE_2026-04-11.md`, and `TWO_SIGN_COMPARISON_NOTE_2026-04-10.md`. |
| Cycle break frontier | `frontier_staggered_cycle_break_frontier.py` | Raw size alone does not break the retained cycle battery through side `18` (`324` nodes) on the three admissible families. The first honest failure appears at `random_geometric side=18 extra=5`, where score drops to `8/9` because native gauge collapses (`J_span=6.63e-07`) while the force-first rows remain intact. | First explicit boundary map beyond the scaled sibling. The earliest clean failure is a gauge/current failure under dense cross-color shortcuts, not a force-sign collapse. See `CYCLE_BREAK_FRONTIER_NOTE_2026-04-10.md`. |
| Cycle break matched slice | `frontier_staggered_cycle_break_slice.py` | Matched frontier slice on `random_geometric side=18` with `extra=4,5,6` against a quality-matched local control. Frontier fails at `extra=5/6` (`8/9`, gauge collapse) while the control stays `9/9` at all three extras. B8 separates cleanly: frontier `J_span` collapses to `~6.63e-07`, control stays `~3e-05`; B9 stays characterized and stable. | Boundary probe that isolates the failure to dense shortcut geometry rather than edge-count or degree scale. See `CYCLE_BREAK_SLICE_NOTE_2026-04-10.md`. |
