...
| Staggered cycle-bearing battery | `frontier_staggered_cycle_battery.py` | **9/9 on 3 cycle-bearing families**: random geometric (36), growing (48), layered cycle (24). Includes native gauge (B8: sin(A) persistent current on graph cycle, R²=0.95-0.9999), iterative backreaction (B5: 15/15 TOWARD), shell/spectral diagnostics (B9: G_eff=12-178, spectral ratio 8-19%). | Retained cycle battery. See `CYCLE_BATTERY_NOTE_2026-04-10.md`. On these irregular graph families the gravity rows are audited shell-radial proxies, not exact lattice-coordinate forces; a later edge-radial sign audit agrees on all three retained families. The force-scale gap (G_eff) is structural (Poisson Green function smoother than 1/r on small graphs), not a sign/linearity failure. Layered gauge holdout is closed (layered cycle family has cycles + gauge PASS). |
| Staggered cycle-bearing battery, scaled | `frontier_staggered_cycle_battery_scaled.py` | **9/9 on all 9 larger-size runs**: random geometric, growing, and layered cycle at side `8`, `10`, `12` (`64`, `100`, `144` nodes). Force stays TOWARD, iterative stability stays `15/15`, norm drift stays `~1e-16`, and native gauge remains strong (`R² = 0.9853-0.9985`). | Retained larger-graph sibling to the base cycle battery. It freezes the claim that the force-first `9/9` battery is not just a tiny-graph artifact. `B9` remains characterization-only, with the layered cycle family staying the cleanest and the growing family showing the largest source-scale gap. See `CYCLE_BATTERY_SCALED_NOTE_2026-04-10.md`. |
...
| Staggered self-gravity scaling | `frontier_staggered_self_gravity_scaling.py` | Scaling sweep across retained graph families. Layered cycle contracts strongly and monotonically (`0.6364 -> 0.6054` width ratio as size grows). Growing and random geometric stay norm-clean and mostly TOWARD, but contraction is weak-to-mixed and settles near unity by `n=100`. | Retained scaling readout for self-gravity. The endogenous force sign is robust, but contraction strength is topology-sensitive rather than universal. See `SELF_GRAVITY_SCALING_NOTE_2026-04-10.md`. |
| Two-field coupling prototype | `frontier_two_field_coupling.py` | Separate Φ field (relaxation+source) and staggered ψ (CN). Φ grows from zero sourced by |ψ|². Force 30/30 TOWARD, norm 9e-16, width contracts to 0.9992 (binding). | First two-field endogenous gravity proof of concept. It is now superseded by the wave-field hardening in `frontier_two_field_wave.py`. |
| Two-field wave coupling | `frontier_two_field_wave.py` | **Hard 5/5 on 3 cycle-bearing families**: random geometric, growing, layered cycle. Φ obeys `d²Φ/dt² = -c²(L+μ²)Φ + β|ψ|²`, force is 30/30 TOWARD, ψ norm stays exact, Φ remains bounded, and all 3/3 tested state families stay TOWARD. Width response is retained as a diagnostic (`0.9997`, `1.0006`, `1.0028`), not a gate. | Current retained two-field result. It upgrades the scalar field from relaxation dynamics to a graph-native wave law without losing the retained force battery. See `STAGGERED_TWO_FIELD_WAVE_NOTE.md`. |
| Two-field retarded / hybrid probe | `frontier_two_field_retarded_probe.py` | Retarded-memory plus damped-wave Φ law on the retained cycle-bearing graph families. Scores `8/9`, `9/9`, `8/9` on random geometric, growing, and layered cycle. Core rows hold: zero-source, linearity, additivity, TOWARD radial sign, iterative stability, norm, and native gauge all pass. The miss is family robustness (`2/3`) on random geometric and layered cycle. | First retained causal-memory field-law candidate beyond the wave prototype. Promising on the main trajectory, but not yet a retained replacement because family-sector robustness still fails on two admissible families. On irregular graphs the sign rows are audited shell-radial proxies rather than exact coordinate-force expectations. See `TWO_FIELD_RETARDED_PROBE_NOTE_2026-04-10.md`. |
| Two-field retarded / hybrid family closure | `frontier_two_field_retarded_family_closure.py` | Sibling closure probe that lifts the retarded-family row to `3/3` on all three admissible cycle-bearing families while preserving the rest of the battery, and extends the same operating-point closure to the causal DAG at `8/9` (`R8` gauge N/A). Rerun: `9/9`, `9/9`, `9/9`, `8/9` on random geometric, growing, layered cycle, and causal DAG. | Positive sibling result, not yet the canonical retarded harness. It shows that the retarded / hybrid law can be made family-robust under an explicit closure loop on both cycle-bearing and DAG families, but that loop still needs to be justified as a retained default rather than a tuned closure recipe. On irregular graphs the sign rows are audited shell-radial proxies rather than exact coordinate-force expectations. See `TWO_FIELD_RETARDED_FAMILY_CLOSURE_NOTE_2026-04-10.md`. |
| Staggered fermion on causal DAG | `frontier_staggered_dag.py` | **6/6 on 3 layered DAG configurations**: force TOWARD, `14/14` N-stability, Born at machine zero, exact norm, measurable forward propagation (`forward_frac=0.1266`), and `3/3` family robustness. | Retained causal-compatibility result for the staggered lane. It shows the force-first architecture is not restricted to periodic lattices or undirected cycle-bearing graphs. See `STAGGERED_DAG_NOTE_2026-04-10.md`. |
...
| Gravity sign audit | `frontier_emergent_geometry_v2.py` + spot checks | Retained cycle-bearing and retarded-family results keep their TOWARD sign under rerun. On the irregular admissible families, the current shell-radial proxy agrees in sign with a stronger edge-radial check. The exploratory emergent-geometry growth lane is mixed: at `n=100, G=50`, `shell_prob` is AWAY while `edge_radial` is TOWARD. | Measurement correction, not a score collapse. Exact `F=-⟨dV/dx⟩` remains the standard on regular lattices; irregular graph batteries should be described as audited radial proxies until a single graph-native force observable is standardized. See `GRAVITY_SIGN_AUDIT_2026-04-10.md`. |
