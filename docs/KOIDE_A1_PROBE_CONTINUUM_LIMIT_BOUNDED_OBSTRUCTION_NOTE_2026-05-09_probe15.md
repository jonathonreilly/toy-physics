...
**No new axioms. No external imports.** Closure must come from:
  ...
  (i) An already-retained limit operation that produces U(1)_b on the
      b-doublet, OR
  (ii) A derivation of U(1)_b from existing retained content via a
...
- **C1 (existence)**: limit is retained content?
...
1. **Admit U(1)_b as a new small continuous primitive.** A 1-dim
   ...
   retained content, so admitting it is the smallest possible
...
with structural reasons why it is not derivable from retained content.
...
- α_LM running (matter-sector dynamical retained content): [`COMPLETE_PREDICTION_CHAIN_2026_04_15.md`](COMPLETE_PREDICTION_CHAIN_2026_04_15.md)
...
1. Verify the proposed limit is **retained content** (cite a retained
