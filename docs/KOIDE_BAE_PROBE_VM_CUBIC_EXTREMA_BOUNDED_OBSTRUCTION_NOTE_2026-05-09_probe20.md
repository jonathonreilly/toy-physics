...
**This probe falsifies that hypothesis** within the retained content.
...
Within retained content (no axioms, no imports, no PDG values):
...
3. **Any tuned `L`** that achieves V_total/BAE compatibility shifts the
   ...
   retained content." Since `L = c_1 + c_2/2` is itself derived from
...
2. Any alternative extension (e.g., operator-valued `V(H)` without
   ...
   derivation from retained content.
...
mechanism within retained content.
