...
**Status (note-side declaration):** structural-identity packaging
subtheorem of the CKM atlas/axiom package. This note packages the five
atlas-leading **off-diagonal** CKM magnitude identities into one
structural-counts surface on top of the cited Wolfenstein and CP-plane
identities. It does not change the scope of the parent CKM theorem and it
does not promote any cross-sector or dimension-uniqueness claim.

**Narrowed claim scope.** This note scopes the five atlas-leading off-diagonal CKM
magnitude identities `(M1)` through `(M5)` and the compact
`|V_ub|_0^2 = alpha_s(v)^3 / (8 n_color^2)` repackaging as exact algebra over
the cited structural-count inputs and the canonical coupling. The conditional
surface is:

1. `lambda^2 = alpha_s(v)/n_pair` is imported from the upstream
   [Wolfenstein lambda/A structural-identities authority](WOLFENSTEIN_LAMBDA_A_STRUCTURAL_IDENTITIES_THEOREM_NOTE_2026-04-24.md);
2. `A^2 = n_pair/n_color`, `rho = 1/n_quark`, and
   `eta^2 = (n_quark-1)/n_quark^2` are imported from the upstream CKM
   CP-phase structural-identity authority and the Thales reading
   `(1-rho)^2 + eta^2 = 1 - rho` is imported from the
   [atlas right-angle subtheorem](CKM_ATLAS_TRIANGLE_RIGHT_ANGLE_THEOREM_NOTE_2026-04-24.md);
3. the canonical `alpha_s(v)` is imported from
   [`ALPHA_S_DERIVED_NOTE.md`](ALPHA_S_DERIVED_NOTE.md)
   via `scripts/canonical_plaquette_surface.py`;
4. PDG comparator deviations are downstream numerical readouts only and
   are not load-bearing for the algebra.

The local algebra closes exactly under those imported inputs. Retained-grade
closure of the upstream Wolfenstein, CP-phase, atlas right-angle, and
`alpha_s` authorities is owned by the upstream-audit lane and is not asserted
by this note.
...
On the cited CKM atlas surface, let
...
`n_color` on the cited CKM surface.
...
The cited Wolfenstein and CP-plane identities give
...
For `|V_ub|_0^2`, use the cited atlas-leading form
...
For `|V_td|_0^2`, use the cited atlas-leading form

```text
|V_td|_0^2 = A^2 lambda^6 ((1-rho)^2 + eta^2).
```

The cited Thales surface gives
...
structural-counts algebra on the cited CKM atlas surface.

## Why This Adds Value

The existing CKM package already had the first-, second-, and third-row
...
- the canonical numerical readout once the cited `alpha_s(v)` is supplied.
