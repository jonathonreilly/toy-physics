...
**Narrowed claim scope.** Audited the five atlas-leading off-diagonal CKM
...
1. `lambda^2 = alpha_s(v)/n_pair` is imported from the upstream
   Wolfenstein lambda/A structural-identities authority;
2. `A^2 = n_pair/n_color`, `rho = 1/n_quark`, and
   ...
   `(1-rho)^2 + eta^2 = 1 - rho` is imported from the atlas right-angle
   subtheorem;
3. the canonical `alpha_s(v)` is imported from `ALPHA_S_DERIVED_NOTE.md`
