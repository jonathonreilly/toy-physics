# G1 Physicist F — Frobenius Uniqueness + Full-W at Physically-Forced m + Quartic Extension
...
**Script:** `scripts/frontier_g1_physicist_f_uniqueness_and_full_w.py`
**Framework convention:** "axiom" means only the single framework axiom `Cl(3)` on `Z^3`.

## Scope and discipline

The G1 Z_3 doublet-block selector gate has two surviving sub-obstructions
on the `claude/g1-complete` integration branch:
...
This note pursues the three attack lines specified in the Physicist F
...
a ledger candidate point, Line 2 closes G1.
...
- **NOT at any of the five ledger candidates** (Schur-Q sqrt(6)/3, F1
  sqrt(6)/2 − sqrt(2)/18, Path-C-a 0.9644, Path-C-b 1.2679, Path-C-c
  0.8000), to tolerance 1e-3;
- **chamber-interior joint minima land on the det=0 singular boundary**,
  confirming Path-B (B) at fixed m in a stronger form (not only joint
...
Path-B m-dependence obstruction. Path B obstructed the joint `(m, delta)`
...
closure of the Path-B m-dependence.**
...
beyond the Path-B-obstructed cubic, possibly producing a unique chamber
...
parity-mixing information beyond the (Path-B-obstructed) cubic.
...
for axiom-native closure — they are also Path-B-obstructed in their
...
scalar baseline is concentrated at cubic order (Path-B-obstructed);
...
**Closure status of G1 remains OPEN.**
...
2. **Fixed-m m-dependence is a distinct obstruction from Path-B (B).**
   Path-B obstructed the joint `(m, delta)` stationarity; this note
...
```
(G-Var)        still requires selection axiom (Path A leading-quadratic)
...
Path-B         m-dependent boundary extremum / joint CPs singular
Quartic         untested
```
...
Path-B         fixed-m restriction also m-dependent and never lands at
...
```bash
PYTHONPATH=scripts python3 scripts/frontier_g1_physicist_f_uniqueness_and_full_w.py
```
...
safe read is: **three new obstruction theorems + narrower gap; G1 remains
OPEN**.
