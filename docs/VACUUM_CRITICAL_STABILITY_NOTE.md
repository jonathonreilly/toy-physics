...
## Why The Prior Claim Was Retired (Gap #7, 2026-05-10)
...
instead use explicit compositeness/RG assumptions to obtain their scalar
sector and low-energy mass predictions.
...
the top-condensate route does not supply a protected tree-level
`lambda_tree = 0` theorem from compositeness alone; it requires explicit
compositeness/RG boundary assumptions (BHL, Hill). The framework has no
current derivation distinguishing pNGB vs NJL behavior on the retained
taste block. So (1) is not safe.

**Negate (2).** A theory with no fundamental scalar can still generate an
effective scalar potential through fermion-bilinear bosonization and loop/RG
matching. BHL-style top-condensate models use compositeness conditions rather
than a theorem that "no elementary scalar" alone forces `lambda_tree = 0`.
So the slogan does not in itself force a vanishing quartic.
...
### QFT-Primitives Pass (cap 250 words)
...
does NOT in general prove a protected vanishing tree-level quartic.**
BHL/top-condensate routes require explicit compositeness/RG boundary
assumptions and produce nonzero low-energy scalar self-coupling and Higgs
mass predictions. Walking TC and holographic models likewise do not
generically force vanishing.
