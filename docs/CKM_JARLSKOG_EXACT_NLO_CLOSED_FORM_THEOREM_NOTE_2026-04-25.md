...
`J̄` on the protected-γ̄ surface is a **degree-4 polynomial in α_s
...
(J6)  Selection rule: J_bar has ONLY alpha_s^3 and alpha_s^4 terms.
      ...
      have coefficient EXACTLY ZERO on the protected-γ̄ surface.
...
- **Upper cutoff**: `J̄` is degree-4 in α_s. No α_s⁵ or higher
  corrections on the protected-γ̄ surface.
...
coefficients** (at α_s³ and α_s⁴). All other coefficients are zero.
This is sharper than "approximately α_s³" or "leading-order Jarlskog";
the closed form has **no truncation error** on the protected-γ̄
surface up to and including α_s⁴.
...
```text
NO alpha_s^5 or alpha_s^6 correction to J_bar on the protected-gamma surface.
J_bar is EXACTLY a degree-4 polynomial in alpha_s with two non-zero coefficients.

J_bar / J_LO = 1 - alpha_s/N_pair^2 EXACTLY at NLO.
```

If a future NNLO Wolfenstein analysis on the protected-γ̄ surface
yields a non-zero `α_s⁵` correction to `J̄`, then either:

- retained `(N2)` `η̄ = √5(4-α_s)/24` itself receives an `α_s²` term
  at NNLO (modifying its retained linear-in-α_s form), or
- the standard Wolfenstein NLO formula `J = A²λ⁶η̄` requires
  modification at higher orders in λ (which it does at λ⁸ and
  beyond).

This makes the polynomial closed form a sharp test of `(N2)` × the
Wolfenstein expansion structure on this surface.
...
2. **Polynomial selection rule (J6)**: `J̄` has only α_s³ and α_s⁴
   ...
   protected-γ̄ surface. The framework's NLO Jarlskog is a
   degree-4 polynomial — the simplest possible all-orders structure.
...
These propositions about the all-orders α_s-structure of the
Jarlskog on the protected-γ̄ surface were not visible from `(N1)`-
...
- `(J6)` NEW selection rule: only α_s³ and α_s⁴ coefficients non-zero;
  all other α_s powers EXACTLY zero.
