...
fixed audit parameter envelope (not a boundary-law theorem, not a holography
proof)
**Claim type:** bounded_theorem (only for the fixed-run numerical diagnostic
on the fixed audit envelope; not a general boundary-law theorem)
...
the fixed audit parameter envelope below. It is *not*:
...
Prior audit review limits the support statement to the exact fixed-run
numerical observation that this script/parameter grid reports high `R^2`
boundary-size linearity for the audited BFS-ball sweep and the stated small
partition check. It should not be cited as a general boundary-law theorem or
holography proof. This addendum is now scoped accordingly.

## Fixed audit parameter envelope
...
Counted BFS-ball sweep (fixed audited surface):
- sides: `{6, 8, 10, 12, 14}`
- couplings: `G in {0, 5, 10, 20}`
- seeds: `{42, 43, 44, 45, 46}`
- total counted BFS-ball fits: `100 = 5 sides x 4 couplings x 5 seeds`

Partition cross-check (fixed audited surface, smaller and separate):
- single side: `10`
...
## Caveats and prior-review limitations

Prior audit review flagged the following limitations explicitly. They remain.
...
- **Fixed grid.** The fixed audit envelope above is the full sweep set.
...
> On the fixed audit parameter envelope above (2D periodic staggered
...
- Identifying the area-law coefficient with the Planck packet `1/4`. See
  `AREA_LAW_COEFFICIENT_GAP_NOTE.md` for the canonical gap analysis.

The path to lift this to theorem status would require analytic closure or
substantially broader independent partition and size checks under the
parameter sweep. That is not attempted in this addendum.
