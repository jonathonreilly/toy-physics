# PR230 Two-Source Taste-Radial Chunks033-034 Package

**Status:** bounded-support / chunks001-034 packaged; no closure

**Checkpoint certificates:**
`outputs/yt_pr230_two_source_taste_radial_chunk033_checkpoint_2026-05-06.json`,
`outputs/yt_pr230_two_source_taste_radial_chunk034_checkpoint_2026-05-06.json`

## Result

Chunks033-034 now pass completed-mode checkpoints and are included in the
two-source taste-radial row package.  The row combiner now reports
`ready=34/63` with `combined_rows_written=false`.

Both chunks preserve the production surface:

- fixed seeds `2026056033` and `2026056034`;
...
- Raw `C_ss` still fails the strict scalar-LSZ first-shell Stieltjes
  ...
  `z=137.39835521329425`.
- The Schur split remains bounded support only.  `C_s|x` fails
  (`z=128.66656381261006`), while `C_x|s` survives only the necessary
  first-shell check (`z=-506.4973030760837`).
...
Chunk035 is active under the row-wave supervisor.  Its logs, live status,
