...
**Date:** 2026-05-10
**Claim type:** bounded_theorem (negative on the conjecture; support-only
on Casimir-value reproduction)
**Sub-gate:** Lane 1 (`alpha_s`) — do quartic-Casimir values determine
the QCD `beta_2`/`beta_3` channel weights?
...
The related open X-L1-MSbar PR (#934) tests the broader Lane 1
`beta_2`/`beta_3` import surface. This V probe is narrower and does not
depend on that unlanded sibling PR: it asks whether quartic-Casimir
values alone can bypass the higher-loop channel-weight obstruction.

This V probe asks: what if the failure were localized? Specifically, the
source stack supplies:

- **Quadratic Casimirs**: `C_F = 4/3, C_A = 3, T_F = 1/2` (giving `beta_0,
  beta_1`)
- **Cl(3) ⊗ Cl(3) tensor structure**: the SU(3) generator algebra is
  embedded in Cl(3) anticommutator products via the graph-first
  SU(3) bridge.

**Conjecture (stress-test).** β_2 and/or β_3 are derivable from supported
quartic Casimirs (`d_F^abcd d_F^abcd / N_F = 5/12`, `d_F^abcd d_A^abcd /
N_F = 5/2`, `d_A^abcd d_A^abcd / N_A = 135/8` for SU(3)) via the
Cl(3) ⊗ Cl(3) tensor structure, bypassing the 3-/4-loop integral
obstruction. The "scalar channel weights" might be quartic-Casimir
COEFFICIENTS, not integrals.

**Goal.** Test whether supported quartic Casimirs from Cl(3) tensor
structure give `beta_2` and/or `beta_3` in closed form.

## 1. Bounded claim (negative on the conjecture)

**Bounded claim (V-L1-Quartic; the conjecture is foreclosed).** On the
repo baseline of the `physical Cl(3) local algebra` and `Z^3 spatial
substrate`, augmented by the explicit SU(3) quartic-Casimir value
basis, the QCD `beta_2` and `beta_3` channel weights are not determined
by Casimir values alone. Specifically:
...
2. **(`beta_3` does contain quartic Casimirs, but their scalar weights
   ...
   - `d_F^abcd d_F^abcd / N_F = 5/12` (supported, group-theoretic)
   - `d_F^abcd d_A^abcd / N_F = 5/2` (supported, group-theoretic)
   - `d_A^abcd d_A^abcd / N_A = 135/8` (supported, group-theoretic)

   These values are pure group theory, reproduced by the runner from
   explicit SU(3) generator algebra. However,
   ...
   These scalars are NOT framework-supported: they require explicit
   ...
   versus lattice cutoff in `<P>`-scheme). Supported Cl(3) ⊗ Cl(3)
   tensor algebra does NOT compute these.

3. **(Casimir VALUES are supported; channel WEIGHTS are not.)** A clean
   structural distinction:
   - **Casimir values** (e.g., `5/12, 5/2, 135/8`): pure group-theoretic
     invariants reproduced by explicit SU(3) matrix algebra. These are
     support-only values, not a retained-status grant.
   - **Channel weights** (e.g., the rational + `zeta_n` numbers
     ...
     machinery. These are NOT supported.

   The conjecture conflates these two. Quartic Casimir VALUES being
   supported does NOT bridge to channel WEIGHTS being supported.

4. **(Hostile-review sharpening: cubic Casimirs at 3-loop also supported
   ...
   of `(C_F, C_A, T_F n_f)`, all supported as values. The MSbar 3-loop
...
5. **(Why 1-loop, 2-loop ARE supported but 3-loop, 4-loop are not.)**
...
### Closed (support-only positive result)

- **Quartic Casimir values at SU(3) are reproduced.** The
  invariants `d_F^abcd d_F^abcd / N_F = 5/12`, `d_F^abcd d_A^abcd /
  N_F = 5/2`, `d_A^abcd d_A^abcd / N_A = 135/8` are pure group-theoretic
  numbers reproduced by explicit SU(3) matrix algebra. This is support
  evidence for the value boundary, not a retained-status promotion.
...
- **β_3 contains quartic Casimirs but their channel weights are
  ...
  supported quartic-Casimir basis is consistent with arbitrary scalar
...
- **β_2 in any scheme.** Three-loop integral primitives remain required.
- **β_3 in any scheme.** Four-loop integral primitives remain required
  in the quartic-Casimir channels too.

### Final bounded statement

```
[SUPPORT — narrow]
SU(3) quartic Casimir invariants are reproduced:
...
β_3 contains quartic-Casimir channels but their scalar weights require
  4-loop integrals; supported quartic-Casimir VALUES alone do not fix
  the WEIGHTS.
...
The quartic-Casimir bridge does NOT derive β_2 or β_3 channel weights.
```
...
1. **Support check 1**: SU(3) quartic Casimir invariants
...
3. **NUMERICAL support check 3**: Reproduce the standard `beta_2`
...
4. **NEGATIVE structural check 4**: For `beta_3`, identify that
   ...
   4-loop integral primitives. Document the supported-vs-not boundary.
...
6. **HOSTILE-REVIEW check 6**: Apply the same analysis to 3-loop
   ...
   (cubic products are supported as values; channel weights are 3-loop
   integrals). This generalizes the negative result: at every loop
   order ≥ 3, supported Casimir products give the SKELETON, not the
   WEIGHTS.

7. **Honest result**: bounded; negative on the conjecture; support-only
   on the narrow Casimir-value reproduction sub-result.

## 5. Dependencies

- Repo baseline: `physical Cl(3) local algebra` on the `Z^3 spatial
  substrate`.
- Related open X-L1-MSbar PR #934 is context only; it is not a
  load-bearing dependency for this standalone quartic-Casimir no-go.
...
- [`YT_EW_COLOR_PROJECTION_THEOREM.md`](YT_EW_COLOR_PROJECTION_THEOREM.md)
  for supported `(C_F, C_A, T_F)` Casimir authority.
- [`YT_P3_MSBAR_TO_POLE_K3_COLOR_FACTOR_RETENTION_NOTE_2026-04-17.md`](YT_P3_MSBAR_TO_POLE_K3_COLOR_FACTOR_RETENTION_NOTE_2026-04-17.md)
  for the precedent pattern: color-tensor skeleton supported, scalar
...
- **A framework-native closed form for `beta_2` or `beta_3` in any scheme.**
  The bounded admission is that the runner reproduces channel skeletons
  and Casimir values but does not derive channel weights.
- **Promotion of any current MSbar import to supported.** The MSbar values
  for `beta_2, beta_3` remain external numerical inputs.
- **Direct contribution to closing Lane 1 alpha_s(M_Z).** This only
  forecloses the quartic-Casimir-value bridge.
- **A new axiom or new primitive.** This probe operates entirely within
  the repo baseline `physical Cl(3) local algebra` on the `Z^3 spatial
  substrate` plus standard SU(3) group-theory/QCD literature comparators.
...
|---|---|---|
| `d_F^abcd d_F^abcd / N_F = 5/12` | support-only group theory | Standard SU(3); this probe |
| `d_F^abcd d_A^abcd / N_F = 5/2` | support-only group theory | Standard SU(3); this probe |
| `d_A^abcd d_A^abcd / N_A = 135/8` | support-only group theory | Standard SU(3); this probe |
| 6-channel `beta_2` skeleton (no quartic) | literature comparator | TVZ 1980 + this probe |
| 14+-channel `beta_3` skeleton (with quartic) | literature comparator | VVL 1997 + this probe |
...
2. The standard MSbar 4-loop QCD beta function `beta_3` first
   ...
   `d_F d_A / N_F`); these channels' values are reproduced but their
...
3. Group-theoretic tensor algebra (Cl(3) ⊗ Cl(3) included) gives
   ...
   WEIGHTS at orders ≥ 3.
4. The quartic-Casimir bridge does not determine `beta_2, beta_3`
   channel weights: no quartic-Casimir bridge
...
supports and the negative conjecture stress-tests, with explicit
ADMITTED lines for the channel-weight obstructions, and a final
summary classifying the probe as `bounded_theorem` (support-only on
Casimir-value reproduction, negative on the conjecture, mostly negative
on full closed-form derivation).
