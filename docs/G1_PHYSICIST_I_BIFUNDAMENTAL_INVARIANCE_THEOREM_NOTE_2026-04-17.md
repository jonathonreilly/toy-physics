# G1 Physicist I — Bifundamental Invariance U(2)_L × U(2)_R on the Active Doublet Block: Obstruction Theorem

**Date:** 2026-04-17
**Branch:** `claude/g1-physicist-i` (off `claude/g1-complete`)
**Status:** **OBSTRUCTION THEOREM** (CASE 3). Five independent sole-axiom derivations converge: the retained atlas does **not** support independent `U(2)_L × U(2)_R` bifundamental invariance on the active-sheet doublet block `K_doublet`. The conditional closure gate that Physicist F identified is therefore **not available**.
**Script:** `scripts/frontier_g1_physicist_i_bifundamental_invariance.py`
**Framework convention:** "axiom" means only the single framework axiom `Cl(3)` on `Z^3`.

## Scope and discipline

Physicist F identified a conditional closure gate for G1: IF independent
...
gate for G1 therefore remains OPEN, and the Frobenius-route conditional
...
[G1_PHYSICIST_F_UNIQUENESS_AND_FULL_W_NOTE_2026-04-17](./G1_PHYSICIST_F_UNIQUENESS_AND_FULL_W_NOTE_2026-04-17.md)
...
Physicist I mission produces an independent derivation of the same
obstruction.
...
(retained,
[G1_Z3_DOUBLET_BLOCK_SELECTOR_SCHUR_BASELINE_PARTIAL_CLOSURE_NOTE_2026-04-17](./G1_Z3_DOUBLET_BLOCK_SELECTOR_SCHUR_BASELINE_PARTIAL_CLOSURE_NOTE_2026-04-17.md)).
...
of Physicist F Line 1). There is no extra invariance to be pinned
down.
...
- **Retained `Z_3` cyclic `C_3[111]`** (retained): the Z_3-parity
  decomposition (
  [G1_PATH_C_HOLONOMY_SELECTOR_NOTE_2026-04-17](./G1_PATH_C_HOLONOMY_SELECTOR_NOTE_2026-04-17.md))
...
- does not close G1 (the flagship gate remains OPEN)
...
The runner `scripts/frontier_g1_physicist_i_bifundamental_invariance.py`
...
D. **Frobenius preserved under bifundamental action on the non-Hermitian
   linear space** (Physicist F Line 1 re-verification): this is the
...
(`PYTHONPATH=scripts python3 scripts/frontier_g1_physicist_i_bifundamental_invariance.py`).
...
**Before this note (after Physicist F):**
```
...
pinned and G1 closes at (sqrt(6)/2 − sqrt(2)/18, sqrt(6)/6 + sqrt(2)/18).
...
- atlas obstruction row in
  ...
  parity-mixing note, Physicist E, and Physicist F
...
Frobenius-route conditional closure gate is unavailable. G1 remains
