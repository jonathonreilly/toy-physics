...
(O6)  Feuerbach: orthic triangle's circumcircle = nine-point circle of
      the original unitarity triangle.

(O7)  Area-ratio closed form:
        |Area_orthic / Area_triangle|
            =  alpha_s (20 + alpha_s) / (3 (80 + alpha_s^2)).

(O8)  Second degeneracy at alpha_s = 4 (where 4 - alpha_s = 0): the
      original triangle itself degenerates (V_3 -> V_1), and so does
      the orthic. Area_orthic vanishes at both alpha_s = 0 and
      alpha_s = 4.
```
...
(O1) The orthic triangle is the triangle with vertices (H_a, H_b, H_c).
     ...
     (Feuerbach 1822). For acute triangles, the orthic angles are
...
(O6) Feuerbach connection: orthic-circumcircle = nine-point circle.
...
(O8) Second-locus degeneracy at alpha_s = 4: the original triangle
     degenerates (V_3 -> V_1), and so does the orthic. Area_orthic
     vanishes at alpha_s = 0 AND at alpha_s = 4.
```
...
### O6: Feuerbach connection

By the classical Feuerbach theorem, the nine-point circle of any
triangle passes through nine canonical points, including the three
altitude feet `H_a, H_b, H_c`. So the orthic triangle's three
vertices lie on the nine-point circle, making it the orthic
circumcircle.
...
### O8: second-locus degeneracy at α_s = 4

At `α_s = 4`: `ρ̄ = (4-4)/24 = 0`, `η̄ = √5(4-4)/24 = 0`, so
`V_3 → V_1 = (0, 0)`. The original unitarity triangle collapses.
Consequently the orthic also collapses. The runner verifies
`Area_orthic | (α_s = 4) = 0`.
...
| O6 Feuerbach | `\|H_i - N9\|² = R9²` for each i | sympy `simplify(diff) == 0` |
...
| ---: | ---: | ---: |
| 0 (LO) | 0.0 | 0.0 |
| 0.118 | 0.001357 | 0.00838 |
| 0.30 | 0.003402 | 0.0224 |
| 1.0 | 0.01101 | 0.0871 |
| 4 | 0.0 | 0.0 |

The orthic triangle is degenerate at α_s = 0 (right-angle limit)
AND at α_s = 4 (full-triangle-degeneration limit), and is
non-degenerate strictly between.
...
5. **NEW Feuerbach verification** (O6): the orthic circumcircle
   equals the nine-point circle on the retained surface, verified
   symbolically.
...
7. **NEW second-locus degeneracy** (O8): at α_s = 4, the original
   triangle degenerates (`V_3 → V_1`) and so does the orthic. The
   Area_orthic polynomial in α_s vanishes at exactly two physical
   loci: α_s = 0 (right-angle limit) and α_s = 4 (vertex-collision
   limit).
...
inscribed in the nine-point circle by Feuerbach. For our retained
unitarity triangle, the orthic carries **two distinct degeneracy
loci** (α_s = 0 and α_s = 4), each tied to a specific geometric
feature:
...
numerator has two physical zeros (`α_s = 0` and `α_s = 4`) PLUS
one unphysical zero (`α_s = -20`, outside the physical α_s range).
Each zero corresponds to a specific structural degeneracy.

The α_s-invariance of `H_b` (NP8 of the nine-point branch) reappears
...
- `(O6)`: NEW retained Feuerbach-connection verification on the
  retained surface.
- `(O7)`: NEW retained orthic-to-original area ratio closed form.
- `(O8)`: NEW retained second-locus degeneracy at α_s = 4.
...
- Does **not** cite unmerged branches as load-bearing (companion
  nine-point branch under review is referenced for the
  α_s-invariance of `H_b`, but the closed forms are derived
  directly here).
...
- Verifies the second-locus degeneracy at α_s = 4.
