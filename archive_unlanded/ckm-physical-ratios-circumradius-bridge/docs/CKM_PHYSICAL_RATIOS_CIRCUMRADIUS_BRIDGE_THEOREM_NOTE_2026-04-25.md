...
+ NLO Wolfenstein protected-γ̄ surfaces. This note derives a
**NEW retained bridge** between **physical CKM ratios** (measured
in B-meson mixing experiments) and the **squared circumradius `R̄²`
of the unitarity triangle**: a clean structural-integer identity
that pulls a measurable observable into the geometric-invariant
language of the retained surface.
...
(R6)  COMPOUND CIRCUMRADIUS IDENTITY (NEW):
      ...
      with NO factor of alpha_s.
```

Both ratios connect **measured CKM observables** to a **classical
...
(R6) NEW COMPOUND CIRCUMRADIUS IDENTITY:
      ...
      with NO alpha_s dependence.

      At LO: R_bar | LO = 1/2, so |V_us V_td/(V_cb V_ts)|^2 | LO = 5/4.

(R7) Numerical comparison to PDG at canonical alpha_s ~ 0.103:
       |V_td/V_ts|  ~  0.207  (PDG: 0.211 +/- 0.005),
       |V_ub/V_cb|  ~  0.093  (PDG: 0.092 +/- 0.008),
       |V_us/V_cb|  ~  5.40   (PDG: 5.49 +/- 0.20).

     Each prediction is within < 2 sigma of PDG.
```
...
ratio is therefore **purely a function of R̄²** — independent of α_s.

At LO: `R̄|_LO = 1/2`, so `|V_us V_td/(V_cb V_ts)|²|_LO = 5/4`.

### R7: PDG comparison

Setting `α_s = 0.103` (framework canonical, extracted from
`α̃ ≈ 90.66°` and `α̃ − π/2 = (√5/20)α_s`):

| Ratio | Framework | PDG | σ |
| --- | ---: | ---: | ---: |
| `|V_td/V_ts|` | 0.207 | 0.211 ± 0.005 | 0.8 |
| `|V_ub/V_cb|` | 0.093 | 0.092 ± 0.008 | 0.1 |
| `|V_us/V_cb|` | 5.40 | 5.49 ± 0.20 | 0.5 |

Framework predictions match PDG within < 2σ for all three ratios.
...
**physical CKM ratios** (measured in experiments) and the
...
2. **NEW compound circumradius identity** (R6):
   ...
   with no `α_s` dependence at all.

3. **NEW retained closed forms** for `|V_us/V_cb|²` and `|V_ub/V_cb|²`
   in pure structural integers.

4. **Numerical PDG validation** (R7): all three ratios match
   experimental measurements within < 2σ at canonical `α_s`.
...
This note shows that **R̄² is also the natural unit for two
PHYSICAL CKM RATIOS** measured in B-meson mixing experiments:

- **|V_td/V_ts|² = (5/3) α_s R̄²** is a measurable ratio that
  controls the relative magnitude of `Δm_d/Δm_s` (B-d-meson vs
  B-s-meson mass-difference observables).
- **|V_us V_td/(V_cb V_ts)|² = 5 R̄²** is a four-element compound
  with α_s-independence.

The framework's protected-γ̄ structure therefore connects
**experimentally accessible CKM observables** to a **classical
geometric invariant** of the same unitarity triangle, with
structural-integer coefficients `(N_quark − 1, N_color)`.

This is the **first explicit bridge** between the framework's
unitarity-triangle geometry (retained `R̄²`) and a physical CKM
observable measured in B-physics experiments. It says that the
B-meson mixing ratio `|V_td/V_ts|²` is **literally measuring the
squared circumradius** (modulo `α_s` and structural integers) of
...
particularly rigid: it requires the **exact cancellation of α_s
between |V_us/V_cb|² and |V_td/V_ts|²**, which holds only because
both ratios have the specific structural-integer scaling derived here.
...
- `(R6)`: NEW retained COMPOUND IDENTITY
  `|V_us V_td/(V_cb V_ts)|² = (N_quark − 1) R̄²`, **α_s-independent**.
- `(R7)`: NEW numerical PDG comparison showing < 2σ agreement for
...
- Compares each ratio's numerical value to PDG at canonical α_s.
