...
Key NEW identities on the retained NLO Wolfenstein protected-gamma_bar surface:
  ...
  (R6) NEW compound CIRCUMRADIUS identity:
       ...
       triangle, with NO factor of alpha_s.

  (R7) Numerical comparison to PDG at canonical alpha_s ~ 0.103:
        |V_td/V_ts|       ~  0.207  (PDG: 0.211 +/- 0.005)
        |V_ub/V_cb|       ~  0.093  (PDG: 0.092 +/- 0.005 estimating)
        |V_us/V_cb|       ~  5.49   (PDG: 0.225/0.041 ~ 5.49).
...
def audit_r7_pdg_comparison(N: dict, S: dict) -> None:
    banner("R7: PDG numerical comparison")
    ...
    # Test at canonical alpha_s ~ 0.103 (framework value extracted from
    # alpha_LO = pi/2 + (sqrt(5)/20) alpha_s with alpha_LO ~ 90.66).
    samples = [
        ("alpha_s = 0.103 (framework canonical)", 0.103),
        ("alpha_s = 0.118 (PDG-ish at M_Z)", 0.118),
    ]

    pdg_values = {
        "|V_td/V_ts|": (0.211, 0.005),  # PDG ~ 0.211
        "|V_ub/V_cb|": (0.092, 0.008),  # estimating
        "|V_us/V_cb|": (5.49, 0.20),    # 0.225/0.041
    }
    ...
    for label, s in samples:
        ...
        print(f"    |V_td/V_ts|  =  {ratio_td_ts:.4f}  (PDG: {pdg_values['|V_td/V_ts|'][0]} +/- {pdg_values['|V_td/V_ts|'][1]})")
        print(f"    |V_ub/V_cb|  =  {ratio_ub_cb:.4f}  (PDG: {pdg_values['|V_ub/V_cb|'][0]} +/- {pdg_values['|V_ub/V_cb|'][1]})")
        print(f"    |V_us/V_cb|  =  {ratio_us_cb:.4f}  (PDG: {pdg_values['|V_us/V_cb|'][0]} +/- {pdg_values['|V_us/V_cb|'][1]})")
        print()

    # Sanity check: framework agrees with PDG at canonical alpha_s.
    s_canonical = 0.103
    ratio_td_ts = float(sp.sqrt(Vtd_Vts_sq).subs(a_s, s_canonical))
    pdg_central, pdg_err = pdg_values["|V_td/V_ts|"]
    sigma = abs(ratio_td_ts - pdg_central) / pdg_err
    print(f"  At alpha_s = 0.103: |V_td/V_ts| within {sigma:.2f} sigma of PDG.")
    check("R7: |V_td/V_ts| at canonical alpha_s within 2 sigma of PDG",
          sigma < 2)
...
def audit_summary() -> None:
    ...
    print("         with NO factor of alpha_s.")
    print()
    print("    (R7) Framework predictions at canonical alpha_s ~ 0.103 match PDG")
    print("         within < 2 sigma for all three ratios |V_td/V_ts|, |V_ub/V_cb|,")
    print("         |V_us/V_cb|.")
