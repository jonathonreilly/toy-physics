...
Using `4 K = abc/R` (with `K` = area), this rewrites as

```text
OK^2  =  R^2 - 3 (4 K R)^2 / perim_sq^2  /  R^2
      =  R^2 - 48 K^2 / perim_sq^2 * R^2 / R^2
      =  R^2 (1 - 48 K^2 / perim_sq^2).
```

For our triangle:

```text
48 K^2 / perim_sq^2  =  48 * (eta_bar / 2)^2 / [(alpha_s^2 - 4 alpha_s + 96)/48]^2
...
which (after clearing the denominator `[4 sqrt(5)(4 - alpha_s)]^2`)
reduces to `P(alpha_s) ≥ 0`.
...
| 0.118 | (0.16290, 0.18158) | (0.50000, −0.00659) | 0.37911 |
| 0.30 | (0.15679, 0.17467) | (0.50000, −0.01677) | 0.37491 |
...
the **Brocard angle** `ω̄` (companion branch
`ckm-barred-brocard-angle-exact-closed-form`, just landed), and the
**Jarlskog** `J̄`. The **symmedian (Lemoine) point** `K` and the
...
6. **NEW deep observation: P(α_s) governs both Brocard bounds (K10).**
   ...
   simultaneously controls:
    - the Brocard inequality `cot(ω̄) ≥ √3` (companion theorem),
...
- Does **not** cite unmerged branches (the companion Brocard-angle
  theorem branch is referenced for context only; the K10 connection
  is self-contained — it derives from `P(α_s)`'s appearance in `OK²`).
