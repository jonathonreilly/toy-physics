...
- So the sharpened corrected claim is: `deep-pocket` is the main `extended` escape route, but not the only one. `Extended` has a route lattice rather than one backup: `high-degree` first, then `deep-pocket`, then pocket-adjacent mixtures or low-degree routes depending on what remains, and finally a weaker but still real non-cavity sparse-structure family once the whole cavity family is stripped out.
