...
- the remaining unmatched residue is now much narrower:
  ...
  - a direct comparer against the historical frozen `pair-only-sensitive` rows shows those surviving generated failures are not just a single shifted load or density branch:
    ...
    - so the post-guard transfer break is now best read as a second nearby domain boundary: a moderate anchor-balance regime with one high-mid-anchor frozen `add4` pocket plus two generated realizations, a skew-wrap right/deep shoulder and a `mode-mix-f` low-support throat, while the immediate taper/skew neighborhood remains failure-only after the zero-support guard

The open problem in this basin is no longer whether the family structure exists, whether the frozen bucket exact-closes, or why `mode-mix-f` sits inside the anchor band. It is determining how stable the current two-realization picture is beyond the bounded representative set: the zero-support collapse edge is already isolated, and the later moderate anchor-balance band plus `mid_anchor_closure_peak <= 10.000` ceiling now reduces to a skew-wrap right/deep shoulder, a `mode-mix-f` low-support throat, and a frozen `add4` mid-anchor knot, while the immediate taper/skew neighborhood that survives the support-collapse guard remains failure-only.
...
- whether the two generated realizations now seen inside the moderate anchor band, a skew-wrap right/deep shoulder and a `mode-mix-f` low-support throat, survive on a broader nearby generated basis or are only a bounded representative-set picture
