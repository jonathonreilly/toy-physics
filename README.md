...
- whether the present three-way beyond-ceiling split is already complete on deeper base sweeps, or whether additional shared-packet subbranches still appear beyond the current `outer-rect`, `taper-hard`, and `skew-wrap` set
...
- low-overlap subtype competition is now much more compressed on the frozen `5504` bucket and exact across the available historical frontier ladder; the first nearby generated edge (`geometry-c/e`) isolates cleanly under a support-collapse guard, and the surviving non-guarded generated pair-only failures (`mode-mix-f`, `local-morph-c`, `mode-mix-d`) now exact-separate from frozen historical `pair-only-sensitive`, `add1-sensitive`, and `add4-sensitive` rows once the moderate anchor-balance band is tightened with `mid_anchor_closure_peak <= 10.000`. Above that ceiling, the current coarse continuation is now cleaner: the held-out `outer-rect` pair at `ultra|mega` shares the same `8/12` mid-packet lift and four-incident flank hinge as the five observed late rows, so the packet-completion law should now be read as the common beyond-ceiling mechanism rather than as a late-only signature. The bounded within-family compare closes the immediate separator on the present basis: the shared packet family already splits into an `outer-rect` low-right-bridge tail, a `taper-hard` positive-intensity two-right-bridge branch, and a `skew-wrap` negative-deep no-right-bridge branch. Finished non-base late guardrails still yield no candidate row at all, and helper-fixed reruns preserve that empty wall in substance. The nearest exhausted `large:exa` and `mirror:exa` misses still show a shared `mid_anchor_closure_peak = 8.000` ceiling, and the widened packet compare plus direct packet-neighborhood compare show that ceiling in local support language: both exhausted-wall misses stop at `mid_candidate_attached_max = 7.000` because their dominant mid packet is missing the same inward left-flank four-incident hinge, while all five observed late rows and the held-out outer-rect pair restore that hinge and its four incident bridge-bridge closures. What remains unresolved is whether deeper base sweeps add more beyond-ceiling shared-packet members, and whether any non-base generated family ever realizes the same packet completion
