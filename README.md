...
The open problem in this basin is no longer whether the family structure exists, whether the frozen bucket exact-closes, why `mode-mix-f` sits inside the anchor band, whether the outer `rect-wrap:local-morph-f` pair is the same mechanism as the frozen `add4` pocket, or whether the nearest widened sparse rect guardrail broadens that outer continuation. The zero-support collapse edge is already isolated, the later moderate anchor-balance band plus `mid_anchor_closure_peak <= 10.000` ceiling still reduces to a skew-wrap right/deep shoulder, a `mode-mix-f` low-support throat, and a frozen `add4` mid-anchor knot, and the `rect-wrap/rect-hard` guardrail still resolves into a heavier beyond-ceiling knot-side continuation under `closure_load >= 75.000`, but the current bounded sparse evidence keeps that continuation local to `rect-wrap:local-morph-f`. The next question is which slightly wider or differently placed beyond-ceiling guardrail yields the first non-rect non-collapse row, and whether the same high-load clause survives there.
