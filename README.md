...
- how to translate the now-confirmed inward left-flank mid-packet completion at relative `(-1, 0)` into a cleaner physical or support-topology law that does not depend on raw cell coordinates
...
- whether any nearby generated family can still break that now-confirmed local packet-completion law even when the coarse anchor-layout statistics remain matched, as in the neutral-balance `skew-wrap` vs `mirror:exa` pair
