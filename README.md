...
- So the sharper corrected claim is: `deep-pocket` is the main `extended` escape route, but not the only one. `Extended` has a small route hierarchy: `high-degree` first, then `deep-pocket`, then pocket-adjacent mixtures, and finally a weaker low-degree-neighbor fallback once the whole cavity family is stripped out.
