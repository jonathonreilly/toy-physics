  ...
  - a direct comparer against the historical frozen `pair-only-sensitive` rows shows those surviving generated failures are not just a single shifted load or density branch:
    ...
    - projecting that same band onto the queued contrast cohorts keeps it exact against `add1` but reveals one small frozen `add4` pocket:
      ...
      - one sparse outer generated guardrail slice over `rect-wrap`/`rect-hard` at `ultra` and `mega` adds no new refined-band generated row:
        ...
        - physically, the outer guardrail does not create a third generated realization inside the refined band; it lands directly on the same mid-anchor-knot side of the ceiling that already excluded the frozen `add4` pocket (`closure_load = 80.000`, `anchor_deep_share_gap = 0.000`, `high_bridge_right_count = 1.000`)
    - so the post-guard transfer break is now best read as a second nearby domain boundary: a moderate anchor-balance regime with one high-mid-anchor frozen `add4` pocket plus two generated realizations, a skew-wrap right/deep shoulder and a `mode-mix-f` low-support throat, while one sparse outer `rect-wrap` guardrail adds no third in-band realization because its only non-collapse failures already sit on the knot side of the refined ceiling

The open problem in this basin is no longer whether the family structure exists, whether the frozen bucket exact-closes, why `mode-mix-f` sits inside the anchor band, or whether one sparse outer guardrail immediately breaks the shoulder/throat picture. The zero-support collapse edge is already isolated, the later moderate anchor-balance band plus `mid_anchor_closure_peak <= 10.000` ceiling still reduces to a skew-wrap right/deep shoulder, a `mode-mix-f` low-support throat, and a frozen `add4` mid-anchor knot, and the `rect-wrap/rect-hard` `ultra/mega` guardrail contributes no new refined-band generated row. The next question is whether the outer `rect-wrap:local-morph-f` pair is physically the same overload-knot branch as the frozen `add4` pocket or a distinct knot-side continuation beyond the refined ceiling.
...
- whether the refined mid-anchor ceiling is excluding one shared knot family across the frozen `add4` pocket and the outer `rect-wrap:local-morph-f` failures, or two different overload mechanisms
- how the exact branch-aware law should be physically interpreted at both the zero-support collapse boundary and the new anchor-balance basin split between right/deep bridge shoulder, low-support throat, and frozen mid-anchor knot
