...
- A thirty-fourth diagnostic then removes the hard-threshold cheat in the opposite direction: it replaces the high-degree motif with smooth hub-exposure scores, using linear and quadratic ramps over neighbor degree on `5..8` and `6..8`.
- Those soft scores do not preserve the mechanism. `soft-linear-5` and both quadratic variants push `compact` back to parity size `7`, while `soft-linear-6` does a little better but still only reaches `compact` parity at `5` and never recovers clean `extended` parity.
- The strongest result is the bundle: even all four smooth hub-exposure scores together fail to recover clean parity in either family, with `compact` still below parity at `-0.06/-0.03` and `extended` still just below the line.
- So the mechanism claim sharpens again: the load-bearing coordinate really is more threshold-like than a smooth hub-exposure proxy. In the current generated-geometry benchmark, a narrow `6..7` near-hub condition works, while broader soft exposure does not.
- A thirty-fifth diagnostic then tests whether `degree` itself was a red herring by swapping in a more graph-native hub notion: thresholded second-hop reach of neighboring nodes.
