	          ...
	          - widening that packet compare to the full exhausted wall gives one cleaner structural separator still:
	            ...
	            - the corresponding mid-anchor bridge-bridge closed-pair maxima remain `12.000` on the late side and `8.000` on the exhausted-wall side
		          ...
		          - a direct packet-neighborhood compare now localizes that lift:
		            ...
		            - the closest `large` miss still keeps that full octagon on the left anchor band, but its dominant mid packet is missing the inward left-flank support node at relative `(-1, 0)`
		            - the `mirror` miss shares that same mid-packet hole, while its dominant left packet is also one support short
		            - that single missing mid support node deletes exactly four bridge-bridge support closures, which is the whole local difference between the exhausted-wall `7/8` packet and the late-branch `8/12` packet
...
- whether the sharper translation is now simply completion of the inward left-flank mid support node at relative `(-1, 0)`, given that both exhausted-wall misses share that exact missing local neighborhood while the closest `large` miss still keeps the full octagon on the left anchor band
...
- whether the neutral-balance `skew-wrap` vs `mirror:exa` pair plus the new packet-neighborhood compare means the remaining transfer law is now completely local packet completion rather than another anchor-layout change, since both rows keep the same high-bridge placement, candidate counts, dense counts, and `mid_candidate_closed_ratio_max = 0.500`
