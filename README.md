...
- A continuing generated-family ladder then pushed the same check out through `11/6`, `13/7`, `15/8`, and `17/9` (`ultra`, `mega`, `giga`, `tera`). The support-level compact result stayed unchanged at every rung, and the parity-level split stayed strikingly stable too: `full-rich` and `ge_7` remained `shared-same`, `ge_6` remained `shared-proxy`, and `share6` plus the exposure bundle remained `extended-only`.
