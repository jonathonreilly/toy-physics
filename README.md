...
- Beyond-ceiling continuation: above that ceiling, the stable coarse law is the shared `8/12` mid-packet lift. On the bounded confirmation set, `mid_anchor_closure_peak >= 10.000`, `mid_candidate_bridge_bridge_closed_pair_max >= 10.000`, `mid_candidate_attached_max >= 7.500`, and the four-incident flank hinge are all equivalent. This broader continuation already includes the held-out `outer-rect` pair at `ultra|mega` as well as the later `base` `peta|exa` non-rect rows, and on the current five-row basis it already splits cleanly into an `outer-rect` low-right-bridge tail, a `taper-hard` positive-intensity two-right-bridge branch, and a `skew-wrap` negative-deep no-right-bridge branch. The nearest wider base sentinel `wider:base:skew-wrap:local-morph-c` falls back to the depleted `7/8` packet with no flank hinge, so the current three-way split does not widen immediately into a fourth shared-packet subbranch; however, the taper-hard intensity clause leaks on that row and therefore needs the shared packet context to remain exact.
...
- whether any wider/deeper base sentinel beyond `wider:base:skew-wrap:local-morph-c` ever rejoins the shared `8/12` packet regime, or whether the current three-way beyond-ceiling split is already the whole immediate shared-packet family
...
- whether the taper-hard positive-intensity split can be rewritten in packet-gated physical language that stays exact once the compare steps outside the current five-row shared-packet basis
...
- low-overlap subtype competition is now much more compressed on the frozen `5504` bucket and exact across the available historical frontier ladder; the first nearby generated edge (`geometry-c/e`) isolates cleanly under a support-collapse guard, and the surviving non-guarded generated pair-only failures (`mode-mix-f`, `local-morph-c`, `mode-mix-d`) now exact-separate from frozen historical `pair-only-sensitive`, `add1-sensitive`, and `add4-sensitive` rows once the moderate anchor-balance band is tightened with `mid_anchor_closure_peak <= 10.000`. Above that ceiling, the current coarse continuation is now cleaner: the held-out `outer-rect` pair at `ultra|mega` shares the same `8/12` mid-packet lift and four-incident flank hinge as the five observed late rows, so the packet-completion law should now be read as the common beyond-ceiling mechanism rather than as a late-only signature. The bounded within-family compare closes the immediate separator on the present basis: the shared packet family already splits into an `outer-rect` low-right-bridge tail, a `taper-hard` positive-intensity two-right-bridge branch, and a `skew-wrap` negative-deep no-right-bridge branch. A first wider-base guardrail now says the immediate answer is still no fourth shared-packet branch: `wider:base:skew-wrap:local-morph-c` drops back to `mid_anchor_closure_peak = 8.000`, `mid_candidate_attached_max = 7.000`, and no flank hinge. But that same row also shows the present taper-hard intensity clause is not a globally stable separator by itself, because `anchor_closure_intensity_gap >= 1.000` already leaks there below the packet ceiling. Finished non-base late guardrails still yield no candidate row at all, and helper-fixed reruns preserve that empty wall in substance. The nearest exhausted `large:exa` and `mirror:exa` misses still show a shared `mid_anchor_closure_peak = 8.000` ceiling, and the widened packet compare plus direct packet-neighborhood compare show that ceiling in local support language: both exhausted-wall misses stop at `mid_candidate_attached_max = 7.000` because their dominant mid packet is missing the same inward left-flank four-incident hinge, while all five observed late rows and the held-out outer-rect pair restore that hinge and its four incident bridge-bridge closures. What remains unresolved is whether deeper base sweeps beyond that nearest wider guardrail add more shared-packet members, and whether any non-base generated family ever realizes the same packet completion
