...
- whether the current beyond-ceiling map is already complete as one shared packet-lift continuation split into `outer-rect`, `taper-hard`, and `skew-wrap` subbranches, or whether deeper base sweeps will add more members
- whether the held-out `outer-rect` pair and the later non-rect rows are best understood as one common beyond-ceiling regime with different subbranch coordinates, rather than as separate mechanisms
