  ...
  - a direct comparer against the historical frozen `pair-only-sensitive` rows shows those surviving generated failures are not just a single shifted load or density branch:
    ...
    - projecting that same band onto the queued contrast cohorts keeps it exact against `add1` but reveals one small frozen `add4` pocket:
      ...
      - the next tested non-rect late guardrails also stay empty:
        ...
        - one tier later, the mirrored late slice still stays empty: on `mirror` at `giga|tera`, the same non-rect late slice yields `scanned_nonrect_combinations = 4` and `first_nonrect_row = none`
        - so no tested late non-rect guardrail has yet produced any beyond-ceiling non-collapse row at all
...
The open problem in this basin is no longer whether the family structure exists, whether the frozen bucket exact-closes, why `mode-mix-f` sits inside the anchor band, whether the outer `rect-wrap:local-morph-f` pair is the same mechanism as the frozen `add4` pocket, or whether the nearest slightly wider or mirrored late guardrail broadens that outer continuation. The zero-support collapse edge is already isolated, the later moderate anchor-balance band plus `mid_anchor_closure_peak <= 10.000` ceiling still reduces to a skew-wrap right/deep shoulder, a `mode-mix-f` low-support throat, and a frozen `add4` mid-anchor knot, and the `rect-wrap/rect-hard` guardrail still resolves into a heavier beyond-ceiling knot-side continuation under `closure_load >= 75.000`, but the current bounded sparse evidence keeps that continuation local to `rect-wrap:local-morph-f` even after testing the next wider `large` late slice, the mirrored `mirror` late slice, and the mirrored family one tier later at `giga|tera`. The next question is which more distant guardrail, if any, yields the first non-rect beyond-ceiling non-collapse row, and whether the same high-load clause survives there.
...
- whether any more distant late guardrail beyond the tested `large` `ultra|mega` slice and `mirror` `ultra|mega|giga|tera` slices yields a non-rect beyond-ceiling non-collapse row at all
