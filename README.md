...
- A small seeded-random perturbation ensemble now pushes on the same question from the opposite direction: not hand-picked nudges, but random removable/addable node choices drawn from each benchmark graph. In the current run, the same split survives that test too.
- Under the random ensemble, `mixed`-frontier overlap still survives in `26/26` cases for both `compact` and `extended`, while robustness overlap survives in `22/26` and `21/26`. By contrast, the exact unperturbed winner is retained in only `7/26` compact and `4/26` extended random perturbation cases.
- That is the strongest selector-free result so far: once we stop insisting on one exact winning motif, the mixed-frontier family looks robust under deterministic nudges, a small high-end weight shift, and now a small seeded-random graph ensemble as well.
...
- The cross-weight perturbation comparison is also still narrow: it currently checks only the high-end pair `w = 0.95` and `w = 1.0`, so it tells us about local weight-robustness near the spent-delay point rather than about the whole retained-weight family.
- The random perturbation ensemble is also intentionally small and seeded: it currently samples only two random variants per scenario and still tracks the unperturbed frontier palette, so it is a first robustness probe rather than a large-scale random-graph study.
