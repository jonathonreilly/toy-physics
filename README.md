...
- On the corrected generated ensemble, the full rich basis now reaches parity at size `3` in both families, but the winning feature split is asymmetric: `compact` still lands on `motif_high_degree_neighbor_fraction`, while `extended` can already land on pocket-adjacent alternatives.
...
- Removing `motif_high_degree_neighbor_fraction` is still the real compact-side hit: `compact` retreats to parity size `7`, and simple share/count/max replacements do not restore the original fast collapse.
- `Extended` behaves differently on the corrected path. It often keeps parity size `3` even when the high-degree feature is removed, typically by switching to `motif_deep_pocket_adjacent_fraction`.
...
- In `compact`, the immediate threshold story survives the fix: `ge_5`, `ge_6`, and `ge_7` all recover parity size `3`, while `ge_8` fails.
...
- On the corrected default benchmark, `reach-14` can recover the compact parity threshold, but that does not survive the broader multi-style hub comparison: on the broader rerun, `reach-14` and `reach-24` both fall back to pocket-based compact winners at parity sizes `8` and `7`.
...
- A final bridge test sharpens that corrected picture. On the canonical sparse route, adding back `motif_high_degree_neighbor_fraction` restores `compact` immediately at parity size `3` in both ensembles. Adding back `motif_pocket_adjacent_fraction` or the whole pocket family only restores `compact` weakly at size `6`, while `motif_deep_pocket_adjacent_fraction` and `motif_low_degree_neighbor_fraction` do not restore `compact` at all. So the `compact` side is near the sparse route, but the clean bridge still comes from the near-hub threshold family rather than from the cavity or low-degree escape routes.
- A follow-up non-hub bridge sweep makes that dependence harder to soften. No tiny non-high-degree combination closes `compact` on the sparse route: `pocket + deep-pocket` still only reaches size `6`, `pocket + low-degree` and the full `pocket + deep-pocket + low-degree` triple only reach size `7`, and the remaining non-hub pairs do not restore parity at all.
- A corrected threshold-proxy rerun then completely overturns the earlier exact-coordinate reading. On the sparse route, `ge_6`, `ge_7`, and `share6` all restore `compact` immediately at parity size `3` in both ensembles, and the full threshold bundle also restores parity by collapsing back to `share6`. What does **not** bridge are `count6`, `share7`, and `count7`, either alone or in any combination that excludes `ge_6`, `ge_7`, and `share6`.
