...
- A fifth diagnostic then bootstraps the basis choice itself instead of fixing a small palette by hand. It combines `16` subset bases (all 3- and 4-metric subsets plus the full five-metric basis) with `12` seeded random projection bases, for `28` total basis views on the same harmonic-continuous candidate pool.
- The full-basis reference still keeps the selected rule on `pc123` in `153/156` cases, and every sampled basis has nonempty `pc123` overlap with that full-basis frontier in all `156/156` cases. So the full derived frontier is not a fragile one-basis artifact.
- The bootstrap stability map is more informative than the smaller basis ablation because it separates ubiquity from dominance. In this run, `S[2,3]/B[3,4]` is the most ensemble-stable `pc123` motif in both families:
  - `compact`: present in `28/28` bases, `1744` basis-case frontiers, `19` top-basis wins
  - `extended`: present in `28/28` bases, `1619` basis-case frontiers, `20` top-basis wins
- But it is not the only ubiquitous motif. Multiple motif-family pairs appear on `pc123` in every sampled basis, so the bootstrap result is not “one motif survives and everything else disappears.” The stronger claim is narrower: some motifs are basis-ubiquitous, and `S[2,3]/B[3,4]` currently dominates that ubiquitous set by aggregate coverage and top-basis wins.
- That is the strongest selector-invariant stability map in the repo so far. It no longer asks which motif wins under one chosen basis, or even a few hand-picked bases, but which motifs keep reappearing on the full derived frontier across a whole ensemble of basis choices.
