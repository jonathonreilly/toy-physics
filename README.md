...
- **Main open problem:** the good topology is still imposed, not generated.
  ...
  intermediate `N`; by `N=80..100` the ceiling returns. The newer 3D
  self-regulating pruning lane is the first bounded positive dynamics result:
  fixed-threshold pruning helps through `N=50`, and adaptive-quantile pruning
  extends the useful window through `N=60` with all seeds still valid, but
  both deletion-only rules still fail by `N=80`. The next make-or-break test
  is therefore no longer “any pruning at all,” but whether a **hard-gap,
  geometry-anchored node placement / node removal law** can be made stable
  with connectivity-aware feedback or coupled birth/death updates, with the
  fallback read that the gap may itself be part of the effective physics
  rather than a derivative of a simpler local growth rule.
...
- [SOURCE_AWARE_MECHANISM_NOTE.md](/Users/jonreilly/Projects/Physics/docs/SOURCE_AWARE_MECHANISM_NOTE.md)
...
- the higher-dimensional story is now split more cleanly than before: modular 3D and 4D substantially improve decoherence, 4D modular channels currently give the strongest large-`N` CL-bath lane on disk, and higher-dimensional chokepoint Born-rule checks are clean; but the stricter 3D joint rerun keeps decoherence more clearly than gravity, even the newer binned 4D true-visibility check is only weak/intermittent, and the distance law remains effectively flat/topological across the tested retained lanes
- the 5D picture is still density-sensitive and narrower than the earlier branch story: under the corrected control pool, the live signal sits inside a very narrow dense modular corner, and the stability-aware follow-up shows that this corner spans multiple nearby rows rather than a single isolated point
- the distance-law closure is now broader than the earlier 4D sweep alone: propagator-power sweeps, fixed-mass locality-shell sweeps, a minimal local saturation nonlinearity, and an induced/effective-distance proxy readout all fail to recover a clean `1/b` law. The only remaining seam that still moved the exponent at all was source-aware coupling, but the explicit non-overlapping template pilots now show it is family-sensitive: the retained 3D family improves while the denser control family, the retained-4D transfer pilot, and the dense 5D follow-up all flip back or fail, even though a separate 4D template-availability scan finds a tiny clean corner in the denser holdout. A newer detector-side purity ablation/rescue pair is also negative for the simple causal story: purity shifts modestly, but the source-aware seam does not collapse or cleanly appear. So the open lane is no longer another local-weight, shell, or purity tweak; it is now an analytic explanation or a deeper architecture change. See [docs/SOURCE_AWARE_MECHANISM_NOTE.md](/Users/jonreilly/Projects/Physics/docs/SOURCE_AWARE_MECHANISM_NOTE.md) for the current mechanism summary.
...
14. **The good decoherence-supporting topology is still imposed rather than generated.**  
    Gap-controlled modular DAGs are now the first family where gravity and decoherence both work together, but the separating gap is currently built into the generator. Most emergence-style attempts still fail to produce a stable local law: feedback-style connection-growth rules and placement rules are negative, and deletion-only pruning is only a bounded partial positive. Fixed-threshold self-regulation helps through `N=50`, adaptive-quantile pruning extends the useful window through `N=60`, but both still fail by `N=80`. The next real theory hurdle is therefore whether a hard-gap node placement or node-removal law can be stabilized with connectivity-aware or birth/death updates, or whether that gap should be treated as part of the effective physics.
