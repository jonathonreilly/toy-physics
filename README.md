...
- **Topology pivot: positive result.** Within the modular / gap-controlled
  ...
  asymptotic lane. The current large-`N` script still measures
  both-slits-open detector-profile contrast rather than a full
  single-vs-double-slit visibility, so the interference claim should be read
  at that narrower level for now.
