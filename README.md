...
- the remaining unmatched residue is now much narrower:
  ...
  - a direct comparer against the historical frozen `pair-only-sensitive` rows shows those surviving generated failures are not just a single shifted load or density branch:
    ...
    - projecting that same band onto the queued contrast cohorts keeps it exact against `add1` but reveals one small frozen `add4` pocket:
      ...
      - but the next more distant base late slice finally does yield the first non-rect beyond-ceiling non-collapse row:
        - on `base` at `peta`, `scanned_nonrect_combinations = 1` and `first_nonrect_row = base:peta:base:taper-hard:local-morph-f`
        - that first non-rect hit still keeps the same compact high-load clause with `closure_load = 76.000`, so `closure_load >= 75.000` survives off the rect tail
...
- which adjacent late guardrails around the new `base:peta` onset also realize a non-rect beyond-ceiling non-collapse row, and whether `closure_load >= 75.000` remains exact there
- whether the new `base:peta:base:taper-hard:local-morph-f` hit shares the same mechanism as the outer `rect-wrap:local-morph-f` tail or opens a distinct late non-rect branch
