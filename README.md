...
- the remaining unmatched residue is now much narrower:
  ...
  - a direct comparer against the historical frozen `pair-only-sensitive` rows shows those surviving generated failures are not just a single shifted load or density branch:
    ...
    - projecting that same band onto the queued contrast cohorts keeps it exact against `add1` but reveals one small frozen `add4` pocket:
      ...
      - but the more distant base late slice does yield a persistent non-rect beyond-ceiling non-collapse branch:
        - on `base` at both `peta` and `exa`, `scanned_nonrect_combinations = 1` and `first_nonrect_row = base:<tier>:base:taper-hard:local-morph-f`
        - the same first non-rect hit repeats with unchanged observables on both tiers: `support_load = 22.000`, `closure_load = 76.000`, `mid_anchor_closure_peak = 12.000`, `anchor_closure_intensity_gap = 4.000`, `anchor_deep_share_gap = 0.667`, `high_bridge_right_count = 2.000`
        - that repeated non-rect hit still keeps the same compact high-load clause, so `closure_load >= 75.000` survives off the rect tail on a persistent late `taper-hard` branch rather than as a one-tier blip
        - physically, the beyond-ceiling continuation is therefore not purely rect-local: it reappears on a more distant non-rect `taper-hard` slice with the same ceiling breach (`mid_anchor_closure_peak = 12.000`) and a stronger right/deep shoulder (`anchor_deep_share_gap = 0.667`, `high_bridge_right_count = 2.000`)
    - so the post-guard transfer break is now best read as a second nearby domain boundary: a moderate anchor-balance regime with one high-mid-anchor frozen `add4` pocket plus two generated realizations inside the refined band, a skew-wrap right/deep shoulder and a `mode-mix-f` low-support throat, while the sparse outer `rect-wrap` failures and the persistent distant `base:taper-hard` late branch form a heavier beyond-ceiling continuation beyond that refined ceiling

The open problem in this basin is no longer whether the family structure exists, whether the frozen bucket exact-closes, why `mode-mix-f` sits inside the anchor band, whether the outer `rect-wrap:local-morph-f` pair is the same mechanism as the frozen `add4` pocket, or whether any non-rect late guardrail ever broadens that outer continuation. The zero-support collapse edge is already isolated, the later moderate anchor-balance band plus `mid_anchor_closure_peak <= 10.000` ceiling still reduces to a skew-wrap right/deep shoulder, a `mode-mix-f` low-support throat, and a frozen `add4` mid-anchor knot, and the `rect-wrap/rect-hard` guardrail still resolves into a heavier beyond-ceiling continuation under `closure_load >= 75.000`; now the more distant base late slice also participates persistently, with `base:taper-hard:local-morph-f` appearing as the first non-rect beyond-ceiling non-collapse row on both `peta` and `exa` while retaining `closure_load = 76.000`. The next question is how broad that late non-rect branch really is, and which compact observables best translate it physically.
...
- whether the persistent `base:taper-hard:local-morph-f` late branch on `peta|exa` is the whole non-rect beyond-ceiling continuation or just its first visible member
- whether the new `base:peta|exa:base:taper-hard:local-morph-f` branch shares the same mechanism as the outer `rect-wrap:local-morph-f` tail or opens a distinct late non-rect branch
