...
**Current read:** no tested transport architecture currently passes gravity scaling, interference, and decoherence scaling simultaneously. G2-style macro averaging is now understood as both a real gravity fix and a real failure mode: it suppresses the near-degenerate path multiplicity that causes gravity saturation, but it also washes out the phase structure that produces fringes and weakens the branch-distinguishing microstructure needed for scalable decoherence. The readout-only alternative is still the cleaner gravity lane. The new transfer compare sharpens that lane further: packet-local flow is a real random-DAG refinement, but the retained cross-family law is still the coarser near-mass `action_channel`. So the next good move is not another single-layer env variant, and not simply “implement two-scale” again. It is to preserve the microscopic corrected transport, keep gravity coarse-graining at the observable/readout layer, and explain when the packet-local refinement helps versus when the broader channel is the honest stable read.
