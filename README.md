	      ...
	      - on the already finished non-base late guardrails, there is still no broader-family transfer evidence for that branch:
	        ...
	        - a direct exhausted-slice compare now shows what that empty wall is hiding:
	          ...
	          - a tighter row-level compare against the closest `large:exa` miss shows that the miss is not short on absolute bridge-bridge closure scale:
	            ...
	            - the focused structural separator is therefore not extra load but anchor placement: `delta_mid_left_bridge_bridge_closed_pair_max >= 0.000`, where `delta = mid - left`
		          - widening that anchor compare to the full exhausted wall gives one clean structural separator:
		            - every observed late row has `mid_candidate_bridge_bridge_closed_pair_max = 12.000`
		            - both exhausted-wall misses stop at `mid_candidate_bridge_bridge_closed_pair_max = 8.000`
		            - `mid_candidate_bridge_bridge_closed_pair_max >= 10.000` exact-separates the whole observed late branch from the whole exhausted wall
...
- whether the most stable structural translation is now simply `mid_candidate_bridge_bridge_closed_pair_max >= 10.000`, since that exact-separates the whole observed late branch from the whole exhausted wall
