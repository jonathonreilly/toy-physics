...
- Once the high-degree route is gone, `extended` first reroutes to `motif_deep_pocket_adjacent_fraction`. If `deep-pocket` is removed as well, it falls back to a pocket-adjacent mixed subset. If the whole pocket family is removed together with the high-degree feature, `extended` still keeps fast parity through `motif_low_degree_neighbor_fraction`, while `compact` loses fast parity entirely. And if that low-degree route is removed too, `extended` still survives through a weaker degree-profile summary (`degree_8_fraction + motif_mean_neighbor_degree`).
- So the sharper corrected claim is: `deep-pocket` is the main `extended` escape route, but not the only one. `Extended` has a small route lattice rather than one backup: `high-degree` first, then `deep-pocket`, then pocket-adjacent mixtures or low-degree routes depending on what remains, and finally a weaker degree-profile fallback once the whole cavity family is stripped out.
