...
The open problem in this basin is no longer whether the family structure exists, whether the frozen bucket exact-closes, why `mode-mix-f` sits inside the anchor band, or whether the outer `rect-wrap:local-morph-f` pair is the same mechanism as the frozen `add4` pocket. The zero-support collapse edge is already isolated, the later moderate anchor-balance band plus `mid_anchor_closure_peak <= 10.000` ceiling still reduces to a skew-wrap right/deep shoulder, a `mode-mix-f` low-support throat, and a frozen `add4` mid-anchor knot, and the `rect-wrap/rect-hard` `ultra/mega` guardrail now resolves into a heavier beyond-ceiling knot-side continuation under `closure_load >= 75.000`. The next question is whether that high-load continuation is a narrow `rect-wrap` tail or the first stable order parameter of a broader beyond-ceiling generated family.
...
- whether `closure_load >= 75.000` is a narrow `rect-wrap:local-morph-f` tail or the first stable order parameter of a broader beyond-ceiling generated continuation
