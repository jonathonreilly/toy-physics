...
- The next decomposition makes that crossing more explicit: for a fixed distorted path, the consistency margin is linear in `w`, with a calculable critical weight `w* = 2 * delay_penalty / retained_total`. The hardest observed cases sit just below `1.0`, and the hardest scenario also switches active rule/path branch near the crossing, so the benchmark is best understood as a piecewise-linear lower envelope rather than a single line in `w`.
