...
- A final post-fix synthesis then collapses those corrected mechanism tables into a simple family split. Across the direct corrected mechanism tables, there are no `compact-fast`-only rows at all: every fast compact mechanism is either also fast in `extended` or disappears. What does recur are `extended-fast` proxy rows, especially deep-pocket and pocket-adjacent winners that stay fast after the compact-side high-degree mechanism fails.
- The stronger shared anchors are now clear too. `full-rich` stays `shared-fast` everywhere, and `ge_7` remains the cleanest thresholded shared row on both the default corrected benchmark and the broader corrected rerun. `ge_6`, `share6`, and the threshold-exposure bundle also stay `shared-fast`, but they often do so by letting `extended` switch to a proxy motif rather than preserving the same feature winner.
- So the corrected family-level claim is now: `compact` still carries the cleaner near-hub threshold story, but `extended` is not a weaker copy of that mechanism. It is more permissive, and it frequently preserves fast parity by rerouting through deep-pocket or pocket-adjacent proxies once the compact-side high-degree coordinate breaks.
- A follow-up route ablation then tests that proxy story directly on the corrected generated-geometry benchmark. Removing `deep-pocket` alone or the whole pocket family alone does nothing at all: both families still sit on the same fast high-degree winner. The escape only starts once `motif_high_degree_neighbor_fraction` is removed.
- Once the high-degree route is gone, `extended` first reroutes to `motif_deep_pocket_adjacent_fraction`. If `deep-pocket` is removed as well, it falls back to a pocket-adjacent mixed subset. If the whole pocket family is removed together with the high-degree feature, `extended` still keeps fast parity through `motif_low_degree_neighbor_fraction`, while `compact` loses fast parity entirely.
- A final fallback ablation then shows that this still is not the end of the `extended` route tree. When the whole cavity family and the low-degree route are all removed, `extended` still survives through a non-cavity sparse-structure family built from degree-basis, neighbor-moment, and two-hop-open summaries. The first surviving row is `degree_8_fraction + motif_mean_neighbor_degree + motif_two_hop_open_fraction`; removing `degree_8` alone only weakens the route to parity size `4` through `degree_1_fraction`, and removing `motif_mean_neighbor_degree` still leaves a fast `degree_8 + two-hop-open` route. Even removing the `degree_8 + mean-neighbor` pair together still leaves a fast `two-hop-open + two-hop-occupied + degree_1` route.
- A direct default-vs-broader ensemble probe then tightens that result further. Widening the generated graph family from `5/3` to `7/4` geometry/procedural variants does not open this sparse-structure route up to `compact` at all: across all six fallback route sets, `compact` stays at `0/6` parity hits in both ensembles, while `extended` stays at `6/6`, with the same degree-profile family in every case and `4/6` fast (`size 3`) rows in both ensembles.
- A residual trace by basis size then shows that this `compact` miss is usually a near miss, not an arbitrary collapse. On most sparse-fallback route sets, the best `compact` row stabilizes around `-0.03/-0.01` mean/worst gap on the default ensemble and `-0.04/-0.07` on the broader one, usually through a degree-profile feature like `degree_5_fraction`. Only when the full degree basis is removed does the miss widen materially, to about `-0.12/-0.13` on the default ensemble and `-0.10/-0.10` on the broader one. So the family split is not a giant cliff, but it is also not closed by widening the graph family alone.
- A final bridge test makes that sharper. On the canonical sparse route, adding back `motif_high_degree_neighbor_fraction` restores `compact` immediately at parity size `3` in both ensembles. Adding back `motif_pocket_adjacent_fraction` or the whole pocket family only restores `compact` weakly at size `6`, while `motif_deep_pocket_adjacent_fraction` and `motif_low_degree_neighbor_fraction` do not restore `compact` at all. So the `compact` side is near the sparse route, but it still depends specifically on the original high-degree coordinate to close the last gap cleanly.
...
- Even immediate threshold-family proxies fail in that bridge role. Adding back `ge_6`, `ge_7`, `share6`, `count6`, or the full threshold bundle leaves the `compact` sparse-route gap unchanged in both ensembles, while the original `motif_high_degree_neighbor_fraction` still restores parity instantly. So, in the current corrected bridge test, the compact-side dependence is not just on a loose near-hub threshold family; it is on that exact binary high-degree-neighbor coordinate.
- So the sharpened corrected claim is: `deep-pocket` is the main `extended` escape route, but not the only one. `Extended` has a route lattice rather than one backup: `high-degree` first, then `deep-pocket`, then pocket-adjacent mixtures or low-degree routes depending on what remains, and finally a weaker but still real non-cavity sparse-structure family once the whole cavity family is stripped out. On the current corrected broader ensemble, that last sparse-structure family still appears genuinely `extended`-specific rather than merely suppressed on the `compact` side.
