...
The corrected-propagator decoherence story is now narrower but more honest. Fixed-DAG field distortions still leave the tested visibility curves unchanged, and the strongest clean suppression on irregular generated DAGs still comes from injected phase noise. But the latest measurement repairs also show that the endogenous node-environment register is not a universal null: once the shared helper keeps the actual last mass node as the environment label and the between-slit mass nodes are left traversable, weak geometry-sensitive decoherence survives on generated DAGs. In the current repaired family, the downstream two-register geometry reaches `D = 4/12` with `ALL = 1/12`, while the slit-adjacent and between-slit geometries each reach `D = 1/12`. So the retained read is now: the non-unitary lane is still weak and not unified into one law, but simple node-level environment tagging is a surviving branch rather than a closed impossibility theorem.
