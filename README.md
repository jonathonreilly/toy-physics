...
| `deep-pocket` | `atomic-standalone` | `family-specific` | `motif_deep_pocket_adjacent_fraction` | `extended_proxy_route_benchmark(no-high-degree)` |
| `pocket` | `atomic-standalone` | `family-specific` | `motif_pocket_adjacent_fraction` | `extended_proxy_route_benchmark`, `extended_atomic_route_overlap_benchmark` |
| `low-degree` | `atomic-standalone` | `family-specific` | `motif_low_degree_neighbor_fraction` | `extended_proxy_route_benchmark(no-high-no-pocket-family)` |
...
- The family-level claim after the benchmark-path fixes is therefore narrower and stronger: `compact` still carries the cleaner near-hub threshold story, while `extended` keeps a more permissive hierarchy of atomic backups, coexistence-only mixed rows, and a final sparse-structure residue.
