...
metadata, `numba_gauge_seed_v1`, fixed seeds `2026056061`/`2026056062`, and
non-readout source-Higgs/taste-radial metadata. Strict scalar-LSZ still fails
on raw `C_ss` (`z=193.5686242048355`); Schur-complement repair keeps `C_x|s`
as a useful finite candidate but only as a necessary one-volume check
(`z=-651.1959236955531`). The cleanest closure target remains certified
canonical `O_H` plus `C_ss/C_sH/C_HH` pole rows, with genuine same-source W/Z
rows as first fallback. Chunk063 is active run-control only until completed,
...
checkpointing and preserve selected-mass-only FH/LSZ, normal-equation cache
metadata, `numba_gauge_seed_v1`, fixed seeds `2026056059`/`2026056060`, and
non-readout source-Higgs/taste-radial metadata. Strict scalar-LSZ still fails
on raw `C_ss` (`z=195.44413991399455`); Schur-complement repair keeps `C_x|s`
as a useful finite candidate but only as a necessary one-volume check
(`z=-631.4581297917338`). The cleanest closure target remains certified
canonical `O_H` plus `C_ss/C_sH/C_HH` pole rows, with genuine same-source W/Z
rows as first fallback. Chunks061-062 are active run-control only until
completed, checkpointed, and packaged; chunk063 remains pending.
...
metadata, `numba_gauge_seed_v1`, fixed seeds `2026056053`/`2026056054`, and
non-readout source-Higgs/taste-radial metadata.  Strict scalar-LSZ still fails
on raw `C_ss` (`z=181.81573887267618`); Schur-complement repair keeps `C_x|s` as a
useful finite candidate but only as a necessary one-volume check
(`z=-585.9748296932755`).  The cleanest closure target remains certified
canonical `O_H` plus `C_ss/C_sH/C_HH` pole rows, with genuine same-source W/Z
rows as first fallback.  Chunks055-056 are active run-control only until
completed, checkpointed, and packaged; chunks057-063 remain pending.
...
normal-equation cache metadata, `numba_gauge_seed_v1`, fixed seeds, and
non-readout source-Higgs metadata.  Strict scalar-LSZ still fails on raw
`C_ss` (`z=170.33620497910093`); Schur-complement repair keeps `C_x|s` as a
useful finite candidate but only as a necessary one-volume check
(`z=-513.0902824488198`).  The `C_x|s` one-pole interpolation remains
model-class support only because positive two-pole endpoint counterfamilies
match the same two points.  The cleanest closure target remains certified
canonical `O_H` plus `C_ss/C_spH/C_HH` pole rows, with genuine same-source
W/Z rows as first fallback.  Successor chunks047-048 are active run-control
...
normal-equation cache metadata, `numba_gauge_seed_v1`, fixed seeds, and
non-readout source-Higgs metadata.  Strict scalar-LSZ still fails on raw
`C_ss` (`z=161.93089030677183`); Schur-complement repair keeps `C_x|s` as a
useful finite candidate but only as a necessary one-volume check
(`z=-484.2781158031889`).  The `C_x|s` one-pole interpolation remains
model-class support only because positive two-pole endpoint counterfamilies
match the same two points.  The cleanest closure target remains certified
canonical `O_H` plus `C_ss/C_spH/C_HH` pole rows, with genuine same-source
W/Z rows as first fallback.  Successor chunks043-044 are active run-control
only until completed, checkpointed, and packaged.
...
completed-mode checkpoints and preserve selected-mass-only FH/LSZ,
`numba_gauge_seed_v1`, fixed seeds, and non-readout source-Higgs metadata.
Strict scalar-LSZ still fails on raw `C_ss` (`z=127.31127155194513`);
Schur-complement repair keeps `C_x|s` as a useful finite candidate but only as
a necessary one-volume check (`z=-459.08170655875074`).  The route selector
still chooses certified canonical `O_H` plus `C_ss/C_spH/C_HH` pole rows as
the cleanest closure target, with genuine same-source W/Z rows as first
fallback.  Successor chunks031-032 are active run-control/log state only until
completed, checkpointed, and packaged.

2026-05-07 chunks027-028 package refresh: chunks001-028 are now packaged
partial `C_sx/C_xx` support with `ready=28/63`; chunks027-028 pass
completed-mode checkpoints and preserve selected-mass-only FH/LSZ,
`numba_gauge_seed_v1`, fixed seeds, and non-readout source-Higgs metadata.
Strict scalar-LSZ still fails on raw `C_ss` (`z=122.86868233770126`);
Schur-complement repair keeps `C_x|s` as a useful finite candidate but only as
a necessary one-volume check (`z=-438.55215485628264`).  The route selector
still chooses certified canonical `O_H` plus `C_ss/C_spH/C_HH` pole rows as
the cleanest closure target, with genuine same-source W/Z rows as first
fallback.  Successor chunks029-030 are run-control/log/empty-directory state
only until completed, checkpointed, and packaged.
...
polefit8x8 manifest chunks remain to launch. Rerun diagnostics still block
the scalar-LSZ shortcut: current `C_ss` fails Stieltjes monotonicity
(`min_z=110.989`), finite-row contact subtraction remains non-identifying
(`spread_z_at_max_q=3909.247`), affine contact cannot repair higher
complete-monotonicity signs, and arbitrary polynomial contact interpolation
remains non-identifying. The global guard now reports zero active FH/LSZ
workers. The closure ranking is unchanged but now non-chunk authority is the
highest-value work: certified `O_H/C_sH/C_HH` pole rows, same-source W/Z
response rows with identity and `g2` certificates, genuine Schur `A/B/C`
rows, a rank-one neutral-scalar theorem, same-surface scalar
contact/denominator authority, or higher-volume honest production evidence.

2026-05-05 update: polefit8x8 chunks061-063 are now active under the global
collision guard. The launch record reports fixed seeds `2026051961`-
`2026051963`, selected mass `0.75`, x8 scalar-two-point noise, eight modes,
isolated output directories, running chunks `[61, 62, 63]`, `missing=0`, and
`all_jobs=3`. A post-launch guard records three active workers and no further
polefit8x8 manifest work remains to launch. This is run-control support only;
do not count chunks061-063 until root artifacts pass the polefit8x8
combiner/postprocessor and aggregate gates. The closure ranking is unchanged:
...
theorem, same-surface scalar contact/denominator authority, or continued
honest production evidence.

2026-05-05 update: polefit8x8 chunks055-060 are now packaged and pass the
homogeneous eight-mode/x8 combiner. The separate polefit8x8 stream has
`60/63` ready chunks and `960/1008` saved configurations. Rerun diagnostics
...
now reports zero active FH/LSZ workers and allows another launch. The closure
ranking is unchanged: certified `O_H/C_sH/C_HH` pole rows, same-source W/Z
...
2026-05-05 update: polefit8x8 chunks055-060 are now active under the global
collision guard. The launch record reports fixed seeds `2026051955`-
`2026051960`, selected mass `0.75`, x8 scalar-two-point noise, eight modes,
isolated output directories, running chunks `[55, 56, 57, 58, 59, 60]`,
`missing=0`, and `all_jobs=6`. A post-launch guard records six active workers
at the global cap and blocks further launches. This is run-control support
only; do not count chunks055-060 until root artifacts pass the polefit8x8
combiner/postprocessor and aggregate gates. The closure ranking is unchanged:
certified `O_H/C_sH/C_HH` pole rows, same-source W/Z response rows with
identity certificates, genuine Schur `A/B/C` rows, a rank-one neutral-scalar
theorem, same-surface scalar contact/denominator authority, or continued
...
2026-05-05 update: polefit8x8 chunks049-054 are now packaged and pass the
homogeneous eight-mode/x8 combiner.  The separate polefit8x8 stream has
`54/63` ready chunks and `864/1008` saved configurations.  Rerun diagnostics
still block the scalar-LSZ shortcut: current `C_ss` fails Stieltjes
monotonicity, finite-row contact subtraction remains non-identifying, affine
contact cannot repair higher complete-monotonicity signs, and arbitrary
polynomial contact interpolation remains non-identifying.  The global guard
...
isolated output directories, running chunks `[43, 44, 45, 46, 47, 48]`,
`missing=0`, and `all_jobs=6`.  A post-launch guard records six active
workers at the global cap and blocks further launches.  This is run-control
support only; do not count chunks043-048 until root artifacts pass the
polefit8x8 combiner/postprocessor and aggregate gates.  The closure ranking is
unchanged: certified `O_H/C_sH/C_HH` pole rows, same-source W/Z response rows
with identity certificates, genuine Schur `A/B/C` rows, a rank-one
neutral-scalar theorem, same-surface scalar contact/denominator authority, or
...
2026-05-05 update: polefit8x8 chunks037-042 are now packaged and pass the
homogeneous eight-mode/x8 combiner.  The separate polefit8x8 stream has
`42/63` ready chunks and `672/1008` saved configurations.  Rerun diagnostics
still block the scalar-LSZ shortcut: current `C_ss` fails Stieltjes
monotonicity, finite-row contact subtraction remains non-identifying, affine
contact cannot repair higher complete-monotonicity signs, and arbitrary
polynomial contact interpolation remains non-identifying.  The global guard
...
2026-05-05 update: polefit8x8 chunks037-042 are now active under the global
collision guard.  The launch record reports fixed seeds `2026051937`-
`2026051942`, selected mass `0.75`, x8 scalar-two-point noise, eight modes,
isolated output directories, running chunks `[37, 38, 39, 40, 41, 42]`,
`missing=0`, and `all_jobs=6`.  A post-launch guard records six active
workers at the global cap and blocks further launches.  This is run-control
support only; do not count chunks037-042 until root artifacts pass the
polefit8x8 combiner/postprocessor and aggregate gates.  The closure ranking is
unchanged: certified `O_H/C_sH/C_HH` pole rows, same-source W/Z response rows
with identity certificates, genuine Schur `A/B/C` rows, a rank-one
neutral-scalar theorem, same-surface scalar contact/denominator authority, or
...
isolated output directories, running chunks `[31, 32, 33, 34, 35, 36]`,
`missing=0`, and `all_jobs=6`.  A post-launch guard records six active
workers at the global cap and blocks further launches.  This is run-control
support only; do not count chunks031-036 until root artifacts pass the
polefit8x8 combiner/postprocessor and aggregate gates.  The closure ranking is
unchanged: certified `O_H/C_sH/C_HH` pole rows, same-source W/Z response rows
with identity certificates, genuine Schur `A/B/C` rows, a rank-one
neutral-scalar theorem, same-surface scalar contact/denominator authority, or
...
forms a finite-shell diagnostic but still blocks retained use on model-class,
FV/IR, and canonical-Higgs/source-overlap authority.  Next production action:
run the global guard and launch chunks019-024 only if capacity is available.
The closure ranking is unchanged: certified `O_H/C_sH/C_HH` pole rows,
same-source W/Z response rows with identity certificates, genuine Schur
`A/B/C` rows, a rank-one neutral-scalar theorem, or honest production
evidence.
...
passes with limiting ESS `296.09790071733823`; target autocorrelation ESS
passes over the current ready set; replacement queue is empty for the current
ready set.  This still does not change the closure ranking.  Chunk022 is a
response-slope outlier, so response stability remains open
(`relative_stdev=0.9050778118183592`, `spread_ratio=5.920283844112204`), and
response-window acceptance, canonical-Higgs/source-overlap, W/Z-response,
scalar-pole/FV/IR/model-class, finite-source-linearity, and multi-radius gates
remain open.  New chunks are useful background support, but the highest-value
foreground move remains a genuine same-surface `O_H/C_sH/C_HH` identity,
same-source W/Z response with
...
2026-05-02 update: seed-controlled FH/LSZ chunks001-010 are ready
(`10/63` of L12, `160/1000` saved configurations).  This makes further chunk
collection a concrete production-support
...
(`n=10`, `relative_stdev=0.9079`, `spread_ratio=5.4765`), so finite `dE/ds`
slopes from `10/63` chunks must remain scout/support only.
...
3. **Full scalar pole-residue/common-dressing theorem.** The current analytic
   ...
   leaves a source/field rescaling freedom, and the free logdet bubble has no
   ...
   scalar denominator proxies.  The flat-toron thermodynamic washout theorem
   narrows this: fixed-holonomy flat-sector dependence disappears for the
   local massive bubble in the thermodynamic limit, but this does not derive
...
4. **Feynman-Hellmann scalar-response route.** Energy slopes with respect to a
  ...
  until its replacement finishes; the chunk002 checkpoint runner now handles
  both that diagnostic state and the future seed-controlled replacement state.
