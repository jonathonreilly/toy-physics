...
| Fresh-artifact intake at PR head | committed-head reopen guard for current closure packet | open checkpoint; no closure artifact present | checks current committed head only; active chunk outputs/logs are excluded; no retained/proposed-retained wording unless a same-surface canonical `O_H`/source-Higgs pole-row packet or strict W/Z accepted-action physical-response packet exists |
...
| Two-source taste-radial chunks001-046 package | finite `C_ss/C_sx/C_xx` row packet | bounded support only | chunks045-046 are packaged; ready=46/63, combined_rows_written=false; active chunks047-048 are run-control only; finite rows remain non-closure until canonical `O_H`, strict source-Higgs pole rows, scalar-LSZ/FV/IR, W/Z, Schur pole, or neutral primitive authority passes |
