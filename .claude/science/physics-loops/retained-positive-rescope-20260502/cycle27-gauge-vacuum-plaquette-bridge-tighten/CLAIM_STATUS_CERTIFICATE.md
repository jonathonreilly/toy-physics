...
**Target row:** gauge_vacuum_plaquette_bridge_support_note (claim_type=positive_theorem, load_bearing_step_class=C)
...
1. Replace the very long author-side `Status:` paragraph (which began
   ...
   `Status authority` headers that leave status decisions to the
...
2. Add a new "Out of scope (admitted-context to this note)" section
   ...
   - the physical 3D Wilson environment boundary character measure
     `Z_6^env(W)` (a principal load-bearing gap outside this note);
   - the full interacting plaquette reduction at `beta = 6`;
   - the reference Perron solves at `rho = 1` and
     `rho = delta_{(p,q),(0,0)}` (reference readouts under explicit
     structural hypotheses, not load-bearing physical claims);
...
- **Out-of-scope content:** the explicit `Z_6^env` derivation, the
  ...
  at structural `rho` hypotheses (reference readouts only), and
...
| 3 | No new load-bearing observed/fitted/admitted introduced | YES (no new imports; pre-existing reference Perron solve values 0.4524..., 0.4225... relabeled as reference readouts under explicit structural rho hypotheses, not load-bearing physical claims) |
| 4 | Algebraic content preserved | YES (primary runner EXACT PASS=6, SUPPORT=2 unchanged; bridge-support stack content preserved) |
| 5 | Tightening separates out-of-scope gaps | YES (`Z_6^env` derivation and full interacting reduction at `beta=6` are named outside the bridge-support stack) |
