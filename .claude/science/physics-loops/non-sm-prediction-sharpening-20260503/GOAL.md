...
(PRD/JHEP) but does not yet isolate a bounded beyond-SM discrimination
surface because the framework mostly reproduces SM observables.

This campaign pivots: **sharpen the framework's existing distinguishing
surfaces into bounded, testable discrimination claims vs the SM**.

## Three targets (all-three depth)

1. **Higgs mass + vacuum stability** (block 01)
   - Framework package conditionally carries an m_H range (~125-130 GeV),
     vacuum-stability surface, and y_t(v) = 0.918 internal value
...
2. **Lorentz violation cubic harmonic K_4** (block 02)
   - Framework LV surface conditionally predicts a specific K_4 angular
     signature with c_44/c_40 = √(5/14), quadratic E/E_Planck suppression
...
3. **Strong CP θ=0 + CKM-only neutron EDM** (block 03)
   - Framework strong-CP surface conditionally carries θ_eff = 0 by structure; d_n(QCD) = 0;
     d_n(CKM) ~ 10⁻³² e·cm
...
Unified beyond-SM discrimination package combining all three bounded
distinguishing surfaces into a coherent falsification framework.
