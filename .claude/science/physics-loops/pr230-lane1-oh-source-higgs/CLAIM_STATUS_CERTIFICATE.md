...
proposal_allowed_reason: current surface has complete finite C_ss/C_sx/C_xx support, top bare-mass response support, W/Z response contracts, and HS/logdet support-only rewrites, but no dynamic Phi/action, canonical O_H identity, neutral primitive transfer, off-diagonal generator, scalar LSZ metric, strict source-Higgs pole rows, strict W/Z production packet, or absolute g2/v authority
...
frontier_yt_pr230_full_positive_closure_assembly_gate.py: PASS=167 FAIL=0
frontier_yt_pr230_campaign_status_certificate.py: PASS=370 FAIL=0
```
