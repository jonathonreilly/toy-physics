...
SUPPORT-ROUTE / COMPARISON-FAMILY — new theorem + verification runner.
This note isolates the literal bivector recipe behind the retained
`n = 3` native-SU(2) construction and packages the arbitrary-`n` scope
extension as the strongest current support-route premise for the
companion CL3 notes. It is suitable to keep on `main` as honest
support-route science, not as a retained closure by itself.
...
**Support-route theorem (new).**

- `Theorem (Native-Gauge Scope).` The retained native-SU(2) construction
  has the literal recipe `(R)` at `n = 3`, and that recipe admits a
  selector-free comparison-family extension.

**Support-route corollary (companion tightness theorem).**
...
- The strongest current CL3 route toward `d_s = 3`, with no cubic
  `Z^3` orbit structure, no hw-orbit semantics, and no SM matter
  content used in the derivation. The remaining status question is
  whether the family-scope premises can themselves be promoted from
  support-route / comparison-family inputs to retained framework rules.
...
This is a strong support-route claim, not yet a retained closure claim,
because:
...
The remaining gap is narrower than before but still real: this note
shows what the retained `n = 3` code is literally doing, and it shows
how that recipe extends cleanly across `n`; it does **not** by itself
settle that the comparison-family extension is already a retained
framework rule rather than the strongest current support-route reading.

## Next

With both this note and the companion tightness theorem on `main` as
support-route science, the immediate follow-ups are:

1. Update the companion `native-su2-tightness-forces-ds3-2026-04-17.md`
   note to point to this note as the scope premise it actually uses.
2. Decide whether the family-scope reading should be promoted from
   support-route / comparison-family status to a retained framework rule.
3. If that status gap is ever closed, reassess whether `d_s = 3` should
   move from axiom-level choice to a derived theorem on `main`.

If a reviewer still objects even at support-route strength, the live
...
- (b) object that the chiral-matrix / staggered-phase construction
  does not genuinely extend to `n ≠ 3`.

Those are narrower and more actionable questions than the old
"interpretation vs theorem" dispute, which is why this note is worth
keeping even before full retained closure is settled.
