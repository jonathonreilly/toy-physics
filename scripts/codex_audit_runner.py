...
def load_reaudit_candidates(criticality_filter: str | None = None,
    ...
    uniformly. The `ready` flag is set to True since by definition these
    candidates have all deps retained-grade.
    """
...
def main() -> int:
    ...
    p.add_argument("--from-reaudit-candidates", action="store_true",
                   help="Pull rows from docs/audit/data/reaudit_candidates.json "
                        ...
                        "Implies --allow-blocked=False since these rows already "
                        "satisfy the dep-readiness condition.")
