#!/usr/bin/env python3
"""Overnight batch 3: combined emergence + mass scaling on pruned graphs.

A. Modular start + pruning at N=80-100: does imposed gap + pruning
   push past the N=80 wall?
B. Mass scaling alpha on pruned graphs: does pruning change the exponent?
C. Dense 3D graphs (npl=60): does the N=100 wall move with density?

PStack experiment: overnight-batch-3
"""
...
def measure_alpha(gen_fn, use_prune=False, n_seeds=16, **gen_kwargs):
    mass_counts = [1, 2, 4, 6, 8, 12, 16]
    results = []
    for target_n in mass_counts:
        per_seed = []
        for seed in range(n_seeds):
            positions, adj, layers = gen_fn(n_layers=15, rng_seed=seed*17+3, **gen_kwargs)
            if use_prune:
        ...
        if per_seed:
            actual_n = per_seed[0][0]; vals = [v for _, v in per_seed]
...
def main():
    n_seeds = 16
    print("=" * 74)
    print("OVERNIGHT BATCH 3")
    print("=" * 74)
    print()

    # A: Modular start + pruning at N=80-100
    print("EXPERIMENT A: Modular gap=3 + pruning (push past N=80)")
    print()
    for label, use_prune in [("Modular gap=3 only", False),
                              ("Modular gap=3 + prune q=0.10", True)]:
        print(f"  [{label}]")
        ...
        for nl in [40, 60, 80, 100]:
            purs = []
            for seed in range(n_seeds):
                positions, adj, layers = gen_3d(n_layers=nl, rng_seed=seed*13+5, gap=3.0)
                if use_prune:
                    adj, _ = adaptive_prune(positions, adj, layers, quantile=0.10, max_iter=3)
    ...
    # B: Mass scaling on pruned vs unpruned
    print("EXPERIMENT B: Mass scaling alpha on pruned graphs")
    print(f"  {'config':>30s}  {'alpha':>7s}")
    print(f"  {'-'*40}")
    for label, use_prune, kwargs in [
        ("Uniform (no prune)", False, {}),
        ("Uniform + prune q=0.10", True, {}),
        ("Modular gap=3 (no prune)", False, {"gap": 3.0}),
        ("Modular gap=3 + prune", True, {"gap": 3.0}),
        ("Modular gap=5 (no prune)", False, {"gap": 5.0}),
        ("Modular gap=5 + prune", True, {"gap": 5.0}),
    ]:
        alpha = measure_alpha(gen_3d, use_prune=use_prune, n_seeds=16, **kwargs)
        print(f"  {label:>30s}  {alpha:7.3f}" if alpha else f"  {label:>30s}  FAIL")
    print()

    # C: Dense graphs (npl=60)
    print("EXPERIMENT C: Dense 3D uniform (npl=60, r=2.7)")
    print()
    for label, use_prune in [("Dense baseline", False), ("Dense + prune", True)]:
        print(f"  [{label}]")
        print(f"  {'N':>4s}  {'pur_cl':>8s}  {'n':>3s}")
        print(f"  {'-'*18}")
        for nl in [30, 40, 60, 80, 100]:
            purs = []
            for seed in range(n_seeds):
    ...
    print("=" * 74)
    print("OVERNIGHT BATCH 3 COMPLETE")
    print("=" * 74)
