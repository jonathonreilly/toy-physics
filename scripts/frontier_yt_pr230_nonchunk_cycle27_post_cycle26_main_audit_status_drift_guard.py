#!/usr/bin/env python3
"""
PR #230 non-chunk cycle-27 post-cycle-26 main-audit-status-drift reopen guard.

Cycle 26 closed origin/main drift through 3189b67ea.  This guard checks the
only new resume fact available at cycle 27: after a fresh fetch, origin/main
advanced again to d04a2f2a1, but the new diff still contains only
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_nonchunk_cycle27_post_cycle26_main_audit_status_drift_guard_2026-05-05.json"
)
LOOP_PACK = (
    ROOT
    / ".claude"
...
CYCLE26_HEAD = "5855a54b779aacf57b04142ba142a885ac41f295"
CYCLE26_REMOTE_MAIN_HEAD = "3189b67ea27e8b8670bd682cde145ead7fd1bae8"
...
def branch_state(future_paths: list[str]) -> dict[str, Any]:
    ...
    main_changed_paths = diff_paths(CYCLE26_REMOTE_MAIN_HEAD, REMOTE_MAIN)
    main_changed_path_classes = Counter(path_class(rel) for rel in main_changed_paths)
    return {
        "cycle26_head": CYCLE26_HEAD,
        "cycle26_remote_main_head": CYCLE26_REMOTE_MAIN_HEAD,
        "local_head": local_head,
        "remote_pr_ref": REMOTE_PR,
        "remote_pr_head": remote_pr_head,
        "remote_main_ref": REMOTE_MAIN,
        "remote_main_head": remote_main_head,
        "cycle26_is_ancestor_of_local_head": git_ok(
            ["merge-base", "--is-ancestor", CYCLE26_HEAD, "HEAD"]
        ),
        "cycle26_is_ancestor_of_remote_pr": git_ok(
            ["merge-base", "--is-ancestor", CYCLE26_HEAD, REMOTE_PR]
        ),
        "cycle26_remote_main_is_ancestor_of_remote_main": git_ok(
            ["merge-base", "--is-ancestor", CYCLE26_REMOTE_MAIN_HEAD, REMOTE_MAIN]
        ),
        "local_head_matches_remote_pr_head": bool(local_head and local_head == remote_pr_head),
        "local_commits_after_cycle26": git(["rev-list", "--count", f"{CYCLE26_HEAD}..HEAD"]),
        "remote_commits_after_cycle26": git(["rev-list", "--count", f"{CYCLE26_HEAD}..{REMOTE_PR}"]),
        "remote_main_commits_after_cycle26_baseline": git(
            ["rev-list", "--count", f"{CYCLE26_REMOTE_MAIN_HEAD}..{REMOTE_MAIN}"]
        ),
        "remote_main_changed_path_count_after_cycle26_baseline": len(main_changed_paths),
        "remote_main_changed_path_class_counts_after_cycle26_baseline": dict(
        ...
        "local_changed_future_paths_after_cycle26": diff_future_paths(
            CYCLE26_HEAD, "HEAD", future_paths
        ),
        "remote_pr_changed_future_paths_after_cycle26": diff_future_paths(
            CYCLE26_HEAD, REMOTE_PR, future_paths
        ),
        "remote_main_changed_future_paths_after_cycle26_baseline": diff_future_paths(
            CYCLE26_REMOTE_MAIN_HEAD, REMOTE_MAIN, future_paths
        ),
    }
...
def main() -> int:
    print("PR #230 non-chunk cycle-27 post-cycle-26 main-audit-status-drift reopen guard")
    print("=" * 72)
    ...
    no_changed_future_path_since_cycle26 = not any(
        [
            branch["local_changed_future_paths_after_cycle26"],
            branch["remote_pr_changed_future_paths_after_cycle26"],
            branch["remote_main_changed_future_paths_after_cycle26_baseline"],
        ]
    )
    ...
    branch_current = (
        branch["cycle26_is_ancestor_of_local_head"]
        and branch["cycle26_is_ancestor_of_remote_pr"]
        and branch["cycle26_remote_main_is_ancestor_of_remote_main"]
        and branch["local_head_matches_remote_pr_head"]
        and bool(branch["remote_main_head"])
    )
    loop_pack_records_stop = (
        "cycle-26 update" in compact_text(OPPORTUNITY_QUEUE)
        and "post-cycle-25 main-audit-status-drift reopen guard" in compact_text(HANDOFF)
        and "post-cycle-25 main audit/status drift is not admissible reopen evidence"
        in compact_text(NO_GO_LEDGER)
        and "proposal_allowed: false" in compact_text(CLAIM_STATUS_CERTIFICATE)
    )
    main_drift_since_cycle26_is_status_only = (
        branch["remote_main_commits_after_cycle26_baseline"] != "0"
        and branch["remote_main_changed_paths_are_status_drift"]
        and not branch["remote_main_changed_future_paths_after_cycle26_baseline"]
    )
    no_reopen_input = (
        source_presence_absent
        and no_local_or_remote_future_path
        and no_changed_future_path_since_cycle26
        and main_drift_since_cycle26_is_status_only
    )
    cycle27_post_cycle26_main_audit_status_drift_guard_passed = all(
        [
            not missing_parents,
            not parent_failures,
    ...
    dramatic_step_gate = {
        ...
        "branch_state": branch,
        "reason": (
            "Cycle 26 already rejects post-cycle-25 origin/main audit/status "
            ...
            "of the listed PR230 same-surface artifacts, so cycle 27 has no "
    ...
    report("branch-has-cycle26-ancestor-and-remote-main-visible", branch_current, json.dumps(branch, sort_keys=True))
    ...
    report("parent-process-stack-closed-through-cycle26", parent_process_stack_closed, "cycle-18 through cycle-26 gates deny continuation")
    report("aggregate-gates-deny-proposal", aggregate_gates_deny, f"assembly={statuses['assembly']} campaign={statuses['campaign']}")
    report("loop-pack-records-cycle26-stop-contract", loop_pack_records_stop, "cycle-26 stop text present")
    report("main-drift-since-cycle26-is-status-only", main_drift_since_cycle26_is_status_only, json.dumps(branch, sort_keys=True))
    report("no-local-or-remote-listed-future-path", no_local_or_remote_future_path, json.dumps(branch, sort_keys=True))
    report("no-changed-listed-future-path-since-cycle26", no_changed_future_path_since_cycle26, json.dumps(branch, sort_keys=True))
    report("no-source-presence-key-set", source_presence_absent, json.dumps(present_by_source, sort_keys=True))
    report(
        "cycle27-post-cycle26-main-audit-status-drift-guard-passed",
        cycle27_post_cycle26_main_audit_status_drift_guard_passed,
        f"passed={cycle27_post_cycle26_main_audit_status_drift_guard_passed}",
    )

    result = {
        "actual_current_surface_status": (
            "exact negative boundary / PR230 non-chunk cycle-27 "
            "post-cycle-26 main-audit-status-drift reopen guard: origin/main "
        ...
        "proposal_allowed_reason": (
            "Cycle 26 already rejects post-cycle-25 origin/main audit/status "
            "drift. The cycle-26 head remains an ancestor of the aligned PR "
        ...
        "exact_next_action": (
            ...
            "cycle-23, cycle-24, cycle-25, cycle-26, cycle-27, assembly, "
