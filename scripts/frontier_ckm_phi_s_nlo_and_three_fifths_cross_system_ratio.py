#!/usr/bin/env python3
"""φ_s NLO closed form + 3/5 cross-system CP ratio theorem audit.
...
Verifies the new identities in
  ...
  (R4) sin(2 alpha_bar) / phi_s = 3/5 EXACTLY at leading order in alpha_s       [NEW α_s-INDEPENDENT]
...
def audit_r4_three_fifths_ratio() -> None:
    ...
    # The α_s factor cancels exactly
...
def audit_r5_joint_identity() -> None:
    ...
    # ⟹ sin(2 alpha_bar)/sin(2 beta_d) = (3/5)(phi_s/sin(2 beta_d)) = (3/5)(-alpha_s/2) = -3 alpha_s/10

    via_combination = (3/5) * (-ALPHA_S_V / 2)
    print(f"  Via (R4) × Thales: (3/5)(-lambda²)   = {via_combination:.10f}")
    check("(R5) consistent with (R4) × Thales cross-system",
...
def audit_pdg_comparators() -> None:
    ...
    check("3/5 cross-system consistency: framework vs LHCb-projection",
