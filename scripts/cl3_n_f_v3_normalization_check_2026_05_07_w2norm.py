...
W2.binary/W2.bridge bounded reductions), is the overall scalar
   N_F := Tr_{V_3}(T_a T_b) / delta_ab
pinned to 1/2 by Cl(3) + Z^3 primitives, or only conditional on
...
admission. The result is a bounded theorem candidate: under the bridge
admission, N_F = 1/2 is supported. Without it, N_F is determined up to
(and only up to) a positive scalar that the Cl(3) + Z^3 substrate cannot fix.
...
def section_C_Killing_propagation(T3):
    ...
    print("  bivector SU(2) (BRIDGE - admitted) supports Tr_{V_3}(T_a T_b) = 1/2.")
    print()
...
def section_F_v3_vs_v(T3):
    ...
    print("  is pinned to N_F = 1/2 by Cl(3) + Z^3 primitives.")
    print()
...
def section_I_wilson_consistency():
    section("SECTION I — Wilson plaquette consistency: N_F = 1/2 supports g_bare = 1")
...
def section_J_bridge_admission():
    ...
    print()
    print("  W2.norm CONDITIONAL SUPPORT PATH:")
    ...
    print("  on both SU(2)s' — the landed W2.bridge note reduces this")
    print("  normalization bridge to the L3a trace-surface admission.")
    print()
...
def section_K_summary():
    ...
    rows = [
        ("V1: Cl(3) bivector trace at per-site (Spin(3)/SU(2))",
         "DIRECT — N_F = 1/2 checked for per-site SU(2)", "Section A, E"),
        ...
        ("V4: Spin double-cover normalization (T = sigma/2)",
         "DIRECT — 1/2 checked at per-site C^2", "Section E"),
        ...
        ("Bridge admission (per-site SU(2) ↔ V_3 SU(2) sub-block)",
         "RESIDUAL ADMISSION — supports N_F = 1/2 if accepted", "Section J"),
    ]
    ...
    print()
    print("  VERDICT: BOUNDED THEOREM CANDIDATE (conditional on bridge admission).")
    print()
    print("  Three structural findings are directly checked:")
    ...
    print()
    print("  Conditional support for N_F = 1/2 on V_3 follows from (a)+(b)+(c) PLUS the")
    ...
    print("  residual admission for this note; the other displayed steps are")
    print("  direct algebraic checks under the named imports.")
