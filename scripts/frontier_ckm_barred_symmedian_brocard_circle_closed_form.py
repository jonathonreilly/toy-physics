...
Key NEW identities on the retained NLO Wolfenstein protected-gamma_bar surface:
  ...
  (K10) The polynomial P(alpha_s) is the SAME polynomial that appears in
        the Brocard inequality via cot(omega)=perim_sq/(4*Area):
        cot(omega_bar)^2 - 3 = P(alpha_s) / [80(4-alpha_s)^2].
...
def audit_k6_OK_squared(N: dict, S: dict) -> sp.Expr:
    banner("K6: OK^2 closed form -- via R_bar^2 (1 - 48 Area^2 / perim_sq^2)")
...
def audit_k10_p_polynomial_appearance(N: dict, S: dict, OK_sq: sp.Expr) -> None:
    ...
    print()
    print("  Significance:")
    print("    - Brocard inequality: cot(omega_bar)^2 - 3 = P(alpha_s) / [80(4 - alpha_s)^2].")
    ...
    cot_omega = sp.simplify(S["perim_sq"] / (4 * S["area"]))
    brocard_ineq_expr = sp.simplify(cot_omega ** 2 - 3)
    brocard_expected = sp.simplify(P_poly / (80 * (4 - a_s) ** 2))
    print(f"\n  P(0) = {P_lo}")
    check("K10: P(alpha_s) is the same polynomial in OK^2 and Brocard inequality",
          P_expanded == sp.expand(P_poly))
    check("K10: cot(omega_bar)^2 - 3 = P(alpha_s) / [80(4 - alpha_s)^2]",
          sp.simplify(brocard_ineq_expr - brocard_expected) == 0)
...
def audit_summary() -> None:
    ...
    print("            - Brocard inequality via cot(omega_bar)^2 - 3 = P / [80(4 - alpha_s)^2],")
