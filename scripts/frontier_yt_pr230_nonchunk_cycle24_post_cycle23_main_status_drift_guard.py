#!/usr/bin/env python3
"""
PR #230 non-chunk cycle-24 post-cycle-23 main-status-drift reopen guard.

Cycle 23 closed origin/main effective-status drift through 04decbdca.  This
guard checks the only new resume fact available at cycle 24: after a fresh
fetch, origin/main advanced again to eaa2130fc, but the new diff still contains
only audit/effective-status surfaces and no listed PR230 same-surface row,
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_nonchunk_cycle24_post_cycle23_main_status_drift_guard_2026-05-05.json"
)
LOOP_PACK = (
    ROOT
    / ".claude"
...
CYCLE23_HEAD = "b6433da894ad8ead06e8b9a3b30dadd7ee7fc4f4"
CYCLE23_REMOTE_MAIN_HEAD = "04decbdca3cf68fecd55afc366c47491945732f0"
...
def branch_state(future_paths: list[str]) -> dict[str, Any]:
    ...
    main_changed_paths = diff_paths(CYCLE23_REMOTE_MAIN_HEAD, REMOTE_MAIN)
    return {
        "cycle23_head": CYCLE23_HEAD,
        "cycle23_remote_main_head": CYCLE23_REMOTE_MAIN_HEAD,
        "local_head": local_head,
        "remote_pr_ref": REMOTE_PR,
        "remote_pr_head": remote_pr_head,
        "remote_main_ref": REMOTE_MAIN,
        "remote_main_head": remote_main_head,
        "cycle23_is_ancestor_of_local_head": git_ok(
            ["merge-base", "--is-ancestor", CYCLE23_HEAD, "HEAD"]
        ),
        "cycle23_is_ancestor_of_remote_pr": git_ok(
            ["merge-base", "--is-ancestor", CYCLE23_HEAD, REMOTE_PR]
        ),
        "cycle23_remote_main_is_ancestor_of_remote_main": git_ok(
            ["merge-base", "--is-ancestor", CYCLE23_REMOTE_MAIN_HEAD, REMOTE_MAIN]
        ),
        "local_head_matches_remote_pr_head": bool(local_head and local_head == remote_pr_head),
        "local_commits_after_cycle23": git(["rev-list", "--count", f"{CYCLE23_HEAD}..HEAD"]),
        "remote_commits_after_cycle23": git(["rev-list", "--count", f"{CYCLE23_HEAD}..{REMOTE_PR}"]),
        "remote_main_commits_after_cycle23_baseline": git(
            ["rev-list", "--count", f"{CYCLE23_REMOTE_MAIN_HEAD}..{REMOTE_MAIN}"]
        ),
        "remote_main_changed_paths_after_cycle23_baseline": main_changed_paths,
        "remote_main_changed_path_classes_after_cycle23_baseline": {
        ...
        "local_changed_future_paths_after_cycle23": diff_future_paths(
            CYCLE23_HEAD, "HEAD", future_paths
        ),
        "remote_pr_changed_future_paths_after_cycle23": diff_future_paths(
            CYCLE23_HEAD, REMOTE_PR, future_paths
        ),
        "remote_main_changed_future_paths_after_cycle23_baseline": diff_future_paths(
            CYCLE23_REMOTE_MAIN_HEAD, REMOTE_MAIN, future_paths
        ),
    }
...
def main() -> int:
    print("PR #230 non-chunk cycle-24 post-cycle-23 main-status-drift reopen guard")
    print("=" * 72)
    ...
    no_changed_future_path_since_cycle23 = not any(
        [
            branch["local_changed_future_paths_after_cycle23"],
            branch["remote_pr_changed_future_paths_after_cycle23"],
            branch["remote_main_changed_future_paths_after_cycle23_baseline"],
        ]
    )
    ...
    branch_current = (
        branch["cycle23_is_ancestor_of_local_head"]
        and branch["cycle23_is_ancestor_of_remote_pr"]
        and branch["cycle23_remote_main_is_ancestor_of_remote_main"]
        and branch["local_head_matches_remote_pr_head"]
        and bool(branch["remote_main_head"])
    )
    loop_pack_records_stop = (
        "cycle-23 update" in compact_text(OPPORTUNITY_QUEUE)
        and "main-effective-status-drift reopen guard" in compact_text(HANDOFF)
        and "repeated main effective-status drift is not admissible reopen evidence"
        in compact_text(NO_GO_LEDGER)
        and "proposal_allowed: false" in compact_text(CLAIM_STATUS_CERTIFICATE)
    )
    main_drift_since_cycle23_is_status_only = (
        branch["remote_main_commits_after_cycle23_baseline"] != "0"
        and branch["remote_main_changed_paths_are_status_drift"]
        and not branch["remote_main_changed_future_paths_after_cycle23_baseline"]
    )
    no_reopen_input = (
        source_presence_absent
        and no_local_or_remote_future_path
        and no_changed_future_path_since_cycle23
        and main_drift_since_cycle23_is_status_only
    )
    cycle24_post_cycle23_main_status_drift_guard_passed = all(
        [
            not missing_parents,
            not parent_failures,
    ...
    dramatic_step_gate = {
        ...
        "branch_state": branch,
        "reason": (
            "Cycle 23 already rejects repeated origin/main status drift as "
            "reopen evidence.  Since then origin/main advanced again only on "
            "audit/effective-status surfaces and still contains none of the "
            "listed PR230 same-surface artifacts, so cycle 24 has no admissible "
    ...
    report("branch-has-cycle23-ancestor-and-remote-main-visible", branch_current, json.dumps(branch, sort_keys=True))
    ...
    report("parent-process-stack-closed-through-cycle23", parent_process_stack_closed, "cycle-18 through cycle-23 gates deny continuation")
    report("aggregate-gates-deny-proposal", aggregate_gates_deny, f"assembly={statuses['assembly']} campaign={statuses['campaign']}")
    report("loop-pack-records-cycle23-stop-contract", loop_pack_records_stop, "cycle-23 stop text present")
    report("main-drift-since-cycle23-is-status-only", main_drift_since_cycle23_is_status_only, json.dumps(branch, sort_keys=True))
    report("no-local-or-remote-listed-future-path", no_local_or_remote_future_path, json.dumps(branch, sort_keys=True))
    report("no-changed-listed-future-path-since-cycle23", no_changed_future_path_since_cycle23, json.dumps(branch, sort_keys=True))
    report("no-source-presence-key-set", source_presence_absent, json.dumps(present_by_source, sort_keys=True))
    report(
        "cycle24-post-cycle23-main-status-drift-guard-passed",
        cycle24_post_cycle23_main_status_drift_guard_passed,
        f"passed={cycle24_post_cycle23_main_status_drift_guard_passed}",
    )

    result = {
        "actual_current_surface_status": (
            "exact negative boundary / PR230 non-chunk cycle-24 "
            "post-cycle-23 main-status-drift reopen guard: origin/main "
        ...
        "proposal_allowed_reason": (
            "Cycle 23 already rejects repeated origin/main status drift.  The "
            "cycle-23 head remains an ancestor of the aligned PR heads, "
        ...
        "exact_next_action": (
            ...
            "cycle-23, cycle-24, assembly, retained-route, and campaign gates "
