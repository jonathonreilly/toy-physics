...
here is admitted as a framework premise, and this runner does
not change the physical Cl(3) local algebra / Z^3 spatial substrate baseline.
...
attempts a NUMERICAL closure of those two scalars by:
   ...
   2. Composition route HK -> Wilson -> MSbar:
        ...
        - Z_n^{HK->W} from single-plaquette matching of <P>_HK vs <P>_W
          to the C-iso SU(3) NLO bounded note
   ...
   3. Cross-check via b_2^MSbar(N_f=6) = -65/2 from TVZ 1980 against
      eq. (*) plugged with computed Z_10, Z_20 and universal b_0=7, b_1=26,
...
Source content not yet available:
...
current source content. What WE CAN derive from upstream <P>_HK and <P>_W
...
What this runner does NOT close:
   ...
   - These remain bounded admissions; they require the HK Feynman
     rules at quadratic + cubic vertex level which are not available here.
...
Positive (PASS expected):
   ...
   - Single-plaquette diff 7/9 s_t^2 verified source input.
...
Bounded admissions (ADMITTED expected, documented):
   ...
   - Fermion-loop b_2^HK(N_f>0) (separate source boundary from C-iso SU(3) NLO).
...
def section1_P1_BZ_integration(c: Counter) -> float:
    ...
    c.record("P_1 BZ integral converges to AFP literature value (relative err < 1e-6)",
             rel_diff < 1e-6,
...
def section2_Z10_W_MS_at_SU3(c: Counter, P1: float) -> float:
    ...
    c.record("Z_10^{W->MS, SU(3)} reproduces AFP eq. (2.10) at <1e-7",
             rel_diff < 1e-7,
...
def section4_HK_to_Wilson_mean_field(c: Counter) -> Tuple[float, float]:
    ...
    The mean-field relation is defined by direct equality of the two
    single-plaquette expansions at the same physical plaquette value. In
    terms of s_t, with canonical xi=1 and s_t = g^2/2:
    ...
    Solving for s_t_W in terms of s_t_HK:
        s_t_W = s_t_HK + ((-(8/9) + (1/9))/(4/3)) s_t_HK^2 + O(s_t_HK^3)
    ...
    The full Z_10^{HK->W} from BZ integration over HK propagators remains
    a bounded frontier beyond this mean-field estimate. For Z_20^{HK->W},
    the mean-field correction is at order s_t^2 in the matching, requiring
    the s_t^3 coefficients of both expansions: <P>_HK at 32/81, <P>_W
    with c_3^W still bounded admission.
    """
    print()
    print("Section 4 -- HK -> Wilson single-plaquette matching (mean-field)")

    # Source-supported <P>_HK Taylor coefficients
    P_HK_c1 = Fraction(4, 3)
    P_HK_c2 = -Fraction(8, 9)
    P_HK_c3 = Fraction(32, 81)
    # Source-supported <P>_W Taylor coefficients (at NLO; c_3 admission per C-iso note)
    P_W_c1 = Fraction(4, 3)
    P_W_c2 = -Fraction(1, 9)
    ...
    # Direct check: <P>_W(s_t_W) = (4/3) s_t_W - (1/9) s_t_W^2 + ...
    ...
    # an asymptotic fit ~ -4.33 in the C-iso note).
    #
    ...
    print()
    print("  ==> Mean-field Z_10^{HK->W}, Z_20^{HK->W} derived from upstream")
...
def section6_cross_check_TVZ(c: Counter,
    ...
    Z_10, Z_20 respectively); HK side at N_f>0 is a separate source boundary
    ...
    For N_f=6, we DOCUMENT the additional bounded admissions:
       - HK action with N_f quark loops: not available here
    ...
    # b_2^HK pure-gauge (source input, N_f=0): 32/81 -- but this is in the
    ...
    # same normalization as b_2^MS. The C-iso note supplies the s_t Taylor
    ...
    print(f"    b_0(N_f=0)       = {b0_nf}  (universal)")
    print(f"    b_1(N_f=0)       = {b1_nf}  (universal)")
    print(f"    b_2^MS(N_f=0)    = {b2_MS_nf} = 2857/2  (TVZ 1980, ADMITTED)")
    print(f"    b_2^HK pure-gauge: 32/81 source Taylor coefficient")
...
def section7_bounded_admissions(c: Counter) -> None:
    ...
    c.admit("Wilson c_3^W asymptotic fit (-4.33)",
            ...
            "NLO bounded note, asymptotic numerical fit")
    c.admit("Z_n^{HK->W} beyond mean-field",
            ...
            "quadratic + cubic vertex level, which are not available here")
    c.admit("N_f-dependence of Z_n^{HK->MS} at N_f > 0",
            "quark-loop tadpoles on both HK and W->MS sides; HK action "
            "is used here at pure-gauge level only (X-L1-MSbar admission)")
    c.admit("Normalization of b_2^HK (pure-gauge 32/81) to MS-beta units",
            "single-plaquette s_t Taylor coefficient is the source input; "
...
def section8_hostile_review(c: Counter) -> None:
    ...
    print("       - Z_n^{HK->W} mean-field derivation from single-plaquette source input.")
    ...
    print("  Answer: IMPORTED from AFP 1996/1997 literature. The 1-loop Symanzik")
    ...
    print("  Answer: At SU(3) at NLO, the single-plaquette matching uses ONLY the")
    print("       upstream <P>_HK = 1 - exp(-(4/3) s_t) Taylor coefficients and")
    print("       the C-iso bounded <P>_W NLO expansion. The 7/12 leading ratio")
    print("       is the upstream source input. The mean-field result is therefore SAFE at")
    ...
    c.record("Q3: Mean-field bound on Z_n^{HK->W} is leading-order safe",
             True,
             detail="(7/12 ratio from C-iso SU(3) NLO source input)")

    print()
    print("  Q4: Does the cross-check pass for b_2^MS(N_f=6) = -65/2?")
    print("  Answer: The cross-check is PARTIAL. At N_f=0, identity (*) round-trips")
    ...
    print()
    print("  Q5: New axioms or status promotions?")
    print("  Answer: NONE. The run produces NUMERICAL CONSEQUENCES of named")
    print("       content + literature comparators. No new content is admitted")
    print("       as a framework premise or status promotion.")
    c.record("Q5: No new axioms or status promotions",
             True,
...
def section9_verdict(c: Counter,
    ...
    if c.fail_count == 0:
        ...
        print("      - P_1 numerically derived to <1e-6 relative")
        print("      - Z_n^{W->MS} reproduced via composition")
        print("      - Mean-field Z_n^{HK->W} derived from upstream <P>_HK, <P>_W")
        ...
        print("      - Was: 'two scalar BZ integrals; source functional form' (PR #1059)")
...
def main() -> int:
    ...
    print("from upstream <P>_HK and <P>_W expansions. Document remaining admissions.")
    print()
