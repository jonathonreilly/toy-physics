...
for the live y_t / m_t lane on the branch.

The purpose is classification, not new dynamics:

- structural bridge ambiguity is already closed on this branch;
...
print("pieces now that structural bridge ambiguity is closed on branch.")
print()
t0 = time.time()
...
print("Derived branch systematics:")
...
report(
    ...
    "all systematic components are explicit nonnegative branch quantities",
)
report(
    "structural bridge ambiguity is closed at branch scale",
    STRUCTURAL_CLASS_RESIDUAL == 0.0,
    "exact Schur class, stability gap, and microscopic admissibility are closed on branch",
)
report(
...
report(
    "the live y_t lane now satisfies the branch bar for derived-with-explicit-systematic wording",
...
print("-" * 78)
print(
    "On this branch, the remaining y_t uncertainty is no longer a structural "
...
print(
    ...
    "is closed at branch scale."
)
print()
print(
    f"So the live branch-safe read is: y_t(v) = {YT_CENTRAL:.4f} "
