...
Source-note runner for:
  docs/KOIDE_X_S4B_COMBINED_HIGGS_EWSB_G2_NOTE_2026-05-08_probeX_S4b_combined.md

Verdict: BOUNDED, NOT CLOSED.

Tests whether the COMBINATION of three retained ingredients —
...
— jointly closes Lane 2 step S4b-op (post-EWSB Higgs-mass operator
...
Tests verified:
  ...
  K1.7  Sister-prediction asymmetry preserved: Higgs prediction is
        ~100x worse than retained sister predictions.
...
single-axiom + single-MC chain (Cl(3), Z^3, <P>=0.5934).
...
print("=" * 72)
print()
print("Verdict: BOUNDED, NOT CLOSED.")
print()
...
print("closes Lane 2 step S4b-op as a positive theorem (~5% to PDG).")
print()
print(f"Retained constants (computed from framework chain, no PDG input):")
...
total += 1
passed += check(
    "v_EW retained matches hierarchy chain to ~+0.03% vs PDG (falsifiability)",
...
total += 1
passed += check(
    "u_0 from <P>^(1/4) matches retained 0.8776",
    abs(U_0 - 0.8776) < 1e-3,
    f"u_0 = {U_0:.6f}",
)
...
# indistinguishable from V''(0) at retained values.
...
total += 1
passed += check(
    "SM Higgs potential FORM not derived from retained content",
