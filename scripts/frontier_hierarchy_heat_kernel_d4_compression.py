...
is D=4-consistent under the admitted per-determinant readout via the
heat-kernel + zeta-regularized free-energy density reading
...
This runner verifies:
  ...
  4. Dimensional consistency: `f` has mass dim 4, `v` has mass dim 1,
     and the D=4 power index 1/D = 1/4 is the admitted mass-dim-1
     readout tested here.
  ...
  7. Stefan-Boltzmann source lineage: T ∝ u^(1/4) inversion is the same
     dimensional analysis as v ∝ f^(1/4); cross-checks numerical
     consistency via the source-note Stefan-Boltzmann constant.
"""
...
def section_1_note_structure():
    ...
    # Cross-references
    check(
        "note cites parent narrow source note",
        "HIERARCHY_MATSUBARA_DETERMINANT_NARROW_THEOREM_NOTE_2026-05-02" in text,
    )
    check(
        "note cites Stefan-Boltzmann source note",
...
def section_2_zeta_log_det():
    ...
    # Cross-check parent narrow source-note closed form
    ...
    for L_t in [2, 4]:
        ...
        check(
            f"closed-form |det(D†D, L_t={L_t})|² matches parent narrow source note",
...
def section_3_weyl_asymptotic():
    ...
    print("  the dimensional-analysis content of (1/4) is a bounded consistency check because it")
    print("  reflects the admitted D=4 taste-count identity 1/D = 4/2^D.")
...
def section_4_dimensional_consistency():
    ...
    for D_test in [1, 2, 3, 4, 5, 6]:
        ...
        # D=1: 4/2 = 2, 1/1 = 1, so the identity also fails.
    ...
    # Cross-check numeric: σ_SB constant from the Stefan-Boltzmann source note
...
def section_5_cross_endpoint_ratio():
    ...
    # Parent narrow source note: |det(D, L_t=4)| / |det(D, L_t=2)|^2 = (7/8)^16
    det_4_over_det_2_sq = Fraction(7, 8) ** 16
    direct = Fraction(7, 2) ** 16 / Fraction(4, 1) ** 16
    check(
        "(7/2)^16 / 4^16 = (7/8)^16 (parent narrow source note)",
...
def section_6_d4_specificity():
    print("\n[Section 6] D=4 specificity under the admitted determinant readout")
    print("-" * 70)
    ...
    check(
        "1/D = 4/2^D holds at D=4 and fails at the checked non-D=4 values",
        Fraction(4, 2**4) == Fraction(1, 4),
        "bounded D=4 consistency identity confirmed",
    )


# ---------------------------------------------------------------------------
# Section 7: Stefan-Boltzmann source lineage
# ---------------------------------------------------------------------------
...
def section_7_stefan_boltzmann_lineage():
    ...
    if SB_NOTE_PATH.exists():
        ...
        check(
            "Stefan-Boltzmann source note exists",
            True,
            str(SB_NOTE_PATH.name),
        )
        check(
            "SB source note includes T^4 dependence",
            "T⁴" in sb_text or "T^4" in sb_text,
        )
        check(
            "SB source note includes (π²/15) prefactor",
...
def section_8_bounded_caveats():
    ...
    expected_caveats = [
        ...
        ("Per-determinant geometric-mean readout admission inherited",
         "per-determinant geometric-mean" in text.lower() or "Per-determinant geometric-mean" in text),
        ("Counterfactual Pass record",
...
def main():
    ...
    print("=" * 70)
    if FAIL == 0:
        print("VERDICT: Heat-kernel D=4 compression bounded support verified.")
        print("(1/4) is D=4-consistent under the admitted determinant readout:")
        print("  (a) v ~ f^(1/4) via Stefan-Boltzmann source lineage;")
        print("  (b) 1/D = 4/2^D = 4/N_taste algebraic identity at D=4.")
        print("(7/8) is the exact rational identity from the parent narrow source note.")
