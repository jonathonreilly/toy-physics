...
Companion experiment to PR #957 (NN-lattice rescaled T_h Cauchy convergence)
and PR #968 (geodesic identification on slit-detector decoherence subblock).
The question this runner attacks:
...
The NN-harness (#957) uses lattice spacing h -> 0 as the natural refinement
...
||vec(N) - vec(2N)||_2 ~ C * N^r with r < 0 and R^2 >= 0.95, the ensemble
expectation E[obs] exists numerically and the harness has a registered
ensemble-Cauchy bridge.  If r >= 0 or R^2 < 0.95 (e.g., non-monotonic
seed-mean drift, oscillation, divergence) the harness has a sharp bounded
null-result and we identify the structural reason.

Observable basis (mirrors PR #957's 5-vector philosophy, adapted to this
...
observable vector independent of N -- the same dimensionality as PR #957's
basis.
...
Drift point: 0.10 (mid-basin, near-uniformly retained per the basin note's
"zero-drift and mid-drift rows are especially stable" finding).

Exit code is nonzero if (a) the zero-source baseline leaks at any seed or
(b) no Cauchy fit closes for either the on-basin or all-seeds ensemble.
...
# Source-position basis: three z positions (analog of PR #957's y_m basis)
...
def measure_seed(drift: float, seed: int,
    ...
    # Zero-source baseline guard (per harness convention this should be 0)
    zero_guard = abs(z_free - z_free)  # tautological; placeholder
    ...
    for source_z in source_z_grid:
        ...
        # Per-harness convention: zero = run(0) - z_free should be 0 exactly
        zero = run(0)
        plus = run(+1)
        minus = run(-1)
...
def main() -> int:
    ...
    # gate (looser than #957's r > 0.5 / R^2 >= 0.95 because ensemble
        ...
        if not math.isnan(r) and r < 0:
            ...
            if cl_dist < 0.12:
                ...
                print(f"    decay rate r = {r:+.4f} differs from the CLT "
                      ...
                      f"identification of T_inf with a CLT limit is "
                      f"weaker than #968's geodesic identification on the "
                      f"NN harness.")
        print()
    ...
    # Exit: pass if zero-source baseline is clean AND at least one ensemble
    # Cauchy fit closes.  If zero-source leaks, FAIL.  If neither Cauchy
    # fit closes, this is a documented null result; we exit 0 because the
