...
KEY INSIGHT:
  The taste staircase now has a sharper constructive placement on branch:
  the endpoint-exchange midpoint theorem fixes the doublet anchor k_B = 8,
  and the adjacent singlet-placement theorem fixes k_A = 7, k_B = 8 on the
  minimal bridge. The newer residual-sharing theorem also fixes
  eps/B = alpha_LM/2 on that bridge, the canonical U_Z3 bridge fixes the
  reduced texture factor exactly as 1/3, and the retained Dirac theorem
  y_nu^eff = g_weak^2/64 then predicts the atmospheric-scale m_3 directly.
  This script uses that benchmark chain and retains a wider staircase scan
  only as a sensitivity check. The remaining open issue is now narrower still:
  not local branch selection or local sheet fixing. The old free-lambda phase-
  lift ambiguity is now gone: exact Z_3 character transfer discretizes that
  family to lambda in {-1,0,+1}, with lambda=+1 the source-oriented branch.
  But the exact Z_3-covariant circulant family is still a physical no-go in
  the heavy-neutrino mass basis, and the full admitted canonical two-Higgs
  lane is now closed negatively on the exact source-phase branch too. So this
  script remains a reduced benchmark rather than the final theorem kernel. The
  raw right-frame blocker is now gone on the generic full-rank patch: the DM
  right orbit already has the intrinsic positive representative Y_+(H)=H^(1/2),
  so the remaining post-canonical bridge is read directly from H through the
  Z_3-basis singlet-doublet slot pair a(H), b(H). But the exact residual-Z_2
  aligned Hermitian core is still intrinsically CP-empty on that route. So the
  live missing object has moved onto the H-side: the active-neutrino Hermitian
  symmetry-breaking law away from the aligned core.
...
# -- Retained weak-sector Dirac coefficient from the Schur theorem --
G_WEAK = 0.653
Y_NU_EFF = G_WEAK**2 / 64.0

# -- Observed (for comparison ONLY, never used as input) --
ETA_OBS = 6.12e-10

# -- Neutrino atmospheric scale (comparison ONLY, never used as input) --
DM2_31_OBS = 2.453e-3   # eV^2
M3_NU_OBS = np.sqrt(DM2_31_OBS)
...
log("""
  ...
  For the explicit worked example below we now use the branch's constructive
  placement:
    A at k=7 (M ~ 6.1e11 GeV)
    B at k=8 (M ~ 5.6e10 GeV)

  The LIGHTEST RH neutrino N_1 has mass M_1 = |B - eps| ~ B * (1 - eps/B).
  On the current minimal symmetric bridge, the split law is now:
    eps/B = alpha_LM/2
""")
...
# Exact split law on the minimal symmetric residual-sharing bridge
...
# =============================================================================
# STEP A (continued): Seesaw prediction
# =============================================================================
log()
log("-" * 78)
log("STEP A (continued): Seesaw prediction")
log("-" * 78)
log("""
  Type-I seesaw on the current diagonal benchmark:
    m_nu ~ y_0^2 * v^2 / M_R

  The retained local Dirac lane already fixes:
    y_0 = y_nu^eff = g_weak^2 / 64

  So once M_1 is fixed by the staircase bridge above, the heaviest light
  neutrino is no longer fitted. It is predicted directly:
    m_3 = y_0^2 * v^2 / M_1
""")

y0 = Y_NU_EFF
y0_sq = y0**2
M3_NU = y0_sq * V_EW**2 / M1 * 1e9
m3_GeV = M3_NU * 1e-9
...
log(f"  y_0 = y_nu^eff = g_weak^2 / 64 = {y0:.6f}")
log()
...
log(f"  Dm^2_31 (pred) = {dm31_pred:.4e} eV^2  (obs: {DM2_31_OBS:.4e})")
log()
...
check("m3_atmospheric_scale", abs(m_nu3 - M3_NU_OBS) / M3_NU_OBS < 0.05,
      f"m_3 = {m_nu3:.4e} eV vs obs sqrt(Dm^2_31) = {M3_NU_OBS:.4e} eV",
      category="DERIVED")

check("dm31_atmospheric_gap", abs(dm31_pred - DM2_31_OBS) / DM2_31_OBS < 0.05,
      f"Dm^2_31 = {dm31_pred:.4e} vs obs {DM2_31_OBS:.4e}",
      category="DERIVED")
...
# The texture factor from the exact Z_3 bridge:
# The canonical U_Z3 basis change is the 3x3 discrete Fourier transform, so
# every overlap square is exactly 1/3 on the retained bridge.
texture_factor = 1.0 / 3.0  # exact on the retained U_Z3 bridge
...
doublet_CP = 2.0 * eps_over_B * np.sin(PHI_CP)
...
log("""
  ...
  => m_tilde = y_0^2 * v^2 / M_1 = m_3 (now predicted on the exact benchmark)
...
for phi_deg in [30, 45, 60, 90]:
    ...
    eta_s, eps_s, kap_s, K_s = compute_eta_scan(k_B, M3_NU, phi_scan, eps_over_B)
...
for m3_scan in [0.02, 0.03, 0.05, 0.08]:
    eta_s, eps_s, kap_s, K_s = compute_eta_scan(k_B, m3_scan, PHI_CP, eps_over_B)
...
log()
for epsB_scan in [0.01, 0.02, eps_over_B, 0.08, 0.15]:
    eta_s, eps_s, kap_s, K_s = compute_eta_scan(k_B, M3_NU, PHI_CP, epsB_scan)
...
log(f"    7. y_nu^eff = g_weak^2/64 = {y0:.6f}                  [EXACT]")
log(f"    8. k_A=7, k_B=8 on the minimal bridge                [EXACT ON BRIDGE]")
log(f"    9. eps/B = alpha_LM/2 = {eps_over_B:.6f}             [EXACT ON BRIDGE]")
log(f"   10. texture factor = 1/3 on U_Z3                      [EXACT ON BRIDGE]")
log(f"   11. m_3 = {m_nu3:.4e} eV and Dm^2_31 = {dm31_pred:.4e} [DERIVED]")
log()
log("  WHAT IS STILL REDUCED / STRUCTURAL:")
log(f"   12. CP phase phi = pi/3 in the reduced epsilon_1 kernel [STRUCTURAL]")
log(f"       -- natural Z_3 phase, but full CP kernel not yet derived")
log(f"   13. Reduced epsilon_1 decomposition into N_2 + N_3      [REDUCED MODEL]")
log(f"       -- the exact circulant local odd-slot route is now sharper:")
log(f"          exact Z_3 character transfer fixes lambda=+1 on the")
log(f"          source-oriented branch, but the whole exact circulant family")
log(f"          still stays real in the heavy-neutrino mass basis")
log()
log("  WHAT IS NOT DERIVED:")
log(f"   14. The active-neutrino Hermitian symmetry-breaking law away from the")
log(f"       residual-Z_2 aligned core")
log(f"   15. Equivalently: the breaking slots away from")
log(f"       d_2=d_3, r_12=r_31, phi=0 on the active Hermitian grammar")
log(f"   16. Solar/full-matrix neutrino closure and spectator/flavor refinements")
log()
...
if ratio > 1:
    bench_read = f"overproduces by {ratio:.1f}x"
elif ratio < 1:
    bench_read = f"underproduces by {1/ratio:.1f}x"
else:
    bench_read = "matches exactly"

log(f"    At the exact minimal-bridge benchmark level k_B={k_B}:")
log(f"      eta ~ {eta_lepto:.1e} ({bench_read})")
log(f"    At the optimal staircase level k_B={best_k}:")
log(f"      eta ~ {eta_best:.1e} (ratio = {ratio_best:.1f}x)")
log(f"    The retained local Dirac lane now selects k_B~=8 through")
log(f"    y_nu^eff = g_weak^2/64 on the exact minimal bridge.")
...
log(f"    This is a BOUNDED prediction because the branch still uses a reduced")
log(f"    leptogenesis CP kernel even though y_nu^eff, k_A=7, k_B=8,")
log(f"    eps/B=alpha_LM/2, the U_Z3 texture factor 1/3, and the atmospheric")
log(f"    benchmark m_3 are now fixed on the minimal bridge. The remaining")
log(f"    missing object is now sharper than a generic flavor texture:")
log(f"    exact Z_3 character transfer already fixes the phase-lift branch to")
log(f"    lambda=+1, but the whole exact Z_3-covariant circulant class still")
log(f"    vanishes in the heavy-neutrino mass basis, while the raw right-frame")
log(f"    blocker is now gone on the generic full-rank patch because the")
log(f"    positive representative Y_+(H)=H^(1/2) makes the remaining bridge")
log(f"    intrinsic from H. The exact remaining obstruction is now H-side:")
log(f"    if the active Hermitian data stay on the residual-Z_2 aligned core,")
log(f"    the intrinsic positive-section CP tensor still vanishes exactly.")
log(f"    So the last-mile denominator object is the Hermitian symmetry-breaking")
log(f"    law away from that aligned core.")
log()
...
log()
log("=" * 78)
log("FULL DERIVATION CHAIN: Cl(3) on Z^3  -->  eta_B (minimal-bridge k_A=7,k_B=8 chain)")
log("=" * 78)
log(f"""
  Cl(3) on Z^3
    |
