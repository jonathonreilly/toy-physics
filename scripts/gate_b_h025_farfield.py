#!/usr/bin/env python3
"""Gate B h=0.25 far-field harness.

Bounded h=0.25 companion to gate_b_farfield_harness.py.

Question:
  On the retained generated-geometry family, does the far-field gravity
  package survive at finer spacing h=0.25?

This harness freezes only:
- far-field TOWARD fraction
- F~M

The replay is intentionally narrow:
- exact grid control
...
def measure_row(label: str, drift: float, restore: float) -> FarRow:
    toward = 0
    total = 0
    fm_vals: list[float] = []
    ...
    return FarRow(label=label, toward=toward, total=total, fm=fm)


def main():
    t_total = time.time()
    print("=" * 72)
    print("GATE B H=0.25 FAR-FIELD HARNESS")
    print("  exact grid vs grown geometry on the retained finer-spacing family")
    print("=" * 72)
    print(f"h={H}, W={PW}, L={int((NL - 1) * H)}, seeds={len(SEEDS)}, z={Z_MASSES}")
    print("growth rule: template previous layer + drift + restore + NN connectivity")
    print()
    print(f"{'geometry':<18} {'TOWARD':>14} {'F~M':>8}")
    ...
    for label, drift, restore in ROWS:
        ...
        print(f"{label:18s} {row.toward:>5d}/{row.total:<5d} ({pct:>5.0%}) {fm_s:>8}")

    print()
    print("SAFE INTERPRETATION")
    print("  This harness freezes the h=0.25 far-field comparison only.")
    print("  Use it to check whether the retained grown-geometry package survives")
    print("  refinement in the same bounded family.")
