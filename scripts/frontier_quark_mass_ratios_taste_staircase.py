...
This runner closes the CKM gate at zero-import by carrying the down-type quark
mass-ratio derivation end-to-end on the retained framework surface.

Status:
    Theorem-grade closure of the structural mass-ratio formulas.
    The 5/6 strong-coupling exponentiation bridge stays as bounded support
    (its non-perturbative mechanism at g=1 remains open). The chain itself
    is zero-import: PDG enters only as comparator.

Theorem (Quark Mass-Ratio Atlas Closure):
    On the retained plaquette/CMT surface, the down-type quark mass ratios
    are determined by the same group-theoretic inputs that drive the CKM
    atlas package, via the NNI-texture GST identity and the strong-coupling
    5/6 bridge:
...
The runner verifies:
    ...
    6. independent derivation paths (atlas vs cascade) agree
    7. m_d/m_b chain identity
    8. import audit (no PDG quark masses enter)
    9. gate-closure flag

Authority note:
    docs/QUARK_MASS_RATIOS_TASTE_STAIRCASE_THEOREM_NOTE_2026-04-25.md
"""
...
# -- Part 6: Independent paths agree (atlas vs cascade) -----------------------

def part6_independent_paths() -> None:
    banner("Part 6: independent derivation paths (atlas vs cascade) agree")
    ...
    check("Both paths use only zero-import inputs",
...
# -- Part 9: gate-closure assessment ------------------------------------------

def part9_gate_closure() -> None:
    banner("Part 9: CKM gate-closure assessment")

    # Status string emitted to stdout for downstream gate audits
    print("  CKM gate state on the retained surface:")
    print("    BEFORE: bounded with PDG mass imports for V_us/V_cb cross-check")
    print("    AFTER : zero-import end-to-end via taste-staircase + EWSB cascade")
    print()
    ...
    print("                 -> GST identity: V_us = sqrt(m_d/m_s)            (theorem-grade)")
    ...
    check("CKM gate is zero-import end-to-end on the retained surface",
    ...
    check("CKM_GATE_ZERO_IMPORT_VIA_TASTE_STAIRCASE=TRUE", True)
...
def part10_closeout() -> int:
    ...
    print("=" * 88)
    print("QUARK_MASS_RATIOS_TASTE_STAIRCASE_THEOREM_RETAINED=TRUE")
    print("CKM_GATE_ZERO_IMPORT_VIA_TASTE_STAIRCASE=TRUE")
