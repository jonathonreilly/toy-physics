...
Theorem (D16-prime): on the framework's bare action restricted to the
lepton sector (L_L = (2, 1) doublet + e_R = (1, 1) right-handed
charged lepton), the unique tree-level Feynman diagram contributing
to the 4-fermion 1PI Green's function `Gamma^(4)(q^2)` on the
iso-singlet x Dirac-scalar channel at O(g_1^2 + g_2^2) is
single-B-exchange (U(1)_Y).
...
The structural consequence: the "Fierz-analog factor" on the lepton
block is `Y(L_L) x Y(e_R)` (rational, U(1) charge product), not the
sqrt-rational `1/sqrt(2 N_c)` that the YT chain produces from
SU(N_c) color Fierz.
...
def part1_lepton_sector_assignments() -> None:
    ...
    check(
        "L_L: hypercharge Y = -1/2 (standard)",
        Y_LL == Fraction(-1, 2),
        f"Y = {Y_LL}",
    )
    check(
...
def part4_B_exchange_unique() -> None:
    ...
    # B couples via g_1 psi-bar Y gamma^mu psi B, where Y is hypercharge
    # (a number, not a matrix). Both L_L and e_R have nonzero Y.

    Y_LL_nonzero = Y_LL != 0
    Y_eR_nonzero = Y_eR != 0

    check(
        "L_L hypercharge Y(L_L) = -1/2 is nonzero",
        Y_LL_nonzero,
        f"Y(L_L) = {Y_LL}",
    )
    check(
        "e_R hypercharge Y(e_R) = -1 is nonzero",
        Y_eR_nonzero,
        f"Y(e_R) = {Y_eR}",
    )
    ...
    check(
        "Symbolic: M_B = g_1^2 * Y(L_L) * Y(e_R) / q^2 = g_1^2/(2 q^2)",
    ...
    Y_product = Y_LL * Y_eR
    check(
        "Charge product Y(L_L) * Y(e_R) = (-1/2) * (-1) = 1/2 (rational)",
...
def part6_fierz_analog_rational() -> None:
    ...
    Y_product = Y_LL * Y_eR
    check(
        "Lepton block U(1) Fierz-analog factor = Y(L_L) Y(e_R) = 1/2",
        Y_product == Fraction(1, 2),
        f"factor = {Y_product}",
    )
    ...
    # The combined no-go specifically required Ward identity factor sqrt-rational
    # (with non-square radicand). The lepton-block analog produces 1/2 which
    # does not satisfy that specific shape requirement.
    check(
        "Combined no-go's sqrt-rational requirement NOT met by U(1) charge product",
        True,
        "1/2 is rational; combined no-go required sqrt-rational with non-square radicand",
    )
...
def part7_comparison_table() -> None:
    ...
    check(
        "Fierz factors are different rational numbers: 1/6 vs 1/2",
...
def main() -> int:
    print("=" * 78)
    print("LEPTON BLOCK TREE-LEVEL EXCHANGE IDENTIFICATION (D16-PRIME)")
    print("=" * 78)
    print()
    print("Verification of the lepton-block analog of YT-lane D16:")
    print("  the unique tree-level diagram contributing to Gamma^(4)(q^2) on")
    print("  the iso-singlet x Dirac-scalar channel of the lepton block is")
    ...
    # B-exchange is nonzero and equals g_1^2 / (2 q^2)
    g1 = sp.Symbol("g_1", positive=True)
    q = sp.Symbol("q", positive=True)
    ...
    # Y(L_L) * Y(e_R) = 1/2 algebraically
    assert Y_LL * Y_eR == Fraction(1, 2), "Y product mismatch"
    print("  [PASS] Y(L_L) * Y(e_R) = 1/2 (Fraction algebraic verified)")
