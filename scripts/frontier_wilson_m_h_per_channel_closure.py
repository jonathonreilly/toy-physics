...
"channel identification". This runner verifies four cases:
  ...
  S = {0..4}   (uniform-16):      closure r_{16}  ≈ 0.26855  (all-orders ref)
...
The runner verifies, at exact rational precision via fractions.Fraction:
  ...
  (Part 9) S = {0..4} uniform-16 cross-check on bracket [0.26, 0.28];
           reproduces r_{16} ≈ 0.26855 of the all-orders note;
...
def part1_note_structure():
    ...
    required = [
        ...
        ("k13 closure value 0.24383 stated",
         "0.24383"),
        ("uniform-16 closure value 0.26855 stated (all-orders ref)",
         "0.26855"),
        ...
        ("Higgs-channel boundary upstream cited",
...
# ---------------------------------------------------------------------------
# Part 9: S = {0..4} uniform-16 cross-check (all-orders value 0.26855)
# ---------------------------------------------------------------------------
def part9_uniform16_cross_check() -> Fraction:
    section("Part 9: S = {0..4} uniform-16 cross-check (all-orders r_{16} ≈ 0.26855)")
    S = [0, 1, 2, 3, 4]
    ...
    print(f"    all-orders note value: 0.26855")
    ...
    all_orders_value = Fraction(26855, 100000)
    diff = abs(r_16 - all_orders_value)
    check(
        "uniform-16 closure matches all-orders note value 0.26855",
        diff < Fraction(1, 10**4),
        f"|r_{{16}} - 0.26855| = {float(diff):.2e}",
    )

    # Sanity: the uniform-16 form is also the (1/64) Σ all-orders form:
    all_orders_form_at_r16 = Fraction(0)
    rsq = r_16 * r_16
    for k in range(5):
        kk_sq = (k - 2) ** 2
        x = kk_sq * rsq
        all_orders_form_at_r16 += BINOM_4[k] * (U_0_SQ - x) / (x + U_0_SQ) ** 2
    all_orders_form_at_r16 /= 64
    general_form = per_channel_readout(r_16, S)
    check(
        "general per-channel readout at S=full equals all-orders (1/64) Σ form",
        general_form == all_orders_form_at_r16,
    )
...
def part13_boundary_check():
    ...
    check(
        "independent audit-lane status authority present",
    ...
    check(
        "channel-boundary alternative convention referenced (561.1, 280.6, ...)",
...
def main() -> int:
    ...
    print("   S = {0..4} uniform-16 closes at r ≈ 0.26855 (all-orders ref).")
    ...
    if FAIL == 0:
        ...
        print(" reproduces the all-orders note value r_{16} ≈ 0.26855. All")
