...
three audit residuals named in
  ...
  Residual 2 — A → A/g rescaling freedom.
    ...
    simultaneously. This is the strengthened (positive_theorem-grade)
    version of the rescaling-freedom-removal claim, lifting it out of
    the 2026-05-03 audit's "decoration" tier.
...
def section_G_ledger_visibility():
    ...
    for cid in upstream_deps:
        check(
            ...
            cid in rows,
            f"effective_status = {rows.get(cid, {}).get('effective_status', 'missing')}",
            kind="BOUNDED",
        )
    ...
    for cid in new_rows:
        if cid in rows:
            check(
                f"new row '{cid}' seeded with audit-state",
                rows[cid].get("audit_status") in ("unaudited", "audited_clean"),
                f"audit_status = {rows[cid].get('audit_status', 'missing')}",
                kind="BOUNDED",
            )
        else:
            check(
...
def main() -> int:
    ...
    if FAIL == 0:
        ...
        print("    Lifts the rescaling-removal claim from decoration to positive_theorem")
        print("    candidate via the joint-form rigidity argument.")
        print()
        ...
        print()
        print("  Recommendation for parent G_BARE_DERIVATION_NOTE.md re-audit:")
        print("    Conditional on independent audit retention of the strengthened")
        print("    2026-05-07 theorem candidates, the parent row is eligible for")
        print("    re-classification from audited_conditional toward audited_clean.")
    else:
