...
kappa = alpha_LM uniform over 16 steps":
  ...
  (O2) Per-step perturbative non-uniformity: with n_taste^(k) = 16-k
       ...
       ~= 2.594 exceeds u_0 ~= 0.878 at retained alpha_LM ~= 0.09067.
...
Pure class-A structural enumeration plus an elementary arithmetic
inequality. No framework axiom or admission consumed as load-bearing.
The retained numerical values alpha_LM ~= 0.09067 and u_0 ~= 0.878
enter only in the numerical (O2) inequality and the (O3) categorical
identity check; they are not load-bearing for the four-obstruction
enumeration.
...
# ----------------------------------------------------------------------------
section("Part 4: (O2) Landau-pole inequality at retained alpha_LM  (T4)")
# T4: At retained alpha_LM = 907/10000 and u_0 = 0.8776... ~= 0.878,
...
u_0_decimal = Decimal("0.87768138")  # u_0 = <P>^{1/4} at retained <P> = 0.5934
...
check(
    ...
    "at retained alpha_LM ~= 0.0907 (Landau-pole crossing).",
    t4_ok,
...
# alpha_LM -> kappa exists in the framework's retained authority chain.
# ----------------------------------------------------------------------------
...
# Banach space declared by the framework's retained authority chain.
...
# Banach-norm-system semantics in the framework's retained chain.)
...
counter_evidence = {
    ...
    "(O3) categorical-type mismatch":
        ...
        "alpha_LM -> kappa exists in framework's retained authority chain.",
    "(O4) Coulombic-tail":
