#!/usr/bin/env python3
"""
Koide Brannen selected-line geometry / Route-3 support runner.

This runner collects two support results for the charged-lepton Brannen lane:

1. exact selected-line geometry on the retained Koide carrier, including the
   rigid-triangle rotation picture and the exact first-branch span `π/12`;
2. a conditional Route-3 Wilson-line support construction showing how the
   same rational `2/9` can appear as a one-clock natural-time phase.

It does NOT prove the remaining physical Brannen-phase bridge

    δ_physical = η_APS

on the actual selected-line `CP^1` carrier.

The Route-3 component remains conditional support:
...
Consequence: the retained selected-line geometry and the Route-3 phase law
converge numerically at `2/9`. This is meaningful support, but not by itself
the missing physical-identification theorem.
"""
...
def main() -> int:
    ...
    check(
        "1.3 W^{d²} = exp(2i)·𝟙 (Route 3 conditional support)",
    ...
    if n_pass == n_total:
        print()
        print("VERDICT: the selected-line geometry is strongly supported and the")
        print("Route-3 Wilson-line law is numerically consistent as a conditional")
        print("one-clock natural-time support route.")
        print()
        print("What this runner supports:")
        print("  - δ(m) is exactly the Euclidean rotation angle of the real")
        print("    Koide amplitude in the plane orthogonal to the singlet axis.")
        print("  - The first-branch span is exactly π/12 = 2π/|O| on the")
        print("    retained cubic/octahedral carrier.")
        print("  - The same rational 2/9 reappears on the conditional Route-3")
        print("    one-clock Wilson-line law.")
        print()
        print("What this runner does not prove:")
        print("  - that the physical Brannen phase on the selected-line carrier")
        print("    is forced by the Route-3 natural-time identification;")
        print("  - or that the ambient 2/9 value is already the physical")
        print("    charged-lepton observable without an extra bridge theorem.")
