#!/usr/bin/env python3
"""Closed-form analytic derivation of C_arm in the rescaled NN harness.

PR #968 identified T_inf on the rescaled NN harness's slit-detector decoherence
subblock as the geodesic continuum operator with arm-width scaling

    sigma_arm(h) = C_arm * h^alpha

A 4-point fit on h <= 0.25 returned
...
small systematic (residual 0.026 in alpha). The constant C_arm = 2.7107 was
not yet derived from first principles. This script closes that gap.
...
residuals against C_arm_numeric = 2.7107. No Monte Carlo, no lattice
propagation - all closed form.
...
# Numerical fit to reproduce / verify against
C_ARM_NUMERIC = 2.7107
ALPHA_NUMERIC = 0.5256
...
def report() -> None:
    print("=" * 78)
    print("ANALYTIC DERIVATION OF C_arm IN T_inf GEODESIC IDENTIFICATION")
    print("=" * 78)
    print()
    ...
    print()
    print(f"Numerical reference (PR #968, 4-point fit, h <= 0.25):")
    ...
    print(f"  residual vs numeric = {100 * reldiff(incoh['C_arm'], C_ARM_NUMERIC):+.2f}%")
    print()
    print("-" * 78)
    ...
    print()
    print("  The coherent post-slit estimate matches numerics to within")
    print(f"  {100 * res_post:.1f}%, well inside the 10% positive-theorem band.")
    ...
    print()
    print("  All four points agree with the rescaled-NN fit to within 4%.")
