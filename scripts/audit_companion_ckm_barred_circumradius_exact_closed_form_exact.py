...
on the retained NLO Wolfenstein protected-gamma-bar surface

  rho_bar  = (4 - alpha_s) / 24            (retained N1)
  eta_bar  = sqrt(5)(4 - alpha_s) / 24     (retained N2)
  R_b_bar^2 = (4 - alpha_s)^2 / 96         (retained N3)
  tan(gamma_bar) = sqrt(5)                 (retained N4 / N8, alpha_s-protected)
  tan(beta_bar)  = sqrt(5)(4 - alpha_s)/(20 + alpha_s)  (retained N5)
  alpha_bar = pi - gamma_0 - beta_bar      (retained N6 angle sum, with gamma_bar = gamma_0 by N4)
...
sympy-based exact-symbolic verification:
  ...
  (b) imports the upstream retained inputs (N1)-(N6) verbatim;
  ...
  (e) verifies (C3) the circumcenter coordinates by direct algebra from
      retained (N1), (N2), (N3) and the perpendicular-bisector
      construction;
  ...
  (h) verifies the structural-integer factorization
      ...
      at retained N_pair = 2, N_quark = 6;
...
def main() -> int:
    ...
    print("precision parametric in alpha_s on the retained NLO protected-gamma-bar surface.")
    print("=" * 88)

    # ---------------------------------------------------------------------
    section("Part 0: symbolic setup and imported retained inputs (N1)-(N6)")
    # ---------------------------------------------------------------------

    alpha_s = Symbol("alpha_s", positive=True, real=True)

    # Retained N1, N2 apex coordinates of the barred unitarity triangle.
    rho_bar = (4 - alpha_s) / Rational(24)
    eta_bar = sqrt(5) * (4 - alpha_s) / Rational(24)

    # Retained N3 side length squared.
    Rb_bar_sq = (4 - alpha_s) ** 2 / Rational(96)

    # Retained N5 protected-gamma-bar tangents.
    tan_beta_bar = sqrt(5) * (4 - alpha_s) / (20 + alpha_s)
    tan_gamma_bar = sqrt(5)  # retained N4
