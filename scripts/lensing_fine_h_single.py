#!/usr/bin/env python3
"""Single (family, seed, b) lensing measurement at H=0.25.

Run one at a time to avoid memory issues. The runner script
calls this with arguments: family_name seed b_value

Usage: python3 lensing_fine_h_single.py Fam1 0 3.0
"""
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 lensing_fine_h_single.py <family> <seed> <b>")
        sys.exit(1)
    ...
    if fam_name not in FAMILIES:
        print(f"Unknown family: {fam_name}")
        sys.exit(1)
    ...
    dt_kubo = time.time() - t1

    # Output as parseable line
    print(f"RESULT {fam_name} seed={seed} b={b:.1f} kubo={k:+.6f} grow={dt_grow:.1f}s kubo={dt_kubo:.1f}s nodes={len(pos)}")
