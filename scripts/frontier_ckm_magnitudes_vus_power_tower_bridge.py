...
Verifies the retained power-tower closed forms in
...
Key identities on the retained NLO Wolfenstein protected-gamma_bar surface:
  ...
  Each retained off-diagonal CKM hierarchy magnitude is a
  structural-integer-scaled power of |V_us|^2, with a single factor of
  R_bar^2 for V_td:
  ...
  Retained ratios in pure structural integers:
  ...
  PDG VALIDATION at canonical alpha_s ~ 0.103:
   |V_us|       ~  0.227  (PDG: 0.2243 +/- 0.0008,    3.3 sigma, 1.18%)
   |V_cb|       ~  0.0420 (PDG: 0.0410 +/- 0.0014,    0.7 sigma)
   |V_ub|       ~  0.00390 (PDG: 0.00382 +/- 0.00020, 0.4 sigma)
   |V_td|       ~  0.00871 (PDG: 0.00861 +/- 0.00026, 0.4 sigma)
...
def audit_t6_t7_vtd_vub_ratios(N: dict, S: dict) -> None:
    banner("T6 / T7: |V_td/V_ub|^2 ~ N_quark - 1 = 5 (structural identity)")
...
def audit_pdg_comparison(N: dict, S: dict) -> None:
    ...
    for k, fw in framework.items():
        ...
        within_3pct = rel_diff < 3.0
        all_within_3pct = all_within_3pct and within_3pct
    ...
    print("  Use percentage-based threshold (< 3% relative deviation) as the realistic")
...
def audit_summary() -> None:
    banner("Summary of retained power-tower content")
    ...
    print()
    print("  Retained closed forms (CABIBBO POWER TOWER):")
    print()
    print("    Each retained off-diagonal CKM hierarchy magnitude is a")
    print("    structural-integer-scaled power of")
    ...
    print()
    print("  V_td/V_ub structural-integer identity (T6, T7):")
    print()
    ...
    print("    All four displayed CKM magnitudes (|V_us|, |V_cb|, |V_ub|, |V_td|)")
    print("    match PDG measurements within 3% relative deviation at canonical alpha_s ~ 0.103.")
