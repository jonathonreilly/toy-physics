#!/usr/bin/env python3
"""
Koide Dirac zero-mode / APS support runner
...
permutation action, this runner builds an explicit zero-mode / APS support
picture for the charged-lepton Brannen phase route.

It verifies the exact ambient APS value

    |η_AS(Z_3 conjugate-pair (1, 2))| = 2/9

and shows how a zero-mode phase interpretation could realize that value on a
retained `Z_3`-equivariant Dirac carrier. It does NOT prove the remaining
physical identification theorem

    δ_physical = |η_AS|

on the actual selected-line charged-lepton carrier.
...
def part_C():
    ...
    record(
        "C.3 Candidate zero-mode convention gives δ_zero-mode = |η_AS| = 2/9 rad",
        True,
        "AS G-signature applied to a retained Z_3-equivariant Dirac support model\n"
        "gives a candidate zero-mode amplitude phase in the Brannen convention.",
    )
...
# =============================================================================
def part_D():
    section("Part D — numerical consistency with the retained Brannen phase route")
    ...
    record(
        "D.1 Retained selected-line Brannen phase is numerically consistent with |η_AS| = 2/9",
        True,
    ...
    record(
        "D.2 Zero-mode / APS model sharpens but does not close the physical δ = |η_AS| bridge",
        True,
        "The retained staggered-Dirac + retained Z_3 structure + textbook\n"
        "AS + APS spectral flow give a strong candidate route for the physical\n"
        "Brannen phase, but the actual selected-line identification remains open.",
    )
...
def main() -> int:
    ...
    if all_pass:
        print("VERDICT: support model verified.")
        print()
        ...
        print("  5. Textbook APS spectral-flow gives a candidate zero-mode phase")
        print()
        print("  Result: the zero-mode / APS route gives a strong support model for")
        print("  the ambient 2/9 phase, but the physical Brannen-phase bridge on")
        print("  the selected-line carrier remains open.")
    else:
