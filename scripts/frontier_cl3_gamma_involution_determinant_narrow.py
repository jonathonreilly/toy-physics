...
Verifies the abstract M_2(C) ≅ Cl(3,0) γ-involution identities plus the
full-lattice determinant-readout boundary at exact symbolic / rational
precision.

This narrow theorem isolates the abstract-algebra content of the
γ-involution `γ(M) = σ_2 M^T σ_2`:
- γ = symplectic adjoint (cofactor transpose);
- γ-norm identity `|M|_γ = √|det(M)|`;
- γ acts as -id on vector and bivector grades and +id on the pseudoscalar;
- per-element γ-norm does NOT close the framework's per-determinant
...
def test_symplectic_adjoint():
    """σ_2 · M^T · σ_2 = adj(M) for symbolic M."""
    ...
    print(f"[symplectic symbolic] γ(M) = adj(M):  {'PASS' if ok else 'FAIL'}")
    if not ok:
        print("  gM =", gM)
        print("  adj_M =", adj_M)
    return ok
...
def test_gamma_norm_symbolic():
    """M · γ(M) = det(M) · I_2 and ⟨M·γ(M)⟩_0 = det(M)."""
    s0, s1, s2, s3 = make_pauli()
    M = make_symbolic_M()
    gM = gamma_involution(M, s2)
    ...
    ok = diff == sp.zeros(2, 2)
    print(f"[gamma-norm symbolic] M·γ(M) = det(M)·I_2:  {'PASS' if ok else 'FAIL'}")
    ...
    print(
        f"[gamma-norm symbolic] ⟨M·γ(M)⟩_0 = det(M):  {'PASS' if ok_scalar else 'FAIL'}"
    )
    return ok and ok_scalar


def test_gamma_norm_numerical(n_trials: int = 10, rng_seed: int = 20260510):
    """|M|_γ = √|det(M)| for ten random rational plus imaginary-rational M."""
    s0, s1, s2, s3 = make_pauli()
    ...
    print(
        f"[gamma-norm numerical] |M|_γ = √|det(M)| on {n_trials} random M: "
        f"{passes}/{n_trials} {'PASS' if passes == n_trials else 'FAIL'}"
    )
    return passes == n_trials


def test_vector_grade_action():
    """γ(σ_i) = -σ_i and γ(I_2) = I_2."""
    s0, s1, s2, s3 = make_pauli()
    ...
    for name, sigma, sign in pairs:
        ...
        print(
            f"[vector grade] γ({name}) = {sign_str}{name}:  "
...
def test_bivector_grade_action():
    """γ(σ_i σ_j) = -σ_i σ_j for i ≠ j (6 ordered pairs / 3 unordered)."""
    s0, s1, s2, s3 = make_pauli()
    ...
    for i in (1, 2, 3):
        for j in (1, 2, 3):
            ...
            print(
                f"[bivector grade] γ(σ_{i}σ_{j}) = -σ_{i}σ_{j}:  "
                f"{'PASS' if ok else 'FAIL'}"
            )
            all_ok = all_ok and ok
    return all_ok
...
def test_pseudoscalar_action():
    """γ(ω) = +ω where ω = σ_1 σ_2 σ_3 = i I_2 in positive chirality."""
    s0, s1, s2, s3 = make_pauli()
    ...
    print(
        f"[pseudoscalar grade] γ(σ_1σ_2σ_3) = +σ_1σ_2σ_3: "
        f"{'PASS' if ok_omega else 'FAIL'}"
    )
    return ok_omega


def test_full_lattice_readout_boundary(n_trials: int = 5, rng_seed: int = 20260511):
    """Per-element γ-norm does not force the full determinant readout.
    ...
    print(
        f"[readout boundary] block-diagonal |det|^{{1/2}} = ∏|M_i|_γ: "
    ...
    print(
        f"[readout boundary] hopping → det(D_hop) ≠ det(M_1)·det(M_2):  "
        f"{'PASS' if has_gap else 'FAIL'}  (gap = {sp.simplify(det_hop - det_per_block_product)})"
    )
    return all_ok and has_gap


def test_basis_grade_action():
    """Verify the γ sign pattern on the positive-chirality Cl(3) basis."""
    s0, s1, s2, s3 = make_pauli()
    ...
    for name, elem in basis.items():
        ...
        print(
            f"[basis grade] γ({name}) = {sign_str}{name}:  "
            f"{'PASS' if ok else 'FAIL'}"
        )
        all_ok = all_ok and ok
    return all_ok
...
def main():
    ...
    print("[Part 1] γ-involution = symplectic adjoint (cofactor)")
    tests.append(("symplectic-adjoint", test_symplectic_adjoint()))
    print()

    print("[Part 2] γ-norm determinant identity (symbolic)")
    tests.append(("gamma-norm-symbolic", test_gamma_norm_symbolic()))
    print()

    print("[Part 3] γ-norm determinant identity (numerical, 10 random M)")
    tests.append(("gamma-norm-numerical", test_gamma_norm_numerical()))
    print()

    print("[Part 4] γ on vector grade (σ_i, I_2)")
    tests.append(("vector-grade", test_vector_grade_action()))
    print()

    print("[Part 5] γ on bivector grade (σ_i σ_j, i≠j)")
    tests.append(("bivector-grade", test_bivector_grade_action()))
    print()

    print("[Part 6] γ on pseudoscalar")
    tests.append(("pseudoscalar-grade", test_pseudoscalar_action()))
    print()

    print("[Part 7] γ action on full Cl(3) 8-element basis")
    tests.append(("basis-grade", test_basis_grade_action()))
    print()

    print("[Part 8] Per-element γ-norm vs per-determinant readout boundary")
    tests.append(("readout-boundary", test_full_lattice_readout_boundary()))
    print()
