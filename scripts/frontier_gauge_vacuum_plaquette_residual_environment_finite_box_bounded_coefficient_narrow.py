...
on the finite weight box B = {(p, q) : 0 <= p, q <= 4}. Specifically:
  ...
  (N1) structural class of R[rho] on H_B: diagonal in the character basis,
       self-adjoint, positive definite, conjugation-symmetric
       (rho_(p,q) = rho_(q,p));
  ...
  (N4) symbolic sympy check that the swap commutator [S, R[rho]] = 0 iff
       rho_(p,q) = rho_(q,p) on the finite box.
...
def main() -> int:
    ...
    # Diagonal coefficient-multiplier consistency on H_B. This deliberately
    # avoids the stronger and false claim that R commutes with arbitrary
    # pointwise class-function multiplication in the character basis.
    D_test = np.diag(np.array([1.0 + 0.1 * i for i in range(len(weights))]))
    diagonal_multiplier_err = float(np.max(np.abs(R @ D_test - D_test @ R)))
    ...
    print(f"  diagonal multiplier commutator     = {diagonal_multiplier_err:.3e}")
    print()
    ...
    check(
        "N1(d) R[rho(6)] commutes with arbitrary diagonal coefficient multipliers on the finite box",
        diagonal_multiplier_err == 0.0,
