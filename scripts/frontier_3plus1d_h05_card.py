...
Tests for EACH kernel (exp(-0.8*theta^2) and cos^2(theta)):
  ...
  4. F~M scaling (6 field strengths)
  5. d_TV
  6. Decoherence
  7. Distance law (z=1,2,3)
"""
...
LAM = 10.0
N_YBINS = 6
PHYS_W = 3          # transverse half-extent (reduced from 4 for h=0.5)
...
class Lattice4D:
    ...
    def propagate(self, field, k, blocked_set):
        ...
        print(f"    [propagate] {elapsed:.1f}s")
        return amps
...
def run_card(kernel_name, weight_fn, h, phys_l):
    ...
    print(f"\n{'='*70}")
    print(f"  3+1D CLOSURE CARD (h=0.5): {kernel_name}")
    ...
    print(f"  max_d={lat.max_d} (lattice steps), hw={lat.hw}")
    print(f"{'='*70}")

    # Flat propagation
    print("  [Flat propagation]")
    ...
    results["k0"] = gk0
    print(f"  2. k=0 = {gk0:.6f}  [{'PASS' if abs(gk0) < 1e-6 else 'CHECK'}]  ({time.time()-t0:.0f}s)")

    # 3. Gravity sign at z=2
    print("  [3. Gravity sign]")
    t0 = time.time()
    ...
    results["grav"] = grav
    results["grav_dir"] = dr
    print(f"  3. Gravity z=2 = {grav:+.6f} ({dr})  [{'PASS' if grav > 0 else 'FAIL'}]  ({time.time()-t0:.0f}s)")
    ...
    if len(m_data) >= 3:
        ...
        if slope is not None:
            fm_alpha = slope
            print(f"      fit: alpha={slope:.3f}, R^2={r2:.4f}")
    ...
    results["dtv"] = dtv
    print(f"  5. d_TV = {dtv:.4f}  [{'PASS' if dtv > 0.1 else 'WEAK'}]  ({time.time()-t0:.0f}s)")
    ...
    print(f"  7. TOWARD: {n_tw}/3  ({time.time()-t0:.0f}s)")
...
def main():
    ...
    print(f"\n{'='*70}")
    print("SIDE-BY-SIDE: 3+1D h=0.5 vs h=1.0 expectations")
    print(f"{'='*70}")
    print()
    ...
    # Verdict
    print(f"\n{'='*70}")
    print("VERDICT: h=0.5 FINER LATTICE")
    print(f"{'='*70}")
