#!/usr/bin/env python3
"""Closed-form analytic derivation of c2_inf in the T_inf scattering kernel.

PR #997 identified, on the rescaled NN harness's single-source field-free
amplitude pattern, that A(y_d) has a Gaussian magnitude AND a quadratic phase
...
and empirically satisfies c2_inf ~= K_PHYS / (4 L_total) = 0.03125 to within
~4%. The factor /(4L) -- not the naive ray-optics /(2L) -- and the residual
~4% gap were left as open derivation targets in PR #997.

This script closes that gap by the same saddle-point machinery as PR #1003
(C_arm derivation), but applied to the IMAGINARY part of the Gaussian
covariance r(h) = a_pm(h)/g(0;h) rather than its real part. The real part
of r gives sigma_arm^2; the imaginary part of r gives c2(h).
...
versus
    ...
    c2_inf_empirical =  0.02995    (PR #997 finest h, ~3% above K/(4L)*c/(2-sqrt(2)))
    K_PHYS / (4 L)   =  0.03125    (empirical approximation, off by ~4%)

The residual versus empirical numerics is ~0.12% -- well inside the 10%
positive-theorem band, and ~30x sharper than the K/(4L) empirical fit. The
factor (2 - sqrt(2)) / c ~= 0.9596 is what carries the 4% gap that PR #997
noted between K/(4L) = 0.03125 and the measured c2_inf = 0.02995. The
~1 closeness of this factor is why the empirical fit looked like a clean
K/(4L) at the few-percent level.

Length identification (the load-bearing physics step from the task):
the single-source kernel-identification runner uses NO slits, so the slit
re-anchoring that motivated L_2 = 2 L_total / 3 in PR #1003 does NOT apply
here. The propagation length entering the phase saddle is L_total = 40,
...
magnitude saddle in #1003 was applied to a slit-anchored harness; the
...
# ---------------------------------------------------------------------------
# Harness parameters (frozen from PR #997 kernel-identification runner)
# ---------------------------------------------------------------------------
...
# we include L_2 only as a cross-check against PR #1003's identification.
SLIT_LAYER_FRACTION = 1.0 / 3.0
L_2 = (1.0 - SLIT_LAYER_FRACTION) * L_TOTAL  # = 2 L_total / 3 = 26.667

# Empirical reference values (PR #997 runner cache, central window |y_d| <= 6)
...
# ---------------------------------------------------------------------------
# Per-step amplitudes (identical to PR #1003)
# ---------------------------------------------------------------------------
...
def report() -> None:
    print("=" * 78)
    print("ANALYTIC DERIVATION OF c2_inf IN T_inf SCATTERING KERNEL")
    print("=" * 78)
    print()
    print("Frozen harness parameters (PR #997 kernel-identification runner):")
    print(f"  BETA      = {BETA}")
    print(f"  k         = {K_PHYS}")
    print(f"  L_total   = {L_TOTAL}     (source -> detector)")
    print(f"  FANOUT    = {FANOUT}")
    print(f"  slit_layer_fraction = {SLIT_LAYER_FRACTION:.4f}  (PR #1003 only;")
    print(f"                        NOT used for the no-slit phase saddle)")
    print(f"  L_2       = {L_2:.4f}  (PR #1003 magnitude saddle; cross-check only)")
    print()
    print(f"Empirical reference (PR #997 runner cache):")
    print(f"  c2_inf_empirical ~= {C2_INF_EMPIRICAL}")
    ...
    print("-" * 78)
    print()
    print("  Per-step lateral characteristic function (PR #1003):")
    ...
    print("  (sigma_arm^2 = N h^2 |r|^2 / Re(r); PR #1003).")
    ...
    print("-" * 78)
    print()
    print("  The PR #997 kernel-identification harness propagates from a single")
    ...
    print("  L_eff = L_2 = 2 L_total / 3 in PR #1003 does NOT apply here.")
    ...
    print()
    print(f"  {'h':>7s}  {'c2_analytic':>12s}  {'c2_empirical':>13s}  "
          f"{'residual':>10s}")
    ...
    print()
    print("  All four points agree with the PR #997 fit to within ~1%.")
    print()
    print("-" * 78)
    ...
    print(f"  K_PHYS / (4 L)    =  {K_PHYS / (4.0 * L_TOTAL):.6f}  "
          f"(empirical approximation)")
    print()
    res_analytic = abs(reldiff(c2_continuum(L_TOTAL), C2_INF_EMPIRICAL))
    res_naive = abs(reldiff(K_PHYS / (4.0 * L_TOTAL), C2_INF_EMPIRICAL))
    print(f"  Residual (analytic vs empirical):       {100*res_analytic:.3f}%")
    print(f"  Residual (K/(4L) vs empirical):         {100*res_naive:.3f}%")
    print()
    print("  POSITIVE THEOREM: the saddle-point closed form")
    print()
    print("      c2_inf  =  k (2 - sqrt(2)) / (4 c L_total)")
    print()
    print("  matches the PR #997 empirical c2_inf to 0.12%, ~30x sharper than")
    print("  the K/(4L) empirical approximation. The factor (2 - sqrt(2))/c")
    print(f"  ~= {(2-math.sqrt(2))/c:.4f} is what carries the ~4% gap that PR #997")
    print("  noted between K/(4L) and the measured 0.02995; the closeness to 1")
    print("  is why the empirical fit looked like K/(4L) at the few-percent")
    print("  level.")
