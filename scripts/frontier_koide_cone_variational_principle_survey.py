...
STATUS: structural symbolic attack on Charged-lepton Koide-cone derivation via the
"retained variational / stationarity principle" variational-principle lane. Follows the
14-agent charged-lepton attack surface (see
...
--------------------
VARIATIONAL_PRINCIPLE_CLOSES_CL_HIERARCHY              - retained principle forces BOTH Q=2/3
                                  AND specific cone position matching
                                  observation. Flagship TOE.
VARIATIONAL_PRINCIPLE_CONE_ONLY              - retained principle forces Q=2/3 but
                                  leaves the cone point free.
VARIATIONAL_PRINCIPLE_STATIONARY_NOT_KOIDE   - candidate principles have stationary
                                  points NOT on the Koide cone.
VARIATIONAL_PRINCIPLE_NO_RETAINED
...
def residual_ratio_check():
    ...
    print("          POINT free. Full charged-lepton closure requires BOTH the cone AND")
...
def summarize_verdict(results: List[dict]) -> str:
    ...
    if any_retained_and_cone_and_point:
        verdict = "VARIATIONAL_PRINCIPLE_CLOSES_CL_HIERARCHY"
    elif any_retained_and_cone:
        verdict = "VARIATIONAL_PRINCIPLE_CONE_ONLY"
    elif any_cone:
        # A candidate forces cone, but none of the retained ones do
        # (cone-forcers are AD-HOC).
        verdict = "VARIATIONAL_PRINCIPLE_NO_RETAINED"
    else:
        # No stationary point on the Koide cone from any candidate.
        verdict = "VARIATIONAL_PRINCIPLE_STATIONARY_NOT_KOIDE"

    print()
    print(f"  VARIATIONAL_PRINCIPLE_VERDICT = {verdict}")
    print()
