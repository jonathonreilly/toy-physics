...
Review-hygiene check, not a physics proof. Verifies that the synthesis:
  ...
  - cross-references the four open-PR source-note proposals (#874-#877);
  ...
  - cross-references retained foundational notes that exist on main.
"""
...
def main() -> int:
    ...
    # ---- four open-PR cross-references (this round) ----
    round_prs = [
    ...
    check(
        "no axiom added stated",
        "Add a new framework axiom" in note or "no new axiom" in note.lower()
        or "No new axiom" in note,
    )
    check(
    ...
    # ---- cross-references to retained foundational notes that exist on main ----
    foundational_refs = [
    ...
    check(
        "synthesis honors the two-axiom surface (no new axiom)",
        ("A1+A2" in note or "A1 + A2" in note)
        and ("two-axiom" in note.lower() or "two framework axioms" in note.lower()
             or "axioms (A1 + A2)" in note),
    )
