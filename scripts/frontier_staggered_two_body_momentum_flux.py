#!/usr/bin/env python3
"""
Staggered two-body momentum-flux probe on the open 3D cubic family.

This is the next credible observable for the staggered self-consistent
two-body lane after centroid variants, detector-side transfer, and the
mid-plane link-current readout all failed to close the trajectory channel.

The readout is packet-local rather than mid-plane global:

  - build a small fixed control volume around each packet center
  - measure the partner-facing bond current across the outer face of that
    control volume
  - compare SHARED vs SELF_ONLY
  - integrate the early-time signal into an impulse proxy

This stays on the same audited open-cubic staggered surface used by the prior
staggered two-body notes, but it is distinct from the failed centroid and
link-current variants because the cut is local to each packet edge.
"""
...
class OpenStaggered3D:
    ...
    def bond_current_x(self, psi: np.ndarray, x_left: int, y: int, z: int) -> float:
        if x_left < 0 or x_left >= self.side - 1:
            return 0.0
        i = self.index(x_left, y, z)
        j = self.index(x_left + 1, y, z)
        hij = -0.5j
        return float(-2.0 * np.imag(np.conj(psi[i]) * hij * psi[j]))

    def partner_face_flux(self, psi: np.ndarray, center: tuple[float, float, float], *, side: str) -> float:
        x0 = int(math.floor(center[0] + 0.5))
        y0 = int(math.floor(center[1] + 0.5))
        z0 = int(math.floor(center[2] + 0.5))

        x_lo = max(0, x0 - WINDOW_RADIUS)
        x_hi = min(self.side - 1, x0 + WINDOW_RADIUS)
        y_lo = max(0, y0 - WINDOW_RADIUS)
        y_hi = min(self.side - 1, y0 + WINDOW_RADIUS)
        z_lo = max(0, z0 - WINDOW_RADIUS)
        z_hi = min(self.side - 1, z0 + WINDOW_RADIUS)

        total = 0.0
        if side == "left":
            # Positive is toward +x, i.e. toward the partner packet on the right.
            face_x = min(self.side - 2, x_hi)
            for y in range(y_lo, y_hi + 1):
                for z in range(z_lo, z_hi + 1):
                    total += self.bond_current_x(psi, face_x, y, z)
            return float(total)

        if side == "right":
            # Positive is toward -x, i.e. toward the partner packet on the left.
            face_x = max(0, x_lo - 1)
            for y in range(y_lo, y_hi + 1):
                for z in range(z_lo, z_hi + 1):
                    total -= self.bond_current_x(psi, face_x, y, z)
...
def run_case(side: int, distance: int, placement: str) -> dict[str, float]:
    ...
    flux_a_hist: list[float] = []
    flux_b_hist: list[float] = []
    ...
    for _ in range(N_STEPS):
        ...
        flux_a_hist.append(
            lat.partner_face_flux(psi_a_shared, packet_a_center, side="left")
            - lat.partner_face_flux(psi_a_self, packet_a_center, side="left")
        )
        flux_b_hist.append(
            lat.partner_face_flux(psi_b_shared, packet_b_center, side="right")
            - lat.partner_face_flux(psi_b_self, packet_b_center, side="right")
        )
    ...
    early = slice(0, min(6, len(flux_a_hist)))
    a_fit = np.asarray(flux_a_hist[early])
    b_fit = np.asarray(flux_b_hist[early])
    ...
    flux_sym_mean = 0.5 * (float(np.mean(a_fit)) + float(np.mean(b_fit)))
    impulse_sym = DT * float(np.sum(a_fit + b_fit))
    ...
    return {
        ...
        "flux_a_mean": float(np.mean(a_fit)),
        "flux_b_mean": float(np.mean(b_fit)),
        "flux_sym_mean": flux_sym_mean,
        "impulse_sym": impulse_sym,
        "inward": float(inward),
        "a_slope": a_slope,
        "b_slope": b_slope,
        "a_r2": r2_a,
...
def summarize(rows: list[dict[str, float]]) -> None:
    ...
    flux_slope, flux_r2 = power_law_fit([row for row in rows if row["inward"] > 0.0], "flux_sym_mean")
    imp_slope, imp_r2 = power_law_fit([row for row in rows if row["inward"] > 0.0], "impulse_sym")
    ...
    print(f"partner-facing flux inward: {inward_rows}/{total}")
    print(f"flux fit (inward rows only): <J_face> ~ d^{flux_slope:+.3f}, R^2={flux_r2:.4f}")
    print(f"impulse fit (inward rows only): <I_face> ~ d^{imp_slope:+.3f}, R^2={imp_r2:.4f}")

    for placement in PLACEMENTS:
        subset = [row for row in rows if row["placement"] == placement]
        count = sum(int(row["inward"]) for row in subset)
        slope_p, r2_p = power_law_fit([row for row in subset if row["inward"] > 0.0], "flux_sym_mean")
        print(
            f"{placement:>13s}: inward {count}/{len(subset)}, "
            f"flux-fit={slope_p:+.3f} (R^2={r2_p:.4f})"
        )
...
def main() -> None:
    t0 = time.time()
    print("=" * 92)
    print("STAGGERED TWO-BODY MOMENTUM-FLUX PROBE")
    print("=" * 92)
    print(
        f"MASS={MASS}, G={G}, MU2={MU2}, DT={DT}, N_STEPS={N_STEPS}, "
        f"SIGMA={SIGMA}, WINDOW_RADIUS={WINDOW_RADIUS}"
    )
    print(f"sides={SIDES}, distances={DISTANCES}, placements={PLACEMENTS}")
    print("Readout: packet-local partner-facing control-volume flux vs self-only")
    print()
        ...
        for side in SIDES:
            for distance in DISTANCES:
                ...
                print(
                    ...
                    f"flux=({row['flux_a_mean']:+.3e},{row['flux_b_mean']:+.3e}) "
                    f"sym={row['flux_sym_mean']:+.3e} "
                    f"imp={row['impulse_sym']:+.3e}"
                )
        print()
