...
The numerical check spans:
  ...
  - additionally: failure under similarity D' = S D S^{-1} (counter-check)
...
# ---------------------------------------------------------------------------
def part8_failure_under_similarity():
    section("Part 8: failure under similarity (D' = S D S^{-1}); identity is congruence-specific")
...
def part10_boundary_check():
    ...
    not_claimed = [
        "claim the parent note's bookkeeping consequence",
        "import or otherwise depend on the parent's four admitted-context",
        "derive (BA-1) from the framework axioms",
...
def main() -> int:
    ...
    part8_failure_under_similarity()
