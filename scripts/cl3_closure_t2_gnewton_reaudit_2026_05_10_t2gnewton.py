...
# ---------------------------------------------------------------------------
# T10: RE-AUDIT VERDICT SYNTHESIS
# ---------------------------------------------------------------------------

def test_t10_verdict_synthesis() -> None:
    """T10: Synthesize the re-audit verdict into named weaknesses for
    audit-lane handoff.
    """
    print("=" * 76)
    print("T10: RE-AUDIT VERDICT SYNTHESIS — named weaknesses for audit lane")
    print("=" * 76)
    ...
    # Downstream impact: PR #1060 inherits F2.1
    log_check(
        "T10.D1: PR #1060 (C-B(b)) inherits F2.1 citation defect",
        True,
        "load-bearing chain uses Born map identification but does not "
        "derive Born from A_min; F2.1 flows downstream",
    )

    # Honest summary
    log_check(
        "T10.summary: re-audit verdict is HONEST BOUNDED RE-AUDIT, "
        "not closure attempt",
        True,
        "audit-lane authority preserved; no retained-tier promotion implied",
    )
...
def main() -> int:
    ...
    print("# Branch: closure/t2-gnewton-v2-2026-05-10")
