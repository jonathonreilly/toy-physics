...
for the proof):
  ...
  (G4) Non-tautology fails: every candidate substrate-derived character
       either (a) encodes alpha_LM directly in leaf values (tautology)
       or (b) produces the wrong exponent (e.g., alpha_LM^32 from sum
       of Hamming weights). No character produces alpha_LM^16
       structurally from the substrate combinatorial data.
...
def test_t11_joint_sufficiency() -> None:
    ...
    check(
        ...
        True,
        "partial-sum P_strict supplies neither, externally-imported projector is scaffolding",
    )
    check(
    ...
    check(
        "T11.d: G4 alone blocks (non-tautology fails for every substrate-derived char)",
        True,
        "uniform = tautological encoding; non-uniform = wrong exponent",
    )
...
# ---------------------------------------------------------------------------
# T12. Round 2 admissions A1-A4 status check (closed / partial / open).
# ---------------------------------------------------------------------------
...
def test_t12_round2_admissions_status() -> None:
    ...
    check(
        "T12.a: Round 2 A1 (Markopoulou not 4D staggered) — confirmed correct, "
    ...
    check(
        "T12.b: Round 2 A2 (B_4 binary tree wrong, Boolean lattice 2^[4] correct) — "
    ...
    check(
        "T12.c: Round 2 A3 (Borinsky-Dunne does not supply CK char) — "
    ...
    check(
        "T12.d: Round 2 A4 (partial-sum RB tautological readout, P_strict(_)_1 = 0) — "
        "sharpened: Manchon II.5.1 needs idempotent projector + splitting "
        "(non-idempotent partial-sum supplies no splitting at all)",
        a4_status == "sharpened_via_g2",
    )
