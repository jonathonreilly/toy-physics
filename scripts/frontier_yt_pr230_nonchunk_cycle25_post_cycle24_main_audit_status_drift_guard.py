#!/usr/bin/env python3
"""
PR #230 non-chunk cycle-25 post-cycle-24 main-audit-status-drift reopen guard.

Cycle 24 closed origin/main status drift through eaa2130fc.  This guard checks
the only new resume fact available at cycle 25: after a fresh fetch,
origin/main advanced again to 0fbd8ecd4, but the new diff still contains only
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_nonchunk_cycle25_post_cycle24_main_audit_status_drift_guard_2026-05-05.json"
)
LOOP_PACK = (
    ROOT
    / ".claude"
...
CYCLE24_HEAD = "5ddef0e16ddd3de4756de81d99bf12f308242072"
CYCLE24_REMOTE_MAIN_HEAD = "eaa2130fc4dd5c8f304b66f6de2eebae90741e71"
...
def branch_state(future_paths: list[str]) -> dict[str, Any]:
    ...
    main_changed_paths = diff_paths(CYCLE24_REMOTE_MAIN_HEAD, REMOTE_MAIN)
    return {
        "cycle24_head": CYCLE24_HEAD,
        "cycle24_remote_main_head": CYCLE24_REMOTE_MAIN_HEAD,
        "local_head": local_head,
        "remote_pr_ref": REMOTE_PR,
        "remote_pr_head": remote_pr_head,
        "remote_main_ref": REMOTE_MAIN,
        "remote_main_head": remote_main_head,
        "cycle24_is_ancestor_of_local_head": git_ok(
            ["merge-base", "--is-ancestor", CYCLE24_HEAD, "HEAD"]
        ),
        "cycle24_is_ancestor_of_remote_pr": git_ok(
            ["merge-base", "--is-ancestor", CYCLE24_HEAD, REMOTE_PR]
        ),
        "cycle24_remote_main_is_ancestor_of_remote_main": git_ok(
            ["merge-base", "--is-ancestor", CYCLE24_REMOTE_MAIN_HEAD, REMOTE_MAIN]
        ),
        "local_head_matches_remote_pr_head": bool(local_head and local_head == remote_pr_head),
        "local_commits_after_cycle24": git(["rev-list", "--count", f"{CYCLE24_HEAD}..HEAD"]),
        "remote_commits_after_cycle24": git(["rev-list", "--count", f"{CYCLE24_HEAD}..{REMOTE_PR}"]),
        "remote_main_commits_after_cycle24_baseline": git(
            ["rev-list", "--count", f"{CYCLE24_REMOTE_MAIN_HEAD}..{REMOTE_MAIN}"]
        ),
        "remote_main_changed_paths_after_cycle24_baseline": main_changed_paths,
        "remote_main_changed_path_classes_after_cycle24_baseline": {
        ...
        "local_changed_future_paths_after_cycle24": diff_future_paths(
            CYCLE24_HEAD, "HEAD", future_paths
        ),
        "remote_pr_changed_future_paths_after_cycle24": diff_future_paths(
            CYCLE24_HEAD, REMOTE_PR, future_paths
        ),
        "remote_main_changed_future_paths_after_cycle24_baseline": diff_future_paths(
            CYCLE24_REMOTE_MAIN_HEAD, REMOTE_MAIN, future_paths
        ),
    }
...
def main() -> int:
    print("PR #230 non-chunk cycle-25 post-cycle-24 main-audit-status-drift reopen guard")
    print("=" * 72)
    ...
    no_changed_future_path_since_cycle24 = not any(
        [
            branch["local_changed_future_paths_after_cycle24"],
            branch["remote_pr_changed_future_paths_after_cycle24"],
            branch["remote_main_changed_future_paths_after_cycle24_baseline"],
        ]
    )
    ...
    branch_current = (
        branch["cycle24_is_ancestor_of_local_head"]
        and branch["cycle24_is_ancestor_of_remote_pr"]
        and branch["cycle24_remote_main_is_ancestor_of_remote_main"]
        and branch["local_head_matches_remote_pr_head"]
        and bool(branch["remote_main_head"])
    )
    loop_pack_records_stop = (
        "cycle-24 update" in compact_text(OPPORTUNITY_QUEUE)
        and "post-cycle-23 main-status-drift reopen guard" in compact_text(HANDOFF)
        and "post-cycle-23 main status drift is not admissible reopen evidence"
        in compact_text(NO_GO_LEDGER)
        and "proposal_allowed: false" in compact_text(CLAIM_STATUS_CERTIFICATE)
    )
    main_drift_since_cycle24_is_status_only = (
        branch["remote_main_commits_after_cycle24_baseline"] != "0"
        and branch["remote_main_changed_paths_are_status_drift"]
        and not branch["remote_main_changed_future_paths_after_cycle24_baseline"]
    )
    no_reopen_input = (
        source_presence_absent
        and no_local_or_remote_future_path
        and no_changed_future_path_since_cycle24
        and main_drift_since_cycle24_is_status_only
    )
    cycle25_post_cycle24_main_audit_status_drift_guard_passed = all(
        [
            not missing_parents,
            not parent_failures,
    ...
    dramatic_step_gate = {
        ...
        "branch_state": branch,
        "reason": (
            "Cycle 24 already rejects post-cycle-23 origin/main status drift as "
            ...
            "listed PR230 same-surface artifacts, so cycle 25 has no admissible "
    ...
    report("branch-has-cycle24-ancestor-and-remote-main-visible", branch_current, json.dumps(branch, sort_keys=True))
    ...
    report("parent-process-stack-closed-through-cycle24", parent_process_stack_closed, "cycle-18 through cycle-24 gates deny continuation")
    report("aggregate-gates-deny-proposal", aggregate_gates_deny, f"assembly={statuses['assembly']} campaign={statuses['campaign']}")
    report("loop-pack-records-cycle24-stop-contract", loop_pack_records_stop, "cycle-24 stop text present")
    report("main-drift-since-cycle24-is-status-only", main_drift_since_cycle24_is_status_only, json.dumps(branch, sort_keys=True))
    report("no-local-or-remote-listed-future-path", no_local_or_remote_future_path, json.dumps(branch, sort_keys=True))
    report("no-changed-listed-future-path-since-cycle24", no_changed_future_path_since_cycle24, json.dumps(branch, sort_keys=True))
    report("no-source-presence-key-set", source_presence_absent, json.dumps(present_by_source, sort_keys=True))
    report(
        "cycle25-post-cycle24-main-audit-status-drift-guard-passed",
        cycle25_post_cycle24_main_audit_status_drift_guard_passed,
        f"passed={cycle25_post_cycle24_main_audit_status_drift_guard_passed}",
    )

    result = {
        "actual_current_surface_status": (
            "exact negative boundary / PR230 non-chunk cycle-25 "
            "post-cycle-24 main-audit-status-drift reopen guard: origin/main "
        ...
        "proposal_allowed_reason": (
            "Cycle 24 already rejects post-cycle-23 origin/main status drift.  The "
            "cycle-24 head remains an ancestor of the aligned PR heads, "
