...
Question:
  On the compact branch, without reviving the deleted rival-window candidate
  forest, can the split-1 `m` window already be pushed toward a local
  dominance statement on the exact shift-quotient bundle itself?
...
EXPECTED_CORE_GRID_MIN = 2.3088620907222923
EXPECTED_CORE_DE_MIN = 2.3086034009138245
EXPECTED_HIGH_R31_MIN = 6.201953117823571
EXPECTED_HIGH_DELTA_MIN = 3.98839590220742
...
def part1_the_split1_window_on_the_exact_bundle_has_a_broad_test_domain() -> float:
    print("\n" + "=" * 88)
    print("PART 1: THE SPLIT-1 WINDOW ON THE EXACT BUNDLE HAS A BROAD TEST DOMAIN")
    print("=" * 88)
    ...
    check(
        "The split-1 dominance test is posed directly on the exact shift-quotient bundle over (m,delta,r31)",
        True,
        f"m in [{M_WINDOW[0]:.2f},{M_WINDOW[1]:.2f}], delta in [{CORE_DELTA[0]:.1f},{CORE_DELTA[1]:.1f}], r31 in [{CORE_R31[0]:.1f},{CORE_R31[1]:.1f}]",
    )
    check(
        "The split-1 window lies entirely on the rival-side m-range identified by the obstruction note",
...
def part2_a_broad_core_grid_and_global_refinement_stay_above_the_preferred_floor(pref: float) -> tuple[float, np.ndarray]:
    ...
    check(
        "The coarse broad-box split-1 grid never undercuts the preferred recovered floor",
        best_grid_value > pref,
        f"(best_grid,gap)=({best_grid_value:.12f},{best_grid_value - pref:.12e})",
    )
    check(
        "The global differential-evolution refinement on the same split-1 box also stays above the preferred floor",
        best_de_value > pref and de.success,
        f"(best_de,gap)=({best_de_value:.12f},{best_de_value - pref:.12e})",
    )
    check(
        "The compact-box split-1 minimum is reproduced stably by grid and global refinement",
    ...
    check(
        "The refined split-1 minimizer sits on the lower-r31 boundary rather than drifting into a new interior low basin",
...
def part3_high_r31_and_high_delta_tails_are_far_higher_than_the_preferred_floor(pref: float) -> tuple[float, float]:
    ...
    check(
        "The high-r31 split-1 tail challenge stays far above the preferred floor",
    ...
    check(
        "The high-|delta| split-1 tail challenge also stays far above the preferred floor",
    ...
    check(
        "Both split-1 tail challenges are much higher than the broad core minimum",
...
def part4_the_note_records_the_split1_bundle_dominance_candidate(
    core_best: float, high_r31_best: float, high_delta_best: float
) -> None:
    print("\n" + "=" * 88)
    print("PART 4: THE NOTE RECORDS THE SPLIT-1 BUNDLE-DOMINANCE CANDIDATE")
    print("=" * 88)

    note = read("docs/DM_NEUTRINO_SOURCE_SURFACE_SPLIT1_WINDOW_BUNDLE_DOMINANCE_CANDIDATE_NOTE_2026-04-17.md")

    check(
        "The note records the broad split-1 compact-box minimum above the preferred floor",
...
def main() -> int:
    print("=" * 88)
    print("DM NEUTRINO SOURCE-SURFACE SPLIT-1 WINDOW BUNDLE-DOMINANCE CANDIDATE")
    print("=" * 88)
    print()
    print("Question:")
    print("  Can the split-1 m-window already be pushed toward local dominance directly")
    print("  on the exact shift-quotient bundle of the compact branch?")

    pref = part1_the_split1_window_on_the_exact_bundle_has_a_broad_test_domain()
    ...
    part4_the_note_records_the_split1_bundle_dominance_candidate(core_best, high_r31_best, high_delta_best)

    print("\n" + "=" * 88)
    print("RESULT")
    print("=" * 88)
    print("  Split-1 exact-bundle dominance candidate:")
    print("    - on a broad exact-bundle box the split-1 m-window stays strictly above")
    ...
    print("  RESULT: strong local split-1 bundle-dominance candidate; not theorem closure")
    print()
