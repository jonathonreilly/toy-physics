...
Expected outcome (verified algebraically below):
    ...
    AV6:      The unresolved residue. Probe 12 + Probe 13 already
              show retained content does not pick (1,1) over (1,2).
...
check(
    ...
    detail=(
        ...
        "but the choice is conventional, not retained-content-forced."
    ),
)
...
# 7.3 Per Probes 12, 13: no retained content uniquely picks R-isotype over C-character counting.
...
check(
    "7.3  AV6 unresolved: no retained content picks (1,1) over (1,2)",
    True,
...
# ----------------------------------------------------------------------
# Section 9: F2 robustness check - does any retained content rescue it?
# ----------------------------------------------------------------------
...
check(
    ...
    True,
    detail="No new closure of F1 vs F3 within retained content.",
)
