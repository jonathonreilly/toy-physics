...
Total expected check count: 6 (one per the structural statements in
the note's §10 "Verification" block).
"""
...
def _assert_eq_int(a: int, b: int, label: str) -> bool:
    ok = a == b
    _line(label, ok)
    return ok
...
def main() -> int:
    ...
    if _line("N = 16 within 1 % of v_obs", n16_ok) or n16_ok:
        if n16_ok:
            passes += 1
        else:
            fails += 1
