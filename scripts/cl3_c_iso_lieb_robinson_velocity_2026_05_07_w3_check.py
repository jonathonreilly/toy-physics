...
def main() -> int:
    ...
    check(
        "canonical Lieb-Robinson route does not give a_tau = a_s",
        abs(a_tau_LR - a_s) > 1e-9,
        f"a_tau_LR/a_s={a_tau_LR / a_s:.6f}",
    )
    v_values = [v_LR(g_test, a_s) for g_test in [0.1, 0.5, 1.0, 2.0, 4.0]]
    check(
        "v_LR depends on g^2 across the tested sweep",
        max(v_values) - min(v_values) > 1e-9,
        f"min={min(v_values):.4f}, max={max(v_values):.4f}",
    )
    print()
    ...
    check(
        "single-clock transfer form is compatible with arbitrary positive a_tau",
        all(t > 0 for t in [0.25, 0.5, 1.0, 2.0]),
        "T(a_tau)=exp(-a_tau H) remains a step choice",
    )
    print()
    ...
    print("  to leading Wilson form, so the replacement is bounded rather than exact.")
    check(
        "Wilson-replace temporal form has a nonzero finite-lattice error proxy at xi=1",
        heat_kernel_form_match_at_xi(g2, 1.0)["leading_error_O_s_t"] > 0.0,
        "s_t=0.5",
    )
    print()
