...
functional consumed, no static-conformal bridge consumed, no PDG /
literature / fitted / admitted imports.
...
parent = ledger["rows"].get("discrete_einstein_regge_lift_note", {})
...
prev = parent.get("previous_audits") or []
if prev:
    last = prev[-1]
    print(f"    last audit_status: {last.get('audit_status')}")

check(
    "parent row exists in ledger",
    bool(parent),
    "parent: discrete_einstein_regge_lift_note",
)
check(
    "parent row claim_type is bounded_theorem (carve-out applies)",
    parent.get("claim_type") in ("bounded_theorem", "positive_theorem"),
    f"claim_type = {parent.get('claim_type')}",
)
...
print("""
  ...
  Audit-lane class:
    ...
    observed/fitted/literature/PDG/Schur/strong-field input. Proof is
    direct dihedral-angle computation in each Coxeter tet plus cubic
    symmetry on Z^3.
