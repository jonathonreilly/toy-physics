...
def check_T8_teleportation_protocol_closure(factor: SiteFactor) -> RalaCheck:
    ...
    for psi_log in test_states_logical:
        ...
        # Pre-record marginal on B: full^dag @ full (treating full_mat with rows=AA_R, cols=B)
        rho_B_marginal = full_mat.conj().T @ full_mat
        ...
        for P, zbit, xbit in P_list:
            ...
            if prob < 1e-12:
                continue
            rho_B_branch = (after_mat.conj().T @ after_mat) / prob
