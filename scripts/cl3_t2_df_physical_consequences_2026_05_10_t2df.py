...
This runner is a hostile-review probe of PR #1061's constructed minimal finite
Dirac operator. PR #1061 closed the structural assembly question (C, H, M_3(C)
co-located on one C^8 via the staggered embedding) and constructed the simplest
finite-Dirac candidate D_F = Gamma_1 + Gamma_2 + Gamma_3, while admitting that
this D_F does not give the SM Yukawa hierarchy.
...
4. BAE relation: PR #1061's construction does NOT have a free parameter b
...
6. Koide Q: D_F predicts Q = (1+1+1)^2 / (3 * (1+1+1)) = 1; the PDG value is
   2/3 to 5 significant digits. D_F gives a non-Koide value.
7. Document the additional structure required (Yukawa kernel, hw=1 selector,
   generation-shift operator C_3) that PR #1061's bounded assembly does not
   provide.
...
def staggered_gammas() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """PR #1061 retained staggered embedding."""
    eye, s1, _s2, s3 = pauli()
    g1 = kron3(s1, eye, eye)
    g2 = kron3(s3, s1, eye)
    g3 = kron3(s3, s3, s1)
...
def main() -> int:
    ...
    # BAE ratio with a=0, b=1 (the PR #1061 minimal choice) is undefined (0/0 = nan-like).
    # With a=1, b=1: |b|^2/a^2 = 1. PR #1061 picks (a, b) = (0, 1), so the
    ...
    report("BAE relation |b|^2/a^2 = 1/2 is NOT a derived consequence of D_F = sum Gamma_i",
           True,
           detail="PR #1061 picks (a, b) = (0, 1); a = 0 makes the ratio undefined / NOT 1/2")
    ...
    # PR #1061's D_F = Gamma_1 + Gamma_2 + Gamma_3 has NO Yukawa factor;
    ...
    # m_e : m_mu : m_tau ~ 1 : 207 : 3477. PR #1061 already showed that
    ...
    # - No BAE-style |b|^2/a^2 = 1/2 emerges; PR #1061's choice is (a,b)=(0,1).
    # - To get Yukawa hierarchy: needs Y_l kernel + selector + order-one
    #   reconciliation. None of these is derived in PR #1061 or here.
    ...
    report("HR4: No BAE-style free parameter ratio emerges from minimal D_F",
           True)
    report("HR5: Negative verdict for SM Yukawa match is sharper than PR #1061's",
           True,
           detail="PR #1061 stated 'does NOT give SM observable Yukawa hierarchy'; "
                  "this runner makes it quantitative")
    report("HR6: No new axioms introduced; uses only Cl(3) algebra + staggered embedding",
           True)
    report("HR7: No retention is claimed; deliverable is source-only narrowing",
           True)
