#!/usr/bin/env python3
"""Run a mixed degree+reach leverage threshold sweep for the hub motif."""
...
from toy_event_physics import (
    ...
    neighbor_leverage_threshold_sets,
)


def main() -> None:
    started = datetime.now().isoformat(timespec="seconds")
    print(f"neighbor leverage threshold sweep started {started}", flush=True)
    total_start = time.time()
    for leverage_set in neighbor_leverage_threshold_sets():
        set_start = time.time()
        row = neighbor_leverage_threshold_benchmark(
            leverage_sets=(leverage_set,),
        )[0]
        print(
