...
def main() -> int:
    ...
    # Counterfactual on lambda^2: lambda^2 = alpha_s -> -2 (2/3) alpha_s^2 (5/6) = -(10/18) alpha_s^2 = -(5/9) alpha_s^2, not /18.
    coeff_cf_lam_alpha = simplify(
        (-Rational(2) * A_sq * lam ** 4 * (1 - rho)).subs(lam ** 2, alpha_s)
    )
    check(
        "counterfactual: at lambda^2 = alpha_s, (K3) coefficient = -(5 alpha_s^2/9) != -(5 alpha_s^2/18)",
