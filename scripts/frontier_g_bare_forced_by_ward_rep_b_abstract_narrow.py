...
gauge theory, no Wilson plaquette action, no physical Cl(3) local algebra
input, no Z^3 spatial substrate input, no SU(N_c) gauge group, no physical
bare-coupling identification, and no PDG / literature / fitted /
admitted-unit-convention import.

The narrow theorem applies in particular to the canonical numerical
...
# Pure polynomial algebra over R. No Ward / lattice / physical Cl(3) local
# algebra / Z^3 spatial substrate / SU(N_c) input.
# ============================================================================
...
positive_branch = [s for s in g_solutions if simplify(s - sqrt(2 * N * c0)) == 0]
...
# ----------------------------------------------------------------------------
section("Part 5: exact-rational sample grid of (N, c0) pairs")
# ----------------------------------------------------------------------------
counter_grid = [
...
# ----------------------------------------------------------------------------
section("Part 8: independence from any Ward / lattice / physical Cl(3) / Z^3 / SU(N_c) input")
# ----------------------------------------------------------------------------
...
forbidden = [
    "g_bare = 1 is uniquely forced by physical Cl(3)",
    "Wilson plaquette action is uniquely forced",
    "g_bare = 1 follows from physical Cl(3) local algebra plus Z^3 spatial substrate",
...
# ----------------------------------------------------------------------------
section("Part 10: ledger context when available (no ledger modification)")
# ----------------------------------------------------------------------------
...
print(f"\n  Parent row state in the current ledger (informational only):")
...
print("""
  ...
  Audit-lane class:
    ...
    literature/Ward/lattice/physical Cl(3) local algebra/Z^3 spatial
    substrate/SU(N_c) input. Proof is one-line
  ...
  framing. The numerical instance (N, c0) = (N_c, 1/6) = (3, 1/6) is just
