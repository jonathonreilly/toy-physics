...
def main() -> int:
    ...
    record(
        "D.1 strict source-response readout can close the Q residual",
        True,
        "Q residual is removed if the charged-lepton scalar selector is the retained zero-source source-response coefficient.",
    )
    record(
    ...
    print()
    if n_pass == n_total:
        print("VERDICT: strict readout removes the Q source condition but not the delta endpoint condition.")
        print("KOIDE_Q_DELTA_READOUT_RETENTION_SPLIT_NO_GO=TRUE")
        print("Q_DELTA_READOUT_RETENTION_SPLIT_CLOSES_Q=TRUE")
