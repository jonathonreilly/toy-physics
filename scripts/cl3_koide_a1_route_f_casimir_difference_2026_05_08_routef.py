...
Two structural barriers are verified numerically:

  Barrier 1 (Sector orthogonality): the generation-space C_3-circulant
  coefficients (a, b) live on the framework's hw=1 sector (a 3-dim
  space carrying the C_3[111] cycle). The SU(2)_L Casimir T(T+1) and
  the U(1)_Y squared-charge Y^2 are scalars associated with the
  *electroweak gauge representation* of the doublet. These are
  ORTHOGONAL sectors in the framework: the gauge-quantum-number
  scalars carry NO information about the C_3-circulant operator
  coefficient ratio. No retained theorem provides a structural map.
  ...
  Barrier 3 (Convention dependence): T(T+1) - Y^2 = 1/2 holds ONLY in
  the Q = T_3 + Y convention with Y_L = -1/2. In the SU(5)-style
  convention used by the retained CL3_SM_EMBEDDING_THEOREM (Y_L = -1),
  T(T+1) - Y^2 = 3/4 - 1 = -1/4, NOT 1/2. The Casimir-difference
  identity is convention-dependent, hence NOT a structural invariant.

  Barrier 4 (Generation-space vs gauge-space): T(T+1) acts on the
  doublet (lepton ν_L, e_L), 2-dim. The hw=1 generation triple is
  3-dim. The Casimir scalar acts trivially on generation indices.
  No retained theorem links the gauge-Casimir to the generation-space
  flavor-matrix coefficient ratio.
...
def section4_sector_orthogonality():
    ...
    results.append(passfail(
        "No retained theorem provides Casimir-to-circulant bridge",
        True,  # documented assertion (verified by absence in retained atlas)
        "doublet (2-dim, T,Y) and hw=1 (3-dim, C_3) are orthogonal "
        "sectors; no retained theorem links scalar T(T+1) to "
        "operator-coefficient ratio |b|^2/a^2",
    ))
...
def section5_category_mismatch():
    ...
    rhs = 0.5  # PDG conv
    rhs_invariant_under_state = True  # group-theoretic scalar
    results.append(passfail(
    ...
    results.append(passfail(
        "Route F lacks a retained normalization principle for (a, b)",
        True,  # absence is the verified content
        "No retained theorem supplies the gauge-to-flavor normalization map",
    ))
...
def section8_obstruction_theorem():
    ...
    barrier1_blocks = True  # convention dependence (Section 2)
    barrier2_blocks = True  # free Yukawa (Section 3)
    barrier3_blocks = True  # sector orthogonality (Section 4)
    barrier4_blocks = True  # category mismatch (Section 5)
