...
def homogenized_one_step_covariance(
    ...
    across vertices and the homogenization is trivially exact. For
    random-edge perturbations, Sigma_step(v) fluctuates with the local
    edge configuration; the average converges to the homogenized
    Sigma_eff that governs the almost-sure long-distance Green's function
    asymptotic (Andres-Deuschel-Slowik 2016 in the conductance-model
    setting).
    """
    d = len(shape)
    n = int(np.prod(shape))
...
def run_periodic_prefactor_family(
    ...
    Used specifically for the Theorem B prefactor check. Periodic BC kills
    ...
    # Use the homogenized covariance (averaged over many vertices) so
    # random-edge perturbations get the correct effective Sigma_eff
    # rather than the per-source-vertex quenched value.
...
def main() -> int:
    ...
    print("  trivially. For random-edge perturbations, the homogenization")
    print("  of Andres-Deuschel-Slowik (2016) gives the almost-sure")
    print("  asymptotic G_omega(x,0) ~ kappa_eff |x|^{2-d} with the same")
    print("  formula evaluated at Sigma_eff.")
    print()
    ...
    for fr in pbc_families:
        ...
        results.append(
            CheckResult(
                name=f"Theorem B prefactor (PBC, h_d fit) on '{fr.name}'",
                passed=passed,
                detail=(
    ...
    if all(prefactor_passes):
        ...
        print("  every periodic graph to within 4%, AND on the random-edge")
        print("  perturbation graphs via Andres-Deuschel-Slowik homogenization.")
