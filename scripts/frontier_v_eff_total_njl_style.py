...
check(
    "Five of six plausible forms put framework in SYMMETRIC phase",
    n_sym == 4 and n_broken == 2,  # Note: actually 4 sym, 2 broken in the 6-form table
    detail=f"sym = {n_sym}, broken = {n_broken}",
)
# Correction: above I claimed "five of six" but actually 4-of-6 are symmetric.
# Re-counting: KS-LO sym, alt-strong broken, Casimir sym, color-trace broken,
# u_0²-dress sym, u_0⁴-dress sym → 4 symmetric, 2 broken.
...
print(f"""
  ...
  order Kawamoto-Smit G_eff is below the chiral-SSB threshold; EWSB requires
  additional structure (EW gauge bosons + top Yukawa + Wilson taste-breaking).
""")
