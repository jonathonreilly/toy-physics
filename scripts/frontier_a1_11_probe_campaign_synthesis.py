...
that the synthesis note:
  ...
  - does not claim A1-condition closure or add/admit a new primitive;
...
# Probe 7 source-note filename.
PROBE7_NOTE = "KOIDE_A1_PROBE_Z2_C3_PAIRING_BOUNDED_OBSTRUCTION_NOTE_2026-05-08_probe7.md"
PROBE12_NOTE = "KOIDE_A1_PROBE_PLANCHEREL_PETER_WEYL_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe12.md"
PROBE13_NOTE = "KOIDE_A1_PROBE_REAL_STRUCTURE_BOUNDED_OBSTRUCTION_NOTE_2026-05-09_probe13.md"
...
def main() -> int:
    ...
    # ---- Same-batch targeted follow-up cross-references ----
    check(f"cross-references probe-7 note: {PROBE7_NOTE}", PROBE7_NOTE in note)
    check(f"cross-references probe-12 note: {PROBE12_NOTE}", PROBE12_NOTE in note)
    check(f"cross-references probe-13 note: {PROBE13_NOTE}", PROBE13_NOTE in note)
    ...
    check(
        "explicit: no primitive admission is added",
        "No admission is added by\nthis note" in note
        or "No admission is added by this note" in note
        or "No\n   mathematical axiom or admission is added here" in note,
    )
    check(
    ...
    # ---- converged-state language ----
    check(
        "campaign declared converged",
        "converged" in note and "generic-route" in note,
    )
    check(
    ...
    if FAIL:
        return 1
    print(
        "Eleven-probe campaign synthesis check passed: converged state recorded, "
