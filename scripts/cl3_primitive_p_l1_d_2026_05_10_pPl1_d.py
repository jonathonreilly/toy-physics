"""
P-L1-D Period-Functor Open Gate --- physical Cl(3) local algebra / Z^3-native period functor
on 1PI Feynman graphs for QCD beta_2/beta_3 channel weights.

Authority role
--------------
Source-note open_gate -- audit verdict and downstream status are set only
by the independent audit lane. No primitive proposed here is admitted, and
no repo-wide axiom or retained surface changes on the basis of this runner.
...
P-L1 Channel-Weight design note. That note left open an explicit
mathematical design question:

   "A function P_Cl(3) : 1PI Graph -> Q[zeta_n] reproducing TVZ/VVL values
    from the physical Cl(3) local algebra on the Z^3 spatial substrate
    alone, without dim-reg or Brown-Schnetz period theory. No such
    functor is currently known."
...
Result structure (open_gate expected)
============================================
Two sub-functors are constructed and tested:
  ...
  P_Cl(3)^Schnetz : Graph -> Z[q] / q^|E|
     ...
     produces finite-field arithmetic diagnostics for the period, but NOT
     the period value.
...
Positive findings (PASS expected):
  ...
  3. P_Cl(3)^Schnetz constructed; computed for explicit 1-loop, 2-loop,
     ...
     enumeration. The c_2-style diagnostic is trivial for 1L and 2L
     graphs and nontrivial for the 3-loop wheel_3 graph; the period-class
     interpretation is conjectural and not used as a coefficient oracle.

Negative findings (PASS expected, testing this construction route):
  4. P_Cl(3)^HK at 3-loop produces beta_2^HK = 32/81 from
     ...
     same missing scheme-conversion input named by the MSbar-native route context.
  5. P_Cl(3)^Schnetz c_2 invariant for primitive 3-loop wheel returns
     an arithmetic diagnostic but NOT the zeta_3 RATIONAL COEFFICIENT.
     The route can distinguish a class-style obstruction but not the
     full period value. This sharpens the gap to "rational coefficient
     extraction is missing, not finite-field point counting".

Open diagnostics (OPEN expected):
  6. Full P_L1-D with codomain Q[zeta_n] reproducing TVZ rationals
     {2857/54, ..., 1/2} from the physical Cl(3) local algebra on the Z^3 spatial substrate alone CANNOT
     be constructed via either route. The missing ingredient is the
...
# ======================================================================
# Existing scalar context (identical to P-L1, MSbar-native route context, V-L1-Quartic probes)
# ======================================================================
...
# ======================================================================
# Counter for PASS/FAIL/OPEN logging
# ======================================================================
...
class Counter:
    ...
    def record_open(self, label: str, reason: str) -> None:
        self.open_count += 1
        print(f"  [OPEN] {label} | {reason}")


# ======================================================================
# SECTION 1 -- Existing support: heat-kernel single-plaquette Taylor
...
def section1_existing_support(c: Counter) -> None:
    """Reproduce existing heat-kernel single-plaquette Taylor coefficients
    and beta_0, beta_1 universal values.  These are the substrate-side
    invariants that P_Cl(3)^HK must reproduce.
    """
    print()
    print("Section 1 -- Existing support")

    # <P>_HK Taylor coefficients (closed form derived from heat-kernel
    # single-plaquette per C_ISO_SU3_NLO context).
    expected = {
        1: Fraction(4, 3),
        2: -Fraction(8, 9),
        3: Fraction(32, 81),
    ...
    # beta_0 = 7 from S1 identification (existing companion theorem):
...
def section2_P_Cl3_HK_functor(c: Counter) -> None:
    ...
    # (DGS Caswell 1974; Jones 1974) and is existing context via S1.
    ...
    c.record_open(
        ...
        "exactly the missing primitive named in MSbar-native route context",
    )
...
def section3_P_Cl3_Schnetz_functor(c: Counter) -> None:
    ...
    of q) mod q.  Brown-Yeats denominator methods make c_2 useful as an
    arithmetic period diagnostic, but this runner does not use c_2 as an
    iff theorem or as a period-evaluation map.
    """
    print()
    ...
    # "6" in "6 zeta_3" -- only a class-level diagnostic.
    c.record(
    ...
    print("    -> P_Cl(3)^Schnetz: well-defined diagnostic; substrate-native;")
    print("       produces finite-field arithmetic data for the period problem.")
    print("    -> CANNOT recover RATIONAL COEFFICIENT in front of zeta_3, only")
    print("       a class-level diagnostic.  This is the structural obstruction.")

    c.record_open(
        "P_Cl(3)^Schnetz diagnostic: c_2-style data is class-level but "
        "not rational coefficient",
        "the finite-field diagnostic is well-defined; turning that "
        "diagnostic into a rational coefficient "
        "would require explicit master-integral evaluation -- "
        "the MSbar-native route context open gate",
    )


# ======================================================================
# SECTION 4 -- Composition: P_Cl(3) := P_Cl(3)^HK x P_Cl(3)^Schnetz
# -- check whether the COMBINED functor closes the L1 open gate
# ======================================================================
...
def section4_composition(c: Counter) -> None:
    ...
    = the full TVZ closed form.  Each of these is its own missing ingredient.

    The honest verdict is an open-gate diagnostic:
      - P_Cl(3)^HK exists and is substrate-native (positive).
      - P_Cl(3)^Schnetz exists and is substrate-native (positive).
      - Together they do NOT close the L1 open gate (negative on closure).
    ...
    # group theory, existing context), the channel weights are not the HK Taylor
    ...
    c.record_open(
        "P_Cl(3) full functor closing L1 open gate: rank-deficient",
    ...
    c.record_open(
        ...
        "Bode-Panagopoulos 2002 -- exactly the MSbar-native route context",
    )

    c.record_open(
        "Missing ingredient (b): rational-coefficient extraction from c_2",
        "the finite-field c_2-style diagnostic does not extract the "
        "rational coefficient in front of zeta_3; this requires "
    ...
    c.record_open(
        ...
        "existing context, the channel-by-channel rational decomposition of the "
...
def section5_loop_by_loop(c: Counter) -> None:
    ...
    by universality + existing group theory; FAILS at beta_2.
    """
    print()
...
def section6_structural_constraints(c: Counter) -> None:
    ...
    open gate.  The L1 channel-weight open gate is now sharpened
    from 'no physical Cl(3) local algebra / Z^3-native period functor known' to:
    ...
    Sharpened open-gate shape:  the missing ingredient on the substrate
    side is NOT one piece but THREE independent ingredients.
    """
    print()
    print("Section 6 -- Sharpened open-gate shape")
    ...
    for ingredient, prior_authority in ingredients:
        print(f"    [SHARPENED] missing ingredient: {ingredient}")
        print(f"                prior authority    : {prior_authority}")

    c.record(
        "Sharpened open-gate shape: 3 independent missing ingredients",
        len(ingredients) == 3,
        "previously named as one piece; now decomposed",
    )

    c.record_open(
        "L1 channel-weight open-gate shape:  3 independent missing ingredients",
        "sharper than P-L1 design note's single 'P_L1-D physical Cl(3) local algebra / Z^3-native "
...
def section7_hostile_review(c: Counter) -> None:
    ...
    Q1.  Is P_Cl(3)^HK really substrate-native, or does it import
       ...
       A.  The heat-kernel single-plaquette <P>_HK_SU(3) = 1 - exp(-(4/3) s_t)
           is existing context via C_ISO_SU3_NLO_CLOSURE_BOUNDED_NOTE.  This IS
    ...
    Q4.  Could a more clever construction succeed where this one fails?
       A.  Possibly, but the sharpened open-gate analysis identifies THREE
    ...
    Q5.  Does this probe LOWER the MSbar-native route context open gate status?
       A.  No.  MSbar-native route context open gate stands unchanged.  This probe
    ...
    Q7.  Could the substrate be EXTENDED to close the open gate?  E.g.,
         add a "primitive period oracle".
       A.  Yes, but that would require explicitly approving a new primitive
           of comparable strength to the original missing primitive (i.e.,
           admitting Brown-Schnetz period theory as primitive).  This was
           the P_L1-C design's structural failure mode and is unchanged here.
    """
    print()
    print("Section 7 -- Hostile-review self-audit")

    audit_qs = [
        ("Q1: P_Cl(3)^HK substrate-native?",  "YES (heat-kernel context)"),
        ...
        ("Q5: Lowers MSbar-native route context?", "NO (extends + sharpens)"),
        ("Q6: Result N_f-dependent?", "NO (rank deficiency at any N_f)"),
        ("Q7: Substrate extension?",  "Would require explicit primitive approval"),
    ]
...
def section8_summary(c: Counter) -> None:
    ...
    print("    open-gate proposal.  Audit verdict and downstream status are set only")
...
def main() -> int:
    print("=" * 70)
    print("Open-Gate Probe P-L1-D --")
    print("  physical Cl(3) local algebra / Z^3-native period functor on 1PI Feynman graphs")
    print("=" * 70)
    print(f"Date: 2026-05-10")
    print(f"Authority role: source-note open_gate")
    print(f"Source-note: docs/PRIMITIVE_P_L1_D_PERIOD_FUNCTOR_NOTE_2026-05-10_pPl1_d.md")
    print(f"Companion source note: P-L1-Channel-Weight")
    print(f"  (P_L1-D left as open design problem)")

    c = Counter()
    section1_existing_support(c)
