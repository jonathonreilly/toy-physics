...
NN harness to the exact 2D mirror harness implemented in
...
(each entry seed-averaged) should pass the bounded Cauchy gate in N:
pairwise increments
...
and decides:
  ...
  NULL  (bounded no-go) : joint fit fails, components non-monotone
...
# seen at N <= 100 damps at larger N.
...
def main() -> int:
    ...
    if joint_cauchy_ok:
        ...
        verdict = "NULL"
        print("  SHARP BOUNDED NULL: the 2D mirror harness does not pass")
        print("  the operator-Cauchy gate on this layer-count grid.")
        non_mono_list = ", ".join(
            o for o, m in component_mono.items() if m == "non-mono"
        )
