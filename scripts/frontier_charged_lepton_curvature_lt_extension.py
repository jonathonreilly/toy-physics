...
def kii_spec_symbolic(L_t: int, m_sq: sp.Expr, u0_sq: sp.Expr) -> sp.Expr:
    ...
    Matches Agent 1's L_t=4 result: on APBC L_t=4, all sin^2(omega_n) = 1/2,
...
def effective_c_lt(L_t: int) -> Tuple[sp.Expr, bool, sp.Expr]:
    ...
    frequencies, and is the natural generalization of Agent 1's c(4) = 7/2
...
def part_B_mixing_mechanisms():
    ...
    print()
    print("    i.e. b ~ y_i y_j at leading order. RELATED TO ONGOING G1 Z_3")
    ...
    print("  cone but is under active work in a SEPARATE thread (G1 Z_3")
...
def main() -> int:
    ...
    print("  nonzero?")
    print()
    print("  Agent 1's L_t=4 anchor: c(4) = 7/2 and b = 0 by translation-")
