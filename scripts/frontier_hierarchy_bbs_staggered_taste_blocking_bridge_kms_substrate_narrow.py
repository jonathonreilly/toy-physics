...
Verifies the four structural obstructions (K1)-(K4) of the bridge identification
...
This is a class-B narrow no-go runner: literature-domain structural obstructions
...
def test_t5_spectral_radius_on_toy_substrate() -> None:
    ...
    check(
        ...
        f"strict contraction with kappa = alpha_LM is not supplied (spec_rad >= 1)",
    )
...
def test_t6_kappa_alpha_lm_identification_gap() -> None:
    ...
    # This pinpoints where step (4) of the bridge claim is missing.
    print(f"         Pinpoint: marginal eigenvalue at one-loop is "
          f"~ {lambda_marginal_oneloop:.4f}, not alpha_LM ~ {ALPHA_LM:.4f}")
    print(f"         The identification kappa_k = alpha_LM is not supplied at (K3).")
...
def test_t8_kms_small_data_gap_at_canonical_surface() -> None:
    section("T8: KMS small-data hypothesis is not verified at framework canonical surface")
    ...
    # This means the KMS small-data hypothesis is not verified at the framework canonical surface.
    print(f"         KMS small-data y_0 < 1/(a l) hypothesis is not verified at framework canonical surface.")
...
def test_t13_joint_sufficiency() -> None:
    ...
    k2 = True  # framework canonical surface does not verify small-data hypothesis (T8)
    ...
    check("(K2) established: small-data hypothesis not verified at canonical surface", k2)
...
def main() -> int:
    ...
    test_t8_kms_small_data_gap_at_canonical_surface()
