...
**EMPIRICAL SUPPORT FINDING**: the retained framework uses a NON-STANDARD Berry
...
**Conclusion**: among the tested conventions, the radian-bridge postulate
`η numerical = δ radian` is the one that matches PDG cleanly. This is strong
numerical support, not a derivation of the physical Brannen-phase bridge.
"""
...
def main() -> int:
    ...
    print("  In this comparison, only this convention matches PDG cleanly.")
    print()
    print("  That is strong numerical support, but not yet a derivation of")
    print("  the physical Brannen-phase bridge from retained structure.")

    record(
        "C.1 Retained radian-bridge postulate is numerically singled out",
        has_negative and max_rel_err < 1e-3,
        "Only δ = η (in radians numerically) matches PDG charged-lepton masses.\n"
        "Standard Berry convention (2π·η) fails in this comparison.",
    )
    ...
    if all_pass:
        ...
        print("strongly supported numerically by the PDG comparison tested here.")
        print()
        print("This means: within the retained framework's combined δ=2/9")
        print("(via AS/APS candidate support) + A1 support + Brannen form,")
        print("the radian-bridge postulate is the unique tested convention")
        print("matching observation in this comparison.")
        print()
