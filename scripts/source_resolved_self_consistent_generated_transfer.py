#!/usr/bin/env python3
"""Self-consistent source-resolved Green transfer on the compact generated DAG family.

This is the alternate-family transfer falsifier for the self-consistent Green
architecture. It mirrors the exact-lattice self-consistency update but runs on
the retained compact generated DAG family.
"""
...
def main() -> None:
    families: list[tuple[list[tuple[float, float, float]], dict[int, list[int]], list[list[int]]]] = []
    for seed in range(N_SEEDS):
        ...
        families.append((positions, adj, layers))

    ref_max = 0.0
    for positions, adj, layers in families:
    ...
    print("=" * 84)
    print("SOURCE-RESOLVED SELF-CONSISTENT GENERATED TRANSFER")
    print("  compact generated DAG family, one self-consistency update")
    print("=" * 84)
    print(
    ...
    for seed, (positions, adj, layers) in enumerate(families):
        det = layers[-1]
        all_ys = [y for _, y, _ in positions]
        cy = sum(all_ys) / len(all_ys)
        mass_ids = _select_mass_nodes(positions, layers[len(layers) // 2], cy)
        free = propagate(positions, adj, [0.0] * len(positions), layers[0], K)
        zero_field = [gain * v for v in _green_field(positions, mass_ids, [0.0] * len(mass_ids), 0.0)]
    ...
    print(f"{'s':>8s} {'inst':>12s} {'self-cons':>12s} {'self/inst':>11s} {'sign':>6s}")
    print("-" * 58)

    for s in SOURCE_STRENGTHS:
        per_seed_inst: list[float] = []
        per_seed_self: list[float] = []
        same_sign = 0
        total = 0
        for positions, adj, layers in families:
            det = layers[-1]
            ...
            field0 = [gain * v for v in _green_field(positions, mass_ids, weights, s)]
            amps0 = propagate(positions, adj, field0, layers[0], K)
            power = [abs(amps0[i]) ** 2 for i in mass_ids]
            weights2 = _normalize_weights(power)
            field_sc = [gain * v for v in _green_field(positions, mass_ids, weights2, s)]
            sc_amps = propagate(positions, adj, field_sc, layers[0], K)
            sc_delta = centroid_y(sc_amps, positions, det) - z_free

            per_seed_inst.append(inst_delta)
            per_seed_self.append(sc_delta)
            if inst_delta == 0.0:
                if sc_delta == 0.0:
                    same_sign += 1
            elif sc_delta != 0.0 and inst_delta * sc_delta > 0:
                same_sign += 1
            total += 1

        inst_mean = _mean(per_seed_inst)
        self_mean = _mean(per_seed_self)
        inst_means.append(inst_mean)
        self_means.append(self_mean)
        ratio = self_mean / inst_mean if abs(inst_mean) > 1e-30 else math.nan
        print(f"{s:8.4f} {inst_mean:+12.6e} {self_mean:+12.6e} {ratio:11.3f} {same_sign:>2d}/{total:<2d}")

    inst_alpha = _fit_power(list(SOURCE_STRENGTHS), [abs(v) for v in inst_means])
    self_alpha = _fit_power(list(SOURCE_STRENGTHS), [abs(v) for v in self_means])
    toward = sum(1 for v in self_means if v > 0)
    mean_ratio = sum(abs(s / i) for s, i in zip(self_means, inst_means) if abs(i) > 1e-30) / len(SOURCE_STRENGTHS)
    ...
    if self_alpha is not None:
        print(f"  self-consistent Green F~M exponent: {self_alpha:.2f}")
    else:
        print("  self-consistent Green F~M exponent: n/a")
    print(f"  TOWARD rows: {toward}/{len(self_means)}")
    print(f"  mean |self/inst| ratio: {mean_ratio:.3f}")
