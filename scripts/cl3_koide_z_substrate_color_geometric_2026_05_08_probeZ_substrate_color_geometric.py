...
Question
--------
The substrate-anomaly hidden-character analysis (sister Probe Y) identified
five admissions NOT forced by SM gauge anomaly cancellation alone. Row 1 was
"N_c = 3 not forced" (anomaly cancellation closes for any N_c ≥ 2).
...
The framework's foundational A1 axiom places Cl(3) at every lattice site.
...
6. K6: Identify two retained bridge routes for the adjoint-carrier
   admission (graph-first SU(3) commutant + Gell-Mann completeness).
7. K7: Reclassify Probe Y substrate-anomaly row 1 admission.
...
supplied by retained content (graph-first SU(3) integration + Gell-Mann
completeness). Conservative tier: BOUNDED THEOREM (positive on rigidity,
...
- Probe Y substrate-anomaly row 1 (admissions bundle): sister probe authority
...
def kZ5_distinction_from_anomaly():
    ...
    matter content (Probe Y substrate-anomaly row 1). Cl(3) dim-counting
    ...
    print(f"    Cancels for any N_c ≥ 2 (Probe Y substrate-anomaly row 1).")
    ...
    print(f"    Depends on local-algebra axiom A1 (Cl(3) site algebra).")
...
def kZ6_bridge_routes():
    """K6: Two retained routes for the adjoint-carrier bridge.
    ...
    """
    print("\n" + "=" * 78)
    print("K6: Adjoint-carrier bridge — two retained routes")
    print("=" * 78)

    print(f"\n  Route A: graph-first SU(3) commutant on taste cube")
    print(f"           (GRAPH_FIRST_SU3_INTEGRATION_NOTE.md)")
    print(f"  ----------------------------------------------------------------")
    print(f"    Per A1, each Cl(3) site induces a 2-dim spinor Hilbert.")
...
def kZ7_probeY_row1_sharpening():
    """K7: Probe Y substrate-anomaly row 1 sharpening.

    Before this probe: row 1 was a free admission ("N_c = 3 not forced").
    After this probe: row 1 is forced under retained graph-first bridge.
    """
    print("\n" + "=" * 78)
    print("K7: Probe Y substrate-anomaly row 1 sharpening")
    print("=" * 78)

    print(f"\n  Probe Y substrate-anomaly row 1 (before):")
    ...
    print(f"            retained SM left-handed content.")
    print(f"    Status: free admission on the substrate-to-carrier bundle.")
    ...
    print(f"            (retained per graph-first SU(3) integration + Gell-Mann completeness).")
    ...
    print(f"\n  After this probe (row 1 reclassified):")
    print(f"    Statement: 'N_c = 3 is forced by Cl(3) substrate algebra under")
    print(f"               retained graph-first adjoint-carrier bridge.'")
    print(f"    Status: forced, with bridge as the bounded admission.")
    print(f"    Bridge admission: already retained per GRAPH_FIRST_SU3_INTEGRATION_NOTE")
    print(f"                      and GELLMANN_COMPLETENESS_THEOREM_NOTE.")
    print(f"    Net effect: row 1 admission transferred from 'free' to 'forced under")
    print(f"                retained bridge' — the bundle's substrate-color hidden")
    print(f"                character is no longer a free admission.")

    print(f"\n  Other Probe Y rows NOT affected by this probe:")
    print(f"    Row 2 (n_gen = 3): separate forcing question (generation count)")
    print(f"    Row 3 (LH content choice): separate forcing question (matter content)")
    print(f"    Row 4 (absolute Y scale): separate forcing question (hypercharge norm)")
    print(f"    Row 5 (A1-condition / BAE): separate forcing question (charged-lepton sector)")
    print(f"    Each row remains at Probe Y's stated status.")

    ok = (joint_admit == [3])
    report("k7-probey-row1-sharpening",
           ok,
           f"Row 1 reclassified: N_c forced to 3 under joint anomaly + Cl(3) constraints; joint = {joint_admit}")
...
def kZ8_tier_verdict():
    ...
    # Bridge admission: named, retained
    bridge_named = True  # K6 named it
    bridge_retained = True  # graph-first + Gell-Mann completeness both retained
    ...
    print(f"  Bridge retained per upstream:            {bridge_retained}")
    ...
    if multi_admit:
    ...
    elif forcing_rigid and bridge_named and not multi_admit:
        ...
        verdict = (
            ...
            f"Both bridge routes (graph-first SU(3) commutant + Gell-Mann "
            f"completeness) are retained per upstream. Tier: BOUNDED THEOREM. "
            f"Audit lane has authority to upgrade to POSITIVE if folding "
            f"the graph-first bridge into the 'retained Cl(3) algebra' surface."
        )
    else:
        tier = "NEGATIVE"
        verdict = (
    ...
    report("k8-tier-verdict",
           ...
           f"Tier: {tier} (rigid forcing N_c² = 9 ⇒ N_c = 3; bridge named retained)")
