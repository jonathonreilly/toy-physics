#!/usr/bin/env python3
"""
PR #230 block25 post-block24 landed checkpoint.

Block24 was the queue-pivot admission checkpoint.  This checkpoint answers the
narrow resume question after that checkpoint landed on draft PR #230: did the
PR head move by anything other than the block24 checkpoint, and does that landed
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_block25_post_block24_landed_checkpoint_2026-05-11.json"
)

PR_REF = "origin/claude/yt-direct-lattice-correlator-2026-04-30"
MAIN_REF = "origin/main"
BLOCK24_INPUT_HEAD = "82a01735f6118dcea381c23c0bc2ff4230cc4e33"
BLOCK24_SUBJECT = "Record PR230 block24 queue pivot checkpoint"
BLOCK25_SUBJECT = "Record PR230 block25 landed checkpoint"

PARENTS = {
    "block24_queue_pivot_admission": "outputs/yt_pr230_block24_queue_pivot_admission_checkpoint_2026-05-11.json",
...
def allowed_block24_path(path: str) -> bool:
    return path in ALLOWED_BLOCK24_PATHS or any(
        path.startswith(prefix) for prefix in ALLOWED_BLOCK24_PREFIXES
    )
...
def main() -> int:
    print("PR #230 block25 post-block24 landed checkpoint")
    print("=" * 72)

    parents = {name: load_json(path) for name, path in PARENTS.items()}
    local_head = run_git(["rev-parse", "HEAD"])
    local_subject = run_git(["log", "-1", "--format=%s", "HEAD"])
    inspection_ref = "HEAD^" if local_subject == BLOCK25_SUBJECT else "HEAD"
    ...
    commits_since_block24_input = [
        line
        for line in run_git(
            ["log", "--format=%H%x09%s", f"{BLOCK24_INPUT_HEAD}..{inspection_ref}"]
        ).splitlines()
        if line
    ]
    paths_since_block24_input = changed_paths(BLOCK24_INPUT_HEAD, inspection_ref)
    disallowed_paths_since_block24_input = [
        path for path in paths_since_block24_input if not allowed_block24_path(path)
    ]
    ...
    block24 = parents["block24_queue_pivot_admission"]
    ...
    only_block24_checkpoint_since_input = (
        len(commits_since_block24_input) == 1
        and commits_since_block24_input[0].endswith(f"\t{BLOCK24_SUBJECT}")
        and not disallowed_paths_since_block24_input
    )
    ...
    checks = {
        "inspection-head-is-current-pr-head-or-parent-of-block25": inspected_head == pr_head
        or local_head == pr_head,
        "inspection-head-is-block24-checkpoint": inspected_subject == BLOCK24_SUBJECT,
        "only-block24-checkpoint-since-block24-input-head": only_block24_checkpoint_since_input,
        ...
        "block24-no-route-admitted": block24.get("checks", {}).get(
            "queue-rank1-source-higgs-not-admitted"
        )
        is True
        and block24.get("checks", {}).get("queue-rank2-wz-not-admitted") is True
        and block24.get("checks", {}).get("queue-rank3-neutral-h3h4-not-admitted")
        is True,
        "block24-forbidden-firewall-clean": block24.get("checks", {}).get(
    ...
    report(
        "inspection-head-is-current-pr-head-or-parent-of-block25",
        checks["inspection-head-is-current-pr-head-or-parent-of-block25"],
        f"HEAD={local_head[:9]} inspected={inspected_head[:9]} PR={pr_head[:9]}",
    )
    report(
        "only-block24-checkpoint-since-block24-input-head",
        checks["only-block24-checkpoint-since-block24-input-head"],
        f"commits={len(commits_since_block24_input)} disallowed_paths={len(disallowed_paths_since_block24_input)}",
    )
    report(
    ...
    report(
        "block24-no-route-admitted",
        checks["block24-no-route-admitted"],
        status(block24),
    )
    report(
    ...
    result = {
        ...
        "block24_input_head": BLOCK24_INPUT_HEAD,
        "commits_since_block24_input_head": commits_since_block24_input,
        "paths_since_block24_input_head": paths_since_block24_input,
        "disallowed_paths_since_block24_input_head": disallowed_paths_since_block24_input,
        ...
        "actual_current_surface_status": (
            "open / block25 post-block24 landed checkpoint; PR head contains "
            "only the block24 checkpoint after the previous queue-pivot input "
        ...
        "proposal_allowed_reason": (
            "Block25 is a landed-checkpoint, not a new closure artifact.  The "
            "current PR head is the block24 checkpoint commit; no required "
        ...
        "strict_non_claims": [
            ...
            "does not treat the block24 checkpoint commit as new physics evidence",
        ],
