...
def main():
    ...
    # 1. Fraction attractive > 80% across all configs
    # 2. Individual CoM approach > 70%
    # 3. Power law exponent within 0.5 of -2.0 (Newton)

    frac_attract = n_attract / total
    frac_indiv = n_indiv / total

    verdict_attract = frac_attract > 0.8
    verdict_indiv = frac_indiv > 0.7
    if len(clean_ds) >= 3:
        verdict_newton = abs(slope - (-2.0)) < 0.5
    else:
        verdict_newton = False

    print(f"  1. Mutual acceleration attractive: {n_attract}/{total} = {frac_attract:.1%} "
          f"{'PASS' if verdict_attract else 'FAIL'} (threshold: 80%)")
    print(f"  2. Individual CoM approach:        {n_indiv}/{total} = {frac_indiv:.1%} "
          f"{'PASS' if verdict_indiv else 'FAIL'} (threshold: 70%)")
    if len(clean_ds) >= 3:
        print(f"  3. Newton scaling |a|~d^alpha:     alpha={slope:.3f} "
              f"{'PASS' if verdict_newton else 'FAIL'} (expect -2.0 +/- 0.5)")
    else:
        print(f"  3. Newton scaling: insufficient data (need 3+ attractive separations)")

    all_pass = verdict_attract and verdict_indiv and verdict_newton
    print()
    if all_pass:
        print("  >>> TWO-BODY MUTUAL ATTRACTION: CONFIRMED <<<")
    else:
        print("  >>> TWO-BODY MUTUAL ATTRACTION: NOT YET CONFIRMED <<<")
        if not verdict_attract:
            print("      - Attraction fraction below threshold")
        if not verdict_indiv:
            print("      - Individual packet approach below threshold")
        if not verdict_newton:
            print("      - Newton scaling not confirmed")
