...
def part1_exact_reparameterization_identity() -> None:
    ...
    check(
        "The completed slot u_small is an explicit nontrivial function of the retained pair (v,w)",
        u_small.free_symbols == {v, w}
        and sp.simplify(sp.diff(u_small, v)) != 0
        and sp.simplify(sp.diff(u_small, w)) != 0,
        detail="u_small depends on v and w, so it is not an independent native slot",
    )
