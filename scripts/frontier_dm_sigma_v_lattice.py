...
log("=" * 78)
log()
log("  After this analysis, the numerator-side provenance of the DM ratio")
log("  inputs changes. This script is NOT full-lane closure authority by")
log("  itself; it is one piece of the direct-observable stack.")
log()
...
log(f"  {'Gauge normalization':>35s}  {'ASSUMED':>12s}  {'BOUNDED':>12s}  {'Rigidity theorem candidate + self-dual support':>30s}")
log("  " + "-" * 95)
log()
...
log("  2. The self-dual point argument is support only; the stronger route is")
log("     the separate gauge-normalization rigidity theorem candidate.")
...
log("  4. The overall DM lane remains BOUNDED even if this numerator-side")
log("     provenance is strengthened.")
log()
...
log("    - sigma_v does not itself close gauge normalization; that question is")
log("      now carried by the separate rigidity-theorem note")
log()
...
log("    - Integrate or refute the gauge-normalization rigidity theorem")
log()
...
log("    the continuum limit. The old self-dual observation is only bounded")
log("    support; the stronger response to the gauge-normalization objection")
log("    now lives in the separate rigidity-theorem note.")
log()
...
log("     coupling alpha_s comes from the plaquette on the framework-defined")
log("     Hamiltonian surface. This strengthens the DM numerator, but the")
log("     full relic lane remains bounded.'")
log()
