...
algebra identity that raw tensor-product operators at distinct lattice sites in
`H = ⊗_x H_x` commute. This is the (M1) clause of the parent
...
verification:
  ...
  (d) verifies that distinct-site Pauli ladder operators commute in the raw
      tensor-product representation, while explicitly not testing a
      Jordan-Wigner / graded tensor-product anticommutation bridge;
...
def main() -> int:
    ...
    # ---------------------------------------------------------------------
    section("Part 3: (L2) raw tensor-product locality for Pauli ladders")
    # ---------------------------------------------------------------------
    # On the raw tensor-product Hilbert space, distinct-site ladder operators
    # commute because they are constructed from independent tensor factors. The
    # Z_2-graded anticommutator would be a separate Jordan-Wigner / graded
    # tensor-product bridge and is intentionally not claimed by this runner.
    check(
        "[σ_+^{(1)}, σ_+^{(2)}] = 0 exact (raw tensor-product L2)",
