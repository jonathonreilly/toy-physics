...
whether retained Cl(3)/Z^3 content + dynamical chiral SSB structural
argument + retained Yukawa flow gives m_b, m_c to ~5% of PDG.

Verdict structure
=================
The probe is bounded_theorem (negative for the heavy-quark gate; with
...
ingredients per Z-S4b-Audit hostile-review pattern:
  ...
  I4 y_b(M_Pl), y_c(M_Pl) species-differentiated BC: NOT RETAINED
     ...
     is the candidate positive mechanism, but A1 equipartition + P1
     sqrt(m)-identification are admitted non-retained primitives).
...
Verdict: BOUNDED THEOREM (negative). The dynamical chiral SSB hypothesis
...
What this DOES NOT close:
  - The Koide circulant Fourier-basis spectrum candidate mechanism
    (A1 + P1 admissions) for species-differentiation.
...
def section2_tier_classification(c: Counter) -> None:
    ...
    c.admit(
        ...
        "Koide circulant rho_down=1.536, rho_up=1.754 candidate; A1 "
        "equipartition + P1 sqrt(m)-identification are admitted "
...
def section5_chiral_ssb_structure(c: Counter) -> None:
    ...
    for q in ("u", "d", "s"):
        ...
        # But m_q^current itself is NOT retained from Cl(3)/Z^3 either
    c.admit(
...
def section8_structural_verdict(c: Counter) -> None:
    ...
    print(
        ...
        "y_q(M_Pl) -- candidates are A1 + P1 in the Koide circulant "
        "Fourier-basis spectrum (NOT retained) per YT_BOTTOM §5.2."
    )
    c.record(
        "Probe V verdict: BOUNDED THEOREM (negative for heavy-quark gate)",
        True,
...
def section9_constraints(c: Counter) -> None:
    ...
    c.record(
        ...
        True,
        "All inputs from retained Cl(3)/Z^3 (S1+Casimir+plaquette) + "
...
def main() -> int:
    ...
    print("-------")
    print(
        "BOUNDED THEOREM (negative for heavy-quark closure gate). "
    ...
    print(
        ...
        "primitive on y_q(M_Pl) -- candidates A1 equipartition + P1 "
