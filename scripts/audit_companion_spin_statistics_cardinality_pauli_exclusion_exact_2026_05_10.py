...
identities:
  ...
  (C2) Fermionic CAR has a 2-dim irreducible single-mode realisation
       (explicit 2×2 construction).
...
def main() -> int:
    ...
    # ---------------------------------------------------------------------
    section("Part 2 (C2): irreducible CAR realisation on dim H = 2")
    # ---------------------------------------------------------------------
    c = Matrix([[0, 1], [0, 0]])
    cdag = Matrix([[0, 0], [1, 0]])
    I2 = eye(2)
    ...
    # ---------------------------------------------------------------------
    section("Part 6: cardinality match — 2-dim irreducible carrier")
    # ---------------------------------------------------------------------
    # Combining (C1) and (C2): finite-dim CCR is impossible, while the
    # one-pair CAR algebra has a 2-dim irreducible/minimal carrier. General
    # finite CAR representations may be direct sums of this carrier.
    check("(Match-a) irreducible CAR Fock carrier dim = 2",
          ket0.shape == (2, 1))
    ...
    print("    (C2) Concrete irreducible CAR realisation on C² (c, c† matrices)")
    print("    (P1) (a†)² = 0 from CAR same-mode anticommutator")
    print("    (P2) n = c† c idempotent, spectrum(n) = {0, 1}")
    print("    Match: 2-dim irreducible CAR Fock carrier = 2-dim Cl(3) faithful irrep")
