...
Goal
====
Bypass the perturbative lattice -> physical matching and instead extract the
leading scalar-channel mass m_eff directly from the EXPONENTIAL DECAY of
gauge-invariant scalar correlators on the retained MC infrastructure:

    C(t) := < P(0) P(t) >  -  < P >^2     ~     A · exp(-m_eff · a · t)

where P is the timeslice-summed real-traced spatial plaquette and a is the
lattice spacing. Convert m_eff (in lattice units) -> m_eff (in GeV) via the
...
2. The leading mass extracted from <P(0) P(t)> is the lightest 0++ scalar
   in this channel for SU(3) pure-gauge. Identifying that with the
...
- NO new axioms or new MC algorithms beyond the retained kernel.
...
Verifies (via 13 PASS/FAIL checks):
  ...
  Part 7: extracted m_eff (lattice units) is positive and finite;
...
def extract_m_eff(C_mean: List[float], C_err: List[float], lt: int,
    ...
    if finite_pos.any():
        ...
        m_eff_err = float(np.sqrt(1.0 / np.sum(w)))
        n_used = -int(finite.sum())  # negative count = upper-bound mode
    else:
        m_eff = float("nan")
        m_eff_err = float("inf")
        n_used = 0
...
def part10_admission_statements(prod_summary: dict) -> None:
    section("Part 10: admission statements (channel + finite-size + "
            "convention)")
    print("  (1) Channel-identification admission: the leading mass extracted")
    print("      from <P_spatial(0) P_spatial(t)> is the lightest 0++ scalar")
    print("      in the spatial-plaquette channel for SU(3) pure-gauge.")
    ...
    print()
    print("  (4) Plateau-fit admission: the m_eff plateau is read off from")
    print("      a small d-window; the bounded result inherits the systematic")
    print("      of the plateau-fit window.  An RC-style multi-state fit is")
    print("      part of the named compute frontier and is NOT closed by")
    print("      this note.")
...
def main() -> int:
    ...
    if FAIL == 0:
        ...
        print("     numba-jitted MC infrastructure (no new axioms / imports);")
        print("   - the leading scalar mass m_eff at the canonical operating")
        print("     point is extracted to within stat error reported above;")
