...
def main() -> int:
    ...
    geometry_values = [4.0 * math.pi / SQRT6 for _ in m_samples]
    check(
        "Both sides of the comparison are branch-level constants rather than functions of the selected-line coordinate m",
        max(transport_values) - min(transport_values) < 1.0e-15
        and max(geometry_values) - min(geometry_values) < 1.0e-15,
        detail="sampling arbitrary m values leaves both quantities unchanged",
        kind="NUMERIC",
    )
    check(
        "So even an exact transport-gap identity would be at most a cross-lane support relation, not an m-selection law",
        abs((transport_values[0] - geometry_values[0]) - (transport_values[-1] - geometry_values[-1])) < 1.0e-15,
        detail="the comparison gap is itself m-independent on the sampled branch points",
        kind="NUMERIC",
    )
