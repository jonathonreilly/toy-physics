...
# ---------------------------------------------------------------------------
# Part 10: Cycle 16 / 19 admission-replacement scope.
# ---------------------------------------------------------------------------

section("Part 10: cycle 16 / cycle 19 admission-replacement scope")

replacement_strings = (
    "cycle 16 / cycle 19 chain",
    "What this addresses",
    "GUT-unification assumption",
)
for s in replacement_strings:
    check(f"note records admission-replacement scope: {s!r}", s in note_text)

# The replacement scope must include the SU(5) gauge-group embedding admission.
check("admission-replacement scope includes SU(5) gauge-group embedding admission",
      "SU(5) gauge-group embedding" in note_text and "admitted" in note_text)
# And the Y_GUT normalization admission.
check("admission-replacement scope includes Y_GUT = √(3/5)·Y_SM normalization admission",
...
if FAIL_COUNT == 0:
    ...
    print("           Cycles 16/19 SU(5)-embedding and Y_GUT-normalization admissions")
    print("           addressed as audit-pending bounded algebraic replacements;")
