#!/usr/bin/env python3
"""4D large-N CL bath sweep on modular causal DAGs.

Goal:
  - Test whether 4D decoherence stays bounded away from 1 through large N
  - Prioritize modular gap=5, with gap=3 as the main comparison
  - Report actual CL-bath purity `pur_cl`, floor `pur_min`, bath contrast
    `S_norm`, decoherence `1 - pur_cl`, and sample count `n_ok`

This is a focused large-N follow-up to the 4D decoherence harness.
The large-N 4D graphs need a denser transverse volume than the earlier
small-N harness, so this script uses a tighter spatial range and a larger
connect radius to keep the detector graph connected through N=100.
"""
                    ...
                    for prev_idx in prev_layer:
                        ...
                        if use_channels and layer > barrier_layer and round(px) > barrier_layer:
                            ...
                            if same_ch and dist <= connect_radius:
                                ...
                                not same_ch
                                and dist <= 2 * connect_radius
                                and rng.random() < crosslink_prob
                            ):
                                adj[prev_idx].append(idx)
...
def run_gap(gap, n_layers_list, n_seeds=4):
    k_band = [3.0, 5.0, 7.0]
    print(f"  [4D Modular gap={gap}] — {n_seeds} seeds")
    print(
        f"  {'N':>4s}  {'pur_cl':>8s}  {'pur_min':>8s}  {'S_norm':>8s}  "
        f"{'decoh':>8s}  {'n_ok':>5s}  verdict"
    )
    print(f"  {'-' * 66}")
    ...
    for nl in n_layers_list:
        ...
        for seed in range(n_seeds):
            ...
            mid_nodes = [
                i
                for li in range(bl_idx + 1, n_actual - 1)
                for i in layer_indices[li]
        ...
        if pur_list:
            ...
            verdict = "PASS" if mp < 0.97 else "CEILING"
            rows.append((nl, mp, mm, ms, len(pur_list), verdict))
            print(
                f"  {nl:4d}  {mp:8.4f}  {mm:8.4f}  {ms:8.5f}  "
                f"{1 - mp:+8.4f}  {len(pur_list):5d}  {verdict}"
            )
        else:
            print(f"  {nl:4d}  FAIL")

    if rows:
        mean_large = sum(r[1] for r in rows if r[0] >= 40) / max(1, sum(1 for r in rows if r[0] >= 40))
        mean_s = sum(r[3] for r in rows if r[0] >= 40) / max(1, sum(1 for r in rows if r[0] >= 40))
        verdict = "BOUND AWAY FROM 1" if all(r[1] < 0.97 for r in rows) else "CEILING RETURNING"
        print()
        print(f"  Large-N mean pur_cl (N>=40): {mean_large:.4f}")
        print(f"  Large-N mean S_norm (N>=40): {mean_s:.5f}")
...
def main():
    print("=" * 74)
    print("4D LARGE-N DECOHERENCE: CL bath on modular DAGs")
    print("  Retained family: modular channels in y, extra transverse z/w dimensions")
    print("  Goal: does 4D stay bounded away from 1 through N=40..100?")
    print("=" * 74)
    print()

    n_layers_list = [40, 60, 80, 100]
    for gap in [5.0, 3.0]:
        run_gap(gap, n_layers_list, n_seeds=4)

    print("=" * 74)
    print("INTERPRETATION")
    print("  gap=5 is the priority lane; gap=3 is the comparison lane")
    print("  Look for pur_cl well below 1 and S_norm staying O(1) at large N")
    print("=" * 74)
