...
of the Atiyah-Singer equivariant G-signature η-invariant for the
retained Z_3 conjugate-pair doublet:

    δ_physical = |η_AS(Z_3 conjugate-pair doublet (1, 2))| = 2/9 rad

This identification is closed axiom-only by two companion retained
theorems that both derive from the retained minimal-axiom stack +
textbook mathematics, introducing no new physical principle:
...
Under retained atlas + these two companion theorems, the three
previously-open Koide items close axiom-only with no observational
input:

    Brannen phase δ = 2/9    — KOIDE_DIRAC_ZERO_MODE_PHASE_THEOREM
    Koide ratio Q = 2/3      — Brannen/Rivero parametrization identity,
                               and independently derived as Z_3 Lefschetz
                               sum (companion runner
                               frontier_koide_q_equals_lefschetz_sum.py)
    Overall scale v_0        — CHARGED_LEPTON_RADIATIVE_YUKAWA_THEOREM
                               + retained hierarchy + Brannen formula
...
This runner verifies:
  ...
  - at the unique m_* on the retained first-branch interval where
    δ(m) = 2/9, the Brannen formula + y_τ = α_LM/(4π) reproduces
...
def main() -> int:
    ...
    print()
    print("Primary claim: on the retained selected line, the physical Brannen phase")
    print("equals |η_AS(Z_3 conjugate-pair (1,2))| = 2/9 rad, where η_AS is the AS")
    print("equivariant G-signature fixed-point contribution.")
    print()
    print("Closed axiom-only by two companion retained theorems (see docs/):")
    ...
    print("  - finds m_* where δ(m) = 2/9, confirms PDG precision match")
    print("  - tests v_0 and m_τ via derived y_τ = α_LM/(4π) + Brannen formula")
    ...
    if all_pass:
        ...
        print()
        print("Closure chain (retained minimal axioms + textbook math):")
        print()
        ...
        print("           identifies the Koide amplitude packet with the near-zero-mode")
        print("           of the retained Z_3-equivariant staggered-Dirac. By textbook")
        print("           AS G-index + APS spectral-flow: δ = |η_AS| = 2/9 exactly.")
        print()
        ...
        print("           give C_τ = 3/4 + 1/4 = 1 (colorless lepton), so")
        print("           y_τ^fw = α_LM/(4π) and m_τ = v_EW · α_LM/(4π).")
        print()
        print("  Step 5 — retained hierarchy v_EW = M_Pl · (7/8)^(1/4) · α_LM^16:")
        print("           v_0 = √m_τ / (1 + √2 cos(2/9)) = 17.7159 √MeV (PDG match).")
        print()
        print("All three previously-open Koide items close axiom-only:")
        print("  - Brannen phase δ = 2/9                   (Steps 1+2)")
        print("  - Koide ratio Q = 2/3                     (Step 3)")
        print("  - Overall scale v_0                       (Steps 4+5)")
        print()
        print("No new retained axioms are introduced. The two companion theorems are")
        print("concrete calculations on retained primitives + textbook mathematics.")
    else:
