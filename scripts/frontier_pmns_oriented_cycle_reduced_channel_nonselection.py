...
def part3_current_exact_constraints_do_not_select_a_unique_reduced_channel_point() -> None:
    ...
    check("So the current exact bank closes the reduced carrier and its observable law, but not the values", True)
    check("Therefore no value-selection law on that reduced channel has yet been derived from the current exact bank", True)
