#!/usr/bin/env python3
"""
Irregular Endogenous Sign Size Portability
==========================================

One-discriminator follow-up for the irregular sign lane.

Question:
  Does the original shell-packet +Phi vs -Phi separator survive graph growth
  on the same low-screening irregular families?

This is intentionally narrower than the packet-family reinforcement:
  - same shell-packet observable
  - same same-surface sign readout
  - same low-screening surface (mu2 = 0.001)
  - only the graph size changes

The three families are replayed at matched size scales:
  - random geometric: side = size
  - growing: n_target = size^2
  - layered cycle: layers = size, width = size

The result decides whether the irregular sign separator is portable under
graph growth or whether it stays frontier-only.
"""
...
def _family_rows(builder, family_name, size, **kwargs):
    rows = []
    for seed in SEEDS:
        ...
        print(
            f"[size] family={family_name:16s} size={size:2d} "
            f"seed={seed} center={center} nodes={n}"
        )
    return rows
...
def main():
    t0 = time.time()
    print("=" * 92)
    print("IRREGULAR ENDOGENOUS SIGN SIZE PORTABILITY")
    print("=" * 92)
    ...
    for size in SIZE_GRID:
        rows.extend(_family_rows(make_random_geometric, "random_geometric", size, side=size))
        rows.extend(_family_rows(make_growing, "growing", size, n_target=size * size))
        rows.extend(_family_rows(make_layered_cycle, "layered_cycle", size, layers=size, width=size))
    ...
    for size in SIZE_GRID:
        ...
        for family in ("random_geometric", "growing", "layered_cycle"):
            fam_rows = [r for r in size_rows if r.family == family]
            print(f"  {family}")
            for metric in ("ball1_margin", "ball2_margin", "depth_margin"):
                hits, total, mean, min_v = _summarize(fam_rows, metric)
                print(
                    f"    {metric:>12s}: pos={hits}/{total} mean={mean:+.4e} min={min_v:+.4e}"
                )
    ...
    shell_ball2_hits, shell_total, _, _ = _summarize(rows, "ball2_margin")
    shell_depth_hits, _, _, _ = _summarize(rows, "depth_margin")

    print("\nVERDICT")
    print("-" * 92)
    if shell_ball2_hits == shell_total and shell_depth_hits == shell_total:
        print(
            "Low-screening sign closure survives the size sweep on the audited irregular surface."
        )
        print(
            "That would support bounded retention, but still only for the shell-packet observable."
        )
    else:
        print(
            "Low-screening size portability does not close the lane."
        )
        print(
            "The irregular sign result remains frontier-only: strong on the original shell packet, but not portable enough under graph growth."
        )
