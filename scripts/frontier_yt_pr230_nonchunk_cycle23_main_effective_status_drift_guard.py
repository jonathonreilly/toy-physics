#!/usr/bin/env python3
"""
PR #230 non-chunk cycle-23 main-effective-status-drift reopen guard.

Cycle 22 closed origin/main audit/effective-status drift through c3fce9a17.
This guard checks the only new resume fact available at cycle 23: after a
fresh fetch, origin/main advanced again to 04decbdca, but the new diff still
contains only audit/effective-status surfaces and no listed PR230 same-surface
row, certificate, or theorem.

The runner does not load, combine, package, duplicate, or rerun chunk data.
It does not edit publication, Planck, alpha, claim-table, or manuscript
surfaces.
"""
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_nonchunk_cycle23_main_effective_status_drift_guard_2026-05-05.json"
)
LOOP_PACK = (
    ROOT
    / ".claude"
...
CYCLE22_HEAD = "dd8446ff0f29cd5024ee81234322b244b702d9da"
CYCLE22_REMOTE_MAIN_HEAD = "c3fce9a17d0cba277485bbbcff335ecbea62c69f"
REMOTE_PR = "origin/claude/yt-direct-lattice-correlator-2026-04-30"
REMOTE_MAIN = "origin/main"
ALLOWED_MAIN_EFFECTIVE_STATUS_DRIFT_PREFIXES = (
    "docs/audit/",
...
def allowed_main_effective_status_drift(path: str) -> bool:
    return path.startswith(ALLOWED_MAIN_EFFECTIVE_STATUS_DRIFT_PREFIXES)
...
def branch_state(future_paths: list[str]) -> dict[str, Any]:
    ...
    main_changed_paths = diff_paths(CYCLE22_REMOTE_MAIN_HEAD, REMOTE_MAIN)
    return {
        "cycle22_head": CYCLE22_HEAD,
        "cycle22_remote_main_head": CYCLE22_REMOTE_MAIN_HEAD,
        "local_head": local_head,
        "remote_pr_ref": REMOTE_PR,
        "remote_pr_head": remote_pr_head,
        "remote_main_ref": REMOTE_MAIN,
        "remote_main_head": remote_main_head,
        "cycle22_is_ancestor_of_local_head": git_ok(
            ["merge-base", "--is-ancestor", CYCLE22_HEAD, "HEAD"]
        ),
        "cycle22_is_ancestor_of_remote_pr": git_ok(
            ["merge-base", "--is-ancestor", CYCLE22_HEAD, REMOTE_PR]
        ),
        "cycle22_remote_main_is_ancestor_of_remote_main": git_ok(
            ["merge-base", "--is-ancestor", CYCLE22_REMOTE_MAIN_HEAD, REMOTE_MAIN]
        ),
        "local_head_matches_remote_pr_head": bool(local_head and local_head == remote_pr_head),
        "local_commits_after_cycle22": git(["rev-list", "--count", f"{CYCLE22_HEAD}..HEAD"]),
        "remote_commits_after_cycle22": git(["rev-list", "--count", f"{CYCLE22_HEAD}..{REMOTE_PR}"]),
        "remote_main_commits_after_cycle22_baseline": git(
            ["rev-list", "--count", f"{CYCLE22_REMOTE_MAIN_HEAD}..{REMOTE_MAIN}"]
        ),
        "remote_main_changed_paths_after_cycle22_baseline": main_changed_paths,
        "remote_main_changed_path_classes_after_cycle22_baseline": {
            rel: path_class(rel) for rel in main_changed_paths
        },
        "remote_main_changed_paths_are_effective_status_drift": bool(main_changed_paths)
        and all(allowed_main_effective_status_drift(rel) for rel in main_changed_paths),
        ...
        "local_changed_future_paths_after_cycle22": diff_future_paths(
            CYCLE22_HEAD, "HEAD", future_paths
        ),
        "remote_pr_changed_future_paths_after_cycle22": diff_future_paths(
            CYCLE22_HEAD, REMOTE_PR, future_paths
        ),
        "remote_main_changed_future_paths_after_cycle22_baseline": diff_future_paths(
            CYCLE22_REMOTE_MAIN_HEAD, REMOTE_MAIN, future_paths
        ),
    }
...
def main() -> int:
    print("PR #230 non-chunk cycle-23 main-effective-status-drift reopen guard")
    print("=" * 72)
    ...
    no_changed_future_path_since_cycle22 = not any(
        [
            branch["local_changed_future_paths_after_cycle22"],
            branch["remote_pr_changed_future_paths_after_cycle22"],
            branch["remote_main_changed_future_paths_after_cycle22_baseline"],
        ]
    )
    ...
    branch_current = (
        branch["cycle22_is_ancestor_of_local_head"]
        and branch["cycle22_is_ancestor_of_remote_pr"]
        and branch["cycle22_remote_main_is_ancestor_of_remote_main"]
        and branch["local_head_matches_remote_pr_head"]
        and bool(branch["remote_main_head"])
    )
    loop_pack_records_stop = (
        "cycle-22 update" in compact_text(OPPORTUNITY_QUEUE)
        and "main-audit-drift reopen guard" in compact_text(HANDOFF)
        and "main audit drift is not admissible reopen evidence" in compact_text(NO_GO_LEDGER)
        and "proposal_allowed: false" in compact_text(CLAIM_STATUS_CERTIFICATE)
    )
    main_drift_is_effective_status_only = (
        branch["remote_main_commits_after_cycle22_baseline"] != "0"
        and branch["remote_main_changed_paths_are_effective_status_drift"]
        and not branch["remote_main_changed_future_paths_after_cycle22_baseline"]
    )
    no_reopen_input = (
        source_presence_absent
        and no_local_or_remote_future_path
        and no_changed_future_path_since_cycle22
        and main_drift_is_effective_status_only
    )
    cycle23_main_effective_status_drift_guard_passed = all(
        [
            not missing_parents,
            not parent_failures,
    ...
    dramatic_step_gate = {
        ...
        "branch_state": branch,
        "reason": (
            "Cycle 22 already rejects origin/main audit/effective-status drift "
            "as reopen evidence.  Since then origin/main advanced again only "
            "on audit/effective-status surfaces and still contains none of the "
            "listed PR230 same-surface artifacts, so cycle 23 has no admissible "
            "non-chunk route to execute."
        ),
    }
    ...
    report("branch-has-cycle22-ancestor-and-remote-main-visible", branch_current, json.dumps(branch, sort_keys=True))
    ...
    report("parent-process-stack-closed-through-cycle22", parent_process_stack_closed, "cycle-18 through cycle-22 gates deny continuation")
    report("aggregate-gates-deny-proposal", aggregate_gates_deny, f"assembly={statuses['assembly']} campaign={statuses['campaign']}")
    report("loop-pack-records-cycle22-stop-contract", loop_pack_records_stop, "cycle-22 stop text present")
    report("main-drift-is-effective-status-only", main_drift_is_effective_status_only, json.dumps(branch, sort_keys=True))
    report("no-local-or-remote-listed-future-path", no_local_or_remote_future_path, json.dumps(branch, sort_keys=True))
    report("no-changed-listed-future-path-since-cycle22", no_changed_future_path_since_cycle22, json.dumps(branch, sort_keys=True))
    report("no-source-presence-key-set", source_presence_absent, json.dumps(present_by_source, sort_keys=True))
    report(
        "cycle23-main-effective-status-drift-guard-passed",
        cycle23_main_effective_status_drift_guard_passed,
        f"passed={cycle23_main_effective_status_drift_guard_passed}",
    )

    result = {
        "actual_current_surface_status": (
            "exact negative boundary / PR230 non-chunk cycle-23 "
            "main-effective-status-drift reopen guard: origin/main advanced "
            "again only on audit/effective-status surfaces and no listed "
            "same-surface artifact is present; positive closure remains open"
        ),
        ...
        "proposal_allowed_reason": (
            "Cycle 22 already rejects origin/main audit/effective-status drift. "
            "The cycle-22 head remains an ancestor of the aligned PR heads, "
            "origin/main advanced again only on audit/effective-status paths, "
            "no listed same-surface artifact appears on branch or remote "
            "surfaces, all six worklist units remain blocked, and aggregate "
        ...
        "nonchunk_scope_remains_stopped": cycle23_main_effective_status_drift_guard_passed,
        ...
        "exact_next_action": (
            ...
            "cycle-23, assembly, retained-route, and campaign gates before any "
            "proposal language."
        ),
