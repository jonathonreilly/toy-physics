...
Companion work:
  ...
  - PR #1003 derived the leading-order coherent-saddle constant
...
predicted variance into an "effective alpha" by reproducing PR #968's
...
L_TOTAL = PHYS_L
L_1 = L_TOTAL / 3.0
L_2 = 2.0 * L_TOTAL / 3.0  # post-slit length used by leading-order PR #1003

# Diagnostic fit values from PR #968 (4-point fit, h <= 0.25).
C_ARM_NUMERIC = 2.7107
ALPHA_NUMERIC = 0.5256

# Fit window used by PR #968.
FIT_HS = (0.25, 0.125, 0.0625, 0.03125)

# Measured per-h sigma_arm values (from PR #968 source data).
MEAS_SIGMA = {
    0.25: 1.3198,
    0.125: 0.8990,
    0.0625: 0.6282,
...
# ---------------------------------------------------------------------------
# Effective alpha from log-log linear fit (PR #968 protocol)
# ---------------------------------------------------------------------------
...
def report() -> int:
    ...
    # ------------------------------------------------------------------
    # Per-h cross-check vs PR #968 measurements
    # ------------------------------------------------------------------
    print("-" * 78)
    print("CROSS-CHECK vs PR #968 measured sigma_arm(h)")
    print("-" * 78)
    ...
    # ------------------------------------------------------------------
    print("-" * 78)
    print("EFFECTIVE ALPHA (log-log linear fit on PR #968 four-point window)")
    print("-" * 78)
    ...
    print()
    print("  Empirical re-fit reproduces PR #968: alpha = 0.5256, C = 2.7107.")
    print()
    ...
    print()
    if gap_alpha <= 0.005:
        print("  POSITIVE THEOREM: NLO saddle matches alpha to within +/- 0.005.")
        verdict = "positive"
