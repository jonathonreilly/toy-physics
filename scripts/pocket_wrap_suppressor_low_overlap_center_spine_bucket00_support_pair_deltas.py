#!/usr/bin/env python3
"""Probe nearest-opposite support motif deltas in center-spine bucket `00`."""
...
def build_support_signature(nodes: set[tuple[int, int]]) -> tuple[set[str], dict[str, float]]:
    if not nodes:
        return set(), {}
    ...
    for support in support_nodes:
        ...
        events.add(f"rid:r{role}:p{pocket_sig}:d{deep_sig}")
        events.add(f"count:{role}:p{len(adj_pocket)}:d{len(adj_deep)}")
    ...
    numeric = {
        ...
        "support_event_count": float(len(events)),
    }
...
def nearest_opposite_rows(
    ...
    *,
    ...
    # enforce deterministic iteration stability
...
def build_rows(
    ...
    *,
    ...
    for source_name in sorted(subtype_by_source):
        ...
        signatures[source_name] = build_support_signature(set(row.nodes))
    ...
    for source_name in sorted(subtype_by_source):
        ...
        numeric = {
            ...
            "pair_shared_event_fraction": _fraction(len(own_events & partner_events), len(own_events | partner_events)),
    ...
    row_cls = make_dataclass(
        ...
        [
            ...
            ("pair_shared_event_fraction", float),
            ("pair_own_only_count", float),
            ("pair_partner_only_count", float),
            ("pair_delta_event_count", float),
            ("delta_support_role_bridge_fraction", float),
            ("delta_support_role_pocket_only_fraction", float),
            ("delta_support_role_deep_only_fraction", float),
            ("delta_support_degree_mean", float),
            ("delta_support_same_role_edge_fraction", float),
            ("delta_support_event_count", float),
            ("abs_delta_support_role_bridge_fraction", float),
            ("abs_delta_support_role_pocket_only_fraction", float),
            ("abs_delta_support_role_deep_only_fraction", float),
            ("abs_delta_support_degree_mean", float),
            ("abs_delta_support_same_role_edge_fraction", float),
            ("abs_delta_support_event_count", float),
...
def render_rows(rows: list[object], selected_fields: tuple[tuple[str, str], ...]) -> str:
    shown = [
        ...
        "nearest_opposite",
        "pair_distance_z",
        "pair_shared_event_fraction",
        "pair_own_only_count",
        "pair_partner_only_count",
        "delta_support_degree_mean",
        "delta_support_same_role_edge_fraction",
        "pair_selected_event_present_count",
    ]
    shown += [field_name for _event, field_name in selected_fields[:4]]

    lines = [
        "Bucket 00 support pair-delta rows",
        "===============================",
        " | ".join(shown),
...
def main() -> None:
    ...
    print(f"center-spine bucket00 support pair-deltas started {started}", flush=True)
    ...
    print()
    print("Center-Spine Bucket 00 Support Pair-Deltas")
    print("==========================================")
    ...
    print()
    print(render_rules(f"Best support pair-delta rules for {args.right_subtype}", add4_rules))
    print()
    print(render_rules(f"Best support pair-delta rules for {args.left_subtype}", add1_rules))
    print()
    print(render_pair_map(rows))
    print()
    print(
        "center-spine bucket00 support pair-deltas completed "
