"""
Exact-tier path-integral tightening of W1 (multi-plaquette numerics).
...
combined paths:
  ...
  Path C: Analytic anisotropic-to-Hamiltonian coupling refinement.
    Derive the next-to-leading order O(s_t^2) corrections beyond the
    T-AT theorem's leading match, tightening Convention C-iso bound.
...
def path_c_analytic_refinement(
    ...
    For SU(2) (tractable proxy for the exact qualitative behavior):
...
def main() -> int:
    ...
    results: dict = {"args": vars(args), "version": "exact-tier-path-integral-2026-05-07"}
