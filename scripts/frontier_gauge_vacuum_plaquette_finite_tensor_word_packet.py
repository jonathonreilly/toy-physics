...
to MODE_MAX = 80 modes (matching the retained_bounded sister authority
GAUGE_VACUUM_PLAQUETTE_RHO_PQ6_WILSON_ENVIRONMENT_BOUNDED_NOTE_2026-05-09).

This is a SPLIT NOTE per the 2026-05-10 audit verdict's repair-target
option (b) on the parent
...
retained_bounded sister runner
...
# Truncation parameters and physical inputs (match retained_bounded sister)
BETA = 6.0
ARG = BETA / 3.0
MODE_MAX = 80
NMAX = 4
...
# Matches sister authority frontier_gauge_vacuum_plaquette_rho_pq_6_wilson_environment_compute
# ---------------------------------------------------------------------------
...
def part1_note_structure():
    ...
    required = [
        ...
        ("retained_bounded sister cited (rho_pq6)",
         "GAUGE_VACUUM_PLAQUETTE_RHO_PQ6_WILSON_ENVIRONMENT_BOUNDED_NOTE_2026-05-09"),
        ...
        ("split-note framing per 2026-05-10 verdict",
         "split note for the one finite tensor-word packet"),
    ]
...
# ---------------------------------------------------------------------------
# Part 5: Forbidden imports
# ---------------------------------------------------------------------------
def part5_forbidden_imports():
    section("Part 5: imports limited to family convention (numpy + scipy)")
    runner_text = Path(__file__).read_text()
    # numpy + scipy.special.iv is the family convention (matches sister
    # retained_bounded runner frontier_gauge_vacuum_plaquette_rho_pq_6_wilson_environment_compute).
    allowed = {
...
def part6_boundary_check():
    ...
    check(
        ...
        "split note for the one finite tensor-word packet" in NOTE_TEXT,
    )
    check(
        "(BA-1) and (BA-2) admitted as bounded textbook inputs",
        "bounded inputs, not derived from `Cl(3)/Z³`" in NOTE_TEXT,
    )
