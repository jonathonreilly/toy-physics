...
Claim scope: define κ_curv := |∂²V_taste/∂m²|_{m=0}| / (N_taste · 2 v²)
...
N_taste = 16 from the staggered-Dirac realization open gate. Then
    ...
    κ_curv = 1/(8 u_0²)    (v cancels exactly).

At u_0 = 0.8776, κ_curv ≈ 0.1623.

CRITICAL FIRST-PRINCIPLES SCOPE:
  κ_curv is structurally a (mass²-coefficient)/(v²) ratio, NOT a |φ|⁴
...
class (E) definitional choice for the (2v²) divisor. No new admissions
...
scope_disclaimers = [
    ...
    "EXPLICITLY DISTINCT",
    "structurally a (mass-squared)/(v²) ratio",  # the honest first-principles description
...
# In dimensionless lattice units (where curvature is in units of (mass scale)²
# with mass scale = 1/lattice spacing, and v likewise normalized), this gives
# κ_curv = (4/u_0²) / (16 · 2) = 1/(8u_0²).

N_taste = sp.Integer(16)

# In dimensionless ratio (v factors cancel because both curvature and 2v²
# share the same mass-scale^2 dimension):
kappa_curv = sp.Abs(curvature) / (N_taste * sp.Integer(2))
# Note: the v² in the denominator and the curvature's mass² units cancel;
# we represent this here via the symbolic identity.
...
check(
    "κ_curv depends only on u_0 (no v dependence after dim-0 ratio)",
...
#   - κ_curv: mass²-coefficient ratio (dim-0 from curvature/v²)
...
# it is structurally different from κ_curv (which is mass²-derived, not |φ|⁴).
...
print(
    ...
    f"\n         κ_curv (this theorem; mass²/v² ratio)        ≈ {kappa_numerical:.4f}"
    f"\n         y_t⁴/(4u_0⁴) (fermion-loop quartic, y_t=0.4) ≈ {fermion_loop_quartic:.4f}"
    f"\n         SM λ_obs(v) (PDG-extracted)                  ≈ {sm_lambda_obs_at_v:.4f}"
    f"\n"
    f"\n         Three structurally different objects. The first is a mass²-derived"
...
# structurally a mass²-derived ratio, not a quartic coupling. Verify the
...
scope_assertions = [
    ...
    "structurally a (mass-squared)/(v²) ratio",  # honest description of κ_curv
