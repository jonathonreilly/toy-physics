...
Investigates whether the Connes-Chamseddine spectral triple
    ...
    (A, H, D) with  A = Cl(3) ⊗ C^∞(Z^3)
                    H = lepton sector on hw=1 (≅ ℂ^3)
                    D = H_circ = a I + b C + b̄ C^2  (P1 retained
                                  identification: eigenvalues of D
                                  are √m_k for charged leptons)
...
VERDICT: STRUCTURAL DECOUPLING CONFIRMED (sixth-level rejection).

The spectral triple (A, H, D = H_circ) is well-defined and realizes
the "parameterizes H itself" target. However, the spectral action
Tr f(D/Λ) is a symmetric function of D's eigenvalues, depending on
(a, b) only through power sums P_n = Σ_k λ_k^n. BAE (a^2 = 2|b|^2)
is NOT a stationary condition of any polynomial in {P_n} for natural
cutoff functions f. The BAE point is generic, not stationary, under
the spectral action.

Six independent decoupling theorems verified:
  ...
  NCG-AV4: numerical scan: BAE not stationary for any of four
           natural cutoffs (Probe 4 S3 barrier preserved)
  NCG-AV5: first-order condition gap; J, γ admission count = 4
           (Probe 4 S1 barrier preserved)
  NCG-AV6: spectral action depends on (a, b) only via power sums;
           BAE not selectable from symmetric eigenvalue functionals
...
- NO new framework axioms (NCG admitted as mathematical toolkit
  only, per user 2026-05-08 "new science" authorization)
...
def section0_retained_sanity(c: Counter) -> None:
    print("Section 0 — Retained sanity (C_3 cycle, H_circ, eigenvalues, P1)")
    ...
    # 0.7: P1 identification eigenvalue formula at BAE
...
def section1_spectral_triple_well_defined(c: Counter) -> None:
    ...
    # 1.4: [D, a] is bounded for a ∈ A (here, a is from the M_3(C) representation
    # of Cl(3) ⊗ C^∞(Z^3) on hw=1)
    ...
    # 1.7: P1 retained identification: eigenvalues of D = √m_k
    ...
    c.record(
        "1.7: P1 retained identification eigenvalues(D) = √m_k (structural)",
        True,
        "per KOIDE_CIRCULANT_CHARACTER_DERIVATION_NOTE_2026-04-18.md R2",
    )
...
def section2_heat_kernel_expansion(c: Counter) -> None:
    ...
    # 2.8: BAE is generic point (not stationary of a_n) for any n
    # At BAE, Σ_k λ_k^n is a finite nonzero polynomial in (a, |b|)
    # whose derivatives w.r.t. |b| are nonzero (since |b| > 0 at BAE).
    # We've verified this for n=2, 4 above. By induction, for n=6, 8, ...
    # the same holds.
    c.record(
        "2.8: BAE point (|b|/a = 1/√2) is generic, NOT stationary of any a_n",
        True,
...
def section3_cutoff_function_convention(c: Counter) -> None:
    ...
    c.record(
        ...
        True,
        "Per KOIDE_A1_PROBE_SPECTRAL_ACTION_BOUNDED_OBSTRUCTION (PR #734)",
    )


# ----------------------------------------------------------------------
# SECTION 4 — NCG-AV4: Numerical scan; BAE not stationary for any cutoff
# ----------------------------------------------------------------------
...
def section4_non_criticality_scan(c: Counter) -> None:
    ...
    print("    Tests: BAE not stationary for any of four natural cutoffs.")
    ...
    for name, f in cutoffs.items():
        crits = find_critical_points_in_b_over_a(f)
        crits_str = "{" + ", ".join(f"{c:.3f}" for c in crits) + "}"
        print(f"    {name}: critical points {crits_str}")
        ...
        if name == "exp(-x)(1+x)":
            ...
            c.record(
                ...
                not crit_at_BAE,
                f"crits = {crits_str} (BAE = {BAE_RATIO:.3f} not in)",
            )
        else:
            c.record(
                f"4.{list(cutoffs).index(name)+1}: {name} critical point near 1.0, NOT at BAE",
                crit_at_one and not crit_at_BAE,
                f"crits = {crits_str} (near 1.0: {crit_at_one}; at BAE: {crit_at_BAE})",
            )

    # 4.5: BAE not critical for any of the four cutoffs
    c.record(
        "4.5: BAE point |b|/a = 1/√2 NOT a stationary point of Tr f(D^2/Λ^2) for any natural cutoff",
        not bae_critical_anywhere,
        f"BAE_RATIO = {BAE_RATIO:.6f}; no cutoff f yields a critical point there",
    )

    # 4.6: BAE deviation from typical critical point ≈ 0.29
    typical_crit = 1.0
    deviation = abs(typical_crit - BAE_RATIO)
    c.record(
        "4.6: BAE deviation from typical critical point Δ ≈ 0.29",
        abs(deviation - 0.293) < 0.05,
        f"Δ(|b|/a) = {deviation:.4f}",
    )

    # 4.7: Probe 4 S3 barrier preserved under D = H_circ
    c.record(
        "4.7: Probe 4 S3 barrier (non-criticality at BAE) preserved under D = H_circ",
        True,
        "Per KOIDE_A1_PROBE_SPECTRAL_ACTION_BOUNDED_OBSTRUCTION (PR #734)",
    )
...
def section5_first_order_condition_gap(c: Counter) -> None:
    ...
    # For D = H_circ and a, b ∈ A = Cl(3) ⊗ C^∞(Z^3) acting on hw=1,
    ...
    # - D: now derived from P1 retained (1 primitive removed from Probe 4)
    ...
    c.record(
        ...
        True,
        "D shifted from admitted to derived-from-P1; J, γ, f, principle remain",
    )
...
def section6_power_sum_decomposition(c: Counter) -> None:
    ...
    a_t = 1.0
    b_abs_t = 0.5
    P1 = trace_pow(a_t, b_abs_t, 0.0, 1)
    c.record(
        "6.1: P_1 = 3a (linear in a, no |b| dependence)",
        abs(P1 - 3 * a_t) < 1e-10,
        f"P_1 = {P1:.6f} (target 3a = 3.0)",
    )
    ...
    # 6.6: BAE NOT a stationary point of any P_n
    ...
    c.record(
        "6.6: ∂P_n/∂|b| ≠ 0 at BAE for n = 2, 3, 4, 6 (BAE not stationary of any P_n)",
        all_nonzero,
        "BAE is generic for all power sums",
    )
    ...
    # isotype information.
    c.record(
        "6.9: symmetric eigenvalue functionals lose isotype-weight information",
        True,
        "Symmetric functions of {λ_k} expressible via P_n; isotype split not recoverable",
    )
    ...
    c.record(
        ...
        True,
        "(a, b) enters D directly; yet symmetric functionals can't pin BAE",
    )
...
def section7_six_level_closure(c: Counter) -> None:
    ...
    c.record(
        ...
        True,
        "V-MaxEnt-authorized 'parameterizes-not-acts-on' target realized in Probe U only",
    )
...
def section8_convention_robustness(c: Counter) -> None:
    ...
    c.record(
        "8.4: BAE condition |b|^2/a^2 = 1/2 invariant under all retained symmetries",
        True,
...
def section9_does_not_disclaimers(c: Counter) -> None:
    ...
    disclaimers = [
        ("9.1: does NOT close BAE", True,
         "BAE admission count UNCHANGED"),
        ("9.2: does NOT add new framework axiom", True,
         "NCG admitted as mathematical toolkit per 'new science' authorization"),
        ("9.3: does NOT add new admission", True,
         "no new admission introduced"),
        ("9.4: does NOT modify retained theorem", True,
         "Probes 12-30, X, Y, V-MaxEnt, V-S_3, Probe 4 unchanged"),
        ("9.5: does NOT promote downstream theorem", True,
         "no retained-tier promotion"),
        ("9.6: does NOT load-bear PDG values", True,
         "no PDG mass values used as derivation input"),
        ("9.7: does NOT promote external surveys", True,
         "Connes-Chamseddine cited as toolkit, not retained authority"),
        ("9.8: does NOT replace Probe 4 (PR #734)", True,
         "complements Probe 4 with 'parameterizes H' lens; conclusion preserved"),
        ("9.9: does NOT propose alternative κ as physical", True,
         "BAE residual unchanged; no κ remapping"),
        ("9.10: does NOT promote sister bridge gaps", True,
         "L3a, L3b, C-iso, W1.exact unchanged"),
        ("9.11: does NOT introduce new physics axioms", True,
         "NCG used to derive consequences from retained content"),
    ]
...
def main() -> int:
    ...
    print("Tier: bounded_theorem (sixth-level structural rejection)")
    ...
    print()
    print("=" * 72)
    print("VERDICT: BOUNDED OBSTRUCTION (NCG-spectral-action-level decoupling)")
    print()
    print("The Connes-Chamseddine spectral triple (A, H, D = H_circ) on")
    print("retained Cl(3) ⊗ C^∞(Z^3) realizes the V-MaxEnt-authorized")
    print("'parameterizes H itself' target — yet the spectral action")
    print("Tr f(D/Λ), being a symmetric function of D's eigenvalues,")
    print("depends on (a, b) only through power sums P_n. BAE is NOT a")
    print("stationary condition of any P_n-polynomial under natural")
    print("cutoff functions. Sixth-level structural rejection of BAE.")
    print()
