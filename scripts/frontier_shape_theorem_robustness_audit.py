...
STATUS: stress-test of a companion runner v2's second-order Gamma_1 return shape theorem
        on the retained Cl(3)/Z^3 + chirality carrier C^16.

a companion runner v2's central claim:
...
We stress-test from seven independent angles:
  ...
  7.  Retained-surface reading of the Dirac-bridge theorem runner:
      do its PASS statements precisely imply a companion runner v2's shape
...
def stress_3_taste_doublet():
    ...
    # Under a companion runner v2: diag_L = (w_O0, w_a, w_b), with w_c irrelevant.
...
def stress_4_alternative_splittings():
    ...
    # Predictions from a companion runner v2's species-intermediate table:
...
def stress_7_retained_reading():
    ...
    print("STRESS 7: does the retained Dirac-bridge theorem PASS set imply "
          "a companion runner v2's shape theorem?")
    print("=" * 78)

    # The Dirac-bridge runner PASS statements relevant to a companion runner v2 are:
    #   (a) P_T1 Gamma_1 P_T1 = 0
    #   (b) P_T1 Gamma_1 (P_O0 + P_T2) Gamma_1 P_T1 = I_6 on full taste-doubled T_1
    #   (c) O_3 adds nothing
    # a companion runner v2's shape theorem asserts more:
    ...
    #     Dirac-bridge runner.
    #
    # So: a companion runner v2's shape theorem is IMPLIED by the Dirac-bridge runner's
