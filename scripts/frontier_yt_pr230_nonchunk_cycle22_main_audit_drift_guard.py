#!/usr/bin/env python3
"""
PR #230 non-chunk cycle-22 main-audit-drift reopen guard.

Cycle 21 closed remote-surface reopen after fetching `origin/main` at
8b9f29fc2.  This guard checks the only new resume fact available at cycle 22:
`origin/main` advanced again, but the new main diff is audit/effective-status
drift rather than a listed PR230 same-surface row, certificate, or theorem.
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_nonchunk_cycle22_main_audit_drift_guard_2026-05-05.json"
)
LOOP_PACK = (
    ROOT
    / ".claude"
...
CYCLE21_HEAD = "c77fe361d5aac5a44604575c67c84c87155a754e"
CYCLE21_REMOTE_MAIN_HEAD = "8b9f29fc2d6fced28e64761becd010e809e47a6c"
REMOTE_PR = "origin/claude/yt-direct-lattice-correlator-2026-04-30"
REMOTE_MAIN = "origin/main"
ALLOWED_MAIN_AUDIT_DRIFT_PREFIXES = (
    "docs/audit/",
...
def allowed_main_audit_drift(path: str) -> bool:
    return path.startswith(ALLOWED_MAIN_AUDIT_DRIFT_PREFIXES)
...
def branch_state(future_paths: list[str]) -> dict[str, Any]:
    ...
    main_changed_paths = diff_paths(CYCLE21_REMOTE_MAIN_HEAD, REMOTE_MAIN)
    return {
        "cycle21_head": CYCLE21_HEAD,
        "cycle21_remote_main_head": CYCLE21_REMOTE_MAIN_HEAD,
        "local_head": local_head,
        "remote_pr_ref": REMOTE_PR,
        "remote_pr_head": remote_pr_head,
        "remote_main_ref": REMOTE_MAIN,
        "remote_main_head": remote_main_head,
        "cycle21_is_ancestor_of_local_head": git_ok(
            ["merge-base", "--is-ancestor", CYCLE21_HEAD, "HEAD"]
        ),
        "cycle21_is_ancestor_of_remote_pr": git_ok(
            ["merge-base", "--is-ancestor", CYCLE21_HEAD, REMOTE_PR]
        ),
        "cycle21_remote_main_is_ancestor_of_remote_main": git_ok(
            ["merge-base", "--is-ancestor", CYCLE21_REMOTE_MAIN_HEAD, REMOTE_MAIN]
        ),
        "local_head_matches_remote_pr_head": bool(local_head and local_head == remote_pr_head),
        "local_commits_after_cycle21": git(["rev-list", "--count", f"{CYCLE21_HEAD}..HEAD"]),
        "remote_commits_after_cycle21": git(["rev-list", "--count", f"{CYCLE21_HEAD}..{REMOTE_PR}"]),
        "remote_main_commits_after_cycle21_baseline": git(
            ["rev-list", "--count", f"{CYCLE21_REMOTE_MAIN_HEAD}..{REMOTE_MAIN}"]
        ),
        "remote_main_changed_paths_after_cycle21_baseline": main_changed_paths,
        "remote_main_changed_path_classes_after_cycle21_baseline": {
            rel: path_class(rel) for rel in main_changed_paths
        },
        "remote_main_changed_paths_are_audit_drift": bool(main_changed_paths)
        and all(allowed_main_audit_drift(rel) for rel in main_changed_paths),
        ...
        "local_changed_future_paths_after_cycle21": diff_future_paths(
            CYCLE21_HEAD, "HEAD", future_paths
        ),
        "remote_pr_changed_future_paths_after_cycle21": diff_future_paths(
            CYCLE21_HEAD, REMOTE_PR, future_paths
        ),
        "remote_main_changed_future_paths_after_cycle21_baseline": diff_future_paths(
            CYCLE21_REMOTE_MAIN_HEAD, REMOTE_MAIN, future_paths
        ),
    }
...
def main() -> int:
    print("PR #230 non-chunk cycle-22 main-audit-drift reopen guard")
    print("=" * 72)
    ...
    no_changed_future_path_since_cycle21 = not any(
        [
            branch["local_changed_future_paths_after_cycle21"],
            branch["remote_pr_changed_future_paths_after_cycle21"],
            branch["remote_main_changed_future_paths_after_cycle21_baseline"],
        ]
    )
    ...
    branch_current = (
        branch["cycle21_is_ancestor_of_local_head"]
        and branch["cycle21_is_ancestor_of_remote_pr"]
        and branch["cycle21_remote_main_is_ancestor_of_remote_main"]
        and branch["local_head_matches_remote_pr_head"]
        and bool(branch["remote_main_head"])
    )
    loop_pack_records_stop = (
        "cycle-21 update" in compact_text(OPPORTUNITY_QUEUE)
        and "remote-surface reopen guard" in compact_text(HANDOFF)
        and "remote branch drift is not admissible reopen evidence" in compact_text(NO_GO_LEDGER)
        and "proposal_allowed: false" in compact_text(CLAIM_STATUS_CERTIFICATE)
    )
    main_drift_is_audit_only = (
        branch["remote_main_commits_after_cycle21_baseline"] != "0"
        and branch["remote_main_changed_paths_are_audit_drift"]
        and not branch["remote_main_changed_future_paths_after_cycle21_baseline"]
    )
    no_reopen_input = (
        source_presence_absent
        and no_local_or_remote_future_path
        and no_changed_future_path_since_cycle21
        and main_drift_is_audit_only
    )
    main_audit_drift_guard_passed = all(
        [
            not missing_parents,
            not parent_failures,
    ...
    dramatic_step_gate = {
        ...
        "branch_state": branch,
        "reason": (
            "Cycle 21 already rejects remote-surface reopen.  Since then "
            "`origin/main` advanced only on audit/effective-status surfaces and "
            "still contains none of the listed PR230 same-surface artifacts, so "
            "cycle 22 has no admissible non-chunk route to execute."
        ),
    }
    ...
    report("branch-has-cycle21-ancestor-and-remote-main-visible", branch_current, json.dumps(branch, sort_keys=True))
    ...
    report("parent-process-stack-closed", parent_process_stack_closed, "cycle-18 through cycle-21 gates deny continuation")
    report("aggregate-gates-deny-proposal", aggregate_gates_deny, f"assembly={statuses['assembly']} campaign={statuses['campaign']}")
    report("loop-pack-records-stop-contract", loop_pack_records_stop, "cycle-21 stop text present")
    report("main-drift-is-audit-only", main_drift_is_audit_only, json.dumps(branch, sort_keys=True))
    report("no-local-or-remote-listed-future-path", no_local_or_remote_future_path, json.dumps(branch, sort_keys=True))
    report("no-changed-listed-future-path-since-cycle21", no_changed_future_path_since_cycle21, json.dumps(branch, sort_keys=True))
    report("no-source-presence-key-set", source_presence_absent, json.dumps(present_by_source, sort_keys=True))
    report("cycle22-main-audit-drift-guard-passed", main_audit_drift_guard_passed, f"passed={main_audit_drift_guard_passed}")

    result = {
        "actual_current_surface_status": (
            "exact negative boundary / PR230 non-chunk cycle-22 main-audit-drift "
            "reopen guard: origin/main advanced only on audit/effective-status "
            "surfaces and no listed same-surface artifact is present; positive "
        ...
        "proposal_allowed_reason": (
            "Cycle 21 already rejects remote-surface reopen.  The cycle-21 head "
            "remains an ancestor of the aligned PR heads, origin/main advanced "
            "only on audit/effective-status paths, no listed same-surface artifact "
            "appears on branch or remote surfaces, all six worklist units remain "
            "blocked, and aggregate gates still deny proposal authority."
        ),
        "audit_required_before_effective_retained": True,
        "bare_retained_allowed": False,
        "cycle22_main_audit_drift_guard_passed": main_audit_drift_guard_passed,
        "nonchunk_scope_remains_stopped": main_audit_drift_guard_passed,
