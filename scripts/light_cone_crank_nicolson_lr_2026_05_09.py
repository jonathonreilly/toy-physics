#!/usr/bin/env python3
"""Crank-Nicolson Lieb-Robinson constant verification.

Companion to `docs/LIGHT_CONE_FRAMING_NOTE.md` rigorization
(audit row `light_cone_framing_note`, audit verdict 2026-05-03,
`audited_conditional`).

The audit verdict flagged that the note validates the staggered Dirac
dispersion v_max(m) = sqrt(m^2 + 1) - m but identifies the observed
Crank-Nicolson containment behavior with standard lattice-QFT
Lieb-Robinson behavior without deriving the LR constant for the actual
Crank-Nicolson hopping kernel.

Upstream: PR #806 / `docs/MICROCAUSALITY_FINITE_RANGE_H_AND_VLR_BRIDGE_THEOREM_NOTE_2026-05-09.md`
derived for the lattice Hamiltonian H itself:

    v_LR(H) = 2 e r J,   r = 1,   J <= |m| + 18 on canonical surface

This runner closes the remaining gap by deriving the corresponding
Lieb-Robinson constant for the **Crank-Nicolson time-evolution operator**

    U_CN = (I - i a_tau H / 2) (I + i a_tau H / 2)^(-1)        (Cayley transform)

This is a different (implicit-time-step) discretization than continuous
exp(-i t H) and admits its own LR analysis via the Neumann series for
the resolvent.
...
def test_n_step_velocity(L: int, J: float, seed: int) -> None:
    ...
    H = build_local_hamiltonian(L, J, seed)
    r = 1
    v_LR_H = 2 * math.e * r * J  # Hamiltonian-side LR velocity (PR #806)
    print(f"     PR #806 derives v_LR(H) = 2 e r J = {v_LR_H:.4f}")
    print(f"     for r = {r}, J = {J}")
    O_0 = site_operator(L, 0)
...
def main() -> int:
    print("=" * 72)
    print("CRANK-NICOLSON LIEB-ROBINSON CONSTANT VERIFICATION")
    print("=" * 72)
    print()
    print("Cites: PR #806 / MICROCAUSALITY_FINITE_RANGE_H_AND_VLR_BRIDGE_THEOREM_NOTE")
    print("       (Hamiltonian-side: v_LR(H) = 2 e r J = 2e * 1 * (|m| + 18))")
    print()
    print("This note: Crank-Nicolson time-evolution-side velocity bound")
    print("           v_LR(CN, a_tau) = v_LR(H) / (1 - a_tau J / 2)")
    print("           = v_LR(H) * (1 + O(a_tau J))   for small a_tau J")
    print()
    ...
    cert_path = (
        ...
        / "outputs"
        / "light_cone_crank_nicolson_lr_certificate_2026_05_09.json"
    )
    cert_path.parent.mkdir(parents=True, exist_ok=True)
    cert = {
        "claim_id": "light_cone_framing_note",
        ...
        "constants": {
            ...
            "J": J,
            "r": 1,
            "v_LR_H_lower_bound": 2 * math.e * 1 * J,
            "v_LR_CN_formula": "v_LR(H) / (1 - a_tau * J / 2)",
        },
        "upstream": [
            "PR #806",
