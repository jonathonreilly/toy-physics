...
def main() -> int:
    ...
    record(
        "the exact coarse bridge admits the same local affine normal form seen in the branch transport kernel",
        aff_rel_l2 < 1.0e-2,
        f"affine residual L2 ratio={aff_rel_l2:.6e}",
        status="BOUNDED",
    )
    record(
        "the nonlocal tail of the exact coarse normal form stays at the measured branch scale",
        abs(nonlocal_ratio - NONLOCAL_RATIO_OP) < 5.0e-4,
        f"normal-form nonlocal ratio={nonlocal_ratio:.6e}, branch ratio={NONLOCAL_RATIO_OP:.6e}",
        status="BOUNDED",
    )
    record(
    ...
    print(
        "On the current branch data, that exact coarse operator admits the same "
