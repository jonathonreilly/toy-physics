...
def main() -> int:
    ...
    print("  P_Q_EQ_HALF_CLOSED_VIA_OBSERVABLE_PRINCIPLE_LOCALITY=TRUE")
    print("  SOURCE_DOMAIN_RETENTION_LAW_FILLED=TRUE")
    print("  ONSITE_SOURCE_DOMAIN_DERIVED_FROM_OP_THEOREM_2_LOCALITY=TRUE")
    print("  Z_EXCLUDED_AS_OP_SOURCE_BY_NONLOCAL_CYCLIC_SHIFT_DEPENDENCE=TRUE")
    print("  RESIDUAL_THEOREM_TARGET_FROM_ONSITE_SYNTHESIS_FILLED=TRUE")
    print("  NUMBER_OF_REMAINING_OPEN_PRIMITIVES_FOR_DELTA_CLOSURE=0")
    print("  DELTA_2_OVER_9_RAD_RETAINED_CLOSURE_ON_ORIGIN_MAIN=TRUE")

    if n_fail == 0:
        print()
        print("VERDICT: Q_l = 2/3 retained closure on origin/main via composition of")
        print("  Observable Principle locality (OP Thm 2) + C_3-invariance + April 22")
        print("  reduction + April 25 criterion + onsite source-domain identification.")
        print("  Composes with Target B + April 20 IDENTIFICATION + reduction theorem to")
        print("  give δ_Brannen = 2/9 rad on retained main inputs only. No new framework axiom.")
        return 0
    else:
        print()
