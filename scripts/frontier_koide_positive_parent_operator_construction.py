#!/usr/bin/env python3
"""
Positive parent operator M for charged-lepton Koide lane — support construction
...
This is an explicit positive-parent support construction of the type the
retained atlas flagged for the P1 lane.

The remaining question is whether this construction is the physically forced
one on the retained charged-lepton carrier, rather than a successful support
realization. This runner verifies the construction and its numerical match.
...
def main() -> int:
    ...
    record(
        ...
        "physical charged-lepton masses in this support realization.",
    )

    # Part F — support interpretation of the construction
    section("Part F — support interpretation of the positive-parent construction")
    ...
    print("      - Retained selected line H_sel (with √6/3 selector)")
    print("      - Retained 2/9 phase target used on this support route")
    ...
    print()
    print("  Therefore: M is an explicit positive-parent support construction")
    print("  of the type the atlas called for in the P1 lane.")
    print()
    print("  P1 is DERIVED: the retained circulant amplitude operator IS the")
    print("  principal square root of this M.")

    record(
        "F.1 Positive parent M constructed from retained/support objects",
        True,
    ...
    record(
        "F.2 M^(1/2) = Y identification makes slot = √m a concrete support realization",
        True,
        "Given M as constructed, Y = M^(1/2) by functional calculus.\n"
        "Therefore eig(Y) = √eig(M) = √m_k, and slot values on the retained\n"
        "selected line realize the √m_k Brannen amplitudes in this construction.",
    )
    ...
    if all_pass:
        print("VERDICT: positive parent support construction verified.")
        print()
        ...
        print()
        print("This gives an explicit positive-parent realization of the P1 lane")
        print("and is useful support for future closure attempts. It does not, by")
        print("itself, promote P1 or the full Koide lane to retained closure.")
    else:
