#!/usr/bin/env python3
"""Canonical corridor equivalence scan for center-spine bucket00 rescue closure."""
...
def main() -> None:
    ...
    print(f"center-spine bucket00 pocket corridor equivalence scan started {started}", flush=True)
    ...
    family_matches = [row for row in exact_rows if row["rescue_match_family"]]
    family_matches.sort(
        key=lambda row: (
            row["term_count"],
            -row["interval"][2],
    ...
    print()
    print("Center-Spine Bucket 00 Pocket Corridor Equivalence Scan")
    print("========================================================")
    ...
    print()
    print("Fixed baseline and family target")
    print("-------------------------------")
    print(f"baseline: {baseline.text}")
    print(f"baseline_missed_rows: {', '.join(_mask_names(rows, miss_mask))}")
    print(f"family_law: {family_law.text}")
    print(f"family_rescue_rows: {', '.join(_mask_names(rows, family_rescue))}")
    print()
    ...
    if missing:
        ...
        for name in missing:
            print(f"- {name}")
    print()
    print("Canonical clause counts")
    print("-----------------------")
    print(f"canonical_weight_classes_scanned: {len(seen_weight_classes)}")
    print(f"exact_clauses_found: {len(exact_rows)}")
    print(f"exact_family_mask_matches: {len(family_matches)}")

    if family_matches:
        min_terms = min(row["term_count"] for row in family_matches)
        min_term_rows = [row for row in family_matches if row["term_count"] == min_terms]
        print(f"minimal_term_count_for_exact_family_match: {min_terms}")
        print(f"minimal_term_match_count: {len(min_term_rows)}")
        print(f"minimal_term_unique: {len(min_term_rows) == 1}")

        by_term: dict[int, int] = {}
        for row in family_matches:
            by_term[row["term_count"]] = by_term.get(row["term_count"], 0) + 1
        print("family_match_count_by_term:")
        for term in sorted(by_term):
            print(f"- terms={term}: {by_term[term]}")
    else:
        print("minimal_term_count_for_exact_family_match: none")
        print("minimal_term_match_count: 0")
        print("minimal_term_unique: False")

    print()
    print("Top exact family-mask matches (rank: term_count then interval width)")
    print("---------------------------------------------------------------")
    if not family_matches:
        print("No exact family-mask matches found in bounded canonical corridor basis.")
    else:
        for idx, row in enumerate(family_matches[: args.top_k], start=1):
            print(
    ...
    print()
    print(
        "center-spine bucket00 pocket corridor equivalence scan completed "
