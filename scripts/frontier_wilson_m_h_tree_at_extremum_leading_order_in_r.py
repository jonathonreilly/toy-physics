...
# ---------------------------------------------------------------------------
# Part 3: Cited upstream files (with graceful same-batch handling)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Part 7: Leading-order matching value r ≈ 0.235 (with PDG as comparison only)
# ---------------------------------------------------------------------------
def part7_leading_closure_value():
    section("Part 7: leading-order matching value r ≈ 0.235 (PDG = comparison only)")
    ...
    # This is the LEADING-ORDER (linear-in-r^2) comparison equation. It is
    ...
    # not a derivation of r; it is the value that would match the comparison
    # mass at leading order under the channel admission.
    ...
    check(
        "leading-order matching value r ≈ 0.235 (with u_0 = 0.8776)",
...
def part10_boundary_check():
    ...
    # Critical: the leading-order matching value r ≈ 0.235 is FLAGGED as
    # conditional on (1) the channel admission, (2) perturbative validity,
    # (3) non-zero Wilson coefficient.
    check(
        "matching value r ≈ 0.235 explicitly flagged as conditional on channel admission",
        "uniform-`N_taste = 16`" in NOTE_TEXT and "non-derived" in NOTE_TEXT,
    )
    check(
        "matching value flagged as conditional on perturbative-expansion validity",
        "perturbative expansion fails" in NOTE_TEXT or "leading-order Taylor expansion (which fails at" in NOTE_TEXT,
    )
    check(
        "matching value flagged as conditional on non-zero Wilson coefficient r",
...
def main() -> int:
    ...
    print("   140.3 GeV. Leading-order comparison match at r ≈ 0.235, conditional on")
    ...
    if FAIL == 0:
        ...
        print(" Leading-order matching to the comparison mass occurs at r ≈ 0.235 with u_0 ≈")
