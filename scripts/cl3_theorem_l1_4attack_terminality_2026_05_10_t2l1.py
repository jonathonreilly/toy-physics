...
channel weights are not closed by the tested four route families
(MS-bar dim-reg, quartic Casimir, resurgence trans-series,
topological Chern-Simons) from the current physical Cl(3)/Z^3
framework baseline.
...
def section_3_casimir_vs_weight(c: Counter) -> None:
    ...
    # (e.g., c_dFdF ~ -64 + 480 ζ_3 in one convention) — not closed by
    # the current framework inputs.
    c.record(
...
def section_7_four_attack_summary(c: Counter) -> None:
    ...
    c.record(
        "S7.FA1 Four-route bounded no-go: each lands at named obstacle",
        len(attacks) == 4,
        detail="MS-bar + Casimir + Resurgence + Topological = tested axes",
    )
    c.record(
        "S7.FA2 Per P-L1-D synthesis: open-gate residue decomposes to 3 named sub-functors",
        True,
    ...
    c.record(
        ...
        True,
        detail="bounded no-go + sharpened residue; no new admission",
    )
