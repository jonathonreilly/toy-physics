#!/usr/bin/env python3
"""Bounded impact-parameter portability probe on retained grown rows.
...
This stays deliberately narrow:
  ...
  - fixed field scale and fixed source layer
  - one retained b sweep shared across rows
  - center portable row plus one additional grown family

The readout is the same detector-line centroid shift used by the retained
grown-geometry distance-law replay, but organized as an explicit portability
probe.
"""
...
FAMILIES = [
    ...
    FamilyCase("grown family 2", 0.05, 0.30),
]
...
def propagate(
    ...
    for i in order:
        ...
        for j in adj.get(i, []):
            ...
            amps[j] += ai * cmath.exp(1j * K * act) * w * h2 / (L * L)
    return amps
...
def _print_row(case: FamilyCase) -> FamilySummary:
    means, null_delta, toward, total, _ = _run_row(case)
...
def main() -> None:
    print("=" * 92)
    print("IMPACT-PARAMETER PORTABILITY PROBE")
    print("  exact/null control first; center portable row plus one additional grown family")
    print("=" * 92)
    ...
    if retained:
        ...
        print(
            "  the centroid readout supports a stable impact-parameter law on "
            "the tested portable grown rows"
        )
    else:
        print(
            "  closure: the centroid readout does not support a stable "
            "impact-parameter law on the tested portable grown rows"
        )
    print("=" * 92)
