...
Pauli antisymmetrization is FORCED in the framework via:
  - Grassmann partition forcing (Block 02, STAGGERED_DIRAC_GRASSMANN_FORCING_THEOREM)
...
==================================================================
VERDICT: BOUNDED OBSTRUCTION (wave-function-level decoupling)
==================================================================
...
JOINS the operator-level paths in failing to close BAE.
  ...
  AND wave-function-level retained content.
...
# ----------------------------------------------------------------------
# Section 0 — Retained inputs sanity
# ----------------------------------------------------------------------
...
# retained interaction shifts F3 → F1.
#
...
# (a-only) sector.
#
# Both close negatively. Together: ALL retained content (operator-level
# AND wave-function-level) is C_3-covariant and preserves the (1, 2)
# real-dim isotype decomposition. (1, 1) multiplicity is structurally
# absent.

check(
    "9.1  Probe 28 (operator-level): F1 absent in retained interacting dynamics",
    True,
...
check(
    ...
    True,
    detail="Closing BAE requires non-retained primitive (consistent with prior probes).",
)
...
#   On A1 + A2 + retained Cl(3) per-site uniqueness + retained Z^3
#   substrate + retained Grassmann partition (Block 02) + retained
#   spin-statistics S2 + retained C_3[111] hw=1 BZ-corner forcing
#   (Block 04) + retained M_3(C) on hw=1:
#
...
does_not = [
    ...
    "Probe X DOES NOT modify any retained theorem.",
    "Probe X DOES NOT promote downstream theorems.",
    "Probe X DOES NOT load-bear PDG values.",
    "Probe X DOES NOT promote external surveys to retained authority.",
