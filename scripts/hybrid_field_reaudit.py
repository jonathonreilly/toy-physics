#!/usr/bin/env python3
"""Hybrid-field re-audit on the retained 3D modular family.

Goal
----
Revisit the older hybrid-field fixed-mass seam under the current
control-clean fit discipline:
  - same retained 3D modular family assumptions as the original pilot
  - fixed-position / fixed-count mass controls
  - full-sweep-positive fit gate
  - same detector readout and k-band across all modes

The point is not to rescue the distance law. The point is to verify whether
the hybrid sits genuinely between the retained endpoints, or whether it was
an accidentally favorable mover under the earlier looser fit gate.

Review discipline:
  - if a mode fails the full-sweep-positive gate, say so plainly
  - if the hybrid sits between endpoints, do not overclaim
  - if a mode weakens one axis while helping the other, say that plainly

PStack experiment: hybrid-field-reaudit
"""
...
def measure_b_sweep(mode_mix: float):
    ...
    fit = _fit_full_positive_power_law(list(TARGET_BS), by_b)
...
def measure_mass_sweep(mode_mix: float):
    ...
    fit = _fit_full_positive_power_law(list(MASS_COUNTS), by_mass)
    return label, rows, fit
...
def main() -> None:
    print("=" * 78)
    print("HYBRID FIELD RE-AUDIT")
    print("  Retained 3D modular DAG family")
    print("  Same geometry / fixed-position controls / full-sweep-positive fit gate")
    print("=" * 78)
    print()
    print("Setup:")
    ...
    det_list = list(layer_indices[-1])
    center_y = statistics.fmean(p[1] for p in positions)
    mass_nodes = select_fixed_mass_nodes(
    ...
    for mix in MIXES:
        ...
        summaries.append((mix, label, b_fit, m_fit, b_rows, m_rows))

    print("=" * 78)
    print("SUMMARY")
    lap_gamma = None
    lap_alpha = None
    for mix, label, b_fit, m_fit, b_rows, m_rows in summaries:
    ...
    print("  Only mixes with full-sweep-positive fits are eligible for a retained claim.")
    print("  If the hybrid sits between the endpoints on one axis but not both, keep")
    print("  the wording narrow.")
    if lap_gamma is not None and lap_alpha is not None:
        print(f"  Laplacian baseline: gamma={lap_gamma:.3f}, alpha={lap_alpha:.3f}")
    print("=" * 78)
