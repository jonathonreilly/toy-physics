#!/usr/bin/env python3
"""5D connectivity diagnostic.

This is a bounded diagnostic for the weak / noisy 5D unified result.
The question is whether the 5D failure is mostly a sparsity / connectivity
problem or a true physics failure.

We keep the geometry simple and sweep a small set of graph-density knobs:
- nodes_per_layer
- connect_radius
- spatial_range

5D here means 4 spatial dimensions plus the causal x-layer direction.
The sweep reports:
- valid setup rate
- rough connectivity statistics
- whether a positive mass-law signal appears anywhere

This is intentionally not a heroic search.
"""
...
def generate_5d_modular_dag(
    ...
    """Generate a 5D modular DAG: x plus four spatial dimensions.

    Post-barrier nodes are channelized in y, while the remaining transverse
    dimensions stay uniform. This mirrors the 3D/4D modular generators.
    """
...
def fit_alpha(pairs):
    positive = [(n, s) for n, s, _, _ in pairs if n > 0 and s > 0]
    if len(positive) < 3:
        return None
...
def measure_config(nodes_per_layer, connect_radius, spatial_range, gap=GAP, n_seeds=N_SEEDS):
    """Measure connectivity and mass-law signal for one 5D parameter set."""
    k_band = K_BAND
...
def main():
    print("=" * 78)
    print("5D CONNECTIVITY DIAGNOSTIC")
    print("  4 spatial dims + 1 causal dim")
    print("  Goal: sparsity/connectivity check, not a heroic search")
    print("=" * 78)
    print()
    ...
    for nodes_per_layer, connect_radius, spatial_range in configs:
        row = measure_config(nodes_per_layer, connect_radius, spatial_range)
        rows.append(row)
    ...
    for row in rows:
        alpha_str = "NA" if row["alpha"] is None else f"{row['alpha']:.3f}"
        print(
            f"  nodes={row['nodes_per_layer']:2d}, rad={row['connect_radius']:3.1f}, "
            f"range={row['spatial_range']:3.1f} -> alpha={alpha_str}, "
            f"valid={row['valid_rate']:.2f}, reach={row['avg_reach']:.3f}"
        )
        if row["mass_summaries"]:
            top = row["mass_summaries"][-1]
            print(
                f"    best mass point: n={top[0]}, shift={top[1]:+.4f}, "
                f"SE={top[2]:.4f}, t={top[3]:+.2f}"
            )
    ...
    if best_alpha_row and best_alpha_row["alpha"] is not None and best_alpha_row["alpha"] > 0.2:
        if best_alpha_row["max_t"] > 2.0:
            print(
                "  A measurable positive mass-law signal appears in the densest"
                "  5D settings, which points to a connectivity problem rather than"
                "  a hard physics failure."
            )
        else:
            print(
                "  A weak positive mass-law slope appears in the densest 5D"
                "  settings, but the t-stats remain sub-significant."
            )
        print(
        ...
        print(
            "  No stable positive mass-law signal emerged in this bounded sweep."
            "  The 5D result still looks connectivity-limited, but this run does"
            "  not recover a robust positive exponent."
        )
