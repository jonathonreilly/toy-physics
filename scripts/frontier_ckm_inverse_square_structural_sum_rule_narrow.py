...
states a purely abstract polynomial-algebra theorem on:
  ...
  - abstract positive real symbols (n_pair, n_color, eta, rho, A)
    satisfying the parametric input identities
...
Conclusions:
  ...
  (T4) positivity gate: eta^2 > 0 iff n_color > n_pair;
...
def main() -> int:
    ...
    # ------------------------------------------------------------------
    # Setup: abstract positive symbols and parametric hypothesis system
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    section("Part 4: (T4) positivity gate eta^2 > 0 iff n_color > n_pair")
    # ------------------------------------------------------------------
  ...
  HYPOTHESIS:
    Let (n_pair, n_color, eta, rho, A) be any abstract positive real
    symbols satisfying
        (H1)  rho      = 1 / (n_pair * n_color),
        (H2)  A^2      = n_pair / n_color,
        (H3)  eta^2    = 1 / n_pair^2  -  1 / n_color^2.
  ...
  Audit-lane class:
    (A) — pure polynomial algebra over positive real symbols. No Cl(3),
    Z^3, CKM atlas, projector split, alpha_s, Wolfenstein coordinate
    identification, charged-lepton, staggered-Dirac, PMNS, PDG observable,
    or specific count tuple is consumed.
