#!/usr/bin/env python3
"""Exact support-side attack on the tensor-observable blocker.

This runner asks whether the retained axiom-side machinery already contains a
nonzero exact tensor observable on the bright block

    A1 x {E_x, T1x}

without using the numerical `eta_floor_tf` residual pipeline.

It checks three exact support-side structures:
  1. the support Green matrix / support Schur Hessian in the adapted basis,
  2. the mixed A1-bright block of that exact Hessian,
  3. the exact support-to-active response operator.

If the support Hessian is block diagonal, the mixed A1-bright block vanishes,
and the support-to-active operator is rank one / charge-only, then the retained
support-side stack cannot produce the desired exact tensor observable.
"""
...
def support_green_matrix() -> tuple[np.ndarray, np.ndarray]:
    ...
    gs = g0p[support, :]
    return gs, np.array(support, dtype=int)
...
def main() -> int:
    print("Exact support-side tensor-observable attack")
    print("=" * 78)

    gs, _ = support_green_matrix()
    ...
    gs_adapted = basis.T @ gs @ basis
    mixed = a1.T @ gs @ bright
    mixed_norm = float(np.max(np.abs(mixed)))

    print("Support Green matrix in adapted basis:")
    print(np.array2string(gs_adapted, precision=6, floatmode="fixed"))
    print("\nMixed A1-bright block (candidate exact tensor observable):")
    ...
    active_e_x = active_op @ e_x
    active_t1x = active_op @ t1x
    active_bright_norm = max(
        float(np.max(np.abs(active_e_x))),
        float(np.max(np.abs(active_t1x))),
    )
    ...
    print(f"max |active_op @ E_x| = {float(np.max(np.abs(active_e_x))):.3e}")
    print(f"max |active_op @ T1x| = {float(np.max(np.abs(active_t1x))):.3e}")

    # Bright-channel directional checks on the exact support scalar.
    q_family = [
        ("e0", e0),
    ...
    print("\nVerdict:")
    print(
        "The retained support-side machinery is scalar/rank-one on the current "
        "A1 block. Its exact Hessian has no mixed A1-bright tensor block, its "
        "support-to-active response is charge-only, and the exact support scalar "
        "delta_A1 is blind to E_x and T1x. So the retained stack cannot produce "
        "a nonzero exact tensor observable on A1 x {E_x, T1x}; the tensor "
        "endpoint coefficients still require a new microscopic tensor operator "
        "or the numerical eta_floor_tf path."
    )
