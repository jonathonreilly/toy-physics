...
def _source_driven_field_layers_raw(lat: Lattice3D, source_strength: float, z_src: float) -> list[list[float]]:
...
def main() -> None:
    ...
    ref_raw = _source_driven_field_layers_raw(lat, max(SOURCE_STRENGTHS), SOURCE_Z)
    ref_max = _field_abs_max(ref_raw)
    field_gain = TARGET_FIELD_MAX / ref_max if ref_max > 1e-30 else 1.0

    # Reduction check.
    zero_dynamic = _scale_field_layers(_source_driven_field_layers_raw(lat, 0.0, SOURCE_Z), field_gain)
    ...
    print(f"  fixed dynamic field gain: {field_gain:.6e}")
    print()
    ...
    for s in SOURCE_STRENGTHS:
        ...
        dyn_field = _scale_field_layers(_source_driven_field_layers_raw(lat, s, SOURCE_Z), field_gain)
