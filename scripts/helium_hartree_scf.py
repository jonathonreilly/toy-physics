#!/usr/bin/env python3
"""Helium atom from Cl(3)/Z³ axioms — variational two-body problem.
...
Steps 1–3: Same as hydrogen (see hydrogen_from_graph_dynamics.py).
...
This IS the Hartree equation, but it is derived here as the first-order
stationarity condition of E[φ] — NOT imported from standard QM.
...
Framework prediction (variational bound):
    E(He) / E(He⁺) ~ 1.424    (coupling-independent)
    Exact (full CI): E(He) / E(He⁺) ~ 1.452

The difference (2%) measures the quality of the product-state ansatz.
...
BLOCKERS:
  1. α_EM (bounded): cannot convert to eV
  2. Electron mass m_e (open): same
  3. Exact two-body solution: N⁶ problem, not tractable
  4. Exchange-correlation beyond product-state ansatz: open lane
==========================================================================
"""
...
def run_experiment() -> None:
    log("=" * 72)
    log("HELIUM ATOM FROM Cl(3)/Z³ AXIOMS — VARIATIONAL BOUND")
    log("Hartree derived as variational stationarity condition, not QM import")
    log("=" * 72)
    log()
    ...
    # ------------------------------------------------------------------
    log("=" * 72)
    log("SUMMARY: Cl(3)/Z³ HELIUM PREDICTIONS")
    log("=" * 72)
    log()
    ...
    log(f"  {'Quantity':>22}  {'Lattice':>9}  {'Target':>9}  {'err%':>7}  Status")
    log("  " + "─" * 62)
    for label, val, ref, note in rows:
        if ref is not None and abs(ref) > 1e-10:
            err = 100.0 * (val - ref) / abs(ref)
            # IE₁/IE₂ is a difference: allow wider tolerance
            tol = 25.0 if "IE₁" in label else 8.0
            status = "PASS" if abs(err) < tol else "WARN"
    ...
    log()
    log("  BLOCKED:")
    log("  ✗ Absolute energy in eV: requires α_EM (bounded ~27%) × m_e (open)")
    log("  ✗ Exchange-correlation: goes beyond product-state ansatz (new lane)")
    log("  ✗ Exact two-body solution: N⁶ lattice, not tractable")
    log()
    log("  HISTORICAL CHECKPOINTS:")
    ...
    log_path = f"logs/{time.strftime('%Y-%m-%d')}-helium_variational.txt"
    try:
