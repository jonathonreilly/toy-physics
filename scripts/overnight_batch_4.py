#!/usr/bin/env python3
"""Overnight batch 4: alpha bias check + dense push to N=150.

A. Alpha bias check: use FIXED mass count (8) across all mass points
   in the scaling sweep. If alpha stays >1 with fixed count, the boost
   is real. If it drops, the variable mass count was inflating.

B. Dense+prune push to N=120-150: how far can we go?

PStack experiment: overnight-batch-4
"""
...
BETA = 0.8; K_BAND = (3.0, 5.0, 7.0); N_YBINS = 8; LAM = 10.0
...
def main():
    print("=" * 74)
    print("OVERNIGHT BATCH 4: Alpha bias check + dense N=150 push")
    print("=" * 74)
    print()

    # A: Fixed-count alpha check
    print("EXPERIMENT A: Fixed mass count=8 across all measurements")
    print("  (Previous sweep used variable count 1-16 → may inflate alpha)")
    print()

    # Measure shift at FIXED mass=8 for varying b (mass position)
    # This is NOT a mass scaling test — it's a shift vs position test
    # The real alpha test needs variable mass count with SAME position
    # Let me do it properly: fix position, vary count
    n_seeds = 24
    mass_counts = [2, 4, 6, 8, 12, 16]

    for label, use_prune in [("Uniform raw", False), ("Uniform + prune", True)]:
        print(f"  [{label}]")
        ...
        for target_n in mass_counts:
            ...
            for seed in range(n_seeds):
                positions, adj, layers = gen_3d(n_layers=15, rng_seed=seed*17+3)
                if use_prune:
                ...
                # FIXED position: always at cy+3 (same for all mass counts)
                cands = sorted([i for i in layers[mid] if positions[i][1] > cy+2],
                               key=lambda i: abs(positions[i][1]-(cy+3)))
                mn = cands[:target_n]
            ...
            if per_seed:
                ...
                if avg > 0: results.append((target_n, avg))
            else: print(f"  {target_n:6d}  FAIL")
        ...
        if len(results) >= 3:
            ...
            if abs(denom) > 1e-10:
                ...
                print(f"\n  Alpha (fixed position): {alpha:.3f}")
        print()

    # B: Dense push to N=120-150
    print("EXPERIMENT B: Dense 3D + prune push to N=120-150")
    print()
    for label, use_prune in [("Dense baseline", False), ("Dense + prune", True)]:
        print(f"  [{label}]")
        print(f"  {'N':>4s}  {'pur_cl':>8s}  {'n':>3s}")
        print(f"  {'-'*18}")
        for nl in [80, 100, 120, 150]:
            purs = []
            for seed in range(16):
                positions, adj, layers = gen_3d(n_layers=nl, npl=60, r=2.7, rng_seed=seed*13+5)
                if use_prune:
                    adj, _ = adaptive_prune(positions, adj, layers, quantile=0.10, max_iter=3)
                pur = measure_decoherence(positions, adj, layers)
                if not math.isnan(pur): purs.append(pur)
            if purs: print(f"  {nl:4d}  {sum(purs)/len(purs):8.4f}  {len(purs):3d}")
