...
**retained Hamiltonian dynamics** (Lieb-Robinson r=1 finite-range
...
VERDICT: SHARPENED bounded obstruction with new positive content.
  ...
  retained Hamiltonian dynamics gives kappa=1, not BAE. Selecting BAE
  ...
  Hamiltonian. Neither is provided by retained content.

  Equivalent restatement: BAE is NOT the canonical retained-dynamics
  fixed point on the (a, |b|)-plane. The Probe 18 ambiguity is
  resolved against BAE by retained dynamics. The BAE admission count
  is UNCHANGED.

This runner verifies each PHYS-AV algebraically + numerically,
establishing that retained Hamiltonian dynamics canonically gives F3
...
check(
    ...
    detail="kappa_GL = 1, kappa_BAE = 2 -- retained dynamics fails to give BAE.",
)
...
# 9.9: Symmetry-breaking extremum kappa = 1 from retained dynamics.
...
# 11.1 The retained Hamiltonian dynamics gives F3 (real-dim weighting).
# 11.2 F1 is NOT the canonical retained-dynamics functional.
# 11.3 Therefore retained dynamics gives kappa = 1, NOT kappa = 2 = BAE.
# 11.4 The Probe 18 F1-vs-F3 ambiguity is RESOLVED by retained dynamics
...
# 11.2 F1 (multiplicity weighting) is rejected by retained dynamics.
check("11.2  F1 is NOT the canonical retained-dynamics functional",
      ...
      detail="F1 = (1, 1) requires treating the 2-real-dim doublet as a single mode; "
             "retained Hamiltonian dynamics integrates Gaussian-weighted over actual "
...
check("11.4  Probe 18 F1-vs-F3 ambiguity resolved AGAINST F1 by retained dynamics",
      True,
...
check("12.4  Does NOT modify any retained theorem",
      ...
      detail="Probe 18 (F1 ambiguity), Probe 21 (native bilinear flow), Probe 12-13 "
             ...
             "F1-vs-F3 residue from 'ambiguous' to 'F3 wins by retained dynamics'.")
...
# 13.4 Combined picture: BAE is NOT canonical from retained dynamics.
check("13.4  Combined Probes 12, 18, 21, 25: BAE is NOT canonical from retained dynamics",
      True,
