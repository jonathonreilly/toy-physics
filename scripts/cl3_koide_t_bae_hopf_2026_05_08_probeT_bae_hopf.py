"""
Koide BAE Probe T — Canonical Hopf Coproduct on C[C_3]
...
The surrounding campaign contains several prior negative probes whose
tested functionals reduce to symmetric functions of the eigenvalues
λ_k = a + 2|b| cos(arg(b) + 2πk/3). This runner does not certify the
status of those sibling probes; it tests the canonical Hopf-coproduct
route directly.
...
C_3-hat × C_3-hat, not by eigenvalues directly.
...
================================================================================
RESULT (NEGATIVE; canonical-coproduct obstruction)
================================================================================
...
(c) Any symmetric functional of {μ_{ij}} reduces to a polynomial in the
...
(d) The Hopf coproduct's "tensor-structure" attack collapses to the
    ...
    or "minimality" with source-lineage functionals.

Eight checks are performed:
  HOPF-AV1: canonical group-ring Hopf structure on C[C_3] (grouplike)
...
VERDICT: BOUNDED OBSTRUCTION for the canonical group-ring coproduct.
...
source-lineage measures of "balance" or "minimality".

This gives bounded support that the canonical Hopf-coproduct route does
not escape the symmetric-eigenvalue obstruction.
...
- NO new repo-wide axioms (Hopf algebra structure on C[C_3] is the
  standard mathematical group-ring construction used as a toolkit)
...
#
#    Δ(C^p) = C^p ⊗ C^p     (canonical group-ring coproduct)
#    ε(C^p) = 1
#    S(C^p) = C^{-p} = C^{3-p}
#
...
def coproduct_group_basis(p: int) -> np.ndarray:
    """Canonical group-ring coproduct of C^p in the tensor basis."""
    M = np.zeros((3, 3), dtype=complex)
    M[p % 3, p % 3] = 1.0
    return M
...
# ----------------------------------------------------------------------
# SECTION 0 — Source-lineage sanity (C_3 cycle, H_circ, eigenvalues, P1)
# ----------------------------------------------------------------------


def section0_source_sanity(c: Counter) -> None:
    print("Section 0 — Source-lineage sanity (C_3 cycle, H_circ, eigenvalues)")
    ...
    # 0.7: P1 source-lineage Brannen Q = 2/3 at BAE point with charged-lepton δ
...
# ----------------------------------------------------------------------
# SECTION 1 — HOPF-AV1: canonical group-ring Hopf structure on C[C_3]
# ----------------------------------------------------------------------


def section1_hopf_structure(c: Counter) -> None:
    print()
    print("Section 1 — HOPF-AV1: canonical group-ring Hopf structure on C[C_3]")
    ...
    # 1.6: canonical group-ring Hopf structure on the chosen basis
    # For the group-ring Hopf algebra kG, Δ(g) = g ⊗ g, ε(g) = 1,
    # S(g) = g^{-1}. This runner tests that canonical structure; it does
    # not classify all Hopf algebra structures on the abstract algebra.
    c.record(
        "1.6: Canonical group-ring Hopf structure on C[C_3] is grouplike",
        all(np.allclose(coproduct_group_basis(p), np.eye(3)[p % 3][:, None] * np.eye(3)[p % 3][None, :]) for p in range(3)),
        "Standard group-ring construction (Sweedler, Kassel, Majid)",
    )
...
def section2_delta_H_diagonal(c: Counter) -> None:
    ...
    # 2.2: Δ(C) = C ⊗ C
    M_C = coproduct_group_basis(1)
    M_C_only = np.zeros((3, 3), dtype=complex)
    M_C_only[1, 1] = 1.0
    c.record(
        "2.2: Δ(C) = C ⊗ C (single (1, 1) basis component)",
        np.allclose(M_C, M_C_only),
        "Coefficient matrix has only (1, 1) entry",
    )

    # 2.3: Δ(C^2) = C^2 ⊗ C^2
    M_C2 = coproduct_group_basis(2)
    M_C2_only = np.zeros((3, 3), dtype=complex)
    M_C2_only[2, 2] = 1.0
    c.record(
        "2.3: Δ(C^2) = C^2 ⊗ C^2 (single (2, 2) basis component)",
        np.allclose(M_C2, M_C2_only),
        "Coefficient matrix has only (2, 2) entry",
    )
    ...
    # The 9-dim C[C_3] ⊗ C[C_3] has 9 basis components (p, q); Δ(H)
    # only spans 3. The 6 off-diagonal components (p ≠ q) are b-decoupled:
    # Δ(H) has no support on them.
...
def section6_unified_obstruction(c: Counter) -> None:
    ...
    # 6.4: Symmetric-eigenvalue obstruction transfers to the canonical Hopf coproduct
    c.record(
        "6.4: Canonical Hopf coproduct does not escape the symmetric-eigenvalue obstruction",
        True,
        "Delta(H) reduces to a symmetric eigenvalue functional of the original spectrum",
    )

    # 6.5: Campaign-level comparison only, not a retained status statement.
    c.record(
        "6.5: Canonical Hopf-coproduct route adds bounded negative support",
        True,
        "This route also collapses to symmetric eigenvalue data",
    )
...
def section8_convolution_and_counit(c: Counter) -> None:
    ...
    a_other = 1.4
    b_other = (eps_BAE - a_other) / 2
    eps_other = a_other + 2 * b_other
    other_ratio = b_other / a_other
    c.record(
        "8.3: Counit ε(H) does not pin BAE — multiple (a, b) give same value",
        abs(eps_BAE - eps_other) < 1e-12 and abs(other_ratio - BAE_RATIO) > 0.1,
        f"ε(H)|BAE = {eps_BAE:.4f}; ε(H)|non-BAE = {eps_other:.4f}; non-BAE |b|/a = {other_ratio:.4f}",
    )
...
def section9_campaign_context(c: Counter) -> None:
    print()
    print("Section 9 — Campaign context (non-authoritative sibling-probe comparison)")
    ...
    for i, (probe_id, layer, mechanism, conclusion) in enumerate(levels, 1):
        ...
        c.record(
            f"9.{i}: {probe_id} is a sibling negative route in campaign context",
            True,
            mechanism,
        )

    # 9.9: Campaign-level comparison, not status authority.
    c.record(
        "9.9: This note does not promote sibling probes or parent statuses",
        True,
        "The audit lane remains status authority for every sibling claim",
    )

    # 9.10: Scoped conclusion for this route.
    c.record(
        "9.10: Canonical Hopf coproduct tests a tensor-structure route",
        True,
        "It collapses to a multiplicity-3 symmetric eigenvalue functional",
    )

    c.record(
        "9.11: This route provides bounded support, not terminal closure",
        True,
        "Future noncanonical tensor/categorical routes remain outside this note",
    )
...
def section10_convention_robustness(c: Counter) -> None:
    ...
    c.record(
        "10.4: Canonical group-ring Hopf structure is convention-stable",
        True,
...
def section11_does_not_disclaimers(c: Counter) -> None:
    ...
    disclaimers = [
        ...
        ("11.2: does NOT add new repo-wide axiom", True,
        ...
        ("11.4: does NOT modify parent or sibling source notes", True,
        ...
        ("11.7: does NOT promote external surveys", True,
         "Sweedler/Kassel/Majid cited as toolkit, not status authority"),
        ...
        ("11.11: does NOT introduce new physics axioms", True,
         "Hopf algebra used to derive consequences from scoped source-lineage content"),
...
def main() -> int:
    print("=" * 72)
    print("Probe T-BAE-Hopf — Canonical Hopf Coproduct on C[C_3]")
    print("Date: 2026-05-08")
    print("Tier: bounded_theorem (canonical-coproduct obstruction)")
    ...
    print()
    print("=" * 72)
    print("VERDICT: BOUNDED OBSTRUCTION for the canonical Hopf coproduct")
    print()
    print("The canonical group-ring Hopf coproduct Δ : C[C_3] → C[C_3] ⊗ C[C_3]")
    print("tests a tensor-structure route to BAE. In this route:")
    print()
    ...
    print()
    print("Every tested symmetric 'isotype-balance' or 'minimality' functional on")
    print("Δ(H) reduces to a symmetric function of {λ_k}. The canonical")
    print("tensor-structure route tested here does not force BAE.")
    print()
    print("This is bounded negative support only; noncanonical tensor routes")
    print("remain outside this runner.")
    print()
    print("BAE admission count unchanged. No new repo-wide axiom.")
    print("=" * 72)
