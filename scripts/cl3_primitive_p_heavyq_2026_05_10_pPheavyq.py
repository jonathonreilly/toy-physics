...
Each candidate is tested against PDG masses and the existing framework
context for m_t = 169.5 GeV (-1.84%). PDG masses are observational
...
- No fitted constants promoted to the retained surface.
...
# Physical Cl(3) local algebra / Z^3 spatial substrate source-stack constants
...
def test_p_heavy_a():
    ...
    print()
    print("Reference (charged-lepton) BAE: rho_lep = sqrt(2) ≈ 1.4142")
    ...
    # existing Brannen-Rivero / BAE plus square-root mass context)
    ...
    check(
        "P-Heavy-A T1: charged-lepton rho fit consistent with sqrt(2) (BAE)",
    ...
    check(
        "P-Heavy-A T2: charged-lepton delta fit consistent with 2/9 (existing context)",
        delta_lep_dist < 0.05,
...
def test_p_heavy_b():
    ...
    print(f"y_t(M_Pl) Ward-identity context = {Y_T_M_PL_RETAINED:.5f}")
    print()
...
def test_p_heavy_c():
    ...
    check(
        "P-Heavy-C T1: at least 2/5 fermions match Casimir-product factorization at 5%",
        sub5 >= 2,
        detail=f"sub-5% fits = {sub5}/5"
    )
    check(
        "P-Heavy-C T2: at least 4/5 fermions match Casimir-product factorization at 20%",
        sub20 >= 4,
...
def main():
    ...
    print()
    print("Framework constants (cited as existing context):")
    ...
    print(f"  Ward context: y_t(M_Pl) = g_lattice/sqrt(6) = {Y_T_M_PL_RETAINED:.6f}")
    ...
    print(f"  - P-Heavy-C: 2/5 fermions match Casimir factorization at 5%")
    print(f"    (b at 0.25%, c at 0.02%); 4/5 match at 20%. Density-of-")
