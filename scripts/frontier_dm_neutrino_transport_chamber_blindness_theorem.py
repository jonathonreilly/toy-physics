#!/usr/bin/env python3
"""
Selector physics validation: eta/eta_obs evaluated at each Z_3 doublet-block
...
Question
--------
  The selector integration branch records three obstruction theorems and one
...
------------------------------------------------------
  ...
  The closure of selector via "eta/eta_obs picks a unique point on the
...
Verdict
-------
  selector NOT CLOSED. Narrower-gap: the atlas-native transport chain
...
---------------------
  ...
  All machinery used is already retained on . No
...
def candidates() -> list[tuple[str, float, float, float, str]]:
  ...
  return [
    (
      ...
      SQRT6_3,
      SQRT6_3,
      "Schur-Q chamber-boundary minimum [SELECTOR_3_DOUBLET_BLOCK_SELECTOR_SCHUR_BASELINE_PARTIAL_CLOSURE]",
    ),
    (
      "B",
      0.613372,
      0.964443,
      1.552431,
      "det(H) chamber-interior critical point [SELECTOR_ATH_C_HOLONOMY]",
    ),
    (
      "C",
      0.385132,
      1.267881,
      0.365112,
      "Tr(H^2) chamber-boundary minimum [SELECTOR_ATH_C_HOLONOMY] (boundary point)",
    ),
    (
      "D",
      0.0,
...
def part3_chamber_scan(retained_ratio: float) -> dict:
  ...
  # all four selector candidates and is well-separated from the normal-form branch
  # singularities.
  n_grid = 25
...
def part4_verdict(retained_ratio: float, records: list[dict]) -> None:
  ...
  print()
  print(" VERDICT: selector NOT CLOSED BY PHYSICS-VALIDATION TEST.")
