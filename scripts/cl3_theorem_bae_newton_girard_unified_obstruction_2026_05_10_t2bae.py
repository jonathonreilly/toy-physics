...
does not supply a selector for the C_3 isotype-weight choice
...
without importing that selector through the chosen functional or
...
def section_6_eight_attack_collapse(c: Counter) -> None:
    ...
    c.record(
        "S6.EA1 All 8 tested-route summaries classified as case (i) or (ii)",
        all_classified,
        detail=f"case (i): {case_i_count}, case (ii): {case_ii_count}, sum = 8",
    )
    c.record(
        "S6.EA2 Tested route summaries cover cases (i) + (ii)",
    ...
    c.record(
        ...
        True,
        detail="verified in Section 1: P_n supplies no BAE selector",
    )
    c.record(
