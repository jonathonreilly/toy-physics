...
def part4_no_proper_quotient_theorem(matrix_units: dict[tuple[int, int], np.ndarray]) -> None:
    ...
    for support_bits in range(1, 8):
        ...
        all_orbits_full = all_orbits_full and (orbit_dim == 3)
        check(
    ...
    check(
        ...
        all_orbits_full,
        "every nonzero support pattern spans all of H_hw=1 under the matrix-unit orbit",
    )
    check(
        "no proper observable-preserving quotient exists on the retained generation surface",
        all_orbits_full,
        "observable descent plus irreducibility leaves no nontrivial invariant kernel",
    )
