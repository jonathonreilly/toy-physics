...
two scalar constants Z_10, Z_20 require LPT integrals not in the current source content").

Verdict structure (bounded_theorem, structurally positive)
==========================================================
Positive source checks (PASS expected):
  1. b_0 = (11 N_color - 2 N_quark)/3 = 7 at N_f=6 (universal, upstream-supported
     via S1 + Casimir).
  ...
  3. Heat-kernel single-plaquette Taylor coefficients at orders 1, 2, 3
     reproduce {4/3, -8/9, 32/81} from upstream
...
Bounded admissions (ADMITTED expected, no derivation):
  7. Z_10^{HK->MS}: scalar 1-loop Brillouin-zone integral over the HK
     propagator; in principle computable on current source content but not
     yet derived.
...
# ======================================================================
# SECTION 1 -- Source support: b_0, b_1, <P>_HK Taylor coefficients
# ======================================================================

def section1_source_support(c: Counter) -> None:
    """Reproduce upstream heat-kernel single-plaquette Taylor coefficients
    and b_0, b_1 universal values from S1 + upstream Casimirs.
    """
    print()
    print("Section 1 -- Source support")

    # Upstream b_0 = (11 N_color - 2 N_quark) / 3 at N_f = 6
    b0 = Fraction(11 * N_COLOR - 2 * N_F, 3)
    c.record("b_0 = 7 at N_f=6 (upstream-supported universal via S1)",
             b0 == 7,
             detail=f"b_0 = {b0}")

    # Upstream b_1 = (34/3) C_A^2 - (20/3) C_A T_F N_f - 4 C_F T_F N_f at N_f = 6
    b1 = (Fraction(34, 3) * C_A**2
          - Fraction(20, 3) * C_A * T_F * N_F
          - 4 * C_F * T_F * N_F)
    c.record("b_1 = 26 at N_f=6 (upstream-supported universal via Casimir)",
             b1 == 26,
             detail=f"b_1 = {b1}")

    # Upstream <P>_HK Taylor coefficients
    # <P>_HK_SU(3)(s_t) = 1 - exp(-(4/3) s_t)
    # c_k = (-1)^(k+1) (4/3)^k / k!
    expected = {
        1: Fraction(4, 3),
    ...
    print()
    print("  Source-side b_2^HK (pure-gauge channel):")
...
def section2_universal_conversion_identity(c: Counter) -> None:
    ...
    The derivation is purely algebraic on the framework's source-supported
...
def section5_HK_side_specifics(c: Counter) -> None:
    ...
    print(f"  Source <P>_HK 3-loop Taylor coefficient: 32/81 = {float(b2_HK_pure_gauge):.6f}")
    c.record("[s_t^3] <P>_HK_SU(3) = 32/81 (source-supported pure-gauge HK 3L)",
             b2_HK_pure_gauge == Fraction(32, 81),
    ...
    print(f"                       = {diff:.4f}")
    print(f"    (with b_0=7, b_1=26 upstream-supported universal inputs, Z_10 and Z_20 admitted)")
    print()
    print("  At pure-gauge N_f=0 the HK 3L coefficient (32/81) is source-supported.")
    print("  Fermion-loop extension at 3L requires HK-with-quark-loops source work,")
...
def section6_bounded_admissions(c: Counter) -> None:
    ...
    c.admit("Z_10^{HK->MS, SU(3)} scalar",
            ...
            "in principle computable on current source content (analogue of "
    ...
    c.admit("Fermion-loop HK-side b_2^HK(N_f) for N_f > 0",
            "single-plaquette HK source covers pure-gauge channel only; "
            "fermion-loop 3L extension requires HK-with-quark-loops "
            "source work (X-L1-MSbar Section 1 boundary)")
...
def section7_hostile_review(c: Counter) -> None:
    ...
    print("      source-supported as derivation or admitted as axiom?")
    print()
    print("  Answer: DERIVATION.  It follows by power-counting expansion of")
    ...
    # Q2: Does this admit a NEW primitive into the framework baseline?
    print()
    print("  Q2: Does this closure note admit a NEW primitive?")
    print("  Answer: NO.  The universal RG identity is already implicit in upstream")
    ...
    print("  Answer: NO at 3-loop accuracy.  Power-counting:")
    ...
    print("  Answer: The factor is 3.  This emerges from BOTH the (z1_BC g_C^2) factor")
    ...
    print("  Answer: NO.  Before: 'HK <-> MSbar conversion is unknown 3-loop content'.")
    ...
    print("      to two scalars with a source-supported functional form.  This is the")
    ...
    print("  Answer: NO at the conversion-structure level.  Eq. (*) holds for")
    ...
    print("      (which is currently source-supported only at pure-gauge); and the Z_10, Z_20")
...
def section8_verdict(c: Counter) -> None:
    ...
    if c.fail_count == 0:
        ...
        print("    - Universal 3L conversion identity (*) source-supported")
        print("      as algebraic identity from RG closure.")
        print("    - b_0=7, b_1=26 (universal, upstream-supported) entered correctly.")
        print("    - <P>_HK Taylor 32/81 (source-supported pure-gauge) entered correctly.")
        print("    - AFP Wilson-action numerical cross-check passed.")
        print()
        print("  Admissions remaining:")
        ...
        print("    - Fermion-loop b_2^HK(N_f) at N_f > 0: separate source work.")
        ...
        print("      (Z_10, Z_20) missing, source-supported algebraic form eq. (*)'.")
...
def main() -> int:
    ...
    print("3-loop scheme conversion from current source content + universal RG")
