...
decomposition. The bridge is a conditional theorem:
    ...
    DERIVE  | <A T̃^n B>_0 - <A>_0 <B>_0 | ≤ ‖A‖ ‖B‖ exp(-n Δ_T)
...
This runner exhibits:
  ...
  E3.  Thermal trace-distance bound (B.8) — finite-temperature
       temporal correlations are bounded by the ground-state decay
       plus an explicit excited-state population q_beta. This runner
       computes q_beta from the tested spectrum instead of replacing it
       with a hidden dimension-independent Boltzmann factor.
...
# ---------------------------------------------------------------------------
# E3: Thermal trace-distance bound (B.8)
# ---------------------------------------------------------------------------

def exhibit_E3(rng, d=8, n_trials=20):
    print("\n--- E3: thermal trace-distance bound (B.8 two-term form) ---")
    print(f"  Verify: |<A T̃^n B>_β - <A>_β<B>_β| ≤ ‖A‖‖B‖ · ( exp(-n Δ_T) + 6 q_beta )")
    print(f"  q_beta is the actual excited-state thermal weight, computed from")
    print(f"  the tested finite spectrum; no dimension-independent Boltzmann")
    print(f"  replacement is used.")
    ...
    for trial in range(n_trials):
        ...
        for beta_a_tau in [0.5, 1.0, 2.0, 4.0]:
            ...
            weights = np.clip(e_T_norm, 0.0, None) ** beta_eff
            weights /= weights.sum()
            q_beta = 1.0 - float(weights[-1])
            ...
            for n in range(1, 9):
                ...
                # Two-term finite-block bound from (B.8).
                bound = nA * nB * (math.exp(-n * delta_T) + 6.0 * q_beta)
                n_total += 1
                if connc <= bound + 1e-10:
                    n_pass += 1
    frac = n_pass / n_total
    print(f"  trace-distance bound holds in {n_pass}/{n_total} (frac = {frac:.3f})")
    return frac >= 0.99
...
def main():
    ...
    print(" Closed-form bridge: gap input → temporal transfer-matrix decay")
    ...
    print()
    if n_pass == n_total:
        print(" verdict: bridge (B.1)–(B.2) verified as closed-form finite-block identity;")
