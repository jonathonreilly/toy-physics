...
This runner strengthens the current carrier-side read on the two explicit
split-2 upper-face neighborhoods identified by
DM_NEUTRINO_SOURCE_SURFACE_CARRIER_SIDE_CONCLUSION_NOTE_2026-04-18.

The strict remaining theorem target on the review surface is still:
  "interval-certified exclusion or dominance on the exact carrier inside
   those two explicit split-2 upper-face neighborhoods."

This runner does not claim that full interval-certified theorem. It provides
stronger support via:

1. a 51 x 51 x 51 dense-grid scan on each box
2. an empirical finite-difference Lipschitz estimate for eta_best
3. seeded constrained optimization searching for hidden rivals

The honest output is:
  - strong support that eta_best stays below 1 on both explicit boxes
  - materially sharper carrier-side narrowing than the earlier sample audit
  - the interval-certified exact-carrier dominance/completeness blocker
    remains open because the Lipschitz bound is empirical rather than rigorous
"""
...
def main() -> int:
    print_section("Split-2 upper-face dense-grid dominance support")
    ...
    record(
        "C.3 CAP_BOX empirical-Lipschitz upper bound max η_best < 1",
        cap_cert_ok,
    ...
    record(
        "C.6 ENDPOINT_BOX empirical-Lipschitz upper bound max η_best < 1",
        ep_cert_ok,
    ...
    if all_pass:
        print("  Strong carrier-side support on both split-2 upper-face neighborhoods")
        print("  at dense-grid plus empirical-Lipschitz rigor.")
        print()
        print("  CAP_BOX:")
        ...
        print()
        print("  This materially sharpens the residual carrier-side blocker, but it does")
        print("  not yet close the interval-certified exact-carrier dominance/completeness")
        print("  theorem target. Full rigorous enclosure of the transport pipeline")
        print("  remains open.")
    else:
