...
This is the next theorem after the three identification attacks and the
pointed source/boundary unification note.

Question:
  Can current retained, origin-free source/boundary data force the simultaneous
  closing point

      source origin = 0,
      CP1 selector descends/absent,
      endpoint torsor basepoint = 0?

Result:
  No.  Within the current residual atlas, the retained unpointed tests are
  invariant under source translations, CP1 line choice on the rank-two real
  primitive, and endpoint torsor translations.  Those transformations preserve
  the unpointed retained data while changing the open charged-lepton readouts.

  Therefore the exact remaining theorem is a retained pointed
  source/boundary-origin law.  Such a law would close the dimensionless lane,
  but this runner does not derive it.
...
def polynomial_translation_invariant_only_constant(symbol: sp.Symbol) -> tuple[bool, list[dict]]:
    """Check finite-degree polynomial translation invariance forces constant."""
    ...
    difference = sp.Poly(sp.expand(polynomial.subs(symbol, symbol + shift) - polynomial), symbol, shift)
    equations = list(difference.coeffs())
    solution = sp.solve(equations, coeffs[1:], dict=True)
    return solution == [{coeffs[1]: 0, coeffs[2]: 0, coeffs[3]: 0, coeffs[4]: 0}], solution
...
def main() -> int:
    ...
    record(
        "A.2 polynomial invariants of the unpointed source-translation fibre are constant",
        q_poly_ok,
        f"P(a+r)=P(a) -> {q_poly_solution}",
    )
    record(
        "A.3 source translation gives retained-equivalent non-Koide countermodels",
        q_value.subs(a, sp.Rational(1, 3)) == sp.Rational(1, 2)
        and ktl_value.subs(a, sp.Rational(1, 3)) == -sp.Rational(3, 8),
        f"a=1/3 -> K_TL={ktl_value.subs(a, sp.Rational(1, 3))}, Q={q_value.subs(a, sp.Rational(1, 3))}",
    )
    ...
    comm_eqs = list(sp.simplify(trace_one_symmetric * rotation - rotation * trace_one_symmetric))
    ...
    scalar_mark = sp.symbols("mu", real=True) * sp.eye(2)
    scalar_expectation = sp.simplify((line.T * scalar_mark * line)[0])
    spectator = sp.sin(alpha) ** 2
    record(
        "B.1 the only retained Z3-equivariant trace-one mark is scalar, not rank-one",
        comm_sol == [{x: sp.Rational(1, 2), y: 0}]
        and sp.simplify(half_identity**2 - half_identity) != sp.zeros(2, 2),
        f"commuting trace-one mark={comm_sol}; (I/2)^2-I/2={sp.simplify(half_identity**2-half_identity)}",
    )
    record(
        "B.2 scalar retained marks are line-blind on CP1",
        scalar_expectation == sp.symbols("mu", real=True),
        f"<line(alpha)|mu I|line(alpha)>={scalar_expectation}",
    )
    record(
        "B.3 CP1 line choice changes the open endpoint spectator while preserving scalar retained data",
    ...
    endpoint_poly_ok, endpoint_poly_solution = polynomial_translation_invariant_only_constant(c)
    phi1, phi2, shift = sp.symbols("phi1 phi2 shift", real=True)
    difference_invariant = sp.simplify(((phi2 + c + shift) - (phi1 + c + shift)) - (phi2 - phi1))
    affine_gluing_invariant = sp.simplify(((phi1 + phi2 + c + shift) - ((phi1 + c + shift) + (phi2 + c + shift) - (c + shift))))
    record(
        "C.1 polynomial invariants of the endpoint torsor are constant",
        endpoint_poly_ok,
        f"P(c+r)=P(c) -> {endpoint_poly_solution}",
    )
    record(
        "C.2 retained endpoint differences and affine gluing are torsor-translation invariant",
    ...
    delta_target_cancel = sp.simplify(eta * sp.cos(sp.pi / 4) ** 2 + eta * sp.sin(sp.pi / 4) ** 2)
    record(
        "D.1 the pointed origin law would imply both dimensionless values",
    ...
    section("E. Nature-grade boundary")

    record(
        "E.1 no origin-free retained invariant can select the closing representative in this atlas",
        q_poly_ok and endpoint_poly_ok and scalar_expectation == sp.symbols("mu", real=True),
        "The residual fibres are translation/line torsors under the retained unpointed tests.",
    )
    record(
        "E.2 the exact next positive theorem is the retained pointed source/boundary origin law",
        True,
        "It must derive source origin, CP1 descent/absence, and endpoint unit basepoint from retained physics.",
    )
    record(
        "E.3 this theorem does not claim full retained Koide closure",
        True,
        "It proves necessity and sufficiency within the residual atlas, but not the missing law itself.",
    )
    ...
    if n_pass == n_total:
        ...
        print("POINTED_ORIGIN_LAW_WOULD_CLOSE_DIMENSIONLESS_LANE=TRUE")
        ...
        print("NEXT_THEOREM=derive_pointed_origin_law_from_lattice_axiom_or_add_as_explicit_primitive")
        return 0
