...
def main():
    ...
    print("  Compare: unguarded vs channel-guarded vs coarse-mass-path-guarded")
    print("=" * 74)
    print()
    ...
    for nl in [80, 100]:
        ...
        for label, use_guard in [("Unguarded q=0.10", False),
                                   ("Channel-guarded q=0.10", True)]:
            ...
            for seed in range(n_seeds):
                ...
                mid_nodes = [i for li_idx in range(bl_idx+1, n_act-1) for i in by_layer[layers[li_idx]]
                ...
                if math.isnan(pb): continue

                # Prune
                if use_guard:
                    layer_list = [by_layer[l] for l in layers]
                    adj_p, _ = channel_guarded_prune(
                        positions, adj, layer_list,
                        mass_nodes, src, det_nodes, blocked_base,
                        quantile=0.10, max_iter=3, min_eff_ratio=0.80)
                else:
                ...
                if gb > 0 and gp < 0: flip_count += 1

            if grav_b_list:
                n_ok = len(grav_b_list)
                print(f"  {label:>30s}  {statistics.fmean(grav_b_list):+7.3f}  "
