...
Builds on PR #957 (operator-Cauchy convergence). PR #957 verified
numerically that the rescaled NN transfer operator T_h converges as
h -> 0 on a fixed 15-dim observable subspace, with continuum limits
MI=1.0, 1-pur=0.5, d_TV=1.0, gravity=0 (at fixed strength), Born=0.

This runner identifies T_∞'s structure on the slit-detector
decoherence subblock. The claim:

    T_∞ is the GEODESIC CONTINUUM OPERATOR on the rescaled NN
    harness with fixed physical length L = PHYS_L. Each slit at
    y = ±SLIT_Y contributes a delta function at its own y on the
    detector. The convergence to delta is governed by

        sigma_arm(h)  ~  C_arm * sqrt(h)

    with C_arm ≈ 2.51 for the canonical BETA = 0.8 angular weight.

This is identification of T_∞, not just existence (which was PR #957).
The geodesic-continuum framing tells us:

- the decoherence observable limits (MI=1, 1-pur=0.5, d_TV=1) are
  the standard "fully-decohered, well-separated two-state" values,
  because the two arms become orthogonal in the continuum
- the gravity-at-fixed-strength continuum is 0 because the geodesic
  arrival point is field-independent in the linear-perturbation
  regime; nontrivial gravity requires renormalization scaling that
  saturates the propagator (PR #945)
- the convergence rate r ~ 1.19 from PR #957's Cauchy fit is
  consistent with the sqrt(h) Gaussian-tail decay rate of the
  arm-overlap integral

The identification is supported by three numerical observations:
...
3. predicted MI / d_TV / 1-pur from a Gaussian-arm model with
...
def main() -> int:
    print("=" * 100)
    print("NN LATTICE RESCALED-LANE CONTINUUM OPERATOR IDENTIFICATION")
    print("  T_∞ = geodesic operator on the rescaled NN harness")
    ...
    # Continuum-limit identification statement
    print("=" * 100)
    print("CONTINUUM IDENTIFICATION")
    print("=" * 100)
    print()
    if pred_ok and var_fit_ok and drift_ok and born_ok:
        print("REGISTERED IDENTIFICATION: T_∞ on the rescaled NN harness's")
        print("slit-detector decoherence subblock is the GEODESIC CONTINUUM")
        print("OPERATOR characterized by:")
        print()
        print(f"  - per-arm centroids at y = ±{SLIT_Y} (slit positions),")
        ...
        print(f"    slit y, giving the saturation values:")
        print(f"      MI_∞    = log2(2) = 1.0   (perfect arm discrimination)")
        print(f"      d_TV_∞  = 1.0            (orthogonal arm supports)")
        print(f"      1-pur_∞ = 0.5            (max mixed in arm-space)")
        print()
        print(f"This is identification of T_∞ (Target A.2 of the bridge plan)")
        print(f"on the slit-detector decoherence subblock. Independent of and")
        print(f"complementary to the operator-existence theorem (PR #957).")
    else:
        print("Identification FAILS one or more guards. No claim made.")
