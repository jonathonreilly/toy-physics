#!/usr/bin/env python3
"""
G5 / fourth-order mixed-Gamma lane — Mixed-Γ_i fourth-order return
=============================================
...
Target:
  a companion runner v2 showed that the retained SECOND-order Γ_1 return
    P_{T_1} Γ_1 (P_{O_0} + P_{T_2}) Γ_1 P_{T_1} = I_3
  is species-democratic, and a companion runner stress-tested iterated
  ...
  Honest TOE-grade reporting. The outcome FOURTH_ORDER_MIXED_GAMMA_NO_RETAINED_MIXED_BREAKING
  is a legitimate (and likely) possibility.

Authority role: frontier attack-surface runner for G5 gap, fourth-order mixed-Gamma lane.
Extends a companion runner v2's second-order survey and a companion runner's iterated-pure
...
# ----------------------------------------------------------------------
# Cl(3) + chirality carrier on C^16 — identical conventions to a companion runner v2
# ----------------------------------------------------------------------
...
def phase0_consistency():
    ...
    check("Second-order return species block = I_3 (a companion runner v2 baseline)",
    ...
    # Pure-Γ_1 fourth-order iterated return through P_not (a companion runner Stress 2)
    K = G1 @ (P_O0 + P_T2) @ G1
    iter4 = restrict_species(P_T1 @ K @ K @ P_T1)
    check("Pure-Γ_1 iterated n=2 through P_O0+P_T2 = I_3 (a companion runner baseline)",
...
def phase4_multiset_koide_check():
    ...
    # which is the SAME 2+1 degenerate structure as a companion runner v2's
...
def phase5_audit_and_verdict():
    ...
    audit_rows = [
        ...
        ("Per-T_2-state weights", "NOT retained — a companion runner v2 Correction-C missing primitive", "Unchanged by mixed-Γ: fourth-order inherits same S_2 obstruction"),
    ]
    ...
    print("     a companion runner v2's Correction-C hw-staggered scheme. Even if one")
    ...
    print("    FOURTH_ORDER_MIXED_GAMMA_CLOSES_CL_HIERARCHY:                 ruled out (no mixed-Γ construction reaches Koide Q=2/3 with observed direction)")
    print("    FOURTH_ORDER_MIXED_GAMMA_CONE_ONLY:                 ruled out (no cone-forcing from mixed-Γ insertions)")
    print("    FOURTH_ORDER_MIXED_GAMMA_S2_BREAKING_BUT_NO_KOIDE:  ruled out (S_2 remains unbroken under retained V_sel Hessian)")
    print("    FOURTH_ORDER_MIXED_GAMMA_NO_RETAINED_MIXED_BREAKING: CONFIRMED — retained mixed-Γ constructions")
    print("        either vanish or reduce to (σ^4, σ^2, σ^2) 2+1 structure.")
    print()
    verdict = "FOURTH_ORDER_MIXED_GAMMA_NO_RETAINED_MIXED_BREAKING"
