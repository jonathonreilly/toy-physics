#!/usr/bin/env python3
"""
PR #230 non-chunk cycle-32 post-cycle-31 main-audit-status-drift reopen guard.

Cycle 31 closed origin/main drift through 905eed76.  This guard checks the
only new resume fact available at cycle 32: after a fresh fetch, origin/main
advanced again to 1c86abee, but the new diff still contains only
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_nonchunk_cycle32_post_cycle31_main_audit_status_drift_guard_2026-05-05.json"
)
LOOP_PACK = (
    ROOT
    / ".claude"
...
CYCLE31_HEAD = "aab6b94fdac754c9969aac587e0fe3bd1eea3f3c"
CYCLE31_REMOTE_MAIN_HEAD = "905eed760dff0e6563f0be942f8183da8591fcff"
...
def branch_state(future_paths: list[str]) -> dict[str, Any]:
    ...
    main_changed_paths = diff_paths(CYCLE31_REMOTE_MAIN_HEAD, REMOTE_MAIN)
    main_changed_path_classes = Counter(path_class(rel) for rel in main_changed_paths)
    return {
        "cycle31_head": CYCLE31_HEAD,
        "cycle31_remote_main_head": CYCLE31_REMOTE_MAIN_HEAD,
        "local_head": local_head,
        "remote_pr_ref": REMOTE_PR,
        "remote_pr_head": remote_pr_head,
        "remote_main_ref": REMOTE_MAIN,
        "remote_main_head": remote_main_head,
        "cycle31_is_ancestor_of_local_head": git_ok(
            ["merge-base", "--is-ancestor", CYCLE31_HEAD, "HEAD"]
        ),
        "cycle31_is_ancestor_of_remote_pr": git_ok(
            ["merge-base", "--is-ancestor", CYCLE31_HEAD, REMOTE_PR]
        ),
        "cycle31_remote_main_is_ancestor_of_remote_main": git_ok(
            ["merge-base", "--is-ancestor", CYCLE31_REMOTE_MAIN_HEAD, REMOTE_MAIN]
        ),
        "local_head_matches_remote_pr_head": bool(local_head and local_head == remote_pr_head),
        "local_commits_after_cycle31": git(["rev-list", "--count", f"{CYCLE31_HEAD}..HEAD"]),
        "remote_commits_after_cycle31": git(["rev-list", "--count", f"{CYCLE31_HEAD}..{REMOTE_PR}"]),
        "remote_main_commits_after_cycle31_baseline": git(
            ["rev-list", "--count", f"{CYCLE31_REMOTE_MAIN_HEAD}..{REMOTE_MAIN}"]
        ),
        "remote_main_changed_path_count_after_cycle31_baseline": len(main_changed_paths),
        "remote_main_changed_path_class_counts_after_cycle31_baseline": dict(
        ...
        "local_changed_future_paths_after_cycle31": diff_future_paths(
            CYCLE31_HEAD, "HEAD", future_paths
        ),
        "remote_pr_changed_future_paths_after_cycle31": diff_future_paths(
            CYCLE31_HEAD, REMOTE_PR, future_paths
        ),
        "remote_main_changed_future_paths_after_cycle31_baseline": diff_future_paths(
            CYCLE31_REMOTE_MAIN_HEAD, REMOTE_MAIN, future_paths
        ),
    }
...
def main() -> int:
    print("PR #230 non-chunk cycle-32 post-cycle-31 main-audit-status-drift reopen guard")
    print("=" * 72)
    ...
    cycle26 = certs["cycle26_post_cycle25_main_audit_status_drift"]
    cycle27 = certs["cycle27_post_cycle26_main_audit_status_drift"]
    cycle28 = certs["cycle28_post_cycle27_main_audit_status_drift"]
    cycle29 = certs["cycle29_post_cycle28_main_audit_status_drift"]
    cycle30 = certs["cycle30_post_cycle29_main_audit_status_drift"]
    cycle31 = certs["cycle31_post_cycle30_main_audit_status_drift"]
    ...
    presence_sources = {
        ...
        "cycle26_post_cycle25_main_audit_status_drift": future_presence(cycle26),
        "cycle27_post_cycle26_main_audit_status_drift": future_presence(cycle27),
        "cycle28_post_cycle27_main_audit_status_drift": future_presence(cycle28),
        "cycle29_post_cycle28_main_audit_status_drift": future_presence(cycle29),
        "cycle30_post_cycle29_main_audit_status_drift": future_presence(cycle30),
        "cycle31_post_cycle30_main_audit_status_drift": future_presence(cycle31),
    }
    ...
    no_changed_future_path_since_cycle31 = not any(
        [
            branch["local_changed_future_paths_after_cycle31"],
            branch["remote_pr_changed_future_paths_after_cycle31"],
            branch["remote_main_changed_future_paths_after_cycle31_baseline"],
        ]
    )
    ...
    branch_current = (
        branch["cycle31_is_ancestor_of_local_head"]
        and branch["cycle31_is_ancestor_of_remote_pr"]
        and branch["cycle31_remote_main_is_ancestor_of_remote_main"]
        and branch["local_head_matches_remote_pr_head"]
        and bool(branch["remote_main_head"])
    )
    loop_pack_records_stop = (
        "cycle-31 update" in compact_text(OPPORTUNITY_QUEUE)
        and "post-cycle-30 main-audit-status-drift reopen guard" in compact_text(HANDOFF)
        and "post-cycle-30 main audit/status drift is not admissible reopen evidence"
        in compact_text(NO_GO_LEDGER)
        and "proposal_allowed: false" in compact_text(CLAIM_STATUS_CERTIFICATE)
    )
    main_drift_since_cycle31_is_status_only = (
        branch["remote_main_commits_after_cycle31_baseline"] != "0"
        and branch["remote_main_changed_paths_are_status_drift"]
        and not branch["remote_main_changed_future_paths_after_cycle31_baseline"]
    )
    no_reopen_input = (
        source_presence_absent
        and no_local_or_remote_future_path
        and no_changed_future_path_since_cycle31
        and main_drift_since_cycle31_is_status_only
    )
    cycle32_post_cycle31_main_audit_status_drift_guard_passed = all(
        [
            not missing_parents,
            not parent_failures,
    ...
    dramatic_step_gate = {
        ...
        "branch_state": branch,
        "reason": (
            "Cycle 31 already rejects post-cycle-30 origin/main audit/status "
            ...
            "of the listed PR230 same-surface artifacts, so cycle 32 has no "
    ...
    report("branch-has-cycle31-ancestor-and-remote-main-visible", branch_current, json.dumps(branch, sort_keys=True))
    ...
    report("parent-process-stack-closed-through-cycle31", parent_process_stack_closed, "cycle-18 through cycle-31 gates deny continuation")
    report("aggregate-gates-deny-proposal", aggregate_gates_deny, f"assembly={statuses['assembly']} campaign={statuses['campaign']}")
    report("loop-pack-records-cycle31-stop-contract", loop_pack_records_stop, "cycle-31 stop text present")
    report("main-drift-since-cycle31-is-status-only", main_drift_since_cycle31_is_status_only, json.dumps(branch, sort_keys=True))
    report("no-local-or-remote-listed-future-path", no_local_or_remote_future_path, json.dumps(branch, sort_keys=True))
    report("no-changed-listed-future-path-since-cycle31", no_changed_future_path_since_cycle31, json.dumps(branch, sort_keys=True))
    report("no-source-presence-key-set", source_presence_absent, json.dumps(present_by_source, sort_keys=True))
    report(
        "cycle32-post-cycle31-main-audit-status-drift-guard-passed",
        cycle32_post_cycle31_main_audit_status_drift_guard_passed,
        f"passed={cycle32_post_cycle31_main_audit_status_drift_guard_passed}",
    )

    result = {
        "actual_current_surface_status": (
            "exact negative boundary / PR230 non-chunk cycle-32 "
            "post-cycle-31 main-audit-status-drift reopen guard: origin/main "
        ...
        "proposal_allowed_reason": (
            "Cycle 31 already rejects post-cycle-30 origin/main audit/status "
            "drift. The cycle-31 head remains an ancestor of the aligned PR "
