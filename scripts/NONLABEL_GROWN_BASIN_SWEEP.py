#!/usr/bin/env python3
"""Tiny basin sweep for the grown-row non-label signed-source transfer.

This script asks whether the retained geometry-sector / non-label connectivity
idea extends beyond the single promoted grown row into a tiny neighborhood of
nearby grown rows.
...
The claim surface is intentionally narrow: this is a basin test, not a family
rebuild or a general transport theorem.
"""
...
def main() -> None:
    print("=" * 90)
    print("NON-LABEL GROWN BASIN SWEEP")
    print("  question: does the geometry-sector idea survive a tiny basin around the")
    print("  retained grown-row fixed-field signed-source transfer?")
    print("=" * 90)
    print(f"h={H}, NL={NL}, drifts={DRIFTS}, restores={RESTORES}, seeds={SEEDS}")
    print()
    ...
    print("drift restore seed | zero         plus         minus        neutral      double       exp    ok")
    print("-" * 90)
    for row in rows:
        print(
            f"{row.drift:>4.2f}  {row.restore:>4.2f}   {row.seed:>2d}  | "
            f"{row.zero:+.3e}  {row.plus:+.3e}  {row.minus:+.3e}  "
            f"{row.neutral:+.3e}  {row.double:+.3e}  {row.exponent:>5.3f}  "
            f"{'YES' if row.signed_ok else 'no'}"
        )

    basin_rows = [r for r in rows if r.signed_ok]
    print()
    print("SAFE READ")
    print(f"  passed rows: {len(basin_rows)}/{len(rows)}")
    if basin_rows:
        drift_vals = sorted({r.drift for r in basin_rows})
        restore_vals = sorted({r.restore for r in basin_rows})
        print(f"  drift coverage: {drift_vals}")
        print(f"  restore coverage: {restore_vals}")
        print(
            f"  mean exponent among passes: {_mean([r.exponent for r in basin_rows]):.6f}"
        )
        print("  this is a bounded positive basin, not just a one-point ridge")
    else:
        print("  no nearby rows passed the signed-source guards")
        print("  the geometry-sector transfer is a one-row positive only")
