#!/usr/bin/env python3
"""
Irregular Endogenous Sign-Closure Probe
=======================================

Question:
  Can one same-surface, graph-native wavefunction observable distinguish
  attractive from repulsive parity coupling on the retained irregular
  bipartite families, without borrowing cubic exact-force semantics?

Design:
  - same graph, same source, same packet, same G
  - compare self-consistent parity runs with +Phi and -Phi
  - use a graph-center shell packet with outward shell phase
  - score only matter-response observables:
      O1  ball-1 capture margin
      O2  ball-2 capture margin
      O3  mean-depth margin
...
Positive margins mean attraction holds the packet closer to the source ball
than repulsion over the same early-time window.
"""
...
def main():
    t0 = time.time()
    print("=" * 92)
    print("IRREGULAR ENDOGENOUS SIGN-CLOSURE PROBE")
    print("=" * 92)
    print("Same-surface parity comparison: +Phi vs -Phi")
    print("Packet: graph-center shell gaussian with outward shell phase")
    ...
    if ball2_hits == total and depth_hits == total:
        print(
            "Strong same-surface sign separation on this graph-native shell-packet surface:"
        )
        print(
            "attraction keeps more probability inside the source ball and lower mean depth"
        )
        print("than repulsion on every audited family/seed/G row.")
        print(
            "This closes endogenous sign selection on the audited shell-packet surface,"
        )
        print(
            "but not universal irregular directional gravity for arbitrary initial states."
        )
    else:
        print(
            "No frozen same-surface sign closure: at least one core shell-packet metric"
        )
        print("fails to separate attraction from repulsion across the audited surface.")
