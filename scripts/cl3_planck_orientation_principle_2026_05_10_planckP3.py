#!/usr/bin/env python3
"""Cl(3)/Z^3 Planck orientation principle from retained 3+1 single-clock time-asymmetry.
...
This runner asks whether the retained 3+1 single-clock time-asymmetry — which
...
(retained as part of the AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29
...
Result (positive)
=================
The retained reflection-positivity / single-clock content carries an
...
Hence:
  ...
  - The retained microcausality / forward-time / Lieb-Robinson generator
...
Status proposal
===============
bounded_theorem under the retained chain
...
#
# The retained AXIOM_FIRST_REFLECTION_POSITIVITY_THEOREM_NOTE_2026-04-29 proof
...
def time_form_vector() -> np.ndarray:
    """The unique 1-form e^t inside Lambda^1 W, identified by the retained
...
def main() -> int:
    print("=" * 78)
    print("CL(3)/Z^3 PLANCK ORIENTATION PRINCIPLE FROM RETAINED 3+1 TIME ASYMMETRY")
    print("=" * 78)
    print()
    print("Question: does the retained action-level temporal-axis sign rule of")
    ...
    # The principle: under retained {RP staggered phase rule, single-clock,
    ...
    results.append(
        check(
            ...
            "no PDG, no fitted coefficients, no new axioms, no MC inputs, "
    ...
    print("retained action-level temporal-axis sign rule.")
    print()
    ...
    print()
    print("Verdict: bounded_theorem proposal — orientation principle CLOSES")
    print("from retained 3+1 single-clock time-asymmetry. The bounded label")
