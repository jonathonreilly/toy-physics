...
Hence the structural degree-of-freedom count of S is exactly 2 at fixed
admitted radiation readout R.

Checks:
    1. Symbolic closed-form table (sympy): each tabulated identity holds.
    2. Forward Jacobian (sympy): the map (H_0, L) -> (H_inf, q_0) has full
       rank at fixed R, confirming the count is sharp.
...
def check_closed_form_table() -> tuple[bool, str]:
    ...
    # Matter-Lambda equality gives M / a_mL^3 = L, so
