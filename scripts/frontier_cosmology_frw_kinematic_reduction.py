...
K2: 1 + z_*   = (2 Omega_Lambda0 / Omega_m0)^(1/3)        [neglecting Omega_r]
...
No observed cosmological value enters the derivations of K1-K5. Observational
...
def part0_retained_inputs() -> None:
    ...
    check(
        "three retained cosmology identities present on main",
        True,
        "Lambda, w = -1, Omega_L = (H_inf/H_0)^2 all retained",
    )
...
def part1_k1_q0() -> None:
    ...
    # With observed Omega_r, q = 0 at slightly different Omega_L
    ...
    # Comparison with observed q_0
    q0_framework = q_from_K1(OMEGA_L_OBS, OMEGA_R_OBS)
    print()
    print(f"  q_0 (framework on observed Omega_L,Omega_r): {q0_framework:.4f}")
    print(f"  q_0 (observed Type Ia SN):                   {Q0_OBS_SNIA:.4f} +/- {Q0_OBS_SNIA_ERR:.2f}")
    print()
    check(
        "q_0 framework matches SN Ia Union3/DES-Y5 within 1 sigma",
...
def part2_k2_z_star() -> None:
    banner("Part 2: K2 - 1 + z_* = (2 Omega_L0 / Omega_m0)^(1/3)")

    def z_star_from_K2(omega_L: float, omega_m: float) -> float:
        return (2.0 * omega_L / omega_m) ** (1.0 / 3.0) - 1.0

    # Derivation sanity: setting q(a_*) = 0 in the Omega_r -> 0 limit
    # gives a_*^3 = Omega_m0 / (2 Omega_L0), so 1 + z_* = (2 Omega_L0 / Omega_m0)^(1/3).

    # Limiting cases
    # If Omega_L = Omega_m, 1+z_* = 2^(1/3) ~ 1.26, z_* ~ 0.26
    z_star_equal = z_star_from_K2(0.5, 0.5)
    check(
        "K2 symmetric point Omega_L = Omega_m gives 1 + z_* = 2^(1/3) ~ 1.26",
    ...
    z_star_zero_L = z_star_from_K2(1e-9, 1.0)
    check(
        "K2 Omega_L -> 0 limit: no acceleration, 1 + z_* -> 0 formally (past eternal deceleration)",
        z_star_zero_L < -0.99,
        f"z_* formally = {z_star_zero_L:.4f} (near -1, no physical transition)",
    )

    # Framework prediction under observed Omega's
    z_star_framework = z_star_from_K2(OMEGA_L_OBS, OMEGA_M_OBS)
    print()
    print(f"  z_* (framework on observed Omegas): {z_star_framework:.4f}")
    print(f"  z_* (SN Ia compilations):           {Z_STAR_OBS:.4f} +/- {Z_STAR_OBS_ERR:.2f}")
    print()
    check(
        "z_* framework matches SN Ia observed within 1 sigma",
        abs(z_star_framework - Z_STAR_OBS) < Z_STAR_OBS_ERR,
        f"framework {z_star_framework:.4f} vs SN {Z_STAR_OBS:.4f} +/- {Z_STAR_OBS_ERR:.2f}",
    )
    check(
        "z_* > 0 under observed Omega_L > 1/3 (accelerating today)",
        z_star_framework > 0.0,
        f"z_* = {z_star_framework:.4f} > 0",
    )
...
def part3_k3_z_mL() -> None:
    ...
    # Framework prediction on observed Omega's
    z_ml_framework = z_mL_from_K3(OMEGA_L_OBS, OMEGA_M_OBS)
    print()
    print(f"  z_mL (framework on observed Omegas): {z_ml_framework:.4f}")
    print(f"  z_mL (Planck-derived):               {Z_ML_OBS:.4f}")
    print()
    check(
        "z_mL framework matches Planck-derived value",
        abs(z_ml_framework - Z_ML_OBS) < 0.01,
        f"framework {z_ml_framework:.4f} vs Planck-derived {Z_ML_OBS:.4f}",
    )
    check(
        "z_mL > 0 under observed Omega_L > Omega_m (Lambda dominates today)",
        z_ml_framework > 0.0,
...
def part4_z_star_gap() -> None:
    banner("Part 4: z_* and z_mL gap - structural 2^(1/3) factor")
...
def part7_joint_reduction() -> None:
    ...
    def observables_from_x(x: float, om_r: float) -> dict:
        ...
        if om_m > 0 and om_L > 0:
            z_star = (2.0 * om_L / om_m) ** (1.0 / 3.0) - 1.0
    ...
    print("    4. z_*           = (2x^2 / Omega_m)^(1/3) - 1 (new K2)")
...
def part8_falsifiability() -> None:
    ...
    # Observed (q_0, z_*, Omega_L) should satisfy K1 and K2 jointly.
    om_r = OMEGA_R_OBS
    om_L = OMEGA_L_OBS
    om_m = 1.0 - om_L - om_r
    q0_K1 = 0.5 * (1.0 + om_r - 3.0 * om_L)
    z_star_K2 = (2.0 * om_L / om_m) ** (1.0 / 3.0) - 1.0
    # The SN-observed (q_0, z_*) pair should be K1/K2 consistent if the framework holds.
    # Observation is consistent at ~1sigma; let's verify the framework prediction is consistent.
    ...
    check(
        "framework (q_0, z_*) pair within 1 sigma of joint observation",
...
def part9_summary() -> None:
    ...
    print("    K2: 1 + z_*   = (2 Omega_L / Omega_m)^(1/3)                 [exact, Omega_r ~ 0]")
    ...
    print()
    print("  MATCHES OBSERVATION (at observed Omega_L = 0.685):")
