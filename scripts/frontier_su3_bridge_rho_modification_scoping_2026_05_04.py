"""SU(3) bridge ρ-modification scoping.

This runner:

  1. Computes the existing all-forward candidate ρ (P = 0.4291).
  ...
  6. Label what kind of additional structure each modification would
     require:
...
The output is a scoping table of extra assumptions that would be needed
to hit the 0.5934 comparator. It does not adopt those assumptions and
does not derive the comparator.
...
def driver() -> int:
    print("=" * 78)
    print("SU(3) Bridge ρ-Modification Scoping: what would hit P = 0.5934?")
    print("=" * 78)
    print()
    ...
    if alpha > 0:
        ...
        if alpha > 100:
            print(f"  *** This is HUGE — orders of magnitude. Treat it as an")
            print(f"      open structural gap, not a small correction. ***")
        elif alpha > 10:
        ...
        print(f"  *** Comparator is unreachable by ANY tested global multiplier. ***")
        support_count += 1
    print()
    ...
    if abs(p_best - P_TARGET) < 0.01:
        print(f"  *** Adjustment to N_components from 8 → {n_comp_best} gives a near-miss. ***")
        print(f"  Interpretation: a different topology or boundary condition would")
        print(f"  be needed; this runner does not supply one.")
        pass_count += 1
    else:
    ...
    print("--- Section E: per-sector modification needed to hit comparator ---")
    ...
    if needed:
        print(f"  Sectors that can be forced to the comparator by one-sector change:")
        ...
        print()
        print(f"  Number of one-sector comparator hits: {len(needed)}")
        if len(needed) > 0:
    ...
    print("--- Section F: scoping verdict — what would be needed to hit 0.5934? ---")
    print()
    ...
    if alpha > 0:
        ...
        print(f"      Would require: a renormalization-group rescaling factor")
    ...
    print(f"      Would require: a different cube topology or boundary condition")
    print(f"      that changes the index-graph component count from 8 to {n_comp_best}.")
    print(f"      → Best N gives P = {p_best:.4f}; "
          f"{'near-miss' if abs(p_best - P_TARGET) < 0.01 else 'does not close'}.")
    print()
    if needed:
        print(f"  M3: PER-SECTOR ρ MODIFICATION (single sector)")
        print(f"      {len(needed)} sectors can be forced to hit the comparator with an appropriate ρ value.")
        ...
        print(f"      Smallest required modification: ρ_{smallest_key} → "
              f"× {smallest_ratio:.2e}.")
        print(f"      Would require: a non-perturbative correction to a SPECIFIC")
        ...
        print(f"      the comparator burden alone.")
    print()
    print(f"  CONCLUSION: To hit P(β=6) = {P_TARGET:.4f} from the tested")
    print(f"  candidate family, one would need ONE OF:")
    ...
    print()
    print(f"  None of these is supplied by this runner. Each is an open")
    print(f"  science item that would need explicit derivation and review before")
    print(f"  it could be used as repo authority.")
    print()
