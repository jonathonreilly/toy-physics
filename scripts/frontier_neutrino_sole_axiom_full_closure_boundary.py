...
def part2_graph_first_reduced_cycle_channel_is_realized_but_not_selected() -> None:
    ...
    check("So the current exact bank does not select a unique reduced-channel value", True)
...
def part3_majorana_stays_closed_negatively_on_the_lower_level_charge_preserving_lane() -> None:
    ...
    check("So the retained Majorana lane does not reopen on the lower-level charge-preserving response layer", True)
