...
def part_survey_prerequisites_absent() -> None:
    ...
    # The survey checks via filesystem absence of D17-prime
    d17_prime_candidates = list(DOCS.glob("*D17_PRIME*.md")) + list(DOCS.glob("*D17PRIME*.md"))
    check(
        "M5-a prerequisite: D17-prime on (2, 1) lepton-doublet block — ABSENT",
        len(d17_prime_candidates) == 0,
        f"docs/*D17_PRIME*.md count: {len(d17_prime_candidates)}",
    )
