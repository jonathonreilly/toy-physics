...
Per physics-loop guidance (deep-work / stuck-fan-out): before declaring
the L_s>=3 Wigner-Racah engine path the only route, run a fan-out of
...
("L_s>=3 multi-week required") by explicitly ruling out simpler
...
def driver() -> int:
    ...
    if closures:
        ...
        print()
        print("  The L_s>=3 Wigner-Racah engine path remains the route.")
        print("  Block 5 verdict (L_s>=3 multi-week genuinely required) is")
        print("  STRENGTHENED by this explicit ruling-out of orthogonal")
    ...
    print("  ~ 0.59. Closure requires the full lattice tensor-network")
    print("  contraction (the Wigner-Racah engine path) or a numerical MC.")
    print()
    ...
    print(f"  L_s>=3 Wigner-Racah path remains the route.")
