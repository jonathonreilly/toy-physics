#!/usr/bin/env python3
"""Probe two-clause closures using joined candidate-identity and lobe-topology features."""
...
def build_rows(
    ...
    *,
    ...
    identity_rows, _events, signature_text, bucket_text = build_candidate_identity_rows(
        log_path,
    ...
    lobe_by_source = {row.source_name: row for row in lobe_rows}
    ...
    for identity in identity_rows:
        ...
        for name, value in vars(lobe).items():
            if name not in {"source_name", "subtype"}:
                features[name] = float(value)
        rows.append(
            {
...
def render_rows(rule_rows: list[dict[str, object]]) -> str:
    lines = [
        "Identity+Lobe Two-Clause Closures",
        "=================================",
...
def main() -> None:
    ...
    print(f"identity+lobe two-clause closure started {started}", flush=True)
    ...
    print()
    print("Identity+Lobe Two-Clause Closure")
    print("================================")
    ...
    print()
    print(
        f"identity+lobe two-clause closure completed {datetime.now().isoformat(timespec='seconds')} total_elapsed={time.time() - total_start:.1f}s",
        flush=True,
    )
