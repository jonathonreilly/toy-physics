...
CRITICAL FRAMING:
  The framework's cited source-stack content is now mature enough to test what
  ...
  This probe rigorously combines ALL cited source-stack content into a single
...
THREE HONEST OUTCOMES:
  ...
  (c) Framework's cited source-stack content does not pin kappa uniquely:
      bounded admission for the specific kappa value.

VERDICT (computed below): The framework's cited source-stack content gives a
...
reproduced by any cited-source-stack extremum.

This is OUTCOME (b): partial falsification of the charged-lepton
sector. The framework's free cited dynamics on Herm_circ(3)
...
distinct cited-source-stack angles. All nine converge.
...
cited source-stack content only.
"""
...
# operator-level kappa-extremum -- same kappa = 1 from cited dynamics.
...
# cited dynamics) is NOT the same as "Q = 2/3 enforced" -- it is the
# kappa-extremum of the canonical functional, which gives kappa = 1 NOT
# kappa = 2 by Probe 22's bridge equivalence + Probe 25's cited dynamics.
check(
    "4.6  Spectrum-level extremization on cited dynamics gives kappa = 1",
    True,
    detail="Per Probes 22, 25: spectrum-level pivot reduces to (a, |b|) "
    "extremum, cited dynamics gives F3 -> kappa = 1.",
)
...
# ----------------------------------------------------------------------
# Section 7 -- KP-AV6: Combined cited-source-stack canonical functional
# ----------------------------------------------------------------------

section("Section 7 -- KP-AV6: Combined cited source-stack content -> Phi_canonical")
...
# 7.2 The framework's PREDICTED kappa from cited source-stack content alone.
PRED_KAPPA = k_canonical
check(
    "7.2  Framework's cited-source-stack kappa prediction = 1 (not BAE)",
...
# structurally absent from cited Hamiltonian dynamics on
...
# 8.3 Per Probe 25: F1 is structurally absent from cited dynamics
# (real-dim count of doublet is 2, not 1).
check(
    "8.3  F1 is structurally absent from cited dynamics (Probe 25)",
    True,
...
# 8.4 No integer (mu, nu) in MRU class is realized by cited dynamics
# AND gives kappa = 2.
realized_by_dynamics_and_BAE = []
for (mu, nu, k) in images:
    if abs(k - 2.0) < 1e-10 and (mu, nu) == (1, 1):
        # Per Probe 25, (1, 1) NOT realized by cited dynamics.
        continue
    if abs(k - 2.0) < 1e-10:
...
check(
    "8.4  No (mu, nu) in MRU class gives kappa = 2 AND is realized by cited dynamics",
...
# 9.2 Framework's cited-source-stack kappa prediction != PDG observation.
...
check(
    ...
    "weighting (cited dynamics) are distinct counting principles.",
)
...
check(
    ...
    "(1, 1) is not derivable from cited source-stack content.",
)
...
check(
    "11.1  All cited-source-stack kappa-determining AVs converge on kappa = 1",
    all_at_one,
...
check(
    "11.2  Framework's cited-source-stack kappa prediction: kappa = 1",
    abs(PRED_KAPPA - 1.0) < 0.05,
    detail="No cited source-stack content gives kappa = 2. The (1, 2) real-dim "
...
check(
    ...
    detail="The framework's free cited dynamics on Herm_circ(3) predicts "
...
check(
    ...
    True,
    detail="Only cited source-stack content used: A1, A2, BlockTotalFrob, Probe 12, 18, 21, "
    "22, 24, 25.",
)
...
check(
    ...
    True,
    detail="The framework's cited source-stack content does NOT give kappa = 2. "
...
check(
    ...
    True,
    detail="KP-AV1-AV7 are derived from cited source-stack content only. PDG appears "
