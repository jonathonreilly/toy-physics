...
For each limit, we test:
    Test C1 (limit existence): is the limit cited source-stack content?
...
check("C1.1 a -> 0 limit is cited source-stack content (CONTINUUM_LIMIT_NOTE.md)", True)
...
phi_at_arb_theta = phi_theta(H_test, 0.41)  # theta = 0.41 is generic, not in cited source-stack content
...
# C2.1: limit retained
check("C2.1 |Lambda| -> infinity is cited source-stack content (thermodynamic limit)", True)
...
check("C3.1 beta_th -> 0 KMS limit is cited source-stack content (KMS theorem note)", True)
...
# Each theta requires constructing it explicitly from cited source-stack content (not just claiming
...
# C6.5: per the retained no-go, generic theta is not in cited source-stack content. So even if
...
print("cited source-stack content, or preserves arg(b) trivially without generating phi_theta as algebra action.")
print()
...
check("C9.3 SU(3) is fixed N=3; 't Hooft large-N is not cited source-stack content for matter sector",
      True)
