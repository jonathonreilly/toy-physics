...
gauge group, no physical bare-coupling identification, and no PDG /
literature / fitted / admitted-unit-convention import.
...
CLAIM_ID = "beta_gbare_rescaling_abstract_identity_narrow_theorem_note_2026-05-10"
...
# ----------------------------------------------------------------------------
section("Part 9: parent / sibling row context (no ledger modification)")
# ----------------------------------------------------------------------------
# The narrow theorem carves out the load-bearing class-A polynomial-algebra
# core that is ALSO embedded inside the existing
# `g_bare_canonical_convention_narrow_theorem_note_2026-05-02` row's
# scoped substitution `beta = 2 N_c / g_bare^2 = 6` at (N_c, g_bare) = (3, 1).
# The narrow theorem here drops the graph-first SU(N_c) dep entirely by
# stating only the abstract symbolic identity.
LEDGER = ROOT / "docs" / "audit" / "data" / "audit_ledger.json"
ledger = json.loads(LEDGER.read_text())
sibling = ledger['rows'].get('g_bare_canonical_convention_narrow_theorem_note_2026-05-02', {})
print(f"\n  Sibling row state on origin/main:")
print(f"    claim_type: {sibling.get('claim_type')}")
print(f"    transitive_descendants: {sibling.get('transitive_descendants')}")
print(f"    load_bearing_step_class: {sibling.get('load_bearing_step_class')}")
print(f"    deps: {sibling.get('deps')}")

check("sibling row exists in ledger (graph-visible)",
      sibling != {},
      detail=f"sibling claim_type={sibling.get('claim_type')!r}")
...
print("""
  ...
  Audit-lane class:
    (A) -- pure polynomial algebra over Q. No external observed/fitted/
    literature/lattice/Cl(3)/SU(N_c) input. Proof is one-line direct
    substitution.
