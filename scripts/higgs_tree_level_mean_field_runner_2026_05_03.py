...
def t8_gap3_lite_demotion():
    ...
    # The note must explicitly cross-reference HIGGS_MASS_DERIVED_NOTE as the
    # actual Higgs-mass route (delegating the prediction).
    derived_route_delegated = (
        "HIGGS_MASS_DERIVED_NOTE.md" in text
        and "3-loop" in text.lower() or "3 loop" in text.lower()
    )
    check(
        "Higgs-mass prediction routed via HIGGS_MASS_DERIVED (3-loop running)",
        derived_route_delegated,
    )
