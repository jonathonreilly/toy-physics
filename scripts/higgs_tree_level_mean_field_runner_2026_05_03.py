...
def t7_step_7_authority_chain():
    ...
    required_sections = [
        ...
        # scope-guard "neither extends nor retires" is checked below using
        # whitespace-normalised text (the phrase can straddle a line wrap)
    ...
    # Step 7 must NOT make a new derivation claim — verify the explicit
    # "neither extends nor retires" disclaim is paired with explicit
    ...
    scope_guard_present = (
        "this note neither extends nor retires its sister" in text_flat.lower()
        or "neither extends nor retires" in text_flat
    )
    no_new_claim = "tree-level formula" in text_flat and "delegated" in text_flat.lower()
    check(
        "Step 7 scope guard: this note neither extends nor retires sisters",
        scope_guard_present,
    )
    check(
