#!/usr/bin/env python3
"""Lorentzian split-delay closure card on the 3D ordered lattice.

THE TEST: Does replacing the uniform valley-linear action S = L*(1-f)
with a Lorentzian split:
  S = L * (1 - f * cos(2*theta))
     = L * (1 - f) at theta=0 (causal: action decreases near mass)
     = L * (1 + f) at theta=pi/2 (spatial: action increases near mass)

still pass Born rule, F∝M, decoherence, and produce TOWARD gravity?

If yes: the model has geometric gravity (geodesics TOWARD mass from
the Lorentzian delay split) AND wave-level Born compliance.

The cos(2*theta) interpolation smoothly transitions from "time
dilation" (causal edges faster near mass) to "spatial stretching"
(spatial edges slower near mass), matching the Schwarzschild metric
structure: g_00 shrinks, g_rr grows.

HEAD-TO-HEAD comparison:
  - Euclidean VL: S = L*(1-f) (current default)
  - Lorentzian VL: S = L*(1 - f*cos(2*theta))

Same lattice, same field, same kernel. Only the action differs.
"""
    ...
    def __init__(self, phys_l, phys_w, h):
        ...
        for dy in range(-self.max_d, self.max_d + 1):
            for dz in range(-self.max_d, self.max_d + 1):
                ...
                # Lorentzian factor: cos(2*theta)
                # At theta=0 (causal): cos(0)=1 → act = L*(1-f)
                # At theta=pi/4: cos(pi/2)=0 → act = L (no field coupling)
                # At theta=pi/2: cos(pi)=-1 → act = L*(1+f)
                lorentz_factor = math.cos(2 * theta)
                self._off.append((dy, dz, L, w, lorentz_factor))
        self._nw = nw

    def propagate(self, field, k, blocked_set, action_mode="euclidean"):
        """Propagate with selectable action mode.

        action_mode:
          "euclidean"  → S = L * (1 - f)          (current default)
          "lorentzian" → S = L * (1 - f*cos(2θ))  (Lorentzian split)
        """
...
def run_card(action_mode, label):
    t_total = time.time()
    ...
    results = {"mode": action_mode, "label": label}
    ...
    results["grav"] = grav
    results["grav_dir"] = dr
    print(f"  3. Gravity z=3 = {grav:+.6f} ({dr})  [{'PASS' if grav > 0 else 'CHECK'}]")
    ...
    print(f"  4. F~M alpha = {fm_alpha:.2f}  [{'PASS' if not math.isnan(fm_alpha) and abs(fm_alpha-1.0)<0.2 else 'CHECK'}]")
    ...
    if len(b_data) >= 3:
        ...
        if peak_i < len(b_data) - 2:
            dist_slope, _ = fit_power(b_data[peak_i:], d_data[peak_i:])
    results["n_toward"] = n_tw
    results["dist_slope"] = dist_slope
    if dist_slope is not None:
        print(f"  7. Distance: {n_tw}/6 TOWARD, tail b^({dist_slope:.2f})")
    else:
        print(f"  7. Distance: {n_tw}/6 TOWARD")

    # 8. k-sweep (quick, 4 values)
    print(f"  8. k-sweep:")
    for kk in [2.0, 5.0, 7.0, 10.0]:
        afk = lat.propagate(field_f, kk, blocked, action_mode)
        pfk = sum(abs(afk[d])**2 for d in det)
        zfk = sum(abs(afk[d])**2*pos[d,2] for d in det)/pfk if pfk > 1e-30 else 0
        amk = lat.propagate(field_m3, kk, blocked, action_mode)
        pmk = sum(abs(amk[d])**2 for d in det)
        zmk = sum(abs(amk[d])**2*pos[d,2] for d in det)/pmk if pmk > 1e-30 else 0
        dk = zmk - zfk
        print(f"      k={kk:.0f}: {dk:+.6f} ({'TOWARD' if dk > 0 else 'AWAY'})")
...
def main():
    print("=" * 70)
    print("LORENTZIAN SPLIT-DELAY CLOSURE CARD")
    print("=" * 70)
    print()
    print("HEAD-TO-HEAD: Euclidean vs Lorentzian action on 3D ordered lattice")
    print()
    print("Euclidean:  S = L * (1 - f)              (current default)")
    print("Lorentzian: S = L * (1 - f * cos(2θ))    (causal-spatial split)")
    print("  θ=0 (causal):   S = L*(1-f) — action decreases (time dilation)")
    print("  θ=π/2 (spatial): S = L*(1+f) — action increases (spatial stretch)")
    print()

    r_euc = run_card("euclidean", "EUCLIDEAN VL: S = L*(1-f)")
    r_lor = run_card("lorentzian", "LORENTZIAN VL: S = L*(1 - f*cos(2θ))")

    # Side-by-side
    print(f"\n{'='*70}")
    print("SIDE-BY-SIDE COMPARISON")
    print(f"{'='*70}")
    print(f"  {'Property':<20} | {'Euclidean':>15} | {'Lorentzian':>15}")
    print(f"  {'-'*55}")
    ...
    for label, key, fmt in [
        ...
        ve = r_euc.get(key)
        vl = r_lor.get(key)
        if isinstance(ve, float) and math.isnan(ve): ve_s = "N/A"
        elif fmt == "s": ve_s = str(ve)
        else: ve_s = f"{ve:{fmt}}"
        if isinstance(vl, float) and math.isnan(vl): vl_s = "N/A"
        elif fmt == "s": vl_s = str(vl)
        else: vl_s = f"{vl:{fmt}}"
        print(f"  {label:<20} | {ve_s:>15} | {vl_s:>15}")
    ...
    lor_born = r_lor["born"] < 1e-10 if not math.isnan(r_lor["born"]) else False
    lor_grav = r_lor["grav"] > 0
    lor_k0 = abs(r_lor["k0"]) < 1e-6
    lor_fm = not math.isnan(r_lor["fm_alpha"]) and abs(r_lor["fm_alpha"] - 1.0) < 0.3

    if lor_born and lor_grav and lor_k0 and lor_fm:
        print(f"\n  LORENTZIAN PASSES ALL CORE TESTS.")
        print(f"  Born: {r_lor['born']:.2e}")
        print(f"  Gravity: {r_lor['grav']:+.6f} {r_lor['grav_dir']}")
        print(f"  F~M: {r_lor['fm_alpha']:.2f}")
        print(f"  k=0: {r_lor['k0']:.6f}")
        print()
        print(f"  The Lorentzian split-delay action produces gravity that is")
        print(f"  both GEOMETRIC (geodesics toward mass, from earlier test)")
        print(f"  and WAVE-COMPLIANT (Born rule at machine precision).")
    elif lor_born and not lor_grav:
        print(f"\n  Born PASSES but gravity is AWAY.")
        print(f"  The Lorentzian split changes the wave resonance window.")
        print(f"  k=5 may no longer be in the attractive window.")
    elif not lor_born:
        print(f"\n  Born FAILS ({r_lor['born']:.2e}).")
        print(f"  The Lorentzian split breaks the Born rule.")
    else:
        print(f"\n  Mixed results. Check individual tests above.")
