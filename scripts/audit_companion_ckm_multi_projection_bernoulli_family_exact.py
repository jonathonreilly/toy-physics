...
with the framework constraint N_quark = N_pair * N_color (retained), and
...
which at retained (N_pair, N_color) = (2, 3) specialize to rho = 1/6 and
A^2 rho = 1/9.

The existing primary runner
(`scripts/frontier_ckm_multi_projection_bernoulli_family.py`) verifies
these identities at exact-Fraction precision at the retained framework
...
exact-symbolic verification:
  ...
  (a) treats (N_pair, N_color, N_quark) as positive-integer symbols
      with the retained framework constraint N_quark = N_pair N_color;
  (b) verifies the universal Bernoulli relation V(N) = M(N)/N
      parametrically over an abstract positive-integer symbol N;
  (c) verifies (B1)-(B6) at retained (N_pair, N_color, N_quark) = (2, 3, 6)
...
def main() -> int:
    ...
    # ---------------------------------------------------------------------
    section("Part 2: (B1)-(B6) at retained (N_pair, N_color, N_quark) = (2, 3, 6)")
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # M(N_color) = N_pair/N_color = A^2 (retained W2). Not a new derivation,
    ...
    check(
        "M(N_color) == A^2 = N_pair/N_color at (N_pair, N_color) = (2, 3) (retained W2)",
    ...
    # Counterfactual: at (p, c) = (3, 2) (swap the values), the roles
    # would interchange to give rho = 1/6 still (since pc is symmetric)
    # but A^2 rho = (c-1)/(p c^2) = 1/(3*4) = 1/12, not 1/9.
    ...
    print("    (B1)-(B6) all six Bernoulli values at retained (2, 3, 6)")
    ...
    print("    (D2) V(N_color) M(N_pair) == A^2 rho = 1/N_color^2 under q = p c")
