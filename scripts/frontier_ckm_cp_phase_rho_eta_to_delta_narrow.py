...
Verifies the standalone polynomial-algebra identities
  ...
  Given abstract real positive symbols (w_axis, w_perp, r^2) with
  partition (P)  w_axis + w_perp = 1,
  and abstract integer k >= 2:

  (T1)  rho^2 + eta^2 = r^2          where rho^2 := r^2 * w_axis, eta^2 := r^2 * w_perp.
  (T2)  cos^2(delta)  = w_axis         where cos^2(delta) := rho^2 / (rho^2 + eta^2).
  (T3)  sin^2(delta)  = w_perp       where sin^2(delta) := eta^2 / (rho^2 + eta^2).
  (T4)  cos^2 + sin^2 = 1            (from (T2) + (T3) + (P)).
  (T5)  tan^2(delta)  = w_perp / w_axis.
  (T6)  Specialising w_axis = 1/k, w_perp = (k-1)/k:
  ...
  (T10) Diagnostic witness on a Wolfenstein-truncated 3x3 matrix whose apex
        (rho, eta) = (1/6, sqrt(5)/6): arctan(eta/rho) = arctan(sqrt(5)),
        with row-normalization / row-orthogonality checked at O(lambda^4).
...
# Partition hypothesis (P): w_axis + w_perp = 1.
# We enforce by substituting w_perp -> 1 - w_axis.
P_subs = {w_perp: 1 - w_axis}
...
# ----------------------------------------------------------------------------
section("Part 2: (T2) cos^2(delta) = w_axis")
# ----------------------------------------------------------------------------
T2_diff = simplify((cos2_delta_def - w_axis).subs(P_subs))
check("(T2) cos^2(delta) - w_axis = 0 under (P)",
      T2_diff == 0,
      detail=f"diff = {T2_diff}")
# Also record the off-(P) form for transparency.
T2_offP = simplify(cos2_delta_def - w_axis / (w_axis + w_perp))
check("(T2-offP) cos^2(delta) = w_axis / (w_axis + w_perp) symbolically (no (P) needed)",
      T2_offP == 0,
...
# Also record the off-(P) form for transparency.
T3_offP = simplify(sin2_delta_def - w_perp / (w_axis + w_perp))
check("(T3-offP) sin^2(delta) = w_perp / (w_axis + w_perp) symbolically (no (P) needed)",
      T3_offP == 0,
...
# ----------------------------------------------------------------------------
section("Part 5: (T5) tan^2(delta) = w_perp / w_axis")
# ----------------------------------------------------------------------------
T5_diff = simplify(tan2_delta_def - w_perp / w_axis)
check("(T5) tan^2(delta) - w_perp/w_axis = 0 symbolically",
      T5_diff == 0,
      detail=f"diff = {T5_diff}")


# ----------------------------------------------------------------------------
section("Part 6: (T6) integer-k specialisation w_axis = 1/k, w_perp = (k-1)/k")
# ----------------------------------------------------------------------------
k_subs = {w_axis: 1 / k, w_perp: (k - 1) / k}
...
# ----------------------------------------------------------------------------
k_six = {w_axis: Rational(1, 6), w_perp: Rational(5, 6)}

T7a_val = simplify(cos2_delta_def.subs(k_six))
check("(T7a) cos^2(delta) = 1/6 exact at (w_axis, w_perp) = (1/6, 5/6)",
      T7a_val == Rational(1, 6),
      detail=f"cos^2 = {T7a_val}")

T7b_val = simplify(tan2_delta_def.subs(k_six))
check("(T7b) tan^2(delta) = 5 exact at (w_axis, w_perp) = (1/6, 5/6)",
      T7b_val == 5,
...
# ----------------------------------------------------------------------------
section("Part 10: (T10) Wolfenstein-truncated matrix witness with apex (rho, eta) = (1/6, sqrt(5)/6)")
# ----------------------------------------------------------------------------
# Construct a Wolfenstein-truncated 3x3 matrix whose leading apex is (1/6, sqrt(5)/6).
...
# of any particular matrix witness.
lam_val = 0.225
...
# ----------------------------------------------------------------------------
section("Part 11: Off-cone sanity (uniform partition w_axis = w_perp = 1/2)")
# ----------------------------------------------------------------------------
# At w_axis = w_perp = 1/2: cos^2 = 1/2, tan^2 = 1, delta = pi/4 = 45 deg.
# This is the "balanced" partition and demonstrates the cone is non-trivial.
uniform_subs = {w_axis: Rational(1, 2), w_perp: Rational(1, 2)}
cos2_uniform = simplify(cos2_delta_def.subs(uniform_subs))
tan2_uniform = simplify(tan2_delta_def.subs(uniform_subs))
check(
    "uniform partition w_axis = w_perp = 1/2 gives cos^2 = 1/2 (off the k=6 cone)",
