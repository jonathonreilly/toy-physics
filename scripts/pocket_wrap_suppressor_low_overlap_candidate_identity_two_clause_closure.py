#!/usr/bin/env python3
"""Probe two-clause closures after adding candidate-identity observables."""
...
from pocket_wrap_suppressor_low_overlap_candidate_identity_bucket import (  # noqa: E402
    build_rows as build_candidate_rows,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--predicate-limit", type=int, default=28)
...
def build_rows(
    ...
    *,
    ...
    support_rows, _sig2, _bucket2 = build_support_rows(
        log_path,
        left_subtype=left_subtype,
        right_subtype=right_subtype,
        signature_terms=signature_terms,
    ...
    candidate_rows, _events, _sig3, _bucket3 = build_candidate_rows(
        log_path,
        left_subtype=left_subtype,
        right_subtype=right_subtype,
        signature_terms=signature_terms,
    ...
    candidate_by_source = {getattr(row, "source_name"): row for row in candidate_rows}

    rows: list[dict[str, object]] = []
    for combined in combined_rows:
        support = support_by_source[combined.source_name]
        candidate = candidate_by_source[combined.source_name]
        feature_map: dict[str, float] = {}
        for field in fields(type(combined)):
            if field.name not in {"source_name", "subtype"}:
                feature_map[field.name] = float(getattr(combined, field.name))
        for field in fields(type(support)):
            if field.name not in {"source_name", "subtype"}:
                feature_map[field.name] = float(getattr(support, field.name))
        for field in fields(type(candidate)):
            if field.name not in {"source_name", "subtype"}:
                feature_map[field.name] = float(getattr(candidate, field.name))
        rows.append(
            {
...
def clause_candidates(predicates: list[tuple[str, int]], *, row_count: int, max_terms: int) -> list[tuple[str, int, int]]:
...
def best_rows(
    ...
    *,
    ...
    for target_subtype in target_subtypes:
        ...
        results.extend(scored_rows[:8])
    return results


def render_rows(rule_rows: list[dict[str, object]]) -> str:
    lines = [
        "Candidate-Identity Two-Clause Closures",
        "======================================",
...
def main() -> None:
    ...
    print(f"candidate-identity two-clause closure started {started}", flush=True)
    ...
    print()
    print("Candidate-Identity Two-Clause Closure")
    print("=====================================")
    ...
    print()
    print(
        f"candidate-identity two-clause closure completed {datetime.now().isoformat(timespec='seconds')} total_elapsed={time.time() - total_start:.1f}s",
        flush=True,
    )
