...
STATUS: explicit construction of the G1-analogous P3 closure route for G5.

Architecture (mirroring G1 Physicist-H / PMNS-as-f(H)):
  ...
  - G5's analogue is the retained second-order Gamma_1 return map
      (w_O0, w_a, w_b)  ->  (m_e, m_mu, m_tau)   (up to overall scale)
    from the Dirac-bridge Gamma_1 second-order return (Agent 10 v2 theorem),
    observationally pinned via PDG charged-lepton masses.

Core steps:
  1. Replicate Agent 10 v2 structural shape theorem: diag(Sigma) = (w_O0, w_a, w_b).
...
Four-outcome verdict:
  ...
  G5_OBSERVATIONAL_PIN_CLOSES = TRUE_NO_PREDICTION
...
# ----------------------------------------------------------------------
# STEP 1: Replicate Agent 10 v2 structural shape theorem
# ----------------------------------------------------------------------


def step1_shape_theorem():
    print("=" * 78)
    print("STEP 1: Replicate Agent 10 v2 structural shape theorem")
    print("=" * 78)
...
def step2_retained_chamber():
    ...
    #
    # (R5) S_2-BROKEN. Agent 10 v2 showed that the retained S_2 symmetry on
...
def step3_observational_pin():
    ...
    # Agent 10 v2 compared against (m_e, m_mu, m_tau) directly (the Phase 3
    # comparison of the Agent 10 v2 runner). We follow the same convention:
    ...
    # the "direction cos-sim" comparison used in Agent 10 v2.)
    ...
    print("     schemes lack (consistent with Agent 10 v2 open primitive).")
    print()
    return w_normalized
...
def step5_cross_check_G1(w_pin):
    ...
    # neutrino side (Physicist-H PMNS-as-f(H) theorem).
    #
    # Is there a retained relationship between G1's chamber coordinates and
    # G5's weight triple (w_O0, w_a, w_b)?
    #
    ...
    #
    # Key architectural fact (from Agent 9's scope theorem and the Dirac-bridge
    ...
    #
    # Build H(m_*, delta_*, q_+*) per Physicist-H construction.
    ...
    # Agent 9 theorem: |evals_H|, evals_H^2 don't match charged-lepton direction.
...
def step6_predictions(w_pin):
    ...
    # G5's analogue: what new falsifiable predictions emerge from the
    # observational pin?
    #
    # The retained map is
    ...
    # 206.77 : 16.82. Any retained S_2-breaking primitive (Agent 12 lane)
    # would predict a specific ratio between w_a - w_b and some retained
    # scalar (alpha_LM, Casimir, etc.). This is a TARGET for Agent 12, not
    # a prediction here.
    #
    ...
    print()
    print("  (P1) RATIO PREDICTION (target for Agent 12):")
...
def step7_closure_class(w_pin, predictions):
    ...
    # G1 closure was labeled "retained-map-plus-observational-promotion (not
    ...
    print(f"  G5 closure class     : {label}")
    ...
    print("    Both cases: Physicist-G-style microscopic-polynomial")
    ...
    check("G5 closure class matches G1 label 'retained-map-plus-observational-promotion'",
          label == "retained-map-plus-observational-promotion")
    return label
...
def step8_verdict(w_pin, chamber, predictions, closure_label):
    ...
    if inside_chamber and unique_pin and koide_ok and any_prediction:
        verdict = "G5_OBSERVATIONAL_PIN_CLOSES = TRUE"
        interp = "Full G5 closure at G1 parity via P3 observational pin."
    elif inside_chamber and unique_pin and koide_ok and not any_prediction:
        verdict = "G5_OBSERVATIONAL_PIN_CLOSES = TRUE_NO_PREDICTION"
...
def main():
    print("=" * 78)
    print("G5 OBSERVATIONAL-PIN CLOSURE — the G1-analogous route")
    print("=" * 78)
    print()
