#!/usr/bin/env python3
"""Bounded action crossover sweep on DAGs with tunable regularity.

This is the frozen replay for the action-regularity crossover memo.
It compares spent-delay and valley-linear on the same generated DAG
family across a regularity interpolation:

  regularity = 0.0  -> random node placement
  regularity = 1.0  -> grid-like node placement

The goal is not to prove a universal bridge. The goal is to freeze the
observed crossover on the tested family as an artifact-backed branch fact.
"""
...
def main():
    print("=" * 60)
    print("ACTION REGULARITY CROSSOVER")
    print("  Spent-delay vs valley-linear at tunable regularity")
    print("  Frozen replay: bounded branch fact, not a universal theorem")
    print("=" * 60)
    ...
    for reg in [0.0, 0.2, 0.4, 0.6, 0.8, 0.95]:
        ...
        delta = v_r - s_r
        rows.append((reg, v_tw, v_tot, v_r, s_tw, s_tot, s_r, delta))
        print(f"  {reg:5.2f}  {v_tw:2d}/{v_tot:2d} ({v_r:4.0%})  {s_tw:2d}/{s_tot:2d} ({s_r:4.0%})  {delta:+5.0%}")

    if rows:
        best = max(rows, key=lambda r: r[-1])
        print("\n  Safe read:")
        print(f"    best observed delta at reg={best[0]:.2f}")
        print("    crossover is branch-specific and only supported on the tested slice")
        print("    no universal bridge theorem is claimed here")
