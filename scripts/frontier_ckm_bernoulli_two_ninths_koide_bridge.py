...
Verifies the new identities in
  docs/CKM_BERNOULLI_TWO_NINTHS_KOIDE_BRIDGE_THEOREM_NOTE_2026-04-25.md
  ...
  (K3) Consistency: K1 = K2 = K5 = K6 = 2/9 <==> N_pair = 2, N_color = 3
...
- Implicit structural primitive N_pair = N_color - 1 (from retained values)
...
def audit_inputs() -> None:
    ...
    print(f"  N_pair = N_color - 1?       {N_PAIR == N_COLOR - 1}  (structural primitive)")
    print()
    ...
    check("N_pair = N_color - 1 (structural primitive)", N_PAIR == N_COLOR - 1)
...
def audit_k3_consistency() -> None:
    banner("(K3) NEW: K1 = K2 = K5 = K6 = 2/9 <==> N_pair = N_color - 1 = 2")
    ...
    print(f"    N_pair = N_color - 1?  N_pair={N_PAIR}, N_color-1={N_COLOR-1}, equal: {N_PAIR == N_COLOR - 1}")
    print(f"    N_color = 3?            {N_COLOR == 3}")

    check("(K3) K1 = K2 (algebraic equality)", K1 == K2)
    check("(K3) K2 = K5 (algebraic equality)", K2 == K5)
    check("(K3) K5 = K6 (algebraic equality)", K5 == K6)
    check("(K3) All four paths converge to 2/9", K1 == K2 == K5 == K6 == TARGET)
    check("(K3) Consistency: N_pair = N_color - 1 (structural primitive)",
          N_PAIR == N_COLOR - 1)
...
def audit_cross_sector_reading() -> None:
    banner("Cross-sector reading (SUPPORT, NOT closure): Koide 2/9 ~ K1 form")
    ...
    print(f"  CKM K1 collapsed form: (N_color - 1)/N_color^2 = {Fraction(N_COLOR - 1, N_COLOR ** 2)}")
    ...
    print("    then Koide variance ratio = K1 collapsed form = 2/9 EXACTLY.")
    ...
    check("CKM K1 = Koide variance form (BOTH equal 2/9, SAME structural form)",
...
def audit_summary() -> None:
    ...
    print()
    print("  NEW (K3): K1 = K2 = K5 = K6 = 2/9 <==> N_pair = N_color - 1 = 2")
    print("            Structural consistency theorem: convergence of all paths is")
    print("            EQUIVALENT to the framework's primitive integer counting.")
    print()
    ...
    print()
    print("  Cross-sector reading (SUPPORT, NOT closure): K1 form matches conjectural")
    print("  Koide variance (N_gen - 1)/N_gen^2 = 2/9 with N_gen = 3 = N_color.")
    print()
    print("  All four CKM paths to 2/9 derived from retained inputs only.")
...
def main() -> int:
    ...
    print("See docs/CKM_BERNOULLI_TWO_NINTHS_KOIDE_BRIDGE_THEOREM_NOTE_2026-04-25.md")
    print("=" * 88)
