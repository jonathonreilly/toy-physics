...
Combines two existing framework pieces:
...
plus the schedule's renormalization choice has an h-stable gravity
response candidate on this harness.
...
Either outcome is bounded evidence, not an audit verdict.
"""
...
def main() -> int:
    ...
    # H-stability criterion: |alpha| close to 0 + small max_rel_dev + R^2 not
    # required to be high (R^2 is degenerate when gravity is h-stable, since
    # the variance in log(|gravity|) collapses).
    h_stable_alpha_max = 0.15
    h_stable_dev_max = 0.05
    h_stable = []
    for scheme in SCHEMES:
        alpha, _C, _r2 = fits[scheme]
        _mean, max_rel = stabilities[scheme]
        if (not math.isnan(alpha)
                and abs(alpha) <= h_stable_alpha_max
                and not math.isnan(max_rel)
                and max_rel <= h_stable_dev_max):
            h_stable.append(scheme)
    if h_stable:
        print(f"  H-STABLE: scheme(s) {h_stable} pass the h-stable criterion")
        print(f"    (|alpha| <= {h_stable_alpha_max}, "
              f"max_rel_dev <= {h_stable_dev_max})")
        print()
        ...
        print("  h-stable gravity response candidate on this harness.")
    else:
        print(f"  NO H-STABLE SCHEDULE: no scheme passes |alpha| <= {h_stable_alpha_max}")
        print(f"    AND max_rel_dev <= {h_stable_dev_max}")
        print()
    ...
    print()
    if not all_born_clean or not all_k0_clean or h_stable:
        return 1
    return 0
