...
Whenever det(H) != 0 (the endpoint half of PNS), the diagonalizing U
is in U(3) automatically, so the constructed PMNS matrix
U_PMNS = U_e^dag U_nu is unitary by U(3) closure. This narrows what PNS
must still supply to the path-continuity half on (0,1) only.
...
# result that IS derivable from Cl(3)/Z3 algebra alone:
#
#   Given any Hermitian Cl(3) endpoint H = H_base + J with det(H) != 0
#   (the endpoint half of PNS), the diagonalizing unitary U is in U(3)
#   automatically, by the spectral theorem on Hermitian matrices, and
#   the constructed PMNS matrix U_PMNS = U_e^dagger U_nu is unitary as
#   a product of two U(3) elements (closure of U(3) under composition).
#
# This narrows what PNS still has to provide: only the path-continuity
# half (no eigenvalue zero-crossing on (0,1)). Endpoint unitarity is
# automatic. The full A-BCC closure stack now reads:
#
#   A-BCC <- PNS_path (sole input) + IVT + det(H_base)>0
#                                      + Cl(3) spectral theorem (U_PMNS unitary).
#
...
def task10_upmns_unitarity_from_spectral_theorem():
    ...
    print("  Claim: Given det(H) != 0 for Hermitian H in Cl(3),")
    print("         the diagonalizing U from H = U diag(lambda) U^dag is in U(3).")
    print("         Then U_PMNS = U_e^dag U_nu is unitary by U(3) closure.")
    print("  This is the algebraic companion to the path-PNS axiom; it is")
    print("  automatic from spectral-theorem closure on Hermitian matrices.")
    ...
    for _ in range(200):
        ...
        if abs(np.linalg.det(H).real) < 1e-9:
            continue  # skip near-singular samples (PNS endpoint half)
    ...
    check("Random Cl(3) Hermitian non-singular: ||U^dag U - I||_F < 1e-10",
          max_resid_sample < 1e-10,
          f"max residual = {max_resid_sample:.2e}")
    check("Random Cl(3) Hermitian non-singular: |det U| = 1 (in U(3))",
...
#
# Sympy verification of the algebraic identity behind T10: for any 3x3
# Hermitian matrix with three distinct real eigenvalues lambda_1, lambda_2,
# lambda_3, the column-normalized eigenvector matrix V satisfies
# V^dagger V = I in exact symbolic arithmetic.
# ---------------------------------------------------------------------------
