...
It deliberately does NOT try to:
  ...
  - Verify the new derivation closes the chain (that's the audit lane's
    job; the PR triggers a re-audit on next pipeline run)
...
Usage:
    ...
    # Pick a specific row
    python3 scripts/science_fix_loop.py --runners <claim_id>
...
- [ ] Read the diff carefully — automated science attempts can be wrong
- [ ] If the new derivation looks valid, merge and let the next audit
      batch re-audit the row
