#!/usr/bin/env python3
"""NN lattice rescaled-lane non-Schrödinger kernel no-go.

Tests the deterministic-rescale NN lane's scattering-kernel candidate on
observables that are NOT the slit-detector decoherence vector.
...
blocked nodes), computed on the deterministic-rescale lane at refinement
values h ∈ {0.5, 0.25, 0.125, 0.0625}.

The lattice configuration is built directly from the local harness
parameters below; no external empirical data is used.
...
If the magnitude is Gaussian-stable and the phase is quadratic-stable,
the scoped harness can be checked against the Schrödinger free-particle
propagator
...
The landed claim is a bounded no-go: the run is expected to show
Gaussian magnitude and quadratic phase while the two effective-mass
extractions disagree by more than 5%. If that expected no-go is not
found, the runner exits nonzero.
...
- Cross-validation: two m_eff extractions disagree by > 5% for the
  proposed no-go

Exit zero only when the scoped no-go and all runner guards pass.
"""
...
def main() -> int:
    print("=" * 100)
    print("NN LATTICE RESCALED-LANE NON-SCHRÖDINGER KERNEL NO-GO")
    ...
    expected_no_go = (mag_pass and phs_pass and not cross_ok
                      and mag_winners[-1] == "gaussian"
                      and phs_winners[-1] == "quadratic")
    ...
    if positive:
        ...
        print()
        print("Expected for this source note:")
        print("  - finest-h magnitude winner is Gaussian with R^2 >= 0.95")
        print("  - finest-h phase winner is quadratic with R^2 >= 0.95")
        print("  - phase-derived and width-derived m_eff disagree by > 5%")
        print()
        ...
        print()
        print("Scoped constraints on the candidate space:")
        if mag_winners[-1] == "gaussian" and mag_pass:
            print("  - magnitude is Gaussian-shaped: T_∞'s scoped kernel has")
            print("    Gaussian-decaying magnitude.")
        else:
        ...
        print(f"  kernel must reproduce the computed magnitude/phase shapes")
        ...
        print(f"  K_PHYS = 5.0. This is a bounded no-go result that")
    ...
    f0 = fit_at_h(rec0)
    _print_fits(f0)
    max_imag0 = max(abs(v) for v in rec0["A_im"])
    max_phase0 = max(abs(p) for p in rec0["phase_raw"])
    k0_phase_pass = max_imag0 < 1e-10 and max_phase0 < 1e-10
    print()
    print(f"  max |Im(A)| at K_PHYS = 0 = {max_imag0:.3e}: "
          f"{'PASS' if max_imag0 < 1e-10 else 'FAIL'}")
