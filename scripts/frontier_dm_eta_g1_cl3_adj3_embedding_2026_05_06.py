...
m_DM = N_sites * v = 16 v as the prior candidate for the dark
...
- HIGGS_MASS_FROM_AXIOM_NOTE (N_sites = 2^d = 16 source)
...
as PARTIAL: 8/3 derived structurally from cited Fierz + chiral cube
primitives; the dynamical step (why this density factor multiplies the
Wilson bare mass for the dark singlet) remains open.
"""
...
def main() -> None:
    ...
    print(f"  Match to cited 8/9: {'PASS' if t3 else 'FAIL'}")
    ...
    print(f"  ρ_{{adj/c}} * 2 r * hw_dark = {rho_carrier} * {2 * r * hw_dark}"
          f" = {N_sites_pred}")
    print(f"  N_sites (source, 2^d for d=4 spacetime APBC): {N_sites_target}")
    ...
    print("-" * 78)
    v = 246.282818290129  # canonical-surface EW v (in GeV)
    ...
    print(f"  v (canonical EW VEV) = {v} GeV")
    ...
    print()
    print("  The factor 8/3 is now derived from cited Fierz + chiral cube")
    ...
    print("  the dark singlet remains open; this is bounded algebraic support,")
    print("  not a parent-status change.")
