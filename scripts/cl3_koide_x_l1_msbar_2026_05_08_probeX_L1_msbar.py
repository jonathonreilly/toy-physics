...
Purpose
=======
Test whether the Cl(3)/Z^3 framework's retained content can derive the
...
Verdict structure
=================
The probe is bounded_theorem (mostly negative on full derivation, with
positive retentions on universal coefficients and on the color-tensor
skeleton).

Positive retentions (PASS expected):
  1. beta_0 = (11 N_color − 2 N_quark)/3 = 7 at N_f=6 (universal,
     retained via S1+Casimir)
  2. beta_1 = (34/3) C_A^2 − (20/3) C_A T_F N_f − 4 C_F T_F N_f = 26
     at N_f=6 (universal at 2-loop)
  3. Color-tensor skeleton at 3-loop: 9-channel decomposition retained
  4. Color-tensor skeleton at 4-loop: extended quartic-Casimir retained
  5. <P>_HK_SU(3)(s_t) = 1 - exp(-(4/3) s_t) closed form retained as
...
# ----------------------------------------------------------------------
# SECTION 1 — POSITIVE RETENTION: beta_0 (1-loop, universal)
# ----------------------------------------------------------------------
...
def section1_beta_0_retained(c: Counter) -> None:
    ...
    At retained N_color=3, N_quark=6: b_3 = (33-12)/3 = 21/3 = 7.

    This coefficient is SCHEME-INDEPENDENT (universal at 1-loop in
    MSbar, MOM, lattice, <P>-scheme — all the same).
    """
    print("Section 1 — POSITIVE RETENTION: beta_0 (1-loop) = 7 universal")
    ...
    print("    → beta_0 is RETAINED on framework via S1+Casimir.")
    print("    → SCHEME-INDEPENDENT: same value in MSbar, MOM, lattice, <P>.")


# ----------------------------------------------------------------------
# SECTION 2 — POSITIVE RETENTION: beta_1 (2-loop, universal)
# ----------------------------------------------------------------------
...
def section2_beta_1_retained(c: Counter) -> None:
    ...
    With retained (C_F=4/3, C_A=3, T_F=1/2, N_f=6):
    ...
    """
    print()
    print("Section 2 — POSITIVE RETENTION: beta_1 (2-loop) = 26 universal")
    ...
    print("    → beta_1 is RETAINED on framework via Casimir algebra.")
...
def section3_three_loop_color_skeleton(c: Counter) -> None:
    ...
    The framework retains the SKELETON (the channel basis from
    Casimir algebra) but NOT the channel weights (scalars).

    This is the same pattern as YT_P3_K_3 retention: color tensors retained,
    integral primitives cited.
    """
    print()
    print("Section 3 — POSITIVE STRUCTURAL: 3-loop Casimir-tensor skeleton retained")
    ...
    print("    → All 9 Casimir channels at 3-loop are framework-retained.")
    print("    → The channel WEIGHTS (scalars c_FFF, ..., c_Ann) are NOT.")
    print("    → This matches the K_2/K_3 retention pattern: skeleton retained,")
...
def section4_four_loop_color_skeleton(c: Counter) -> None:
    ...
    These are STILL retained Casimir algebra (the quartic invariants
    are computed from group theory alone; no integral primitives needed).
    """
    print()
    print("Section 4 — POSITIVE STRUCTURAL: 4-loop quartic-Casimir basis retained")
    ...
    print("    → 4-loop quartic Casimir basis is framework-retained (group theory only).")
    print("    → Channel weights at 4-loop NOT framework-retained.")
...
def section6_scheme_distinction_real(c: Counter) -> None:
    ...
    # alpha_bare = g_bare^2/(4 pi) = 1/(4 pi) [retained g_bare=1]
...
def section7_beta_2_bounded_admission(c: Counter) -> None:
    ...
    SCHEME-DEPENDENT integral primitives outside retained content.
    ...
    Neither set is in retained content. The framework has the
    ...
    for name in channels_3loop_names:
        c.admit(
            ...
            "outside retained Cl(3)/Z^3 content",
        )

    print("    → 9 scalar 3-loop channel weights are NOT framework-retained.")
...
def section8_beta_3_bounded_admission(c: Counter) -> None:
    ...
    print("    → 17+ channel scalars at 4-loop NOT framework-retained.")
...
def section9_msbar_literature_comparator(c: Counter) -> None:
    ...
    NOTE on sign: the standard convention puts beta(g) =
      −beta_0 g^3/(16 pi^2) − beta_1 g^5 ... so beta_n > 0 for
    asymptotic freedom. The value 65/2 is in the convention
    sign(beta_n) = + for asymptotic freedom; the formula above gives
    −65/2 in the opposite convention. We verify the magnitude.
...
def section10_lattice_scheme_comparator(c: Counter) -> None:
    ...
    d_1 in closed form from retained content alone. The 2-loop
    coefficient d_2 (Lüscher-Weisz 1995) is a published lattice number
    not on the framework retention surface.
    ...
    c.record(
        ...
        True,
        "literature comparator only; full Brillouin-zone integral not in retained content",
    )
...
def section11_verdict(c: Counter) -> None:
    ...
    print("=" * 72)
    print()
    print("Claim type: bounded_theorem (mostly negative on full derivation,")
    print("            with positive retentions at 1-loop, 2-loop, and")
    print("            color-tensor skeletons at 3-loop and 4-loop)")
    print()
    print("POSITIVE retentions:")
    ...
    print("  - Confirms that beta_0=7, beta_1=26 ARE retained (already on main)")
    print("  - Adds structural retention of 3-loop, 4-loop color skeletons")
