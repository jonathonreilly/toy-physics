...
by upgrading the three atoms (AC_phi, AC_lambda, AC_phi_lambda) from
structural-only checks to interval-certified rigorous content where
applicable, and demoting the AC_phi obstruction to a retained-class
structural no-go theorem.

Atoms after this rigorization pass:

  AC_lambda  --  CLOSED via interval-certified Kawamoto-Smit
  ...
  AC_phi     --  CONVERTED FROM OBSTRUCTION TO STRUCTURAL NO-GO.
...
def AC_lambda_block_diagonal_certificate() -> Dict[str, object]:
    ...
    return {
        ...
        "AC_lambda_closure_status": "rigorously closed via interval-certified Kawamoto-Smit block-diagonality",
        "upstream_authority": (
...
def AC_phi_no_go_certificate() -> Dict[str, object]:
    ...
    return {
        ...
        "AC_phi_no_go_status": (
            "RECLASSIFIED FROM 'OPEN OBSTRUCTION' TO 'STRUCTURAL NO-GO "
            "WITHIN A_MIN'. The C_3-preserved interpretation note "
...
def main() -> int:
    ...
    print(f"\nAC_LAMBDA RIGOROUSLY CLOSED: {AC_lambda_pass}")
    ...
    print(f"\nAC_PHI STRUCTURAL NO-GO INTERVAL-CERTIFIED: {AC_phi_no_go}")
    ...
    print("=" * 78)
    summary = {
        "AC_lambda_rigorously_closed_interval_cert": AC_lambda_pass,
        "AC_phi_structural_no_go_interval_cert": AC_phi_no_go,
    ...
    if all_pass:
        ...
        print("  AC_lambda  -- RIGOROUSLY CLOSED (interval certificate)")
        print("  AC_phi     -- STRUCTURAL NO-GO WITHIN A_MIN")
        print("                (demoted from open residual)")
        ...
        print("  - AC_lambda: closed via Kawamoto-Smit interval cert")
        print("  - AC_phi: structural no-go (not 'admitted obstruction')")
