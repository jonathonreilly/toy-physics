...
This runner answers that on the current package by constructing local
...
def main() -> int:
    ...
    print(f"Conservative package budget      = {base.CONSERVATIVE_REL_BUDGET:.6e}")
    print()
    ...
    record(
        "the reduced nonlocal tail remains on the same intrinsic package nonlocal scale across the microscopic locality tube",
        max_tail < 1.20 * base.NONLOCAL_RATIO_OP,
        f"max reduced tail ratio={max_tail:.6e}, package ratio={base.NONLOCAL_RATIO_OP:.6e}",
        status="BOUNDED",
    )
    record(
        "the remaining y_t ambiguity is no longer microscopic admissibility on the current tested scale",
    ...
    print(
        "So the current package no longer needs a separate microscopic-admissibility "
