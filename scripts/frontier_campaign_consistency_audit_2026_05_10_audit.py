#!/usr/bin/env python3
"""Review-hygiene runner for the 2026-05-10 cross-PR campaign consistency audit.

Verifies the structural/textual properties of
docs/CAMPAIGN_CONSISTENCY_AUDIT_CLEAN_NOTE_2026-05-10_audit.md without
running any physics computation. The audit note is meta, audit-only;
...
NOTE = REPO / "docs" / "CAMPAIGN_CONSISTENCY_AUDIT_CLEAN_NOTE_2026-05-10_audit.md"
...
def main() -> int:
    if not NOTE.exists():
        print(f"FAIL: missing audit note at {NOTE}")
        return 1
    ...
    # 1. Meta classification + audit-only framing.
    results.append(
        fmt(
            "meta classification + audit-only",
            check_present(
                text,
                "**Type:** meta",
                "**Claim type:** meta",
    ...
    results.append(
        fmt(
            "verdict CLEAN",
            check_present(text, "**Verdict:** **CLEAN**", "no contradictions"),
        )
    )
    ...
    results.append(
        fmt(
            ...
            "frontier_campaign_consistency_audit_2026_05_10_audit.py" in text,
        )
    )
    ...
    results.append(
        fmt(
            ...
            "frontier_campaign_consistency_audit_2026_05_10_audit.txt" in text,
        )
    )
