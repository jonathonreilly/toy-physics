...
def part3_minimal_local_spectral_law_no_go() -> None:
    ...
    trace_expr = -2 * sp.sqrt(6) / 3 + 4 * sp.sqrt(2) / 9
    ...
    check(
        ...
        sp.simplify(sp.diff(gap_sq_expr / trace_expr, x) - 8 * x / trace_expr) == 0,
        detail="Gap(K2)^2 / Tr(K2) is still affine in x^2 because Tr(K2) is constant",
    )
