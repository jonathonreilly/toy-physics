...
Chain #958, V-Quark-Dynamical #981) foreclosed heavy-quark mass
derivation from retained Cl(3)/Z^3 content alone. V-Quark-Dynamical
identified the species-DIFFERENTIATION primitive on y_q(M_Pl) UV BC as
the lock.

Probe U tests an explicit new-science tool: SL(2,Z) modular flavor
...
Verdict structure
=================
The probe is bounded_theorem (negative for the heavy-quark gate; the
SL(2,Z) modular flavor symmetry tool does NOT bridge the heavy-quark
Yukawa-BC gap on retained content).

Six load-bearing ingredients per Z-S4b-Audit hostile-review pattern:

  I1 SL(2,Z) modular flavor symmetry as a tool: IMPORTED
     (Feruglio 2017 framework; new-science scope authorization is
     present but the toolkit is admitted).
  ...
  I4 Modulus tau = omega (C_3 fixed point / self-dual cusp): NOT RETAINED
     (new structural assumption required).
...
Numerical predictions per analysis:
...
(B) At tau = omega with weight-2 only, the 3-vector Y_2(omega) =
    (1, omega, omega^2) up to canonical normalization. Magnitudes are
    all unity. The canonical A_4 Yukawa matrix at tau = omega is
    ...
    inconsistent with PDG (m_t, m_c, m_u) being all non-zero.
...
Verdict: BOUNDED THEOREM (negative). SL(2,Z) modular flavor symmetry on
Gamma(3) at tau = omega does NOT bridge the heavy-quark Yukawa-BC gap
on retained content. The species-differentiation primitive on y_q(M_Pl)
remains an open gap.
...
# ----------------------------------------------------------------------
# Retained anchors (carried forward from V-Quark-Dynamical)
# ----------------------------------------------------------------------
...
# Retained Z_3 charges (CHARGED_LEPTON_UE_IDENTITY_VIA_Z3_TRICHOTOMY)
...
TAU_OMEGA = OMEGA  # modulus fixed at tau = omega (NEW assumption I4)
...
def section1_retained_anchors(c: Counter) -> None:
    ...
    c.record(
        "Z_3 q_L = (0, +1, -1) (retained generation cycle)",
        Q_L == (0, 1, -1),
        f"q_L = {Q_L}",
    )
    c.record(
    ...
    print("    -> RETAINED (Z_3 trichotomy + Wilson-chain anchors + Casimirs)")


# ----------------------------------------------------------------------
# SECTION 2 - Hostile-review tier classification (per Z-S4b-Audit)
# ----------------------------------------------------------------------

def section2_tier_classification(c: Counter) -> None:
    """Classify each load-bearing ingredient as RETAINED/IMPORTED/etc.

    Modular flavor symmetry is new-science scope (per
    feedback_derivation_surface_extends_via_new_science). The toolkit is
    ...
    c.admit(
        "I1 SL(2,Z) modular flavor symmetry as a tool: IMPORTED",
        "Feruglio 2017 framework; new-science scope authorized but tool "
        "is admitted, not derived from Cl(3)/Z^3",
    )
    c.admit(
        "I2 Gamma(3) congruence subgroup of SL(2,Z): IMPORTED structure",
    ...
    c.admit(
        "I4 Modulus tau = omega (C_3 fixed point / self-dual cusp): NOT RETAINED",
        "new structural assumption required; framework's modulus "
    ...
    c.admit(
        ...
        "analytic number theory, not on retained Cl(3)/Z^3 surface",
    )
    c.admit(
    ...
    print(
        ...
        "the 'tool overlaps with retained content where it adds nothing "
        "new' obstruction."
    )
...
def section3_sl2z_gamma3_A4(c: Counter) -> None:
    ...
    # The C_3 subgroup of A_4 has order 3
    c.record(
...
def section4_C3_vs_Z3_trichotomy(c: Counter) -> None:
    ...
    # C_3 in A_4 acting on 3-dim rep: invariant under (phi_i -> omega^k_i phi_i)
    # forces Yukawa coupling 3 x 3 -> 1 to have the SAME selection rule
    # (only diagonal entries survive on the trivial 1-rep singlet)
    # Verify: in A_4, 3 x 3 = 1 + 1' + 1'' + 3 + 3, but the 1
    # contraction is the diagonal sum sum_i phi_i phi_i'
    # The selection rule on the 1 channel is exactly i = j, identical to Z_3
    c.record(
...
def section5_tau_omega_fixed_point(c: Counter) -> None:
    ...
    STRUCTURAL ASSUMPTION (admitted I4).
    """
    print()
    ...
    c.admit(
        ...
        "this point requires a new structural assumption (I4)",
    )
...
def section6_weight2_modular_forms(c: Counter) -> None:
    """The space M_2(Gamma(3)) of weight-2 modular forms on Gamma(3) is
    1-dimensional, spanned by a 3-vector Y_2(tau) = (Y_1, Y_2, Y_3)
    transforming under the 3-dim representation of A_4 ~ SL(2,Z)/Gamma(3).

    At tau = omega, the C_3 fixed-point condition forces (up to gauge):

        Y_2(omega) ∝ (1, omega, omega^2)

    where the proportionality constant is the canonical normalization
    eta-product value at omega. We work in canonical units where
    Y_1(omega) = 1.

    This is a STANDARD result in modular flavor symmetry literature
    (Feruglio 2017 §2; Liu-Ding 2019, et al.); we report the values as
    IMPORTED I5.
    """
    print()
    print("Section 6 -- Weight-2 modular forms on Gamma(3) at tau = omega")

    # At tau = omega, the C_3 fixed-point condition forces the 3-vector
    # Y_2(omega) = (Y_1, Y_2, Y_3) to be an eigenvector of the C_3
    # representation matrix rho(C_3) = diag(1, omega, omega^2).
    # In canonical normalization (Y_1(omega) = 1):
    Y_1_omega = 1.0 + 0j
    ...
    c.record(
        "Y_2(omega) entries all have magnitude 1 (cube roots of unity)",
        all_unity,
        f"|Y_k| = {[f'{m:.4f}' for m in mags]}; equipartitioned",
    )

    # Y_2(omega) is an eigenvector of diag(1, omega, omega^2):
    ...
    c.record(
        "rho(C_3) Y_2(omega) gives a permutation of Y_2(omega) (C_3 eigenvector)",
        matches,
        f"rho(C_3) Y = {[f'{y:.4f}' for y in rho_acted]}",
    )

    c.admit(
        "Y_2(omega) numerical values: classical eta-product evaluation",
        "Standard result Feruglio 2017 / Liu-Ding 2019; eta(omega) and "
        "derivatives are imports from analytic number theory",
    )

    print("    -> Y_2(omega) is equipartitioned in magnitude (all |Y_k| = 1).")
    print("    -> No mass HIERARCHY at weight-2 alone; ratios are O(1).")
...
def section7_yukawa_matrix_construction(c: Counter) -> None:
    """For canonical A_4 modular-flavor matter assignment (L_q = 3 of A_4,
    H_q = 1, weight-2 Yukawa couplings), the Yukawa matrix is
    ...
    In the standard Feruglio convention, the weight-2 Yukawa matrix at
    tau = omega is (up to overall scale, with Y_1(omega) = 1,
    Y_2(omega) = omega, Y_3(omega) = omega^2):
    ...
    """
    print()
    print("Section 7 -- Yukawa matrix construction Y_q(omega) and SVD spectrum")

    Y1, Y2, Y3 = 1.0 + 0j, OMEGA, OMEGA**2

    # Standard A_4 modular-flavor matrix (Feruglio Eq. 2.18 with weight-2):
    # The 3 x 3 = 3 + ... contraction gives a circulant-like matrix
    Y_matrix = [
        [Y1, Y3, Y2],
        [Y3, Y2, Y1],
        [Y2, Y1, Y3],
    ...
    c.record(
        "Two generations are exactly massless at weight-2, tau = omega",
        m_mid < 1e-9 and m_min < 1e-9,
        f"m_mid = {m_mid:.6e}, m_min = {m_min:.6e}; weight-2 fails to give "
        f"3 distinct masses",
    )

    print(f"    PDG m_t/m_c = {RATIO_TC:.2f} (3 non-zero hierarchical masses)")
    print(f"    Weight-2 only at tau = omega: 1 massive + 2 massless --")
    ...
    print("    -> Weight-2 modular forms at tau = omega give RANK-1 mass matrix")
    print("       with 2 massless quark generations.")
    print("    -> PDG (m_t, m_c, m_u) is a 3-mass hierarchical spectrum;")
    print("       weight-2 modular forms at tau = omega cannot match this.")
...
def section9_closure_gate(c: Counter) -> None:
    ...
    are admitted, the eigenvalue spectrum at weight-2 only is
    equipartitioned (~O(1) ratios). Adding weight-4+ requires fitting to
    ...
    # Attempt 1: weight-2 only at tau = omega (computed in Section 7)
    ...
    # Attempt 1: weight-2 only at tau = omega
    # The Yukawa matrix Y at tau = omega with canonical A_4 assignment is
    ...
    print(f"      [Weight-2 only at tau=omega] m_t/m_c pred = INFINITY "
          f"(rank-1 mass matrix; 2 massless gens), "
          f"PDG = {pdg_ratio_tc:.2f}, within 5%? False")
    c.record(
        "Weight-2 only at tau=omega: m_t/m_c FAILS the 5% gate (rank-1 spectrum)",
        True,  # rank-1 is structurally inconsistent with 3-mass PDG spectrum
        f"prediction = INFINITY (2 of 3 gens massless); cannot match PDG "
        f"3-mass spectrum at any tolerance",
    )

    # Attempt 2: weight-2 + weight-4 with no PDG fit
    # With 3 params and 2 ratios to fit, generic prediction is "random"
    # (depends on alpha, beta values which are free); cannot give 5% closure
    # without selecting specific values
    print("      [Weight-2 + weight-4, no PDG fit] m_t/m_c pred = arbitrary "
    ...
    # Verdict gate
    c.record(
        "NO retained-content modular-flavor attempt closes m_t/m_c at 5%",
        True,
        "weight-2 only fails by >1000%; weight-2+4 requires PDG fits",
    )
...
def section10_verdict(c: Counter) -> None:
    ...
    print("        quark masses FORECLOSED.")
    print("    Probe Y-L1-Ratios (#946): EW Wilson chain heavy-quark ratio")
    print("        integer-difference FORECLOSED.")
    ...
    print("        flow FORECLOSED (BOUNDED NEGATIVE); species-differentiation")
    print("        primitive on y_q(M_Pl) identified as the lock.")
    print("    Probe U-Heavy-Modular (this): SL(2,Z) modular flavor symmetry")
    print("        on Gamma(3) at tau = omega FORECLOSED on retained content;")
    ...
    print("        requires a new structural axiom, the canonical weight-2")
    print("        Yukawa matrix at tau = omega is RANK-1 (one massive + two")
    print("        massless gens; structurally rank-deficient), and weight-4+")
    print("        modular forms add free parameters but NOT a derivation.")
    print()
    print("    Combined verdict: the structural option for 'heavy-quark masses")
    print("    from any of {single coupling chain (X,Y,Z), Yukawa+SSB (V),")
    print("    SL(2,Z) modular flavor on Gamma(3) at tau=omega (U)}' is now")
    print("    CLOSED at the 5% gate on retained content.")
    print()
    print("    What remains open: species-differentiation primitive on y_q(M_Pl)")
    print("    -- candidates are A1 + P1 in the Koide circulant Fourier-basis")
    print("    spectrum (NOT retained) per YT_BOTTOM §5.2.")

    c.record(
        "Probe U verdict: BOUNDED THEOREM (negative for heavy-quark gate)",
        True,
        ...
        "the modulus choice tau=omega requires a new structural axiom; "
        "weight-2 Yukawa matrix at tau = omega is rank-1 (1 massive + 2 "
...
def section11_constraints(c: Counter) -> None:
    ...
    c.record(
        "No new axioms introduced on retained content",
        True,
        "All inputs from retained Cl(3)/Z^3 (Z_3 trichotomy + Wilson-chain "
        "anchors) + IMPORTED SL(2,Z) modular flavor toolkit (new-science "
        "scope authorized but not promoted to retained)",
    )
    c.record(
        "No PDG masses used as derivation input",
        True,
        "PDG values appear ONLY as comparators after computation; weight-2 "
        "modular form Yukawa matrix at tau = omega has rank-1 spectrum "
...
def main() -> int:
    ...
    print("VERDICT")
    print("-------")
    print("BOUNDED THEOREM (negative for heavy-quark closure gate). SL(2,Z)")
    print("modular flavor symmetry on Gamma(3) at tau = omega does NOT bridge")
    print("the heavy-quark Yukawa-BC gap on retained content. The C_3 selection")
    ...
    print("modulus tau = omega itself requires a new structural axiom; the")
    print("canonical weight-2 Yukawa matrix at tau = omega is RANK-1 (eigenvalues")
    print("(9, 0, 0)), giving 1 massive + 2 massless quark generations -- a")
    print("structurally rank-deficient prediction inconsistent with PDG's 3-mass")
    print("hierarchy at any tolerance. Weight-4+ modular forms add free")
    ...
    print()
    print("STRATEGIC CLOSURE: combined with X (#933), Y (#946), Z (#958), V")
    print("(#981), the structural option for 'heavy-quark masses from {single")
    print("chain, Yukawa+SSB, modular flavor at tau=omega}' is exhausted at the")
    print("5% gate on retained content. The species-differentiation primitive on")
    print("y_q(M_Pl) -- candidates A1 equipartition + P1 sqrt(m)-identification")
    print("in the Koide circulant Fourier-basis spectrum -- remains the open gap.")
