#!/usr/bin/env python3
"""
PR #230 non-chunk cycle-33 post-cycle-32 main-audit-status-drift reopen guard.

Cycle 32 closed origin/main drift through 1c86abee.  This guard checks the
only new resume fact available at cycle 33: after a fresh fetch, origin/main
advanced again to 93f51543, but the new diff still contains only audit,
effective-status, and runner-cache surfaces and no listed PR230 same-surface
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_nonchunk_cycle33_post_cycle32_main_audit_status_drift_guard_2026-05-05.json"
)
LOOP_PACK = (
    ROOT
    / ".claude"
...
CYCLE32_HEAD = "5492b9bb9e1ac0cc93a17be550f834ba50366ceb"
CYCLE32_REMOTE_MAIN_HEAD = "1c86abee1266693aed4892a970f6134a77cc321c"
REMOTE_PR = "origin/claude/yt-direct-lattice-correlator-2026-04-30"
REMOTE_MAIN = "origin/main"
ALLOWED_MAIN_DRIFT_PREFIXES = (
    "docs/audit/",
...
def allowed_main_drift(path: str) -> bool:
    return path.startswith(ALLOWED_MAIN_DRIFT_PREFIXES)
...
def branch_state(future_paths: list[str]) -> dict[str, Any]:
    ...
    main_changed_paths = diff_paths(CYCLE32_REMOTE_MAIN_HEAD, REMOTE_MAIN)
    main_changed_path_classes = Counter(path_class(rel) for rel in main_changed_paths)
    return {
        "cycle32_head": CYCLE32_HEAD,
        "cycle32_remote_main_head": CYCLE32_REMOTE_MAIN_HEAD,
        "local_head": local_head,
        "remote_pr_ref": REMOTE_PR,
        "remote_pr_head": remote_pr_head,
        "remote_main_ref": REMOTE_MAIN,
        "remote_main_head": remote_main_head,
        "cycle32_is_ancestor_of_local_head": git_ok(
            ["merge-base", "--is-ancestor", CYCLE32_HEAD, "HEAD"]
        ),
        "cycle32_is_ancestor_of_remote_pr": git_ok(
            ["merge-base", "--is-ancestor", CYCLE32_HEAD, REMOTE_PR]
        ),
        "cycle32_remote_main_is_ancestor_of_remote_main": git_ok(
            ["merge-base", "--is-ancestor", CYCLE32_REMOTE_MAIN_HEAD, REMOTE_MAIN]
        ),
        "local_head_matches_remote_pr_head": bool(local_head and local_head == remote_pr_head),
        "local_commits_after_cycle32": git(["rev-list", "--count", f"{CYCLE32_HEAD}..HEAD"]),
        "remote_commits_after_cycle32": git(["rev-list", "--count", f"{CYCLE32_HEAD}..{REMOTE_PR}"]),
        "remote_main_commits_after_cycle32_baseline": git(
            ["rev-list", "--count", f"{CYCLE32_REMOTE_MAIN_HEAD}..{REMOTE_MAIN}"]
        ),
        "remote_main_changed_path_count_after_cycle32_baseline": len(main_changed_paths),
        "remote_main_changed_path_class_counts_after_cycle32_baseline": dict(
            sorted(main_changed_path_classes.items())
        ),
        "remote_main_changed_paths_are_allowed_drift": bool(main_changed_paths)
        and all(allowed_main_drift(rel) for rel in main_changed_paths),
        ...
        "local_changed_future_paths_after_cycle32": diff_future_paths(
            CYCLE32_HEAD, "HEAD", future_paths
        ),
        "remote_pr_changed_future_paths_after_cycle32": diff_future_paths(
            CYCLE32_HEAD, REMOTE_PR, future_paths
        ),
        "remote_main_changed_future_paths_after_cycle32_baseline": diff_future_paths(
            CYCLE32_REMOTE_MAIN_HEAD, REMOTE_MAIN, future_paths
        ),
    }
...
def main() -> int:
    print("PR #230 non-chunk cycle-33 post-cycle-32 main-audit-status-drift reopen guard")
    print("=" * 72)
    ...
    no_changed_future_path_since_cycle32 = not any(
        [
            branch["local_changed_future_paths_after_cycle32"],
            branch["remote_pr_changed_future_paths_after_cycle32"],
            branch["remote_main_changed_future_paths_after_cycle32_baseline"],
        ]
    )
    ...
    branch_current = (
        branch["cycle32_is_ancestor_of_local_head"]
        and branch["cycle32_is_ancestor_of_remote_pr"]
        and branch["cycle32_remote_main_is_ancestor_of_remote_main"]
        and branch["local_head_matches_remote_pr_head"]
        and bool(branch["remote_main_head"])
    )
    loop_pack_records_stop = (
        "cycle-32 update" in compact_text(OPPORTUNITY_QUEUE)
        and "post-cycle-31 main-audit-status-drift reopen guard" in compact_text(HANDOFF)
        and "post-cycle-31 main audit/status drift is not admissible reopen evidence"
        in compact_text(NO_GO_LEDGER)
        and "proposal_allowed: false" in compact_text(CLAIM_STATUS_CERTIFICATE)
    )
    main_drift_since_cycle32_is_allowed_only = (
        branch["remote_main_commits_after_cycle32_baseline"] != "0"
        and branch["remote_main_changed_paths_are_allowed_drift"]
        and not branch["remote_main_changed_future_paths_after_cycle32_baseline"]
    )
    no_reopen_input = (
        source_presence_absent
        and no_local_or_remote_future_path
        and no_changed_future_path_since_cycle32
        and main_drift_since_cycle32_is_allowed_only
    )
    cycle33_post_cycle32_main_audit_status_drift_guard_passed = all(
        [
            not missing_parents,
            not parent_failures,
    ...
    dramatic_step_gate = {
        ...
        "branch_state": branch,
        "reason": (
            "Cycle 32 already rejects post-cycle-31 origin/main audit/status "
            "drift as reopen evidence. Since then origin/main advanced again "
            "only on audit, effective-status, and runner-cache surfaces and "
            "still contains none of the listed PR230 same-surface artifacts, "
            "so cycle 33 has no admissible non-chunk route to execute."
        ),
    }
    ...
    report("branch-has-cycle32-ancestor-and-remote-main-visible", branch_current, json.dumps(branch, sort_keys=True))
    ...
    report("parent-process-stack-closed-through-cycle32", parent_process_stack_closed, "cycle-18 through cycle-32 gates deny continuation")
    report("aggregate-gates-deny-proposal", aggregate_gates_deny, f"assembly={statuses['assembly']} campaign={statuses['campaign']}")
    report("loop-pack-records-cycle32-stop-contract", loop_pack_records_stop, "cycle-32 stop text present")
    report("main-drift-since-cycle32-is-audit-status-log-only", main_drift_since_cycle32_is_allowed_only, json.dumps(branch, sort_keys=True))
    report("no-local-or-remote-listed-future-path", no_local_or_remote_future_path, json.dumps(branch, sort_keys=True))
    report("no-changed-listed-future-path-since-cycle32", no_changed_future_path_since_cycle32, json.dumps(branch, sort_keys=True))
    report("no-source-presence-key-set", source_presence_absent, json.dumps(present_by_source, sort_keys=True))
    report(
        "cycle33-post-cycle32-main-audit-status-drift-guard-passed",
        cycle33_post_cycle32_main_audit_status_drift_guard_passed,
        f"passed={cycle33_post_cycle32_main_audit_status_drift_guard_passed}",
    )

    result = {
        "actual_current_surface_status": (
            "exact negative boundary / PR230 non-chunk cycle-33 "
            "post-cycle-32 main-audit-status-drift reopen guard: origin/main "
            "advanced again only on audit/effective-status/runner-cache surfaces "
            "and no listed same-surface artifact is present; positive closure "
            "remains open"
        ),
        ...
        "proposal_allowed_reason": (
            "Cycle 32 already rejects post-cycle-31 origin/main audit/status "
            "drift. The cycle-32 head remains an ancestor of the aligned PR "
            "heads, origin/main advanced again only on audit/effective-status/"
            "runner-cache paths, no listed same-surface artifact appears on "
            "branch or remote surfaces, all six worklist units remain blocked, "
            "and aggregate gates still deny proposal authority."
        ),
        ...
        "strict_non_claims": [
            ...
            "does not treat origin/main audit/effective-status/runner-cache drift as same-surface physics evidence",
        ...
        "exact_next_action": (
            ...
            "cycle-30, cycle-31, cycle-32, cycle-33, assembly, retained-route, and "
