"""3 ⊗ 3 ⊗ 3 baryon color singlet check on cited SU(3)_c."""
...
def main() -> None:
    ...
    print()
    N_c = 3
    print(f"  N_c = {N_c} (from cited Cl(3) color automorphism note)")
    ...
    for perm in itertools.permutations(range(3)):
        ...
        for i in range(3):
            for j in range(i+1, 3):
                if pi[i] > pi[j]:
                    sign = -sign
        # Pick a nonzero component; most basis components vanish, so avoid
        # whole-vector division by zero.
        nonzero_idx = np.argmax(np.abs(singlet))
        observed_sign = float(np.real(permuted_singlet[nonzero_idx] / singlet[nonzero_idx]))
