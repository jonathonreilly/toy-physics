...
Verifies the new identities in
  ...
  (C8) Inscribed-angle / law of sines: 2 R_bar sin(alpha_bar) = 1    [unit base]
...
def audit_c3_circumcenter() -> None:
    ...
    # x_cc = 1/2 by perpendicular bisector of the fixed unit base V_1V_2
    ...
    print(f"    (alpha_s-INDEPENDENT: midpoint of fixed unit base V_1V_2)")
    print()
...
def audit_c8_law_of_sines() -> None:
    banner("(C8) NEW: 2 R_bar sin(alpha_bar) = 1 (unit base from law of sines)")
    ...
    print(f"  Unit-base length (rescaled triangle)          = 1.0")

    check("(C8) 2 R_bar sin(alpha_bar) = 1 (unit base = chord = 1, law of sines)",
...
def audit_summary() -> None:
    ...
    print("            R_bar^2 is degree-2 on this NLO surface - no higher terms there")
    print()
    print("  NEW (C7): R_bar cos(alpha_bar) = y_cc (chord-distance theorem)")
    print()
    print("  NEW (C8): 2 R_bar sin(alpha_bar) = 1 (unit base via law of sines)")
    print()
    ...
    print("  Scope: exact on the retained NLO protected-gamma surface only;")
    print("         no physical NNLO or all-orders CKM control is claimed.")
