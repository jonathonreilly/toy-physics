#!/usr/bin/env python3
"""Gate B grown trapping frontier v2.

This is a narrow follow-up to the retained grown trapping/frontier probe.

Question:
  Can the same retained Gate B grown row carry one stronger structural
  frontier observable than plain escape/frontier_bias, while still reducing
  exactly back to the retained grown baseline at eta = 0?
...
def main() -> None:
    t0 = time.time()
    print("=" * 104)
    print("GATE B GROWN TRAPPING FRONTIER V2")
    print("  retained grown row only: drift=0.2, restore=0.7")
    print("  promoted observable: frontier_radius_shift(eta) = <|z|>_det(eta) - <|z|>_det(0)")
    ...
    print(
        "eta=0 exact reduction: escape=1.000 and frontier_radius_shift=0.000 "
        "by construction on the retained grown baseline"
    )
    print()
    ...
    print("  The promoted observable is the detector frontier radius shift, not just escape.")
    print("  If the radius shift rises with eta, the trap is pushing surviving mass")
    print("  outward on the detector shell; if not, this frontier probe is too weak.")
    print()
