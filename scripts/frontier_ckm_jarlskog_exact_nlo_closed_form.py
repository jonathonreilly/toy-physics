...
Verifies the new identities in
  ...
  (J6) Selection rule: only alpha_s^3 and alpha_s^4 contribute on the NLO surface
...
def audit_j6_selection_rule() -> None:
    ...
    print("\n  Verifying upper cutoff inside the NLO surface (no alpha_s^5, alpha_s^6, ...):")
    ...
    check("(J6) J_bar is degree-4 polynomial on the NLO surface: no alpha_s^5+ terms there",
          abs(residual) < 1e-18)
...
def audit_alpha_s_independence() -> None:
    ...
    check("EXACT closed forms hold at ALL tested alpha_s on the NLO surface",
          all_pass)
...
def audit_summary() -> None:
    ...
    print("  NEW (J6): Only alpha_s^3 and alpha_s^4 contribute on the NLO surface")
    ...
    print("         no physical NNLO or all-orders CKM Jarlskog control is claimed.")
