...
def main() -> int:
    ...
    # ---------------------------------------------------------------------
    # Sample 1: exact abstract on-cone vector.  Choose a_0 = sqrt(2)
    # and z = 1 in the character basis, so a_0^2 = 2 |z|^2 by
    # construction.  This gives a positive real-space vector.
    v_sample_1 = (
        (sqrt(2) + 2) / sqrt(3),
        (sqrt(2) - 1) / sqrt(3),
        (sqrt(2) - 1) / sqrt(3),
    ...
    check(
        "exact sample 1: abstract on-cone vector gives Q = 2/3",
        simplify(Q_sample_1 - Rational(2, 3)) == 0,
        detail=f"Q (sample 1) = {Q_sample_1}",
    )

    # Sample 2: generic positive rational vector. Verify Q is the
    ...
    print("    FP sanity at three abstract samples (on-cone, generic, off-cone)")
