#!/usr/bin/env python3
"""NN lattice rescaled-lane response-vector Cauchy test.
...
same bridge gap from a different angle: numerical Cauchy convergence
of the chosen 15-component response vector, independent of any
strength-scaling question.
...
Together these 15 numbers per h form a fixed source/observable response
vector (three point sources by five canonical observables). They live in
R^15 independent of h.
...
give a summable tail estimate for the chosen 15-dimensional response
vector under the fitted geometric model. That is a bounded numerical
Cauchy certificate, not a full operator-norm theorem.

If the full response-vector fit has r <= 0 or poor R^2, the Cauchy test
fails and no bounded response-vector convergence claim is made.
Per-component fits are diagnostics; the Born component is expected to sit
at the float64 noise floor.
...
Exit code is nonzero if any guard fails or if the full response-vector
Cauchy fit fails.
"""
...
def main() -> int:
    print("=" * 110)
    print("NN LATTICE RESCALED-LANE RESPONSE-VECTOR CAUCHY TEST")
    ...
    print(f"  Strength fixed at {PHYS_STRENGTH:.2e} (linear-in-strength is "
          f"NOT assumed; this tests response-vector convergence at fixed parameters)")
    print("=" * 110)
    ...
    if fine_pts:
        ...
        if cauchy_ok:
            ...
            print(f"    ||vec_inf - vec(h={h_finest})|| <~ "
                  f"{tail_estimate:.4e}")
            print()
            print(f"  BOUNDED NUMERICAL CAUCHY CERTIFICATE for the chosen")
            print(f"  15-component response vector.")
            print(f"  Under the fitted geometric model, the response vector")
            print(f"  has decay rate >= {r:.3f} and a summable tail estimate.")
        else:
            print(f"  CAUCHY TEST FAILS — r = {r:.4f} (need > 0.5) or "
                  f"R^2 = {r2:.4f} (need >= 0.95)")
            print(f"  No registered response-vector Cauchy claim on this basis.")
    else:
