...
that on the retained NLO Wolfenstein protected-gamma-bar surface

  tan(gamma_bar) = sqrt(5)                       (retained N4 / N8, alpha_s-protected)
  tan(beta_bar)  = sqrt(5)(4 - alpha_s)/(20 + alpha_s)  (retained N5)
  alpha_bar      = pi - gamma_0 - beta_bar       (retained N6, with gamma_bar = gamma_0 by N4)
...
sympy-based exact-symbolic verification:
  ...
  (b) imports retained `tan(beta_bar)`, `tan(gamma_bar)`, and the angle
      sum (N6) verbatim;
...
not a new source note, no status promotion. The cited inputs (retained
N4, N5, N6, structural counts) are imported from upstream authority
...
def main() -> int:
    ...
    print("on the retained NLO protected-gamma-bar surface.")
    print("=" * 88)

    # ---------------------------------------------------------------------
    section("Part 0: symbolic setup and retained inputs")
    # ---------------------------------------------------------------------
    alpha_s = Symbol("alpha_s", positive=True, real=True)
    N_pair = Rational(2)
    N_quark = Rational(6)

    # Retained N5 + N4 tangents.
    ...
    check(
        "(A10) linear coefficient (alpha_s^1) is sqrt(5)/20 (matches retained N7)",
