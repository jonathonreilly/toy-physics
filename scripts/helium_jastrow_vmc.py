#!/usr/bin/env python3
"""Helium: exchange-correlation beyond Hartree via Jastrow VMC.
...
Same kinetic operator and Coulomb kernel as helium_hartree_scf.py.
...
This is NOT imported from quantum mechanics. It follows directly from:
...
The Jastrow correction captures a fraction of the correlation energy
(full CI - Hartree gap). The fraction depends on r_J, optimized here.
...
# ---------------------------------------------------------------------------
# Lattice operators (identical to helium_hartree_scf.py)
# ---------------------------------------------------------------------------
...
def run_experiment() -> None:
    log("=" * 72)
    log("HELIUM JASTROW VMC — EXCHANGE-CORRELATION FROM Cl(3)/Z³ AXIOMS")
    log("Cusp condition derived from Z³ Green's function kernel, not imported")
    log("=" * 72)
    log()

    # Coupling setup: same as helium_hartree_scf.py
    G_EM  = 0.5
    G_NUC = 2.0 * G_EM
    ...
    # ------------------------------------------------------------------
    log("=" * 72)
    log("SUMMARY: JASTROW VMC FROM Cl(3)/Z³")
    log("=" * 72)
    log()
    ...
    log()
    log("  BLOCKED:")
    log("  ✗ Absolute energy in eV: requires α_EM (bounded) × m_e (open)")
    log("  ✗ Exact two-body result: N⁶ problem; Jastrow/VMC avoids this")
    log("=" * 72)

    os.makedirs("logs", exist_ok=True)
    log_path = f"logs/{time.strftime('%Y-%m-%d')}-helium_jastrow_vmc.txt"
    try:
