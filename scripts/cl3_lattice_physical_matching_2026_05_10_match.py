...
note LATTICE_PHYSICAL_MATCHING_THEOREM_BOUNDED_OBSTRUCTION_NOTE_2026-05-10_match.md):
  ...
  T1: Steps S1, S2, S5 are retained_bounded.
  T2: Step S3 retained_bounded conditional on staggered-Dirac realization gate.
...
# ----------------------------------------------------------------------
# Part 2: framework retained values used in the per-step decomposition
# These are NOT new derivations -- they are quoted from cited authorities.
# ----------------------------------------------------------------------
section("Part 2: retained framework values (quoted, not derived here)")

# Retained from COMPLETE_PREDICTION_CHAIN_2026_04_15.md
P_iso = 0.5934  # SU(3) plaquette MC at beta=6 (single retained MC datum)
v_GeV = 246.28  # hierarchy theorem v
N_taste = 16  # uniform-channel input (open: depends on staggered-Dirac realization gate)

# Retained from EXACT_TIER_EWITNESS_BOUNDED_NOTE
P_sigma_HamLim = 0.4410
P_sigma_HamLim_stat_vol = 0.0006
P_sigma_HamLim_C_iso_xi4 = 0.013

# Retained from C_ISO_SU3_NLO_CLOSURE_BOUNDED_NOTE
...
check(
    "P_iso retained value sourced (Engels 1990: 0.594)",
    abs(P_iso - 0.5934) < 1e-6,
    f"P_iso={P_iso}",
)
check("v retained from hierarchy theorem", abs(v_GeV - 246.28) < 1e-6, f"v={v_GeV} GeV")
check(
...
check(
    "Hamilton-limit P_sigma retained value",
...
S1_P = P_iso
check(
    "S1 retained: <P>_iso(beta=6) = 0.5934 (single MC datum)",
...
S5_v = v_GeV
check(
    "S5 retained: v = M_Pl * (7/8)^(1/4) * alpha_LM^16 = 246.28 GeV",
...
# ----------------------------------------------------------------------
section(
    "Part 5: theorem T6 / wrong-direction proof using Hamilton-limit retained <P>"
)

m_H_HamLim = m_H_tree_from_P(P_sigma_HamLim, v_GeV)
check(
    "Hamilton-limit retained <P>_sigma = 0.4410 yields m_H_tree = 151.0 GeV",
...
step_classification = {
    "S1": "retained_bounded",  # named C-iso engineering, open framework gates
    "S2": "retained",  # algebraic
    "S3": "retained_bounded",  # conditional on staggered-Dirac realization
    "S4": "load_bearing_residual",  # tree-level mean-field readout (matching-core)
    "S5": "retained_bounded",  # hierarchy theorem
    "S6": "retained_bounded",  # composition of S1-S5
