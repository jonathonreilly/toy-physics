...
running coupling on retained Cl(3)/Z^3 substrate, can give beta_2 (and
...
If retained Cl(3)/Z^3 content gives the Stokes constant S_IR in closed
form, beta_2 and beta_3 would gain a structural identity.

Verdict structure
=================
The probe is bounded_theorem. New science (resurgence) is admitted as a
mathematical TOOL acting on retained content per the
derivation-surface-extends authorization (memo 2026-05-10). The probe
finds:

Positive structural retentions (PASS expected):
  1. Borel-plane IR renormalon at z = 4*pi / beta_0 = 4*pi / 7 (retained via beta_0)
  2. UV renormalons at z = -4*pi / (beta_0 * n) for n = 1, 2, 3, ... (retained)
  3. Asymptotic factorial growth rate (beta_0/(4*pi))^(n+1) (retained)
  4. Renormalon picture skeleton retained from beta_0

Bounded admissions (PASS=ADMITTED expected, no derivation):
  5. Stokes constant S_IR closed form: NOT retained (requires QCD
     instanton sector ↔ Cl(3)/Z^3 identification, not pre-justified)
...
- NO new axioms (resurgence is a mathematical tool, not a physics axiom)
...
def section1_setup(c: Counter) -> None:
    """Verify retained beta_0, beta_1 from probe X-L1-MSbar."""
    print("Section 1 — Retained content from probe X-L1-MSbar")

    c.record(
        "Retained beta_0 = 7 from S1 + Casimir (X-L1)",
        BETA_0 == Fraction(7),
        f"= {BETA_0}",
    )
    c.record(
        "Retained beta_1 = 26 from Casimir algebra (X-L1)",
        BETA_1 == Fraction(26),
        f"= {BETA_1}",
    )
    print(
        "    -> beta_0, beta_1 retained as starting point for resurgence "
        "analysis."
    )
...
def section2_borel_ir_renormalon(c: Counter) -> None:
    ...
    This position is RETAINED via beta_0 alone — no additional integral
    primitives needed.
    """
    print()
    print("Section 2 — POSITIVE STRUCTURAL: Borel-plane IR renormalon position")
    ...
    c.record(
        "z_* ≈ 1.7952 in dimensionless Borel-plane units (with retained beta_0=7)",
        abs(z_star - 1.7952) < 1e-3,
        f"z_* = {z_star:.4f}",
    )

    print(
        "    -> Borel-plane IR singularity location is RETAINED via beta_0=7."
    )
    print(
        "    -> This is a structural identity: depends only on retained content."
    )
...
def section3_uv_renormalons(c: Counter) -> None:
    ...
    These are also retained via beta_0 alone.
    """
    print()
    print("Section 3 — POSITIVE STRUCTURAL: UV renormalon positions")
    ...
    print("    -> UV renormalon ladder is RETAINED via beta_0=7.")
    print(
...
def section4_asymptotic_growth_rate(c: Counter) -> None:
    ...
    This is RETAINED via beta_0.
    """
    print()
    print("Section 4 — POSITIVE STRUCTURAL: asymptotic factorial growth rate")
    ...
    c.record(
        "Geometric base ≈ 0.557 (with retained beta_0 = 7)",
        abs(growth_rate - 0.5570) < 1e-3,
        f"= {growth_rate:.4f}",
    )

    print(
        "    -> Growth-rate base 1/z_* = beta_0/(4*pi) is RETAINED via beta_0."
    )
    print(
...
def section6_stokes_constant_bounded(c: Counter) -> None:
    ...
    The Cl(3)/Z^3 framework retains:
      - SU(3) Casimir algebra,
      - Z^3 discrete cycle structure (3-fold center symmetry analogue),
    ...
    Thus S_IR is NOT closed-form derivable from retained content.

    A candidate follow-on probe would postulate an identification
    QCD instanton sector ↔ Cl(3)/Z^3 monopole/Z^3 sector and try to
    derive S_IR from this identification. This would be a NEW STRUCTURAL
    CONJECTURE per the derivation-surface-extends authorization, and is
    NOT load-bearing for this probe.
    """
    print()
    print(
    ...
    c.admit(
        ...
        "requires QCD instanton moduli computation; not in retained "
        "Cl(3)/Z^3 stack; identification of YM instantons with framework "
    ...
    print(
        "    -> Candidate follow-on: postulate QCD-instanton ↔ Cl(3)/Z^3 "
        "identification."
    )
...
def section7_subleading_exponent_bounded(c: Counter) -> None:
    ...
    beta_0 = 7 and beta_1 = 26 are taken on retained content. But this is
    ...
    c.admit(
        ...
        "not in retained content; same obstruction as for beta_2 channel "
        "weights",
    )
...
def section9_numerical_comparator(c: Counter) -> None:
    ...
    c.admit(
        "Closed-form derivation of S_IR_eff(n=2), S_IR_eff(n=3) from retained",
...
def section10_hostile_review(c: Counter) -> None:
    ...
    actually buys retained content for beta_2/beta_3, or whether it just
    ...
    Hostile question 2: Does the renormalon picture give NEW retention
    beyond what probe X-L1-MSbar already had?
      ...
      asymptotic growth rate). These ARE new retentions (different
      content from X-L1's color skeleton). But neither closes beta_2.

    Hostile question 3: Is the QCD-instanton ↔ Cl(3)/Z^3 identification
    rigorous?
    ...
    beta_2/beta_3?
      Answer: it documents that resurgence + retained content alone is
      not sufficient. New mathematical TOOLS beyond leading resurgence
      (e.g., trans-asymptotic resummation, exact Borel transform on
      Cl(3)/Z^3 lattice, full instanton ↔ Z^3 identification) remain
      candidate follow-ons.

    Verdict: bounded; positive structural retentions documented; mostly
    negative on closed-form derivation; honest scope.
    """
    print()
    ...
    c.record(
        "Borel-plane IR renormalon position 4*pi/beta_0 is structural retention",
        True,
        "depends on retained beta_0 alone; new content beyond X-L1",
    )

    print("    Hostile Q2: Does this add retention beyond X-L1?")
    c.record(
        "Probe U adds Borel-plane geometry retention (orthogonal to X-L1's color skeleton)",
        True,
        "different structural content; X-L1 retains color tensors, U retains Borel geometry",
    )

    print("    Hostile Q3: Is QCD-instanton ↔ Cl(3)/Z^3 identification rigorous?")
    c.admit(
        "Identification of QCD instanton sector with Cl(3)/Z^3 monopole sector",
    ...
    c.admit(
        ...
        "S_IR comes from outside retained content",
    )
    ...
    print(
        "      Cl(3)/Z^3 substrate, full instanton ↔ Z^3 identification all"
    )
    print("      remain candidate follow-on probes (not load-bearing here).")

    print(
        "    -> Hostile-review verdict: probe is honestly bounded with "
        "specific positive retentions and clearly named obstructions."
    )
...
def section11_verdict(c: Counter) -> None:
    ...
    print("=" * 72)
    print()
    print("Claim type: bounded_theorem")
    print("            (positive on structural retentions of Borel-plane")
    print("             geometry and asymptotic growth from retained beta_0;")
    print("             negative on closed-form derivation of beta_2, beta_3")
    print("             via resurgence)")
    print()
    print("New science (admitted): resurgence theory as mathematical TOOL")
    print("                       acting on retained content per memo 2026-05-10")
    print("                       (derivation_surface_extends_via_new_science).")
    print()
    print("POSITIVE structural retentions:")
    print("  + IR renormalon at z_* = 4*pi/beta_0 = 4*pi/7 (retained via beta_0)")
    print("  + UV renormalon ladder at z = -4*pi/(7*n), n = 1, 2, 3, ...")
    print("  + Asymptotic growth rate (beta_0/(4*pi))^(n+1) = (7/(4*pi))^(n+1)")
    print("  + Renormalon picture skeleton from retained beta_0")
    print("  + Leading retained piece of subleading exponent: 1 - beta_1/beta_0^2 ≈ 23/49")
    print()
    print("BOUNDED admissions:")
    print("  - Stokes constant S_IR closed form: NOT retained (instanton ↔ Cl(3)/Z^3)")
    ...
    print("  - Confirms beta_0 = 7 retained (already on main via X-L1)")
    print("  - ADDS structural retention of Borel-plane geometry (new content)")
    print("  - ADDS asymptotic factorial growth form (new content)")
    print("  - Does NOT close beta_2 or beta_3 (bounded admission stands)")
    print("  - Identifies candidate follow-on: postulate QCD-instanton ↔ Cl(3)/Z^3")
    print()
