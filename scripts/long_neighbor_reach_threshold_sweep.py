#!/usr/bin/env python3
"""Run a graph-native neighbor-reach threshold sweep for the hub motif."""
...
from toy_event_physics import (
    ...
    neighbor_reach_threshold_sets,
)


def main() -> None:
    started = datetime.now().isoformat(timespec="seconds")
    print(f"neighbor reach threshold sweep started {started}", flush=True)
    total_start = time.time()
    for reach_set in neighbor_reach_threshold_sets():
        set_start = time.time()
        row = neighbor_reach_threshold_benchmark(
