#!/usr/bin/env python3
"""T1.4: extend the cited staggered C8 cube to the full 96-dimensional Connes-
...
It then:
  ...
  * tests the order-one condition [[D_F, a], J b J^-1] = 0 on the lifted
    algebra and reports the same boundary as the C8 construction: order-one survives
    only for vacuous scalar D_F, the minimal staggered candidate violates it.

The deliverable boundary is identical in kind to the C8 construction: the C8 cube and the
...
def main() -> int:
    ...
    print("Claim boundary: bounded theorem/construction; not a closure.")
    ...
    # ----- staggered Cl(3) on the C^8 taste factor -----
    ...
    # Sanity: same qualitative answer as the C^8 result; the tensoring on
    # C^3 x C^4 just multiplies the violation footprint by the lift identity.
    report(
        "order-one outcome on H_F_tot matches the C^8 boundary",
    ...
    print("Outcome class: BOUNDED_THEOREM (open D_F selection unchanged)")
