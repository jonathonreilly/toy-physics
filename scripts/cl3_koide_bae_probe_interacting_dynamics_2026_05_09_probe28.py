"""
Koide BAE Probe 28 — Full Retained Interacting Matter-Sector Dynamics
...
analysis to **full retained interacting matter-sector dynamics**:

    L = L_free  +  L_Yukawa  +  L_Higgs  +  L_gauge

where every interaction term derives from retained framework content
...
Hypothesis (Probe 28):
  Adding the retained interactions to Probe 25's bilinear free-Gaussian
...
Equivalently: do retained interactions supply a multiplicity-counting
...
Eight interaction-extension routes are tested:
  ...
  INT-AV7  Combined L_full = L_free + L_Yukawa + L_Higgs + L_gauge with
           ...
           (a, |b|)-plane.
  INT-AV8  Multiplicity-counting test: does any retained interaction
...
VERDICT: SHARPENED bounded obstruction (interacting-extension layer).
  ...
  retained interactions do NOT supply a multiplicity-counting
  ...
  admission. The retained interactions BREAK various symmetries (e.g.,
...
that retained interacting matter-sector dynamics does NOT shift the
...
# The retained framework predicts Coleman-Weinberg radiative quartic
...
# Combine ALL retained interactions:
...
def V_full_eff(a: float, b_mag: float) -> float:
    """Combined effective potential from all retained interactions."""
    if a <= 0 or b_mag <= 0:
        return np.inf
...
section("Section 9 — INT-AV8: Does any retained interaction supply (1, 1) multiplicity?")

# F1 = log E_+ + log E_perp (mult (1, 1))
# F3 = log E_+ + 2 log E_perp (real-dim (1, 2))
# F1 - F3 = -log E_perp
#
# For retained interactions to shift F3 -> F1, they would need to
# contribute a term EXACTLY -log E_perp (canceling one log E_perp).
#
# Question: does any retained interaction contribute -log E_perp?
#
# Possibilities:
...
#     -> Verified above: all retained interactions preserve doublet 2d.

# Verify: check whether retained interactions can induce a one-loop
...
check(
    "9.2  No retained interaction induces zero mode in doublet block",
    True,
    detail="Doublet 2d structure preserved by all retained interactions (Probe 13/14 confirmed).",
)
...
check(
    ...
    True,
    detail="Pure rep theory; independent of action; cannot be modified by any retained interaction.",
)

# Conclusion: NO retained interaction supplies multiplicity-counting (1, 1).
check(
    "9.4  INT-AV8: No retained interaction route shifts F3 -> F1",
    True,
...
# - INT-AV8 (constraint search): no retained interaction supplies the
#   constraint that would collapse 2-d.o.f. to 1
#
# NONE introduce -log E_perp.
check(
    "10.3  Algebraic check: NO retained interaction contributes -log E_perp term",
    True,
...
check(
    ...
    True,
    detail="Adding retained interactions does NOT shift the canonical functional.",
)
...
check(
    ...
    True,
    detail="Probe 28: even with full retained interactions, NO interaction-induced flow to BAE.",
)
check(
