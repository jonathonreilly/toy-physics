...
verification:
  ...
  (d) verifies the Hermitian-lift commutator [Theta_H, H] = 0 with
      Theta_H = P K;
  (e) verifies (C3) Theta^2 = I;
  (f) verifies the counterfactual: a generic anti-Hermitian operator
      with non-zero real-symmetric component does NOT satisfy (3),
      breaking the composite invariance — confirming D real is
      load-bearing.
...
def main() -> int:
    ...
    print("    (C3) Theta^2 = I on both instances")
    print("    Counterfactual: non-real D breaks premise (3) => breaks (C1)")
