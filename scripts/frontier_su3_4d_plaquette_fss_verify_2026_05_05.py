...
`PLAQUETTE_4D_MC_FSS_BOUNDED_THEOREM_NOTE_2026-05-05.md`.
...
- asserts a minimum scientific bar:
    ...
    H. the comparator distance |P_inf(M1) - 0.5934| is reported (NOT
       asserted to be small; reported for transparency).

A FAIL on (A)-(G) means the bridge has not been demonstrated and the bounded
theorem cannot be claimed at the current data set. That is the honest
outcome and is what the source note treats as the open frontier.
"""
...
def main() -> int:
    ...
    if len(per_L) >= 2:
        ...
        for i in range(len(sorted_by_L) - 1):
            small = sorted_by_L[i]
            big = sorted_by_L[i + 1]
            # As L grows, 1/V decreases, P should decrease toward P_inf if c1 > 0
            # We just require consistency within combined SE:
            # |P(L_i) - P(L_{i+1})| should not exceed something huge given SEs
            diff = small["mean"] - big["mean"]
            combined_se = math.hypot(small["se"], big["se"])
            if not (-3 * combined_se <= diff or diff >= -3 * combined_se):
                violations.append(f"L={small['L']}->{big['L']}: diff={diff:.4f}, 3SE={3 * combined_se:.4f}")
        check(
            "E. P(L) is FSS-consistent (per-pair difference is within 3 combined SE for any sign)",
            not violations,
