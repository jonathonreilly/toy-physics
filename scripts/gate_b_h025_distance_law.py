#!/usr/bin/env python3
"""Gate B h=0.25 grown-geometry distance-law transfer harness.

Bounded companion to gate_b_grown_distance_law.py.

Question:
  On the retained h=0.25 generated-geometry family, does the far-field
  distance-law tail stay positive and declining on the moderate-drift row,
  compared with the exact-grid row?

This harness keeps the comparison narrow:
- exact grid
- grown geometry with the retained moderate-drift row

It freezes only the distance-tail readout and the far-field TOWARD fraction.
"""
...
def main():
    t0 = time.time()
    print("=" * 74)
    print("GATE B H=0.25 GROWN-GEOMETRY DISTANCE-LAW HARNESS")
    print("  exact grid vs grown geometry on the retained h=0.25 family")
    print("=" * 74)
    ...
    print("  This harness freezes the h=0.25 distance-law comparison only.")
    print("  It should be read as a bounded refinement check on the same")
    print("  moderate-drift grown family, not as a full Gate B closure.")
