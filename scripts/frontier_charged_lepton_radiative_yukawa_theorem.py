#!/usr/bin/env python3
"""
Charged-lepton radiative Yukawa support runner

On the retained Cl(3)/Z³ lattice with the retained staggered-Dirac (minimal
axiom 3) and retained lattice coupling α_LM = 1/(4π u_0), this runner sharpens
the charged-lepton radiative scale lane.

It verifies that the textbook one-loop lattice-PT factor α_LM/(4π) is already
retained in the atlas, and it builds an explicit charged-lepton Casimir route
to the candidate relation

    y_τ^fw = α_LM / (4π)

under standard electroweak normalization choices. This is strong support for
the `m_τ` / `v_0` lane, not a fully axiom-native retained closure theorem.
...
def part_E():
    ...
    record(
        "E.1 m_τ support chain matches PDG using retained atlas + textbook EW Casimirs",
        ...
        "The load-bearing electroweak normalization choice remains textbook,\n"
        "so this is support rather than promoted retained closure.",
    )
...
def main() -> int:
    ...
    if all_pass:
        print("VERDICT: support chain verified.")
        print()
        ...
        print()
        print("  Result: the charged-lepton radiative/Yukawa route is materially")
        print("  strengthened and matches PDG at 0.006%, but it still depends on")
        print("  textbook electroweak normalization choices and remains a support lane.")
    else:
