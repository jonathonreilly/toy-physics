...
def main() -> int:
    ...
    if not (math.isnan(q) or math.isnan(p) or p == 0):
        ...
        fit_ok = (
            not math.isnan(q_all) and not math.isnan(p_all)
            and not math.isnan(r2_all) and r2_all >= 0.99
            and not math.isnan(q) and q > 0
            and not math.isnan(p) and p > 0
            and not math.isnan(r2) and r2 >= 0.99
        )
        if abs(s_at_finest) < SAT_THRESHOLD:
            ...
            print(f"  the fitted critical schedule exits the stated small-field")
            print(f"  regime on this harness; deeper structural change needed.")
            analysis_ok = fit_ok
        if not fit_ok:
            print()
            print("  FIT GUARD FAIL — need q > 0, p > 0 and R^2 >= 0.99 on")
            print("  both the full-grid and fine-h response-surface fits.")
    else:
        print("  FIT GUARD FAIL — critical-schedule estimate unavailable.")
    print()

    if fail or not (born_ok and k0_ok) or not analysis_ok:
        return 1
    return 0
