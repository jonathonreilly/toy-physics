...
This script reproduces the local diagnostics behind three correction
surfaces. It is source-only: its job is to make the corrections
reviewable without relying on open PR verdicts.
...
(B) Y-S4b-RGE positive -> bounded downgrade.
    ...
    Review context: #956 (downgrade), #1023 (I4 refinement).
...
(C) m_t Ward-BC trajectory artifact.
    ...
    Review context: #1022.
    Claim: high-UV trajectories of the existing 1-loop SM RGE with
    alpha_s(v) = 0.1033 compress to an IR band above the Ward-BC
    trajectory landing 0.973 (m_t ~ 169 GeV), not onto that landing.
    Verify by forward integration from several UV BCs at MPl.
"""
...
def verify_qfp_attractor_mislocation() -> tuple[bool, dict[str, object]]:
    """Verify (C): the IR QFP attractor for y_t under the alpha_s(v) anchor lies
    ...
    toward an above-one IR band (NOT 0.95).
    ...
    -7 across the range (existing 1-loop content; we are reproducing the
    PR #1022 diagnostic, not the full multi-threshold SM).
    """
    # IR anchor: alpha_s(v) = 0.1033 per the existing CMT identification.
    alpha_s_v = 0.1033
    g3_v = math.sqrt(4.0 * math.pi * alpha_s_v)
    # EW gauge anchors at v (rough values; subdominant for QFP location).
    g2_v = 0.65
    g1_v = 0.46
    ...
    # UV BCs in [0.7, 1.8] should all converge to a tight band above
    # the Ward-BC landing.
...
def main() -> int:
    ...
    print("-" * 78)
    print(
        "Review context: #1049. Claim: tau_M(H) = Tr(pi_+(H)) + Tr(pi_perp(H)) "
    ...
    print("-" * 78)
    print(
        "Review context: #956 + #1023. Claim: closure carries >=3 named imports "
    ...
    print("-" * 78)
    print(
        "Review context: #1022. Claim: high-UV QFP band sits above the Ward-BC landing; "
