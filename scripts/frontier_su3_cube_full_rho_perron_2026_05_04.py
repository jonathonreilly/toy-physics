...
Per the counterfactual pass (PR #511), the framework's stated target is
the L_s=2 APBC spatial cube with 12 plaquettes / 24 links (per the
existing runner frontier_su3_cube_perron_solve.py), where the all-
forward convention is invariant under APBC for self-conjugate sectors.
...
def driver() -> int:
    ...
    print(f"  ({BRIDGE_SUPPORT_UPPER:.4f}); the gap is structurally fixed at")
    print(f"  ~ 0.16 = ~ 543× ε_witness, regardless of NMAX_rho.")
    print()
    if gap_factor > 100:
    ...
    print(f"  Block 5 reference (PR #501): {P_CANDIDATE_REFERENCE:.6f}")
    ...
    print(f"             (gap ~ 543× ε_witness, structurally fixed).")
    print(f"  Counterfactual A from PR #511 verified: framework target geometry")
    print(f"  identified, value computed, but not closing — consistent with")
    print(f"  prior Block 5 finding (PR #501).")
