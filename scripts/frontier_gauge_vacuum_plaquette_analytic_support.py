#!/usr/bin/env python3
"""
Gauge-vacuum plaquette analytic support.

Exact content:
1. local SU(3) one-plaquette block
2. scalar 3+1 temporal ratio
3. exact 3+1 plaquette/link incidence factor

Support-level content:
1. dimension-4 scalar-density insertion candidate
2. resulting analytic plaquette candidate at beta=6

Canonical plaquette authority on main remains the same-surface evaluated value.
"""
...
from frontier_scalar_3plus1_temporal_ratio import (
    dim4_scalar_density_factor,
    completion_ratio,
    scalar_bridge_coefficient,
)
...
def effective_beta_candidate(beta: float) -> float:
    return beta * coordination_factor_3plus1() * dim4_scalar_density_factor()
...
def main() -> int:
    ...
    beta_eff = effective_beta_candidate(bare_beta)
    ...
    print("=" * 78)
    print("GAUGE-VACUUM PLAQUETTE ANALYTIC SUPPORT")
    print("=" * 78)
    print()
    print("Exact subtools")
    print("  local SU(3) block: P_1plaq(beta_loc) = d/d beta_loc log Z_1plaq(beta_loc)")
    print("  scalar ratio:      A_inf / A_2 = 2 / sqrt(3)")
    ...
    print(f"  scalar A_2                   = {fmt(a2)}")
    print(f"  scalar A_inf (Lt=4096 proxy) = {fmt(a4096)}")
    print(f"  scalar ratio                 = {fmt(ratio)}")
    print(f"  incidence factor             = {fmt(coord)}")
    print()
    print("Support candidate")
    print(f"  Gamma_sc,cand                = {fmt(factor)}")
    print(f"  beta_eff,cand                = {fmt(beta_eff)}")
    print(f"  P_cand(6)                    = {fmt(p_eff)}   (mode cutoff m = {mode_eff})")
    print(f"  Weyl check at beta_eff,cand  = {fmt(p_eff_weyl)}")
    print(f"  canonical same-surface P     = {fmt(CANONICAL_PLAQUETTE)}")
    print(f"  candidate minus canonical    = {p_eff - CANONICAL_PLAQUETTE:+.12f}")
    print()
    ...
    exact_checks.append(check_close("scalar completion ratio", ratio, 2.0 / math.sqrt(3.0), 1.0e-15))
    exact_checks.append(check_close("3+1 incidence factor", coord, 1.5, 1.0e-15))

    support_checks: list[tuple[bool, str]] = []
    support_checks.append(
        check_close(
            "dimension-4 scalar-density insertion candidate",
            factor,
            (2.0 / math.sqrt(3.0)) ** 0.25,
            1.0e-15,
        )
    )
    support_checks.append(check_close("Bessel/Weyl agreement at beta_eff,cand", p_eff, p_eff_weyl, 1.0e-12))
    support_checks.append(
        check_true(
            "analytic support candidate stays close to canonical plaquette",
            abs(p_eff - CANONICAL_PLAQUETTE) < 2.0e-4,
            f"|P_cand(6) - P_can| = {abs(p_eff - CANONICAL_PLAQUETTE):.6e} < 2e-4",
        )
    )
