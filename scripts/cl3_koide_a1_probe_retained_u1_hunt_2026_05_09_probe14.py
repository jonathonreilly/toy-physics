...
Tests whether the currently inventoried retained U(1) candidates project
onto the U(1)_b SO(2) angular quotient on the b-doublet of the Brannen
circulant H = aI + bC + b̄C² on hw=1 ≅ C^3.
...
VERDICT: STRUCTURAL OBSTRUCTION.
  ...
  All 9 retained U(1) candidates examined here fail at least one of Tests
  2–4. None projects onto U(1)_b on the b-doublet from the examined retained
  content. The residue (U(1)_b) remains a derivation target requiring either:
    (A) explicit user-approved admission of a new continuous primitive, or
...
M_test = (np.arange(64, dtype=float).reshape(8, 8)
          + 1j * np.arange(64, 128, dtype=float).reshape(8, 8))
commutator_random = omega_pseudo @ M_test - M_test @ omega_pseudo
...
print("either:")
print("  (A) Explicit user-approved admission of U(1)_b as a NEW continuous")
print("      primitive (a 1-dim Lie-algebra extension of the discrete C_3), OR")
