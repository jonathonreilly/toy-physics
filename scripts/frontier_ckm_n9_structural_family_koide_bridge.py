...
Verifies the complete n/9 family in
  ...
  F2: 2/9 = N_pair / N_color^2                          [accepted Bernoulli support readouts]
...
def audit_complete_family() -> None:
    banner("Complete n/9 support family from retained CKM inputs (F1-F9)")
    ...
    check("F1 = 1/9 (Wolfenstein x CP-phase product)", F1 == Fraction(1, 9))
    check("F2 = 2/9 (Bernoulli support readout)", F2 == Fraction(2, 9))
    check("F3 = 3/9 = 1/3 (W2 complement)", F3 == Fraction(3, 9))
    check("F4 = 4/9 (A^4)", F4 == Fraction(4, 9))
...
def audit_g4_generating_set() -> None:
    banner("(G4) Generating set {A^2, rho, eta^2, N_pair, N_color, 1} spans support family")
    ...
    check("(G4) Generating set spans support family (demonstrated)", True)
...
def audit_summary() -> None:
    ...
    print()
    print("  NEW (F1-F9 support family): every n/9 for n in {1,...,9} derivable")
...
def main() -> int:
    ...
    print("See docs/CKM_N9_STRUCTURAL_FAMILY_KOIDE_BRIDGE_SUPPORT_NOTE_2026-04-25.md")
    print("=" * 88)
