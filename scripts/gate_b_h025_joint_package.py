#!/usr/bin/env python3
"""Gate B h=0.25 joint package harness.

Bounded companion to gate_b_grown_joint_package.py.
...
session, so this script freezes a smaller compact refinement probe instead.

The goal is not a broad search. The goal is a reviewable retained chain for
the h=0.25 analogue of the grown-geometry joint package.
"""
...
def main():
    t0 = time.time()
    print("=" * 76)
    print("GATE B H=0.25 JOINT PACKAGE HARNESS")
    ...
    print("  This harness freezes the h=0.25 joint interference/decoherence")
    print("  comparison only.")
    print("  Promote the retained grown row only if it stays close to the exact grid.")
    print("  Treat the result as a bounded scaling check, not a full closure claim.")
