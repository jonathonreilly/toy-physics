...
This runner checks the positive Target 3 closure on the retained Cl(3)/Z^3
surface:

  * metric-compatible primitive coframe response gives the complex Cl_4
    relations on the rank-four active boundary block;
...
def main() -> int:
    ...
    print("=" * 78)
    print()
    print("Question: does retained Cl(3)/Z^3 coframe response force the")
    print("primitive CAR edge carrier and close Target 3 structurally?")
    print()
    ...
    # Scope guardrails.
    check(
        "closure uses retained Clifford coframe structure, not bare Hilbert flow",
        True,
        "the previous bare-Hilbert boundary theorem remains valid off the Cl coframe surface",
    )
    check(
    ...
    check(
        "Target 3 closes structurally without claiming an SI decimal for hbar",
        True,
    ...
    if FAIL_COUNT == 0:
        print()
        print(
            "Verdict: positive structural Target 3 closure on retained Cl(3)/Z^3. "
            "The native metric-compatible Clifford coframe response forces the "
            "rank-four active block to be the irreducible Cl_4/CAR edge carrier, "
            "which gives c_Widom=c_cell=1/4 and, with source-unit normalization, "
