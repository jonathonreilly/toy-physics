...
def driver() -> int:
    ...
    if all(n == 2 for n in leg_per_link_w):
        ...
        print("  - legs use distinct link IDs (the links one row over).")
        print("  So each link ID is in exactly 2 plaquettes.")
    ...
    print()
    print("  Both the all-forward traversal (matching the framework's")
    print(f"  candidate runner, P = {P_CANDIDATE_REPORTED:.4f}) and the")
    print("  standard Wilson +-+- traversal applied to the L_s=2 PBC cube")
    print(f"  give P-values FAR BELOW the bridge target {BRIDGE_SUPPORT_TARGET:.4f}")
    print(f"  (gap >> epsilon_witness = {EPSILON_WITNESS:.3e}).")
    print()
    ...
    print()
    print("  CONCLUSION: the multi-week L_s>=3 Wigner-Racah engine work")
    print("  IS GENUINELY REQUIRED for the gauge-scalar temporal observable")
    print("  bridge closure. The framework-built Block 1-4 infrastructure")
    ...
    print("  that no L_s=2 PBC convention closes the bridge gap.")
    print()
    ...
    print("  derivable. To escape the no-go, additional retained")
    ...
    print()
    print("Headline:")
    print(f"  L_s=2 PBC cube — both conventions verified:")
    ...
    print(f"  Verdict: L_s>=3 Wigner-Racah work IS genuinely required.")
