...
Key NEW identities on the retained NLO Wolfenstein protected-gamma_bar surface:
  ...
  (V6) Sum of squared deviations from the mean:
        Sigma (r_i - mean)^2  =  p_2 - 2 mean p_1 + 4 mean^2
                              ...
                              =  N_pair^4 (N_pair^3 - 2 N_pair^0 + ?)
                                  Wait, let me re-derive symbolically.
                              =  128 - 32 + 16 = 112.
...
def audit_v8_galois_structure(N: dict, S: dict) -> None:
    ...
    print("  The 4 roots are organised into two orbits of size 2 each (CC pairs).")

    # No symbolic check needed -- this is a classification statement.
    check("V8: Galois group classification recorded (Z/2)^3 acting on roots via Klein 4 image",
          True)
