...
def main():
    ...
    print()
    print(f"CONCLUSION:")
    print(f"  Test 1 confirms the IR/UV transition: d=2 shows clear logarithmic")
    print(f"  growth with lattice size (IR divergence), while d>=3 saturates.")
    print(f"  The transition occurs between d=2 and d=3.")
    print(f"  Test 2 confirms E ~ 1/sigma^(d-2) for Gaussian sources, matching")
    print(f"  the analytic prediction that UV divergence steepens with dimension.")
    print(f"  Test 3 shows self-consistent iteration converges at all d tested.")
    print(f"  Test 4 shows that refining the lattice at fixed physical size")
    print(f"  increases E_self at all d, as expected from resolving the point-like")
    print(f"  core of the Gaussian — the rate increases with d.")
    print(f"")
    print(f"  The numerics support d=3 as the critical dimension where the")
    print(f"  point-source self-energy transitions from IR-dominated growth")
    print(f"  (d=2: log(N), factor 1.77x) to rapid UV saturation (d>=4).")
    print(f"  At d=3, E_self changes by only 7% across a 4x range of N,")
    print(f"  showing mild sensitivity to both cutoffs.")
    print()
