#!/usr/bin/env python3
"""
PR #230 block27 post-block26 landed checkpoint.

Block26 was the post-block25 landed checkpoint.  This checkpoint answers the
narrow resume question after that checkpoint landed on draft PR #230: did the
PR head move by anything other than the block26 checkpoint, and does that landed
...
OUTPUT = (
    ROOT
    / "outputs"
    / "yt_pr230_block27_post_block26_landed_checkpoint_2026-05-11.json"
)

PR_REF = "origin/claude/yt-direct-lattice-correlator-2026-04-30"
MAIN_REF = "origin/main"
BLOCK26_INPUT_HEAD = "8b0d95db83c6f8458b0547c1da32e690941e36a3"
BLOCK26_SUBJECT = "Record PR230 block26 post-block25 checkpoint"
BLOCK27_SUBJECT = "Record PR230 block27 post-block26 checkpoint"

PARENTS = {
    "block26_post_block25_landed": "outputs/yt_pr230_block26_post_block25_landed_checkpoint_2026-05-11.json",
...
def allowed_block26_path(path: str) -> bool:
    return path in ALLOWED_BLOCK26_PATHS or any(
        path.startswith(prefix) for prefix in ALLOWED_BLOCK26_PREFIXES
    )
...
def main() -> int:
    print("PR #230 block27 post-block26 landed checkpoint")
    print("=" * 72)

    parents = {name: load_json(path) for name, path in PARENTS.items()}
    local_head = run_git(["rev-parse", "HEAD"])
    local_subject = run_git(["log", "-1", "--format=%s", "HEAD"])
    inspection_ref = "HEAD^" if local_subject == BLOCK27_SUBJECT else "HEAD"
    ...
    commits_since_block26_input = [
        line
        for line in run_git(
            ["log", "--format=%H%x09%s", f"{BLOCK26_INPUT_HEAD}..{inspection_ref}"]
        ).splitlines()
        if line
    ]
    paths_since_block26_input = changed_paths(BLOCK26_INPUT_HEAD, inspection_ref)
    disallowed_paths_since_block26_input = [
        path for path in paths_since_block26_input if not allowed_block26_path(path)
    ]
    ...
    block26 = parents["block26_post_block25_landed"]
    ...
    only_block26_checkpoint_since_input = (
        len(commits_since_block26_input) == 1
        and commits_since_block26_input[0].endswith(f"\t{BLOCK26_SUBJECT}")
        and not disallowed_paths_since_block26_input
    )
    ...
    checks = {
        "inspection-head-is-current-pr-head-or-parent-of-block27": inspected_head == pr_head
        or local_head == pr_head,
        "inspection-head-is-block26-checkpoint": inspected_subject == BLOCK26_SUBJECT,
        "only-block26-checkpoint-since-block26-input-head": only_block26_checkpoint_since_input,
        ...
        "block26-no-route-admitted": block26.get("checks", {}).get(
            "queue-rank1-source-higgs-not-admitted"
        )
        is True
        and block26.get("checks", {}).get("queue-rank2-wz-not-admitted") is True
        and block26.get("checks", {}).get("queue-rank3-neutral-h3h4-not-admitted")
        is True,
        "block26-forbidden-firewall-clean": block26.get("checks", {}).get(
    ...
    report(
        "inspection-head-is-current-pr-head-or-parent-of-block27",
        checks["inspection-head-is-current-pr-head-or-parent-of-block27"],
        f"HEAD={local_head[:9]} inspected={inspected_head[:9]} PR={pr_head[:9]}",
    )
    report(
        "only-block26-checkpoint-since-block26-input-head",
        checks["only-block26-checkpoint-since-block26-input-head"],
        f"commits={len(commits_since_block26_input)} disallowed_paths={len(disallowed_paths_since_block26_input)}",
    )
    report(
    ...
    report(
        "block26-no-route-admitted",
        checks["block26-no-route-admitted"],
        status(block26),
    )
    report(
    ...
    result = {
        ...
        "block26_input_head": BLOCK26_INPUT_HEAD,
        "commits_since_block26_input_head": commits_since_block26_input,
        "paths_since_block26_input_head": paths_since_block26_input,
        "disallowed_paths_since_block26_input_head": disallowed_paths_since_block26_input,
        ...
        "actual_current_surface_status": (
            "open / block27 post-block26 landed checkpoint; PR head contains "
            "only the block26 checkpoint after the previous landed-checkpoint input "
        ...
        "proposal_allowed_reason": (
            "Block27 is a landed-checkpoint, not a new closure artifact.  The "
            "current PR head is the block26 checkpoint commit; no required "
        ...
        "strict_non_claims": [
            ...
            "does not treat the block26 checkpoint commit as new physics evidence",
        ],
