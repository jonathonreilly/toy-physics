...
PR #968 fitted sigma_arm(h) = C * h^alpha as a 2-parameter log-linear
regression on the four fine-h points h <= 0.25 and obtained

    C_arm_unconstrained = 2.7107
    alpha_unconstrained = 0.5256
    R^2                 = 0.9996

PR #1003 derived from a coherent path-integral saddle on the per-step
lateral characteristic function the closed form
...
is a fit-protocol artifact: the unconstrained fit lets alpha drift to
0.5256 to absorb the finite-h cosine phase factor in the saddle, which
biases the prefactor C upward to compensate. The geodesic continuum
prediction is alpha = 1/2 exactly (PR #968 sec. "Why geodesic"); the
right way to extract the prefactor is therefore an alpha-constrained
fit, equivalently a per-h evaluation of
...
#968's runner cache, computes C_arm(h) under the geodesic constraint
alpha = 1/2, and reports the residual against PR #1003's analytic
2.4855 and against PR #968's unconstrained fit 2.7107. It also
includes a freshly measured row at h = 0.015625 (one factor of 2
finer than PR #968's finest grid point), measured by the same
function used in PR #968's runner.

Outcome registered (this run):

  - residual at h = 0.03125 closes to 0.504% under alpha-constrained fit
  - residual at h = 0.015625 closes to 0.260%
  - the 8.31% gap reported in PR #1003 is fully explained by fit
    protocol; the analytic and numerical agree pointwise at the finest
    measured h to better than half a percent
...
def C_arm_analytic() -> float:
    """Closed-form geodesic constant from PR #1003.
...
# ---------------------------------------------------------------------------
# PR #968 raw data: per-h sigma_arm values from
#   logs/runner-cache/lattice_nn_rescaled_continuum_identification.txt
# (cached on main; identical to PR #968's runner output).
# ---------------------------------------------------------------------------
...
# PR #968 unconstrained fit (printed verbatim from the cached log)
C_ARM_UNCONSTRAINED = 2.7107
ALPHA_UNCONSTRAINED = 0.5256
...
def measure_finer_row() -> Optional[Dict[str, float]]:
    """Measure sigma_arm at H_FINER using the PR #968 runner's function."""
    try:
    ...
    print(f"  [info] measuring h = {H_FINER} via PR #968's "
...
def main() -> int:
    ...
    print("  Geodesic constraint alpha = 1/2 exact (PR #968 sec. 'Why geodesic')")
    print("  Estimator: C_arm(h) := sigma_arm(h) / sqrt(h),  per-h, no log-fit")
    print("=" * 100)
    print()

    C_an = C_arm_analytic()
    print(f"Analytic constant (PR #1003 closed form):")
    ...
    print()
    print(f"Unconstrained 2-parameter fit (PR #968):")
    ...
    for r in rows:
        ...
        if abs(h - H_FINER) < 1e-9:
            ...
            flag = "fine-h (used in PR #968 fit)"
        else:
    ...
    print(f"                             = {C_extrap:.6f}")
    print()
    print(f"  PR #1003 analytic          = {C_an:.6f}")
    print(f"  PR #968  unconstrained fit = {C_ARM_UNCONSTRAINED:.6f}")
    print()
    ...
    if target is not None:
        ...
        if sub_one_percent:
            print("    BEST CASE: residual < 1% under alpha-constrained fit.")
            print("    The 8.3% reported gap was fit-protocol artifact.")
            print("    Analytic and numerical agree pointwise at h = 0.03125.")
        elif target_passes:
            print("    GOOD CASE: residual <= 2% under alpha-constrained fit.")
        else:
    ...
    if finer_row is not None:
        ...
        if abs(res_fnew) < 1.0:
            ...
            print("    consistent with the geodesic saddle being the leading-")
    ...
    return 0 if abs(res_an_extrap) <= 2.0 else 1
