...
Negative on the conjecture (PASS=no-go expected):
  4. Finite CS / knot-polynomial data does not derive the beta_n
     channel-weight vector; field membership alone is not a derivation.
  ...
  9. Hostile review: even granting all bounded imports, the final
     identification (knot polynomial = beta_n weight) is NOT derived.

Bounded imports (reported explicitly, no derivation):
...
- NO new axioms (CS theory and Z^3 -> T^3 are bounded stress-test
  imports here, not repository axioms)
...
class Counter:
    """Simple counter for PASS / FAIL / bounded-import outcomes."""
    ...
    def admit(self, name: str, detail: str = "") -> None:
        if detail:
            print(f"  [BOUNDED-IMPORT] {name} | {detail}")
        else:
            print(f"  [BOUNDED-IMPORT] {name}")
        self.admitted += 1
    ...
    def summary(self) -> None:
        ...
        print(
            ...
            f"BOUNDED_IMPORT={self.admitted}"
        )
        if self.failed:
...
# ----------------------------------------------------------------------
# Constants from supported framework + bounded stress-test imports
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# SECTION 4 — NEGATIVE: finite CS data does not derive beta_n weights
# ----------------------------------------------------------------------

def section4_value_field_mismatch(c: Counter) -> None:
    """Finite CS / knot-polynomial data does not derive beta_n weights.

    The CS outputs used here are finite integers and cyclotomic-algebraic
    values at fixed level. Since Q is contained in cyclotomic fields, bare
    field membership cannot be the no-go: the no-go is that the CS data and
    level shift do not construct the specific QCD beta_2 channel vector, and
    the beta_3 zeta(3) terms are imported literature comparators with no
    derived CS map in this probe.
    """
    print()
    print("Section 4 — NEGATIVE: finite CS data does not derive beta_n weights")
    ...
    c.record(
        "beta_3 contains zeta_3 terms in the literature comparator",
        True,
        "this probe derives no CS map producing those terms",
    )

    print("    -> Q is contained in cyclotomic fields, so field containment alone")
    print("       is not the theorem. The no-go is absence of a derived map from")
    print("       finite CS/knot data to the QCD channel-weight vector.")
    print("    -> The beta_3 zeta(3) terms remain literature comparators, not")
    print("       framework-derived outputs of this topological construction.")
...
def section8_o13_direct_application(c: Counter) -> None:
    ...
    # Example: beta_2 channel weight 2857/54 -- is it of form q*2*pi
    # for rational q? Since pi is irrational, a nonzero rational divided
    # by 2*pi is irrational.
    ...
    c.record(
        ...
        target != 0,
        f"{target}/(2*pi) is irrational because pi is irrational",
    )

    # For all 6 TVZ channels -- none are zero, so none are q*2*pi with
    # rational q.
    n_rational_over_2pi = sum(1 for w in TVZ_CHANNEL_WEIGHTS.values() if w == 0)
    c.record(
...
def section9_hostile_review(c: Counter) -> None:
    ...
    set of bounded stress-test imports:
      (a) Z^3 -> T^3 via periodic boundary conditions,
      (b) CS theory on T^3 with SU(3) at some level k,
    ...
    Answer: NO. The four imports provide finite CS / knot-polynomial data.
    No map from those data to the QCD beta_n channel vector is derived from
    retained structure or from the imports themselves.

    To assert such a map would be to introduce it as a new primitive bridge,
    not a derivation.
    """
    print()
    print("Section 9 — HOSTILE REVIEW: final identification is not derived")

    # The 4 bounded stress-test imports.
    admissions = [
        ("Z^3 -> T^3 via periodic BCs", "bounded stress-test import"),
        ("CS-on-T^3 with SU(3)_k", "standard topological QFT import"),
        ("Wilson loops on knots", "standard CS Wilson-loop import"),
        ("Level shift k -> k+h^vee", "standard CS regularization import"),
    ]
    for adm, src in admissions:
        c.admit(f"Import: {adm}", src)

    # The load-bearing identification (NOT derived)
    c.admit(
        "Identification (knot polynomial value) <-> (beta_n channel weight)",
        "NOT derived from retained structure or from the bounded imports; "
        "introducing it would be a new primitive bridge, not a derivation",
    )

    # Therefore the conjecture fails even under maximal admissions
    c.record(
        "Even granting all imports, the load-bearing bridge is NOT derived",
        True,
        "the topological-CS route fails because the final map is undefined",
    )
    ...
    print("       derived; it would be a new primitive bridge, not a derivation.")


# ----------------------------------------------------------------------
# SECTION 10 — BOUNDED IMPORTS
# ----------------------------------------------------------------------

def section10_bounded_admission(c: Counter) -> None:
    """Bounded import: beta_2 and beta_3 channel weights are not
    derived in this probe by any topological route. The CS / knot
    polynomial bridge fails.

    In context with other negative probes, this adds one more bounded
    stress test against attempts to derive the channel-weight vector
    without the imported loop-integral data.
    """
    print()
    print("Section 10 — BOUNDED IMPORTS")
    ...
    c.admit(
        ...
        "no derived CS map + O13 + dimensional gap + underspecification",
    )
    c.admit(
        "Topological-CS bridge for beta_3",
        "no derived CS map to the literature zeta_3 terms",
    )

    # Synthesis with prior probes
    c.record(
        "Topological attack route contributes bounded negative evidence",
        True,
        "this note does not promote a repo-wide terminality theorem",
    )

    print("    -> Topological / Chern-Simons / knot-polynomial route foreclosed.")
    print("    -> Repo-wide channel-weight status is left to the audit/dependency lane.")
...
def section11_verdict(c: Counter) -> None:
    ...
    print("  - Finite CS / knot-polynomial data does not derive the QCD")
    print("    beta_n channel-weight vector")
    ...
    print("  - Even granting all imports, the load-bearing knot-polynomial =")
    print("    beta_n-weight identification is NOT derived; would be a new")
    print("    primitive bridge")
    print()
    print("BOUNDED imports:")
    ...
    print("  - This is bounded negative evidence only; it does not promote")
    print("    a repo-wide channel-weight terminality theorem.")
...
def main() -> int:
    ...
    assert counter.passed >= 47
    assert counter.admitted == 9
    if counter.failed > 0:
        return 1
    return 0
