...
def certified_branch_search() -> list[Branch]:
    ...
    if len(branches) != 3:
        raise RuntimeError(
            f"exhaustive chart cover did not stabilize to exactly three stationary branches (found {len(branches)})"
        )
    return branches
...
def certify_global_minimum(branches: list[Branch]) -> None:
    ...
    lag_hess = finite_hessian(relative_action_from_chart, low.representative) - lam * finite_hessian(
        closure_residual, low.representative
    )
