...
This runner compares two explicitly defined reduced measure candidates on the
C_3-equivariant Hermitian circulant surface

    H = aI + bC + bbar C^2

with the Brannen Amplitude Equipartition condition

    |b|^2/a^2 = 1/2.

It asks whether any saddle, Gaussian, or full-measure observable distinguishes
M1 from M2 in a way that can be compared against PDG data.
...
# ----------------------------------------------------------------------
# Section 0 — Reduced model setup sanity
# ----------------------------------------------------------------------

section("Section 0 — Reduced model setup sanity")
...
# 0.3 Tr(C) = Tr(C^2) = 0 (doublet basis traceless)
...
# ----------------------------------------------------------------------
# Section 1 — Saddle-level observables: M1 = M2 identically
# ----------------------------------------------------------------------

section("Section 1 — Saddle-level observables (M1 = M2 identically)")

# At BAE saddle: |b|^2/a^2 = 1/2, Q = 2/3
# Both M1 and M2 force this in the reduced model.
...
# H_M1 = diag(-12, -24), H_M2 = diag(-6, -12) = H_M1/2 in the
# corresponding two-coordinate reduced model. We re-derive the one-dimensional
# constrained version here.
#
...
check(
    "2.3  Hessian ratio M1/M2 = 2 (reduced-model factor)",
...
print(f"    var_M1 / var_M2 = {var_ratio:.6f}  (= 4/5 in the full-measure parametrization)")
check(
    "5.3  var_M1 / var_M2 = 4/5 (full-measure parametrization)",
...
print("  The current reduced model does not elect a canonical primitive.")
...
# ----------------------------------------------------------------------
# Section 9 — Does-not disclaimers and observability boundary
# ----------------------------------------------------------------------
...
print("    - Elect M1 or M2 as the canonical primitive.")
print("    - Close Brannen Amplitude Equipartition as an audited claim.")
print("    - Introduce new axioms.")
print("    - Modify upstream claim content.")
...
check("9.1  Does not elect M1 or M2", True)
check("9.2  Does not close Brannen Amplitude Equipartition", True)
check("9.3  Does not load-bear PDG", True)
check("9.4  Does not introduce new axioms", True)
check("9.5  Does not modify upstream claim content", True)
