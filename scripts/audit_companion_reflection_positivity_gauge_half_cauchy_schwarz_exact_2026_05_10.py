...
The narrow theorem's load-bearing content is the bounded measure-
theoretic identity on a finite measured space (X, μ, Θ) with a
measure-preserving involution Θ, a Θ-invariant real half-action, and
reflection-Hermitian observables:

  (G1) ⟨ Θ(F) · F ⟩ = ‖ψ² F‖²_{L²(X, μ)}     (norm-square factor.)
...
on the finite cyclic set Z/N with involution Θ(k) = -k mod N. The
finite model captures only the explicitly symmetric norm-square
identity; it does not validate the full Wilson plaquette or
fermion-sector reflection-positivity theorem.
...
def main() -> int:
    ...
    print("Goal: sympy verification of the bounded (G1)-(G3) norm-square identity")
    print("=" * 88)
    ...
    def S_full(k: int) -> Rational:
        """Symmetric two-half action with no boundary term."""
    ...
    def F1(k):
        ...
        # This early test function is not load-bearing; the checked identity
        # below uses reflection-Hermitian F.
        if k in (0, 1, 2, 3):
    ...
    # ABSTRACT identity (G1) for the bounded symmetric case:
    ...
    def norm_sq_psi_sq_F_sym(F):
        """Compute ‖ψ²_sym F‖²_{L²(X, μ)} = (1/N) Σ_k |ψ²_sym(k) F(k)|²."""
        total = 0
        for k in range(N):
    ...
    print("    (G1) ⟨Θ(F)·F⟩ = ‖ψ² F‖²_{L²} under explicit symmetry hypotheses")
