...
def compute_eta(k_B, m3_eV=M3_NU, phi=PHI_CP, epsB=0.041):
...
for label, y_val, comment in yukawa_choices:
    if y_val is None:
        ...
        M1_default = M_PL * ALPHA_LM**5 * (1 - 0.041)
...
log(f"    This gives k_B ~ 15, far from 7-8.")
log()
...
log()
log("  BUT: No structural argument selects y_nu = alpha_LM^{1.6} (k_B=7)")
log(f"  or y_nu = alpha_LM^{{2.1}} (k_B=8) from the Cl(3) axiom.")
log("  The structural choices (y=1, y=alpha, y=alpha^2, ...) give")
log("  k_B = 13, 15, 17, ... -- all far from 7-8.")
log()
log("  VERDICT (Arg 3): The seesaw formula DOES NOT uniquely select k_B.")
log("  With STRUCTURAL Yukawas (integer powers of alpha_LM), the seesaw")
log("  predicts k_B = 13-17, far from the leptogenesis window k_B = 7-8.")
log("  The calibrated y_0 at k_B = 7-8 is perturbative but not structural.")
log()
...
for n_power in range(0, 8):
    ...
    # The M_1 at k_int:
    M1_struct = M_PL * ALPHA_LM ** k_int * (1 - 0.041)
...
log("    n=0 (y=1):      k_B ~ 12.6, m_3 ~ unphysical at k_B=13")
log("    n=1 (y=alpha):   k_B ~ 14.6, eta negligible")
log("    n=2 (y=alpha^2): k_B ~ 16.7, below EW scale")
log()
log("  NONE of the structural Yukawa choices give k_B in the 7-8 range.")
log("  The seesaw and leptogenesis constraints point to DIFFERENT k_B")
log("  when y_nu is constrained to be a structural power of alpha_LM.")
log()
...
# The full result was obtained in frontier_neutrino_masses.py by fitting
# rho = B/A = 1.93 and eta = eps/A = 0.041.
...
log(f"  {'3. Seesaw (y=alpha_LM)':>35s} | {'NO':>14s} | {'~15':>14s}")
log(f"  {'3. Seesaw (y=1)':>35s} | {'NO':>14s} | {'~13':>14s}")
...
log("""
  ...
  SEESAW with perturbative y_nu:
    y_nu = alpha_LM   -> k_B ~ 15 (M_R ~ 10^4 GeV)
    y_nu = alpha_LM^2 -> k_B ~ 17 (below EW!)
    y_nu = 1           -> k_B ~ 13 (M_R ~ 10^6 GeV)

  LEPTOGENESIS (observed eta):
    k_B = 7-8 (M_R ~ 10^{10-11} GeV)

  The gap is ~5-8 staircase levels (factor 10^{5-8} in M_R).
  This is the well-known "gravitino problem" / seesaw-leptogenesis tension:
    - Large M_R (high k_B) makes leptogenesis easy but the seesaw
      requires large y_nu (non-perturbative)
    - Small M_R (low k_B) keeps y_nu perturbative but makes the
      DI bound on epsilon_1 too small for sufficient eta
...
log("  " + "-" * 78)
log()
log("  At k_B = 7: y_0 ~ 0.028 = alpha_LM^{1.48} (between alpha and alpha^2)")
log("  At k_B = 8: y_0 ~ 0.0085 = alpha_LM^{2.00} (exactly 2 taste factors!)")
log()
...
log("=" * 78)
log()
log("  QUESTION: Can the framework uniquely select k_B?")
log()
log("  ANSWER: NO.")
log()
...
log("    3. Seesaw:          Perturbative y_nu -> k_B = 13-15, not 7-8")
...
log("    5. Interpolation:   Continuous k_B = {:.2f} has no structural meaning".format(
        k_B_exact if k_B_exact else float('nan')))
log()
log("  HOWEVER, there is a notable coincidence at k_B = 8:")
log(f"    - The seesaw-calibrated y_0 = {y0_at_k8:.4f} is close to")
log(f"      alpha_LM^2 = {ALPHA_LM**2:.4f} (2-link gauge vertex coupling)")
log(f"    - eta/eta_obs = 0.33 (within factor of 3)")
log()
log("  If y_nu = alpha_LM^2 is accepted as the structural neutrino Yukawa")
log("  (arising from a 2-link gauge vertex insertion), then k_B = 8 is")
log("  self-consistent AND gives eta within factor 3 of observation.")
log()
log("  THE HONEST CLAIM FOR THE DM GATE:")
log()
log("  eta is bounded:  2 x 10^{-10}  <  eta  <  2.3 x 10^{-9}")
log("                   (staircase levels k_B = 7 and 8)")
log()
log("  The observed eta = 6.12 x 10^{-10} falls within this band.")
log("  The staircase level k_B is a discrete structural ambiguity")
log("  that cannot be resolved from the Cl(3) axiom alone.")
log()
