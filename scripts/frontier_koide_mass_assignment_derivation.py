...
This runner demonstrates that the AS/APS-based derivation of δ = 2/9
CLOSES this gap: once δ = 2/9 is forced by the Atiyah-Singer equivariant
G-signature, the Brannen formula
...
def main() -> int:
    ...
    section("Part D — Closure of the bounded-status gap")
    ...
    print()
    print("  Closure chain (all retained or explicitly computed):")
    ...
    if all_pass:
        print("VERDICT: charged-lepton mass-assignment is framework-derived, not pinned.")
        print()
        print("This closes the 'bounded' status note flagged by the retained")
        print("CHARGED_LEPTON_MASS_HIERARCHY_REVIEW_NOTE_2026-04-17 for the Koide lane.")
        print()
        print("Full closure:")
        print("  - δ = 2/9:  explicit Lefschetz + Berry phase on Z_3 (1, 2)")
        print("  - Q = 2/3:  Brannen parametrization identity + Z_3 Lefschetz sum")
        print("  - v_0:      1-loop Yukawa + retained hierarchy")
        print("  - Three masses distinct: forced by three distinct envelopes")
        print()
        print("All charged-lepton Koide items derived from retained primitives")
        print("+ textbook mathematics. Set-equality framing (companion runner")
        print("frontier_koide_name_free_set_equality.py) eliminates any residual")
        print("naming-convention dependency.")
    else:
