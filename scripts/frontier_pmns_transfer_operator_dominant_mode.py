...
def part2_the_seed_pair_is_reconstructed_exactly_from_the_transfer_modes() -> None:
    ...
    check("So the dominant-mode law fixes the aligned active seed pair", True,
          f"(xbar,ybar)=({rec_xbar:.6f},{rec_ybar:.6f})")
...
def part3_the_route_is_blind_to_the_5_real_corner_breaking_source() -> None:
    ...
    check("Therefore the transfer-operator route fixes the aligned seed pair but not the breaking carrier", True,
          "zero-sum off-seed carrier is outside the transfer image")
...
def part4_the_existing_weak_axis_seed_patch_is_recovered_exactly() -> None:
    ...
    check("The transfer kernel reproduces the existing weak-axis seed law structurally", True,
          "dominant and subdominant modes reconstruct the seed pair")
