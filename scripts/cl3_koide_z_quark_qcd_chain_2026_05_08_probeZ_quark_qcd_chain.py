...
Verdict: NEGATIVE — parallel QCD-confinement chain
  m_q = Lambda_QCD * C * alpha_s(M_Z)^n_q
does NOT simultaneously close (m_t, m_b, m_c) at integer or simple-rational
n_q with any single retained-derivable structural prefactor C at the 5% mass
precision gate.

Tests:
  Step 1 (sanity): retained QCD content
    Lambda_MSbar^(5) = 210 MeV (CONFINEMENT_STRING_TENSION_NOTE Step 4)
    alpha_s(M_Z) = 0.1181 (ALPHA_S_DERIVED_NOTE bounded retained)
  Step 2 (compute n_q with C=1): no heavy quark closes at integer n_q to 5%.
  Step 3 (test retained-structural C with integer n_q): per-quark hits exist
  ...
  Step 6 (cross-ratio test): m_b/m_c approx alpha_s^(-0.558), 0.058 from -1/2.
    The Probe-Y EW-chain m_b/m_c ~ alpha_LM^(-1/2) coincidence (0.0037 from
  ...
  Step 8 (no PDG as derivation input): PDG values used only as comparators.

No new axioms, no new imports beyond retained Cl(3)/Z^3 + retained-bounded
QCD content.
"""
...
def main():
    ...
    # =========================================================================
    heading("SECTION 1: RETAINED QCD-CHAIN CONSTANTS")
    ...
    # Retained-bounded QCD content
    alpha_s_v = alpha_bare / (u_0 ** 2)              # vertex-power chain
    alpha_s_MZ = 0.1181                              # bounded retained
    ...
    print(f"  retained alpha_s(v)      = {alpha_s_v:.10f}")
    print(f"  retained alpha_s(M_Z)    = {alpha_s_MZ}")
    print(f"  retained log(alpha_s)    = {log_alpha_s_MZ:.10f}")
    print(f"  retained Lambda^(5)      = {Lambda_MSbar_5} GeV (5-flavor MS-bar)")
    print(f"  retained Lambda^(4)      = {Lambda_MSbar_4} GeV (4-flavor MS-bar)")
    print(f"  retained Lambda^(3)      = {Lambda_MSbar_3} GeV (3-flavor MS-bar)")
    print(f"  retained Lambda^(3)_fw   = {Lambda_3_framework} GeV (framework Method 1)")

    if check("alpha_s(M_Z) = 0.1181 (retained-bounded)",
    ...
    if check("Lambda_MSbar^(5) = 210 MeV (retained-bounded per CONFINEMENT_STRING_TENSION_NOTE)",
    ...
    if check("alpha_s(v) = alpha_bare/u_0^2 (retained vertex-power identity)",
    ...
    print("  PDG comparators (used only for n_q computation; never as derivation input):")
    for name, (mass, scheme) in fermions_PDG.items():
        print(f"    m_{name:<3} = {mass:.6e} GeV ({scheme})")

    # Check that PDG values are not used as derivation input
    if check("PDG values are comparators only (not in derivation chain)",
             True,
             "PDG enters only via post-hoc comparison; chain inputs are retained-only"):
        pass_count += 1
    else:
        fail_count += 1
    ...
    if check("1-loop Lambda within order-of-magnitude of retained Lambda^(5)",
    ...
    # =========================================================================
    # Section 5: Test retained-structural C with integer n_q
    # =========================================================================
    heading("SECTION 5: RETAINED-STRUCTURAL C WITH INTEGER n_q")
    ...
    # Verdict: no single C works for all three
    print()
    ...
    if common_C is None:
        print(f"  NO single retained-structural C works for all three heavy quarks at 5%")
    else:
        print(f"  WARNING: common C={common_C[0]} works for all three at 5%: {common_C[2]}")

    if check("NO single retained-structural C closes (m_t, m_b, m_c) at 5% with integer n_q",
             common_C is None,
    ...
    print("  the failure mode: no unified retained-structural C admits a chain")
    ...
    if common_C_rat is None:
        print(f"    NO single retained-structural C works for all three heavy quarks at 5% (q<=6)")
    else:
    ...
    if common_C_rat_1pct is None:
        print(f"    NO single retained-structural C works for all three at 1% (q<=6)")
    else:
        print(f"    common C={common_C_rat_1pct[0]} works at 1% (q<=6): {common_C_rat_1pct[2]}")

    if check("NO single retained-structural C closes (m_t, m_b, m_c) at 1% mass gate (q<=6)",
    ...
    # Probe Y EW Wilson chain: m_b/m_c via alpha_LM
    ...
    if check("EW chain m_b/m_c ~ alpha_LM^(-1/2) at <0.01 from -1/2 (Probe Y observation)",
    ...
    # =========================================================================
    # Section 9: Sensitivity to retained-bounded inputs
    # =========================================================================
    heading("SECTION 9: SENSITIVITY TO Lambda_QCD AND alpha_s(M_Z)")

    print("  Verdict robustness across Lambda choices:")
    Lambda_choices = {
    ...
    if check("Verdict (no single-C closure) robust across Lambda^(3,4,5) choices",
             all_lambda_fail,
    ...
    print()
    print("  Verdict robustness across alpha_s(M_Z) PDG envelope:")
    alpha_s_choices = {
    ...
    if check("Verdict robust across alpha_s(M_Z) PDG envelope (+/- 0.0009)",
             all_alpha_fail,
    ...
    print("  Routes foreclosed by Probes X, Y, Z:")
    print("    [X] EW Wilson chain heavy-quark ABSOLUTE masses     -> NEG (21-57% errors)")
    print("    [Y] EW Wilson chain heavy-quark RATIO integer-diffs -> NEG (5.4-16.1% errors)")
    ...
    print()
    print("  Heavy-quark masses require generation-dependent prefactors C_q OR a")
    print("  qualitatively different mechanism than any single coupling-chain at")
    print("  integer or simple-rational exponents.")

    if check("All three coupling-chain routes (EW absolute, EW ratio, QCD chain) closed",
             True,
             "no further single-chain salvage path remains for heavy-quark masses"):
        pass_count += 1
    else:
        fail_count += 1
    ...
    print()
    print(f"  Verdict: BOUNDED (NEGATIVE)")
    print(f"  - Parallel QCD-confinement chain m_q = Lambda_QCD * C * alpha_s^n_q")
    print(f"    does NOT close (m_t, m_b, m_c) at 5% with any single retained-")
    print(f"    structural C and integer or simple-rational n_q.")
    print(f"  - The route is FORECLOSED, complementing Probe X (absolute) and")
    print(f"    Probe Y (ratio) negative results.")
    ...
    print(f"  - The Probe-Y EW-chain m_b/m_c ~ alpha_LM^(-1/2) coincidence does")
