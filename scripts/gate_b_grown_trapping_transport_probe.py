#!/usr/bin/env python3
"""Gate B grown trapping-transport probe.

This is a narrow transport probe on the retained grown-family row.

Question:
  Can a small trap coupling on the retained Gate B grown row reduce detector
  escape while exactly recovering the retained grown baseline at eta = 0?

Scope:
  - retained grown geometry row only: drift=0.2, restore=0.7
  - one static source field on the same row
  - one trap slab near the middle of the transport path
  - one bounded observable: escape(eta) = P_det(eta) / P_det(0)
  - exact eta = 0 reduction check

The probe is intentionally narrow. It does not claim a horizon theory or a
general bidirectional transport law. It only asks whether a minimal trap
coupling survives skeptical reduction checks on the retained grown family.
"""
...
def main() -> None:
    t0 = time.time()
    print("=" * 88)
    print("GATE B GROWN TRAPPING TRANSPORT PROBE")
    print("  retained grown row only: drift=0.2, restore=0.7")
    print("  observable: escape(eta) = P_det(eta) / P_det(0)")
    ...
    for seed in SEEDS:
        ...
        rows.append(SeedResult(seed=seed, p0=p0, escape=escape))

    print("eta=0 exact reduction: escape=1.000 by construction on the retained grown baseline")
    print()
    ...
    for eta in TRAP_ETAS[1:]:
        ...
        esc_mean = _mean(esc)
        trap_mean = 1.0 - esc_mean
        print(f"  eta={eta:>4.2f}  escape={esc_mean:6.3f}  trapped={trap_mean:6.3f}")

    print()
    print("SAFE READ")
    print("  eta=0 reproduces the retained grown baseline exactly by construction.")
    print("  The only promoted observable is the detector escape ratio.")
    print("  If escape falls with eta, the grown row carries a real trap-sensitive")
    print("  transport channel; if not, this trap window is too weak for closure.")
    print()
