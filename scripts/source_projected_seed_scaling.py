#!/usr/bin/env python3
"""Seed-scaling study for the source-projected modular seam.

Question
--------
Using the same code path and the same nominal modular cell, how do the
source-projected b and M exponents soften as we increase seed count?

This script keeps the measurement path identical to the tiebreaker and only
changes the number of seeds. It reports whether the bounded claim
(negative b, positive M) survives at each seed count.

PStack experiment: source-projected-seed-scaling
"""
...
def _paired_delta(
    ...
    field_with = field_source_projected(
        positions,
        adj,
        mass_nodes,
    ...
    for seed in range(seed_count):
        ...
        for b in TARGET_BS:
            ...
            if len(mass_nodes) != MASS_COUNT_FIXED:
                continue
            delta = _paired_delta(positions, adj, src, det_list, mass_nodes)
...
def main() -> None:
    print("=" * 112)
    print("SOURCE-PROJECTED SEED-SCALING STUDY")
    print("  family: modular gap=3.0, npl=40")
    print("  code path: identical across seed counts; only seed count changes")
    ...
    print("SIDE-BY-SIDE")
    print(
        f"{'seeds':>5s}  {'acc':>3s}  {'b alpha':>14s}  {'b R^2':>8s}  "
        f"{'M alpha':>14s}  {'M R^2':>8s}  verdict"
    )
    print(f"{'-' * 112}")
    ...
    print("  Read the seed ladder as sample-size softening on one shared code path.")
    print("  Bounded means b_alpha < 0 and M_alpha > 0 survive the full sweep at that seed count.")
    print("  If the sign flips by 64 seeds, the seam is seed-fragile near the best cell.")
    print("=" * 112)
