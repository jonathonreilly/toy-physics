#!/usr/bin/env python3
"""
G1 Physicist-E observable-bank-extension attempt.
...
----------------------------------------------
    ...
    exhausted.  The G1 selector law cannot be closed by extending the
...
------------------
    ...
    Concrete observable-by-observable verdicts (below):
      ...
      O8. eta / eta_obs on the PMNS-assisted route
            Already handled by G1 Physics-Validation: the PMNS-assisted
...
------------------------------
    ...
    All machinery is already retained on claude/g1-complete.  No new
...
# ---------------------------------------------------------------------------
# Candidate point table (five candidates, matching G1 parity-mixing ledger)
# ---------------------------------------------------------------------------
...
def candidates() -> list[tuple[str, float, float, float, str]]:
    ...
    return [
        ...
        ("B",  0.613372,  0.964443, 1.552431, "Path C-a det(H) interior crit"),
        ("C",  0.385132,  1.267881, 0.365112, "Path C-b Tr(H^2) boundary min"),
        ("D",  0.0,       0.799987, 1.000000, "Path C-c K_12 char-match (curve sample)"),
...
def part1_observable_survey() -> None:
    ...
    print()
    print("  An observable closes G1 iff all four hold.")
    ...
    survey = [
        ...
        ("O8  eta/eta_obs on PMNS-assisted route",
         ...
         "[G1_PHYSICS_VALIDATION_ETA_AT_CANDIDATES_NOTE]"),
    ]
...
# candidate (including Physicist-E's new parity-mixing F1-min point).
# ---------------------------------------------------------------------------
...
def part5_verdict() -> None:
    print("\n" + "=" * 88)
    print("PART 5: VERDICT  (CASE 3 + CASE 4; G1 REMAINS OPEN)")
    print("=" * 88)
    print()
    ...
    print()
    print("  Therefore G1 is NOT closed by observable-bank extension on the")
    ...
    print()
    print("  None of P1/P2/P3 is supplied by this runner.  Physicist-E's")
    print("  role is to DOCUMENT the observable bank exhaustion and thereby")
    print("  force the next G1 attempt onto one of these three explicit")
    print("  follow-up lanes.")
    check(
        "Observable-bank-extension verdict is OBSTRUCTION + NEW-GAP (G1 open)",
        True,
        "see narrower-gap statement in G1_PHYSICIST_E_OBSERVABLE_CLOSURE_THEOREM_NOTE",
    )
...
def main() -> int:
    ...
    print("Branch:  claude/g1-physicist-e  (off claude/g1-complete)")
