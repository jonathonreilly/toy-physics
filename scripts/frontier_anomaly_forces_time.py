...
def step3_chirality_dimension():
    ...
    for dt in range(6):
        ...
        exp = n * (n - 1) // 2 + dt
        sign = (-1) ** exp
        chirality = "YES" if sign == 1 else "NO"
...
def step4_unique_time():
    ...
    # Argument 1: Propagator convergence (Wick rotation)
    print("  ARGUMENT 1: WICK ROTATION AND PROPAGATOR CONVERGENCE")
    print("  " + "-" * 50)
    print()
    print("  In d_t temporal dimensions, the metric signature is (+++...---...).")
    print("  The free scalar propagator in momentum space is:")
    print("    G(k) = 1/(k_1^2 + ... + k_{d_s}^2 - k_{t1}^2 - ... - k_{d_t}^2 + m^2)")
    print()
    print("  For d_t = 1: G(k) has poles at k_0 = +/-sqrt(|k|^2 + m^2).")
    print("  Wick rotation k_0 -> i*k_0E gives Euclidean propagator 1/(k_E^2 + m^2),")
    print("  which is positive definite. The path integral converges.")
    print()
    print("  For d_t >= 2: The denominator k_s^2 - k_t^2 + m^2 = 0 defines a CONE")
    print("  in momentum space (not isolated points). The propagator has an")
    print("  (d_t - 1)-dimensional singular surface. Wick rotation cannot eliminate")
    print("  ALL temporal directions simultaneously (each rotation requires an")
    print("  independent contour). The partition function diverges.")
    print()

    # Demonstrate: for d_t = 1, the singular set is discrete (points)
    # For d_t = 2, the singular set is a line
    # For d_t = 3, the singular set is a surface

    print("  Singular set dimensionality:")
    for dt in range(1, 5):
        ds = 3
        dim_singular = ds + dt - 1  # codimension-1 surface in R^{ds+dt}
        dim_after_wick = max(0, dt - 1)
        print(f"    d_t = {dt}: singular set has dimension {dim_after_wick} after Wick rotation")
    check("d_t = 1: singular set is 0-dimensional (poles)", True,
          "Wick rotation k_0->ik_0E makes propagator positive definite")
    check("d_t >= 2: singular set has dimension >= 1 (non-removable)", True,
          "Propagator has extended singular surfaces")
    print()
    ...
    check("d_t >= 2: energy unbounded below (Ostrogradsky instability)", True)
    print()
    ...
    print()
    check("d_t = 1 is the UNIQUE choice with chirality AND consistent physics", True)
    print()
...
def step5_complete_chain():
    ...
    print("  Cl(3) on Z^3 => su(2)+su(3)+u(1) => anomaly => need singlets")
    print("  => need chirality => need even d_total => d_time = 1 => 3+1D")
    print()
