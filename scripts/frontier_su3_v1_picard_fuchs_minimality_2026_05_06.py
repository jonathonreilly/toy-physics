...
row by itself:
  ...
  (B) MINIMALITY CERTIFICATE (lower-order exclusion)
      ...
      If no non-trivial null vector exists for any of these (r, d), no
      checked-window order ≤ 2 ODE with polynomial coefficients of degree
      ≤ D_MAX annihilates J(β).

  (C) UNIQUENESS AT (r=3, d=2)
      Verify the known ODE is (up to scalar) the unique element of the kernel
      at (r=3, d=2). This certifies uniqueness in the checked ansatz.

  (D) BESSEL-IDENTITY CONSISTENCY
      For each k ∈ {-2, ..., +2}, verify the recurrence
      6(N+1)(N+4)(N+5) a_{N+1} = N(N+1) a_N + 2(2N+3) a_{N-1} + a_{N-2}
      that derives from the ODE holds for the checked Taylor coefficients,
      where the a_n are computed independently from the Bessel-determinant
      sum (Bars 1980).

The all-order interpretation depends on the external holonomic rank and
creative-telescoping inputs. The runner's direct authority is the finite,
exact certificate surface below.
...
def main():
    ...
    output = {
        ...
        "summary": {
            "pass": pass_count,
            "fail": fail_count,
            "bounded_certificate_passed": fail_count == 0,
