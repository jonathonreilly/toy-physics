...
print("=" * 88)
print("  Substep-4 AC_λ Separate-Closure (Sharpened, Partial)")
...
# =============================================================================
# Section 3 — AC_λ.label: labeling-convention bridge under retained meta
#
# Characterize AC_λ.label (the kind-of-label question) as a
# labeling-convention bridge under the retained meta companion notes
...
# species-kind? Under retained meta:
...
check(
    "AC_φ remains bounded structural no-go candidate within A_min",
    True,
...
#                              + retained-meta companion-note conditional
...
inherited_dependencies = {
    ...
    "C3pres retained meta source-note (PR #728, audit-pending)",
    "ConvU retained meta source-note (PR #729, audit-pending)",
    "BAErename retained meta source-note (PR #790, audit-pending)",
}

check(
    "Atom count reduction: 3 atoms → 2 atoms",
    n_atoms_before == 3 and n_atoms_after == 2,
    f"before={atoms_before}, after={atoms_after}",
)

check(
    "AC_λ removed from residual atomic stack",
    "AC_λ" in atoms_before and "AC_λ" not in atoms_after,
    "subsumed by Kawamoto-Smit upstream + retained-meta conditional",
)
...
check(
    ...
    "1 upstream + 3 retained-meta companion notes",
)
...
check(
    "No new axioms added",
    True,
    "only A1+A2 + cited upstream theorem authorities + retained meta",
)
...
print("  AC_λ.label : labeling-convention bridge (retained meta)")
print()
print("  Conjunction AC_λ closes as SHARPENED BOUNDED sub-result of")
print("  substep-4 with NO admitted observation of its own beyond")
print("  inherited Kawamoto-Smit upstream.")
print()
print("  Substep-4 admission count: 3 atoms → 2 atoms (PARTIAL RATCHET)")
