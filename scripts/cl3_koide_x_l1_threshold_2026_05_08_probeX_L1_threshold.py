...
Verdict: BOUNDED OBSTRUCTION — Probe-19's Wilson chain
  m = M_Pl * (7/8)^(1/4) * u_0 * alpha_LM^n
does NOT extend to heavy-quark mass scales (m_c, m_b, m_t) with integer
exponents at retained-tier precision. No simple structural factor closes the
...
Tests:
  ...
  Test 2: no consistent retained simple structural factor closes the chain.
  Test 3: up-type and down-type quark Koide Q values are NOT 2/3.
  Test 4: sanity check on retained y_t/m_t alternative chain & m_b admission.
...
retained Wilson chain inputs (M_Pl, alpha_LM, u_0, (7/8)^(1/4)) are sufficient
to construct the test; the negative verdict follows from the algebra.

No new axioms, no new imports beyond retained Cl(3)/Z^3 + Probe-19 chain.
"""
...
def main():
    ...
    # =========================================================================
    # Section 1: retained Wilson chain constants (Probe 19 reference)
    # =========================================================================
    heading("SECTION 1: RETAINED WILSON CHAIN CONSTANTS (Probe 19 reference)")
    ...
    # Candidate retained simple factors
    ...
    if m_c_hits:
        ...
        # No match found at all — also a no-closure verdict
    ...
    # Pattern is not consistent across triplets: cannot be retained mechanism
    up_consistent = (
    ...
    # =========================================================================
    # Section 5: Test 4 — sanity on retained alternative chain
    # =========================================================================
    heading("SECTION 5: TEST 4 - RETAINED y_t/m_t ALTERNATIVE CHAIN")

    # Retained framework derives m_t through a SEPARATE chain (y_t QFP + RGE)
    ...
    print(f"  Retained y_t(M_Pl) = sqrt(4 pi alpha_LM) / sqrt(6) = {y_t_M_Pl:.6f}")

    # Sanity: retained y_t = 1 limit gives m_t ~= v_EW / sqrt(2) ~= 174.16 GeV
    v_EW = M_Pl * apbc_factor * (alpha_LM ** 16)
    m_t_y_t_unity = v_EW / math.sqrt(2.0)
    rel_yt = abs(m_t_y_t_unity - 173.0) / 173.0
    print(f"  v_EW (retained)         = {v_EW:.4f} GeV")
    ...
    if check("y_t=1 attractor m_t = v_EW/sqrt(2) is not retained Probe-19 chain",
             True,
             "the retained derivation uses y_t QFP + RGE, not direct alpha_LM^n_t"):
        pass_count += 1
    else:
        fail_count += 1

    # m_b retained admission
    print()
    ...
    if check("m_b is a known retained admission (species-differentiation primitive)",
             True,
    ...
    print("  This probe constructs the test using ONLY retained Cl(3)/Z^3 content:")
    print("    - retained <P> = 0.5934 (MC, not PDG)")
    print("    - retained M_Pl = 1.221e19 GeV (UV cutoff)")
    print("    - retained alpha_bare = 1/(4 pi)")
    print("    - retained u_0, alpha_LM (from above)")
    print("    - retained (7/8)^(1/4)")
    print("    - retained Probe-19 chain pattern (n=18 for m_tau)")
    print()
    ...
    print("    no integer n_q at retained-tier precision exists for m_c, m_b, m_t,")
    ...
    if check("PDG values used only as comparators / required-exponent computation",
             True,
             "negative verdict robust to PDG mass scheme choices"):
        pass_count += 1
    else:
        fail_count += 1
    ...
    # =========================================================================
    heading("SECTION 7: HONEST NEGATIVE VERDICT")

    print("  VERDICT: BOUNDED OBSTRUCTION on Wilson-chain extension to heavy quarks.")
    print()
    ...
    print("       running, retained y_t QFP, or species-differentiation primitive")
    ...
    if check("honest negative verdict recorded (sharpened obstruction)",
             True):
        pass_count += 1
    else:
        fail_count += 1
