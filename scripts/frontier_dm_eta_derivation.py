...
def part4_eta_derivation(vT, vT_err, gamma_over_t4, H_ew, sph_over_H,
    ...
    def compute_eta_for_vt(vt_val: float):
        nbs_val = compute_nbs(vt_val)
        eta_prod_val = s_over_ngamma * nbs_val
        exp_val = np.exp(-esph_coeff * vt_val)
        gamma_broken_val = sph_over_H * exp_val
        survival_val = np.exp(-gamma_broken_val) if gamma_broken_val < 500 else 0.0
        eta_final_val = eta_prod_val * survival_val
        return nbs_val, eta_prod_val, exp_val, gamma_broken_val, survival_val, eta_final_val
    ...
    # Production
    nbs_prod, eta_prod, exp_esph, gamma_broken_over_H, survival, eta_final = compute_eta_for_vt(vT)
    ...
    for vt in vt_table:
        nbs_i, eta_prod_i, exp_i, gbh_i, surv_i, eta_i = compute_eta_for_vt(vt)
    ...
    for i, vt in enumerate(vt_scan):
        _, _, _, _, _, eta_scan[i] = compute_eta_for_vt(vt)
    ...
    for label, vt_val in [('central', vT), ('low', vT - vT_err), ('high', vT + vT_err)]:
        _, _, _, _, _, eta_at[label] = compute_eta_for_vt(vt_val)
...
def main():
    ...
    eta_final, eta_at, crossings, vt_opt, eta_max = part4_eta_derivation(
        vT, vT_err, gamma_over_t4, H_ew, sph_over_H, esph_coeff, g_star, cp_params
    )
