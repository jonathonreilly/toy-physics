...
def main() -> None:
    ...
    print(f"peer_branch_rows={sum(1 for row in rows if getattr(row, 'subtype') == 'peer_motif')}")
    print()
    print(render_rules("Candidate peer-branch coordinate-band rules", peer_rules))
    print()
    print(render_rules("Candidate non-peer-core coordinate-band rules", non_peer_rules))
    print()
    print("Rows")
    print("----")
    for row in rows:
