...
This is **PR 1 of a 5-PR engine roadmap** supporting a future
gauge-scalar bridge review by supplying a primitive for a later explicit
boundary character measure rho_(p,q)(6) computation on the unmarked
spatial Wilson environment for the L_s=2 APBC spatial cube. See
...
for all triples of SU(3) irreps lambda = (a, b), mu = (p, q),
nu = (x, y) in the finite dominant-weight box
(a, b, p, q, x, y) <= NMAX.
...
def vandermonde_squared(phi1: float, phi2: float) -> float:
    """|Delta(z)|^2 = 64 * product_{i<j} sin^2((phi_i - phi_j) / 2) for SU(3),
...
def driver(nmax: int = NMAX_DEFAULT, n_grid: int = N_GRID_DEFAULT) -> int:
    ...
    print("=" * 78)
    print(f"SUMMARY: BOUNDED PASS={pass_count} FAIL={fail_count}")
    print("=" * 78)
    print()
    print("Engine status: PR 1 deliverable — bounded fusion multiplicities N^ν_(λ,μ)")
    print("computed for all triples within the finite dominant-weight box.")
    print()
    ...
    print("  PR 5: L_s=2 APBC cube geometry + ρ_(p,q)(6) computation + P(6) bridge-gate check")
    print()
