#!/usr/bin/env python3
"""
Two-Representation Ward Re-Read on g_bare
=========================================

Path 2 support candidate: verify the retained tree-level Ward-identity
theorem's two representations (Rep A: OGE; Rep B: H_unit matrix
element) as a re-readable algebraic system that supports the absolute
bare pair (y_t_bare, g_bare) = (1/sqrt(6), 1), not merely the ratio
1/sqrt(6).
...
No Monte Carlo, no running, no plaquette evaluation. This is a
structural / algebraic support candidate from retained Ward-theorem
content.
"""
...
log()
log("  Two-representation Ward support candidate on g_bare:")
...
log()
log("  Support candidate: under the Rep A = Rep B re-read,")
log("    (y_t_bare, g_bare) = (1/sqrt(6), 1) is supported absolutely")
log("    by retained tree-level Ward-theorem content (no new axioms).")
log()
