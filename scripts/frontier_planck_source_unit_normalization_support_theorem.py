...
def main() -> int:
    ...
    if passed == total:
        print(
            "Verdict: positive support-theorem closure on the retained "
            "coframe/CAR carrier surface. The Target 3 coframe-response "
            "derivation supplies the carrier premise. The retained 1/(4*pi) "
            "is the bare Green-kernel coefficient; the physical source scale "
            "is fixed by c_cell=lambda/4, hence lambda=1 and "
