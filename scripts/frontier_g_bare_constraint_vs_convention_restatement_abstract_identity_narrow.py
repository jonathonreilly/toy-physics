...
abstract symbolic variables (g, β, K) ∈ Q_{>0}^3 connected by
...
The theorem characterizes, by polynomial-algebra dimension counting alone:
  ...
  (T1) Under one-axis admission (S_K) — K fixed, (g, β) free — the
       solution set is the 1-parameter rational curve { (g, K/g²) : g > 0 }.
  ...
  (T4) Under zero-axis admission, the solution set is the 2-parameter
       rational surface { (g, β, β·g²) : g, β > 0 }.

This is class-A pure polynomial algebra over Q_{>0}. No Cl(3), no Wilson
action, no SU(N_c) gauge group, no canonical Gell-Mann normalization, no
physical bare-coupling identification, no PDG/literature/fitted/admitted
unit convention is consumed.
"""
...
# ----------------------------------------------------------------------------
section("(T1) One-axis admission (S_K): solution curve β = K / g² over Q_{>0}")
# ----------------------------------------------------------------------------
...
# Check at rational (K, β) where K/β is a perfect square in Q_{>0}.
...
for K_val, beta_val in rational_grid_T2:
    ...
    # Also check that g is uniquely positive (no ± ambiguity in Q_{>0})
...
# ----------------------------------------------------------------------------
# Parameterization (g, β) ∈ Q_{>0}^2 ↦ (g, β, β·g²) ∈ Q_{>0}^3 surjects
...
print("""
  ...
  HYPOTHESIS:
    Let (g, β, K) ∈ Q_{>0}^3 be abstract symbolic positive rationals
  ...
  Audit-lane class:
    (A) — pure polynomial algebra over Q_{>0}. No Cl(3) / Wilson /
    SU(N_c) / Gell-Mann / physical bare-coupling identification.
