#!/usr/bin/env python3
"""Probe oriented support-pair and candidate-anchored support context in center-spine bucket `00`."""
...
def build_support_context(
    ...
    support_to_adj_pocket: dict[tuple[int, int], set[tuple[int, int]]] = {}
    support_to_adj_deep: dict[tuple[int, int], set[tuple[int, int]]] = {}
    for support in support_nodes:
        ...
        support_to_adj_pocket[support] = adj_pocket
        support_to_adj_deep[support] = adj_deep
        for cell in adj_pocket | adj_deep:
            candidate_to_supports.setdefault(cell, set()).add(support)

    total_edges = 0
    bridge_bridge_edges = 0
    bridge_other_edges = 0
    same_role_edges = 0
    candidate_shared_edges = 0
    oriented_events: set[str] = set()
    ...
    for left in support_nodes:
        for right in graph_neighbors(left, support_set, wrap_y=False):
            if right not in support_set or right <= left:
                continue
            total_edges += 1
            role_left = roles[left]
            role_right = roles[right]
            roles_sorted = tuple(sorted((role_left, role_right)))
            if role_left == "bridge" and role_right == "bridge":
                bridge_bridge_edges += 1
            if "bridge" in roles_sorted and roles_sorted[0] != roles_sorted[1]:
                bridge_other_edges += 1
            if roles_sorted[0] == roles_sorted[1]:
                same_role_edges += 1

            shared_candidates = (support_to_adj_pocket[left] | support_to_adj_deep[left]) & (
                support_to_adj_pocket[right] | support_to_adj_deep[right]
            )
            if shared_candidates:
                candidate_shared_edges += 1
            ...
            adx = anchor[0] - min_x
            ady = anchor[1] - min_y
            shared_pocket = len(support_to_adj_pocket[left] & support_to_adj_pocket[right])
            shared_deep = len(support_to_adj_deep[left] & support_to_adj_deep[right])
            oriented_events.add(
                "spair:"
                + f"{roles_sorted[0]}:{roles_sorted[1]}:"
                + f"d{dx},{dy}:"
                + f"a{adx},{ady}:"
                + f"p{shared_pocket}:d{shared_deep}"
            )

    candidate_cells = sorted(candidate_to_supports)
    candidate_wedge_count = 0
    candidate_bridge_touch_count = 0
    candidate_degree_total = 0
    candidate_edge_total = 0

    for cell in candidate_cells:
        support_neighbors = sorted(candidate_to_supports[cell])
        candidate_degree_total += len(support_neighbors)
        if any(roles[support] == "bridge" for support in support_neighbors):
            candidate_bridge_touch_count += 1

        local_edges = 0
        local_set = set(support_neighbors)
        for support in support_neighbors:
            for neighbor in graph_neighbors(support, local_set, wrap_y=False):
                if neighbor in local_set and neighbor > support:
                    local_edges += 1
        candidate_edge_total += local_edges
        if len(support_neighbors) >= 2 and local_edges > 0:
            candidate_wedge_count += 1

    numeric = {
        "support_edge_density": _fraction(total_edges, len(support_nodes)),
        "bridge_bridge_edge_fraction": _fraction(bridge_bridge_edges, total_edges),
        "bridge_other_edge_fraction": _fraction(bridge_other_edges, total_edges),
        "same_role_edge_fraction": _fraction(same_role_edges, total_edges),
        "candidate_shared_edge_fraction": _fraction(candidate_shared_edges, total_edges),
        "candidate_wedge_fraction": _fraction(candidate_wedge_count, len(candidate_cells)),
        "candidate_bridge_touch_fraction": _fraction(candidate_bridge_touch_count, len(candidate_cells)),
        "candidate_support_degree_mean": _fraction(candidate_degree_total, len(candidate_cells)),
        "candidate_support_edge_mean": _fraction(candidate_edge_total, len(candidate_cells)),
    }
    return oriented_events, numeric
...
def build_rows(
    ...
    *,
    ...
    for source_name in sorted(subtype_by_source):
        ...
        events, numeric = build_support_context(set(row.nodes))
    ...
    row_cls = make_dataclass(
        "SupportPairContextRow",
        [
            ("source_name", str),
            ("subtype", str),
            ("support_edge_density", float),
            ("bridge_bridge_edge_fraction", float),
            ("bridge_other_edge_fraction", float),
            ("same_role_edge_fraction", float),
            ("candidate_shared_edge_fraction", float),
            ("candidate_wedge_fraction", float),
            ("candidate_bridge_touch_fraction", float),
            ("candidate_support_degree_mean", float),
            ("candidate_support_edge_mean", float),
            ("oriented_event_present_count", float),
        ]
    ...
    for source_name, subtype, numeric, events in raw:
        row_values = {
            ...
            "subtype": subtype,
            **numeric,
            "oriented_event_present_count": float(sum(event in events for event in selected)),
        }
...
def render_rows(rows: list[object], selected_events: tuple[str, ...]) -> str:
    shown = [
        ...
        "bridge_bridge_edge_fraction",
        "bridge_other_edge_fraction",
        "same_role_edge_fraction",
        "candidate_shared_edge_fraction",
        "candidate_wedge_fraction",
        "candidate_support_degree_mean",
        "candidate_support_edge_mean",
        "oriented_event_present_count",
    ]
    shown += [_sanitize(event) for event in selected_events[:4]]

    lines = [
        "Bucket 00 oriented support-pair context rows",
        "===========================================",
        " | ".join(shown),
...
def render_neighbor_deltas(
    ...
    *,
    ...
    lines = [
        "Nearest-neighbor oriented support-pair deltas (add4 vs add1)",
        "=======================================================",
    ]

    for add4_row in add4_rows:
        nearest = min(add1_rows, key=lambda row: zdistance(add4_row, row, stats))
        add4_events, _numeric = build_support_context(set(frontier_rows[add4_row.source_name].nodes))
        add1_events, _numeric = build_support_context(set(frontier_rows[nearest.source_name].nodes))
...
def main() -> None:
    ...
    print(f"center-spine bucket00 support-pair context started {started}", flush=True)
    ...
    print()
    print("Center-Spine Bucket 00 Oriented Support-Pair Context")
    print("====================================================")
    ...
    print()
    print(render_rules(f"Best support-pair context rules for {args.right_subtype}", add4_rules))
    print()
    print(render_rules(f"Best support-pair context rules for {args.left_subtype}", add1_rules))
    print()
    print(
    ...
    print()
    print(
        "center-spine bucket00 support-pair context completed "
