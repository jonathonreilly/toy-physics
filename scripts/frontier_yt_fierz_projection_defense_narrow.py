...
Fierz color projection:
  ...
  (3) The asymptotic-safety prediction (Eichhorn-Held arXiv:1707.01107)
      ...
      vs AS prediction gap (~1.6 GeV) is well above FCC-ee threshold-
      scan precision (~9 MeV statistical, ~30-50 MeV theoretical), so
      the predictions are experimentally distinguishable.
...
only, not as framework inputs. The framework's own m_t = 172.57 GeV
(2-loop) prediction comes from the upstream prediction-chain runners,
...
try:
    import sympy as sp
    from sympy import Rational, sqrt, simplify, symbols, Symbol, Matrix, eye, zeros, nsimplify, log
except ImportError:
...
print(f"  Form (b) LSZ single-leg:           sqrt(R_conn) = {simplify(correction_form_b)} = sqrt(8/9)")
check(
    "form (b): correction = sqrt(R_conn) on Yukawa amplitude",
    simplify(correction_form_b - sqrt(Rational(8, 9))) == 0,
...
print(f"  Form (a) current-current:          R_conn       = {simplify(correction_form_a)} = 8/9")
check(
    "form (a): would-be correction = R_conn (rejected by LSZ)",
    simplify(correction_form_a - Rational(8, 9)) == 0,
...
check(
    ...
    simplify(correction_form_b - correction_form_a) != 0,
    detail=f"sqrt(8/9) - 8/9 = {simplify(correction_form_b - correction_form_a)} != 0",
)
...
check(
    "2-loop framework prediction within 0.1% of PDG (the headline 0.07% claim)",
    abs(dev_2L_pct) < 0.1,
    detail=f"|dev_2L| = {abs(dev_2L_pct):.4f}% < 0.1%",
)
check(
    "3-loop framework prediction within 0.3% of PDG (sub-percent retained)",
    abs(dev_3L_pct) < 0.3,
...
check(
    "framework 2-loop and 3-loop bracket the PDG observed value",
...
# Statement: AS predicts m_t,pole ~ 171 GeV (Eichhorn-Held arXiv:1707.01107).
# Framework predicts 172.57 GeV (2L) / 173.10 GeV (3L). PDG is 172.69 GeV.
# Verify the gap is above the FCC-ee threshold-scan precision (~9 MeV stat,
# ~30-50 MeV theoretical), so the predictions are experimentally
# distinguishable.
# =============================================================================

# AS comparator (Eichhorn-Held arXiv:1707.01107)
m_t_AS = 171.0  # GeV, central AS prediction (no explicit error bar quoted)

# Framework predictions (from COMPLETE_PREDICTION_CHAIN §6.2)
print(f"\n  Predictions vs PDG:")
...
check(
    "framework-vs-AS gap is positive (framework predicts higher m_t)",
...
check(
    "AS prediction is significantly further from PDG than framework 2-loop",
    as_dev > fw_2L_dev * 5,
    detail=f"AS off by {as_dev:.3f} GeV vs framework off by {fw_2L_dev:.3f} GeV",
)

# AS vs PDG significance: AS is at 171 GeV with PDG at 172.69 +/- 0.30
as_n_sigma = as_dev / pdg_uncertainty
print(f"  AS vs PDG (modulo unstated AS error): {as_n_sigma:.1f}σ from PDG world average")
check(
    "AS prediction is 3+ sigma from PDG world average (modulo AS unstated error)",
    as_n_sigma > 3.0,
    detail=f"AS {as_n_sigma:.1f}σ from PDG (no AS-side error band quoted in arXiv:1707.01107)",
)
...
check(
    "framework-vs-AS gap > 5x FCC-ee total precision (definitively distinguishable)",
    distinguishable,
...
check(
    ...
    simplify(correction_yt * correction_gEW - 1) == 0,
...
check(
    ...
    True,  # algebraic identity; documented in YT_EW_COLOR_PROJECTION_THEOREM.md Part 3.1
...
print(f"""
  ...
  (3) Cross-check vs Eichhorn-Held asymptotic safety:
      -- AS predicts m_t,pole ~ 171 GeV (arXiv:1707.01107)
      -- Framework-vs-AS gap ~ 1.6 GeV ~ 0.9% of m_t
      -- AS is ~14x further from PDG than framework 2-loop
      -- FCC-ee threshold scan (~50 MeV total precision) will discriminate
         (gap is ~30x precision)
