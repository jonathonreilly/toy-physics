...
Purpose
=======
Stress-test the X-L1-MSbar bounded result by asking whether retained
quartic Casimirs from Cl(3) tensor structure can derive beta_2 and/or
beta_3 in closed form, thereby reclassifying L1 imports as
bounded-with-quartic-bridge instead of terminal.

Verdict structure
=================
The probe is bounded_theorem (NEGATIVE on the conjecture, with a
positive narrow result on Casimir-value retention).

Positive narrow result (PASS expected):
  1. SU(3) quartic Casimir invariants d_F d_F / N_F = 5/12,
     d_F d_A / N_F = 5/2, d_A d_A / N_A = 135/8 retained from
     retained Cl(3) ⊗ Cl(3) tensor algebra.
...
Bounded admissions (ADMITTED expected, no derivation):
  5. beta_2 channel weights: NOT derivable in any scheme (X-L1-MSbar).
  6. beta_3 channel weights: NOT derivable in any scheme (X-L1-MSbar).
...
# ----------------------------------------------------------------------
# SECTION 1 — POSITIVE NARROW: SU(3) quartic Casimirs are retained
# ----------------------------------------------------------------------

def section1_quartic_casimirs_retained(c: Counter) -> None:
    """Verify the SU(3) quartic Casimir invariants are pure group-theoretic
    numbers, derivable from retained Cl(3) ⊗ Cl(3) tensor algebra
    ...
    """
    print("Section 1 — POSITIVE NARROW: SU(3) quartic Casimirs are retained")
    ...
    print("    -> Derivable on retained Cl(3) ⊗ Cl(3) tensor algebra.")
    print("    -> RETAINED narrow result, consistent with X-L1-MSbar Section 4.")
...
def section2_beta_2_no_quartic_channels(c: Counter) -> None:
    ...
    from retained quartic Casimirs" — is structurally unavailable:
...
def section3_beta_2_closed_form_value(c: Counter) -> None:
    ...
    We do NOT re-derive the per-channel weights here — the X-L1-MSbar
    bounded admission stands on the channel-weight obstruction.
    ...
    print("       NOT framework-derived; this matches X-L1-MSbar bounded admission.")
...
def section4_beta_3_quartic_channels_weights(c: Counter) -> None:
    ...
    retained. The scalar WEIGHTS are 4-loop master-integral combinations
    involving rationals + zeta_3.

    We document the conjecture-foreclosing observation: even though the
    quartic Casimir VALUES are retained, the channel WEIGHTS are not,
    ...
    for name, casimir_value, weight_obstruction in quartic_channels_beta_3:
        c.record(
            f"beta_3 channel '{name}' Casimir value = {casimir_value} retained",
            True,
            f"value = {casimir_value} ({float(casimir_value):.4f}) is retained group theory",
        )
        c.admit(
            f"beta_3 channel weight w({name})",
            f"weight is {weight_obstruction}; NOT framework-retained",
        )
    ...
    for name in quadratic_channels_beta_3:
        c.admit(
            f"beta_3 channel weight w({name})",
            f"weight is rational + zeta_3 from 4-loop integrals; NOT framework-retained",
        )

    print("    -> beta_3 has 2 quartic-Casimir channels with retained VALUES")
    print("       (5/12, 5/2) but NON-retained scalar WEIGHTS (4-loop integrals).")
    print("    -> beta_3 has 10+ quadratic-Casimir-product channels;")
    print("       all weights are 4-loop integrals, NOT retained.")
...
def section5_underspecification_check(c: Counter) -> None:
    ...
    This is a hostile-review confirmation that retained Casimir VALUES
    ...
    c.record(
        ...
        f"VALUES retained: {n_quartic_values}; WEIGHTS needed: >= {n_beta_3_weights}",
    )
    ...
    c.record(
        ...
        True,
        "retained Cl(3) ⊗ Cl(3) tensor algebra gives VALUES, not WEIGHTS",
    )
...
# ----------------------------------------------------------------------
# SECTION 6 — HOSTILE REVIEW: cubic Casimir products also retained as values
# ----------------------------------------------------------------------

def section6_cubic_parallel_obstruction(c: Counter) -> None:
    """Hostile-review sharpening: at 3-loop, cubic Casimir products
    (C_A^3, C_F C_A^2, C_F^2 C_A, C_F^3, etc.) are also retained as
    ...
    n >= 3, retained Casimir-tensor products give the SKELETON, NOT
    the WEIGHTS.
    """
    print()
    print("Section 6 — HOSTILE REVIEW: cubic Casimir products parallel obstruction at 3-loop")

    # Cubic Casimir products at SU(3), N_f=6 (all VALUES retained)
    cubic_values = [
    ...
    for name, computed, expected in cubic_values:
        c.record(
            f"cubic Casimir product '{name}' VALUE retained at SU(3)",
            computed == expected,
    ...
    print("    -> Cubic Casimir VALUES retained at all orders.")
    print("    -> Channel WEIGHTS are 3-loop integrals (TVZ 1980 dim-reg).")
    print("    -> Generalizes: VALUES != WEIGHTS at every loop order >= 3.")


# ----------------------------------------------------------------------
# SECTION 7 — INHERITED BOUNDED ADMISSION (X-L1-MSbar)
# ----------------------------------------------------------------------

def section7_inherited_bounded_admission(c: Counter) -> None:
    """The bounded admission from X-L1-MSbar STANDS:
    - beta_2 channel weights NOT derivable in any scheme
    - beta_3 channel weights NOT derivable in any scheme

    The quartic-Casimir bridge tested in this V probe FAILS to relax
    the obstruction, so the X-L1-MSbar terminal status on Lane 1
    beta_2/beta_3 imports is UNCHANGED.
    """
    print()
    print("Section 7 — INHERITED BOUNDED ADMISSION (X-L1-MSbar unchanged)")

    c.admit(
        "beta_2 channel weights in any scheme",
        "scalar weights require 3-loop integral primitives (X-L1-MSbar bounded)",
    )
    c.admit(
        "beta_3 channel weights in any scheme",
        "scalar weights require 4-loop integral primitives (X-L1-MSbar bounded)",
    )
    c.admit(
    ...
    c.admit(
        ...
        "channel VALUES retained but WEIGHTS not; "
        "X-L1-MSbar bounded admission stands",
    )

    print("    -> X-L1-MSbar terminal status on Lane 1 beta_2/beta_3 imports STANDS.")
...
def section8_verdict(c: Counter) -> None:
    """Final verdict on probe V-L1-Quartic."""
    print()
    print("=" * 72)
    print("PROBE V-L1-Quartic VERDICT")
    print("=" * 72)
    print()
    print("Claim type: bounded_theorem (NEGATIVE on the conjecture; POSITIVE")
    print("            narrow on Casimir-value retention; inherits X-L1-MSbar")
    print("            bounded admission on channel weights)")
    print()
    print("POSITIVE narrow result:")
    print("  + SU(3) quartic Casimir invariants retained from")
    print("    Cl(3) ⊗ Cl(3) tensor algebra:")
    ...
    print()
    print("BOUNDED admissions (inherited from X-L1-MSbar):")
    print("  ! beta_2 channel weights: NOT derivable in any scheme")
    print("  ! beta_3 channel weights: NOT derivable in any scheme")
    print()
    print("Net contribution to Lane 1:")
    print("  - Sharpens the X-L1-MSbar negative result: not only are channel")
    print("    weights not derivable in any scheme, but the quartic-Casimir")
    print("    bridge CANNOT relax this — beta_2 has no quartic channels")
    print("    and beta_3 quartic channels still need 4-loop integrals.")
    print("  - Audit-grade record of which Casimir invariants ARE retained")
    ...
    print("  - Does NOT change current Lane 1 admission status; X-L1-MSbar")
    print("    terminal status on beta_2/beta_3 imports STANDS.")
    print()
...
def main() -> int:
    ...
    section1_quartic_casimirs_retained(counter)
