...
Companion experiment to the rescaled-NN closure (PRs #957, #968, #1003, #1007,
#1054, #1055, #1056) and the alt-connectivity negative companion (#1008).
...
passes, we record the structural diagnosis -- analogous to #1008's
...
Exit code: 0 always.  This runner is an exploratory bridge-method probe,
not an audit gate; whether the Gate B family admits the operator-Cauchy
bridge is itself the deliverable.
"""
...
def main() -> int:
    ...
    if res_cauchy_ok or ens_cauchy_ok:
        ...
        print("  mirrors the alt-connectivity negative companion (#1008)")
        print("  and confirms that the operator-Cauchy bridge method is")
        print("  a feature of the rescaled-NN lane (PRs #957-#1056), not")
        print("  of every CL3 harness.")
