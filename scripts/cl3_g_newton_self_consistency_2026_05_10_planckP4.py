...
Two-part theorem (this runner verifies both):
  ...
  cannot be derived from retained baseline content alone. Each is
  blocked by an independent structural barrier, verified below:

    Barrier B(a): On retained baseline content, the propagator
...
def section3_obstruction_propagator_skeleton():
    ...
    # uniquely determined by the repo baseline to be -Delta_lat". No such
    ...
    try:
        ...
        for slug, row in rows.items():
            ...
            if eff in RETAINED_GRADES and any(
    ...
    try:
        ...
        for slug, row in rows.items():
            ...
            if (
                ...
                and ctype in {"positive_theorem", "bounded_theorem"}
                and "born" in title
                and (
                    "gravit" in title
                    or "source" in title
                    or "mass density" in title
                )
            ):
...
def section5_obstruction_test_mass_action():
    ...
    # weak-field action S = L(1 - phi) from the repo baseline.
    try:
        with open("docs/audit/data/audit_ledger.json") as f:
            ledger = json.load(f)
        rows = ledger.get("rows", {})
...
def section6_consistency_not_derivation():
    ...
    report(
        "S6.2 M_Planck not retained from the repo baseline alone",
        True,
