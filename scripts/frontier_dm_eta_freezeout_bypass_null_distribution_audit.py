...
Addresses the multiple-comparisons question for the bounded candidate audit.
...
QUESTION FOR THE CANDIDATE.
  ...
  identities), the audit's match is a strong signal within this chosen
  null family. If it is in the top 50%, the audit is much less informative.
...
def main() -> None:
    ...
    check(
        "Total candidates enumerated > 100 (null-family coverage sanity check)",
        len(candidates) > 100,
        f"n = {len(candidates)}",
    )
    check(
    ...
    if rank and rank <= 0.05 * len(candidates):
        ...
        log("  proximity to the freeze-out target within this chosen null family.")
        log("  It is a low-complexity, non-generic near-match in that family, not")
        log("  just another typical single-block candidate.")
    else:
        log("  CONCLUSION: within this chosen null family the candidate is not")
        log("  strongly distinguished from the broader multiple-comparisons pool.")
    log()
