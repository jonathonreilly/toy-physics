...
Tests:
  Step 1 (retained): m_H_tree = v / (2 u_0) = 140.31 GeV  (+12.0% vs PDG)
  Step 2 (partial structural correction): m_H_partial = (v/(2u_0)) * sqrt(1 - 2 alpha_s(v))
                                          = 124.98 GeV  (-0.21% vs PDG)
  Step 3 (systematic exhaustion): no integer-exponent retained-content Wilson chain
                                  gives m_H to retained-tier (<= 0.1%) precision.
  Step 4 (sister-prediction comparison): retained sister predictions (v_EW, m_t, m_tau)
...
and Step 2 partial m_H_partial = 124.98 GeV are computed entirely from retained
framework constants (M_Pl, alpha_bare, u_0, alpha_LM, alpha_s(v), (7/8)^{1/4}).

No new axioms, no new imports. All verifications use only retained Wilson-chain
content per COMPLETE_PREDICTION_CHAIN_2026_04_15.md.
"""
...
def main():
    ...
    alpha_s_v = alpha_bare / u_0 ** 2  # CMT physical alpha_s at scale v (retained)
    ...
    # =========================================================================
    # Section 2: Step 1 — retained tree-level m_H_tree = v / (2 u_0)
    # =========================================================================
    heading("SECTION 2: STEP 1 — RETAINED TREE-LEVEL m_H_tree = v / (2 u_0)")
    ...
    # alpha_s(v) is retained (CMT physical strong coupling at scale v)
    ...
    if not (one_minus_2as > 0):
        ...
        if check("(1 - 2 alpha_s) factor is structurally suggestive but fit-selected",
                 True,
                 "alpha_s(v) is retained CMT; coefficient '2' is one-loop multiplicity guess"):
            pass_count += 1
        else:
            fail_count += 1
    ...
    # Retained Wilson-chain predictions:
    ...
    sister_predictions = [
        ("v_EW", v_EW, 246.22, "Hierarchy theorem (retained)"),
    ...
    print("  Per HIGGS_MASS_FROM_AXIOM_NOTE Step 6 (retained):")
    ...
    print("  the retained CMT physical strong coupling at scale v. It does NOT")
    ...
    print("       SELECTED BY FIT — not derived from retained content.")
    print()
    print("  (R2) RETAINED-TIER OBSTRUCTION (unchanged):")
    print("       No retained-content-only Wilson-chain expression of the form")
    ...
    print("            from retained content. The 0.21% precision is 3x to 12x")
    print("            worse than retained-tier sister predictions (v_EW, m_t, m_tau)")
    print("            which achieve 0.03% to 0.07%.")
    print()
    print("  Closure:  No simple retained-content Wilson-chain expression with")
