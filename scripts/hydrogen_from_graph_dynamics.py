#!/usr/bin/env python3
"""Hydrogen atom from Cl(3)/Z³ axioms.
...
is NOT the Schrödinger equation. It is the spectral problem for the
graph operator H_g acting on the lattice Hilbert space l²(Z³).
We solve it by exact diagonalization on a finite grid.

--------------------------------------------------------------------------
STEP 4: COUPLING CONSTANT  [status: BOUNDED in the framework]
--------------------------------------------------------------------------
The coupling g encodes Z × e² in the framework's natural units:

    g = Z × 4π × α_EM

The framework's EW running chain (EW_COUPLING_DERIVATION_NOTE.md) gives:

    α_EM^{-1}(EW) = α_LM^{-1} + b₁/(2π) × ln(M_Pl/v)
                  = 11.03 + 25.08 = 36.11
    → α_EM(EW) ≈ 1/36.1  (vs. measured 1/127.9 at EW scale; 1-loop, ~27% off)

Running from M_Z to atomic scale gives α_EM ≈ 1/137 (open lane, not yet exact).

Consequence: ABSOLUTE energy scale (in eV) is bounded, not derived.
DIMENSIONLESS LEVEL RATIOS are coupling-independent — these are the
framework's clean structural predictions.
...
HISTORICAL COMPARISON (checkpoint, not foundation):
  ...
  Framework prediction:     E_n ∝ -1/n²   (structural, coupling-relative)
  Absolute scale requires α_EM × m_e — both bounded, not derived exactly.

BLOCKERS:
  1. α_EM exact derivation: open lane (bounded at ~27% at 1-loop)
  2. Electron mass m_e: open (mass hierarchy derivation needed)
  Without these, cannot convert lattice energy to eV; can predict structure.
==========================================================================
"""
...
def run_experiment() -> None:
    log("=" * 72)
    log("HYDROGEN ATOM FROM Cl(3)/Z³ AXIOMS")
    log("No SE assumed — derived from graph Laplacian + lattice Green's function")
    log("=" * 72)
    log()
    ...
    log("  Spectral problem: H_g ψ = E ψ               [not SE; lattice spectral theory]")
    log("  Coupling g = Z × 4π × α_EM                  [BOUNDED: EW running ~27% off]")
    log()
    ...
    log("  These are pure Z³-geometry predictions, not QM imports.")
    log(f"  Time: {time.time()-t0:.1f}s")
    log()
    ...
    # ------------------------------------------------------------------
    log("=" * 72)
    log("SUMMARY: Cl(3)/Z³ HYDROGEN PREDICTIONS")
    log("=" * 72)
    log()
    ...
    log()
    log("  BLOCKED (requires further framework derivation):")
    log("  - Absolute energy in eV: needs α_EM × m_e  (both bounded)")
    log("  - α_EM exact: open lane  (1-loop gives ~27% error at EW scale)")
    log("  - Electron mass m_e: open  (mass hierarchy not yet derived)")
    log()
    log("  HISTORICAL CHECKPOINT:")
    log("  The 1/n² structure matches the hydrogen Rydberg series exactly.")
    log("  This is a pure Z³-geometry prediction, not a QM import.")
    log("=" * 72)

    os.makedirs("logs", exist_ok=True)
    log_path = f"logs/{time.strftime('%Y-%m-%d')}-hydrogen_from_graph_dynamics.txt"
    try:
