...
- allow only intrinsic branch-scale local/nonlocal perturbations
...
def main() -> int:
    ...
    print("the intrinsic branch-scale local/nonlocal budgets, remains in the same")
    ...
    print(f"  conservative branch budget   = {CONSERVATIVE_REL_BUDGET:.6e}")
    print()
    ...
    record(
        "the entire admissible class stays inside the conservative branch response budget",
        max_resp_rel < CONSERVATIVE_REL_BUDGET,
        f"max response-vs-ref L2 = {max_resp_rel:.6e}, budget = {CONSERVATIVE_REL_BUDGET:.6e}",
    )
    record(
        "the affine slope is unique at branch scale across the admissible class",
        max_slope_rel < 0.15,
        f"max slope relative drift = {max_slope_rel:.6e}",
        status="BOUNDED",
    )
    record(
        "the affine intercept is unique at branch scale across the admissible class",
    ...
    print("-" * 78)
    print(
        "Within the intrinsic branch-scale local/nonlocal remainder budgets, the "
